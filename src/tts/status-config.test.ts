...
beforeAll(() => {
  fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mo-tts-status-"));
});
  ...
  await withEnvAsync(
    {
      ...
      USERPROFILE: home,
      MO_HOME: undefined,
      MO_STATE_DIR: path.join(home, ".mo"),
    },
...
describe("resolveStatusTtsSnapshot", () => {
  it("uses prefs overrides without loading speech providers", async () => {
    await withStatusTempHome(async (home) => {
      const prefsPath = path.join(home, ".mo", "settings", "tts.json");
  ...
  it("derives the default prefs path from MO_CONFIG_PATH when set", async () => {
    await withStatusTempHome(async (home) => {
      const stateDir = path.join(home, ".mo-dev");
