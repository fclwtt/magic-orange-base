...
describe("OpenClawSchema marketplaces config", () => {
  it("accepts hosted feed and local source profiles", () => {
    ...
    expect(marketplaces?.feeds?.acme.url).toBe("https://packages.acme.example/mo/feed");
    expect(marketplaces?.sources?.["acme-git"].type).toBe("git");
  });

  it.each([
    "http://packages.acme.example/mo/feed",
    "https://token@packages.acme.example/mo/feed",
    "https://user:pass@packages.acme.example/mo/feed",
    "https://packages.acme.example/mo/feed?token=secret",
    "https://packages.acme.example/mo/feed#access-token",
    "not a url",
