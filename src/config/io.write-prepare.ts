...
import { normalizeConfiguredProviderCatalogModelId } from "@mo/model-catalog-core/provider-model-id-normalization";
...
export function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  mo config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  mo config set ${policyPath} "pairing"`,
  ].join("\n");
}
