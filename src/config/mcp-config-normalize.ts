...
const CLI_MCP_TYPE_TO_MO_TRANSPORT: Record<string, OpenClawMcpHttpTransport | "stdio"> = {
...
export function resolveOpenClawMcpTransportAlias(
  ...
  const mapped = CLI_MCP_TYPE_TO_MO_TRANSPORT[normalizeMcpString(value)];
...
export function isKnownCliMcpTypeAlias(value: unknown): boolean {
  return Object.hasOwn(CLI_MCP_TYPE_TO_MO_TRANSPORT, normalizeMcpString(value));
}
