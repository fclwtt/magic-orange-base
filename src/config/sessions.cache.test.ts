...
describe("Session Store Cache", () => {
  ...
  afterEach(() => {
    clearSessionStoreCacheForTest();
    delete process.env.MO_SESSION_CACHE_TTL_MS;
    delete process.env.MO_SESSION_SERIALIZED_CACHE_MAX_BYTES;
  });

  it("bounds the serialized session store cache by total bytes", () => {
    process.env.MO_SESSION_SERIALIZED_CACHE_MAX_BYTES = "64";
    clearSessionStoreCacheForTest();
  ...
  it("should respect MO_SESSION_CACHE_TTL_MS=0 to disable cache", async () => {
