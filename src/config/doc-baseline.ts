...
import { sortUniqueStrings } from "@mo/normalization-core/string-normalization";
...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.MO_CONFIG_DOC_BASELINE_DEBUG === "1") {
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    MO_STATE_DIR: path.join(os.tmpdir(), "mo-config-doc-baseline-state"),
