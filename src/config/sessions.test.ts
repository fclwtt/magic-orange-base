...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mo-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ MO_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    ...
    ).toBe("discord:friends-of-mo#general");
  });
  ...
  it("derives session transcripts dir from MO_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { MO_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when MO_STATE_DIR differs from stored paths", () => {
