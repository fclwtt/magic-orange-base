...
const ETC_MO_DIR = path.join(ROOT_DIR, "etc", "mo");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "mo.json");
...
function etcOpenClawPath(...parts: string[]) {
  return path.join(ETC_MO_DIR, ...parts);
}
...
describe("resolveConfigIncludeWritePath", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "mo-include-write-path-" }, async (tempRoot) => {
        ...
        expect(
          resolveConfigIncludeWritePath({
            configPath: path.join(configDir, "mo.json"),
...
describe("security: path traversal protection (CWE-22)", () => {
  ...
  describe("edge cases", () => {
    ...
    it("allows include files when the config root path is a symlink", async () => {
      await withTempDir({ prefix: "mo-includes-symlink-" }, async (tempRoot) => {
    ...
    it("rejects include files that are hardlinked aliases", async () => {
      ...
      await withTempDir({ prefix: "mo-includes-hardlink-" }, async (tempRoot) => {
        const configDir = path.join(tempRoot, "config");
    ...
    it("rejects oversized include files", async () => {
      await withTempDir({ prefix: "mo-includes-big-" }, async (tempRoot) => {
        const configDir = path.join(tempRoot, "config");
        await fs.mkdir(configDir, { recursive: true });
        ...
        expect(() =>
          resolveConfigIncludes({ $include: "./big.json5" }, path.join(configDir, "mo.json")),
...
describe("MO_INCLUDE_ROOTS allowlist", () => {
  ...
  it("resolves a symlinked include whose realpath lands inside an allowed root", async () => {
    await withTempDir({ prefix: "mo-includes-allowed-symlink-" }, async (tempRoot) => {
      const configDir = path.join(tempRoot, "config");
  ...
  it("rejects a symlinked include that escapes both the config directory and every allowed root", async () => {
    await withTempDir({ prefix: "mo-includes-allowed-escape-" }, async (tempRoot) => {
      const configDir = path.join(tempRoot, "config");
      const allowedDir = path.join(tempRoot, "allowed");
