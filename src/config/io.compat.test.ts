...
describe("config io paths", () => {
  ...
  it("uses ~/.mo/mo.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".mo", 19001);
      const io = createIoForHome(home);
      expect(io.configPath).toBe(configPath);
    });
  });

  it("defaults to ~/.mo/mo.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".mo", "mo.json"));
    });
  });

  it("uses MO_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { MO_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".mo", "mo.json"));
    });
  });

  it("honors explicit MO_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".mo", 20002, "custom.json");
      const io = createIoForHome(home, { MO_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("does not warn about newer config during internal update handoff reads", async () => {
    await withTempHome(async (home) => {
      ...
      const io = createConfigIO({
        configPath,
        env: { HOME: home, MO_UPDATE_POST_CORE: "1" } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger,
      });
      io.loadConfig();
