...
import { collectManifestModelIdNormalizationPolicies } from "@mo/model-catalog-core/provider-model-id-normalization";
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    ...
    logger.warn(
      [
        ...
        "Check: `mo --version`, `which mo`, and `mo gateway status --deep`.",
        "If unexpected, update PATH so `mo` points to the version you want, or reinstall the Gateway service from that same OpenClaw install.",
      ].join("\n"),
    );
  }
}
...
function shouldSuppressFutureVersionWarningForEnv(env: NodeJS.ProcessEnv): boolean {
  return (
    isTruthyEnvValue(env.MO_UPDATE_IN_PROGRESS) ||
    isTruthyEnvValue(env.MO_UPDATE_POST_CORE)
  );
}
...
export function createConfigIO(
  ...
  function ensureShippedPluginInstallConfigRecordsMigratedForWrite(
    ...
    try {
      ...
      throw new Error(
        `Config write blocked: shipped plugins.installs records in ${configPath} could not be migrated into the plugin index. Fix state directory permissions or run mo plugins registry --refresh, then retry. ${formatErrorMessage(
          err,
        )}`,
        { cause: err },
      );
  ...
  async function writeConfigFileLocal(
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.MO_TEST_CONFIG_OVERWRITE_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
      if (!isVerbose() && deps.env.MO_CONFIG_OVERWRITE_LOG !== "1" && !shouldLogInVitest) {
        return;
      }
      deps.logger.warn(
        formatConfigOverwriteLogMessage({
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.MO_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
      const shouldLogBenignMissingMeta =
        isVerbose() || deps.env.MO_CONFIG_WRITE_ANOMALY_LOG === "1" || shouldLogInVitest;
...
// module scope. `MO_CONFIG_PATH` (and friends) are expected to work even
...
export function loadConfig(options?: {
  ...
  // reparsing mo.json on hot code paths.
