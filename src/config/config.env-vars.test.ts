...
describe("config env vars", () => {
  ...
  it("preserves Windows case-insensitive env precedence in merged runtime env", () => {
    ...
    try {
      const merged = createConfigRuntimeEnv(
        { env: { vars: { MO_LOAD_SHELL_ENV: "1" } } } as OpenClawConfig,
        { OpenClaw_Load_Shell_Env: "0" },
      );

      expect(merged.MO_LOAD_SHELL_ENV).toBe("0");
  ...
  it("blocks dangerous startup env vars from config env", async () => {
    await withEnvOverride(
      ...
      async () => {
        const config = {
          env: {
            vars: {
              ...
              MO_INCLUDE_ROOTS: "/tmp/evil-include-root",
              mo_allow_older_binary_destructive_actions: "1",
        ...
        expect(entries.MO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS).toBeUndefined();
        expect(entries.MO_INCLUDE_ROOTS).toBeUndefined();
        expect(entries.mo_allow_older_binary_destructive_actions).toBeUndefined();
  ...
  it("loads ${VAR} substitutions from ~/.mo/.env on repeated runtime loads", async () => {
    await withTempHome(async (_home) => {
      await withEnvOverride({ BRAVE_API_KEY: undefined }, async () => {
        const stateDir = process.env.MO_STATE_DIR?.trim();
        if (!stateDir) {
          throw new Error("Expected MO_STATE_DIR to be set by withTempHome");
        }
  ...
  it("drops dangerous and empty values from the state-dir .env file", async () => {
    await withTempHome(async (_home) => {
      ...
      expect(vars.MO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS).toBeUndefined();
      expect(vars.EMPTY).toBeUndefined();
      expect(vars.VALID).toBe("ok");
    });
  });

  it("respects MO_STATE_DIR when reading state-dir .env vars", async () => {
    await withTempHome(async (_home) => {
      const customStateDir = path.join(process.env.MO_STATE_DIR ?? "", "custom-state");
      await writeStateDirDotEnv("CUSTOM_KEY=from-override\n", {
        stateDir: customStateDir,
      });
      expect(
        readStateDirDotEnvVars({
          MO_STATE_DIR: customStateDir,
        }).CUSTOM_KEY,
      ).toBe("from-override");
    });
  });
