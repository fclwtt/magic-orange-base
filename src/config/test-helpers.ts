...
import { withTempHome as withTempHomeBase } from "mo/plugin-sdk/test-env";
  ...
  try {
    return await withTempHomeBase(fn, {
      ...
      env: {
        ...
        MO_MPM_CATALOG_PATHS: undefined,
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.MO_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected MO_STATE_DIR or explicit stateDir for .env test setup");
  }
