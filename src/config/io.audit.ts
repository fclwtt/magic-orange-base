...
export function createConfigWriteAuditRecordBase(params: {
  ...
  return {
    ...
    watchMode: params.env.MO_WATCH_MODE === "1",
    watchSession: normalizeAuditLabel(params.env.MO_WATCH_SESSION),
    watchCommand: normalizeAuditLabel(params.env.MO_WATCH_COMMAND),
...
export type ConfigAuditScrubResult = {
  ...
  // to swap the file. Caller should re-run `mo doctor --fix` once the
