...
describe("appendAssistantMessageToSessionTranscript", () => {
  ...
  it("uses configured session.store when storePath is omitted", async () => {
    ...
    try {
      setTestEnvValue("MO_STATE_DIR", path.join(tempDir, "default-state"));
      ...
      if (previousStateDir === undefined) {
        ...
        setTestEnvValue("MO_STATE_DIR", previousStateDir);
      }
      fs.rmSync(tempDir, { recursive: true, force: true });
    }
  });
  ...
  it("emits transcript update events for delivery mirrors", async () => {
    ...
    expect(message?.provider).toBe("mo");
  ...
  it("idempotently appends identified channel finals while preserving repeated replies", async () => {
    ...
    if (first.ok && replay.ok && nextTurn.ok) {
      ...
      expect(JSON.parse(lines[1]).message.moDeliveryMirror).toEqual({
        kind: "channel-final",
  ...
  it("scans past trailing non-assistant entries (e.g. mo.cache-ttl) to find the latest assistant text", async () => {
    // Regression for mo/mo#83427: the cache-ttl custom entry was
  ...
  it("does not reuse an older matching assistant message across turns", async () => {
    ...
    if (olderResult.ok && latestResult.ok && mirrorResult.ok) {
      ...
      expect(messageLine.message.provider).toBe("mo");
