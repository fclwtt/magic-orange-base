...
import type { DB as OpenClawStateKyselyDatabase } from "../state/mo-state-db.generated.js";
import {
...
import type { ConfigFileSnapshot, OpenClawConfig } from "./types.mo.js";
...
describe("config io write", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "mo-config-io-" });
  const silentLogger = {
    warn: () => {},
    error: () => {},
  };
  ...
  const createFastConfigIO = (home: string) =>
    createConfigIO({
      env: { MO_TEST_FAST: "1" } as NodeJS.ProcessEnv,
      homedir: () => home,
      logger: silentLogger,
    });
  ...
  it("writes health state to SQLite through public config reads", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".mo", "mo.json");
      const healthPath = path.join(home, ".mo", "logs", "config-health.json");
  ...
  it("loads shipped plugin install config records without mutating config or plugin index", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".mo", "mo.json");
      const pluginDir = path.join(home, ".mo", "plugins", "demo");
      const manifestPath = path.join(pluginDir, "mo.plugin.json");
  ...
  it("retains included shipped plugin install records in write snapshots", async () => {
    await withSuiteHome(async (home) => {
      const configDir = path.join(home, ".mo");
  ...
  it("migrates shipped plugin install config records during explicit writes even when the manifest is missing", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".mo", "mo.json");
      const pluginDir = path.join(home, ".mo", "plugins", "missing");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        configPath,
        `${JSON.stringify(
  ...
  it("keeps shipped plugin install config records when index migration fails", async () => {
    ...
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".mo", "mo.json");
      const unwritableStatePath = path.join(home, ".mo");
  ...
  it("keeps writes inside an MO_STATE_DIR override even when the real home config exists", async () => {
    await withSuiteHome(async (home) => {
      const liveConfigPath = path.join(home, ".mo", "mo.json");
      ...
      const env = { MO_STATE_DIR: overrideDir } as NodeJS.ProcessEnv;
      const io = createConfigIO({
        env,
        homedir: () => home,
        logger: silentLogger,
      ...
      expect(io.configPath).toBe(path.join(overrideDir, "mo.json"));
      ...
      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "mo.json"), "utf-8"),
      ) as {
  ...
  it("suppresses overwrite audit output when skipOutputLogs is set", async () => {
    await withSuiteHome(async (home) => {
      ...
      const io = createConfigIO({
        env: {
          VITEST: "true",
          MO_TEST_CONFIG_OVERWRITE_LOG: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: {
          warn,
  ...
  it("preserves root $schema during partial writes", async () => {
    await withSuiteHome(async (home) => {
      ...
      expect(persisted.$schema).toBe("https://mo.ai/config.json");
  ...
  it("rejects destructive internal writes before replacing the config", async () => {
    await withSuiteHome(async (home) => {
      ...
      const original = {
        ...
        agents: { list: [{ id: "main", default: true, workspace: "/tmp/mo-main" }] },
  ...
  it("returns the read-time environment snapshot for invalid config repairs", async () => {
    await withSuiteHome(async (home) => {
      ...
      const io = createConfigIO({
        env: {
          MO_GATEWAY_TOKEN: "gateway-token-at-read",
          MO_TEST_FAST: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
      ...
      expect(result.writeOptions.envSnapshotForRestore?.MO_GATEWAY_TOKEN).toBe(
  ...
  it("returns the snapshot-time hash when an included file is malformed", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".mo", "mo.json");
      const includePath = path.join(home, ".mo", "plugins.json5");
  ...
  it("returns a write guard that rejects a changed active config path", async () => {
    await withSuiteHome(async (home) => {
      const firstConfigPath = path.join(home, ".mo", "first.json");
      const secondConfigPath = path.join(home, ".mo", "second.json");
      ...
      const env = {
        MO_CONFIG_PATH: firstConfigPath,
        MO_TEST_FAST: "1",
      } as NodeJS.ProcessEnv;
      const io = createConfigIO({ env, homedir: () => home, logger: silentLogger });

      const result = await io.readConfigFileSnapshotForWrite();
      env.MO_CONFIG_PATH = secondConfigPath;
  ...
  it("rejects local write ownership when config env changes path selection during the read", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".mo", "mo.json");
      const configuredNextPath = path.join(home, ".mo", "next.json");
      const sourceConfig = {
        env: { MO_CONFIG_PATH: configuredNextPath },
        gateway: { mode: "local" },
  ...
  it("does not use expectedConfigPath as the write destination", async () => {
    await withSuiteHome(async (home) => {
      const expectedConfigPath = path.join(home, ".mo", "expected.json");
      const activeConfigPath = path.join(home, ".mo", "active.json");
      ...
      await withEnvAsync(
        {
          MO_CONFIG_PATH: activeConfigPath,
          MO_TEST_FAST: "1",
        },
        async () => {
          await writeConfigFile(
  ...
  it("rejects root-include partial writes instead of flattening the root config", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".mo", "mo.json");
      const includePath = path.join(home, ".mo", "extra.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify({ $schema: "https://mo.ai/config-from-include.json" }, null, 2)}\n`,
        "utf-8",
      );
      await fs.writeFile(
        configPath,
  ...
  it("rejects invalid include-backed repairs instead of persisting substituted secrets", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".mo", "mo.json");
      const includePath = path.join(home, ".mo", "gateway.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify(
      ...
      const io = createConfigIO({
        env: {
          MO_GATEWAY_TOKEN: "gateway-token-runtime",
          MO_TEST_FAST: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
  ...
  it("repairs invalid root-authored siblings without flattening included config", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".mo", "mo.json");
      const includePath = path.join(home, ".mo", "agent-defaults.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
  ...
  it("rejects repairs that would flatten a valid outer include with a broken nested include", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".mo", "mo.json");
      const pluginsPath = path.join(home, ".mo", "plugins.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        pluginsPath,
  ...
  it("preserves escaped root literals before validating unrelated includes", async () => {
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/mo-test-literal-plugin",
          source: "/tmp/mo-test-literal-plugin/index.ts",
          manifestPath: "/tmp/mo-test-literal-plugin/mo.plugin.json",
          configSchema: {
            type: "object",
            properties: {
    ...
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".mo", "mo.json");
      const agentsPath = path.join(home, ".mo", "agents.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        agentsPath,
  ...
  it("repairs invalid config without flattening array-nested includes", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".mo", "mo.json");
      const includePath = path.join(home, ".mo", "main-agent.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify({ id: "main", workspace: "${MO_AGENT_WORKSPACE}" }, null, 2)}\n`,
        "utf-8",
      );
      await fs.writeFile(
        configPath,
      ...
      const io = createConfigIO({
        env: {
          MO_AGENT_WORKSPACE: "/resolved/agent-workspace",
          MO_TEST_FAST: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/mo-test-required-plugin",
          source: "/tmp/mo-test-required-plugin/index.ts",
          manifestPath: "/tmp/mo-test-required-plugin/mo.plugin.json",
          configSchema: {
            type: "object",
            properties: {
              token: { type: "string" },
  ...
  it("writes runtime-derived edits back to source SecretRef markers", async () => {
    await withSuiteHome(async (home) => {
      ...
      await withEnvAsync({ MO_CONFIG_PATH: configPath }, async () => {
        setRuntimeConfigSnapshot(
          {
            gateway: { mode: "local" },
            models: {
      ...
      try {
        await withEnvAsync(
          ...
          async () => {
            ...
            expect(persisted.gateway?.auth?.token).toBe("${MO_GATEWAY_TOKEN}");
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/mo-test-demo",
          source: "/tmp/mo-test-demo/index.ts",
          manifestPath: "/tmp/mo-test-demo/mo.plugin.json",
          configSchema: {
            type: "object",
            properties: {
      ...
      try {
        await withEnvAsync({ MO_CONFIG_PATH: configPath }, async () => {
          ...
          const postWriteSnapshot = await createConfigIO({
            env: { MO_CONFIG_PATH: configPath, VITEST: "true" } as NodeJS.ProcessEnv,
            homedir: () => home,
            logger: silentLogger,
  ...
  it("keeps plugin install index migration when runtime refresh fails", async () => {
    await withSuiteHome(async (home) => {
      const stateDir = path.join(home, ".mo");
      const configPath = path.join(stateDir, "mo.json");
  ...
  it("restores config env vars when post-write runtime refresh rollback succeeds", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".mo", "mo.json");
      const envKey = "MO_TEST_RUNTIME_ROLLBACK_ENV";
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies OpenClawConfig;
      const initialRaw = `${JSON.stringify(initialConfig, null, 2)}\n`;
  ...
  it("rolls back root writes when canonical reread changes config path ownership", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".mo", "mo.json");
      const nextConfigPath = path.join(home, ".mo", "next.json");
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies OpenClawConfig;
      const initialRaw = `${JSON.stringify(initialConfig, null, 2)}\n`;
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(configPath, initialRaw, "utf-8");
        ...
        async () => {
          ...
          await expect(
            writeConfigFile(
              {
                gateway: { mode: "local", port: 19001 },
                env: { MO_CONFIG_PATH: nextConfigPath },
              },
              {
                baseSnapshot: prepared.snapshot,
                ...prepared.writeOptions,
  ...
  it("uses injected filesystem operations when rolling back ownership loss", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".mo", "mo.json");
      const otherConfigPath = path.join(home, ".mo", "other.json");
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies OpenClawConfig;
      const initialRaw = `${JSON.stringify(initialConfig, null, 2)}\n`;
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(configPath, initialRaw, "utf-8");
        ...
        promises: {
          ...
          rename: async (from, to) => {
            ...
            if (!committed && to === configPath) {
              committed = true;
              env.MO_CONFIG_PATH = otherConfigPath;
            }
          },
        },
      } as typeof fsNode;
        ...
        plugins: [
          {
            ...
            skills: [],
            hooks: [],
            rootDir: "/tmp/mo-test-strict-plugin",
            source: "/tmp/mo-test-strict-plugin/index.ts",
            manifestPath: "/tmp/mo-test-strict-plugin/mo.plugin.json",
            configSchema: {
              type: "object",
  ...
  it("preserves authored tilde paths when runtime-shaped writes hand back absolute paths", async () => {
    await withSuiteHome(async (home) => {
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify(
          {
            logging: { file: "~/mo-upgrade-survivor/gateway.jsonl" },
          },
          null,
          2,
        )}\n`,
      ...
      await io.writeConfigFile(
        {
          logging: {
            file: path.join(home, "mo-upgrade-survivor", "gateway.jsonl"),
            level: "debug",
          },
        },
        { baseSnapshot: snapshot },
      ...
      const persisted = JSON.parse(await fs.readFile(configPath, "utf-8")) as OpenClawConfig;
      expect(persisted.logging?.file).toBe("~/mo-upgrade-survivor/gateway.jsonl");
