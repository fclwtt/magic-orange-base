...
describe("redactConfigSnapshot", () => {
          ...
          plugins: [
            {
              ...
              manifestPath: "/private/plugin/root/mo.plugin.json",
            },
          ],
          diagnostics: [],
        },
  ...
  security: {
    installPolicy: {
      ...
      exec: {
        source: "exec",
        command: "/usr/local/bin/mo-install-policy",
        env: {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/mo/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/mo/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/mo/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/mo/nickserv-password.txt");
