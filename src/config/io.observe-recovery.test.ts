...
describe("config observe recovery", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "mo-config-observe-recovery-"));
  });
  ...
  function makeDeps(
    ...
    return {
      ...
      configPath,
      auditPath: path.join(home, ".mo", "logs", "config-audit.jsonl"),
      warn,
    };
  }
  ...
  it("loadConfig clears env vars from the discarded clobbered config before rereading backup", async () => {
    await withSuiteHome(async (home) => {
      ...
      await writeConfigRaw(configPath, {
        meta: { lastTouchedVersion: "2026.5.28" },
        env: { vars: { MO_CLOBBER_ONLY: "bad" } },
      });

      const config = io.loadConfig();

      expect(config.gateway?.mode).toBe("local");
      expect(env.MO_CLOBBER_ONLY).toBeUndefined();
    });
  });
  ...
  it("validates backup candidates without leaking their env into live state", async () => {
    await withSuiteHome(async (home) => {
      ...
      expect(env.MO_BACKUP_ONLY).toBeUndefined();
    });
  });
  ...
  it("logs async health-state write failures", async () => {
    await withSuiteHome(async (home) => {
      ...
      await expectPathMissing(path.join(home, ".mo", "logs", "config-health.json"));
