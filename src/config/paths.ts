...
/**
 * Nix mode detection: When MO_NIX_MODE=1, the gateway is running under Nix.
 * In this mode:
...
const NEW_STATE_DIRNAME = ".mo";
const CONFIG_FILENAME = "mo.json";
...
/** Build a homedir thunk that respects MO_HOME for the given env. */
 ...
 * Can be overridden via MO_STATE_DIR.
 * Default: ~/.mo
 */
export function resolveStateDir(
  ...
  const override = env.MO_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, effectiveHomedir);
  }
  const newDir = newStateDir(effectiveHomedir);
  if (env.MO_TEST_FAST === "1") {
    return newDir;
  }
...
export function normalizeStateDirEnv(env: NodeJS.ProcessEnv = process.env): void {
  ...
  const moOverride = env.MO_STATE_DIR?.trim();
  if (moOverride) {
    env.MO_STATE_DIR = resolveUserPath(moOverride, env, effectiveHomedir);
  }
}
 ...
 * outside the config directory. Set via `MO_INCLUDE_ROOTS` as a
 ...
 * directory containing `mo.json`.
 */
export function resolveIncludeRoots(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = envHomedir(env),
): string[] {
  const raw = env.MO_INCLUDE_ROOTS?.trim();
  if (!raw) {
    return [];
  }
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
 ...
 * Can be overridden via MO_CONFIG_PATH.
 * Default: ~/.mo/mo.json (or $MO_STATE_DIR/mo.json)
 */
export function resolveCanonicalConfigPath(
  env: NodeJS.ProcessEnv = process.env,
  stateDir: string = resolveStateDir(env, envHomedir(env)),
): string {
  const override = env.MO_CONFIG_PATH?.trim();
  if (override) {
...
export function resolveConfigPath(
  ...
  const stateOverride = env.MO_STATE_DIR?.trim();
  const candidates = [
...
export function resolveDefaultConfigCandidates(
  ...
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
  const explicit = env.MO_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveUserPath(explicit, env, effectiveHomedir)];
  }

  const candidates: string[] = [];
  const moStateDir = env.MO_STATE_DIR?.trim();
  if (moStateDir) {
    const resolved = resolveUserPath(moStateDir, env, effectiveHomedir);
 ...
 * Default: os.tmpdir()/mo-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `mo-${uid}` : "mo";
 ...
 *
 * Precedence:
 * - `MO_OAUTH_DIR` (explicit override)
...
export function resolveOAuthDir(
  ...
  const override = env.MO_OAUTH_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, envHomedir(env));
  }
...
export function resolveGatewayPort(
  ...
  const envRaw = env.MO_GATEWAY_PORT?.trim();
