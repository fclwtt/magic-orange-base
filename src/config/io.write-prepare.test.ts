...
describe("config io write prepare", () => {
        ...
        sourceConfig: {
          plugins: {
            ...
            installs: {
              "mo-web-search": {
                source: "npm",
                spec: "@ollama/mo-web-search",
                installPath: "/tmp/mo-web-search",
                resolvedName: "@ollama/mo-web-search",
        ...
        nextConfig: {
          plugins: {
            ...
            installs: {
              "mo-web-search": {
                source: "npm",
                spec: "@ollama/mo-web-search@0.2.2",
                installPath: "/tmp/mo-web-search",
                resolvedName: "@ollama/mo-web-search",
                resolvedVersion: "0.2.2",
              },
  ...
  it('formats actionable guidance for dmPolicy="open" without wildcard allowFrom', () => {
    ...
    expect(message).toContain("mo config set channels.telegram.allowFrom '[\"*\"]'");
    expect(message).toContain('mo config set channels.telegram.dmPolicy "pairing"');
  });
  ...
  it("rejects writes that would flatten a root include", () => {
    const sourceConfig = {
      $schema: "https://mo.ai/config-from-include.json",
      gateway: { mode: "local" },
    };
