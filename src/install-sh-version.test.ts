...
function resolveInstallerVersionCases(params: { stdinCwd: string }): string[] {
  const output = execFileSync(
    ...
    [
      "-c",
      `${versionHelperSource}
fake_mo_decorated() { printf '%s\\n' 'OpenClaw 2026.3.10 (abcdef0)'; }
fake_mo_raw() { printf '%s\\n' "OpenClaw dev's build"; }
MO_BIN=fake_mo_decorated resolve_mo_version
MO_BIN=fake_mo_raw resolve_mo_version
(
  cd "$1"
  source /dev/stdin <<'MO_STDIN_INSTALLER'
${versionHelperSource}
fake_mo_stdin() { printf '%s\\n' 'OpenClaw 2026.3.10 (abcdef0)'; }
MO_BIN=fake_mo_stdin
resolve_mo_version
MO_STDIN_INSTALLER
)`,
      "mo-version-test",
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const hostileCwd = makeTempDir(tempRoots, "mo-install-stdin-");
