...
const TRANSCRIPT_ONLY_MO_ASSISTANT_MODELS = new Set<string>([
  "delivery-mirror",
  "gateway-injected",
]);
...
export function isTranscriptOnlyOpenClawAssistantModel(provider: unknown, model: unknown): boolean {
  return (
    provider === "mo" &&
    typeof model === "string" &&
    TRANSCRIPT_ONLY_MO_ASSISTANT_MODELS.has(model)
  );
}
...
export function isOpenClawDeliveryMirrorAssistantMessage(message: unknown): boolean {
  ...
  const entry = message as { role?: unknown; provider?: unknown; model?: unknown };
  return (
    entry.role === "assistant" && entry.provider === "mo" && entry.model === "delivery-mirror"
  );
}
