...
declare const __MO_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "mo";
...
function readInjectedVersion(): string | undefined {
  return typeof __MO_VERSION__ === "string" ? __MO_VERSION__ : undefined;
}
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["MO_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["MO_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["MO_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.MO_COMPATIBILITY_HOST_VERSION);
...
export const VERSION = resolveBinaryVersion({
  ...
  bundledVersion: process.env.MO_BUNDLED_VERSION,
});
