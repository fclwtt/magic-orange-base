...
type SecretRegistryEntry = {
  id: string;
  configFile: "mo.json" | "auth-profiles.json";
  pathPattern: string;
...
const DEBUG_COVERAGE_BATCHES = process.env.MO_DEBUG_RUNTIME_COVERAGE === "1";
...
const PLUGIN_OWNED_MO_COVERAGE_EXCLUSIONS = new Set([
...
const previousTrustBundledPluginsDir = process.env.MO_TEST_TRUST_BUNDLED_PLUGINS_DIR;

process.env.MO_BUNDLED_PLUGINS_DIR ??= "extensions";
process.env.MO_TEST_TRUST_BUNDLED_PLUGINS_DIR ??= "1";
...
afterAll(() => {
  ...
  if (previousTrustBundledPluginsDir === undefined) {
    ...
    process.env.MO_TEST_TRUST_BUNDLED_PLUGINS_DIR = previousTrustBundledPluginsDir;
  }
});
...
function collectOpenClawCoverageEntries(options: {
  ...
  return COVERAGE_REGISTRY_ENTRIES.filter(
    (entry) =>
      entry.configFile === "mo.json" &&
      entry.id.startsWith("plugins.entries.") === options.includePluginEntries &&
      !PLUGIN_OWNED_MO_COVERAGE_EXCLUSIONS.has(entry.id),
  );
}
...
async function expectOpenClawCoverageBatchResolved(
  ...
  for (const [index, entry] of batch.entries()) {
    const envId = toCoverageEnvRefId("MO_SECRET_TARGET", entry.id);
...
const MO_CORE_COVERAGE_BATCHES = buildCoverageBatches(
  collectOpenClawCoverageEntries({ includePluginEntries: false }),
);
const MO_PLUGIN_COVERAGE_BATCHES = buildCoverageBatches(
...
describe("secrets runtime target coverage", () => {
  beforeAll(async () => {
    ...
    const googleChatBatch = MO_CORE_COVERAGE_BATCHES.find((batch) =>
      batch.some((entry) => entry.id === "channels.googlechat.serviceAccount"),
    );
    if (googleChatBatch) {
      await expectOpenClawCoverageBatchResolved("mo.json core", googleChatBatch);
    }
  });

  describe("mo.json core and channel registry targets", () => {
    test.each(MO_CORE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectOpenClawCoverageBatchResolved("mo.json core", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });

  describe("mo.json plugin registry targets", () => {
    test.each(MO_PLUGIN_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectOpenClawCoverageBatchResolved("mo.json plugins", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });
  ...
  describe("auth-profiles registry targets", () => {
    test.each(AUTH_PROFILE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      ...
      async ({ batch }) => {
        ...
        for (const [index, entry] of batch.entries()) {
          const envId = toCoverageEnvRefId("MO_AUTH_SECRET_TARGET", entry.id);
