...
async function writeJsonFile(filePath: string, value: unknown): Promise<void> {
  if (path.basename(filePath) === "mo-agent.sqlite") {
...
async function createAuditFixture(): Promise<AuditFixture> {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-secrets-audit-"));
  const stateDir = path.join(rootDir, ".mo");
...
describe("secrets audit", () => {
  ...
  it("does not flag non-sensitive routing headers in mo config", async () => {
    await writeJsonFile(fixture.configPath, {
      models: {
        providers: {
          openai: {
  ...
  it("keeps request headers in mo config covered by plaintext audit", async () => {
    await writeJsonFile(fixture.configPath, {
      models: {
        providers: {
          openai: {
  ...
  it("does not flag mo.json model provider apiKey marker values as plaintext", async () => {
    await writeJsonFile(fixture.configPath, {
      models: {
        providers: {
          lmstudio: {
