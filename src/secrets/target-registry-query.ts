...
function getCompiledSecretTargetRegistryState() {
  ...
  const openClawCompiledSecretTargets = compiledSecretTargetRegistry.filter(
    (entry) => entry.configFile === "mo.json",
  );
...
function getCompiledCoreOpenClawTargetState() {
  ...
  const openClawCompiledSecretTargets = getCoreSecretTargetRegistry()
    .filter((entry) => entry.configFile === "mo.json")
...
function getCompiledChannelOpenClawTargets(
  ...
  const compiledEntries =
    loadChannelSecretContractApi({
      ...
      ?.secretTargetRegistryEntries?.filter((entry) => entry.configFile === "mo.json")
...
/**
 * Resolves an mo.json config path to the matching plan-capable secrets target.
 */
...
/**
 * Discovers configured secret-bearing values in mo.json using the full registry.
 */
...
/**
 * Discovers configured mo.json targets, optionally limited to selected registry ids.
 */
