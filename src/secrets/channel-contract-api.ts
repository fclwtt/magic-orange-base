...
function resolvePluginContractApiPath(rootDir: string): string | null {
  ...
  // (per package.json `mo.runtimeExtensions`), while flat-layout plugins keep
  // them at <rootDir>/. Search both, preferring dist/ when running from built mo
...
function loadExternalChannelSecretContractFromRecord(
  ...
  try {
    ...
    if (process.env.MO_DEBUG_CHANNEL_CONTRACT_API === "1") {
