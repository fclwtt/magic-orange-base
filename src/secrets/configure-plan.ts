/** Builds the interactive `mo secrets configure` target list and apply plan. */
...
/** Credential target shown by `mo secrets configure` before a SecretRef is selected. */
...
export function buildConfigureCandidatesForScope(params: {
  ...
  const moCandidates = discoverConfigSecretTargets(params.config)
  ...
  return [...moCandidates, ...authCandidates].toSorted((a, b) =>
