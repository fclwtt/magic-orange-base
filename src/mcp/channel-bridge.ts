...
export class OpenClawChannelBridge {
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`mo mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
  ...
  private async sendNotification(notification: ServerNotification): Promise<void> {
    ...
    try {
      ...
      process.stderr.write(`mo mcp: notification ${notification.method} failed\n`);
      if (this.verbose) {
        process.stderr.write(
          `mo mcp: notification ${notification.method} error: ${String(error)}\n`,
        );
      }
    }
  }
