...
async function runSetupWizardOnce(
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("mo doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
    ...
    await prompter.note(
      [
        ...
        ...(compatibilityNotices.length > 4
          ...
          : []),
        "",
        `Review: ${formatCliCommand("mo doctor")}`,
        `Inspect: ${formatCliCommand("mo plugins inspect --all")}`,
      ].join("\n"),
  ...
  const quickstartHint = t("wizard.setup.flowQuickstartHint", {
    command: formatCliCommand("mo configure"),
  });
  ...
  if (
    ...
    runtime.error(
      "Invalid --flow. Use quickstart, manual, advanced, or import. Example: mo onboard --flow quickstart",
    );
    runtime.exit(1);
    return;
  }
  ...
  if (opts.importFrom || isSetupImportFlowChoice(flow)) {
    ...
    if (!migratedSnapshot.valid) {
      throw new Error("Migration produced an invalid OpenClaw config. Run `mo doctor`.");
    }
  ...
  let localGatewayToken = process.env.MO_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword = process.env.MO_GATEWAY_PASSWORD;
  try {
