...
export const zh_TW = {
  ...
  wizard: {
    ...
    remote: {
      ...
      insecureRemoteUrl:
        "遠端主機請使用 wss://，或透過 SSH 通道使用 ws://127.0.0.1/localhost。緊急繞過：可信私有網路可設定 MO_ALLOW_INSECURE_PRIVATE_WS=1。",
    ...
    skills: {
      ...
      docsLine: "文件：https://docs.mo.ai/skills",
      enterEnv: "輸入 {env}",
    ...
    irc: {
      ...
      nextStatusCommand: "命令：mo channels status --probe",
    ...
    search: {
      ...
      configured: "已設定",
      configureLaterHint: "稍後可用 mo configure --section web 設定",
      docsLine: "文件：{url}",
    ...
    finalize: {
      ...
      controlUiDocs: "文件：https://docs.mo.ai/web/control-ui",
      ...
      gatewayTokenStored:
        "儲存位置：$MO_CONFIG_PATH（預設 ~/.mo/mo.json）中的 gateway.auth.token，或 MO_GATEWAY_TOKEN。",
      ...
      securityReminder:
        "在你的電腦上執行 agent 存在風險，請加固設定：https://docs.mo.ai/security",
      ...
      webDocs: "文件：https://docs.mo.ai/tools/web",
      ...
      whatNow: '下一步：https://mo.ai/showcase（"What People Are Building"）。',
      whatNowTitle: "下一步",
      workspaceBackupTitle: "工作區備份",
      workspaceDocs: "文件：https://docs.mo.ai/concepts/agent-workspace",
