...
import { ensureOpenClawExecMarkerOnProcess } from "./infra/mo-exec-env.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "mo.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "mo.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  if (!waitingForCompileCacheRespawn) {
    ...
    if (shouldForceReadOnlyAuthStore(process.argv)) {
      process.env.MO_AUTH_STORE_READONLY = "1";
    }
    ...
    if (!ensureCliRespawnReady()) {
      ...
      if (!parsedContainer.ok) {
        console.error(`[mo] ${parsedContainer.error}`);
        process.exit(2);
      }
      ...
      if (!parsed.ok) {
        ...
        console.error(`[mo] ${parsed.error}`);
        process.exit(2);
      }

      const containerTargetName = resolveCliContainerTarget(process.argv);
      if (containerTargetName && parsed.profile) {
        console.error("[mo] --container cannot be combined with --profile/--dev");
        process.exit(2);
      }
