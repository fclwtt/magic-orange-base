...
describe("mcp loopback server", () => {
  it("passes session, account, message channel, and inbound event headers into shared tool resolution", async () => {
    ...
    const response = await sendRaw({
      ...
      headers: jsonHeaders({
        ...
        "x-mo-session-id": "session-123",
        "x-mo-account-id": "work",
        "x-mo-message-channel": "telegram",
        "x-mo-current-channel-id": "telegram:chat123",
        "x-mo-current-thread-ts": "42",
        "x-mo-current-message-id": "reply-message-1",
        "x-mo-current-inbound-audio": "true",
        "x-mo-inbound-event-kind": "room_event",
        "x-mo-source-reply-delivery-mode": "message_tool_only",
        "x-mo-require-explicit-message-target": "true",
      }),
      body: mcpToolsListBody(),
    });
  ...
  it("binds an attach grant's session and ignores ALL spoofed context headers (no scope-shop)", async () => {
    ...
    const response = await sendRaw({
      ...
      headers: jsonHeaders({
        ...
        "x-mo-message-channel": "telegram",
        "x-mo-account-id": "victim-account",
        "x-mo-current-channel-id": "telegram:victim-chat",
        "x-mo-current-thread-ts": "999",
        "x-mo-source-reply-delivery-mode": "automatic",
        "x-mo-inbound-event-kind": "room_event",
      }),
      body: mcpToolsListBody(),
    });
    ...
    const sendYield = async (
      ...
      return await sendLoopbackToolCall({
        ...
        headers: {
          ...
          "x-mo-session-id": "session-reused",
          "x-mo-cli-capture-key": captureKey,
        },
      });
    };
    ...
    const sendToolsList = async (
      ...
      await sendLoopbackToolsList({
        ...
        headers: {
          "x-session-key": "agent:main:telegram:group:chat123",
          "x-mo-message-channel": "telegram",
          "x-mo-inbound-event-kind": inboundEventKind,
          ...(sourceReplyDeliveryMode
            ? { "x-mo-source-reply-delivery-mode": sourceReplyDeliveryMode }
            : {}),
          ...(currentInboundAudio ? { "x-mo-current-inbound-audio": "true" } : {}),
    ...
    expect(
      (
        await sendLoopbackToolCall({
          ...
          headers: { "x-mo-cli-capture-key": captureKey },
        })
      ).status,
    ).toBe(200);
  ...
  it("times out stalled request bodies and closes uploads after flushing 408", async () => {
    const previousTimeout = process.env.MO_MCP_LOOPBACK_BODY_TIMEOUT_MS;
    process.env.MO_MCP_LOOPBACK_BODY_TIMEOUT_MS = "20";
    try {
      server = await startMcpLoopbackServer(0);
      const runtime = getActiveMcpLoopbackRuntime();
      if (!runtime) {
      ...
      if (previousTimeout === undefined) {
        delete process.env.MO_MCP_LOOPBACK_BODY_TIMEOUT_MS;
      } else {
        process.env.MO_MCP_LOOPBACK_BODY_TIMEOUT_MS = previousTimeout;
      }
    }
  });
...
describe("createMcpLoopbackServerConfig", () => {
  it("builds a server entry with env-driven headers", () => {
    ...
    expect(config.mcpServers?.mo?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(config.mcpServers?.mo?.alwaysLoad).toBe(true);
    expect(config.mcpServers?.mo?.headers?.Authorization).toBe(
      "Bearer ${MO_MCP_TOKEN}",
    );
    expect(config.mcpServers?.mo?.headers?.["x-mo-session-id"]).toBe(
      "${MO_MCP_SESSION_ID}",
    );
    expect(config.mcpServers?.mo?.headers?.["x-mo-message-channel"]).toBe(
      "${MO_MCP_MESSAGE_CHANNEL}",
    );
    expect(config.mcpServers?.mo?.headers?.["x-mo-current-channel-id"]).toBe(
      "${MO_MCP_CURRENT_CHANNEL_ID}",
    );
    expect(config.mcpServers?.mo?.headers?.["x-mo-current-thread-ts"]).toBe(
      "${MO_MCP_CURRENT_THREAD_TS}",
    );
    expect(config.mcpServers?.mo?.headers?.["x-mo-current-message-id"]).toBe(
      "${MO_MCP_CURRENT_MESSAGE_ID}",
    );
    expect(config.mcpServers?.mo?.headers?.["x-mo-current-inbound-audio"]).toBe(
      "${MO_MCP_CURRENT_INBOUND_AUDIO}",
    );
    expect(config.mcpServers?.mo?.headers?.["x-mo-source-reply-delivery-mode"]).toBe(
      "${MO_MCP_SOURCE_REPLY_DELIVERY_MODE}",
    );
    expect(
      config.mcpServers?.mo?.headers?.["x-mo-require-explicit-message-target"],
    ).toBe("${MO_MCP_REQUIRE_EXPLICIT_MESSAGE_TARGET}");
    expect(config.mcpServers?.mo?.headers?.["x-mo-cli-capture-key"]).toBe(
      "${MO_MCP_CLI_CAPTURE_KEY}",
    );
    expect(config.mcpServers?.mo?.headers).not.toHaveProperty("x-mo-sender-is-owner");
  });
