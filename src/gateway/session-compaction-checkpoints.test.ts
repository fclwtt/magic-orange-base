...
import { CURRENT_SESSION_VERSION, SessionManager } from "mo/plugin-sdk/agent-sessions";
import type { AssistantMessage } from "mo/plugin-sdk/llm";
...
describe("session-compaction-checkpoints", () => {
  ...
  test("async capture stores pre-compaction identity without copying the transcript", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-checkpoint-async-"));
    tempDirs.push(dir);
  ...
  test("async capture derives session metadata without synchronous SessionManager.open", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-checkpoint-async-metadata-"));
    tempDirs.push(dir);
  ...
  test("async capture follows terminal leaf controls instead of their inactive parent", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-checkpoint-leaf-control-"));
    tempDirs.push(dir);
  ...
  test("async leaf scans ignore controls with dangling references", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-checkpoint-invalid-leaf-"));
    tempDirs.push(dir);
    const sessionFile = path.join(dir, "session.jsonl");
    await fs.writeFile(
      sessionFile,
  ...
  test("async capture scans bounded metadata without copying oversized transcripts", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-checkpoint-async-oversized-"));
    tempDirs.push(dir);
  ...
  test("bounded capture falls back to a forkable raw tail when the leaf target is older", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-checkpoint-bounded-leaf-"));
    tempDirs.push(dir);
    const sessionFile = path.join(dir, "session.jsonl");
    await fs.writeFile(
      sessionFile,
  ...
  test("async fork creates a checkpoint branch transcript without SessionManager sync reads", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-checkpoint-fork-"));
    tempDirs.push(dir);
  ...
  test("async fork truncates mutable checkpoint sources at the stored leaf", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-checkpoint-fork-leaf-"));
    tempDirs.push(dir);
  ...
  test("file-backed checkpoint store restores from the stored transcript boundary", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-checkpoint-store-"));
    tempDirs.push(dir);
  ...
  test("async fork migrates legacy checkpoint snapshots before writing a current header", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-checkpoint-legacy-fork-"));
    tempDirs.push(dir);
  ...
  test("async fork skips JSON-valid garbage transcript entries", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-checkpoint-garbage-fork-"));
    tempDirs.push(dir);
