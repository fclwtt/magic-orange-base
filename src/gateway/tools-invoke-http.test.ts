...
const gatewayAuthHeaders = () => ({ "x-mo-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-mo-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-mo-message-to": "channel:24514",
        "x-mo-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
