...
function buildSnapshot(config: OpenClawConfig): ConfigFileSnapshot {
  ...
  return buildTestConfigSnapshot({
    path: "/tmp/mo-startup-secrets-test.json",
    exists: true,
    raw,
    parsed: config,
    valid: true,
...
function installDiagnosticsTimelineEnv() {
  const root = mkdtempSync(path.join(tmpdir(), "mo-startup-secrets-timeline-"));
  const timelinePath = path.join(root, "timeline.jsonl");
  const previousDiagnostics = process.env.MO_DIAGNOSTICS;
  const previousTimelinePath = process.env.MO_DIAGNOSTICS_TIMELINE_PATH;
  process.env.MO_DIAGNOSTICS = "timeline";
  process.env.MO_DIAGNOSTICS_TIMELINE_PATH = timelinePath;

  return {
    timelinePath,
    cleanup: () => {
      if (previousDiagnostics === undefined) {
        delete process.env.MO_DIAGNOSTICS;
      } else {
        process.env.MO_DIAGNOSTICS = previousDiagnostics;
      }
      if (previousTimelinePath === undefined) {
        delete process.env.MO_DIAGNOSTICS_TIMELINE_PATH;
      } else {
        process.env.MO_DIAGNOSTICS_TIMELINE_PATH = previousTimelinePath;
      }
...
describe("gateway startup config secret preflight", () => {
  const previousSkipChannels = process.env.MO_SKIP_CHANNELS;
  const previousSkipProviders = process.env.MO_SKIP_PROVIDERS;

  afterEach(() => {
    if (previousSkipChannels === undefined) {
      delete process.env.MO_SKIP_CHANNELS;
    } else {
      process.env.MO_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.MO_SKIP_PROVIDERS;
    } else {
      process.env.MO_SKIP_PROVIDERS = previousSkipProviders;
    }
  });
  ...
  it("activates no-SecretRef startup config without importing the full secrets runtime", async () => {
    vi.resetModules();
    const agentDir = mkdtempSync(path.join(tmpdir(), "mo-startup-fast-path-"));
    const runtimeImport = vi.fn();
    const prepareRuntimeSecretsSnapshot = vi.fn(async ({ config }) => preparedSnapshot(config));
    const activateRuntimeSecretsSnapshot = vi.fn();
  ...
  it("keeps the full secrets runtime path when startup config has a SecretRef", async () => {
    const harness = createGatewayStartupSecretsRuntimeHarness("mo-startup-secret-ref-");
  ...
  it("keeps the full secrets runtime path when auth profile files are present", async () => {
    const harness = createGatewayStartupSecretsRuntimeHarness("mo-startup-auth-store-");
    writeFileSync(
