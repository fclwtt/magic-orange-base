...
export function warnLegacyOpenClawEnvVars(env: NodeJS.ProcessEnv = process.env): void {
  ...
  process.emitWarning(
    [
      `Legacy ${detectedPrefixes} environment variables were detected (${legacyVarCount} total), but OpenClaw only reads MO_* names now.`,
      "Rename them by replacing the legacy prefix with MO_; the old names are ignored.",
    ].join("\n"),
    { code: "MO_LEGACY_ENV_VARS", type: "DeprecationWarning" },
  );
  warned = true;
}
