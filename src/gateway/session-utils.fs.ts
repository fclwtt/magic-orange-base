...
export function attachOpenClawTranscriptMeta(
  ...
  const existing =
    record["__mo"] &&
    typeof record["__mo"] === "object" &&
    !Array.isArray(record["__mo"])
      ? (record["__mo"] as Record<string, unknown>)
      : {};
  return {
    ...record,
    __mo: {
...
function buildOversizedTranscriptRecord(line: string): TailTranscriptRecord {
  ...
  const record: Record<string, unknown> = {
    ...
    message: {
      ...
      __mo: { truncated: true, reason: "oversized" },
    },
  };
  return { record };
}
...
function extractTranscriptTokenEstimateFromLine(line: string): {
  ...
  try {
    ...
    const isDeliveryMirror =
      role === "assistant" && modelProvider === "mo" && model === "delivery-mirror";
    if (isDeliveryMirror) {
      return null;
    }
    ...
    const model =
      typeof message.model === "string"
        ...
        : typeof parsed.model === "string"
          ? parsed.model.trim()
          : undefined;
    const isDeliveryMirror = modelProvider === "mo" && model === "delivery-mirror";
