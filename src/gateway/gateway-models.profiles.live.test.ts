...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.MO_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_CREDENTIAL_PRECEDENCE = REQUIRE_PROFILE_KEYS ? "profile-first" : "env-first";
const PROVIDERS = parseFilter(process.env.MO_LIVE_GATEWAY_PROVIDERS);
const GATEWAY_LIVE_SMOKE = isTruthyEnvValue(process.env.MO_LIVE_GATEWAY_SMOKE);
const THINKING_LEVEL = resolveGatewayLiveThinkingLevel({
  raw: process.env.MO_LIVE_GATEWAY_THINKING,
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.MO_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
...
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.MO_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const QUIET_LIVE_LOGS = process.env.MO_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["MO_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveSetupTimeoutMs(
  raw = process.env.MO_LIVE_GATEWAY_SETUP_TIMEOUT_MS,
): number {
  return Math.max(1_000, toInt(raw, 180_000));
}

function resolveGatewayLiveMaxModels(): number {
  const gatewayRaw = process.env.MO_LIVE_GATEWAY_MAX_MODELS?.trim();
  if (gatewayRaw) {
    return Math.max(0, toInt(gatewayRaw, 0));
  }
  const rawModels = process.env.MO_LIVE_GATEWAY_MODELS?.trim();
  ...
  return resolveHighSignalLiveModelLimit({
    rawMaxModels: process.env.MO_LIVE_MAX_MODELS,
    useExplicitModels,
...
function resolveGatewayLiveModelTimeoutMs(
  gatewayModelTimeoutRaw = process.env.MO_LIVE_GATEWAY_MODEL_TIMEOUT_MS,
  liveModelTimeoutRaw = process.env.MO_LIVE_MODEL_TIMEOUT_MS,
...
function enterProductionEnvForLiveRun() {
  const previous = {
    ...
    testFast: process.env.MO_TEST_FAST,
  };
...
function restoreProductionEnvForLiveRun(previous: {
  ...
  if (previous.testFast === undefined) {
    ...
    process.env.MO_TEST_FAST = previous.testFast;
  }
...
describe("resolveGatewayLiveMaxModels", () => {
  const originalGatewayModels = process.env.MO_LIVE_GATEWAY_MODELS;
  const originalGatewayMax = process.env.MO_LIVE_GATEWAY_MAX_MODELS;
  const originalSharedMax = process.env.MO_LIVE_MAX_MODELS;
  ...
  afterEach(() => {
    restoreEnvValue("MO_LIVE_GATEWAY_MODELS", originalGatewayModels);
    restoreEnvValue("MO_LIVE_GATEWAY_MAX_MODELS", originalGatewayMax);
    restoreEnvValue("MO_LIVE_MAX_MODELS", originalSharedMax);
  });

  it("defaults modern gateway sweeps to the curated high-signal cap", () => {
    delete process.env.MO_LIVE_GATEWAY_MODELS;
    delete process.env.MO_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.MO_LIVE_MAX_MODELS;
  ...
  it("defaults small gateway sweeps to the curated small-model cap", () => {
    process.env.MO_LIVE_GATEWAY_MODELS = "small";
    delete process.env.MO_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.MO_LIVE_MAX_MODELS;
  ...
  it("keeps explicit gateway model lists uncapped unless a cap is provided", () => {
    process.env.MO_LIVE_GATEWAY_MODELS = "openai/gpt-5.5,anthropic/claude-opus-4-6";
    delete process.env.MO_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.MO_LIVE_MAX_MODELS;

    expect(resolveGatewayLiveMaxModels()).toBe(0);

    process.env.MO_LIVE_GATEWAY_MAX_MODELS = "2";
...
describe("enterProductionEnvForLiveRun", () => {
  it("clears Vitest fast-reply flags while preserving caller state", () => {
    ...
    try {
      ...
      restoreOptionalEnv("MO_TEST_FAST", previous.testFast);
    }
  });
});
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const runtimeEnv = enterProductionEnvForLiveRun();
  const previous = {
    configPath: process.env.MO_CONFIG_PATH,
    token: process.env.MO_GATEWAY_TOKEN,
    skipChannels: process.env.MO_SKIP_CHANNELS,
    skipGmail: process.env.MO_SKIP_GMAIL_WATCHER,
    skipCron: process.env.MO_SKIP_CRON,
    skipCanvas: process.env.MO_SKIP_CANVAS_HOST,
    disableBonjour: process.env.MO_DISABLE_BONJOUR,
    logLevel: process.env.MO_LOG_LEVEL,
    agentDir: process.env.MO_AGENT_DIR,
    stateDir: process.env.MO_STATE_DIR,
  };
  ...
  if (QUIET_LIVE_LOGS) {
    process.env.MO_DISABLE_BONJOUR = "1";
    process.env.MO_LOG_LEVEL = "silent";
  }
  ...
  const tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-live-state-"));
  setTestEnvValue("MO_STATE_DIR", tempStateDir);
  ...
  setTestEnvValue("MO_AGENT_DIR", tempAgentDir);

  const workspaceDir = resolveAgentWorkspaceDir(params.cfg, agentId);
  await fs.mkdir(workspaceDir, { recursive: true });
  await fs.mkdir(path.join(workspaceDir, ".mo"), { recursive: true });
  await fs.writeFile(
    path.join(workspaceDir, ".mo", "workspace-state.json"),
    `${JSON.stringify(
      {
        version: 1,
  ...
  const toolProbePath = path.join(workspaceDir, `.mo-live-tool-probe.${nonceA}.txt`);
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-live-"));
  const tempConfigPath = path.join(tempDir, "mo.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  setTestEnvValue("MO_CONFIG_PATH", tempConfigPath);
  ...
  try {
    ...
    restoreOptionalEnv("MO_CONFIG_PATH", previous.configPath);
    restoreOptionalEnv("MO_GATEWAY_TOKEN", previous.token);
    restoreOptionalEnv("MO_SKIP_CHANNELS", previous.skipChannels);
    restoreOptionalEnv("MO_SKIP_GMAIL_WATCHER", previous.skipGmail);
    restoreOptionalEnv("MO_SKIP_CRON", previous.skipCron);
    restoreOptionalEnv("MO_SKIP_CANVAS_HOST", previous.skipCanvas);
    restoreOptionalEnv("MO_DISABLE_BONJOUR", previous.disableBonjour);
    restoreOptionalEnv("MO_LOG_LEVEL", previous.logLevel);
    restoreOptionalEnv("MO_AGENT_DIR", previous.agentDir);
    restoreOptionalEnv("MO_STATE_DIR", previous.stateDir);
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
    ...
    async () =>
      await withSuppressedGatewayLiveWarnings(async () => {
        ...
        if (selectedCandidates.length < candidates.length) {
          logProgress(
            `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via MO_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
          );
        }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    try {
      ...
      const nonceA = randomUUID();
      const nonceB = randomUUID();
      toolProbePath = path.join(workspaceDir, `.mo-live-zai-fallback.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
