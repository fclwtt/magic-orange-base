...
async function withGatewayChatHarness(
  ...
  const createSessionDir = async () => {
    const sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-gw-"));
  ...
  try {
    ...
    if (process.env.MO_CONFIG_PATH) {
      await fs.rm(process.env.MO_CONFIG_PATH, { force: true });
    }
    clearConfigCache();
...
function readOpenClawSeq(message: unknown): number | undefined {
  ...
  const metadata = (message as Record<string, unknown>)["__mo"];
...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("MO_CONFIG_PATH missing in gateway test environment");
  }
...
describe("gateway server chat", () => {
  ...
  test("chat.send starts the next WebChat turn after the prior internal run finishes", async () => {
    ...
    try {
      ...
      expect(dispatchOptions[0]?.promptCacheKey).toEqual(
        expect.stringMatching(/^mo-webchat-[a-f0-9]{32}$/u),
      );
  ...
  test("chat.history offset pages overread context before filtering stale announce replies", async () => {
    await withGatewayChatHarness(async ({ ws, createSessionDir }) => {
      ...
      const page = await rpcReq<{
        messages?: Array<{ __mo?: { seq?: number } }>;
        nextOffset?: number;
        hasMore?: boolean;
      }>(ws, "chat.history", {
        sessionKey: "main",
  ...
  test("chat.send diagnostics timeline carries run correlation attributes", async () => {
    const timelineDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-chat-timeline-"));
  ...
  test("chat.history hard-caps single oversized nested payloads", async () => {
    await withGatewayChatHarness(async ({ ws, createSessionDir }) => {
      ...
      expect(messages[0]).toMatchObject({
        __mo: { id: "msg-huge", truncated: true, reason: "oversized" },
      });
