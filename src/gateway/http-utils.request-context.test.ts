...
describe("resolveGatewayRequestContext", () => {
  it("uses normalized x-mo-message-channel when enabled", () => {
    const result = resolveGatewayRequestContext({
      req: createReq({ "x-mo-message-channel": " Custom-Channel " }),
      model: "mo",
  ...
  it("preserves normal explicit session-key overrides", () => {
    const result = resolveGatewayRequestContext({
      req: createReq({ "x-mo-session-key": "customer-case-42" }),
      model: "mo",
      sessionPrefix: "openai",
      defaultMessageChannel: "webchat",
    });
  ...
  it.each([
    ...
    expect(() =>
      resolveGatewayRequestContext({
        req: createReq({ "x-mo-session-key": sessionKey }),
        model: "mo",
        sessionPrefix: "openai",
        defaultMessageChannel: "webchat",
      }),
  ...
  it("does not build session state for explicit unknown agent ids", () => {
    expect(() =>
      resolveGatewayRequestContext({
        req: createReq({ "x-mo-agent-id": "missing-agent" }),
        model: "mo",
        sessionPrefix: "openai",
        defaultMessageChannel: "webchat",
      }),
  ...
  it("drops self-asserted scopes for bearer-authenticated requests", () => {
    const scopes = resolveTrustedHttpOperatorScopes(
      createReq({
        ...
        "x-mo-scopes": "operator.admin, operator.write",
      }),
      tokenAuth,
    );
...
describe("resolveHttpSenderIsOwner", () => {
  it("requires operator.admin on a trusted HTTP scope-bearing request", () => {
    expect(
      resolveHttpSenderIsOwner(createReq({ "x-mo-scopes": "operator.admin" }), noneAuth),
    ).toBe(true);
    expect(
      resolveHttpSenderIsOwner(createReq({ "x-mo-scopes": "operator.write" }), noneAuth),
    ).toBe(false);
  });
...
describe("resolveOpenAiCompatibleHttpSenderIsOwner", () => {
  ...
  it("still requires operator.admin for trusted scope-bearing requests", () => {
    expect(
      resolveOpenAiCompatibleHttpSenderIsOwner(
        createReq({ "x-mo-scopes": "operator.write" }),
        { authMethod: "trusted-proxy", trustDeclaredOperatorScopes: true },
      ),
    ).toBe(false);
    expect(
      resolveOpenAiCompatibleHttpSenderIsOwner(
        createReq({ "x-mo-scopes": "operator.admin" }),
        { authMethod: "trusted-proxy", trustDeclaredOperatorScopes: true },
      ),
    ).toBe(true);
  });
...
describe("authorizeOpenAiCompatibleHttpModelOverride", () => {
  it("allows shared-secret bearer callers to use x-mo-model", () => {
    expect(
      authorizeOpenAiCompatibleHttpModelOverride(
        createReq({ authorization: "Bearer secret", "x-mo-model": "openai/gpt-5.4" }),
        { authMethod: "token", trustDeclaredOperatorScopes: false },
      ),
    ).toEqual({ allowed: true });
  });

  it("allows trusted admin callers to use x-mo-model", () => {
    expect(
      authorizeOpenAiCompatibleHttpModelOverride(
        createReq({
          "x-mo-scopes": "operator.admin, operator.write",
          "x-mo-model": "openai/gpt-5.4",
        }),
        { authMethod: "trusted-proxy", trustDeclaredOperatorScopes: true },
      ),
    ).toEqual({ allowed: true });
  ...
  it("rejects trusted write-only callers that try to use x-mo-model", () => {
    expect(
      authorizeOpenAiCompatibleHttpModelOverride(
        createReq({
          "x-mo-scopes": "operator.write",
