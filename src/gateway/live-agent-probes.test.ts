...
describe("live-agent-probes", () => {
  ...
  it("builds a retryable cron prompt with provider-specific fallback wording", () => {
    ...
    expect(claudeRetryPrompt).toContain("search/load MCP tools for `mo cron` or `cron`");
    expect(claudeRetryPrompt).toContain("mcp__mo__cron");
    expect(claudeRetryPrompt).toContain("Do not use Claude native `CronCreate`");
    expect(claudeRetryPrompt).not.toContain("mo-tools");
    expect(
      buildLiveCronProbeMessage({
        agent: "future-agent",
        argsJson: spec.argsJson,
