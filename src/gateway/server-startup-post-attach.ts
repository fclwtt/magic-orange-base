...
const SKIP_STARTUP_MODEL_PREWARM_ENV = "MO_SKIP_STARTUP_MODEL_PREWARM";
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  const { normalizeProviderId } = await import("@mo/model-catalog-core/provider-id");
  if (
      ...
      await measureStartup(params.startupTrace, "sidecars.acp.identity-reconcile", async () => {
        const [{ getAcpSessionManager }, { ACP_SESSION_IDENTITY_RENDERER_VERSION }] =
          await Promise.all([
            ...
            import("@mo/acp-core/runtime/session-identifiers"),
          ]);
