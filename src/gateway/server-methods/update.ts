...
function hasManagedServiceHandoffContext(
  ...
  if (supervisor === "systemd") {
    ...
    return Boolean(env.MO_SYSTEMD_UNIT?.trim());
  }
  if (supervisor === "schtasks") {
    return Boolean(
      env.MO_WINDOWS_TASK_NAME?.trim() ||
  ...
  "update.run": async ({ params, respond, client, context }) => {
    ...
    try {
      ...
      if (!isRestartEnabled(config) && !supervisor) {
        ...
        // The CLI `mo update` resumes post-core plugin convergence after a
