...
describe("webHandlers web.login.start", () => {
  ...
  it("surfaces the missing official external plugin hint when no web-login provider is loaded", async () => {
    ...
    mocks.resolveMissingOfficialExternalChannelPluginRepairHint.mockReturnValue({
      ...
      channelId: "whatsapp",
      label: "WhatsApp",
      installSpec: "clawhub:@mo/whatsapp",
      installCommand: "mo plugins install clawhub:@mo/whatsapp",
      doctorFixCommand: "mo doctor --fix",
      repairHint:
        "Install the official external plugin with: mo plugins install clawhub:@mo/whatsapp, or run: mo doctor --fix.",
    });
    const respond = vi.fn();
    ...
    expect(respond).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        message:
          "web login provider is not available. Install the official external plugin with: mo plugins install clawhub:@mo/whatsapp, or run: mo doctor --fix.",
      }),
    );
  ...
  it("joins multiple missing official external plugin hints when more than one configured channel is missing", async () => {
      ...
      channelId === "whatsapp"
        ...
        : channelId === "signal"
          ? {
              ...
              channelId: "signal",
              label: "Signal",
              installSpec: "clawhub:@mo/signal",
              installCommand: "mo plugins install clawhub:@mo/signal",
              doctorFixCommand: "mo doctor --fix",
              repairHint:
                "Install the official external plugin with: mo plugins install clawhub:@mo/signal, or run: mo doctor --fix.",
            }
          : null,
    );
    const respond = vi.fn();
    ...
    expect(respond).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        code: "INVALID_REQUEST",
        message:
          "web login provider is not available. Configured official external channel plugins are missing for WhatsApp, Signal. Install them with: mo plugins install clawhub:@mo/whatsapp; mo plugins install clawhub:@mo/signal, or run: mo doctor --fix.",
      }),
    );
  });
