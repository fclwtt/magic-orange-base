...
import { asOptionalRecord } from "@mo/normalization-core/record-coerce";
  ...
  it("ignores user messages that merely contain canvas-shaped text", () => {
    const previewJson = JSON.stringify({
      ...
      view: {
        backend: "canvas",
        id: "cv_user_text",
        url: "/__mo__/canvas/documents/cv_user_text/index.html",
...
describe("projectRecentChatDisplayMessages", () => {
  ...
  it("keeps media-only user messages while dropping empty text-only user messages", () => {
    const mediaOnly = {
      role: "user",
      content: "",
      MediaPath: "/tmp/mo/user-upload.png",
      timestamp: 1,
    };
    const multiMediaOnly = {
      role: "user",
      content: "",
      MediaPaths: ["/tmp/mo/first.png", "/tmp/mo/second.jpg"],
      timestamp: 2,
    };
    const result = projectRecentChatDisplayMessages([
      mediaOnly,
  ...
  it("merges delayed TTS supplements into their original assistant message", () => {
    ...
    const result = projectRecentChatDisplayMessages([
      ...
      },
      {
        ...
        moTtsSupplement: { textSha256, spokenText },
        timestamp: 4,
      },
    ]);
    ...
    const messages = [
      {
        ...
        __mo: { seq: 1, recordTimestampMs: cutoff - 86_400_000 },
      },
      {
        role: "assistant",
        content: [{ type: "text", text: "real reply" }],
        __mo: { seq: 2, recordTimestampMs: cutoff - 86_400_000 },
      },
      {
        role: "user",
        content: [{ type: "text", text: "[Inter-session message] sourceTool=subagent_announce" }],
        provenance: announceProvenance,
        __mo: { seq: 3, recordTimestampMs: cutoff - 1_000 },
      },
      {
        role: "assistant",
        content: [{ type: "text", text: "fanfic lore-bible summary" }],
        __mo: { seq: 4, recordTimestampMs: cutoff - 1_000 },
      },
      {
        role: "user",
        content: [{ type: "text", text: "fresh user turn" }],
        __mo: { seq: 5, recordTimestampMs: cutoff + 5_000 },
      },
    ];
    const out = dropPreSessionStartAnnouncePairs(messages, cutoff);
    expect(out.map((m) => asOptionalRecord(asOptionalRecord(m)?.["__mo"])?.["seq"])).toEqual([
      1, 2, 5,
    ]);
  });
    ...
    const messages = [
      {
        ...
        content: [{ type: "text", text: "[Inter-session message] sourceTool=subagent_announce" }],
        provenance: announceProvenance,
        __mo: { seq: 1, recordTimestampMs: cutoff + 1_000 },
      },
      {
        role: "assistant",
        content: [{ type: "text", text: "current-session reply" }],
        __mo: { seq: 2, recordTimestampMs: cutoff + 2_000 },
      },
    ];
    const out = dropPreSessionStartAnnouncePairs(messages, cutoff);
  ...
  it("keeps an adjacent assistant reply when only the announce user predates the cutoff", () => {
    const messages = [
      ...
      },
      {
        ...
        __mo: { seq: 2, recordTimestampMs: cutoff + 1_000 },
      },
    ];
    const out = dropPreSessionStartAnnouncePairs(messages, cutoff);
...
describe("logs.tail", () => {
  ...
  it("falls back to latest rolling log file when today is missing", async () => {
    const tempDir = await fsPromises.mkdtemp(path.join(os.tmpdir(), "mo-logs-"));
    const older = path.join(tempDir, "mo-2026-01-20.log");
    const newer = path.join(tempDir, "mo-2026-01-21.log");
    ...
    setLoggerOverride({ file: path.join(tempDir, "mo-2026-01-22.log") });
  ...
  it("redacts sensitive CLI tokens from returned lines", async () => {
    const tempDir = await fsPromises.mkdtemp(path.join(os.tmpdir(), "mo-logs-"));
    const file = path.join(tempDir, "mo-2026-01-22.log");
