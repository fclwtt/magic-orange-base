...
import { isAudioFileName } from "@mo/media-core/mime";
import { asOptionalRecord } from "@mo/normalization-core/record-coerce";
import type { FastMode } from "@mo/normalization-core/string-coerce";
...
export function buildOversizedHistoryPlaceholder(message?: unknown): Record<string, unknown> {
  ...
  const rawMetadata =
    message && typeof message === "object"
      ? (message as Record<string, unknown>)["__mo"]
      : undefined;
  const metadata =
...
async function findSourceReplyTranscriptMirrorByIdempotencyKey(
  ...
  if (found?.message.provider !== "mo" || found.message.model !== "delivery-mirror") {
    return null;
  }
  return found;
}
...
function readChatHistoryMessageId(message: unknown): string | undefined {
  const metadata = asOptionalRecord(asOptionalRecord(message)?.["__mo"]);
...
export const chatHandlers: GatewayRequestHandlers = {
  ...
  "chat.send": async ({ params, respond, context, client }) => {
    ...
    try {
      ...
      // See: https://github.com/mo/mo/issues/3658
                  ...
                  if (shouldAppendAssistantTranscript) {
                    ...
                    if (appended.ok) {
                      ...
                      message = {
                        ...
                        timestamp: nowValue,
                        ...(ttsSupplementMarker
                          ? { moTtsSupplement: ttsSupplementMarker }
                          : {}),
