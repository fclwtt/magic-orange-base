...
export async function handleOpenAiEmbeddingsHttpRequest(
  ...
  if (requestModel !== MO_MODEL_ID && !resolveAgentIdFromModel(requestModel, cfg)) {
    sendJson(res, 400, {
      error: {
        message: "Invalid `model`. Use `mo` or `mo/<agentId>`.",
  ...
  const overrideModel =
    normalizeOptionalString(getHeader(req, "x-mo-model")) ||
