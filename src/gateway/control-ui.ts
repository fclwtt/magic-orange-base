...
import { detectMime } from "@mo/media-core/mime";
import {
  asDateTimestampMs,
  resolveTimestampMsToIsoString,
} from "@mo/normalization-core/number-coercion";
...
const ROOT_PREFIX = "/";
const CONTROL_UI_ASSISTANT_MEDIA_PREFIX = "/__mo__/assistant-media";
...
const CONTROL_UI_NAMESPACE_PREFIX = "/__mo__/";
...
// `/__mo__/`, so a browser that opens the default entry infers
// `/__mo__` as its base path (see `inferBasePathFromPathname`) and fetches
// `/__mo__/control-ui-config.json`. Accept that namespaced alias so the
...
// Single-underscore `/__mo` prefix used by the pre-base-path-relative
...
// `/__mo/control-ui-config.json`, so current main and the v2026.6.1
// release serve and document that exact path under an empty base path.
const LEGACY_CONTROL_UI_NAMESPACE_PREFIX = "/__mo";

// The old documented no-base-path bootstrap endpoint
// (`/__mo/control-ui-config.json`, single underscore). It is derived from
// the legacy `/__mo` namespace joined with the canonical config constant
 ...
 * single-underscore suffix `${basePath}/__mo/control-ui-config.json` that
 ...
 * the default-namespace alias `/__mo__/control-ui-config.json`, which is
 * what the default `/__mo__/` entry requests after inferring its base path
...
function matchesControlUiBootstrapConfigPath(pathname: string, basePath: string): boolean {
  ...
  // `/__mo` namespace, so joining it with the base path yields
  // `${basePath}/__mo/control-ui-config.json` (or the bare legacy path
  ...
  // The default `/__mo__/` namespace alias only applies when no base path
