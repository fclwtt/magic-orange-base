...
vi.mock("../config/paths.js", async () => {
  ...
  return {
    ...actual,
    STATE_DIR: "/tmp/mo-state",
    resolveConfigPath: vi.fn(() => "/tmp/mo-state/mo.json"),
    resolveGatewayPort: vi.fn(() => 18789),
    resolveStateDir: vi.fn((env: NodeJS.ProcessEnv = process.env) =>
      env.MO_STATE_DIR?.trim() ? actual.resolveStateDir(env) : "/tmp/mo-state",
    ),
  };
});
...
describe("startGatewayPostAttachRuntime", () => {
  beforeEach(() => {
    ...
    vi.stubEnv("MO_SKIP_CHANNELS", "0");
    vi.stubEnv("MO_SKIP_PROVIDERS", "0");
  ...
  it("skips heavy restart sentinel refresh when no sentinel file exists", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-no-sentinel-"));
    try {
  ...
  it("refreshes the restart sentinel when the sentinel row exists", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-sentinel-"));
    try {
      await writeRestartSentinel(
        ...
        { MO_STATE_DIR: stateDir } as NodeJS.ProcessEnv,
      );
      await withEnvAsync({ MO_STATE_DIR: stateDir }, async () => {
  ...
  it("detects restart sentinel rows in explicit state directories", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-sentinel-state-"));
    try {
      await writeRestartSentinel(
        {
          kind: "update",
  ...
  it("avoids sync filesystem probes while checking restart sentinel presence", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-async-sentinel-"));
    try {
      await writeRestartSentinel(
        {
          kind: "update",
  ...
  it("defers qmd memory backend startup refresh until the idle delay elapses", async () => {
    ...
    try {
      await startGatewaySidecars({
        ...
        defaultWorkspaceDir: "/tmp/mo-workspace",
        deps: {} as never,
  ...
  it("cleans startup session locks with bounded concurrency", async () => {
    ...
    let maxActive = 0;
    const cleanedLock = {
      lockPath: "/tmp/mo-state/agents/main/sessions/a.jsonl.lock",
      pid: null,
      pidAlive: false,
      createdAt: null,
      ageMs: null,
  ...
  it("starts channels when channel startup is enabled", async () => {
    await withEnvAsync(
      {
        MO_SKIP_CHANNELS: undefined,
        MO_SKIP_PROVIDERS: undefined,
      },
      async () => {
  ...
  it("starts and reports plugin services after channel startup completes", async () => {
    await withEnvAsync(
      { MO_SKIP_CHANNELS: undefined, MO_SKIP_PROVIDERS: undefined },
      async () => {
  ...
  it("emits a startup trace span when channel startup is skipped", async () => {
    ...
    await withEnvAsync(
      { MO_SKIP_CHANNELS: "1", MO_SKIP_PROVIDERS: undefined },
      async () => {
        await startGatewaySidecars({
          cfg: { hooks: { internal: { enabled: false } } } as never,
          pluginRegistry: createPostAttachParams().pluginRegistry,
    ...
    expect(logChannels.info).toHaveBeenCalledWith(
      "skipping channel start (MO_SKIP_CHANNELS=1 or MO_SKIP_PROVIDERS=1)",
    );
  });
