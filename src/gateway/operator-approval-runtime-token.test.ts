...
const envSnapshot = captureEnv(["HOME", "MO_HOME"]);

const tempHomes: string[] = [];

function useTempHome(): string {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), "mo-approval-runtime-"));
  tempHomes.push(home);
  setTestEnvValue("HOME", home);
  setTestEnvValue("MO_HOME", home);
  return home;
}

function execApprovalsPath(home: string): string {
  return path.join(home, ".mo", "exec-approvals.json");
}

function writeExecApprovalsToken(home: string, token: string): void {
  fs.mkdirSync(path.join(home, ".mo"), { recursive: true });
  fs.writeFileSync(
