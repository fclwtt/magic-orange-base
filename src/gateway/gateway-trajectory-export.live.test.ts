...
const CODEX_HARNESS_LIVE = process.env.MO_LIVE_CODEX_HARNESS === "1";
const CODEX_HARNESS_DEBUG = process.env.MO_LIVE_CODEX_HARNESS_DEBUG === "1";
...
function snapshotEnv(): LiveEnvSnapshot {
  return snapshotLiveEnv(["MO_TRAJECTORY", "MO_TRAJECTORY_DIR"]);
}
...
describeLive("gateway live trajectory export", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempDir = await fs.mkdtemp(path.join(process.cwd(), ".tmp-mo-trajectory-live-"));
      cleanup.push(async () => {
      ...
      process.env.MO_TRAJECTORY = "1";
      process.env.MO_TRAJECTORY_DIR = trajectoryDir;
      ...
      const bundleDir = path.join(workspaceDir, ".mo", "trajectory-exports", "bundle");
