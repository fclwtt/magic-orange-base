...
const { createOpenClawTools } = await import("../agents/mo-tools.js");
...
beforeAll(async () => {
  envSnapshot = captureEnv(["MO_GATEWAY_PORT", "MO_GATEWAY_TOKEN"]);
  ...
  process.env.MO_GATEWAY_PORT = String(gatewayPort);
  process.env.MO_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send gateway loopback", () => {
  ...
  it(
    ...
    async () => {
      const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-sessions-send-route-"));
