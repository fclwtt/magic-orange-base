...
const gatewayEnv = {
  MO_GATEWAY_TOKEN: "env-token", // pragma: allowlist secret
  MO_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
function withGatewayAuthEnv<T>(env: NodeJS.ProcessEnv, fn: () => T): T {
  return withEnv(
    {
      MO_GATEWAY_TOKEN: env.MO_GATEWAY_TOKEN,
      MO_GATEWAY_PASSWORD: env.MO_GATEWAY_PASSWORD,
      MO_SERVICE_KIND: env.MO_SERVICE_KIND,
    },
    fn,
  );
}
