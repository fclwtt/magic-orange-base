...
const WRITE_SCOPE_HEADER = { "x-mo-scopes": "operator.write" };
...
describe("OpenAI-compatible embeddings HTTP API (e2e)", () => {
  it("embeds string and array inputs", async () => {
    ...
    const qualified = await postEmbeddings(
      ...
      { "x-mo-model": "openai/text-embedding-3-small" },
    );
    expect(qualified.status).toBe(200);
    const qualifiedJson = (await qualified.json()) as { model?: string };
    expect(qualifiedJson.model).toBe("mo/default");
  ...
  it("rejects disallowed x-mo-model provider overrides", async () => {
    const res = await postEmbeddings(
      {
        model: "mo/default",
        input: "hello",
      },
      { "x-mo-model": "ollama/nomic-embed-text" },
    );
    await expectInvalidEmbeddingRequest(
      res,
  ...
  it("rejects x-mo-model for trusted write-only callers", async () => {
