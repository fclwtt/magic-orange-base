...
type CallGatewayBaseOptions = {
   ...
   * Bypasses MO_GATEWAY_URL and MO_GATEWAY_PORT for this call only.
   */
...
function resolveGatewayCallTimeout(
  ...
  const hasEnvHandshakeTimeout =
    Boolean(process.env.MO_HANDSHAKE_TIMEOUT_MS) ||
    Boolean(process.env.VITEST && process.env.MO_TEST_HANDSHAKE_TIMEOUT_MS);
...
async function resolveGatewayCallContext(
  ...
  const envUrlOverride =
    cliUrlOverride || opts.localPortOverride !== undefined
      ? undefined
      : trimToUndefined(process.env.MO_GATEWAY_URL);
