...
describe("gateway codex harness live helpers", () => {
  ...
  it("accepts current status prose that reports healthy session context without the session id", () => {
    const text = [
      ...
      "",
      "Session context is healthy: `22k/272k` tokens used, `0` compactions, `53%` cache hit. Current workspace is `/tmp/mo-live-codex-harness/workspace/dev`.",
    ].join("\n");
  ...
  it("accepts current app-server status prose without the OpenClaw prefix", () => {
    const text = [
      "Status: running on `openai/gpt-5.5` in `/tmp/mo-live-codex-harness/workspace/dev`.",
      "",
  ...
  it("accepts workspace-only healthy status prose emitted by current codex", () => {
    const text =
      "Working normally. Current workspace: `/tmp/mo-live-codex-harness/workspace/dev`.";
  ...
  it("accepts the online idle status emitted by current codex", () => {
    const text =
      "I'm online in `/tmp/mo-live-codex-harness-KiaUQ4/workspace/dev`, with workspace-write access. No active task is running right now.";
  ...
  it("accepts the normal-work status emitted by current codex", () => {
    const text =
      "Working normally. Current cwd is `/tmp/mo-live-codex-harness/workspace/dev`, sandbox is workspace-write, network is restricted, and the current date is 2026-05-09 UTC.";
