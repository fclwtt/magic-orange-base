...
function shouldLogMcpLoopbackHttp(): boolean {
  return (
    isTruthyEnvValue(process.env.MO_CLI_BACKEND_LOG_OUTPUT) ||
    isTruthyEnvValue(process.env.MO_LIVE_CLI_BACKEND_DEBUG)
  );
}
...
export function resolveMcpHttpBodyTimeoutMs(): number {
  return readPositiveIntEnv("MO_MCP_LOOPBACK_BODY_TIMEOUT_MS", DEFAULT_MCP_BODY_TIMEOUT_MS);
}

export function resolveMcpCliCaptureKey(req: IncomingMessage): string | undefined {
  return normalizeOptionalString(getHeader(req, "x-mo-cli-capture-key"));
}
...
export function resolveMcpRequestContext(
  ...
  return {
    ...
    sessionId: normalizeOptionalString(getHeader(req, "x-mo-session-id")),
    messageProvider:
      normalizeMessageChannel(getHeader(req, "x-mo-message-channel")) ?? undefined,
    currentChannelId: normalizeOptionalString(getHeader(req, "x-mo-current-channel-id")),
    currentThreadTs: normalizeOptionalString(getHeader(req, "x-mo-current-thread-ts")),
    currentMessageId: normalizeOptionalString(getHeader(req, "x-mo-current-message-id")),
    currentInboundAudio: normalizeMcpBooleanHeader(
      getHeader(req, "x-mo-current-inbound-audio"),
    ),
    accountId: normalizeOptionalString(getHeader(req, "x-mo-account-id")),
    inboundEventKind: normalizeMcpInboundEventKind(getHeader(req, "x-mo-inbound-event-kind")),
    sourceReplyDeliveryMode: normalizeMcpSourceReplyDeliveryMode(
      getHeader(req, "x-mo-source-reply-delivery-mode"),
    ),
    requireExplicitMessageTarget: normalizeMcpBooleanHeader(
      getHeader(req, "x-mo-require-explicit-message-target"),
    ),
    senderIsOwner: auth.senderIsOwner,
  };
}
