...
import { asFiniteNumber } from "@mo/normalization-core/number-coercion";
import { asOptionalRecord as readRecord } from "@mo/normalization-core/record-coerce";
import { normalizeOptionalString } from "@mo/normalization-core/string-coerce";
import { MO_RUNTIME_CONTEXT_CUSTOM_TYPE } from "../agents/internal-runtime-context.js";
...
import { isOpenClawDeliveryMirrorAssistantMessage } from "../shared/transcript-only-mo-assistant.js";
...
function sanitizeChatHistoryContentBlock(
  ...
  if ("moReasoningReplay" in entry) {
    delete entry.moReasoningReplay;
    changed = true;
  }
...
function buildMessageToolVisibleReplyMirror(
  ...
  const transcriptMeta = readRecord((pending.completionAnchor ?? pending.anchor)["__mo"]);
  if (transcriptMeta) {
    mirror["__mo"] = { ...transcriptMeta };
  }
  return mirror;
}
...
function readTtsSupplementMarker(
  ...
  const marker = message.moTtsSupplement;
...
function readChatHistoryRecordTimestampMs(message: unknown): number | undefined {
  const meta = readRecord(readRecord(message)?.["__mo"]);
...
function isDisplayHiddenProjectedMessage(message: Record<string, unknown>): boolean {
  ...
  return message.role === "custom" && message.customType === MO_RUNTIME_CONTEXT_CUSTOM_TYPE;
}
...
function moAssistantModel(message: Record<string, unknown>): string | undefined {
...
function isDuplicateAcpGatewayInjectedMessage(
  ...
  if (
    moAssistantModel(previousVisible) !== "acp-runtime" ||
    moAssistantModel(current) !== "gateway-injected"
  ) {
    return false;
  }
