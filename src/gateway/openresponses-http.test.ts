...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("MO_CONFIG_PATH is required for gateway config tests");
  }
  ...
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "mo", input: "hi" }),
      });
      ...
      expect(invalidModelJson.error?.message).toBe(
        "Invalid `model`. Use `mo` or `mo/<agentId>`.",
      );
      ...
      const resSessionOverride = await postResponses(
        ...
        {
          "x-mo-agent-id": "beta",
          "x-mo-session-key": "agent:beta:openresponses:custom",
        },
      );
      ...
      const resReservedSessionOverride = await postResponses(
        port,
        { model: "mo", input: "hi" },
        { "x-mo-session-key": "agent:main:subagent:spoofed" },
      );
      ...
      expect(reservedSessionJson.error?.message).toBe(
        "`x-mo-session-key` cannot use reserved internal session namespaces.",
      );
      expect(agentCommand).toHaveBeenCalledTimes(0);

      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "mo/beta", input: "hi" });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resDefaultAlias = await postResponses(port, { model: "mo/default", input: "hi" });
      ...
      {
        agentCommand.mockClear();
        const res = await postResponses(port, { model: "mo/missing-agent", input: "hi" });
        expect(res.status).toBe(400);
        const json = (await res.json()) as { error?: { type?: string; message?: string } };
        expect(json.error?.type).toBe("invalid_request_error");
        expect(json.error?.message).toBe("Unknown agent 'missing-agent'.");
      ...
      const resChannelHeader = await postResponses(
        port,
        { model: "mo", input: "hi" },
        { "x-mo-message-channel": "custom-client-channel" },
      );
      ...
      expect(invalidOverrideJson.error?.message).toBe("Invalid `x-mo-model`.");
      expect(agentCommand).toHaveBeenCalledTimes(0);
