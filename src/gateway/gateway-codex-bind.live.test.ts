...
async function writePluginBindingApproval(params: {
  ...
  const moDir = path.join(params.homeDir, ".mo");
  await fs.mkdir(moDir, { recursive: true });
  await fs.writeFile(
    path.join(moDir, "plugin-binding-approvals.json"),
    `${JSON.stringify(
      {
        version: 1,
        approvals: [
...
function resolveCodexBindModelProvider(): string | undefined {
  const configured = process.env.MO_LIVE_CODEX_BIND_PROVIDER?.trim();
  if (configured) {
    return configured;
  }
  return process.env.MO_LIVE_CODEX_HARNESS_AUTH === "api-key" ? "openai" : undefined;
}
...
describeLive("gateway live (native Codex conversation binding)", () => {
  it(
    ...
    async () => {
      ...
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mo-live-codex-bind-"));
      ...
      const bindModel =
        process.env.MO_LIVE_CODEX_BIND_MODEL?.trim() || DEFAULT_CODEX_BIND_MODEL;
