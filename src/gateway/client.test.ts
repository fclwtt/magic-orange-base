...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("mo doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  beforeEach(async () => {
    envSnapshot.restore();
    delete process.env.MO_ALLOW_INSECURE_PRIVATE_WS;
    delete process.env.MO_PROXY_ACTIVE;
    delete process.env.MO_PROXY_LOOPBACK_MODE;
  ...
  it("allows ws:// hostnames with MO_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.MO_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://mo-gateway.ai:18789",
      onConnectError,
    });
...
describe("GatewayClient close handling", () => {
  ...
  it("clears stale token on device token mismatch close", () => {
    const onClose = vi.fn();
    const env = { MO_HOME: "/tmp/custom-mo-home" };
...
describe("GatewayClient connect auth payload", () => {
  ...
  it("loads stored device auth from the provided env", () => {
    ...
    const env = {
      ...process.env,
      MO_STATE_DIR: "/tmp/mo-client-service-state",
