...
function createGatewayStartupTrace() {
  const logEnabled = isTruthyEnvValue(process.env.MO_GATEWAY_STARTUP_TRACE);
  ...
  const eventLoopTimelineEnabled = () =>
    ...
    isTruthyEnvValue(process.env.MO_DIAGNOSTICS_EVENT_LOOP);
...
function formatRuntimeGatewayAuthTokenWarning(): string {
  ...
  if (!isNixMode) {
    return `${base} Persist one with \`mo config set gateway.auth.mode token\` and \`mo config set gateway.auth.token <token>\`.`;
  }
  return [
    base,
    "In Nix mode, set gateway.auth.token in your Nix-managed OpenClaw config and rebuild.",
    "For the first-party Nix flow, see https://github.com/mo/nix-mo#quick-start and https://docs.mo.ai/install/nix.",
  ].join(" ");
}
...
export type GatewayServerOptions = {
   ...
   * reparsing mo.json during server startup.
   */
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    isVitestRuntimeEnv() && process.env.MO_TEST_MINIMAL_GATEWAY === "1";
