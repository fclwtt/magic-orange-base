...
const mocks = vi.hoisted(() => ({
  ...
  resolveWideAreaDiscoveryDomain: vi.fn<ResolveWideAreaDiscoveryDomain>(() => "mo.internal."),
  writeWideAreaGatewayZone: vi.fn<WriteWideAreaGatewayZone>(async () => ({
    changed: true,
    zonePath: "/tmp/mo.internal.db",
  })),
  formatBonjourInstanceName: vi.fn((name: string) => `${name} (OpenClaw)`),
  resolveBonjourCliPath: vi.fn(() => "/usr/local/bin/mo"),
...
async function expectSshPortOmitted(rawPort: string) {
  ...
  process.env.MO_SSH_PORT = rawPort;
...
function startStuckDiscovery(timeoutMs: string) {
  vi.useFakeTimers();
  useDevelopmentDiscoveryEnv();
  process.env.MO_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS = timeoutMs;
...
describe("startGatewayDiscovery", () => {
  ...
  it("skips local discovery services for truthy MO_DISABLE_BONJOUR values", async () => {
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.MO_DISABLE_BONJOUR = "yes";
  ...
  it("keeps wide-area DNS-SD publishing active when local discovery is off", async () => {
    ...
    const result = await startGatewayDiscovery({
      ...
      wideAreaDiscoveryDomain: "mo.internal.",
      tailscaleMode: "serve",
      mdnsMode: "off",
      gatewayDiscoveryServices: [service],
      logDiscovery: logs,
    ...
    expect(zoneParams.domain).toBe("mo.internal.");
    ...
    expect(logs.info.mock.calls).toEqual([
      ["wide-area DNS-SD updated (mo.internal. → /tmp/mo.internal.db)"],
    ]);
    expect(result.bonjourStop).toBeNull();
  });
