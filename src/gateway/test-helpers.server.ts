...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.MO_CONFIG_PATH) {
    configPaths.add(process.env.MO_CONFIG_PATH);
  }
  if (process.env.MO_STATE_DIR) {
    configPaths.add(path.join(process.env.MO_STATE_DIR, "mo.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "mo-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.MO_STATE_DIR = path.join(tempHome, ".mo");
  delete process.env.MO_CONFIG_PATH;
  delete process.env.MO_AGENT_DIR;
}
...
function applyGatewaySkipEnv() {
  ...
  process.env.MO_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "mo-test-no-bundled-extensions")
    : "mo-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".mo-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".mo-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".mo-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>mo-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__moTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__moTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["MO_GATEWAY_TOKEN"]);
  const prev = process.env.MO_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.MO_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.MO_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
