...
const MO_CLI_GATEWAY_TIMEOUT_MS = 30_000;
const MO_CLI_CHILD_TIMEOUT_MS = MO_CLI_GATEWAY_TIMEOUT_MS + 45_000;
...
export function buildLiveCronProbeMessage(params: {
  ...
  if (params.attempt === 0) {
    return (
      "Use the OpenClaw MCP cron tool from server `mo`. " +
      "If it is not already visible, search/load MCP tools for `mo cron` or `cron`, " +
      "then call the matching OpenClaw MCP tool; Claude-style names may appear as `mcp__mo__cron`. " +
  ...
  if (claudeLike) {
    return (
      "Retry the OpenClaw MCP cron tool from server `mo` now. " +
      "If it is not already visible, search/load MCP tools for `mo cron` or `cron`, " +
      "then call the matching OpenClaw MCP tool; Claude-style names may appear as `mcp__mo__cron`. " +
      "Do not use Claude native `CronCreate`, `CronList`, or `CronDelete`; those are not OpenClaw proof. " +
  ...
  return (
    ...
    "Retry the OpenClaw MCP cron tool from server `mo` now. " +
    "If the harness shows Claude-style MCP names, use `mcp__mo__cron`. " +
    `Use these exact JSON arguments: ${params.argsJson}. ` +
    "Preserve job.sessionTarget and job.sessionKey exactly as provided. " +
    `If the cron job is created, reply exactly: ${params.exactReply}. ` +
    "If the tool call is cancelled, the job is not created, or you cannot confirm creation, " +
...
export async function runOpenClawCliJson<T>(args: string[], env: NodeJS.ProcessEnv): Promise<T> {
  ...
  const cliArgs = args.includes("--timeout")
    ? args
    : [...args, "--timeout", String(MO_CLI_GATEWAY_TIMEOUT_MS)];
  const { stdout, stderr } = await execFileAsync(process.execPath, ["mo.mjs", ...cliArgs], {
    cwd: process.cwd(),
    env: childEnv,
    timeout: MO_CLI_CHILD_TIMEOUT_MS,
    maxBuffer: 1024 * 1024,
  });
  ...
  if (!trimmed) {
    throw new Error(
      [
        `mo ${args.join(" ")} produced no JSON stdout`,
  ...
  try {
    ...
    throw new Error(
      [
        `mo ${args.join(" ")} returned invalid JSON`,
        `stdout: ${trimmed}`,
        stderr.trim() ? `stderr: ${stderr.trim()}` : undefined,
