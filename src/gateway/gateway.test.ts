...
async function createEmptyBundledPluginsDir(tempHome: string): Promise<string> {
  const bundledPluginsDir = path.join(tempHome, "mo-test-empty-bundled-plugins");
...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".mo", "extensions", params.id);
...
async function setupGatewayTempHome(params: { prefix: string; minimalGateway?: boolean }) {
  const envSnapshot = captureEnv([
    ...
    ...(params.minimalGateway ? (["MO_TEST_MINIMAL_GATEWAY"] as const) : []),
  ]);
  ...
  setTestEnvValue("MO_SKIP_PROVIDERS", "1");
  if (params.minimalGateway) {
    setTestEnvValue("MO_TEST_MINIMAL_GATEWAY", "1");
  } else {
    deleteTestEnvValue("MO_TEST_MINIMAL_GATEWAY");
  }

  const workspaceDir = path.join(tempHome, "mo");
  await fs.mkdir(workspaceDir, { recursive: true });
  setTestEnvValue("MO_BUNDLED_PLUGINS_DIR", await createEmptyBundledPluginsDir(tempHome));
  setTestEnvValue("MO_DISABLE_BUNDLED_PLUGINS", "1");
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "mo-minimal-gateway-home-"));
      const configPath = await createGatewayConfigPath(tempHome);
      const bundledPluginsDir = path.join(tempHome, "mo-test-no-bundled-extensions");
      setTestEnvValue("HOME", tempHome);
      setTestEnvValue("MO_STATE_DIR", path.join(tempHome, ".mo"));
      setTestEnvValue("MO_CONFIG_PATH", configPath);
      setTestEnvValue("MO_SKIP_CHANNELS", "1");
      ...
      setTestEnvValue("MO_SKIP_BROWSER_CONTROL_SERVER", "1");
      setTestEnvValue("MO_SKIP_PROVIDERS", "1");
      setTestEnvValue("MO_BUNDLED_PLUGINS_DIR", bundledPluginsDir);
      setTestEnvValue("MO_TEST_MINIMAL_GATEWAY", "1");
