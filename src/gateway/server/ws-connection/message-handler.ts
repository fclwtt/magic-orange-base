...
/**
 * Lazily resolve the local node host's nodeId from ~/.mo/node.json.
 * Process-stable: only changes on `mo node install`, which requires restart.
 */
...
function resolveTrustedProxyControlUiScopes(params: {
  ...
  const rawHeader = firstHeaderValue(params.upgradeReq.headers["x-mo-scopes"]);
  ...
  const handleMessage = async (data: RawData) => {
    ...
    try {
      ...
      if (!client) {
        ...
        // matching instanceId against ~/.mo/node.json nodeId). SSH-tunneled remote
