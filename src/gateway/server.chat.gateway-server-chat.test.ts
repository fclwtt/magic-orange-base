...
describe("gateway server chat", () => {
  ...
  const withMainSessionStore = async <T>(
    ...
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-gw-"));
    try {
  ...
  test("sessions.send accepts dashboard messages for existing sessions", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-sessions-send-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("sessions.send creates a configured agent main session before sending", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-sessions-send-agent-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
  ...
  test("sessions.steer accepts dashboard follow-up messages for existing sessions", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-sessions-steer-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("sessions.abort stops active dashboard runs", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-sessions-abort-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("sessions.abort resolves active runs by runId without a caller session key", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-sessions-abort-runid-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("handles chat send and history flows", async () => {
    ...
    try {
      ...
      const sendPolicyDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-gw-"));
      ...
      const agentBlockedDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-gw-"));
      ...
      const historyDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-gw-"));
  ...
  test("chat.history mirrors current-session message tool sends before NO_REPLY", async () => {
    ...
    expect(
      historyMessages.some((message) => {
        ...
        const entry = message as { role?: unknown; moMessageToolMirror?: unknown };
        return entry.role === "assistant" && Boolean(entry.moMessageToolMirror);
      }),
    ).toBe(true);
  });
    ...
    expect(
      historyMessages.some(
        (message) =>
          ...
          Boolean((message as { moMessageToolMirror?: unknown }).moMessageToolMirror),
      ),
    ).toBe(true);
    expect(historyMessages).not.toContainEqual(
      expect.objectContaining({ provider: "mo", model: "delivery-mirror" }),
    );
  });
