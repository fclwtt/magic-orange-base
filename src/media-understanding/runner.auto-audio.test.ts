...
async function runAutoAudioCase(params: {
  ...
  await withAudioFixture("mo-auto-audio", async ({ ctx, media, cache }) => {
...
describe("runCapability auto audio entries", () => {
  ...
  it("skips OpenAI audio auto-selection when only ChatGPT OAuth is available", async () => {
    ...
    try {
      await withAudioFixture("mo-auto-audio-oauth-skip", async ({ ctx, media, cache }) => {
  ...
  it("passes workspaceDir to auto-selected audio provider execution auth", async () => {
    ...
    await withAudioFixture("mo-auto-audio-workspace-auth", async ({ ctx, media, cache }) => {
      const result = await runCapability({
        capability: "audio",
        cfg: {
          models: {
  ...
  it("uses the provider audio default instead of the active Codex chat model", async () => {
    ...
    await withAudioFixture("mo-auto-audio-codex", async ({ ctx, media, cache }) => {
  ...
  it("prefers provider keys over auto-detected local whisper", async () => {
    const binDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-auto-audio-bin-"));
    try {
  ...
  it("uses mistral when only mistral key is configured", async () => {
    const isolatedAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-audio-agent-"));
    let runResult: Awaited<ReturnType<typeof runCapability>> | undefined;
    try {
      await withEnvAsync(
        {
          ...
          MO_AGENT_DIR: isolatedAgentDir,
        },
        async () => {
          await withAudioFixture("mo-auto-audio-mistral", async ({ ctx, media, cache }) => {
            const providerRegistry = createProviderRegistry({
              openai: {
                id: "openai",
                capabilities: ["audio"],
