...
import { mergeInboundPathRoots } from "@mo/media-core/inbound-path-policy";
import { findNormalizedProviderValue } from "@mo/model-catalog-core/provider-id";
import {
...
async function probeAntigravityCliCandidate(command: string): Promise<string | null> {
  ...
  const probeDir = await fs.mkdtemp(
    path.join(resolvePreferredOpenClawTmpDir(), "mo-antigravity-probe-"),
  );
  try {
...
async function resolveAntigravityCliBinary(): Promise<string | null> {
  ...
  const resolved = (async () => {
    const configured = process.env.MO_ANTIGRAVITY_CLI?.trim();
