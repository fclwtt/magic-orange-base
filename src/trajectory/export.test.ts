...
describe("exportTrajectoryBundle", () => {
  it("sanitizes session ids in default export directory names", () => {
    ...
    expect(outputDir).toBe(
      path.join(
        ...
        ".mo",
        "trajectory-exports",
        "mo-trajectory-___evil_-2026-04-22T08-00-00",
      ),
    );
  });
    ...
    fs.writeFileSync(
      ...
      [
        ...
        },
        {
          ...
          sessionId: "session-1",
          data: {
            harness: { type: "mo", token: rawSecrets[3] },
            metadata: {
  ...
  it("uses the recorded runtime pointer before current environment overrides", async () => {
    ...
    const previous = process.env.MO_TRAJECTORY_DIR;
    process.env.MO_TRAJECTORY_DIR = envRuntimeDir;
    try {
      const bundle = await exportTrajectoryBundle({
        outputDir,
        sessionFile,
      ...
      if (previous === undefined) {
        ...
        process.env.MO_TRAJECTORY_DIR = previous;
      }
    }
  });
