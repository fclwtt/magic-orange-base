...
describe("trajectory metadata", () => {
  it("redacts harness argv and local paths with the support redaction rules", () => {
    ...
    process.argv = [
      "node",
      "/Users/tester/project/mo.js",
      "--api-key",
      "super-secret",
      "--config=/Users/tester/.mo/mo.json",
    ];
    try {
      const metadata = buildTrajectoryRunMetadata({
        env: {
          HOME: "/Users/tester",
          MO_STATE_DIR: "/Users/tester/.mo",
        },
      ...
      expect(harness.invocation).toEqual([
        ...
        "--api-key",
        "<redacted>",
        "--config=$MO_STATE_DIR/mo.json",
      ]);
      expect(harness.entrypoint).toBe("~/project/mo.js");
  ...
  it("redactPathForSupport returns empty string for null/undefined input", () => {
    const ctx: SupportRedactionContext = { env: {}, stateDir: "/tmp/.mo" };
