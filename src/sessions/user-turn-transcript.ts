...
import { mimeTypeFromFilePath } from "@mo/media-core/mime";
...
function buildPersistedUserTurnMessage(params: UserTurnInput): PersistedUserTurnMessage {
  ...
  // the live turn) — see https://github.com/mo/mo/issues/3658.
  ...
  const message = {
    ...
    ...(params.senderIsOwner === undefined
      ? {}
      : { __mo: { senderIsOwner: params.senderIsOwner } }),
    ...mediaFields,
...
function isBeforeAgentRunBlockedMessage(message: AgentMessage): boolean {
  const marker = (message as { __mo?: { beforeAgentRunBlocked?: unknown } })["__mo"]
...
export function preparePersistedUserTurnMessageForTranscriptWrite(
  ...
  const nextUserMessage = provenance
    ...
    : nextMessage;
  const originalOpenClaw = (message as unknown as { __mo?: unknown })["__mo"];
  const senderIsOwner =
  ...
  const nextOpenClaw =
    nextRecord["__mo"] && typeof nextRecord["__mo"] === "object"
      ? (nextRecord["__mo"] as Record<string, unknown>)
      : {};
  return {
    ...nextRecord,
    ...(idempotencyKey ? { idempotencyKey } : {}),
    ...(typeof senderIsOwner === "boolean"
      ? { __mo: { ...nextOpenClaw, senderIsOwner } }
      : {}),
