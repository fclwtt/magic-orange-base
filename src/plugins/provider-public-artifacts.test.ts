...
describe("provider public artifacts", () => {
  ...
  const originalTrustBundledPluginsDir = process.env.MO_TEST_TRUST_BUNDLED_PLUGINS_DIR;
  ...
  afterEach(() => {
    ...
    if (originalTrustBundledPluginsDir === undefined) {
      ...
      process.env.MO_TEST_TRUST_BUNDLED_PLUGINS_DIR = originalTrustBundledPluginsDir;
    }
  ...
  it("resolves multi-provider policy artifacts by manifest-owned provider id", async () => {
    const bundledPluginsDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-provider-policy-"));
        ...
        plugins: [
          {
            ...
            hooks: [],
            origin: "bundled",
            manifestPath: "/tmp/openai/mo.plugin.json",
            providers: ["openai"],
        ...
        plugins: [
          {
            ...
            hooks: [],
            origin: "bundled",
            manifestPath: "/tmp/anthropic/mo.plugin.json",
            providers: ["anthropic"],
  ...
  it("does not cache manifest-owned provider policy aliases across bundled metadata changes", async () => {
    const bundledPluginsDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "mo-provider-policy-refresh-"),
    );
        ...
        plugins: [
          {
            ...
            hooks: [],
            origin: "bundled",
            manifestPath: "/tmp/owner/mo.plugin.json",
            providers: ["alias"],
            rootDir: "/tmp/owner",
            skills: [],
