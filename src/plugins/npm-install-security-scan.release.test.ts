...
const REQUIRED_REVIEWED_PUBLISHABLE_CRITICAL_FINDINGS = new Set([
  "@mo/acpx:dangerous-exec:src/codex-auth-bridge.ts",
  "@mo/acpx:dangerous-exec:src/runtime-internals/mcp-proxy.mjs",
  "@mo/codex:dangerous-exec:src/app-server/sandbox-exec-server/http.ts",
  "@mo/codex:dangerous-exec:src/app-server/sandbox-exec-server/processes.ts",
  "@mo/codex:dangerous-exec:src/app-server/transport-stdio.ts",
  "@mo/codex:dangerous-exec:src/node-cli-sessions.ts",
  "@mo/discord:dangerous-exec:src/voice/audio.ts",
  "@mo/google-meet:dangerous-exec:src/node-host.ts",
  "@mo/google-meet:dangerous-exec:src/realtime.ts",
  "@mo/matrix:dangerous-exec:src/matrix/deps.ts",
  "@mo/raft:dangerous-exec:src/gateway.ts",
  "@mo/signal:dangerous-exec:src/daemon.ts",
  "@mo/voice-call:dangerous-exec:src/tunnel.ts",
  "@mo/voice-call:dangerous-exec:src/webhook/tailscale.ts",
]);

const OPTIONAL_REVIEWED_PUBLISHABLE_DIST_CRITICAL_FINDINGS = new Set([
  "@mo/acpx:dangerous-exec:dist/mcp-proxy.mjs",
  "@mo/acpx:dangerous-exec:dist/service-<hash>.js",
  "@mo/codex:dangerous-exec:dist/client-<hash>.js",
  "@mo/google-meet:dangerous-exec:dist/index.js",
  "@mo/slack:dynamic-code-execution:dist/outbound-payload.test-harness-<hash>.js",
  "@mo/voice-call:dangerous-exec:dist/runtime-entry-<hash>.js",
]);
...
function stageScannerRelevantPackedFiles(
  ...
  const stageDir = mkdtempSync(join(tmpdir(), "mo-plugin-npm-scan-"));
...
function collectPublishablePluginPackages(): PublishablePluginPackage[] {
  return listPublishablePluginPackageDirs()
    .flatMap((packageDir) => {
      ...
      let packageJson: {
        name?: unknown;
        mo?: { release?: { publishToNpm?: unknown } };
      };
      try {
      ...
      if (packageJson.mo?.release?.publishToNpm !== true) {
        return [];
      }
