...
function hasTrustedOpenClawRootIndicator(params: {
  ...
  const hasOpenClawBin =
    (typeof params.packageJson.bin === "string" &&
      normalizeLowercaseStringOrEmpty(params.packageJson.bin).includes("mo")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.mo === "string");
  const hasOpenClawEntrypoint = fs.existsSync(path.join(params.packageRoot, "mo.mjs"));
...
const PLUGIN_SDK_PACKAGE_NAMES = ["mo/plugin-sdk", "@mo/plugin-sdk"] as const;
...
const PRIVATE_PLUGIN_SDK_SUBPATH_OWNERS: readonly PrivatePluginSdkSubpathOwner[] = [
  {
    ...
    officialInstalledPackageName: "@mo/codex",
...
const WORKSPACE_PACKAGE_ALIAS_ENTRIES: WorkspacePackageAliasEntry[] = [
  ...
  {
    packageName: "@mo/gateway-protocol",
  ...
  {
    packageName: "@mo/media-generation-core",
  ...
  {
    packageName: "@mo/normalization-core",
  ...
  {
    packageName: "@mo/model-catalog-core",
...
function readBundledPluginPackageName(packageJsonPath: string): string | null {
  ...
  return name.startsWith("@mo/") ? name : null;
}
...
function shouldIncludePrivateLocalOnlyPluginSdkSubpaths() {
  return process.env.MO_ENABLE_PRIVATE_QA_CLI === "1";
}
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (Object.hasOwn(aliasMap, `mo/plugin-sdk/${subpath}`)) {
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  const result: Record<string, string> = {
    ...(extensionApiAlias
      ? { "mo/extension-api": normalizeJitiAliasTargetPath(extensionApiAlias) }
      : {}),
