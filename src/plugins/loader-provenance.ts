...
function formatPluginInspectCommand(pluginId: string): string {
  return `mo plugins inspect ${quoteCliArg(pluginId)}`;
}
...
export function warnWhenAllowlistIsOpen(params: {
  ...
  const remediation = truncated
    ? "Run 'mo plugins list --enabled --verbose' to enumerate every discovered plugin id, inspect trusted ids with 'mo plugins inspect <id>', and add the ones you trust to plugins.allow in mo.json."
    : `To trust them explicitly, set plugins.allow in mo.json (e.g. "plugins": { "allow": [${autoDiscoverable
        .map((entry) => JSON.stringify(entry.id))
        .join(
          ", ",
        )}] }). Run 'mo plugins list --enabled --verbose' or ${inspectCommands} to confirm plugin ids.`;
