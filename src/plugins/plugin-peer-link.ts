  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === "ENOENT") {
      return {
        packageName,
        packageDir: params.packageDir,
        reason: `missing ${path.join(nodeModulesDir, "mo")}`,
      };
    }
    throw error;
  }

  const linkPath = path.join(nodeModulesDir, "mo");
...
export async function auditOpenClawPeerDependencyLink(params: {
  ...
  if (!hostRoot) {
    return {
      packageName,
      packageDir: params.packageDir,
      reason: "could not locate mo package root",
    };
  }
...
async function ensureRealNodeModulesDir(params: {
  ...
  try {
    ...
    if (!existing.isDirectory() || existing.isSymbolicLink()) {
      params.logger.warn?.(
        `Skipping mo peerDependency link because ${nodeModulesDir} is not a real directory.`,
      );
      return null;
    }
  ...
  try {
    ...
    if (existing) {
      if (!existing.isSymbolicLink()) {
        if (params.peerName === "mo" && existing.isDirectory()) {
          const existingPackageName = await readPackageName(linkPath);
          if (existingPackageName === "mo") {
        ...
        params.logger.warn?.(
          `Skipping mo peerDependency link because ${linkPath} already exists and is not a symlink.`,
        );
        return "skipped";
      }
...
/**
 * Symlink the host mo package for plugins that declare it as a peer.
...
export async function linkOpenClawPeerDependencies(params: {
  ...
  const peers = Object.keys(params.peerDependencies).filter((name) => name === "mo");
  if (peers.length === 0) {
  ...
  if (!hostRoot) {
    params.logger.warn?.(
      "Could not locate mo package root to symlink peerDependencies; plugin may fail to resolve mo at runtime.",
    );
...
export async function relinkOpenClawPeerDependenciesInManagedNpmRoot(params: {
  ...
  for (const packageDir of await listManagedNpmRootPackageDirs(params.npmRoot)) {
    ...
    if (!Object.hasOwn(peerDependencies, "mo")) {
      continue;
    }
    checked += 1;
