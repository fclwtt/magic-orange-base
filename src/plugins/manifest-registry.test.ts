...
function makeTempDir() {
  return makeTrackedTempDir("mo-manifest-registry", tempDirs);
}
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "mo.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "mo.plugin.json");
  const linkedManifest = path.join(rootDir, "mo.plugin.json");
...
describe("loadPluginManifestRegistry", () => {
  ...
  it("prefers dev source bundled plugins over installed globals with the same id", () => {
    ...
    const registry = loadPluginManifestRegistry({
      env: hermeticEnv({ MO_DEV_SOURCE_ROOT: devSourceRoot }),
      installRecords: {
        codex: {
          source: "npm",
          installPath: globalDir,
    ...
    const registry = loadPluginManifestRegistry({
      installRecords: {
        "diagnostics-prometheus": {
          source: "npm",
          installPath: dir,
          resolvedName: "@mo/diagnostics-prometheus",
      ...
      candidates: [
        createPluginCandidate({
          ...
          rootDir: dir,
          packageName: "@mo/diagnostics-prometheus",
          origin: "global",
        }),
      ],
    });
  ...
  it("marks official diagnostics-otel config paths trusted when the install record matches", () => {
    ...
    const registry = loadPluginManifestRegistry({
      installRecords: {
        "diagnostics-otel": {
          source: "npm",
          spec: "@mo/diagnostics-otel",
          installPath: dir,
          resolvedName: "@mo/diagnostics-otel",
          resolvedVersion: "2026.5.18",
          resolvedSpec: "@mo/diagnostics-otel@2026.5.18",
        },
      },
      candidates: [
        createPluginCandidate({
          ...
          rootDir: dir,
          packageName: "@mo/diagnostics-otel",
          origin: "config",
        }),
      ],
    });
  ...
  it("reports non-bundled providerAuthEnvVars as deprecated compat metadata", () => {
    ...
    expectDiagnosticFields(registry, {
      ...
      source: path.join(dir, "mo.plugin.json"),
  ...
  it("hydrates supplemental official external catalog contracts for lagging npm manifests", () => {
    ...
    const registry = loadRegistry([
      createPluginCandidate({
        idHint: "wecom-mo-plugin",
        rootDir: dir,
        origin: "global",
        packageName: "@wecom/wecom-mo-plugin",
      }),
    ]);
  ...
  it("ignores provider catalog entries outside the plugin root", () => {
    ...
    expectDiagnosticFields(registry, {
      ...
      source: path.join(pluginDir, "mo.plugin.json"),
  ...
  it("preserves host-trusted plugin contracts from plugin manifests", () => {
    ...
    writeManifest(dir, {
      id: "workflow-harness",
      contracts: {
        agentToolResultMiddleware: ["mo", "codex"],
  ...
  it("hydrates bundled channel config metadata onto manifest records", () => {
    ...
    const registry = loadRegistry([
      createPluginCandidate({
        ...
        rootDir: dir,
        origin: "bundled",
        bundledManifestPath: path.join(dir, "mo.plugin.json"),
        bundledManifest: {
          id: "telegram",
          configSchema: { type: "object" },
          channels: ["telegram"],
  ...
  it.each([
    {
      ...
      env: { MO_VERSION: "2026.3.21" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      env: { MO_VERSION: "2026.4.30" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      minHostVersion: "2026.3.22",
      expectedMessage: "plugin manifest invalid | mo.install.minHostVersion must use",
      expectWarn: false,
    },
    {
      name: "warns distinctly when host version cannot be determined",
      minHostVersion: ">=2026.3.22",
      env: { MO_VERSION: "unknown" } as NodeJS.ProcessEnv,
  ...
  it("accepts legacy bare minHostVersion metadata for recorded installed globals", () => {
    ...
    expectNoRegistryDiagnosticContains(registry, "mo.install.minHostVersion must use");
  });
  ...
  it("skips installed plugins whose package plugin API range is newer than the current host", () => {
    ...
    const registry = loadRegistryForPluginApiCase({
      rootDir: dir,
      pluginApi: ">=2026.5.27",
      env: { MO_VERSION: "2026.5.10-beta.1" } as NodeJS.ProcessEnv,
    });
  ...
  it("skips installed plugins whose package plugin API metadata is malformed", () => {
    ...
    const registry = loadRegistryForPluginApiCase({
      rootDir: dir,
      pluginApi: 20260527,
      env: { MO_VERSION: "2026.5.27" } as NodeJS.ProcessEnv,
    });

    expect(registry.plugins).toStrictEqual([]);
    expectRegistryDiagnosticContains(
      registry,
      "plugin manifest invalid | package.json mo.compat.pluginApi must be a string",
    );
  ...
  it("loads installed plugins when a beta host is on the package plugin API floor", () => {
    ...
    const registry = loadRegistryForPluginApiCase({
      rootDir: dir,
      pluginApi: ">=2026.5.27",
      env: { MO_VERSION: "2026.5.27-beta.1" } as NodeJS.ProcessEnv,
    });
  ...
  it("suppresses duplicate warning when global candidates come from the same package artifact", () => {
    ...
    const manifest = { id: "opik-mo", configSchema: { type: "object" } };
  ...
  it("resolves load-path manifests from the current env home", () => {
    ...
    const first = loadPluginManifestRegistry({
      ...
      env: hermeticEnv({
        HOME: homeA,
        MO_HOME: undefined,
        MO_STATE_DIR: path.join(homeA, ".state"),
      }),
    });
    const second = loadPluginManifestRegistry({
      ...
      env: hermeticEnv({
        HOME: homeB,
        MO_HOME: undefined,
        MO_STATE_DIR: path.join(homeB, ".state"),
      }),
    });
