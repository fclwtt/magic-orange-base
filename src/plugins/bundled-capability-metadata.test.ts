  ...
  return listExtensionPackagePaths(extensionsDir)
    .filter((packagePath) => {
      const packageJson = JSON.parse(fs.readFileSync(packagePath, "utf-8")) as {
        mo?: OpenClawPackageManifest;
      };
      return normalizeBundledPluginStringList(packageJson.mo?.extensions).length > 0;
    })
    .map(
      (packagePath) =>
        JSON.parse(
          fs.readFileSync(path.join(path.dirname(packagePath), "mo.plugin.json"), "utf-8"),
        ) as PluginManifest,
    )
