...
const suiteTempRootTracker = createSuiteTempRootTracker("mo-plugin-install-npm-spec");
...
function writeInstalledNpmPlugin(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      mo: params.mo ?? { extensions: ["./dist/index.js"] },
      ...(params.dependency
  ...
  fs.writeFileSync(
    path.join(pluginDir, "mo.plugin.json"),
    JSON.stringify({
...
function prunePluginLocalOpenClawPeerLinks(npmRoot: string) {
  ...
  for (const entry of fs.readdirSync(nodeModulesDir, { withFileTypes: true })) {
    ...
    for (const packageDir of packageDirs) {
      ...
      fs.rmSync(path.join(packageNodeModulesDir, "mo"), {
        recursive: true,
        force: true,
      });
    }
    ...
    async (argv: string[], options?: { cwd?: string }) => {
      ...
      if (viewPackage) {
        return successfulSpawn(
          JSON.stringify({
            ...
            ...(viewPackage.mo ? { mo: viewPackage.mo } : {}),
          }),
        );
      }
      ...
      if (isManagedNpmInstallCommand(argv)) {
        ...
        for (const packageName of Object.keys(manifest.dependencies ?? {})) {
          if (packageName === "mo") {
            const moRoot = path.join(npmRoot, "node_modules", "mo");
            fs.mkdirSync(moRoot, { recursive: true });
            fs.writeFileSync(
              path.join(moRoot, "package.json"),
              JSON.stringify({ name: "mo", version: "0.0.0-test" }),
              "utf8",
            );
            continue;
          }
          ...
          if (pkg.materializesRootOpenClaw) {
            ...
            fs.writeFileSync(
              path.join(moRoot, "package.json"),
              JSON.stringify({ name: "mo", version: "2026.5.3" }),
              "utf8",
            );
          }
      ...
      if (argv[0] === "npm" && argv[1] === "uninstall") {
        ...
        if (packageName === "mo") {
          ...
          fs.rmSync(path.join(npmRoot, "node_modules", "mo"), {
            recursive: true,
            force: true,
          });
          return successfulSpawn();
...
beforeEach(() => {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: "mo", version: "0.0.0-test" }, null, 2)}\n`,
    "utf8",
  );
...
beforeAll(async () => {
  ...
  const archivePath = path.join(stateDir, "mo-pack-demo-1.2.3.tgz");
  fs.writeFileSync(archivePath, "fixture pack contents", "utf8");

  mockNpmViewAndInstallMany([
    {
      packageName: "@mo/pack-demo",
      version: "1.2.3",
      pluginId: "pack-demo",
      npmRoot,
    ...
    {
      spec: "@mo/voice-call@0.0.1",
      packageName: "@mo/voice-call",
      version: "0.0.1",
      pluginId: "voice-call",
      npmRoot,
    },
  ...
  const dependencySpec = managedManifest.dependencies?.["@mo/pack-demo"];
...
describe("installPluginFromNpmSpec", () => {
  it("installs npm pack archives through the managed npm root", async () => {
    ...
    expect(result.targetDir).toBe(resolveTestPluginPackageDir(npmRoot, "@mo/pack-demo"));
    expect(result.npmResolution?.resolvedSpec).toBe("@mo/pack-demo@1.2.3");
    expect(result.npmResolution?.integrity).toBe("sha512-pack-demo");
    expect(result.npmTarballName).toBe("mo-pack-demo-1.2.3.tgz");
    expectNpmInstallIntoProject({
      calls,
      npmRoot,
      packageName: "@mo/pack-demo",
    });
    expect(dependencySpec).toMatch(/^file:\.\/_mo-pack-archives\/.+\.tgz$/);
  ...
  it("rejects npm pack archive metadata with traversal package names", async () => {
    ...
    expect(fs.existsSync(path.join(npmRoot, "_mo-pack-archives"))).toBe(false);
  ...
  it("updates staged npm pack archives when dangerous-looking code is present", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@mo/pack-demo";
    const archiveV1Path = path.join(stateDir, "mo-pack-demo-1.0.0.tgz");
    const archiveV2Path = path.join(stateDir, "mo-pack-demo-2.0.0.tgz");
  ...
  it("installs staged npm pack archives with dangerous-looking code", async () => {
    ...
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@mo/pack-demo";
    const archivePath = path.join(stateDir, "mo-pack-demo-2.0.0.tgz");
  ...
  it("installs npm plugins into .mo/npm", async () => {
    ...
    expect(result.targetDir).toBe(resolveTestPluginPackageDir(npmRoot, "@mo/voice-call"));
    expect(result.npmResolution?.resolvedSpec).toBe("@mo/voice-call@0.0.1");
  ...
  it("repairs omitted current-platform packages with a fresh npm cache", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@mo/codex-fixture";
    ...
    expect(repairCacheDir).toContain("mo-npm-cache-");
  ...
  it("quarantines and rebuilds a corrupt managed npm project after npm from-argument failures", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@mo/voice-call";
    const warnings: string[] = [];
    const npmProjectRoot = resolvePluginNpmProjectDir({ npmDir: npmRoot, packageName });
    ...
    const quarantineParent = path.join(npmProjectRoot, "_mo-quarantined-npm-projects");
  ...
  it.runIf(process.platform !== "win32")(
    "does not let managed mo peer links poison later npm installs",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      expect(
        fs.lstatSync(path.join(peerPluginDir, "node_modules", "mo")).isSymbolicLink(),
      ).toBe(true);
      ...
      if (!second.ok) {
        expect(second.error).not.toContain("peer-plugin/node_modules/mo");
      }
      expect(
        fs.lstatSync(path.join(peerPluginDir, "node_modules", "mo")).isSymbolicLink(),
      ).toBe(true);
  ...
  it("rejects managed npm plugins when their mo peer link cannot be repaired", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const warnings: string[] = [];
    ...
    mockNpmViewAndInstall({
      spec: "@mo/codex@2026.5.7",
      packageName: "@mo/codex",
      version: "2026.5.7",
      pluginId: "@mo/codex",
      npmRoot,
      peerDependencies: { mo: ">=2026.5.7" },
    });
    ...
    expect(result.error).toContain("@mo/codex");
    expect(result.error).toContain("plugin-local node_modules/mo link");
    expect(
      warnings.some((warning) => warning.includes("Could not locate mo package root")),
    ).toBe(true);
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@mo/codex"))).toBe(false);
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@mo/codex",
    });
  ...
  it("rejects exact npm plugins whose package compatibility requires a newer host", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    vi.stubEnv("MO_COMPATIBILITY_HOST_VERSION", "2026.5.10-beta.1");

    mockNpmViewAndInstall({
      spec: "@mo/whatsapp@2026.5.27",
      packageName: "@mo/whatsapp",
      version: "2026.5.27",
      pluginId: "whatsapp",
      npmRoot,
      peerDependencies: { mo: ">=2026.5.27" },
      mo: {
        extensions: ["./dist/index.js"],
    ...
    expect(fs.existsSync(path.join(npmRoot, "node_modules", "@mo", "whatsapp"))).toBe(false);
    expect(fs.existsSync(path.join(npmRoot, "package.json"))).toBe(false);
    expect(
  ...
  it("installs the newest compatible npm version for unpinned plugins", async () => {
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@mo/whatsapp@2026.5.26");
    expect(result.npmResolution?.version).toBe("2026.5.26");
    expect(warnings.join("\n")).toContain("using newest compatible @mo/whatsapp@2026.5.26");
    expect(
      JSON.parse(
        fs.readFileSync(
          path.join(resolveTestPluginPackageDir(npmRoot, "@mo/whatsapp"), "package.json"),
          "utf8",
        ),
      ).version,
    ).toBe("2026.5.26");
  ...
  it("preserves an existing npm plugin by resolving update metadata to a compatible version", async () => {
    ...
    expect(managedManifest.dependencies?.["@mo/whatsapp"]).toBe("2026.5.26");
    expect(
      runCommandWithTimeoutMock.mock.calls.some(([argv]) => isManagedNpmInstallCommand(argv)),
    ).toBe(true);
  });
  ...
  it("resolves incompatible prerelease tags to a compatible prerelease version", async () => {
    ...
    mockNpmViewAndInstallMany([
      {
        spec: "@mo/msteams@beta",
        packageName: "@mo/msteams",
      ...
      {
        spec: "@mo/msteams@2026.5.28-beta.3",
        packageName: "@mo/msteams",
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@mo/msteams@2026.5.28-beta.3");
    expect(result.npmResolution?.version).toBe("2026.5.28-beta.3");
    expect(warnings.join("\n")).toContain(
      "using newest compatible @mo/msteams@2026.5.28-beta.3",
    );
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@mo/msteams",
    ...
    expect(managedManifest.dependencies?.["@mo/msteams"]).toBe("2026.5.28-beta.3");
  });
  ...
  it.runIf(process.platform !== "win32")(
    "repairs root mo materialized by npm peer handling",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      expect(fs.existsSync(path.join(npmProjectRoot, "node_modules", "mo"))).toBe(false);
      const lockfile = JSON.parse(
        fs.readFileSync(path.join(npmProjectRoot, "package-lock.json"), "utf8"),
      ) as {
        packages?: Record<string, unknown>;
      };
      expect(lockfile.packages?.["node_modules/mo"]).toBeUndefined();
      expect(
        fs.lstatSync(path.join(requiredPeerPluginDir, "node_modules", "mo")).isSymbolicLink(),
      ).toBe(true);
    },
  );

  it("repairs stale managed mo root packages before npm plugin installs", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@mo/discord",
    });
    fs.mkdirSync(path.join(npmProjectRoot, "node_modules", "mo"), { recursive: true });
    fs.writeFileSync(
      path.join(npmProjectRoot, "package.json"),
      JSON.stringify(
        {
      ...
      `${JSON.stringify(
        {
          ...
          packages: {
            ...
            "node_modules/mo": {
              version: "2026.5.4",
              resolved: "https://registry.npmjs.org/mo/-/mo-2026.5.4.tgz",
            },
          },
          dependencies: {
            mo: {
    ...
    fs.writeFileSync(
      path.join(npmProjectRoot, "node_modules", "mo", "package.json"),
      JSON.stringify({
        name: "mo",
        version: "2026.5.4",
      }),
    ...
    mockNpmViewAndInstall({
      spec: "@mo/discord@beta",
      packageName: "@mo/discord",
      version: "2026.5.5-beta.1",
      pluginId: "discord",
      npmRoot,
      peerDependencies: { mo: ">=2026.5.5-beta.1" },
    ...
    expect(manifest.dependencies).not.toHaveProperty("mo");
    expect(manifest.dependencies?.["@mo/discord"]).toBe("2026.5.5-beta.1");
    const lockfile = JSON.parse(
      fs.readFileSync(path.join(npmProjectRoot, "package-lock.json"), "utf8"),
    ) as {
      packages?: Record<string, unknown>;
    ...
    expect(lockfile.packages?.["node_modules/mo"]).toBeUndefined();
    expect(lockfile.dependencies?.mo).toBeUndefined();
  });

  it("preserves the active host mo runtime package during npm plugin installs", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const hostPackageRoot = path.join(npmRoot, "node_modules", "mo");
    ...
    mockNpmViewAndInstall({
      spec: "@xdarkicex/mo-memory-libravdb@1.4.69",
      packageName: "@xdarkicex/mo-memory-libravdb",
      version: "1.4.69",
    ...
    expect(baseManifest.dependencies?.mo).toBe("2026.5.12-beta.6");
    expect(baseManifest.dependencies?.["@xdarkicex/mo-memory-libravdb"]).toBeUndefined();
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@xdarkicex/mo-memory-libravdb",
    });
    ...
    expect(projectManifest.dependencies?.["@xdarkicex/mo-memory-libravdb"]).toBe("1.4.69");
    expect(fs.existsSync(hostPackageRoot)).toBe(true);
    expect(result.targetDir).toBe(
      resolveTestPluginPackageDir(npmRoot, "@xdarkicex/mo-memory-libravdb"),
    );
    expect(
  ...
  it("rolls back the managed npm root when npm install fails", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (JSON.stringify(argv) === JSON.stringify(npmViewArgv("@mo/voice-call@0.0.1"))) {
          return successfulSpawn(
            JSON.stringify({
              name: "@mo/voice-call",
              version: "0.0.1",
        ...
        if (argv[0] === "npm" && argv[1] === "uninstall") {
          if (!(argv as string[]).includes("--legacy-peer-deps")) {
            fs.mkdirSync(path.join(options?.cwd ?? npmRoot, "node_modules", "mo"), {
              recursive: true,
            });
          }
          return successfulSpawn("");
    ...
    await expect(
      fs.promises.access(path.join(npmProjectRoot, "node_modules", "mo")),
    ).rejects.toHaveProperty("code", "ENOENT");
  });

  it("does not fail rollback snapshots on plugin-local mo peer symlinks", async () => {
    const npmRoot = path.join(suiteTempRootTracker.makeTempDir(), "npm");
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@mo/codex",
    ...
    const peerLink = path.join(installedDir, "node_modules", "mo");
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (JSON.stringify(argv) === JSON.stringify(npmViewArgv("@mo/codex@0.0.2"))) {
          return successfulSpawn(
            JSON.stringify({
              name: "@mo/codex",
              version: "0.0.2",
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (isManagedNpmInstallCommand(argv)) {
          ...
          const manifest = JSON.parse(
            fs.readFileSync(path.join(npmProjectRoot, "package.json"), "utf8"),
          ) as { overrides?: Record<string, unknown>; mo?: { managedOverrides?: string[] } };
          if (installAttempts === 1) {
            expect(manifest.overrides?.["node-domexception"]).toBe(
              "npm:@nolyfill/domexception@1.0.28",
            );
            expect(manifest.mo?.managedOverrides).toEqual([
              "axios",
              "nested",
              "node-domexception",
            ]);
          ...
          expect(manifest.mo?.managedOverrides).toEqual(["axios", "nested"]);
        }
  ...
  it("rejects duplicate npm installs unless update mode is requested", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const installRoot = resolveTestPluginPackageDir(npmRoot, "@mo/voice-call");
  ...
  it("preserves previously installed sibling plugins during npm install", async () => {
    ...
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@mo/voice-call"))).toBe(true);
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@mo/whatsapp"))).toBe(true);
  });
  ...
  it("rejects implicit prerelease npm specs with beta guidance", async () => {
    ...
    if (!rejected.ok) {
      ...
      expect(rejected.error).toContain('"@mo/voice-call@beta"');
    }
  });
  ...
  it("falls back to the latest stable version for official prerelease packages", async () => {
    ...
    expect(officialFallback.npmResolution?.resolvedSpec).toBe("@mo/voice-call@0.0.1");
    expect(warnings.join("\n")).toContain("falling back to stable @mo/voice-call@0.0.1");
  });
  ...
  it("keeps stable correction versions when resolving official npm packages", async () => {
    ...
    expect(stableCorrection.npmResolution?.resolvedSpec).toBe("@mo/voice-call@2026.5.3-1");
  ...
  it("uses the newest prerelease when an official package has no stable versions", async () => {
    ...
    expect(prereleaseOnly.npmResolution?.resolvedSpec).toBe("@mo/voice-call@0.0.2-beta.1");
    expect(prereleaseOnlyWarnings.join("\n")).toContain("has no stable npm versions yet");
    expect(prereleaseOnlyWarnings.join("\n")).toContain(
      "using newest prerelease @mo/voice-call@0.0.2-beta.1",
    );
  });
  ...
  it("accepts explicit prerelease npm dist-tags", async () => {
    ...
    expect(accepted.npmResolution?.resolvedSpec).toBe("@mo/voice-call@0.0.2-beta.1");
    expectNpmInstallIntoProject({
      calls: runCommandWithTimeoutMock.mock.calls,
      npmRoot,
      packageName: "@mo/voice-call",
