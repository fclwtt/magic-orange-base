...
async function listMarketplaceDownloadTempDirs(): Promise<string[]> {
  ...
  return entries
    .filter(
      (entry) => entry.isDirectory() && entry.name.startsWith("mo-marketplace-download-"),
    )
...
describe("marketplace plugins", () => {
  ...
  it("lists plugins from a local marketplace root", async () => {
    await withTempDir("mo-marketplace-test-", async (rootDir) => {
  ...
  it("resolves Claude-style plugin@marketplace shortcuts from known_marketplaces.json", async () => {
    await withTempDir("mo-marketplace-test-", async (homeDir) => {
      const openClawHome = path.join(homeDir, "mo-home");
      ...
      const shortcut = await withEnvAsync(
        { HOME: homeDir, MO_HOME: openClawHome },
