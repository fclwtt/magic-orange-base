...
const DISABLED_BUNDLED_PLUGINS_DIR = path.join(os.tmpdir(), "mo-empty-bundled-plugins");
const TEST_TRUST_BUNDLED_PLUGINS_DIR_ENV = "MO_TEST_TRUST_BUNDLED_PLUGINS_DIR";
...
export function areBundledPluginsDisabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = normalizeOptionalLowercaseString(env.MO_DISABLE_BUNDLED_PLUGINS);
  ...
  try {
    return fs.readdirSync(pluginsDir, { withFileTypes: true }).some((entry) => {
      ...
      return (
        ...
        fs.existsSync(path.join(pluginDir, "mo.plugin.json"))
      );
    });
  } catch {
    return false;
...
function createBundledPluginsDirCacheKey(env: NodeJS.ProcessEnv): string {
  return JSON.stringify({
    disabled: env.MO_DISABLE_BUNDLED_PLUGINS ?? "",
    override: env.MO_BUNDLED_PLUGINS_DIR ?? "",
...
function resolveBundledPluginsDirUncached(env: NodeJS.ProcessEnv): string | undefined {
  ...
  const override = env.MO_BUNDLED_PLUGINS_DIR?.trim();
