...
async function makeTempDir(label: string): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), `mo-${label}-`));
  tempDirs.push(dir);
  return dir;
}
...
async function packPlugin(params: {
  ...
  await fs.writeFile(
    path.join(packageDir, "mo.plugin.json"),
    `${JSON.stringify(
      {
        id: params.pluginId,
...
describe("installPluginFromNpmSpec e2e", () => {
  it("installs the newest compatible stable package when npm latest requires a newer plugin API", async () => {
    ...
    const versions = [
      ...
      await packPlugin({
        ...
        version: "2026.5.27",
        rootDir,
        mo: incompatibleOpenClaw,
      }),
    ];
    const registry = await startStaticRegistry([{ packageName, latest: "2026.5.27", versions }]);
    process.env.NPM_CONFIG_REGISTRY = registry;
    process.env.npm_config_registry = registry;
    const previousHostVersion = process.env.MO_COMPATIBILITY_HOST_VERSION;
    process.env.MO_COMPATIBILITY_HOST_VERSION = "2026.5.10-beta.1";
    ...
    try {
      ...
      if (previousHostVersion === undefined) {
        ...
        process.env.MO_COMPATIBILITY_HOST_VERSION = previousHostVersion;
      }
    }
  });

  it("scrubs root mo materialized by required npm peers", async () => {
    ...
    const registry = await startStaticRegistry([
      ...
      { packageName: "mo", latest: "2026.0.0", versions: openClawVersions },
    ]);
    process.env.NPM_CONFIG_REGISTRY = registry;
    process.env.npm_config_registry = registry;
    ...
    const rawOpenClawLockEntry = rawLock.packages?.["node_modules/mo"] as
    ...
    expect(lock.packages?.["node_modules/mo"]).toBeUndefined();
    await expect(
      fs.lstat(path.join(projectRoot, "node_modules", "mo")),
    ).rejects.toHaveProperty("code", "ENOENT");
    await expect(
      fs
        .lstat(path.join(result.targetDir, "node_modules", "mo"))
  ...
  it("plans peers from installed optional dependencies", async () => {
    ...
    const rootManifest = JSON.parse(
      ...
      dependencies?: Record<string, string>;
      mo?: { managedPeerDependencies?: string[] };
    };
    expect(["1.0.0", "^1.0.0"]).toContain(rootManifest.dependencies?.[runtimePeer]);
    expect(rootManifest.mo?.managedPeerDependencies ?? []).toContain(runtimePeer);
  });
  ...
  it("leaves legacy flat-root peer dependencies alone during isolated later installs", async () => {
    ...
    expect(rootManifest.mo?.managedPeerDependencies ?? []).not.toContain(runtimePeer);
  });
  ...
  it("falls back to the legacy npm peer mode inside the plugin project when npm cannot plan third-party peers", async () => {
    ...
    expect(rootManifest.mo?.managedPeerDependencies ?? []).not.toContain(missingPeer);
    await expect(
      fs.lstat(path.join(projectRoot, "node_modules", blockedPlugin, "package.json")),
    ).resolves.toBeTruthy();
  });
  ...
  it("does not take ownership of an existing root dependency observed as a peer", async () => {
    ...
    expect(rootManifest.dependencies?.[blockedPlugin]).toBeUndefined();
    expect(rootManifest.dependencies?.[runtimePeer]).toBeUndefined();
    expect(rootManifest.mo?.managedPeerDependencies ?? []).not.toContain(
  ...
  it("scrubs host peers inside each isolated npm project", async () => {
    ...
    const registry = await startStaticRegistry([
      {
        ...
        versions: [
          await packPlugin({
            packageName: codexName,
            peerDependencies: { mo: ">=2026.5.5-beta.2" },
            peerDependenciesMeta: { mo: { optional: true } },
            pluginId: codexName,
            version: "1.0.0",
            rootDir,
          }),
    ...
    await expect(
      fs
        .lstat(path.join(first.targetDir, "node_modules", "mo"))
        .then((stat) => stat.isSymbolicLink()),
    ).resolves.toBe(true);
    await expect(
      fs
        .lstat(path.join(second.targetDir, "node_modules", "mo"))
        .then((stat) => stat.isSymbolicLink()),
    ).resolves.toBe(true);
  });

  it("keeps an earlier isolated mo peer link after later plugin installs", async () => {
    ...
    const peerLink = path.join(first.targetDir, "node_modules", "mo");
    ...
    expect(manifest.dependencies?.mo).toBeUndefined();
    const lock = JSON.parse(
