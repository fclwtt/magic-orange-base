...
import { normalizeModelCatalog } from "@mo/model-catalog-core/model-catalog-normalize";
import { normalizeModelCatalogProviderId } from "@mo/model-catalog-core/model-catalog-refs";
import type { ModelCatalog } from "@mo/model-catalog-core/model-catalog-types";
...
export const PLUGIN_MANIFEST_FILENAME = "mo.plugin.json";
...
export type PluginManifestQaRunner = {
  /** Subcommand mounted beneath `mo qa`, for example `matrix`. */
  commandName: string;
...
// package.json "mo" metadata (used for setup/catalog)
...
export function resolvePackageExtensionEntries(
  ...
  if (!isRecord(rawOpenClaw)) {
    return {
      status: "invalid",
      entries: [],
      error: "package.json mo must be an object",
    };
  }
  ...
  if (!Array.isArray(raw)) {
    return {
      status: "invalid",
      entries: [],
      error: "package.json mo.extensions must be an array",
    };
  }
  ...
  for (const [index, entry] of raw.entries()) {
    ...
    if (!normalized) {
      return {
        status: "invalid",
        entries: [],
        error: `package.json mo.extensions[${index}] must be a non-empty string`,
      };
    }
