...
function formatUnresolvedOpenClawPeerLinkError(packageName: string): string {
  return `Installed plugin ${packageName} declares mo as a peer dependency, but OpenClaw could not create a plugin-local node_modules/mo link. Run from a packaged OpenClaw install or reinstall OpenClaw, then retry.`;
}
...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing mo.extensions; update the plugin package to include mo.extensions (for example ["./dist/index.js"]). See https://docs.mo.ai/help/troubleshooting#plugin-install-fails-with-missing-mo-extensions';
...
const MANAGED_NPM_PACK_ARCHIVE_DIR = "_mo-pack-archives";
const MANAGED_NPM_PROJECT_QUARANTINE_DIR = "_mo-quarantined-npm-projects";
...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_MO_EXTENSIONS: "missing_mo_extensions",
  MISSING_PLUGIN_MANIFEST: "missing_plugin_manifest",
  EMPTY_MO_EXTENSIONS: "empty_mo_extensions",
  INVALID_MO_EXTENSIONS: "invalid_mo_extensions",
...
function validateOpenClawPackageCompatibility(params: {
  ...
  if (!pluginApiRangeCheck.ok) {
    return {
      ok: false,
      error: `invalid package.json mo.compat.pluginApi: ${pluginApiRangeCheck.error}`,
...
function validateOpenClawPackageInstallCompatibility(params: {
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json mo.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${params.pluginId}" requires OpenClaw >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set MO_VERSION and retry.`,
...
function ensureOpenClawExtensions(params: { manifest: PackageManifest }):
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_MO_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json mo.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_MO_EXTENSIONS,
    };
  }
  if (resolved.status === "invalid") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.INVALID_MO_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function resolveTrustedOfficialPrereleaseResolution(params: {
  ...
  if (!params.spec.name.startsWith("@mo/")) {
    return null;
  }
...
async function rollbackManagedNpmPluginInstall(params: {
  ...
  if (params.packageName !== "mo") {
    try {
      ...
      params.logger.warn?.(
        `Failed to repair managed npm mo peer after rollback: ${String(error)}`,
      );
    }
  }
  try {
...
async function createManagedNpmPluginInstallRollbackSnapshot(params: {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-npm-plugin-rollback-"));
...
async function listManagedNpmRootPackageNames(npmRoot: string): Promise<Set<string>> {
  ...
  for (const entry of entries.toSorted((left, right) => left.name.localeCompare(right.name))) {
    if (entry.name === ".bin" || entry.name === "mo") {
      continue;
    }
...
function resolveRequiredPlatformPackageNames(
  ...
  if (!Array.isArray(raw)) {
    return {
      ok: false,
      error: "package.json mo.install.requiredPlatformPackages must be an array",
    };
  }
  const packageNames = new Set<string>();
  for (const value of raw) {
    if (typeof value !== "string") {
      return {
        ok: false,
        error:
          "package.json mo.install.requiredPlatformPackages must contain only npm package names",
      };
    }
    ...
    if (specError || !parsed || parsed.selectorKind !== "none") {
      return {
        ok: false,
        error: `package.json mo.install.requiredPlatformPackages contains invalid package name: ${value}`,
      };
    }
  ...
  const runManagedNpmInstall = async (
    ...
    if (params.packageName !== "mo") {
      ...
      if (repairedOpenClawPeer) {
        logger.info?.(`Repaired stale mo peer dependency in ${npmRoot}`);
      }
    }
    ...
    if (omittedPlatformPackages.length > 0) {
      ...
      try {
        freshCacheDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-npm-cache-"));
    ...
    if (params.packageName !== "mo") {
      ...
      if (repairedOpenClawPeer) {
        logger.info?.(`Repaired stale mo peer dependency in ${npmRoot} after npm install`);
      }
    }
    try {
      await relinkOpenClawPeerDependenciesInManagedNpmRoot({
      ...
      return await rollbackFailedManagedNpmInstall({
        ok: false,
        error: `Failed to repair mo peer links after npm install: ${String(error)}`,
      });
    }
...
> {
  ...
  try {
    ...
    backupTempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-npm-pack-archive-"));
...
async function runInstallSourceScan(params: {
  ...
  try {
    ...
    return {
      ok: false,
      error: `${params.subject} installation blocked: code safety scan failed (${String(err)}). Run "mo security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
}
...
> {
  ...
  if (!ocManifestResult.ok && params.requirePluginManifest) {
    return {
      ok: false,
      error: `package missing valid mo.plugin.json: ${ocManifestResult.error}`,
...
export async function installPluginFromNpmSpec(
  ...
  const policyTempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-npm-policy-"));
  try {
...
export async function installPluginFromPath(
  ...
  return {
    ...
    error:
      "Plain file plugin installs are not supported. Install a plugin directory or archive that contains mo.plugin.json, or list standalone plugin files in plugins.load.paths.",
  };
}
