...
async function repairOpenClawPeerLinksForNpmInstalls(params: {
  ...
  for (const [pluginId, record] of Object.entries(params.config.plugins?.installs ?? {})) {
    ...
    try {
      ...
      params.logger.warn?.(
        `Could not repair mo peer link for "${pluginId}" due to invalid install path: ${String(err)}`,
      );
      continue;
    }
    ...
    try {
      ...
      if (peerLinkRepair.skipped > 0) {
        params.logger.warn?.(
          `Could not repair mo peer link for "${pluginId}" at ${installPath}: ${warnings.join("; ") || "peer link repair was skipped"}`,
        );
        continue;
      }
      ...
      params.logger.warn?.(
        `Could not repair mo peer link for "${pluginId}" at ${installPath}: ${String(err)}`,
      );
    }
  }
  return repaired;
        ...
        const message =
          newerExactPinnedDefaultLine && effectiveSpec
            ? `${pluginId} is pinned to ${effectiveSpec} (installed ${currentLabel}); ` +
              ...
              `Pass \`mo plugins update ${newerExactPinnedDefaultLine.packageName}@latest\` to follow the registry default line.` +
