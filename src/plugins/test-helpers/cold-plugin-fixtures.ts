...
export function createColdPluginFixture(options: ColdPluginFixtureOptions): ColdPluginFixture {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        name: options.packageName ?? "@example/mo-cold-control-plane",
  ...
  fs.writeFileSync(
    path.join(options.rootDir, "mo.plugin.json"),
    JSON.stringify(
      {
        id: pluginId,
...
export function createColdPluginHermeticEnv(
  ...
  return {
    ...process.env,
    MO_HOME: path.join(homeDir, "home"),
    MO_BUNDLED_PLUGINS_DIR: options.bundledPluginsDir,
