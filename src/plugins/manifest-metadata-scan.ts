...
import { normalizeOptionalString as normalizeTrimmedString } from "@mo/normalization-core/string-coerce";
...
const PLUGIN_MANIFEST_FILENAME = "mo.plugin.json";
...
function resolveUserPath(value: string, env: NodeJS.ProcessEnv): string {
  if (value === "~" || value.startsWith("~/")) {
    const home = env.MO_HOME ?? env.HOME ?? env.USERPROFILE ?? os.homedir();
...
function resolveStateDir(env: NodeJS.ProcessEnv): string {
  const override = normalizeTrimmedString(env.MO_STATE_DIR);
  if (override) {
