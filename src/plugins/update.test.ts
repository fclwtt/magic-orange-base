...
import { bundledPluginRootAt } from "mo/plugin-sdk/test-fixtures";
...
function createSuccessfulNpmUpdateResult(params?: {
  ...
  return {
    ok: true,
    pluginId: params?.pluginId ?? "opik-mo",
    targetDir: params?.targetDir ?? "/tmp/opik-mo",
...
function createSuccessfulClawHubUpdateResult(params?: {
  ...
  return {
    ...
    targetDir: params?.targetDir ?? "/tmp/mo-plugins/legacy-chat",
    ...
    plugins: {
      installs: {
        "mo-codex-app-server": {
          source: "npm" as const,
          spec: params.spec,
          installPath: "/tmp/mo-codex-app-server",
          ...(params.resolvedName ? { resolvedName: params.resolvedName } : {}),
          ...(params.resolvedSpec ? { resolvedSpec: params.resolvedSpec } : {}),
        },
      },
...
function createInstalledPackageDir(params: {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-plugin-update-test-"));
  tempDirs.push(dir);
  fs.writeFileSync(
...
function createOpenClawPeerLinkFixtures(plugins: Array<{ pluginId: string; packageName: string }>) {
  const peerTarget = fs.mkdtempSync(path.join(os.tmpdir(), "mo-peer-target-"));
  ...
  const installPaths = Object.fromEntries(
    plugins.map(({ pluginId, packageName }) => [
      ...
      createInstalledPackageDir({
        name: packageName,
        version: "2026.5.4",
        peerDependencies: { mo: ">=2026.5.4" },
      }),
    ]),
  );
  const peerLinkPath = (pluginId: string) =>
    path.join(installPaths[pluginId], "node_modules", "mo");
...
function createBundledSource(params?: { pluginId?: string; localPath?: string; npmSpec?: string }) {
  ...
  return {
    ...
    npmSpec: params?.npmSpec ?? `@mo/${pluginId}`,
  };
}
...
function expectCodexAppServerInstallState(params: {
  ...
  const install = params.result.config.plugins?.installs?.["mo-codex-app-server"];
  expect(install?.source).toBe("npm");
  expect(install?.spec).toBe(params.spec);
  expect(install?.installPath).toBe("/tmp/mo-codex-app-server");
...
describe("updateNpmInstalledPlugins", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "mo-codex-app-server",
        spec: "mo-codex-app-server@0.2.0-beta.3",
        integrity: "sha512-old",
        installPath: "/tmp/mo-codex-app-server",
      }),
      pluginIds: ["mo-codex-app-server"],
      specOverrides: {
        "mo-codex-app-server": "mo-codex-app-server@0.2.0-beta.4",
      },
      installerResult: createSuccessfulNpmUpdateResult({
        pluginId: "mo-codex-app-server",
        targetDir: "/tmp/mo-codex-app-server",
        version: "0.2.0-beta.4",
      }),
      expectedCall: {
        spec: "mo-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      },
    },
  ] as const)(
  ...
  it("trusts official catalog npm updates when the installed package matches the catalog", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@mo/acpx",
          version: "2026.5.2-beta.2",
          resolvedSpec: "@mo/acpx@2026.5.2-beta.2",
        },
      }),
    );
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        ...
        spec: "@mo/acpx",
        installPath,
        resolvedName: "@mo/acpx",
        resolvedSpec: "@mo/acpx@2026.5.2-beta.1",
    ...
    expect(npmInstallCall()?.spec).toBe("@mo/acpx");
    expect(npmInstallCall()?.expectedPluginId).toBe("acpx");
    expect(npmInstallCall()?.trustedSourceLinkedOfficialInstall).toBe(true);
    expect(result.config.plugins?.installs?.acpx?.spec).toBe("@mo/acpx@2026.5.2-beta.2");
  });
  ...
  it("pins unchanged official npm records during official sync", async () => {
    ...
    expect(result.config.plugins?.installs?.acpx?.spec).toBe("@mo/acpx@2026.5.2");
  ...
  it("keeps integrity drift checks for exact prerelease-only official pins", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@mo/voice-call",
          version: "0.0.2-beta.1",
          resolvedSpec: "@mo/voice-call@0.0.2-beta.1",
        },
      }),
    );
    ...
    await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        pluginId: "voice-call",
        spec: "@mo/voice-call@0.0.2-beta.1",
        installPath,
        resolvedName: "@mo/voice-call",
        resolvedSpec: "@mo/voice-call@0.0.2-beta.1",
  ...
  it("keeps integrity drift checks for exact official pins during beta fallback", async () => {
    ...
    installPluginFromNpmSpecMock
      .mockResolvedValueOnce({
        ok: false,
        error: "No matching version found for @mo/acpx@beta",
    ...
    expect(npmInstallCall(0)?.spec).toBe("@mo/acpx@beta");
    expect(npmInstallCall(0)?.expectedIntegrity).toBeUndefined();
    expect(npmInstallCall(1)?.spec).toBe("@mo/acpx");
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@mo/acpx",
          version: "2026.5.3-1",
          resolvedSpec: "@mo/acpx@2026.5.3-1",
        },
      }),
    );
  ...
  it("does not skip unchanged npm plugins when package metadata requires a newer plugin API", async () => {
    vi.stubEnv("MO_COMPATIBILITY_HOST_VERSION", "2026.5.28-beta.3");
    const installPath = createInstalledPackageDir({
      name: "@mo/msteams",
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@mo/msteams",
          version: "2026.5.28-beta.3",
          resolvedSpec: "@mo/msteams@2026.5.28-beta.3",
        },
      }),
    );
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        ...
        resolvedSpec: "@mo/msteams@2026.5.28-beta.4",
        integrity: "sha512-newer",
        shasum: "newer",
      }),
      pluginIds: ["msteams"],
    ...
    expect(npmInstallCall()?.spec).toBe("@mo/msteams");
  ...
  it("repairs missing mo peer links before skipping unchanged npm plugins", async () => {
    const installPath = createInstalledPackageDir({
      name: "@mo/codex",
      version: "2026.5.3",
      peerDependencies: { mo: ">=2026.5.3" },
    });
    mockNpmViewMetadata({
      name: "@mo/codex",
      version: "2026.5.3",
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@mo/codex",
          version: "2026.5.3",
          resolvedSpec: "@mo/codex@2026.5.3",
        },
      }),
    );
    const config: OpenClawConfig = {
    ...
    expect(npmInstallCall()?.spec).toBe("@mo/codex");
    expect(npmInstallCall()?.mode).toBe("update");
  ...
  it("skips unchanged npm plugins when the mo peer link already resolves", async () => {
    const installPath = createInstalledPackageDir({
      name: "@mo/codex",
      version: "2026.5.3",
      peerDependencies: { mo: ">=2026.5.3" },
    });
    fs.mkdirSync(path.join(installPath, "node_modules", "mo"), { recursive: true });
    mockNpmViewMetadata({
      name: "@mo/codex",
      version: "2026.5.3",
      integrity: "sha512-same",
  ...
  it("repairs mo peer links after batch npm updates prune earlier plugin links", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@mo/brave-plugin" },
      { pluginId: "codex", packageName: "@mo/codex" },
      { pluginId: "discord", packageName: "@mo/discord" },
    ];
  ...
  it("repairs sibling mo peer links after a targeted npm update prunes the shared install tree", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@mo/brave-plugin" },
      { pluginId: "codex", packageName: "@mo/codex" },
      { pluginId: "discord", packageName: "@mo/discord" },
      ...
      return Promise.resolve(
        createSuccessfulNpmUpdateResult({
          ...
          npmResolution: {
            name: "@mo/codex",
            version: "2026.5.4",
            resolvedSpec: "@mo/codex@2026.5.4",
          },
        }),
      );
    });
  ...
  it("continues repairing sibling mo peer links after one recorded npm install cannot be relinked", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@mo/brave-plugin" },
      { pluginId: "codex", packageName: "@mo/codex" },
    ];
      ...
      config: {
        plugins: {
          installs: {
            broken: {
              source: "npm",
              spec: "@mo/broken-plugin",
              installPath: brokenInstallPath,
              resolvedName: "@mo/broken-plugin",
              resolvedVersion: "2026.5.4",
              resolvedSpec: "@mo/broken-plugin@2026.5.4",
            },
            ...Object.fromEntries(
              plugins.map(({ pluginId, packageName }) => [
                pluginId,
    ...
    expect(warnMessages).toEqual([
      `Could not repair mo peer link for "broken" at ${brokenInstallPath}: Skipping mo peerDependency link because ${path.join(brokenInstallPath, "node_modules")} is not a real directory.`,
    ]);
  });
  ...
  it("expands home-relative install paths before checking installed npm versions", async () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "mo-plugin-update-home-"));
    tempDirs.push(home);
    const installPath = path.join(home, ".mo", "extensions", "lossless-claw");
    ...
    const result = await withEnvAsync({ HOME: home }, () =>
      updateNpmInstalledPlugins({
        config: createNpmInstallConfig({
          pluginId: "lossless-claw",
          spec: "@martian-engineering/lossless-claw",
          installPath: "~/.mo/extensions/lossless-claw",
          resolvedName: "@martian-engineering/lossless-claw",
          resolvedVersion: "0.9.0",
          resolvedSpec: "@martian-engineering/lossless-claw@0.9.0",
          integrity: "sha512-same",
  ...
  it("preserves exact official npm pins when official install sync is not requested", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@mo/codex",
          version: "2026.5.28",
          resolvedSpec: "@mo/codex@2026.5.28",
        },
      }),
    );
    ...
    expect(npmInstallCall()?.spec).toBe("@mo/codex@2026.5.28");
    expect(npmInstallCall()?.expectedPluginId).toBe("codex");
    expect(npmInstallCall()?.trustedSourceLinkedOfficialInstall).toBe(true);
    expect(result.changed).toBe(false);
    expectRecordFields(result.outcomes[0], {
  ...
  it("reinstalls missing exact official npm pins without official install sync", async () => {
    const extensionsDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-missing-plugin-"));
  ...
  it.each(["@acme/demo@1.2.3", "@acme/demo@v1.2.3"])(
    ...
    async (spec) => {
      ...
      expectRecordFields(result.outcomes[0], {
        ...
        currentVersion: "1.2.3",
        nextVersion: "1.2.3",
        message: `demo is pinned to ${spec} (installed 1.2.3); registry default resolves to 1.2.4. Pass \`mo plugins update @acme/demo@latest\` to follow the registry default line.`,
      });
    },
  );
  ...
  it("updates disabled trusted official ClawHub installs through the catalog spec", async () => {
    installPluginFromClawHubMock.mockResolvedValue(
      createSuccessfulClawHubUpdateResult({
        ...
        version: "2026.5.4",
        clawhubPackage: "@mo/diagnostics-otel",
      }),
    );
    ...
    const config = createClawHubInstallConfig({
      ...
      clawhubFamily: "code-plugin",
      clawhubChannel: "official",
      spec: "clawhub:@mo/diagnostics-otel@2026.5.3",
    });
    const result = await updateNpmInstalledPlugins({
      config: {
        ...config,
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@mo/diagnostics-otel");
    ...
    expectRecordFields(result.config.plugins?.installs?.["diagnostics-otel"], {
      source: "clawhub",
      spec: "clawhub:@mo/diagnostics-otel",
      version: "2026.5.4",
      clawhubPackage: "@mo/diagnostics-otel",
      clawhubChannel: "official",
    });
  ...
  it("updates bare trusted official ClawHub installs through the catalog spec", async () => {
    installPluginFromClawHubMock.mockResolvedValue(
      createSuccessfulClawHubUpdateResult({
        ...
        version: "2026.5.4",
        clawhubPackage: "@mo/diagnostics-prometheus",
      }),
    );
        ...
        plugins: {
          installs: {
            "diagnostics-prometheus": {
              source: "clawhub",
              spec: "clawhub:@mo/diagnostics-prometheus@2026.5.3",
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@mo/diagnostics-prometheus");
    ...
    expectRecordFields(result.config.plugins?.installs?.["diagnostics-prometheus"], {
      source: "clawhub",
      spec: "clawhub:@mo/diagnostics-prometheus",
      version: "2026.5.4",
      clawhubPackage: "@mo/diagnostics-prometheus",
      clawhubChannel: "official",
    });
  ...
  it("does not skip a risk-gated ClawHub update when the installed package is missing", async () => {
    ...
    const installPath = fs.mkdtempSync(path.join(os.tmpdir(), "mo-plugin-update-missing-"));
  ...
  it("aborts exact pinned npm plugin updates on integrity drift by default", async () => {
    ...
    installPluginFromNpmSpecMock.mockImplementation(
      async (params: {
        ...
        const proceed = await params.onIntegrityDrift?.({
          ...
          resolution: {
            integrity: "sha512-new",
            resolvedSpec: "@opik/opik-mo@0.2.5",
            version: "0.2.5",
          },
        });
        if (proceed === false) {
          return {
            ok: false,
            error: "aborted: npm package integrity drift detected for @opik/opik-mo@0.2.5",
          };
        }
    ...
    expect(warn).toHaveBeenCalledWith(
      'Integrity drift for "opik-mo" (@opik/opik-mo@0.2.5): expected sha512-old, got sha512-new',
    );
    expect(result.changed).toBe(false);
    expect(result.config).toBe(config);
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update opik-mo: aborted: npm package integrity drift detected for @opik/opik-mo@0.2.5",
      },
    ]);
  });
  ...
  it.each([
    {
      ...
      installerResult: {
        ok: false,
        code: "npm_package_not_found",
        error: "Package not found on npm: @mo/missing.",
      },
      config: createNpmInstallConfig({
        pluginId: "missing",
        spec: "@mo/missing",
      ...
      pluginId: "missing",
      expectedMessage: "Failed to check missing: npm package not found for @mo/missing.",
    },
    {
  ...
  it.each([
    {
      ...
      config: createCodexAppServerInstallConfig({
        spec: "mo-codex-app-server@beta",
        resolvedName: "mo-codex-app-server",
        resolvedSpec: "mo-codex-app-server@0.2.0-beta.3",
      }),
      expectedSpec: "mo-codex-app-server@beta",
    ...
    },
    {
      ...
      installerResult: {
        ...
        extensions: ["index.ts"],
        npmResolution: {
          name: "mo-codex-app-server",
          version: "0.2.0-beta.4",
          resolvedSpec: "mo-codex-app-server@0.2.0-beta.4",
        },
      },
      config: createCodexAppServerInstallConfig({
        spec: "mo-codex-app-server",
      }),
      specOverrides: {
        "mo-codex-app-server": "mo-codex-app-server@beta",
      },
      expectedSpec: "mo-codex-app-server@beta",
      expectedRecordSpec: "mo-codex-app-server@beta",
      expectedVersion: "0.2.0-beta.4",
      expectedResolvedSpec: "mo-codex-app-server@0.2.0-beta.4",
    },
  ] as const)(
    "$name",
    async ({
      ...
      expectNpmUpdateCall({
        spec: expectedSpec,
        expectedPluginId: "mo-codex-app-server",
      });
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@mo/acpx",
          version: "2026.5.3-beta.1",
          resolvedSpec: "@mo/acpx@2026.5.3-beta.1",
        },
      }),
    );
  ...
  it("falls back to the default npm spec when a beta tag is unavailable", async () => {
    installPluginFromNpmSpecMock
      .mockResolvedValueOnce({
        ok: false,
        error:
          "npm ERR! code ETARGET\nnpm ERR! No matching version found for mo-codex-app-server@beta.",
      })
      .mockResolvedValueOnce(
        createSuccessfulNpmUpdateResult({
          pluginId: "mo-codex-app-server",
          ...
          npmResolution: {
            name: "mo-codex-app-server",
            version: "0.2.6",
            resolvedSpec: "mo-codex-app-server@0.2.6",
          },
        }),
      );
    ...
    expect(npmInstallCall(0)?.spec).toBe("mo-codex-app-server@beta");
    expect(npmInstallCall(1)?.spec).toBe("mo-codex-app-server");
    expect(warnMessages).toEqual([
      'Plugin "mo-codex-app-server" has no beta npm release for mo-codex-app-server@beta; using mo-codex-app-server instead. Core update can still complete.',
    ]);
    expectCodexAppServerInstallState({
      result,
      spec: "mo-codex-app-server",
    ...
    expect(result.outcomes[0]?.message).toBe(
      "Updated mo-codex-app-server: unknown -> 0.2.6. (warning: beta channel fallback used mo-codex-app-server because mo-codex-app-server@beta could not be used).",
    );
    expect(result.outcomes[0]?.channelFallback).toEqual({
      requestedSpec: "mo-codex-app-server@beta",
      usedSpec: "mo-codex-app-server",
      requestedLabel: "@beta",
      usedLabel: "@latest",
      reason: "unavailable",
      message:
        "plugin channel fallback: mo-codex-app-server used @latest because @beta was unavailable",
    });
  });
  ...
  it("reports npm beta fallback as tentative during dry-run checks", async () => {
    ...
    expect(result.outcomes[0]?.message).toBe(
      "Would update mo-codex-app-server: unknown -> 0.2.6. (warning: beta channel fallback would use mo-codex-app-server because mo-codex-app-server@beta could not be used).",
    );
    expect(result.outcomes[0]?.channelFallback?.message).toBe(
      "plugin channel fallback: mo-codex-app-server would use @latest because @beta was unavailable",
    );
  });
  ...
  it("falls back to the default npm spec when the beta package exists but is invalid", async () => {
    ...
    expect(npmInstallCall(1)?.spec).toBe("mo-codex-app-server");
    expect(warnMessages).toEqual([
      'Plugin "mo-codex-app-server" failed beta npm update for mo-codex-app-server@beta; using mo-codex-app-server instead. Core update can still complete.',
    ]);
    expectCodexAppServerInstallState({
      result,
      spec: "mo-codex-app-server",
    ...
    expect(result.outcomes[0]?.channelFallback).toMatchObject({
      ...
      usedLabel: "@latest",
      reason: "failed",
      message: "plugin channel fallback: mo-codex-app-server used @latest after @beta failed",
    });
  });
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update mo-codex-app-server: npm package not found for mo-codex-app-server.",
        channelFallback: {
          requestedSpec: "mo-codex-app-server@beta",
          usedSpec: "mo-codex-app-server",
          requestedLabel: "@beta",
          ...
          reason: "failed",
          message:
            "plugin channel fallback: mo-codex-app-server used @latest after @beta failed",
        },
      },
    ]);
  });
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to check mo-codex-app-server: npm package not found for mo-codex-app-server.",
        channelFallback: {
          requestedSpec: "mo-codex-app-server@beta",
          usedSpec: "mo-codex-app-server",
          requestedLabel: "@beta",
          ...
          reason: "failed",
          message:
            "plugin channel fallback: mo-codex-app-server would use @latest after @beta failed",
        },
      },
    ]);
  });
  ...
  it("does not fall back to npm for blocked official ClawHub artifact downloads", async () => {
    ...
    installPluginFromClawHubMock.mockResolvedValueOnce({
      ...
      code: "clawhub_download_blocked",
      error:
        'ClawHub blocked artifact download for "@mo/discord@2026.5.16-beta.5"; install was not started. ClawHub /api/v1/packages/%40mo%2Fdiscord/versions/2026.5.16-beta.5/artifact/download failed (403): Blocked: this package release has been flagged as malicious and cannot be downloaded.',
    ...
    const result = await updateNpmInstalledPlugins({
      config: createClawHubInstallConfig({
        ...
        installPath,
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@mo/discord",
        clawhubFamily: "code-plugin",
        clawhubChannel: "official",
        spec: "clawhub:@mo/discord",
      }),
      pluginIds: ["discord"],
      updateChannel: "beta",
      disableOnFailure: true,
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@mo/discord@beta");
    expect(installPluginFromNpmSpecMock).not.toHaveBeenCalled();
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        message:
          'Skipped discord ClawHub update: ClawHub blocked artifact download for "@mo/discord@2026.5.16-beta.5"; install was not started. ClawHub /api/v1/packages/%40mo%2Fdiscord/versions/2026.5.16-beta.5/artifact/download failed (403): Blocked: this package release has been flagged as malicious and cannot be downloaded. Existing installed plugin left unchanged.',
      },
    ]);
  ...
  it("uses the default npm spec when beta ClawHub falls back before an artifact block", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValueOnce(
      createSuccessfulNpmUpdateResult({
        pluginId: "discord",
        targetDir: "/tmp/mo-plugins/discord",
        version: "2026.5.16",
        npmResolution: {
          name: "@mo/discord",
          version: "2026.5.16",
          resolvedSpec: "@mo/discord@2026.5.16",
        },
      }),
    );
    ...
    expect(clawHubInstallCall(0)?.spec).toBe("clawhub:@mo/discord@beta");
    expect(clawHubInstallCall(1)?.spec).toBe("clawhub:@mo/discord");
    expect(npmInstallCall()?.spec).toBe("@mo/discord");
    expectRecordFields(result.config.plugins?.installs?.discord, {
      source: "npm",
      spec: "@mo/discord@2026.5.16",
      installPath: "/tmp/mo-plugins/discord",
      version: "2026.5.16",
    });
    expect(result.outcomes[0]?.message).toBe(
      "Updated discord: 2026.5.12 -> 2026.5.16. (warning: official ClawHub artifact fallback used @mo/discord).",
    );
    expect(warnMessages).toEqual([
      'Plugin "discord" has no beta ClawHub release for clawhub:@mo/discord@beta; using clawhub:@mo/discord instead. Core update can still complete.',
      'Plugin "discord" could not download official ClawHub artifact for clawhub:@mo/discord; using npm @mo/discord instead. Core update can still complete.',
    ]);
  });
  ...
  it("reports npm dry-run versions for trusted official ClawHub artifact fallback", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValueOnce({
      ...
      npmResolution: {
        name: "@mo/discord",
        version: "2026.5.16-beta.5",
        resolvedSpec: "@mo/discord@2026.5.16-beta.5",
      },
    });
    ...
    expect(npmInstallCall()?.spec).toBe("@mo/discord@beta");
    expect(npmInstallCall()?.dryRun).toBe(true);
    expect(result.outcomes).toEqual([
      {
        pluginId: "discord",
        ...
        message:
          "discord is up to date (2026.5.16-beta.5). (warning: official ClawHub artifact fallback would use @mo/discord@beta).",
      },
    ]);
  });
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update discord: artifact unavailable (ClawHub clawhub:@mo/discord@beta).",
      },
    ]);
  });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@mo/voice-call",
      targetDir: "/tmp/mo-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(npmInstallCall()?.spec).toBe("@mo/voice-call");
    expect(npmInstallCall()?.expectedPluginId).toBe("voice-call");
    expect(result.config.plugins?.allow).toEqual(["@mo/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@mo/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@mo/voice-call");
    expect(result.config.plugins?.entries?.["@mo/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expectRecordFields(result.config.plugins?.installs?.["@mo/voice-call"], {
      source: "npm",
      spec: "@mo/voice-call",
      installPath: "/tmp/mo-voice-call",
      version: "0.0.2",
    });
  ...
  it("keeps authored plugin config shape when only the install key migrates", async () => {
    ...
    expect(result.config.plugins).toEqual({
      installs: {
        "@mo/voice-call": expect.objectContaining({
          source: "npm",
          spec: "@mo/voice-call",
          installPath: "/tmp/mo-voice-call",
        }),
  ...
  it("migrates context engine slot when a plugin id changes during update", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@mo/context-engine",
      targetDir: "/tmp/mo-context-engine",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.slots?.contextEngine).toBe("@mo/context-engine");
    expectRecordFields(result.config.plugins?.installs?.["@mo/context-engine"], {
      source: "npm",
      spec: "@mo/context-engine",
      installPath: "/tmp/mo-context-engine",
      version: "0.0.2",
    });
  ...
  it("forwards dangerous force unsafe install to plugin update installers", async () => {
    ...
    expect(npmInstallCall()?.spec).toBe("mo-codex-app-server@beta");
    expect(npmInstallCall()?.dangerouslyForceUnsafeInstall).toBe(true);
    expect(npmInstallCall()?.expectedPluginId).toBe("mo-codex-app-server");
  });

  it("reuses the recorded managed extensions root when updating external plugins", async () => {
    const installPath = "/var/mo/extensions/demo";
    const extensionsDir = "/var/mo/extensions";
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("forwards an explicit env to bundled plugin source resolution", async () => {
    resolveBundledPluginSourcesMock.mockReturnValue(new Map());
    const env = { MO_HOME: "/srv/mo-home" } as NodeJS.ProcessEnv;
  ...
  it("installs an externalized bundled plugin and rewrites its old bundled path plugin index", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        targetDir: "/tmp/mo-plugins/legacy-chat",
        version: "2.0.0",
        npmResolution: {
          name: "@mo/legacy-chat",
          version: "2.0.0",
          resolvedSpec: "@mo/legacy-chat@2.0.0",
        },
      }),
    );
    ...
    expect(npmInstallCall()?.spec).toBe("@mo/legacy-chat");
    expect(npmInstallCall()?.mode).toBe("update");
    ...
    expectRecordFields(result.config.plugins?.installs?.["legacy-chat"], {
      source: "npm",
      spec: "@mo/legacy-chat",
      installPath: "/tmp/mo-plugins/legacy-chat",
      version: "2.0.0",
      resolvedName: "@mo/legacy-chat",
  ...
  it("marks official externalized bundled npm installs as trusted", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        pluginId: "voice-call",
        targetDir: "/tmp/mo-plugins/voice-call",
        version: "0.0.2-beta.1",
      }),
    );
  ...
  it("falls back from ClawHub to npm only when the ClawHub package is absent", async () => {
    ...
    expect(result.summary.warnings).toEqual([
      "ClawHub clawhub:legacy-chat@2026.5.1-beta.2 unavailable for legacy-chat; falling back to npm @mo/legacy-chat.",
    ]);
    expect(result.summary.errors).toStrictEqual([]);
    expectRecordFields(result.config.plugins?.installs?.["legacy-chat"], {
      source: "npm",
      ...
      externalizedBundledPluginBridges: [
        {
          bundledPluginId: "voice-call",
          preferredSource: "clawhub",
          clawhubSpec: "clawhub:@mo/voice-call",
          npmSpec: "@mo/voice-call",
          channelIds: ["voice-call"],
        },
      ],
  ...
  it("externalizes bundled plugins that were enabled by default", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        targetDir: "/tmp/mo-plugins/default-chat",
        version: "2.0.0",
      }),
    );
    ...
    expect(npmInstallCall()?.spec).toBe("@mo/default-chat");
    expect(npmInstallCall()?.mode).toBe("update");
    ...
    expectRecordFields(result.config.plugins?.installs?.["default-chat"], {
      source: "npm",
      spec: "@mo/default-chat",
      installPath: "/tmp/mo-plugins/default-chat",
      version: "2.0.0",
    });
  });
      ...
      config: {
        ...
        plugins: {
          ...
          installs: {
            "legacy-chat": {
              source: "npm",
              spec: "@mo/legacy-chat@1.2.3",
              resolvedSpec: "@mo/legacy-chat@1.2.3",
              installPath: "/tmp/mo-plugins/legacy-chat",
            },
          },
        },
