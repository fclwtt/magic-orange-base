...
export const MO_DEV_SOURCE_ROOT_ENV = "MO_DEV_SOURCE_ROOT";
...
export function resolveOpenClawDevSourceRoot(env: NodeJS.ProcessEnv = process.env): string | null {
  const rawRoot = env[MO_DEV_SOURCE_ROOT_ENV]?.trim();
  if (!rawRoot) {
    return null;
  }
  ...
  if (readPackageName(path.join(realRoot, "package.json")) !== "mo") {
    return null;
  }
