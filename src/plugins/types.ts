...
} from "@mo/model-catalog-core/model-catalog-types";
...
export type ProviderPlugin = {
  ...
  /**
   * Legacy auth-profile ids that should be retired by `mo doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `mo doctor` should offer.
   *
...
export type OpenClawPluginCliContext = {
   ...
   *
   * For root CLI registrations this is the root `mo` program. For nested
...
export type OpenClawPluginNodeCliFeatureOptions = {
  /** Explicit node feature command names owned under `mo nodes`. */
  commands?: string[];
  /**
   * Parse-time command descriptors for lazy node feature CLI registration.
   *
   * Descriptors are registered under `mo nodes`, so a descriptor named
   * `"camera"` exposes `mo nodes camera`.
   */
...
export type OpenClawPluginDefinition = {
  ...
  version?: string;
  /**
   * @deprecated Declare exclusive plugin kind in `mo.plugin.json` via
...
/** Migration source implemented by a plugin and orchestrated by `mo migrate`. */
...
export type OpenClawPluginApi = {
  ...
  /**
   * Register a plugin-owned node feature command group under `mo nodes`.
   *
  ...
  /** Register an importer for `mo migrate` (migration capability). */
