...
function deriveIdHint(params: {
  ...
  // package is scoped (example: @mo/voice-call -> voice-call).
...
function pushInvalidPackageExtensionDiagnostic(params: {
  ...
  if (params.resolution.status === "empty") {
    params.diagnostics.push({
      level: "error",
      source: params.source,
      message: "package.json mo.extensions is empty",
    });
    return true;
  }
  return false;
...
function addLegacyNpmDeclarationDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message: `legacy npm plugin declaration ignored for "${declaration.pluginId}"; run "mo doctor --fix" to install ${declaration.npmSpec} into the managed plugin root`,
  });
  return true;
}
      ...
      for (const extraPath of extra) {
        ...
        if (bundledAlias) {
          result.diagnostics.push({
            level: "warn",
            source: trimmed,
            message: `ignored plugins.load.paths entry that points at OpenClaw's ${bundledAlias.kind} bundled plugin directory; remove this redundant path or run mo doctor --fix`,
          });
          continue;
        }
        discoverFromPath({
