...
function createTempRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "mo-manifest-metadata-"));
  tempRoots.push(root);
  return root;
}
...
describe("listOpenClawPluginManifestMetadata", () => {
  ...
  it("prefers the active bundled manifest over stale persisted bundled installs", () => {
    ...
    writeJson(path.join(bundledRoot, "openai", "mo.plugin.json"), {
      id: "openai",
      providerEndpoints: [{ endpointClass: "openai-public", hosts: ["api.openai.com"] }],
    });
    writeJson(path.join(staleBundledRoot, "openai", "mo.plugin.json"), {
      id: "openai",
      providers: ["openai"],
    });
    writePersistedInstalledPluginIndexSync(
      {
        ...
        plugins: [
          {
            pluginId: "openai",
            manifestPath: path.join(staleBundledRoot, "openai", "mo.plugin.json"),
