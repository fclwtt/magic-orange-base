...
const fixtureRoot = makeTrackedTempDir("mo-sdk-alias-root", fixtureTempDirs);
let tempDirIndex = 0;
...
function createTrustedOpenClawPackageFixture(version: string) {
  ...
  fs.writeFileSync(path.join(root, "mo.mjs"), "export {};\n", "utf-8");
  fs.writeFileSync(
...
function createExtensionApiAliasFixture(params?: {
  ...
  fs.writeFileSync(
    path.join(root, "package.json"),
    JSON.stringify({ name: "mo", type: "module" }, null, 2),
    "utf-8",
  );
  fs.writeFileSync(path.join(root, "mo.mjs"), "export {};\n", "utf-8");
  fs.writeFileSync(srcFile, params?.srcBody ?? "export {};\n", "utf-8");
...
function createBundledPluginPackagePublicSurfaceAliasFixture() {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "@mo/slack", type: "module" }, null, 2),
    "utf-8",
  );
...
function createUserInstalledPluginSdkAliasFixture() {
  ...
  const externalPluginRoot = path.join(makeTempDir(), ".mo", "extensions", "demo");
  const externalPluginEntry = path.join(externalPluginRoot, "index.ts");
  mkdirSafeDir(externalPluginRoot);
  fs.writeFileSync(
    externalPluginEntry,
    [
      'import { definePluginEntry } from "mo/plugin-sdk/plugin-entry";',
...
function expectPluginSdkAliasTargets(
  ...
  expect(fs.realpathSync(aliases["mo/plugin-sdk"] ?? "")).toBe(
  ...
  if (params.channelRuntimePath) {
    expect(fs.realpathSync(aliases["mo/plugin-sdk/channel-runtime"] ?? "")).toBe(
  ...
  if (params.pluginEntryPath) {
    expect(fs.realpathSync(aliases["mo/plugin-sdk/plugin-entry"] ?? "")).toBe(
...
function expectCwdFallbackPluginSdkAliasResolution(params: {
  ...
  const resolved = withCwd(params.fixture.root, () =>
    resolvePluginSdkAlias({
      ...
      distFile: "channel-runtime.js",
      modulePath: "/tmp/tsx-cache/mo-loader.js",
...
describe("plugin sdk alias helpers", () => {
  it.each([
    ...
    },
    {
      ...
      modulePath: () => "/tmp/tsx-cache/mo-loader.js",
      argv1: (root: string) => path.join(root, "mo.mjs"),
      srcFile: "index.ts",
      distFile: "index.js",
      env: { NODE_ENV: undefined },
      expected: "src" as const,
  ...
  it("resolves extension-api aliases from an explicit dev source root", () => {
    ...
    expect(fs.realpathSync(aliases["mo/extension-api"] ?? "")).toBe(
  ...
  it("adds private qa plugin-sdk subpaths for trusted local checkouts when enabled", () => {
    ...
    const subpaths = withEnv({ MO_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: path.join(fixture.root, "src", "plugins", "loader.ts"),
      }),
    );
  ...
  it("resolves public QA plugin-sdk aliases without enabling private QA mode", () => {
    ...
    const subpaths = withEnv({ MO_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({ modulePath: sourcePluginEntry }),
    );
    const aliases = withEnv(
      { MO_ENABLE_PRIVATE_QA_CLI: undefined, NODE_ENV: undefined },
    ...
    expect(fs.realpathSync(aliases["mo/plugin-sdk/qa-runner-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceQaRunnerPath),
    );
    expect(fs.realpathSync(aliases["mo/plugin-sdk/qa-live-transport-scenarios"] ?? "")).toBe(
      fs.realpathSync(distQaLiveTransportScenariosPath),
    );
    expect(aliases["mo/plugin-sdk/qa-runtime"]).toBeUndefined();
  });
  ...
  it("adds non-QA private Codex helper subpaths only for trusted Codex plugins", () => {
    ...
    const { packageRoot: installedCodexRoot, pluginEntry: installedCodexEntry } =
      writeInstalledPluginEntry({
        installRoot: path.join(makeTempDir(), ".mo", "npm"),
        packageName: "@mo/codex",
      });
    ...
    const shadowCodexRoot = path.join(makeTempDir(), ".mo", "extensions", "codex-shadow");
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: "@mo/codex", type: "module" }, null, 2),
      "utf-8",
    );
    fs.writeFileSync(shadowCodexEntry, 'export const plugin = "shadow";\n', "utf-8");

    const codexSubpaths = withEnv({ MO_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceCodexEntry,
      }),
    );
    const otherSubpaths = withEnv({ MO_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceOtherEntry,
      }),
    );
    const installedCodexSubpaths = withCwd(installedCodexRoot, () =>
      withEnv({ MO_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
        listPluginSdkExportedSubpaths({
          modulePath: installedCodexEntry,
          argv1: path.join(fixture.root, "mo.mjs"),
        }),
      ),
    );
  ...
  it("does not reuse a non-private cached subpath list after private qa gets enabled", () => {
    ...
    const privateSubpaths = withEnv({ MO_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: path.join(fixture.root, "src", "plugins", "loader.ts"),
      }),
    );
  ...
  it("adds private qa plugin-sdk aliases for source plugins when enabled", () => {
    ...
    const aliases = withEnv({ MO_ENABLE_PRIVATE_QA_CLI: "1", NODE_ENV: undefined }, () =>
      buildPluginLoaderAliasMap(sourcePluginEntry),
    );
    ...
    expect(fs.realpathSync(aliases["mo/plugin-sdk/qa-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceQaRuntimePath),
    );
    expect(fs.realpathSync(aliases["mo/plugin-sdk/qa-channel"] ?? "")).toBe(
      fs.realpathSync(sourceQaChannelPath),
    );
    expect(fs.realpathSync(aliases["mo/plugin-sdk/qa-channel-protocol"] ?? "")).toBe(
      fs.realpathSync(sourceQaChannelProtocolPath),
    );
    expect(fs.realpathSync(aliases["mo/plugin-sdk/qa-lab"] ?? "")).toBe(
  ...
  it("aliases non-QA private plugin-sdk subpaths for trusted Codex runtime loading", () => {
    ...
    const installedAliases = withCwd(installedCodexRoot, () =>
      withEnv({ MO_ENABLE_PRIVATE_QA_CLI: undefined, NODE_ENV: undefined }, () =>
        buildPluginLoaderAliasMap(
          installedCodexEntry,
          path.join(fixture.root, "mo.mjs"),
          undefined,
    ...
    expect(fs.realpathSync(aliases["mo/plugin-sdk/codex-mcp-projection"] ?? "")).toBe(
      fs.realpathSync(sourceCodexMcpProjectionPath),
    );
    expect(fs.realpathSync(aliases["mo/plugin-sdk/codex-native-task-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceCodexNativeTaskRuntimePath),
    );
    expect(
      fs.realpathSync(installedAliases["mo/plugin-sdk/codex-mcp-projection"] ?? ""),
    ).toBe(fs.realpathSync(distCodexMcpProjectionPath));
    expect(
      fs.realpathSync(installedAliases["mo/plugin-sdk/codex-native-task-runtime"] ?? ""),
    ).toBe(fs.realpathSync(distCodexNativeTaskRuntimePath));
    expect(fs.realpathSync(devRootAliases["mo/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(devRootAlias),
    );
    expect(fs.realpathSync(devRootAliases["mo/plugin-sdk/codex-mcp-projection"] ?? "")).toBe(
      fs.realpathSync(devCodexMcpProjectionPath),
    );
    expect(
      fs.realpathSync(devRootAliases["mo/plugin-sdk/codex-native-task-runtime"] ?? ""),
    ).toBe(fs.realpathSync(devCodexNativeTaskRuntimePath));
    expect(aliases["mo/plugin-sdk/qa-runtime"]).toBeUndefined();
    expect(otherAliases["mo/plugin-sdk/codex-mcp-projection"]).toBeUndefined();
    expect(otherAliases["mo/plugin-sdk/codex-native-task-runtime"]).toBeUndefined();
    expect(installedOtherAliases["mo/plugin-sdk/codex-mcp-projection"]).toBeUndefined();
    expect(installedOtherAliases["mo/plugin-sdk/codex-native-task-runtime"]).toBeUndefined();
    expect(shadowCodexAliases["mo/plugin-sdk/codex-mcp-projection"]).toBeUndefined();
    expect(shadowCodexAliases["mo/plugin-sdk/codex-native-task-runtime"]).toBeUndefined();
  });
  ...
  it("aliases the SSRF internal helper only for bundled local IPC owner plugins", async () => {
    ...
    const entryBody = [
      'import { ssrfInternal } from "mo/plugin-sdk/ssrf-runtime-internal";',
    ...
    const { packageRoot: installedOllamaRoot, pluginEntry: installedOllamaEntry } =
      writeInstalledPluginEntry({
        installRoot: path.join(makeTempDir(), ".mo", "npm"),
        packageName: "@mo/ollama",
      });

    const sourceSubpaths = withEnv({ MO_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceOllamaEntry,
      }),
    );
    const sourceBrowserSubpaths = withEnv({ MO_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceBrowserEntry,
      }),
    );
    const privateQaOtherSubpaths = withEnv({ MO_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      listPluginSdkExportedSubpaths({
    ...
    const privateQaOtherAliases = withEnv(
      { MO_ENABLE_PRIVATE_QA_CLI: "1", NODE_ENV: undefined },
      () => buildPluginLoaderAliasMap(sourceOtherPluginEntry),
    );
    ...
    expect(fs.realpathSync(sourceAliases["mo/plugin-sdk/ssrf-runtime-internal"] ?? "")).toBe(
      fs.realpathSync(sourceSsrFInternalPath),
    );
    expect(
      fs.realpathSync(sourceBrowserAliases["mo/plugin-sdk/ssrf-runtime-internal"] ?? ""),
    ).toBe(fs.realpathSync(sourceSsrFInternalPath));
    expect(fs.realpathSync(distAliases["mo/plugin-sdk/ssrf-runtime-internal"] ?? "")).toBe(
      fs.realpathSync(distSsrFInternalPath),
    );
    expect(
      fs.realpathSync(distBrowserAliases["mo/plugin-sdk/ssrf-runtime-internal"] ?? ""),
    ).toBe(fs.realpathSync(distSsrFInternalPath));
    expect(
      fs.realpathSync(distRuntimeAliases["mo/plugin-sdk/ssrf-runtime-internal"] ?? ""),
    ).toBe(fs.realpathSync(distSsrFInternalPath));
    expect(
      fs.realpathSync(distRuntimeBrowserAliases["mo/plugin-sdk/ssrf-runtime-internal"] ?? ""),
    ).toBe(fs.realpathSync(distSsrFInternalPath));
    expect(otherAliases["mo/plugin-sdk/ssrf-runtime-internal"]).toBeUndefined();
    expect(privateQaOtherAliases["mo/plugin-sdk/ssrf-runtime-internal"]).toBeUndefined();
    expect(installedAliases["mo/plugin-sdk/ssrf-runtime-internal"]).toBeUndefined();
  ...
  it("aliases workspace packages to source when dist artifacts are missing", () => {
    ...
    expect(fs.realpathSync(aliases["@mo/gateway-client"] ?? "")).toBe(
      fs.realpathSync(gatewayClient.srcFile),
    );
    expect(fs.realpathSync(aliases["@mo/gateway-client/timeouts"] ?? "")).toBe(
      fs.realpathSync(gatewayClientTimeouts.srcFile),
    );
    expect(fs.realpathSync(aliases["@mo/gateway-protocol"] ?? "")).toBe(
      fs.realpathSync(gatewayProtocol.srcFile),
    );
    expect(fs.realpathSync(aliases["@mo/gateway-protocol/schema"] ?? "")).toBe(
      fs.realpathSync(gatewayProtocolSchema.srcFile),
    );
    expect(fs.realpathSync(aliases["@mo/markdown-core"] ?? "")).toBe(
      fs.realpathSync(markdownCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@mo/markdown-core/tables"] ?? "")).toBe(
      fs.realpathSync(markdownCoreTables.srcFile),
    );
    expect(fs.realpathSync(aliases["@mo/media-generation-core"] ?? "")).toBe(
      fs.realpathSync(mediaGenerationCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@mo/media-generation-core/model-ref"] ?? "")).toBe(
      fs.realpathSync(mediaGenerationModelRef.srcFile),
    );
    expect(fs.realpathSync(aliases["@mo/media-core"] ?? "")).toBe(
      fs.realpathSync(mediaCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@mo/media-core/mime"] ?? "")).toBe(
      fs.realpathSync(mediaCoreMime.srcFile),
    );
    expect(fs.realpathSync(aliases["@mo/acp-core"] ?? "")).toBe(
      fs.realpathSync(acpCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@mo/acp-core/runtime/types"] ?? "")).toBe(
      fs.realpathSync(acpCoreRuntimeTypes.srcFile),
    );
    expect(fs.realpathSync(aliases["@mo/normalization-core"] ?? "")).toBe(
      fs.realpathSync(normalizationCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@mo/normalization-core/string-coerce"] ?? "")).toBe(
      fs.realpathSync(normalizationStringCoerce.srcFile),
    );
    expect(fs.realpathSync(aliases["@mo/terminal-core"] ?? "")).toBe(
      fs.realpathSync(terminalCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@mo/terminal-core/theme"] ?? "")).toBe(
      fs.realpathSync(terminalCoreTheme.srcFile),
    );
    expect(fs.realpathSync(aliases["@mo/net-policy"] ?? "")).toBe(
      fs.realpathSync(netPolicy.srcFile),
    );
    expect(fs.realpathSync(aliases["@mo/net-policy/ip"] ?? "")).toBe(
      fs.realpathSync(netPolicyIp.srcFile),
    );
    expect(fs.realpathSync(aliases["@mo/model-catalog-core/provider-id"] ?? "")).toBe(
  ...
  it("aliases workspace package subpaths to dist when available", () => {
    ...
    expect(fs.realpathSync(aliases["@mo/gateway-client/readiness"] ?? "")).toBe(
      fs.realpathSync(gatewayClient.distFile),
    );
    expect(fs.realpathSync(aliases["@mo/gateway-protocol/connect-error-details"] ?? "")).toBe(
      fs.realpathSync(gatewayProtocol.distFile),
    );
    expect(fs.realpathSync(aliases["@mo/markdown-core/render"] ?? "")).toBe(
      fs.realpathSync(markdownCore.distFile),
    );
    expect(fs.realpathSync(aliases["@mo/media-generation-core/catalog"] ?? "")).toBe(
      fs.realpathSync(mediaGenerationCore.distFile),
    );
    expect(fs.realpathSync(aliases["@mo/media-core/read-response-with-limit"] ?? "")).toBe(
      fs.realpathSync(mediaCoreRootDistFile),
    );
    expect(fs.realpathSync(aliases["@mo/acp-core/normalize-text"] ?? "")).toBe(
      fs.realpathSync(acpCoreRootDistFile),
    );
    expect(fs.realpathSync(aliases["@mo/normalization-core/record-coerce"] ?? "")).toBe(
      fs.realpathSync(normalizationCoreRootDistFile),
    );
    expect(fs.realpathSync(aliases["@mo/terminal-core/links"] ?? "")).toBe(
      fs.realpathSync(terminalCoreRootDistFile),
    );
    expect(fs.realpathSync(aliases["@mo/net-policy/redact-sensitive-url"] ?? "")).toBe(
      fs.realpathSync(netPolicy.distFile),
    );
    expect(
      fs.realpathSync(aliases["@mo/model-catalog-core/provider-model-id-normalize"] ?? ""),
  ...
  it("derives acp-core aliases from packaged root dist when package metadata is absent", () => {
    ...
    expect(fs.realpathSync(aliases["@mo/acp-core/runtime/errors"] ?? "")).toBe(
  ...
  it("aliases bundled plugin package public surfaces for source plugin transforms", () => {
    ...
    expect(fs.realpathSync(aliases["@mo/slack/api.js"] ?? "")).toBe(
      fs.realpathSync(sourceApiPath),
    );
    expect(fs.realpathSync(aliases["@mo/slack/runtime-api.js"] ?? "")).toBe(
      fs.realpathSync(sourceRuntimeApiPath),
    );
    expect(aliases["@mo/slack/test-api.js"]).toBeUndefined();
    expect(aliases["@mo/slack/internal.js"]).toBeUndefined();
  });
  ...
  it("aliases bundled plugin package test surfaces only in private QA mode", () => {
    ...
    expect(fs.realpathSync(aliases["@mo/slack/test-api.js"] ?? "")).toBe(
  ...
  it("falls back to source plugin-sdk subpath aliases when dist chunks are stale", () => {
    ...
    expect(fs.realpathSync(distAliases["mo/plugin-sdk/provider-entry"] ?? "")).toBe(
  ...
  it("resolves plugin-sdk aliases for user-installed plugins via the running mo argv hint", () => {
    const {
      externalPluginEntry,
      externalPluginRoot,
      fixture,
    ...
    const aliases = withCwd(externalPluginRoot, () =>
      withEnv({ NODE_ENV: undefined }, () =>
        buildPluginLoaderAliasMap(externalPluginEntry, path.join(fixture.root, "mo.mjs")),
      ),
    );
  ...
  it("resolves plugin-sdk aliases for user-installed plugins via moduleUrl hint", () => {
    ...
    // This covers installations where argv1 does not resolve to the mo root
    // (e.g. single-binary distributions or custom process launchers).
    // Use mo.mjs which is created by createPluginSdkAliasFixture (bin+marker mode).
    // Use fixture.root as cwd so process.cwd() fallback also resolves to fixture, not the
    // real mo repo root in the test runner environment.
    const loaderModuleUrl = pathToFileURL(path.join(fixture.root, "mo.mjs")).href;
    ...
    // runner binary, inside the mo repo), which resolves before moduleUrl is checked.
  ...
  it("scopes plugin loader module config by plugin-sdk resolution", () => {
    ...
    expect(fs.realpathSync(auto.aliasMap["mo/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(sourceRootAlias),
    );
    expect(fs.realpathSync(dist.aliasMap["mo/plugin-sdk"] ?? "")).toBe(
  ...
  it("loads source runtime shims through the non-native module loading boundary", async () => {
    ...
    fs.writeFileSync(
      ...
      `import { resolveOutboundSendDep } from "@mo/plugin-sdk/channel-outbound";
...
`,
    ...
    const withAlias = createJiti(sourceLoaderBaseUrl, {
      ...buildPluginLoaderJitiOptions({
        "mo/plugin-sdk/channel-outbound": copiedChannelRuntimeShim,
        "@mo/plugin-sdk/channel-outbound": copiedChannelRuntimeShim,
      }),
      tryNative: false,
    });
  ...
  it("falls back to ancestor runtime candidates when package-root markers are unavailable", () => {
    const root = makeTempDir();
    const distFile = path.join(root, "dist", "plugins", "runtime", "index.js");
    const loaderCachePath = path.join(root, ".cache", "tsx", "mo-loader.js");
    mkdirSafeDir(path.dirname(distFile));
  ...
  it("uses the default startup argv hint for runtime fallback when argv1 is omitted", () => {
    ...
    const loaderCachePath = path.join(loaderCacheRoot, "tsx", "mo-loader.js");
    ...
    process.argv[1] = path.join(root, "bin", "mo");
    try {
      expect(
        resolvePluginRuntimeModulePath({
          modulePath: loaderCachePath,
  ...
  it("resolves runtime fallback through symlinked startup argv", () => {
    const root = makeTempDir();
    const distFile = path.join(root, "dist", "plugins", "runtime", "index.js");
    const binFile = path.join(root, "bin", "mo");
    const shimRoot = makeTempDir();
    const shimFile = path.join(shimRoot, "bin", "mo");
    const loaderCachePath = path.join(makeTempDir(), "tsx", "mo-loader.js");
    mkdirSafeDir(path.dirname(distFile));
  ...
  it("resolves runtime fallback through npm .bin startup argv", () => {
    const root = makeTempDir();
    const packageRoot = path.join(root, "node_modules", "mo");
    ...
    const binFile = path.join(root, "node_modules", ".bin", "mo");
    const loaderCachePath = path.join(makeTempDir(), "tsx", "mo-loader.js");
    mkdirSafeDir(path.dirname(distFile));
...
describe("buildPluginLoaderAliasMap memoization", () => {
  ...
  it("returns different references for different modulePath inputs", () => {
    ...
    expect(aliasA["mo/plugin-sdk"]).not.toBe(aliasB["mo/plugin-sdk"]);
  });
  ...
  it("returns different references when an explicit dev source root differs", () => {
    ...
    expect(fs.realpathSync(stableAliases["mo/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(stableRootAlias),
    );
    expect(fs.realpathSync(devAliases["mo/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(devRootAlias),
    );
  });
  ...
  it("does not reuse a public alias map after private qa aliases are enabled", () => {
    ...
    const publicAliases = withEnv({ MO_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      buildPluginLoaderAliasMap(entry),
    );
    const privateAliases = withEnv({ MO_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      buildPluginLoaderAliasMap(entry),
    );

    expect(publicAliases).not.toBe(privateAliases);
    expect(publicAliases["mo/plugin-sdk/qa-runtime"]).toBeUndefined();
    expect(fs.realpathSync(privateAliases["mo/plugin-sdk/qa-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceQaRuntimePath),
    );
  });
  ...
  it("does not reuse a development alias map in production mode", () => {
    ...
    expect(fs.realpathSync(developmentAliases["mo/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(sourceRootAlias),
    );
    expect(fs.realpathSync(productionAliases["mo/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(distRootAlias),
    );
  });
...
describe("buildPluginLoaderJitiOptions", () => {
  it("scopes jiti fs cache by OpenClaw package version and install metadata", () => {
    ...
    expect(fsCache).toContain(path.join(tmpDir, "jiti", "mo", "1.2.3-beta.4") + path.sep);
  ...
  it("preserves jiti's tmpdir guard when TMPDIR resolves to cwd", () => {
    ...
    expect(guardedFsCache).toContain(path.join("jiti", "mo", "1.2.3-beta.4") + path.sep);
    expect(guardedFsCache.startsWith(path.join(root, "jiti") + path.sep)).toBe(false);
    expect(respectedFsCache).toContain(
      path.join(root, "jiti", "mo", "1.2.3-beta.4") + path.sep,
    );
  });
  ...
  it("adds the versioned fs cache directory to plugin loader jiti options", () => {
    ...
    const options = withEnv({ TMPDIR: tmpDir }, () =>
      buildPluginLoaderJitiOptions(
        { "mo/plugin-sdk": path.join(root, "dist", "plugin-sdk", "root-alias.cjs") },
        { modulePath: path.join(root, "dist", "plugins", "loader.js") },
      ),
    );

    expect(options.fsCache).toContain(path.join(tmpDir, "jiti", "mo", "2.0.0"));
  });
  ...
  it("pre-normalizes and marks alias maps for source transforms", () => {
    ...
    const aliasMap = {
      "mo/plugin-sdk/core": "/repo/src/plugin-sdk/core.ts",
      "mo/plugin-sdk": "/repo/src/plugin-sdk/root-alias.cjs",
      "@mo/plugin-sdk": "/repo/src/plugin-sdk/root-alias.cjs",
    };
