...
describe("removePluginFromConfig", () => {
  ...
  it("removes absolute load path for a workspace-relative install source path", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(tempRoot, "mo-uninstall-portable-source-"));
    try {
...
describe("uninstallPlugin", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      config: createPluginConfig({
        ...
        loadPaths: ["/missing/mo/plugin", "/keep/this/plugin"],
      }),
      expectedActions: {
        entry: false,
        install: true,
  ...
  it("uninstalls npm-managed packages through npm before deleting the package directory", async () => {
    ...
    const pluginDir = path.join(npmRoot, "node_modules", "@mo", "kitchen-sink");
    ...
    const plan = planPluginUninstall({
      config: createPluginConfig({
        entries: createSinglePluginEntries("mo-kitchen-sink-fixture"),
        installs: {
          "mo-kitchen-sink-fixture": {
            source: "npm",
            spec: "@mo/kitchen-sink@1.0.0",
            installPath: pluginDir,
          },
        },
      }),
      pluginId: "mo-kitchen-sink-fixture",
      deleteFiles: true,
      extensionsDir,
    });
    ...
    expect(applied).toEqual({ directoryRemoved: true, warnings: [] });
    expectNpmUninstallCommand({ packageName: "@mo/kitchen-sink", npmRoot });
    await expectPathAccessState(pluginDir, "missing");
  });
  ...
  it("repairs remaining npm plugin mo peer links after npm uninstall prunes them", async () => {
    const stateDir = path.join(tempDir, "state");
    const npmRoot = path.join(stateDir, "npm");
    ...
    const peerLink = path.join(peerPluginDir, "node_modules", "mo");
    ...
    await expectPathAccessState(path.join(npmRoot, "node_modules", "mo"), "missing");
  ...
  it("prunes managed peer dependencies after their owning npm plugin is uninstalled", async () => {
    ...
    expect(rootManifest.mo?.managedPeerDependencies ?? []).not.toContain("runtime-peer");
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("returns managed install path for copied path installs", () => {
    const extensionsDir = path.join(os.tmpdir(), "mo-uninstall-safe");
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    ...
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        ...
        installPath: "/tmp/not-mo-plugin-install/my-plugin",
      },
      extensionsDir,
    });
  ...
  it("uses configured installPath when npm installed it under the managed npm root", () => {
    const stateDir = path.join(os.tmpdir(), "mo-uninstall-safe");
    const extensionsDir = path.join(stateDir, "extensions");
    const installPath = path.join(stateDir, "npm", "node_modules", "@mo", "kitchen-sink");
  ...
  it("uses configured installPath when npm installed it under a managed npm project", () => {
    ...
    const installPath = path.join(npmRoot, "node_modules", "@mo", "kitchen-sink");
    ...
    expect(
      resolveUninstallDirectoryTarget({
        ...
        installRecord: {
          source: "npm",
          spec: "@mo/kitchen-sink@latest",
          installPath: path.join(os.tmpdir(), "npm", "node_modules", "@mo", "kitchen-sink"),
        },
        extensionsDir,
      }),
    ).toBe(resolvePluginInstallDir("mo-kitchen-sink-fixture", extensionsDir));
  });

  it("uses configured installPath when it is under the recorded managed extensions root", () => {
    const currentExtensionsDir = path.join(os.tmpdir(), "mo-uninstall-current", "extensions");
    const recordedExtensionsDir = path.join(
      os.tmpdir(),
      "mo-uninstall-recorded",
      "extensions",
    );
    const installPath = resolvePluginInstallDir("my-plugin", recordedExtensionsDir);
