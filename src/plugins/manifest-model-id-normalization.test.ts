...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-model-id-normalization-"));
  tempDirs.push(dir);
  return dir;
}
  ...
  writePersistedInstalledPluginIndexSync(
    {
      ...
      plugins: [
        {
          pluginId: "normalizer",
          manifestPath: path.join(params.pluginDir, "mo.plugin.json"),
...
function createCurrentSnapshot(params: {
  ...
  const index: InstalledPluginIndex = {
    ...
    plugins: [
      {
        pluginId: "normalizer",
        manifestPath: `/tmp/normalizer-${params.manifestHash}/mo.plugin.json`,
...
describe("manifest model id normalization", () => {
  ...
  it("keeps process metadata stable across manifest edits and reflects state-dir changes", () => {
    ...
    setTestEnvValue("MO_STATE_DIR", stateDirA);
    deleteTestEnvValue("MO_HOME");
    setTestEnvValue("MO_DISABLE_BUNDLED_PLUGINS", "1");
    deleteTestEnvValue("MO_BUNDLED_PLUGINS_DIR");
    ...
    setTestEnvValue("MO_STATE_DIR", stateDirB);
    clearPluginMetadataLifecycleCaches();
