...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "mo", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".mo", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".mo", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("ignores untrusted explicit env override for the stock source root", () => {
    const homeDir = path.resolve(path.sep, "tmp", "mo-home");
    const rawEnv = {
    ...
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".mo", "extensions"),
      },
    });
  });
});
