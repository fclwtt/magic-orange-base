...
describe("loadEnabledBundleMcpConfig", () => {
  ...
  it("uses a provided manifest registry instead of rediscovering bundle plugins", async () => {
    const homeDir = await tempHarness.createTempDir("mo-bundle-mcp-home-");
    const workspaceDir = await tempHarness.createTempDir("mo-bundle-mcp-workspace-");
