...
function isPackageRootOpenClawPeerSymlink(segments: string[]): boolean {
  return (
    (segments.length === 2 && segments[0] === "node_modules" && segments[1] === "mo") ||
    (segments.length === 3 &&
...
function collectManifestRuntimeDependencyNames(manifest: PackageManifest): string[] {
  ...
  for (const dependencyName of Object.keys(manifest.peerDependencies ?? {})) {
    if (dependencyName !== "mo" && isInstallScannableDependencyName(dependencyName)) {
      dependencyNames.add(dependencyName);
    }
  }
...
function resolvePolicySource(params: {
  ...
  if (params.requestKind === "skill-install") {
    switch (params.origin?.type) {
      case "clawhub":
        return { kind: "clawhub", authority: "mo", mutable: false, network: true };
      case "git":
        return {
          kind: "git",
      ...
      case "mo-bundled":
        return { kind: "bundled", authority: "mo", mutable: false, network: false };
      case "mo-managed":
      case "mo-extra":
        return { kind: "managed", authority: "mo", mutable: false, network: false };
      default:
...
function shouldBypassOpenClawInstallFriction(params: {
  ...
  return (
    source.authority === "mo" && (source.kind === "bundled" || source.kind === "managed")
  );
}
