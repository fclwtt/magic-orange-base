...
import { closeOpenClawStateDatabaseForTest } from "../state/mo-state-db.js";
...
describe("official external plugin catalog", () => {
  ...
  it("ships the official plugin catalog as a feed-shaped bundled fallback", () => {
    ...
    expect(officialExternalPluginCatalog).toMatchObject({
      schemaVersion: 1,
      id: "mo-official-external-plugins",
      sequence: 1,
    });
  ...
  it("accepts live ClawHub marketplace entries with trusted install candidates", () => {
            ...
            candidates: [
              {
                ...
                package: "@expediagroup/expedia-mo",
                version: "1.0.4",
                integrity:
    ...
    expect(resolveOfficialExternalPluginId(entry)).toBe("@expediagroup/expedia-mo");
    expect(resolveOfficialExternalPluginInstall(entry)).toEqual({
      clawhubSpec: "clawhub:@expediagroup/expedia-mo@1.0.4",
  ...
  it("loads a hosted feed with conditional headers and checksum metadata", async () => {
    const body = JSON.stringify({
      ...
      entries: [
        {
          name: "@mo/hosted-proof",
          kind: "plugin",
          mo: {
            plugin: { id: "hosted-proof", label: "Hosted Proof" },
            install: { npmSpec: "@mo/hosted-proof", defaultChoice: "npm" },
          },
        },
      ],
    });
    ...
    expect(result.entries.map((entry) => entry.name)).toEqual(["@mo/hosted-proof"]);
  ...
  it("uses the default local feed profile for hosted catalog loading", async () => {
    const body = JSON.stringify({
      ...
      entries: [
        {
          name: "@mo/default-profile-proof",
          kind: "plugin",
          mo: { plugin: { id: "default-profile-proof" } },
        },
      ],
    });
    ...
    expect(result.source).toBe("hosted");
    expect(result.entries.map((entry) => entry.name)).toEqual(["@mo/default-profile-proof"]);
  });
  ...
  it("accepts the live ClawHub feed source ref by default", async () => {
    const body = JSON.stringify({
      ...
      entries: [
        {
          type: "plugin",
          id: "@mo/live-feed-proof",
          title: "Live Feed Proof",
          version: "1.0.0",
          state: "available",
          publisher: { id: "mo", trust: "official" },
          install: {
            candidates: [
              {
                sourceRef: "public-clawhub",
                package: "@mo/live-feed-proof",
                version: "1.0.0",
                integrity:
                  "sha256:b355dda04403becaab8bbab069fd1e7b0578262e7459e598cc5b19615b5bdab9",
              },
    ...
    expect(result.source).toBe("hosted");
    expect(result.entries.map((entry) => entry.id)).toEqual(["@mo/live-feed-proof"]);
    expect(resolveOfficialExternalPluginInstall(result.entries[0])).toEqual({
      clawhubSpec: "clawhub:@mo/live-feed-proof@1.0.0",
      defaultChoice: "clawhub",
      expectedIntegrity: "sha256-s1XdoEQDvsqri7qwaf0eewV4Ji50WeWYzFsZYVtb2rk=",
    });
  });
  ...
  it("loads hosted catalog profiles from OpenClaw config", async () => {
    const config = {
      marketplaces: {
        feeds: { acme: { url: "https://packages.acme.example/mo/feed" } },
    ...
    const result = await loadConfiguredHostedOfficialExternalPluginCatalogEntries(config, {
      feedProfile: "acme",
      fetchImpl: vi.fn(async (url: RequestInfo | URL) => {
        expect(expectRequestUrl(url)).toBe("https://packages.acme.example/mo/feed");
        return new Response(body, { status: 200 });
      }),
      snapshotStore: null,
    });
  ...
  it("keeps direct hosted feed URL overrides constrained to the public allowlist", async () => {
    ...
    const result = await loadHostedOfficialExternalPluginCatalogEntries({
      feedUrl: "https://packages.acme.example/mo/feed",
      fetchImpl,
      snapshotStore: null,
    });
  ...
  it("requires manifest install source refs when the default feed profile URL is overridden", async () => {
    ...
    const result = await loadHostedOfficialExternalPluginCatalogEntries({
      catalogConfig: {
        feeds: { "clawhub-public": { url: "https://packages.acme.example/mo/feed" } },
        sources: { "acme-npm": { type: "npm", registry: "https://packages.acme.example/npm/" } },
      },
      fetchImpl: vi.fn(async (url: RequestInfo | URL) => {
        expect(expectRequestUrl(url)).toBe("https://packages.acme.example/mo/feed");
  ...
  it("preserves default feed manifest installs for direct default hosted feed URL refreshes", async () => {
    const body = JSON.stringify({
      ...
      entries: [
        {
          name: "@mo/direct-default-missing-source-ref",
          kind: "plugin",
          mo: {
            plugin: { id: "direct-default-missing-source-ref" },
            install: { npmSpec: "@mo/direct-default-missing-source-ref" },
          },
        },
      ],
    });
    ...
    expect(result.source).toBe("hosted");
    expect(result.entries.map((entry) => entry.name)).toEqual([
      "@mo/direct-default-missing-source-ref",
    ]);
  });
  ...
  it("requires manifest install source refs for custom local feed profiles", async () => {
    ...
    const implicitNameInstall = {
      ...
      kind: "plugin",
      mo: { plugin: { id: "implicit-name-install" } },
    };
  ...
  it("filters hosted feed entries that reference unknown local source profiles", async () => {
    const knownEntry = {
      name: "@mo/source-ref-known",
      kind: "plugin",
      install: {
        candidates: [{ sourceRef: "public-clawhub", package: "@mo/source-ref-known" }],
      },
      mo: { plugin: { id: "source-ref-known" } },
    };
    const unknownEntry = {
      name: "@mo/source-ref-unknown",
      kind: "plugin",
      install: {
        candidates: [{ sourceRef: "attacker-npm", package: "@mo/source-ref-unknown" }],
      },
      mo: { plugin: { id: "source-ref-unknown" } },
    };
    const missingEntry = {
      name: "@mo/source-ref-missing",
      kind: "plugin",
      install: { candidates: [{ package: "@mo/source-ref-missing" }] },
      mo: { plugin: { id: "source-ref-missing" } },
    };
    const manifestInstallWithoutSourceRef = {
      name: "@mo/source-ref-manifest-missing",
      kind: "plugin",
      install: {
        candidates: [
          { sourceRef: "public-clawhub", package: "@mo/source-ref-manifest-missing" },
        ],
      },
      mo: {
        plugin: { id: "source-ref-manifest-missing" },
        install: { npmSpec: "@mo/source-ref-manifest-missing" },
      },
    };
    const body = JSON.stringify({
      schemaVersion: 1,
    ...
    ).toEqual(["@mo/source-ref-known", "@mo/source-ref-manifest-missing"]);
    ...
    expect(result.entries.map((entry) => entry.name)).toEqual([
      "@mo/source-ref-known",
      "@mo/source-ref-manifest-missing",
    ]);
  });
  ...
  it("writes a validated hosted feed snapshot after a successful fetch", async () => {
    ...
    const body = JSON.stringify({
      ...
      entries: [
        {
          name: "@mo/snapshot-write-proof",
          kind: "plugin",
          mo: { plugin: { id: "snapshot-write-proof" } },
        },
      ],
    });
  ...
  it("fails explicit refreshes when required snapshot persistence fails", async () => {
    const body = JSON.stringify({
      ...
      entries: [
        {
          name: "@mo/snapshot-write-fail-proof",
          kind: "plugin",
          mo: { plugin: { id: "snapshot-write-fail-proof" } },
        },
      ],
    });
    const snapshotStore = {
  ...
  it("reads the latest accepted snapshot in offline mode without fetching", async () => {
    ...
    const body = JSON.stringify({
      ...
      entries: [
        {
          name: "@mo/offline-snapshot-proof",
          kind: "plugin",
          mo: { plugin: { id: "offline-snapshot-proof" } },
        },
      ],
    });
    ...
    expect(result.entries.map((entry) => entry.name)).toEqual(["@mo/offline-snapshot-proof"]);
  ...
  it("persists hosted feed snapshots in OpenClaw state for HTTP 304 reuse", async () => {
    const stateDir = mkdtempSync(path.join(os.tmpdir(), "mo-hosted-catalog-"));
    try {
      const body = JSON.stringify({
        schemaVersion: 1,
        id: "mo-official-external-plugins",
        ...
        entries: [
          {
            name: "@mo/sqlite-snapshot-proof",
            kind: "plugin",
            mo: { plugin: { id: "sqlite-snapshot-proof" } },
          },
        ],
      });
  ...
  it("reads and updates hosted catalog snapshots in the SQLite store", async () => {
    const stateDir = mkdtempSync(path.join(os.tmpdir(), "mo-hosted-store-"));
    try {
  ...
  it("uses the last known good snapshot when the hosted feed returns HTTP 304", async () => {
    const body = JSON.stringify({
      ...
      entries: [
        {
          name: "@mo/snapshot-proof",
          kind: "plugin",
          mo: { plugin: { id: "snapshot-proof" } },
        },
      ],
    });
    const seeded = await loadHostedOfficialExternalPluginCatalogEntries({
    ...
    expect(result.source).toBe("hosted-snapshot");
    expect(result.entries.map((entry) => entry.name)).toEqual(["@mo/snapshot-proof"]);
    if (result.source === "hosted-snapshot") {
      expect(result.error).toContain("HTTP 304");
  ...
  it("does not use a stale snapshot when HTTP 304 validators do not match", async () => {
    const body = JSON.stringify({
      ...
      entries: [
        {
          name: "@mo/stale-snapshot-proof",
          kind: "plugin",
          mo: { plugin: { id: "stale-snapshot-proof" } },
        },
      ],
    });
    const seeded = await loadHostedOfficialExternalPluginCatalogEntries({
  ...
  it("uses a valid snapshot before bundled fallback when hosted validation fails", async () => {
    const body = JSON.stringify({
      ...
      entries: [
        {
          name: "@mo/snapshot-validation-proof",
          kind: "plugin",
          mo: { plugin: { id: "snapshot-validation-proof" } },
        },
      ],
    });
    const seeded = await loadHostedOfficialExternalPluginCatalogEntries({
  ...
  it("does not use a stale snapshot when hosted validation fails with unmatched validators", async () => {
    const body = JSON.stringify({
      ...
      entries: [
        {
          name: "@mo/stale-validation-snapshot-proof",
          kind: "plugin",
          mo: { plugin: { id: "stale-validation-snapshot-proof" } },
        },
      ],
    });
    const seeded = await loadHostedOfficialExternalPluginCatalogEntries({
  ...
  it("does not use a snapshot that violates the expected checksum", async () => {
    const body = JSON.stringify({
      ...
      entries: [
        {
          name: "@mo/snapshot-pin-proof",
          kind: "plugin",
          mo: { plugin: { id: "snapshot-pin-proof" } },
        },
      ],
    });
    const seeded = await loadHostedOfficialExternalPluginCatalogEntries({
  ...
  it("prefers feed install candidates before legacy install metadata", () => {
    expect(
      ...
      clawhubSpec: "clawhub:@mo/candidate-package@1.2.3",
      defaultChoice: "clawhub",
      expectedIntegrity: "sha256-s1XdoEQDvsqri7qwaf0eewV4Ji50WeWYzFsZYVtb2rk=",
      minHostVersion: ">=2026.6.1",
      allowInvalidConfigRecovery: true,
  ...
  it("lists the externalized provider and capability plugins with install metadata", () => {
    const providers = [
      ["arcee", "@mo/arcee-provider"],
      ["cerebras", "@mo/cerebras-provider"],
      ["chutes", "@mo/chutes-provider"],
      ["cloudflare-ai-gateway", "@mo/cloudflare-ai-gateway-provider"],
      ["deepinfra", "@mo/deepinfra-provider"],
      ["deepseek", "@mo/deepseek-provider"],
      ["groq", "@mo/groq-provider"],
      ["kilocode", "@mo/kilocode-provider"],
      ["kimi", "@mo/kimi-provider"],
      ["qianfan", "@mo/qianfan-provider"],
      ["qwen", "@mo/qwen-provider"],
    ] as const;
    const plugins = [
      ["exa", "@mo/exa-plugin"],
      ...
      ["parallel", "@mo/parallel-plugin"],
      ["perplexity", "@mo/perplexity-plugin"],
    ] as const;
    const newlyExternalized = [
      ["clickclack", "@mo/clickclack"],
      ["fireworks", "@mo/fireworks-provider"],
      ["irc", "@mo/irc"],
      ["mattermost", "@mo/mattermost"],
      ["moonshot", "@mo/moonshot-provider"],
      ["searxng", "@mo/searxng-plugin"],
      ["signal", "@mo/signal"],
      ["sms", "@mo/sms"],
      ["tavily", "@mo/tavily-plugin"],
      ["tencent", "@mo/tencent-provider"],
      ["venice", "@mo/venice-provider"],
      ["vercel-ai-gateway", "@mo/vercel-ai-gateway-provider"],
      ["zai", "@mo/zai-provider"],
    ] as const;
  ...
  it("advertises StepFun with its ClawHub package and plugin API floor", () => {
    expect(resolveOfficialExternalPluginInstall(expectCatalogEntry("stepfun"))).toEqual({
      clawhubSpec: "clawhub:@mo/stepfun-provider",
      npmSpec: "@mo/stepfun-provider",
      defaultChoice: "npm",
      minHostVersion: ">=2026.6.9",
    });
  });
  ...
  it("resolves third-party channel lookup aliases to published plugin ids", () => {
    ...
    const wecomByPlugin = expectCatalogEntry("wecom-mo-plugin");
    const yuanbaoByChannel = expectCatalogEntry("yuanbao");

    expect(resolveOfficialExternalPluginId(wecomByChannel)).toBe("wecom-mo-plugin");
    expect(resolveOfficialExternalPluginId(wecomByPlugin)).toBe("wecom-mo-plugin");
    expect(resolveOfficialExternalPluginInstall(wecomByChannel)?.npmSpec).toBe(
      "@wecom/wecom-mo-plugin@2026.5.7",
    );
    expect(resolveOfficialExternalPluginId(yuanbaoByChannel)).toBe("mo-plugin-yuanbao");
    expect(resolveOfficialExternalPluginInstall(yuanbaoByChannel)?.npmSpec).toBe(
      "mo-plugin-yuanbao@2.15.0",
    );
  });
  ...
  it("keeps official launch package specs on the production package names", () => {
    ...
    expect(resolveOfficialExternalPluginInstall(expectCatalogEntry("diffs-language-pack"))).toEqual(
      {
        npmSpec: "@mo/diffs-language-pack",
        clawhubSpec: "clawhub:@mo/diffs-language-pack",
        defaultChoice: "npm",
        minHostVersion: ">=2026.5.27",
      },
    );
    expect(resolveOfficialExternalPluginInstall(expectCatalogEntry("llama-cpp"))?.npmSpec).toBe(
      "@mo/llama-cpp-provider",
    );
  });
  ...
  it("lists GMI Cloud as an official external provider", () => {
    ...
    expect(resolveOfficialExternalPluginInstall(gmi)).toEqual({
      clawhubSpec: "clawhub:@mo/gmi-provider",
      npmSpec: "@mo/gmi-provider",
      defaultChoice: "npm",
      minHostVersion: ">=2026.6.8",
    });
  });
  ...
  it("lists Cohere as an official external provider", () => {
    ...
    expect(resolveOfficialExternalPluginInstall(cohere)).toEqual({
      clawhubSpec: "clawhub:@mo/cohere-provider",
      npmSpec: "@mo/cohere-provider",
      defaultChoice: "npm",
      minHostVersion: ">=2026.6.8",
    });
  });
  ...
  it("keeps Groq available through the cold-install auth catalog", () => {
    ...
    const authChoice = groq.mo?.providers?.find((provider) => provider.id === "groq")
      ?.authChoices?.[0];
