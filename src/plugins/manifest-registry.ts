...
function buildRecord(params: {
  ...
  return {
    ...
    format: params.candidate.format ?? "mo",
...
function pushNonBundledChannelConfigDescriptorDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    pluginId: sanitizeForLog(params.record.id),
    source: sanitizeForLog(params.record.manifestPath),
    message: `channel plugin manifest declares ${safeMissingChannels.join(", ")} without channelConfigs metadata; add mo.plugin.json#channelConfigs so config schema and setup surfaces work before runtime loads. Channels without channelConfigs still appear in channel listings, but setup UI may be limited.`,
  });
}
...
export function loadPluginManifestRegistry(
  ...
  for (const candidate of candidates) {
    ...
    const isBundleRecord = (candidate.format ?? "mo") === "bundle";
    const manifestRes:
