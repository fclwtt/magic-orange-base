...
function makeTempDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-plugin-loader-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("createPluginModuleLoader", () => {
  it("loads bundled JavaScript without creating a module loader", async () => {
    ...
    process.env.MO_BUNDLED_PLUGINS_DIR = pluginRoot;
  ...
  it("loads packaged JavaScript without creating a module loader", async () => {
    ...
    process.env.MO_BUNDLED_PLUGINS_DIR = makeTempDir();
