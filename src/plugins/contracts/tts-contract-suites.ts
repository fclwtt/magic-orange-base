...
import type { ResolvedTtsConfig, SpeechProviderPlugin } from "mo/plugin-sdk/speech-core";
import { withEnv, withEnvAsync } from "mo/plugin-sdk/test-env";
...
type TtsRuntimeModule = typeof import("mo/plugin-sdk/tts-runtime");
type TtsCoreModule = typeof import("mo/plugin-sdk/speech-core");
...
let completeSimple: typeof import("mo/plugin-sdk/llm").completeSimple;
...
async function loadTtsCore(): Promise<TtsCoreModule> {
  ttsCorePromise ??= import("mo/plugin-sdk/speech-core");
...
async function setupSummarizationMocks() {
  ...
  ({ completeSimple } = await import("mo/plugin-sdk/llm"));
...
export function describeTtsAutoApplyContract() {
  describe("tts auto-apply contract", () => {
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.MO_TTS_PREFS;
      process.env.MO_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.MO_TTS_PREFS = prevPrefs;
      }
    };
