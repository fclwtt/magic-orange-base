...
function makeRepoFixture(): string {
  const repoRoot = mkdtempSync(join(tmpdir(), "mo-config-boundary-"));
...
describe("config boundary guard", () => {
  ...
  it("flags broad config-runtime barrel imports in production code", () => {
    ...
    writeFixture(
      ...
      "extensions/telegram/src/index.ts",
      [
        'import type { OpenClawConfig } from "mo/plugin-sdk/config-runtime";',
        'import { requireRuntimeConfig } from "mo/plugin-sdk/config-runtime";',
        'type Loader = typeof import("mo/plugin-sdk/config-runtime").getRuntimeConfig;',
    ...
    expect(collectDeprecatedInternalConfigApiViolations({ repoRoot })).toEqual([
      "extensions/telegram/src/index.ts:1 use narrow plugin-sdk config subpaths instead of mo/plugin-sdk/config-runtime",
      "extensions/telegram/src/index.ts:2 use narrow plugin-sdk config subpaths instead of mo/plugin-sdk/config-runtime",
      "extensions/telegram/src/index.ts:3 use narrow plugin-sdk config subpaths instead of mo/plugin-sdk/config-runtime",
    ]);
  });
  ...
  it("flags broad config-runtime test mocks outside compat guard fixtures", () => {
    ...
    writeFixture(
      ...
      'vi.mock("mo/plugin-sdk/config-runtime", () => ({}));',
    );

    expect(collectDeprecatedInternalConfigApiViolations({ repoRoot })).toEqual([
      "extensions/telegram/src/index.test.ts:1 use narrow plugin-sdk config subpaths instead of mo/plugin-sdk/config-runtime",
    ]);
  });
  ...
  it("allows narrow config SDK subpaths in production code", () => {
    ...
    writeFixture(
      ...
      [
        ...
        'import { requireRuntimeConfig } from "mo/plugin-sdk/plugin-config-runtime";',
        'type Loader = typeof import("mo/plugin-sdk/runtime-config-snapshot").getRuntimeConfig;',
