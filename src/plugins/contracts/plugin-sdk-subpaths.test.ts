...
import * as commandAuthSdk from "mo/plugin-sdk/command-auth";
import type {
  ChannelMessageActionContext as CoreChannelMessageActionContext,
  OpenClawPluginApi as CoreOpenClawPluginApi,
  PluginRuntime as CorePluginRuntime,
} from "mo/plugin-sdk/core";
import * as providerEntrySdk from "mo/plugin-sdk/provider-entry";
import type { GetReplyOptions as ReplyRuntimeGetReplyOptions } from "mo/plugin-sdk/reply-runtime";
import * as zalouserSdk from "mo/plugin-sdk/zalouser";
...
describe("plugin-sdk subpath exports", () => {
  ...
  beforeAll(() => {
    deprecatedChannelRuntimeMatches = findRepoFilesContaining({
      ...
      pattern:
        /(?:from\s+|import\s+(?:type\s+)?|import\s*\(\s*)["']mo\/plugin-sdk\/channel-runtime(?=["'])/u,
      exclude: [
  ...
  it("keeps runtime entry subpaths importable", async () => {
    const coreSdk = await importResolvedPluginSdkSubpath("mo/plugin-sdk/core");
    const channelActionsSdk = await importResolvedPluginSdkSubpath(
      "mo/plugin-sdk/channel-actions",
    );
    const globalSingletonSdk = await importResolvedPluginSdkSubpath(
      "mo/plugin-sdk/global-singleton",
    );
    const pluginEntrySdk = await importResolvedPluginSdkSubpath("mo/plugin-sdk/plugin-entry");
    const channelLifecycleSdk = await importResolvedPluginSdkSubpath(
      "mo/plugin-sdk/channel-lifecycle",
    );
    ...
    const channelReplyPipelineSdk = await importResolvedPluginSdkSubpath(
      "mo/plugin-sdk/channel-reply-pipeline",
    );
    const representativeModules = [];
    for (const id of representativeRuntimeSmokeSubpaths) {
      representativeModules.push(await importResolvedPluginSdkSubpath(`mo/plugin-sdk/${id}`));
    }
