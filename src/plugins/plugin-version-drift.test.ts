...
function npmRecord(
  ...
  const resolvedName = overrides.resolvedName ?? "@mo/whatsapp";
  return {
    source: "npm",
...
describe("detectPluginVersionDrift", () => {
  it("returns empty drifts when all externalized plugins match the gateway", () => {
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        ...
        discord: npmRecord("2026.5.4", { resolvedName: "@mo/discord" }),
      },
    });
  ...
  it("treats a build-qualifier suffix on either side as matching (2026.5.4-1 ≈ 2026.5.4)", () => {
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        ...
        discord: npmRecord("2026.5.4-1", { resolvedName: "@mo/discord" }),
      },
    });
  ...
  it("ignores official catalog installs pinned to independent package versions", () => {
    const result = detectPluginVersionDrift({
      gatewayVersion: "2026.5.4",
      installRecords: {
        "mo-plugin-yuanbao": npmRecord("2.13.1", {
          resolvedName: "mo-plugin-yuanbao",
          spec: "mo-plugin-yuanbao@2.13.1",
        }),
      },
    });
  ...
  it("ignores exact catalog pins even when the pin matches the gateway version", () => {
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        "wecom-mo-plugin": npmRecord("2026.5.6", {
          resolvedName: "@wecom/wecom-mo-plugin",
          spec: "@wecom/wecom-mo-plugin@2026.5.6",
        }),
      },
    });
  ...
  it("skips plugins with no recorded version (cannot detect drift)", () => {
    const result = detectPluginVersionDrift({
      gatewayVersion: "2026.5.4",
      installRecords: {
        whatsapp: { source: "npm", spec: "@mo/whatsapp@latest" },
      },
    });
  ...
  it("skips plugins that are explicitly disabled in config", () => {
    ...
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        whatsapp: npmRecord("2026.5.3"),
        discord: npmRecord("2026.5.3", { resolvedName: "@mo/discord" }),
      },
      config,
    });
  ...
  it("returns drifts sorted by pluginId for deterministic output", () => {
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        whatsapp: npmRecord("2026.5.3"),
        discord: npmRecord("2026.5.3", { resolvedName: "@mo/discord" }),
        matrix: npmRecord("2026.5.3", { resolvedName: "@mo/matrix" }),
      },
    });
...
describe("resolvePluginVersionDriftUpdateCommand", () => {
  it("uses an exact npm package target when the drifted install is pinned", () => {
    ...
    ).toBe("mo plugins update @mo/brave-plugin@2026.6.10-beta.1");
  });
  ...
  it("keeps plugin-id updates for floating npm install records", () => {
    expect(
      resolvePluginVersionDriftUpdateCommand({
        ...
        source: "npm",
        packageName: "@mo/brave-plugin",
        spec: "@mo/brave-plugin",
      }),
    ).toBe("mo plugins update brave");
  });
