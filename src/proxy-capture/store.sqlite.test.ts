...
function makeStore() {
  const root = makeTempDir(cleanupDirs, "mo-proxy-capture-");
  return new DebugProxyCaptureStore({ env: { MO_STATE_DIR: root } });
}

function makeStateEnv(prefix: string): NodeJS.ProcessEnv {
  const root = makeTempDir(cleanupDirs, prefix);
  return { MO_STATE_DIR: root };
}
...
describe("DebugProxyCaptureStore", () => {
  it("keeps the cached store open until the last lease releases", () => {
    const options = { env: makeStateEnv("mo-proxy-capture-lease-") };
  ...
  it("tracks and closes cached stores independently across paths", () => {
    const first = acquireDebugProxyCaptureStore({
      env: makeStateEnv("mo-proxy-capture-first-"),
    });
    const second = acquireDebugProxyCaptureStore({
      env: makeStateEnv("mo-proxy-capture-second-"),
    });
  ...
  it("preserves the shipped path-based Plugin SDK overloads", () => {
    const root = makeTempDir(cleanupDirs, "mo-proxy-capture-legacy-sdk-");
  ...
  it("uses rollback journaling for captures on NFS-backed volumes", () => {
    ...
    const store = new DebugProxyCaptureStore({
      env: makeStateEnv("mo-proxy-capture-nfs-"),
    });
    try {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const env = makeStateEnv("mo-proxy-capture-permissions-");
      ...
      expect(store.dbPath).toBe(path.join(root, "state", "mo.sqlite"));
