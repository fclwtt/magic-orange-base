...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[mo proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[mo proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`mo proxy coverage\` for details.`,
  );
}
