...
async function makeSettings(): Promise<DebugProxySettings> {
  testRoot = await mkdtemp(join(tmpdir(), "mo-debug-proxy-managed-proxy-"));
  ...
  process.env.MO_STATE_DIR = testRoot;
  return {
    enabled: true,
    required: false,
...
describe("debug proxy managed-proxy direct upstream policy", () => {
  const originalProxyActive = process.env["MO_PROXY_ACTIVE"];
  const originalAllowDirect =
    process.env["MO_DEBUG_PROXY_ALLOW_DIRECT_CONNECT_WITH_MANAGED_PROXY"];

  beforeEach(async () => {
    await cleanupTestDirs();
    delete process.env["MO_PROXY_ACTIVE"];
    delete process.env["MO_DEBUG_PROXY_ALLOW_DIRECT_CONNECT_WITH_MANAGED_PROXY"];
  });
  ...
  afterEach(async () => {
    if (originalProxyActive === undefined) {
      ...
      process.env["MO_PROXY_ACTIVE"] = originalProxyActive;
    }
  ...
  it("uses shared truthy parsing for managed proxy mode", () => {
    process.env["MO_PROXY_ACTIVE"] = "true";
  ...
  it("allows direct upstreams with explicit diagnostic override", () => {
    process.env["MO_PROXY_ACTIVE"] = "1";
    process.env["MO_DEBUG_PROXY_ALLOW_DIRECT_CONNECT_WITH_MANAGED_PROXY"] = "1";
