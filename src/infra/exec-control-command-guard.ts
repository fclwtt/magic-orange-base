...
function stripOpenClawPackageRunner(argv: string[]): string[] {
  ...
  if (
    ...
    normalizeCommandBaseName(argv[1]) === "mo"
  ) {
    return argv.slice(1);
  }
  if (
    ...
    normalizeCommandBaseName(argv[2]) === "mo"
  ) {
    return argv.slice(2);
  }
  if (commandName === "npx" || commandName === "bunx") {
    ...
    if (normalizeCommandBaseName(argv[idx]) === "mo") {
...
export function parseOpenClawChannelsLoginShellCommand(raw: string): boolean {
  ...
  const moArgv = stripOpenClawPackageRunner(argv);
  return (
    normalizeCommandBaseName(moArgv[0]) === "mo" &&
    (moArgv[1] === "channels" || moArgv[1] === "channel") &&
    moArgv[2] === "login"
  );
}
  ...
  if (unsafeKind === "channel-login") {
    throw new Error(
      [
        ...
        "Run `mo channels login` in a terminal on the gateway host, or use the channel-specific login agent tool when available (for WhatsApp: `whatsapp_login`).",
      ].join(" "),
    );
  }
}
