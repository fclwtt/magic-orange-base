...
describe("command-analysis risks", () => {
  ...
  it("builds executable payload candidates through carriers and shell wrappers", () => {
    ...
    expect(buildCommandPayloadCandidates(["sudo", "MO_ENV=1", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    expect(buildCommandPayloadCandidates(["exec", "-a", "mo", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    ).toContain("mo channels login --channel whatsapp");
  });
