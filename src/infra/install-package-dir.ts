...
import { isRecord as isObjectRecord } from "@mo/normalization-core/record-coerce";
...
const STAGED_NPM_PROJECT_CONFIG_PREFIX = ".mo-install-hidden-npmrc-";
...
export async function installPackageDir(params: {
  ...
  try {
    ...
    stageDir = await fs.mkdtemp(path.join(installBaseRealPath, ".mo-install-stage-"));
  ...
  if (params.mode === "update" && (await pathExists(canonicalTargetDir))) {
    const backupRoot = path.join(installBaseRealPath, ".mo-install-backups");
