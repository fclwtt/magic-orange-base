...
describe.runIf(process.platform !== "win32")("findGatewayPidsOnPortSync", () => {
  it("parses lsof output and filters non-mo/current processes", () => {
  ...
  it("kills stale gateway pids discovered on the gateway port", () => {
    ...
    spawnSyncMock
      .mockReturnValueOnce({
        error: undefined,
        status: 0,
        stdout: [`p${stalePidA}`, "cmo", `p${stalePidB}`, "cmo-gateway"].join("\n"),
      })
      .mockReturnValue({
        error: undefined,
        status: 1,
...
describe("triggerOpenClawRestart", () => {
  it("does not kickstart after bootstrap registers an unloaded LaunchAgent", () => {
    setPlatform("darwin");
    withEnv(
      { VITEST: undefined, NODE_ENV: undefined, HOME: "/Users/test", MO_PROFILE: "default" },
      () => {
        ...
        expect(result).toEqual({
          ...
          method: "launchctl",
          tried: [
            `launchctl kickstart -k gui/${uid}/ai.mo.gateway`,
            `launchctl bootstrap gui/${uid} /Users/test/Library/LaunchAgents/ai.mo.gateway.plist`,
          ],
        });
      },
    );
      ...
      () => {
        ...
        expect(result).toEqual({
          ...
          tried: [
            `launchctl kickstart -k gui/${uid}/ai.mo.gateway`,
            `launchctl bootstrap gui/${uid} /Users/test/Library/LaunchAgents/ai.mo.gateway.plist`,
            `launchctl kickstart gui/${uid}/ai.mo.gateway`,
          ],
        });
      },
    );
