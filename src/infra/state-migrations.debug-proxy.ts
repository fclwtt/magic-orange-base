...
function resolveLegacyDebugProxyCapturePaths(
  ...
  return {
    sourcePath: env.MO_DEBUG_PROXY_DB_PATH?.trim() || path.join(rootDir, "capture.sqlite"),
    blobDir: env.MO_DEBUG_PROXY_BLOB_DIR?.trim() || path.join(rootDir, "blobs"),
  };
}
...
export function detectLegacyDebugProxyCaptureSidecar(
  ...
  if (
    ...
    path.resolve(resolveOpenClawStateSqlitePath({ ...env, MO_STATE_DIR: stateDir }))
  ) {
