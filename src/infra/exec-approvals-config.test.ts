...
describe("exec approvals wildcard agent", () => {
  it("merges wildcard allowlist entries with agent entries", () => {
    ...
    const prevOpenClawHome = process.env.MO_HOME;

    try {
      process.env.MO_HOME = dir;
      const approvalsPath = path.join(dir, ".mo", "exec-approvals.json");
      ...
      if (prevOpenClawHome === undefined) {
        ...
        process.env.MO_HOME = prevOpenClawHome;
      }
    }
  });
});
