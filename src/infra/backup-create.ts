...
import { resolveDateTimestampMs } from "@mo/normalization-core/number-coercion";
...
async function createStateSqliteBackupPlan(params: {
  ...
  for (const archiveSourcePath of discovery.snapshotPaths) {
    ...
    const sourcePath = path.join(params.tempDir, `mo-state-db-${snapshots.length}.sqlite`);
    try {
...
export async function createBackupArchive(
  ...
  const tempDir = await fs.mkdtemp(path.join(tempRoot, "mo-backup-"));
