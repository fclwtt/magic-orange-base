...
const createTempDir = () => tempDirs.make("mo-state-migrations-test-");
...
describe("state migrations", () => {
  ...
  it("defers an unambiguous legacy merge through a final store symlink", async () => {
    ...
    expect(result.warnings).toContain(
      `Deferred legacy session migration in final-component symlink store ${targetStorePath}; configure one canonical session.store path, then rerun mo doctor --fix`,
    );
  });
  ...
  it("preserves a singleton final symlink through all session migration phases", async () => {
    ...
    expect(result.warnings).toEqual([
      ...
      `Deferred legacy session migration in final-component symlink store ${storePath}; configure one canonical session.store path, then rerun mo doctor --fix`,
    ]);
  });
  ...
  it("preserves ACP metadata through a singleton fixed-store symlink", async () => {
    ...
    expect(result.warnings).toContain(
      `Deferred ACP metadata migration in final-component symlink store ${configuredStorePath}; configure one canonical session.store path, then rerun mo doctor --fix`,
    );
  ...
  it("runs plugin doctor migrations after repairing shared state schema", async () => {
    ...
    const env = createEnv(stateDir);
    const cfg = createConfig();
    const stateDbPath = path.join(stateDir, "state", "mo.sqlite");
  ...
  it("runs plugin doctor migrations against the canonical state dir after state-dir repair", async () => {
    ...
    const canonicalStateDir = path.join(root, ".mo");
  ...
  it("migrates legacy plugin binding approvals from the home state dir when using a custom state dir", async () => {
    ...
    const env = createEnv(stateDir);
    const cfg = createConfig();
    const sourcePath = path.join(root, ".mo", "plugin-binding-approvals.json");
