...
describe("ensureOpenClawCliOnPath", () => {
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/mo-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "mo");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function resetBootstrapEnv(pathValue = "/usr/bin") {
    ...
    delete process.env.MO_PATH_BOOTSTRAPPED;
    delete process.env.MO_ALLOW_PROJECT_LOCAL_BIN;
  ...
  it("prepends the bundled app bin dir when a sibling mo exists", () => {
  ...
  it("keeps the current runtime directory ahead of system PATH hardening", () => {
    const tmp = abs("/tmp/mo-path/case-runtime-dir");
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "mo");
      ...
      if (envValue === undefined) {
        ...
        process.env.MO_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it.each([
    ...
    },
    {
      ...
      setup: () => {
        const tmp = abs("/tmp/mo-path/case-linuxbrew");
