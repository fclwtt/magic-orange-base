...
import { closeOpenClawAgentDatabasesForTest } from "../state/mo-agent-db.js";
import {
  closeOpenClawStateDatabase,
  openOpenClawStateDatabase,
} from "../state/mo-state-db.js";
import { withOpenClawTestState } from "../test-utils/mo-test-state.js";
import {
...
function makeResult(overrides: Partial<BackupCreateResult> = {}): BackupCreateResult {
  return {
    ...
    archiveRoot: "mo-backup-2026-01-01",
    archivePath: "/tmp/mo-backup.tar.gz",
    dryRun: false,
...
describe("formatBackupCreateSummary", () => {
  const backupArchiveLine = "Backup archive: /tmp/mo-backup.tar.gz";
  ...
  it.each([
    {
      ...
      expected: [
        ...
        "- state: ~/.mo",
        "Skipped 1 path:",
        "- workspace: ~/Projects/mo (covered by ~/.mo)",
        "Created /tmp/mo-backup.tar.gz",
        ...
        assets: [
          {
            ...
            displayPath: "~/.mo/config.json",
          },
          {
            kind: "credentials",
            sourcePath: "/oauth",
            archivePath: "archive/oauth",
            displayPath: "~/.mo/oauth",
          },
        ],
      }),
      expected: [
        backupArchiveLine,
        "Included 2 paths:",
        "- config: ~/.mo/config.json",
        "- credentials: ~/.mo/oauth",
  ...
  it("surfaces the volatile skip count in the summary", () => {
    expect(
      ...
      "Backup archive: /tmp/mo-backup.tar.gz",
      "Included 1 path:",
      "- state: ~/.mo",
      "Created /tmp/mo-backup.tar.gz",
...
describe("buildExtensionsNodeModulesFilter", () => {
  it("excludes dependency trees only under state extensions", () => {
    ...
    expect(filter("/state/extensions/demo/mo.plugin.json")).toBe(true);
  ...
  it("normalizes Windows path separators", () => {
    const filter = buildExtensionsNodeModulesFilter("C:\\Users\\me\\.mo\\");

    expect(filter(String.raw`C:\Users\me\.mo\extensions\demo\index.js`)).toBe(true);
    expect(
      filter(String.raw`C:\Users\me\.mo\extensions\demo\node_modules\dep\index.js`),
    ).toBe(false);
  });
});
...
describe("createBackupArchive", () => {
    ...
    await withOpenClawTestState(
      ...
      async (state) => {
        ...
        try {
          ...
          expect(path.basename(result.archivePath)).toContain("mo-backup.tar.gz");
  ...
  it("falls back to epoch when injected nowMs and Date.now are outside Date range", async () => {
    await withOpenClawTestState(
      {
        layout: "state-only",
        prefix: "mo-backup-invalid-fallback-now-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
      ...
      async (state) => {
        ...
        try {
          ...
          const archivedDbEntry = entries.find((entry) =>
            entry.endsWith("/state/state/mo.sqlite"),
          );
          expect(archivedDbEntry).toBeDefined();
          expect(entries.some((entry) => entry.endsWith("/state/state/mo.sqlite-wal"))).toBe(
            false,
          );
  ...
  it("snapshots per-agent SQLite auth stores without deleted secret pages", async () => {
    await withOpenClawTestState(
      ...
      async (state) => {
        ...
        const sqlite = requireNodeSqlite();
        const liveDbPath = path.join(state.agentDir(), "mo-agent.sqlite");
        const deletedSecretMarker = "MO_DELETED_SECRET_PAGE_MARKER";
        ...
        const entries = await listArchiveEntries(result.archivePath);
        const archivedDbEntry = entries.find((entry) =>
          entry.endsWith("/state/agents/main/agent/mo-agent.sqlite"),
        );
        expect(archivedDbEntry).toBeDefined();
        expect(
          entries.some((entry) =>
            entry.endsWith("/state/agents/main/agent/mo-agent.sqlite-wal"),
          ),
        ).toBe(false);
  ...
  it("snapshots nested live SQLite databases with transaction continuity", async () => {
    await withOpenClawTestState(
      {
        layout: "state-only",
        prefix: "mo-backup-nested-sqlite-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
  ...
  it("fails instead of raw-copying malformed nested SQLite databases", async () => {
    await withOpenClawTestState(
      {
        layout: "state-only",
        prefix: "mo-backup-malformed-sqlite-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
  ...
  it("omits pre-existing orphan SQLite sidecars without failing backup", async () => {
    await withOpenClawTestState(
      {
        layout: "state-only",
        prefix: "mo-backup-orphan-sqlite-sidecars-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
  ...
  it("omits transient memory reindex databases and sidecars", async () => {
    await withOpenClawTestState(
      {
        layout: "state-only",
        prefix: "mo-backup-memory-reindex-lock-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
  ...
  it("preserves noncanonical symlinked SQLite paths without dereferencing them", async () => {
    ...
    await withOpenClawTestState(
      {
        layout: "state-only",
        prefix: "mo-backup-symlinked-sqlite-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
  ...
  it("snapshots the canonical global SQLite symlink as a complete regular file", async () => {
    ...
    await withOpenClawTestState(
      {
        layout: "state-only",
        prefix: "mo-backup-global-sqlite-symlink-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
        ...
        const linkedDbPath = state.statePath("state", "mo.sqlite");
        await state.writeConfig({
          agents: {
            list: [{ id: "main", default: true, workspace: state.workspaceDir }],
          },
        ...
        try {
          ...
          const entries = await listArchiveEntryDetails(result.archivePath);
          const archivedDbEntries = entries.filter((entry) =>
            entry.path.endsWith("/state/state/mo.sqlite"),
          );
  ...
  it("fails when the canonical global SQLite path is not a file", async () => {
    await withOpenClawTestState(
      {
        layout: "state-only",
        prefix: "mo-backup-global-sqlite-directory-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
        const globalDbPath = state.statePath("state", "mo.sqlite");
  ...
  it("omits installed plugin node_modules from the real archive while keeping plugin files", async () => {
    await withOpenClawTestState(
      ...
      async (state) => {
        ...
        await fs.writeFile(
          path.join(stateDir, "extensions", "demo", "mo.plugin.json"),
          '{"id":"demo"}\n',
          "utf8",
        );
        await fs.writeFile(
        ...
        expect(entrySuffixes).toContain("/state/extensions/demo/mo.plugin.json");
  ...
  it("dereferences hardlinks instead of emitting restore-hostile Link entries", async () => {
    await withOpenClawTestState(
      {
        layout: "state-only",
        prefix: "mo-backup-hardlink-",
        scenario: "minimal",
      },
      async (state) => {
        const stateDir = state.stateDir;
        const outputDir = state.path("backups");
        const sourcePath = path.join(stateDir, "workspace-adx", "mo-src", "node_modules");
  ...
  it("does not duplicate the root manifest when the system tempdir is the state dir itself", async () => {
    await withOpenClawTestState(
      {
        layout: "state-only",
        prefix: "mo-backup-tmp-equals-state-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
        ...
        try {
          ...
          expect(entries.some((entry) => entry.includes("/mo-state-db-"))).toBe(false);
