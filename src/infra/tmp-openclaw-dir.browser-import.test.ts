...
describe("tmp-mo-dir browser-safe import", () => {
  it("loads when a browser fs shim omits constants", async () => {
    const resultKey = `__moTmpDirBrowserImport_${crypto.randomUUID().replaceAll("-", "_")}`;
    ...
    const bundled = await build({
      ...
      stdin: {
        contents: `
          import { POSIX_MO_TMP_DIR, resolvePreferredOpenClawTmpDir } from "./src/infra/tmp-mo-dir.ts";
          globalThis.${resultKey} = {
            posixTmpDir: POSIX_MO_TMP_DIR,
            resolverType: typeof resolvePreferredOpenClawTmpDir,
          };
        `,
        loader: "ts",
        resolveDir: process.cwd(),
        sourcefile: "tmp-mo-dir-browser-entry.ts",
      },
      write: false,
    });
