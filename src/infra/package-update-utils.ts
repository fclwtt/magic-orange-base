...
import { readRootJsonObjectSync } from "@mo/fs-safe/json";
...
/** Return true when an installed package needs an mo peer link repair. */
export function installedPackageNeedsOpenClawPeerLinkRepair(dir: string): boolean {
  ...
  try {
    fsSync.statSync(path.join(dir, "node_modules", "mo"));
    return false;
  } catch (error) {
