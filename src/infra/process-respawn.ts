...
const PNPM_VERSIONED_MO_ENTRY_PATTERN =
  /^(.*?)([\\/])node_modules\2\.pnpm\2mo@[^\\/]+\2node_modules\2mo\2.+$/;
...
function rewritePnpmVersionedOpenClawEntryPath(entryPath: string): string {
  ...
  return entryPath.replace(
    PNPM_VERSIONED_MO_ENTRY_PATTERN,
    "$1$2node_modules$2mo$2mo.mjs",
  );
}
 ...
 * - MO_NO_RESPAWN=1: caller should keep in-process restart behavior (tests/dev)
...
export function restartGatewayProcessWithFreshPid(
  ...
  if (isTruthy(process.env.MO_NO_RESPAWN)) {
...
export function respawnGatewayProcessForUpdate(
  ...
  if (isTruthy(process.env.MO_NO_RESPAWN)) {
    return { mode: "disabled", detail: "MO_NO_RESPAWN" };
  }
