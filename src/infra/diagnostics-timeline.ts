...
const MO_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION = "mo.diagnostics.v1";
...
export function isDiagnosticsTimelineEnabled(options: DiagnosticsTimelineOptions = {}): boolean {
  ...
  return (
    (isDiagnosticFlagEnabled("timeline", config, env) ||
      ...
      isTruthyEnvValue(env.MO_DIAGNOSTICS)) &&
    typeof env.MO_DIAGNOSTICS_TIMELINE_PATH === "string" &&
    env.MO_DIAGNOSTICS_TIMELINE_PATH.trim().length > 0
  );
}
...
function serializeTimelineEvent(event: DiagnosticsTimelineEvent, env: NodeJS.ProcessEnv): string {
  const normalized = {
    schemaVersion: MO_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION,
    type: event.type,
    timestamp: event.timestamp ?? new Date().toISOString(),
    name: event.name,
    ...(env.MO_DIAGNOSTICS_RUN_ID ? { runId: env.MO_DIAGNOSTICS_RUN_ID } : {}),
    ...(env.MO_DIAGNOSTICS_ENV ? { envName: env.MO_DIAGNOSTICS_ENV } : {}),
    pid: process.pid,
...
export function emitDiagnosticsTimelineEvent(
  ...
  const path = env.MO_DIAGNOSTICS_TIMELINE_PATH?.trim();
  if (!path) {
    return;
  }
