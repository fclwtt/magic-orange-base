...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__mo_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "mo-root");
...
function setPackageRoot(root: string, name = "mo") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./mo-root.js").resolveOpenClawPackageRootSync,
  asyncResolver: typeof import("./mo-root.js").resolveOpenClawPackageRoot,
  opts: Parameters<typeof import("./mo-root.js").resolveOpenClawPackageRootSync>[0],
...
vi.mock("./mo-root.fs.runtime.js", () => ({
  openClawRootFsSync: mockFsModule(),
  openClawRootFs: mockFsPromisesModule(),
}));

describe("resolveOpenClawPackageRoot", () => {
  let resolveOpenClawPackageRoot: typeof import("./mo-root.js").resolveOpenClawPackageRoot;
  let resolveOpenClawPackageRootSync: typeof import("./mo-root.js").resolveOpenClawPackageRootSync;
  let clearOpenClawPackageRootCaches: typeof import("./mo-root.js").testing.clearOpenClawPackageRootCaches;
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "mo");
        const pkgRoot = path.join(project, "node_modules", "mo");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "mo");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "mo.mjs")));
    ...
    {
      name: "returns null for non-mo package roots",
      setup: () => {
        const pkgRoot = fx("not-mo");
        setPackageRoot(pkgRoot, "not-mo");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "mo.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "mo");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
