...
describe("tailscale helpers", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["MO_TEST_TAILSCALE_BINARY", "NODE_ENV", "VITEST"]);
    process.env.MO_TEST_TAILSCALE_BINARY = "tailscale";
  ...
  it("allows the test binary override in explicit test environments", () => {
    process.env.MO_TEST_TAILSCALE_BINARY = "/tmp/test-tailscale";
  ...
  it("ignores the test binary override outside test environments", () => {
    process.env.MO_TEST_TAILSCALE_BINARY = "/tmp/attacker-tailscale";
  ...
  it("enableTailscaleServe passes a configured service name", async () => {
    ...
    await enableTailscaleServe(3000, exec as never, "svc:mo");
    ...
    expectExecCall(
      ...
      1,
      tailscaleBin,
      ["serve", "--service=svc:mo", "--bg", "--yes", "3000"],
      {
        maxBuffer: 200_000,
        timeoutMs: 15_000,
      },
  ...
  it("disableTailscaleServe disables only the configured service name", async () => {
    ...
    await disableTailscaleServe(exec as never, "svc:mo");

    expect(exec).toHaveBeenCalledTimes(1);
    expectExecCall(exec, 1, tailscaleBin, ["serve", "clear", "svc:mo"], {
      maxBuffer: 200_000,
      timeoutMs: 15_000,
    });
  });
