...
import { bundledDistPluginFile } from "mo/plugin-sdk/test-fixtures";
...
const execFileSyncMock = vi.hoisted(() => vi.fn(() => "/tmp/mo-test-global-npmrc\n"));
...
async function writeGlobalPackageJson(packageRoot: string, version = "1.0.0") {
  await fs.writeFile(
    ...
    JSON.stringify({ name: "mo", version }),
    "utf-8",
  );
}
...
describe("update global helpers", () => {
  ...
  it("prefers explicit package spec overrides", () => {
    envSnapshot = captureEnv(["MO_UPDATE_PACKAGE_SPEC"]);
    process.env.MO_UPDATE_PACKAGE_SPEC = "file:/tmp/mo.tgz";

    expect(resolveGlobalInstallSpec({ packageName: "mo", tag: "latest" })).toBe(
      "file:/tmp/mo.tgz",
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "mo",
        tag: "beta",
        env: { MO_UPDATE_PACKAGE_SPEC: "mo@next" },
      }),
    ).toBe("mo@next");
  });
  ...
  it("maps main and explicit install specs for global installs", () => {
    expect(resolveGlobalInstallSpec({ packageName: "mo", tag: "main" })).toBe(
      MO_MAIN_PACKAGE_SPEC,
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "mo",
        tag: "github:mo/mo#feature/my-branch",
      }),
    ).toBe("github:mo/mo#feature/my-branch");
    expect(
      resolveGlobalInstallSpec({
        packageName: "mo",
        tag: "https://example.com/mo-main.tgz",
      }),
    ).toBe("https://example.com/mo-main.tgz");
  });
  ...
  it("resolves portable Git paths from process-local app data only", async () => {
    await withMockedWindowsPlatform(async () => {
      await withTempDir({ prefix: "mo-update-portable-git-" }, async (base) => {
  ...
  it("classifies main and raw install specs separately from registry selectors", () => {
    ...
    expect(isExplicitPackageInstallSpec("github:mo/mo#main")).toBe(true);
    expect(isExplicitPackageInstallSpec("https://example.com/mo-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("file:/tmp/mo-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("/tmp/mo-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("mo-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("beta")).toBe(false);

    expect(canResolveRegistryVersionForPackageTarget("latest")).toBe(true);
    expect(canResolveRegistryVersionForPackageTarget("2026.3.22")).toBe(true);
    expect(canResolveRegistryVersionForPackageTarget("main")).toBe(false);
    expect(canResolveRegistryVersionForPackageTarget("github:mo/mo#main")).toBe(false);
    expect(canResolveRegistryVersionForPackageTarget("/tmp/mo-main.tgz")).toBe(false);
  });

  it("detects install managers from resolved roots and on-disk presence", async () => {
    await withTempDir({ prefix: "mo-update-global-" }, async (base) => {
      const npmRoot = path.join(base, "npm-root");
      const pnpmRoot = path.join(base, "pnpm-root");
      const bunRoot = path.join(base, ".bun", "install", "global", "node_modules");
      const pkgRoot = path.join(pnpmRoot, "mo");
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.mkdir(path.join(npmRoot, "mo"), { recursive: true });
      await fs.mkdir(path.join(bunRoot, "mo"), { recursive: true });
      ...
      await fs.rm(path.join(npmRoot, "mo"), { recursive: true, force: true });
      await fs.rm(path.join(pnpmRoot, "mo"), { recursive: true, force: true });
  ...
  it("prefers the owning npm prefix when PATH npm points at a different global root", async () => {
    await withMockedPlatform("darwin", async () => {
      await withTempDir({ prefix: "mo-update-npm-prefix-" }, async (base) => {
        ...
        expect(globalInstallArgs("npm", "mo@latest", pkgRoot)).toEqual([
          brewNpm,
          "i",
          "-g",
          "mo@latest",
        ...
        expect(globalInstallFallbackArgs("npm", "mo@latest", pkgRoot)).toEqual([
          brewNpm,
          "i",
          "-g",
          "mo@latest",
  ...
  it("keeps npm ownership but avoids per-Node npm commands for reinstall", async () => {
    await withMockedPlatform("darwin", async () => {
      await withTempDir({ prefix: "mo-update-node-version-prefix-" }, async (base) => {
        ...
        for (const layout of layouts) {
          ...
          const pkgRoot = path.join(nodeManagedRoot, "mo");
          ...
          expect(globalInstallArgs("npm", "mo@latest", pkgRoot), layout.name).toEqual([
            "npm",
            "i",
            "-g",
            "mo@latest",
          ...
          expect(globalInstallFallbackArgs("npm", "mo@latest", pkgRoot), layout.name).toEqual(
            [
              "npm",
              "i",
              "-g",
  ...
  it("does not infer npm ownership from path shape alone when the owning npm binary is absent", async () => {
    await withTempDir({ prefix: "mo-update-npm-missing-bin-" }, async (base) => {
  ...
  it("honors an explicitly selected direct npm node_modules package root", async () => {
    await withTempDir({ prefix: "mo-update-managed-service-root-" }, async (base) => {
      const managedNpmRoot = path.join(base, ".mo", "npm", "node_modules");
      const pkgRoot = path.join(managedNpmRoot, "mo");
      ...
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.mkdir(path.join(otherPnpmRoot, "mo"), { recursive: true });
  ...
  it("preserves bun ownership for direct node_modules package roots", async () => {
    await withTempDir({ prefix: "mo-update-managed-bun-root-" }, async (base) => {
      envSnapshot = captureEnv(["BUN_INSTALL"]);
      process.env.BUN_INSTALL = path.join(base, ".bun");
      const bunRoot = path.join(process.env.BUN_INSTALL, "install", "global", "node_modules");
      const pkgRoot = path.join(bunRoot, "mo");
      const pathNpmRoot = path.join(base, "shell", "lib", "node_modules");
      await fs.mkdir(pkgRoot, { recursive: true });
  ...
  it("prefers npm.cmd for win32-style global npm roots", async () => {
    await withMockedWindowsPlatform(async () => {
      await withTempDir({ prefix: "mo-update-win32-npm-prefix-" }, async (base) => {
        const npmPrefix = path.join(base, "Roaming", "npm");
        const npmRoot = path.join(npmPrefix, "node_modules");
        const pkgRoot = path.join(npmRoot, "mo");
  ...
  it("detects custom pnpm global layouts from virtual-store package roots", async () => {
    await withTempDir({ prefix: "mo-update-pnpm-virtual-root-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      ...
      await expect(
        ...
        command: "pnpm",
        globalRoot: customGlobalRoot,
        packageRoot: path.join(customGlobalRoot, "mo"),
      });
    });
  });

  it("does not infer pnpm ownership without pnpm node_modules metadata", async () => {
    await withTempDir({ prefix: "mo-update-pnpm-shape-only-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      const pkgRoot = path.join(customGlobalRoot, "mo");
      const defaultPnpmRoot = path.join(base, "default-pnpm", "5", "node_modules");
      ...
      await expect(
        ...
        packageRoot: path.join(defaultPnpmRoot, "mo"),
      });
    });
  });
  ...
  it("builds install argv and npm fallback argv", () => {
    ...
    expect(globalInstallArgs("npm", "mo@latest")).toEqual([
      "npm",
      "i",
      "-g",
      "mo@latest",
    ...
    expect(globalInstallArgs("pnpm", "mo@latest")).toEqual([
      "pnpm",
      "add",
      "-g",
      "mo@latest",
    ]);
    expect(globalInstallArgs("pnpm", "github:mo/mo#release/2026.5.12")).toEqual([
      "pnpm",
      "add",
      "-g",
      "--allow-build=mo",
      "github:mo/mo#release/2026.5.12",
    ]);
    expect(
      globalInstallArgs("pnpm", "mo@git+https://github.com/mo/mo.git"),
    ).toEqual([
      "pnpm",
      "add",
      "-g",
      "--allow-build=mo",
      "mo@git+https://github.com/mo/mo.git",
    ]);
    expect(globalInstallArgs("bun", "mo@latest")).toEqual([
      "bun",
      "add",
      "-g",
      "mo@latest",
    ...
    expect(globalInstallFallbackArgs("npm", "mo@latest")).toEqual([
      "npm",
      "i",
      "-g",
      "mo@latest",
    ...
    expect(globalInstallFallbackArgs("pnpm", "mo@latest")).toBeNull();
    expect(
      globalInstallArgs({ manager: "pnpm", command: "/opt/homebrew/bin/pnpm" }, "mo@latest"),
    ).toEqual(["/opt/homebrew/bin/pnpm", "add", "-g", "mo@latest"]);
    expect(globalInstallArgs("pnpm", "mo@latest", null, "/opt/pnpm-global")).toEqual([
      "pnpm",
      "add",
      "-g",
      "--global-dir",
  ...
  it("builds npm staged install argv with an explicit prefix", () => {
    expect(globalInstallArgs("npm", "mo@latest", null, "/tmp/stage")).toEqual([
      "npm",
      "i",
      "-g",
      "--prefix",
    ...
    expect(globalInstallFallbackArgs("npm", "mo@latest", null, "/tmp/stage")).toEqual([
      "npm",
      "i",
      "-g",
      "--prefix",
  ...
  it("resolves npm prefix layouts for normal global roots", () => {
    expect(resolveNpmGlobalPrefixLayoutFromGlobalRoot("/opt/mo/lib/node_modules")).toEqual({
      prefix: "/opt/mo",
      globalRoot: "/opt/mo/lib/node_modules",
      binDir: "/opt/mo/bin",
    });
  ...
  it("cleans only renamed package directories", async () => {
    await withTempDir({ prefix: "mo-update-cleanup-" }, async (root) => {
      await fs.mkdir(path.join(root, ".mo-123"), { recursive: true });
      await fs.mkdir(path.join(root, ".mo-456"), { recursive: true });
      await fs.writeFile(path.join(root, ".mo-file"), "nope", "utf8");
      await fs.mkdir(path.join(root, "mo"), { recursive: true });
      ...
      await expect(
        ...
        removed: [".mo-123", ".mo-456"],
      });
      const packageDirStat = await fs.stat(path.join(root, "mo"));
      const markerFileStat = await fs.stat(path.join(root, ".mo-file"));
  ...
  it("checks installed dist against the packaged inventory", async () => {
    await withTempDir({ prefix: "mo-update-global-pkg-" }, async (packageRoot) => {
  ...
  it("reports bundled plugin install stages during installed dist verification", async () => {
    await withTempDir({ prefix: "mo-update-global-plugin-stage-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
      await fs.mkdir(path.join(packageRoot, "dist", "extensions", "brave"), { recursive: true });
      await writePackageDistInventory(packageRoot);

      for (const stageDir of [".mo-install-stage", ".mo-install-stage-retry"]) {
      ...
      await expect(collectInstalledGlobalPackageErrors({ packageRoot })).resolves.toEqual([
        "unexpected packaged dist file dist/extensions/brave/.mo-install-stage-retry/node_modules/typebox/build/compile/code.mjs",
        "unexpected packaged dist file dist/extensions/brave/.mo-install-stage/node_modules/typebox/build/compile/code.mjs",
      ]);
    });
  });

  it("flags global package roots that resolve into source checkouts", async () => {
    await withTempDir({ prefix: "mo-update-global-source-checkout-" }, async (base) => {
      const checkoutRoot = path.join(base, "checkout");
      const globalRoot = path.join(base, "prefix", "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "mo");
  ...
  it("does not require private QA sidecars when the inventory is missing", async () => {
    await withTempDir({ prefix: "mo-update-global-legacy-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
  ...
  it("fails closed on newer installs when the inventory is missing", async () => {
    await withTempDir(
      { prefix: "mo-update-global-missing-inventory-new-" },
      async (packageRoot) => {
  ...
  it("rejects invalid inventory files during global verify", async () => {
    await withTempDir(
      { prefix: "mo-update-global-invalid-inventory-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
  ...
  it("verifies legacy sidecars for installed bundled plugins without inventory", async () => {
    await withTempDir({ prefix: "mo-update-global-legacy-plugin-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
      await writeBundledPluginPackageJson(packageRoot, "telegram", "@mo/telegram");
  ...
  it("still enforces critical sidecars when the inventory omits them", async () => {
    await withTempDir(
      { prefix: "mo-update-global-critical-sidecars-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeBundledPluginPackageJson(packageRoot, "telegram", "@mo/telegram");
        await writePackageDistInventory(packageRoot);
  ...
  it("ignores stale metadata for non-packaged private QA plugins during inventory verify", async () => {
    await withTempDir(
      { prefix: "mo-update-global-stale-private-qa-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeBundledPluginPackageJson(packageRoot, "qa-lab", "@mo/qa-lab");
        await writePackageDistInventory(packageRoot);
