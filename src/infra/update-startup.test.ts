...
} from "../test-utils/mo-test-state.js";
import {
...
const {
  ...
  startManagedServiceUpdateHandoffMock: vi.fn(async () => ({
    ...
    pid: 12345,
    command: "mo update --yes --channel beta --timeout 2700",
    logPath: "/tmp/mo-handoff.log",
  })),
}));

vi.mock("./mo-root.js", async () => {
  const actual = await vi.importActual<typeof import("./mo-root.js")>("./mo-root.js");
  return {
    ...actual,
...
describe("update-startup", () => {
  ...
  beforeEach(async () => {
    ...
    testState = await createOpenClawTestState({
      layout: "state-only",
      prefix: "mo-update-check-suite-",
      env: {
        MO_NO_AUTO_UPDATE: undefined,
        MO_SERVICE_KIND: undefined,
        MO_SERVICE_MARKER: undefined,
        MO_GATEWAY_SERVICE_PID: undefined,
        MO_LAUNCHD_LABEL: undefined,
        MO_SYSTEMD_UNIT: undefined,
        MO_WINDOWS_TASK_NAME: undefined,
    ...
    if (!loaded) {
      ({ resolveOpenClawPackageRoot } = await import("./mo-root.js"));
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveOpenClawPackageRoot).mockResolvedValue("/opt/mo");
  ...
  it.each([
    ...
    expect(log.info).toHaveBeenCalledWith(
      `update available (latest): v2.0.0 (current v1.0.0). Run: ${formatCliCommand("mo update")}`,
    );
  ...
  it("honors MO_NO_AUTO_UPDATE for configured auto-updates", async () => {
    mockPackageUpdateStatus("beta", "2.0.0-beta.1");
    process.env.MO_NO_AUTO_UPDATE = "1";
    const log = { info: vi.fn() };
    const runAutoUpdate = createAutoUpdateSuccessMock();
    ...
    const disabledLogCall = log.info.mock.calls.find(
      ([message]) => message === "auto-update disabled by MO_NO_AUTO_UPDATE",
    );
    expect(disabledLogCall).toEqual([
      "auto-update disabled by MO_NO_AUTO_UPDATE",
      {
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/mo/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
    ...
    expect(options.env).toEqual({ MO_AUTO_UPDATE: "1" });
  });
