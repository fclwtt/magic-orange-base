...
describe("resolveLocalVitestMaxWorkers", () => {
  ...
  it("lets MO_VITEST_MAX_WORKERS override the inferred cap", () => {
    expect(
      resolveLocalVitestMaxWorkers(
        {
          MO_VITEST_MAX_WORKERS: "2",
        },
        {
          cpuCount: 10,
          loadAverage1m: 0,
  ...
  it("respects the legacy MO_TEST_WORKERS override too", () => {
    expect(
      resolveLocalVitestMaxWorkers(
        {
          MO_TEST_WORKERS: "3",
...
describe("resolveLocalVitestScheduling", () => {
  ...
  it("allows disabling the system throttle probe explicitly", () => {
    expect(
      resolveLocalVitestScheduling(
        {
          MO_VITEST_DISABLE_SYSTEM_THROTTLE: "1",
        },
        {
          cpuCount: 16,
          loadAverage1m: 0.5,
    ...
    expect(
      externalPatterns.some(
        (pattern) =>
          ...
          pattern.test("/tmp/mo-pnpm-node-modules/some-dep/dist/index.mjs"),
      ),
    ).toBe(true);
    expect(
      externalPatterns.some(
        (pattern) =>
          pattern instanceof RegExp &&
          pattern.test("/tmp/mo-pnpm-node-modules/vite/dist/client/env.mjs"),
      ),
    ).toBe(false);
  });
});
...
describe("test scripts", () => {
  it("keeps test scripts on the native thread-first configs", () => {
    ...
    expect(pkg.scripts?.["test:gateway"]).toBe(
      "node scripts/run-with-env.mjs MO_GATEWAY_PROJECT_SHARDS=1 -- node scripts/run-vitest.mjs run --config test/vitest/vitest.gateway.config.ts",
    );
