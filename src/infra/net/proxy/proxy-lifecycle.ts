...
import { isLoopbackIpAddress } from "@mo/net-policy/ip";
...
function captureProxyEnv(): ProxyEnvSnapshot {
  return {
    ...
    NO_PROXY: process.env["NO_PROXY"],
    MO_PROXY_ACTIVE: process.env["MO_PROXY_ACTIVE"],
    MO_PROXY_LOOPBACK_MODE: process.env["MO_PROXY_LOOPBACK_MODE"],
    MO_PROXY_CA_FILE: process.env["MO_PROXY_CA_FILE"],
  };
}
...
function applyProxyEnv(
  ...
  process.env["MO_PROXY_LOOPBACK_MODE"] = loopbackMode;
  if (proxyCaFile) {
    process.env["MO_PROXY_CA_FILE"] = proxyCaFile;
  } else {
    delete process.env["MO_PROXY_CA_FILE"];
  }
...
function resolveProxyUrl(config: ProxyConfig | undefined): string {
  const candidate = config?.proxyUrl?.trim() || process.env["MO_PROXY_URL"]?.trim();
  if (!candidate) {
    throw new Error(
      "proxy: enabled but no HTTP proxy URL is configured; set proxy.proxyUrl " +
        "or MO_PROXY_URL to an http:// or https:// forward proxy.",
    );
  }
...
export function ensureInheritedManagedProxyRoutingActive(): void {
  if (process.env["MO_PROXY_ACTIVE"] !== "1") {
    return;
  }
  ...
  const proxyCaFile = resolveManagedProxyCaFileForUrl({
    proxyUrl,
    caFileOverride: process.env["MO_PROXY_CA_FILE"],
  });
