...
describe("isGatewayArgv", () => {
  ...
  it("matches known entrypoints across slash and case variants", () => {
    ...
    expect(isGatewayArgv(["bun", "/srv/mo/scripts/run-node.mjs", "gateway"])).toBe(true);
    expect(isGatewayArgv(["node", "/srv/mo/mo.mjs", "gateway"])).toBe(true);
    expect(isGatewayArgv(["tsx", "/srv/mo/src/entry.ts", "gateway"])).toBe(true);
    expect(isGatewayArgv(["tsx", "/srv/mo/src/index.ts", "gateway"])).toBe(true);
  });

  it("matches the mo executable but gates the gateway binary behind the opt-in flag", () => {
    expect(isGatewayArgv(["C:\\bin\\mo.cmd", "gateway"])).toBe(true);
    expect(isGatewayArgv(["/usr/local/bin/mo-gateway", "gateway"])).toBe(false);
    expect(
      isGatewayArgv(["/usr/local/bin/mo-gateway", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
    expect(
      isGatewayArgv(["C:\\bin\\mo-gateway.EXE", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
  });
  ...
  it("rejects unknown gateway argv even when the token is present", () => {
    expect(isGatewayArgv(["node", "/srv/mo/custom.js", "gateway"])).toBe(false);
