...
function resolveGatewayServiceRecovery(
  ...
  if (supervisor === "systemd") {
    const override = env.MO_SYSTEMD_UNIT?.trim();
    const unit = override
      ? override.endsWith(".service")
        ? override
        : `${override}.service`
      : `${resolveGatewaySystemdServiceName(env.MO_PROFILE)}.service`;
    return { kind: "systemd", unit };
  }
  if (supervisor === "launchd") {
    const label =
      env.MO_LAUNCHD_LABEL?.trim() || resolveGatewayLaunchAgentLabel(env.MO_PROFILE);
  ...
  if (supervisor === "schtasks") {
    const taskName =
      env.MO_WINDOWS_TASK_NAME?.trim() || resolveGatewayWindowsTaskName(env.MO_PROFILE);
...
async function resolveHandoffSpawn(params: {
  ...
  if (!systemdRunPath) {
    throw new Error(
      "systemd-run is required to start the managed update handoff outside mo-gateway.service",
    );
  }
