...
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "mo.exec-approval";
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.MO_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.MO_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set MO_APNS_TEAM_ID and MO_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.MO_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.MO_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.MO_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set MO_APNS_PRIVATE_KEY_P8 or MO_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading MO_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
