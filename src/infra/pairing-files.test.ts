...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/mo-state", "devices")).toEqual({
      dir: path.join("/tmp/mo-state", "devices"),
      pendingPath: path.join("/tmp/mo-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/mo-state", "devices", "paired.json"),
    });
  });
