...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.MO_WIDE_AREA_DOMAIN ?? null;
  try {
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*mo-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "mo";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "mo");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "mo-gw");
  ...
  records.push(`_mo-gw._tcp IN PTR ${instanceLabel}._mo-gw._tcp`);
  records.push(`${instanceLabel}._mo-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._mo-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; mo-content-hash: ${contentHash}\n${contentBody}`;
}
