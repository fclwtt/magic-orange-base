...
import { readResponseWithLimit } from "@mo/media-core/read-response-with-limit";
import { resolveTimerTimeoutMs } from "@mo/normalization-core/number-coercion";
import {
...
export const DEFAULT_APNS_RELAY_BASE_URL = "https://ios-push-relay.mo.ai";
export const DEFAULT_APNS_SANDBOX_RELAY_BASE_URL = "https://ios-push-relay-sandbox.mo.ai";
...
const GATEWAY_DEVICE_ID_HEADER = "x-mo-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-mo-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-mo-gateway-signed-at-ms";
...
export function normalizeApnsRelayBaseUrl(
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !readAllowHttp(env.MO_APNS_RELAY_ALLOW_HTTP)) {
      throw new Error(
        "http relay URLs require MO_APNS_RELAY_ALLOW_HTTP=true (development only)",
      );
    }
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.MO_APNS_RELAY_BASE_URL);
  ...
  if (!baseUrl) {
    return {
      ok: false,
      error:
        "APNs relay config missing: set gateway.push.apns.relay.baseUrl or MO_APNS_RELAY_BASE_URL for relay registrations without the hosted relay origin",
    };
  }
  ...
  return {
    ...
    value: {
      ...
      timeoutMs: normalizeTimeoutMs(
        env.MO_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs,
      ),
    },
  };
}
