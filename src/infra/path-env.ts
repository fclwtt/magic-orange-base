...
function candidateBinDirs(
  ...
  // Bundled macOS app: `mo` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "mo");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.MO_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "mo"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `mo`
