...
export const POSIX_MO_TMP_DIR = "/tmp/mo";
...
export function resolvePreferredOpenClawTmpDir(
  ...
  const fallback = (): string => {
    const suffix = uid === undefined ? "mo" : `mo-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[mo] tightened permissions on temp dir: ${candidatePath}`);
      return resolveDirState(candidatePath) === "available";
    } catch {
      return false;
    }
  ...
  const preferredDir = POSIX_MO_TMP_DIR;
