...
import { normalizeOptionalString as readOptionalString } from "@mo/normalization-core/string-coerce";
...
import { resolveOpenClawPackageRootSync } from "./mo-root.js";
...
function applyManagedNpmRootOverrides(params: {
  ...
  for (const key of readManagedOverrideKeys(params.manifest.mo)) {
...
export async function upsertManagedNpmRootDependency(params: {
  ...
  const managedDependencyNames = new Set(readManagedPeerDependencyKeys(manifest.mo));
  ...
  const moMetadata = buildManagedOpenClawMetadata({
    current: manifest.mo,
    managedOverrideKeys,
...
function scrubHostPeerFromLockPackage(value: unknown): boolean {
  ...
  let changed = false;
  if (isRecord(value.peerDependencies) && "mo" in value.peerDependencies) {
    const peerDependencies = { ...value.peerDependencies };
    delete peerDependencies.mo;
  ...
  if (isRecord(value.peerDependenciesMeta) && "mo" in value.peerDependenciesMeta) {
    const peerDependenciesMeta = { ...value.peerDependenciesMeta };
    delete peerDependenciesMeta.mo;
...
async function collectNpmResolvedManagedNpmRootPeerDependencyPins(params: {
  ...
  const dependencies = readDependencyRecord(manifest.dependencies);
  const previousManagedPeerDependencies = readManagedPeerDependencyKeys(manifest.mo);
  ...
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mo-managed-peer-plan-"));
  try {
    delete dependencies.mo;
    await writeJson(
    ...
    await copyPathIfExists(
      path.join(params.npmRoot, "_mo-pack-archives"),
      path.join(tempRoot, "_mo-pack-archives"),
    );
...
export async function readManagedNpmRootPeerDependencySnapshot(params: {
  ...
  const manifest = await readManagedNpmRootManifest(path.join(params.npmRoot, "package.json"));
  const dependencies = readDependencyRecord(manifest.dependencies);
  const managedPeerDependencies = readManagedPeerDependencyKeys(manifest.mo).toSorted();
...
export async function restoreManagedNpmRootPeerDependencySnapshot(params: {
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  for (const packageName of readManagedPeerDependencyKeys(manifest.mo)) {
    delete dependencies[packageName];
  }
  Object.assign(dependencies, params.snapshot.dependencies);
  const overrides = readOverrideRecord(manifest.overrides);
  const currentManagedOverrideKeys = readManagedOverrideKeys(manifest.mo);
...
/** Remove stale managed-root mo peer installs while preserving active host links. */
export async function repairManagedNpmRootOpenClawPeer(params: {
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  const hasManifestDependency = "mo" in dependencies;
  const hasLockDependency = await managedNpmRootLockfileHasOpenClawPeer(params.npmRoot);
  const hasPackageDir = await pathExists(path.join(params.npmRoot, "node_modules", "mo"));
  ...
  try {
    ...
    if (result.code !== 0) {
      params.logger?.warn?.(
        `npm ${hasManifestDependency ? "uninstall mo" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${result.stderr.trim() || result.stdout.trim()}`,
      );
    }
  } catch (error) {
    params.logger?.warn?.(
      `npm ${hasManifestDependency ? "uninstall mo" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${String(error)}`,
    );
  }
...
async function readManagedNpmRootOpenClawHostState(params: {
  ...
  const managedOpenClawPackageDir = path.join(params.npmRoot, "node_modules", "mo");
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if (
        ...
        "mo" in rootPackage.dependencies
      ) {
        return true;
      }
      if ("node_modules/mo" in parsed.packages) {
        return true;
      }
    }
    return isRecord(parsed.dependencies) && "mo" in parsed.dependencies;
  } catch (err) {
    if ((err as NodeJS.ErrnoException).code === "ENOENT") {
      return false;
    }
...
async function scrubManagedNpmRootOpenClawPeer(params: {
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  if ("mo" in dependencies) {
    const { mo: _removed, ...nextDependencies } = dependencies;
    await fs.writeFile(
      manifestPath,
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if (isRecord(rootPackage) && isRecord(rootPackage.dependencies)) {
        ...
        if ("mo" in dependenciesValue) {
          delete dependenciesValue.mo;
          parsed.packages[""] = { ...rootPackage, dependencies: dependenciesValue };
          lockChanged = true;
        }
      }
      if ("node_modules/mo" in parsed.packages) {
        delete parsed.packages["node_modules/mo"];
        lockChanged = true;
      }
    }
    if (isRecord(parsed.dependencies) && "mo" in parsed.dependencies) {
      const dependenciesLocal = { ...parsed.dependencies };
      delete dependenciesLocal.mo;
  ...
  const moPackageDir = path.join(params.npmRoot, "node_modules", "mo");
  if (!params.preservePackageDir && (await pathExists(moPackageDir))) {
    await fs.rm(moPackageDir, { recursive: true, force: true });
  }
  const binDir = path.join(params.npmRoot, "node_modules", ".bin");
  await Promise.all(
    ["mo", "mo.cmd", "mo.ps1"].map((binName) =>
