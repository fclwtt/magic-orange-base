...
const DEFAULT_EXEC_APPROVALS_STATE_DIR = "~/.mo";
...
export function resolveExecApprovalsTranscriptPath(): string {
  return process.env.MO_STATE_DIR?.trim()
    ? `$MO_STATE_DIR/${EXEC_APPROVALS_FILE}`
...
function hasUnmigratedLegacyExecApprovals(filePath: string): boolean {
  if (!process.env.MO_STATE_DIR?.trim()) {
    return false;
  }
...
function isReadOnlySecurityAuditSuppressionInspection(argv: string[]): boolean {
  ...
  let offset = command === "pnpm" && argv[1] === "mo" ? 1 : 0;
  if (normalizeCommandName(argv[offset]) !== "mo") {
    return false;
  }
  offset += 1;
