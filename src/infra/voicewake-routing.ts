...
import { isRecord as isPlainObject } from "@mo/normalization-core/record-coerce";
...
function openStateDatabase(stateDir?: string) {
  return openOpenClawStateDatabase({
    env: stateDir ? { ...process.env, MO_STATE_DIR: stateDir } : process.env,
  });
}
...
export async function setVoiceWakeRoutingConfig(
  ...
  runOpenClawStateWriteTransaction(
    ...
    baseDir ? { env: { ...process.env, MO_STATE_DIR: baseDir } } : {},
  );
  return next;
}
