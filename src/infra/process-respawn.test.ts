...
function expectLaunchdSupervisedWithoutKickstart(params?: { launchJobLabel?: string }) {
  ...
  process.env.MO_LAUNCHD_LABEL = "ai.mo.gateway";
...
describe("restartGatewayProcessWithFreshPid", () => {
  it("returns disabled when MO_NO_RESPAWN is set", () => {
    process.env.MO_NO_RESPAWN = "1";
    const result = restartGatewayProcessWithFreshPid();
    expect(result.mode).toBe("disabled");
    expect(spawnMock).not.toHaveBeenCalled();
  });

  it("keeps MO_NO_RESPAWN ahead of inherited supervisor hints", () => {
    clearSupervisorHints();
    setPlatform("darwin");
    process.env.MO_NO_RESPAWN = "1";
    process.env.LAUNCH_JOB_LABEL = "ai.mo.gateway";
  ...
  it("returns supervised when OpenClaw launchd markers are present on macOS (no kickstart)", () => {
    clearSupervisorHints();
    expectLaunchdSupervisedWithoutKickstart({ launchJobLabel: "ai.mo.gateway" });
  });
  ...
  it("returns supervised for a real gateway XPC launchd job without the injected marker", () => {
    clearSupervisorHints();
    setPlatform("darwin");
    process.env.XPC_SERVICE_NAME = "ai.mo.gateway";
  ...
  it("does not treat inherited XPC_SERVICE_NAME as launchd supervision", () => {
    clearSupervisorHints();
    setPlatform("darwin");
    process.env.XPC_SERVICE_NAME = "ai.mo.mac";
    process.env.MO_PROFILE = "mac";
  ...
  it("uses in-process restart on unmanaged Unix so custom supervisors keep the tracked PID", () => {
    delete process.env.MO_NO_RESPAWN;
    clearSupervisorHints();
    setPlatform("linux");
  ...
  it("returns supervised when MO_LAUNCHD_LABEL is set (stock launchd plist)", () => {
    clearSupervisorHints();
    expectLaunchdSupervisedWithoutKickstart();
  });

  it("returns supervised when MO_SYSTEMD_UNIT is set", () => {
    clearSupervisorHints();
    setPlatform("linux");
    process.env.MO_SYSTEMD_UNIT = "mo-gateway.service";
    const result = restartGatewayProcessWithFreshPid();
    expect(result.mode).toBe("supervised");
    expect(spawnMock).not.toHaveBeenCalled();
  });
  ...
  it("returns supervised when OpenClaw gateway task markers are set on Windows", () => {
    clearSupervisorHints();
    setPlatform("win32");
    process.env.MO_SERVICE_MARKER = "mo";
  ...
  it("ignores node task script hints for gateway restart detection on Windows", () => {
    clearSupervisorHints();
    setPlatform("win32");
    process.env.MO_TASK_SCRIPT = "C:\\mo\\node.cmd";
    process.env.MO_TASK_SCRIPT_NAME = "node.cmd";
    process.env.MO_SERVICE_MARKER = "mo";
    process.env.MO_SERVICE_KIND = "node";
...
describe("respawnGatewayProcessForUpdate", () => {
  it("keeps MO_NO_RESPAWN semantics for update restarts", () => {
    clearSupervisorHints();
    process.env.MO_NO_RESPAWN = "1";

    const result = respawnGatewayProcessForUpdate();

    expect(result).toEqual({ mode: "disabled", detail: "MO_NO_RESPAWN" });
    expect(spawnMock).not.toHaveBeenCalled();
  });
  ...
  it("allows detached respawn on unmanaged Windows during updates", () => {
    ...
    expect(spawnMock).toHaveBeenCalledWith(
      process.execPath,
      ["C:\\mo\\node_modules\\mo\\mo.mjs", "gateway", "run"],
      {
        detached: true,
        env: process.env,
        stdio: "inherit",
  ...
  it("rewrites a pnpm-versioned OpenClaw entry before detached update respawn", () => {
    ...
    process.argv = [
      "/usr/local/bin/node",
      "/app/node_modules/.pnpm/mo@2026.6.5/node_modules/mo/dist/entry.js",
      "gateway",
      "run",
    ];
    ...
    expect(spawnMock).toHaveBeenCalledWith(
      process.execPath,
      ["/app/node_modules/mo/mo.mjs", "gateway", "run"],
      {
        detached: true,
        env: process.env,
        stdio: "inherit",
