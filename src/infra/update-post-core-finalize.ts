...
// update.
//
// `runGatewayUpdate` (git mode) runs `mo doctor --fix` with
// `MO_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE=1`, which makes the doctor
// pass DEFER configured-plugin repair to a later convergence step (see
// `shouldDeferConfiguredPluginInstallRepair`). The `mo update` CLI resumes
...
// binary's hidden `mo update finalize` entrypoint — the designed
...
// re-enters the full `mo update` CLI. Re-run convergence on no-op retries:
...
export async function runPostCoreFinalizeAfterGatewayUpdate(params: {
  ...
  try {
    ...
    if (params.preUpdateConfig) {
      sourceConfigDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-update-post-core-"));
