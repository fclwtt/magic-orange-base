...
describe("sqlite WAL maintenance", () => {
  ...
  it("uses rollback journaling for databases on NFS-backed volumes", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-sqlite-nfs-"));
    try {
      const db = createMockDb();
      ...
      const maintenance = configureSqliteWalMaintenance(db, {
        ...
        databasePath: path.join(tempDir, "missing", "mo.sqlite"),
      });
  ...
  it.each([
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-sqlite-network-"));
    try {
      const db = createMockDb();
      ...
      configureSqliteWalMaintenance(db, {
        checkpointIntervalMs: 0,
        databasePath: path.join(tempDir, "mo.sqlite"),
      });
  ...
  it.each([
    String.raw`\\server\share\mo.sqlite`,
    String.raw`\\?\UNC\server\share\mo.sqlite`,
    "//server/share/mo.sqlite",
    "//?/UNC/server/share/mo.sqlite",
  ...
  it("uses rollback journaling for mapped Windows network drives", () => {
    const db = createMockDb();
    const databasePath = String.raw`Z:\state\mo.sqlite`;
    vi.spyOn(process, "platform", "get").mockReturnValue("win32");
    const realpath = vi
      .spyOn(fs.realpathSync, "native")
      .mockReturnValue(String.raw`\\server\share\state\mo.sqlite`);
  ...
  it("does not treat namespaced Windows local drives as UNC paths", () => {
    const db = createMockDb();
    const databasePath = String.raw`\\?\C:\state\mo.sqlite`;
    vi.spyOn(process, "platform", "get").mockReturnValue("win32");
  ...
  it("uses rollback journaling when Windows cannot classify an opened drive path", () => {
    const db = createMockDb();
    const databasePath = String.raw`Z:\restricted\mo.sqlite`;
    vi.spyOn(process, "platform", "get").mockReturnValue("win32");
  ...
  it("refuses fuse.sshfs mountinfo entries", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-sqlite-sshfs-"));
    try {
      const db = createMockDb();
      vi.spyOn(fs, "statfsSync").mockReturnValue(statfsFixture(0));
      vi.spyOn(fs, "readFileSync").mockReturnValue(
  ...
  it("refuses symlinked paths into fuse.sshfs mounts", () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-sqlite-sshfs-link-"));
    ...
    try {
      ...
      expect(() =>
        configureSqliteWalMaintenance(createMockDb(), {
          checkpointIntervalMs: 0,
          databasePath: path.join(linkedDir, "mo.sqlite"),
        }),
  ...
  it("matches raw mount paths when the existing path canonicalizes elsewhere", () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-sqlite-sshfs-prefix-"));
    ...
    try {
      ...
      expect(() =>
        configureSqliteWalMaintenance(createMockDb(), {
          checkpointIntervalMs: 0,
          databasePath: path.join(rawMountDir, "mo.sqlite"),
        }),
      ).toThrow(/SSHFS.*refusing to open/);
    } finally {
      fs.rmSync(tempDir, { recursive: true, force: true });
  ...
  it("uses macOS SMB mount filesystem names", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-sqlite-smb-"));
    try {
      const db = createMockDb();
      vi.spyOn(fs, "statfsSync").mockReturnValue(statfsFixture(0));
      vi.spyOn(fs, "readFileSync").mockImplementation(() => {
  ...
  it.each([
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-sqlite-sshfs-macfuse-"));
    try {
      const db = createMockDb();
      vi.spyOn(fs, "statfsSync").mockReturnValue(statfsFixture(0));
      vi.spyOn(fs, "readFileSync").mockImplementation(() => {
  ...
  it("keeps WAL enabled for non-remote macFUSE mounts", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-sqlite-macfuse-"));
    try {
      const db = createMockDb();
      vi.spyOn(fs, "statfsSync").mockReturnValue(statfsFixture(0));
      vi.spyOn(fs, "readFileSync").mockImplementation(() => {
