...
async function withRestartSentinelStateDir(run: () => Promise<void>): Promise<void> {
  await withTempDir({ prefix: "mo-sentinel-" }, async (tempDir) => {
    try {
      await withEnvAsync({ MO_STATE_DIR: tempDir }, run);
    } finally {
      closeOpenClawStateDatabaseForTest();
    }
  });
...
describe("restart sentinel", () => {
  ...
  it("imports a legacy file sentinel into sqlite once", async () => {
    await withRestartSentinelStateDir(async () => {
      ...
      const legacyPath = path.join(process.env.MO_STATE_DIR ?? "", "restart-sentinel.json");
  ...
  it("keeps old config restart sentinels readable without restart-required stats", async () => {
    await withRestartSentinelStateDir(async () => {
      const filePath = path.join(process.env.MO_STATE_DIR ?? "", "restart-sentinel.json");
      const payload = {
  ...
  it("formats config write success notices as restart required when marked", () => {
    const payload = {
      ...
      doctorHint: "Run mo doctor --non-interactive",
...
describe("restart sentinel message dedup", () => {
  ...
  it("formats the non-interactive doctor command as actionability guidance", () => {
    expect(formatDoctorNonInteractiveHint({ PATH: "/usr/bin:/bin" })).toBe(
      "Recommended follow-up: run mo doctor --non-interactive in a terminal or approvals-capable OpenClaw surface.",
    );
  });
  ...
  it("keeps profile-aware doctor guidance actionable outside constrained delivery surfaces", () => {
    expect(
      ...
      "Recommended follow-up: run mo --profile isolated doctor --non-interactive in a terminal or approvals-capable OpenClaw surface.",
    );
  });
});
