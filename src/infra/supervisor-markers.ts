...
const SUPERVISOR_HINTS = {
  launchd: ["MO_LAUNCHD_LABEL"],
  systemd: ["MO_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["MO_WINDOWS_TASK_NAME"],
} as const;
...
function hasOpenClawGatewayServiceMarker(env: NodeJS.ProcessEnv): boolean {
  return (
    env.MO_SERVICE_MARKER?.trim() === "mo" &&
    env.MO_SERVICE_KIND?.trim() === "gateway"
  );
}

function isCurrentGatewayLaunchdJob(env: NodeJS.ProcessEnv): boolean {
  const expectedLabel = resolveGatewayLaunchAgentLabel(env.MO_PROFILE);
  if (
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.MO_SERVICE_MARKER?.trim();
