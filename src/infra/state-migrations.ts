...
function resolveDefaultExecApprovalsStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env, homedir), ".mo");
}
...
function detectLegacyExecApprovalsMigration(params: {
  ...
  return {
    ...
    targetPath,
    hasLegacy:
      Boolean(params.env.MO_STATE_DIR?.trim()) &&
...
async function migrateLegacyTaskRunsSidecar(params: {
  ...
  try {
    ...
    runOpenClawStateWriteTransaction(
      ...
      { env: { ...process.env, MO_STATE_DIR: params.stateDir } },
    );
    if (importedTasks > 0) {
      changes.push(
...
const DEFAULT_VOICEWAKE_TRIGGERS = ["mo", "claude", "computer"];
...
function migrateLegacyVoiceWakeSettings(params: {
  ...
  const changes: string[] = [];
  const warnings: string[] = [];
  const env = { ...process.env, MO_STATE_DIR: params.stateDir };
...
async function withPluginStateImportEnv<T>(
  ...
  const previous = process.env.MO_STATE_DIR;
  process.env.MO_STATE_DIR = plan.stateDir;
  try {
    return await run();
  } finally {
    if (previous === undefined) {
      ...
      process.env.MO_STATE_DIR = previous;
    }
  }
}
...
function aliasedSessionStoreMigrationWarning(params: {
  ...
  return `Deferred ${params.subject} ${params.count} ambiguous session key(s) in aliased store ${params.storePath}; remove filesystem aliases or configure one canonical session.store path, then rerun mo doctor --fix`;
}

function unresolvedSessionStoreIdentityWarning(subject: string, storePath: string): string {
  return `Deferred ${subject} for ${storePath}; filesystem identity could not be established for every configured store path. Restore path access or configure one canonical session.store path, then rerun mo doctor --fix`;
}

function distinctSessionStoreAliasWarning(subject: string, storePath: string): string {
  return `Deferred ${subject} in aliased store ${storePath}; atomic replacement cannot update distinct filesystem aliases as one operation. Remove filesystem aliases or configure one canonical session.store path, then rerun mo doctor --fix`;
}
...
export async function autoMigrateLegacyStateDir(params: {
  ...
  const warnings: string[] = [];
  const changes: string[] = [];
  const hasCustomStateDir = Boolean(env.MO_STATE_DIR?.trim());
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        warnings.push(
          `Rollback failed; set MO_STATE_DIR=${targetDir} to avoid split state: ${String(rollbackErr)}`,
        );
...
async function migrateLegacySessions(
  ...
  if (detected.sessions.targetStoreAliases.hasFinalSymlink) {
    warnings.push(
      `Deferred legacy session migration in final-component symlink store ${detected.sessions.targetStorePath}; configure one canonical session.store path, then rerun mo doctor --fix`,
    );
    return { changes, warnings };
  }
  ...
  if (!targetReadable) {
    if (options.recoverCorruptTargetStore) {
      ...
      warnings.push(
        `Target sessions store unreadable; left untouched to avoid overwriting at ${detected.sessions.targetStorePath}. Run mo doctor --fix to archive it and retry the legacy merge.`,
      );
    }
  }
...
function migrateLegacyStateSchema(detected: LegacyStateDetection): {
  ...
  return repairOpenClawStateDatabaseSchema({
    env: { ...process.env, MO_STATE_DIR: detected.stateDir },
  });
}
...
export async function migrateOrphanedSessionKeys(params: {
  ...
  for (const [mappedStorePath, storeAgentIds] of storeMap) {
    ...
    if (storeAliases.hasFinalSymlink) {
      warnings.push(
        `Deferred session key migration in final-component symlink store ${storePath}; configure one canonical session.store path, then rerun mo doctor --fix`,
      );
      continue;
    }
...
async function migrateLegacyAcpSessionMetadata(params: {
  ...
  for (const { target, agentIds, aliasCandidates } of storeGroups) {
    ...
    if (hasLegacyAcpMetadata && storeAliases.hasFinalSymlink) {
      warnings.push(
        `Deferred ACP metadata migration in final-component symlink store ${storePath}; configure one canonical session.store path, then rerun mo doctor --fix`,
      );
      continue;
    }
...
export async function autoMigrateLegacyState(params: {
  ...
  const hasCustomAgentDir = env.MO_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim();
