...
import { resolveOpenClawStateSqlitePath } from "../state/mo-state-db.paths.js";
import {
...
async function runHelperWithExistingSentinel(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-handoff-helper-test-"));
  tempDirs.add(tmpDir);
  ...
  const helperParams = JSON.parse(await fs.readFile(args[1] ?? "", "utf-8")) as Record<
    ...
    unknown
  >;
  const env = { MO_STATE_DIR: tmpDir } as NodeJS.ProcessEnv;
...
async function runHelperWithCommand(params: {
  ...
  const { execFile } =
    await vi.importActual<typeof import("node:child_process")>("node:child_process");
  const { startManagedServiceUpdateHandoff } = await import("./update-managed-service-handoff.js");
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-handoff-recovery-test-"));
  tempDirs.add(tmpDir);
    ...
    `${JSON.stringify(
      {
        ...
        stateDatabasePath: resolveOpenClawStateSqlitePath({ MO_STATE_DIR: tmpDir }),
        logPath: path.join(tmpDir, "handoff.log"),
        sensitivePaths: [],
...
async function writeFakeSystemctl(): Promise<{ binDir: string; recordPath: string }> {
  const binDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-recovery-bin-"));
  tempDirs.add(binDir);
...
async function writeFakeLaunchctl(): Promise<{ binDir: string; recordPath: string }> {
  const binDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-launchctl-bin-"));
  tempDirs.add(binDir);
...
describe("managed service update handoff", () => {
  it("strips process supervisor hints while preserving service identity for the CLI handoff", async () => {
    ...
    const serviceIdentityEnv = {
      MO_LAUNCHD_LABEL: "com.example.mo.test",
      MO_SYSTEMD_UNIT: "mo-test.service",
      MO_WINDOWS_TASK_NAME: "OpenClaw Test Gateway",
    ...
    expect(result.command).toBe("mo update --yes --timeout 1800");
    ...
    expect(helperParams.stateDatabasePath).toMatch(/mo\.sqlite$/u);
    ...
    expect(options.env.MO_UPDATE_RUN_HANDOFF).toBe("1");
  ...
  it("launches systemd handoffs through a transient user scope", async () => {
    const { startManagedServiceUpdateHandoff } =
      await import("./update-managed-service-handoff.js");
    const binDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-systemd-run-bin-"));
    tempDirs.add(binDir);
    ...
    const result = await startManagedServiceUpdateHandoff({
      ...
      env: {
        PATH: binDir,
        MO_SYSTEMD_UNIT: "mo-gateway.service",
    ...
    expect(args.slice(0, 4)).toEqual([
      ...
      "--scope",
      "--collect",
      "--unit=mo-update-handoff-123.scope",
    ]);
    ...
    expect(options.detached).toBe(true);
    expect(options.env.MO_SYSTEMD_UNIT).toBe("mo-gateway.service");
  ...
  it("starts the managed gateway service when the update command fails after handoff", async () => {
    ...
    const result = await runHelperWithCommand({
      ...
      serviceRecovery: { kind: "systemd", unit: "mo-gateway.service" },
      pathPrepend: binDir,
    });

    expect(result.code).toBe(7);
    await expect(fs.readFile(recordPath, "utf-8")).resolves.toBe(
      "--user start mo-gateway.service\n",
    );
  });
  ...
  it("retries launchd start when bootstrap reports an already-loaded label", async () => {
    ...
    const result = await runHelperWithCommand({
      ...
      serviceRecovery: {
        kind: "launchd",
        uid: 501,
        label: "com.example.mo",
        plistPath: "/Users/test/Library/LaunchAgents/com.example.mo.plist",
      },
      pathPrepend: binDir,
    });
    ...
    await expect(fs.readFile(recordPath, "utf-8")).resolves.toBe(
      [
        "kickstart gui/501/com.example.mo",
        "enable gui/501/com.example.mo",
        "bootstrap gui/501 /Users/test/Library/LaunchAgents/com.example.mo.plist",
        "kickstart gui/501/com.example.mo",
        "",
      ].join("\n"),
    );
    ...
    const cases = [
      {
        ...
        env: { MO_LAUNCHD_LABEL: "com.example.mo.test", HOME: "/Users/test" },
        expected: {
          kind: "launchd",
          uid: typeof process.getuid === "function" ? process.getuid() : 501,
          label: "com.example.mo.test",
          plistPath: "/Users/test/Library/LaunchAgents/com.example.mo.test.plist",
        },
      },
      {
        supervisor: "schtasks" as const,
        env: { MO_WINDOWS_TASK_NAME: "OpenClaw Test Gateway" },
  ...
  it("sweeps stale handoff temp directories while keeping fresh handoff logs", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-handoff-cleanup-test-"));
    tempDirs.add(tmpDir);
    ...
    const unrelatedDir = path.join(tmpDir, "mo-other-temp");
