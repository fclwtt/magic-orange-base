...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "mo.internal", expected: "mo.internal." },
    { value: "mo.internal.", expected: "mo.internal." },
    { value: "  mo.internal  ", expected: "mo.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { MO_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { MO_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
    ...
    },
    {
      ...
      params: {
        env: { MO_WIDE_AREA_DOMAIN: "foo/bar" } as NodeJS.ProcessEnv,
      },
      expected: null,
    },
  ...
  it("builds valid zone paths under the DNS config directory", () => {
    ...
    const zonePath = getWideAreaZonePath("mo.internal.");

    expect(zonePath).toBe(path.join(dnsDir, "mo.internal.db"));
    expect(path.relative(dnsDir, zonePath)).toBe("mo.internal.db");
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expectZoneRecords(txt, [
      ...
      `_mo-gw._tcp IN PTR studio-london._mo-gw._tcp`,
      `studio-london._mo-gw._tcp IN SRV 0 0 18789 studio-london`,
      `displayName=Mac Studio (OpenClaw)`,
      `gatewayPort=18789`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/mo`,
  ...
  it.each([
    ...
    },
    {
      ...
      records: [
        `$ORIGIN mo.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._mo-gw._tcp IN TXT`,
        `displayName=Mac Studio (OpenClaw)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("mo.internal."),
      changed: false,
    });
    expect(writeSpy).not.toHaveBeenCalled();
  });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("mo.internal."),
      expectedZoneText,
      "utf-8",
    );
  });
