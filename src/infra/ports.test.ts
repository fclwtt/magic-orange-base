...
describe("ports helpers", () => {
  ...
  it("prints an OpenClaw-specific hint when port details look like another OpenClaw instance", async () => {
    ...
    await handlePortError(
      new PortInUseError(18789, "node dist/index.js mo gateway"),
      18789,
      "gateway start",
      runtime,
    ).catch(() => {});
...
describeUnix("inspectPortUsage", () => {
  ...
  it("falls back to ss when lsof is unavailable", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "ps") {
        if (argv.includes("command=")) {
          return {
            stdout: "node /tmp/mo/dist/index.js gateway --port 18789\n",
            stderr: "",
            code: 0,
          };
        }
    ...
    try {
      ...
      expect(result.listeners[0]?.commandLine).toContain("mo");
      ...
      if (command === "ps") {
        ...
        if (argv.includes("command=")) {
          return {
            stdout:
              pid === "111"
                ? "node /tmp/newer-mo/dist/index.js logs --follow\n"
                : pid === "222"
                  ? "node /tmp/older-mo/dist/index.js gateway run\n"
  ...
  it("deduplicates repeated lsof listener records for one process address", async () => {
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "ps") {
        if (argv.includes("command=")) {
          return {
            stdout: "node /tmp/mo/dist/index.js gateway run\n",
            stderr: "",
            code: 0,
          };
        }
  ...
  it("reports multiple lsof socket records for one process", async () => {
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "ps") {
        if (argv.includes("command=")) {
          return {
            stdout: "node /tmp/newer-mo/dist/index.js logs --follow\n",
            stderr: "",
            code: 0,
          };
        }
  ...
  it("reports established gateway client connections from netstat", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === getWindowsPowerShellExePath()) {
        return {
          stdout:
            '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\me\\AppData\\Roaming\\npm\\node_modules\\mo\\dist\\index.js logs --follow\r\n',
          stderr: "",
          code: 0,
        };
      }
    ...
    expect(result.connections[0]?.commandLine).toContain("mo");
  });
  ...
  it("uses PowerShell process command lines to classify OpenClaw listeners", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === getWindowsPowerShellExePath()) {
        return {
          stdout:
            '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\me\\AppData\\Roaming\\npm\\node_modules\\mo\\dist\\index.js gateway run\r\n',
          stderr: "",
          code: 0,
        };
      }
  ...
  it("falls back to wmic when PowerShell cannot read the command line", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === getWindowsWmicExePath()) {
        return {
          stdout: "CommandLine=node.exe C:\\mo\\dist\\index.js gateway run\r\n",
          stderr: "",
          code: 0,
        };
      }
