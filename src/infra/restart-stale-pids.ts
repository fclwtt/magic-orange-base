...
import { uniqueValues } from "@mo/normalization-core/string-normalization";
...
function parsePidsFromLsofOutput(
  ...
  for (const entry of parseLsofEntries(stdout)) {
    ...
    if (entry.cmd && normalizeLowercaseStringOrEmpty(entry.cmd).includes("mo")) {
      pids.push(entry.pid);
      continue;
    }
...
/**
 * Windows: find listening PIDs on the port, then verify each is an mo
...
function findGatewayPidsOnPortWithProtectedPidSync(
  ...
  if (process.platform === "win32") {
    ...
    // command-line verification to find only mo gateway processes.
 ...
 * Returns only PIDs that belong to mo gateway processes (not the current process).
 */
