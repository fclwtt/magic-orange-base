...
const testEnvSnapshot = captureEnv(["MO_HOME", "MO_STATE_DIR"]);
...
describe("exec approvals store helpers", () => {
  it("expands home-prefixed default file and socket paths", () => {
    ...
    expect(path.normalize(resolveExecApprovalsPath())).toBe(
      path.normalize(path.join(dir, ".mo", "exec-approvals.json")),
    );
    expect(path.normalize(resolveExecApprovalsSocketPath())).toBe(
      path.normalize(path.join(dir, ".mo", "exec-approvals.sock")),
    );
    expect(resolveExecApprovalsDisplayPath()).toBe("~/.mo/exec-approvals.json");
  });

  it("uses MO_STATE_DIR for default file and socket paths", () => {
    const dir = createHomeDir();
    ...
    expect(resolveExecApprovalsTranscriptPath()).toBe("$MO_STATE_DIR/exec-approvals.json");
  ...
  it("fails closed without writing target approvals before state migration runs", () => {
    ...
    fs.writeFileSync(
      ...
      `${JSON.stringify({
        version: 1,
        socket: {
          path: path.join(dir, ".mo", "exec-approvals.sock"),
          token: "legacy-token",
        },
        defaults: {
          security: "deny",
  ...
  it("accepts a symlinked MO_HOME as the trusted approvals root", () => {
    ...
    setTestEnvValue("MO_HOME", linkedHome);

    saveExecApprovals({ version: 1, defaults: { security: "full" }, agents: {} });

    expect(
      fs.readFileSync(path.join(realHome, ".mo", "exec-approvals.json"), "utf8"),
  ...
  it("refuses to traverse symlinked approvals components below a symlinked home", () => {
    ...
    fs.symlinkSync(realHome, linkedHome, "dir");
    fs.symlinkSync(linkedStateTarget, path.join(realHome, ".mo"), "dir");
    setTestEnvValue("MO_HOME", linkedHome);
