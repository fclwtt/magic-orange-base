...
const CREDENTIAL_AND_GATEWAY_ENV_KEYS = [
  ...
  "MO_LIVE_ANTHROPIC_KEY",
  "MO_LIVE_ANTHROPIC_KEYS",
  "MO_LIVE_GEMINI_KEY",
  "MO_LIVE_OPENAI_KEY",
  "MO_GATEWAY_TOKEN",
  "MO_GATEWAY_PASSWORD",
...
const BUNDLED_TRUST_ROOT_ENV_LINES = [
  "MO_BROWSER_CONTROL_MODULE=data:text/javascript,boom",
  "MO_BUNDLED_HOOKS_DIR=./attacker-hooks",
  "MO_BUNDLED_PLUGINS_DIR=./attacker-plugins",
  "MO_BUNDLED_SKILLS_DIR=./attacker-skills",
  "MO_SKIP_BROWSER_CONTROL_SERVER=1",
] as const;
...
async function withDotEnvFixture(run: (fixture: DotEnvFixture) => Promise<void>) {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "mo-dotenv-test-"));
  const cwdDir = path.join(base, "cwd");
  const stateDir = path.join(base, "state");
  setTestEnvValue("MO_STATE_DIR", stateDir);
...
describe("loadDotEnv", () => {
  it("loads ~/.mo/.env as fallback without overriding CWD .env", async () => {
  ...
  it("loads the Ubuntu gateway.env compatibility fallback after ~/.mo/.env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        setTestEnvValue("HOME", base);
        const defaultStateDir = path.join(base, ".mo");
        setTestEnvValue("MO_STATE_DIR", defaultStateDir);
        await writeEnvFile(path.join(defaultStateDir, ".env"), "FOO=from-global\n");
        await writeEnvFile(
          path.join(base, ".config", "mo", "gateway.env"),
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "MO_STATE_DIR=./evil-state",
            "MO_CONFIG_PATH=./evil-config.json",
        ...
        expect(process.env.MO_STATE_DIR).toBe(stateDir);
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "MO_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "MO_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "MO_LIVE_GEMINI_KEY=sk-gemini-live",
            "MO_LIVE_OPENAI_KEY=sk-openai-live",
            "MO_GATEWAY_TOKEN=attacker-token",
            "MO_GATEWAY_PASSWORD=attacker-password",
            "MO_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "MO_AGENT_DIR=./evil-agent",
            `MO_BUNDLED_PLUGINS_DIR=${bundledPluginsDir}`,
            "MO_OAUTH_DIR=./evil-oauth",
  ...
  it("blocks MO_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "MO_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.MO_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.MO_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "MO_ALLOW_PLUGIN_INSTALL_OVERRIDES=1",
            'MO_PLUGIN_INSTALL_OVERRIDES={"codex":"npm-pack:/tmp/codex.tgz"}',
          ].join("\n"),
        );

        delete process.env.MO_ALLOW_PLUGIN_INSTALL_OVERRIDES;
        delete process.env.MO_PLUGIN_INSTALL_OVERRIDES;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.MO_ALLOW_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
        expect(process.env.MO_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "MO_PINNED_PYTHON=./attacker-python",
            "MO_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );

        delete process.env.MO_PINNED_PYTHON;
        delete process.env.MO_PINNED_WRITE_PYTHON;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.MO_PINNED_PYTHON).toBeUndefined();
        expect(process.env.MO_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "MO_PINNED_PYTHON=/trusted/python",
            "MO_PINNED_WRITE_PYTHON=/trusted/write-python",
        ...
        expect(process.env.MO_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.MO_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "MO_LIVE_GEMINI_KEY=sk-gemini-live",
            "MO_LIVE_OPENAI_KEY=sk-openai-live",
            "MO_GATEWAY_TOKEN=trusted-token",
            "MO_GATEWAY_PASSWORD=trusted-password",
            "MO_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        clearEnv(CREDENTIAL_AND_GATEWAY_ENV_KEYS);
        ...
        expect(process.env.MO_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.MO_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.MO_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.MO_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.MO_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.MO_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.MO_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "MO_STATE_DIR=./evil-state\n");
...
describe("loadCliDotEnv", () => {
  it("blocks MO_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(path.join(cwdDir, ".env"), "MO_STATE_DIR=./evil-state\n");
  ...
  it("does not load gateway.env when MO_STATE_DIR is explicitly set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        const customStateDir = path.join(base, "custom-state");
        setTestEnvValue("HOME", base);
        setTestEnvValue("MO_STATE_DIR", customStateDir);
        await writeEnvFile(
          path.join(base, ".config", "mo", "gateway.env"),
          "FOO=from-gateway\n",
        );
        ...
        expect(process.env.FOO).toBeUndefined();
        expect(process.env.MO_STATE_DIR).toBe(customStateDir);
        expect(process.env.BAR).toBeUndefined();
      });
    });
  });
  ...
  it("keeps the legacy state-dir fallback for CLI dotenv loading", async () => {
    await withIsolatedEnvAndCwd(async () => {
      const base = await fs.mkdtemp(path.join(os.tmpdir(), "mo-dotenv-legacy-"));
      const cwdDir = path.join(base, "cwd");
      const legacyStateDir = path.join(base, ".clawdbot");
      setTestEnvValue("HOME", base);
      deleteTestEnvValue("MO_STATE_DIR");
      delete process.env.MO_TEST_FAST;
      await fs.mkdir(cwdDir, { recursive: true });
  ...
  it("blocks workspace .env takeover vars before loading the global fallback", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        delete process.env.SAFE_KEY;
        deleteTestEnvValue("MO_CONFIG_PATH");
        delete process.env.MO_BUNDLED_PLUGINS_DIR;
        delete process.env.NODE_OPTIONS;
        delete process.env.NODE_REDIRECT_WARNINGS;
        delete process.env.NODE_REPL_EXTERNAL_MODULE;
        delete process.env.NODE_REPL_HISTORY;
        ...
        expect(process.env.MO_STATE_DIR).toBe(stateDir);
        expect(process.env.MO_CONFIG_PATH).toBeUndefined();
        expect(process.env.MO_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.NODE_OPTIONS).toBeUndefined();
        expect(process.env.NODE_REDIRECT_WARNINGS).toBeUndefined();
        expect(process.env.NODE_REPL_EXTERNAL_MODULE).toBeUndefined();
        expect(process.env.NODE_REPL_HISTORY).toBeUndefined();
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        const plugin: PluginManifestRecord = {
          ...
          manifestPath: "/plugins/runtime-cloud/mo.plugin.json",
          providerAuthEnvVars: {
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        const runtimeControlKeys = [
          ...
          "MO_WORKSPACE_DIR",
          "MO_MDNS_HOSTNAME",
          "MO_SESSION_CACHE_TTL_MS",
          "MO_UPDATE_PACKAGE_SPEC",
          "MO_GATEWAY_PORT",
          "MO_GATEWAY_URL",
          "MO_CLAWHUB_URL",
          "CLAWHUB_URL",
          ...
          "CLAWHUB_AUTH_TOKEN",
          "CLAWHUB_CONFIG_PATH",
          "MO_DISABLE_BUNDLED_PLUGINS",
          "MO_ALLOW_INSECURE_PRIVATE_WS",
          "MO_BROWSER_EXECUTABLE_PATH",
          "MO_WHATSAPP_WEB_SOCKET_URL",
          "EXAMPLE_API_HOST",
          "HOMEBREW_BREW_FILE",
          "HOMEBREW_PREFIX",
          "IRC_HOST",
          ...
          "MO_CACHE_TRACE",
          "MO_CACHE_TRACE_FILE",
          "MO_CACHE_TRACE_MESSAGES",
          "MO_CACHE_TRACE_PROMPT",
          "MO_CACHE_TRACE_SYSTEM",
          "MO_SHOW_SECRETS",
          "MO_PLUGIN_CATALOG_PATHS",
          "MO_MPM_CATALOG_PATHS",
          "MO_NODE_EXEC_HOST",
          "MO_NODE_EXEC_FALLBACK",
          "MO_ALLOW_PROJECT_LOCAL_BIN",
          "PATH",
          "HOMEBREW_BREW_FILE",
          "HOMEBREW_PREFIX",
          "SystemRoot",
  ...
  it("still allows user-defined non-control vars through workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        ...
        expect(process.env.APP_GITHUB_REPO).toBe("mo/mo");
