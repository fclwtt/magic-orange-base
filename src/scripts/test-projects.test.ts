...
describe("test-projects args", () => {
  ...
  it("routes boundary targets to the boundary config", () => {
    expect(buildVitestRunPlans(["src/infra/mo-root.test.ts"])).toEqual([
      {
        config: "test/vitest/vitest.boundary.config.ts",
        forwardedArgs: [],
        includePatterns: ["src/infra/mo-root.test.ts"],
        watchMode: false,
      },
    ]);
  });
  ...
  it("keeps conservative local full-suite runs on leaf project configs", () => {
    const originalVitestMaxWorkers = process.env.MO_VITEST_MAX_WORKERS;
    const originalTestWorkers = process.env.MO_TEST_WORKERS;
    const originalProjectParallel = process.env.MO_TEST_PROJECTS_PARALLEL;
    const originalLeafShards = process.env.MO_TEST_PROJECTS_LEAF_SHARDS;
    const originalCi = process.env.CI;
    const originalActions = process.env.GITHUB_ACTIONS;
    try {
      process.env.MO_VITEST_MAX_WORKERS = "1";
      delete process.env.MO_TEST_WORKERS;
      delete process.env.MO_TEST_PROJECTS_PARALLEL;
      delete process.env.MO_TEST_PROJECTS_LEAF_SHARDS;
      ...
      if (originalVitestMaxWorkers === undefined) {
        delete process.env.MO_VITEST_MAX_WORKERS;
      } else {
        process.env.MO_VITEST_MAX_WORKERS = originalVitestMaxWorkers;
      }
      if (originalTestWorkers === undefined) {
        delete process.env.MO_TEST_WORKERS;
      } else {
        process.env.MO_TEST_WORKERS = originalTestWorkers;
      }
      if (originalProjectParallel === undefined) {
        delete process.env.MO_TEST_PROJECTS_PARALLEL;
      } else {
        process.env.MO_TEST_PROJECTS_PARALLEL = originalProjectParallel;
      }
      if (originalLeafShards === undefined) {
        delete process.env.MO_TEST_PROJECTS_LEAF_SHARDS;
      } else {
        process.env.MO_TEST_PROJECTS_LEAF_SHARDS = originalLeafShards;
      }
  ...
  it("uses a bounded local default for full-suite project parallelism", () => {
    expect(
      resolveParallelFullSuiteConcurrency(
        58,
        {
          MO_TEST_PROJECTS_LEAF_SHARDS: "1",
        },
        {
          cpuCount: 8,
          loadAverage1m: 0,
  ...
  it("gives parallel Vitest shards separate filesystem module caches", () => {
    ...
    expect(firstEnv?.MO_VITEST_FS_MODULE_CACHE_PATH?.replaceAll("\\", "/")).toBe(
      "/repo/node_modules/.experimental-vitest-cache/0-test-vitest-vitest.gateway.config.ts",
    );
    expect(specs[1]?.env.MO_VITEST_FS_MODULE_CACHE_PATH?.replaceAll("\\", "/")).toBe(
  ...
  it("preserves explicit Vitest filesystem module cache paths", () => {
    ...
    expect(
      applyParallelVitestCachePaths(specs, {
        cwd: "/repo",
        env: {
          MO_VITEST_FS_MODULE_CACHE_PATH: "/tmp/cache",
        },
      }),
    ).toBe(specs);
  });
    ...
    expect(buildVitestRunPlans(["test/helpers/temp-dir.ts"])).toEqual([
      {
        ...
        includePatterns: [
          ...
          "test/scripts/resolve-mo-ref.test.ts",
        ],
        watchMode: false,
      },
      {
      ...
      },
      {
        ...
        forwardedArgs: [
          "src/state/mo-agent-db.test.ts",
          "src/state/mo-state-db.test.ts",
  ...
  it("writes scoped include files for routed extension runs", () => {
    ...
    expect(spec?.includeFilePath).toContain("mo-vitest-include-");
    expect(spec?.env.MO_VITEST_INCLUDE_FILE).toBe(spec?.includeFilePath);
  });

  it("rejects explicit test file targets that do not exist", () => {
    expect(findUnmatchedExplicitTestTargets(["src/not-a-real-mo-test.test.ts"])).toEqual([
      {
        target: "src/not-a-real-mo-test.test.ts",
  ...
  it("rejects explicit non-test file targets with no sibling tests", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-test-targets-"));
    try {
  ...
  it("skips channel contract configs with no matching external include patterns", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-contract-include-"));
    try {
      ...
      const specs = createVitestRunSpecs(
        ...
        {
          baseEnv: {
            MO_VITEST_INCLUDE_FILE: includeFile,
