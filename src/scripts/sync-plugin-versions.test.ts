...
describe("syncPluginVersions", () => {
  ...
  it("preserves workspace mo devDependencies and plugin host floors", () => {
    const rootDir = makeTempDir(tempDirs, "mo-sync-plugin-versions-");
    ...
    expect(summary.updated).toContain("@mo/imessage");
    expect(updatedPackage.version).toBe("2026.4.1");
    expect(updatedPackage.devDependencies?.mo).toBe("workspace:*");
    expect(updatedPackage.peerDependencies?.mo).toBe(">=2026.4.1");
    expect(updatedPackage.mo?.install?.minHostVersion).toBe(">=2026.3.30");
    expect(updatedPackage.mo?.compat?.pluginApi).toBe(">=2026.4.1");
    expect(updatedPackage.mo?.build?.moVersion).toBe("2026.4.1");
  });

  it("reports pending version sync without writing in check mode", () => {
    const rootDir = makeTempDir(tempDirs, "mo-sync-plugin-versions-check-");
    ...
    expect(summary.updated).toEqual(["@mo/discord"]);
    expect(unchangedPackage.version).toBe("2026.4.1");
    expect(unchangedPackage.peerDependencies?.mo).toBe(">=2026.4.1");
    expect(unchangedPackage.mo?.compat?.pluginApi).toBe(">=2026.4.1");
  });

  it("uses the base release version for beta changelog entries", () => {
    const rootDir = makeTempDir(tempDirs, "mo-sync-plugin-versions-beta-changelog-");
    ...
    expect(summary.changelogged).toEqual(["@mo/matrix"]);
