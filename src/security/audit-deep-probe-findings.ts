...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("mo status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("mo security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set MO_GATEWAY_TOKEN/MO_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("mo security audit --deep")}".`,
    });
  }
  return findings;
}
