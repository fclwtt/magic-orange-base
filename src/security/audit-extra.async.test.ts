...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mo-security-audit-async-"));
  ...
  it("audits canonical auth profile SQLite store permissions", async () => {
    ...
    const databasePath = path.join(agentDir, "mo-agent.sqlite");
    ...
    expect(readableAuthTargets).toEqual(
      expect.arrayContaining([
        expect.stringContaining("mo-agent.sqlite"),
        expect.stringContaining("mo-agent.sqlite-wal"),
        expect.stringContaining("mo-agent.sqlite-shm"),
        expect.stringContaining("mo-agent.sqlite-journal"),
      ]),
    );
  });
});
