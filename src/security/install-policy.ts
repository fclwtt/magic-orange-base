...
import type { OpenClawConfig, SecurityConfig } from "../config/types.mo.js";
...
export type InstallPolicySource = {
  kind:
    ...
    | "upload"
    | "workspace";
  authority: "mo" | "official" | "third-party" | "unknown" | "user";
  mutable: boolean;
  network: boolean;
};
...
export async function runInstallPolicy(params: {
  ...
  const input = JSON.stringify({
    protocolVersion: 1,
    moVersion: resolveRuntimeServiceVersion(params.env ?? process.env),
    ...params.request,
  });
