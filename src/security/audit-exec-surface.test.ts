...
describe("security audit exec surface findings", () => {
  // Redirect the OpenClaw home (MO_HOME wins over HOME/USERPROFILE in
  // `resolveRawHomeDir`) to a per-test tempdir so `saveExecApprovals` never
  // touches the real `~/.mo/exec-approvals.json` on the host running
  // the suite.
