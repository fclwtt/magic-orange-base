...
const sandboxRootTracker = createSuiteTempRootTracker({ prefix: "mo-docker-setup-" });

const prestartContainerEnvFlags = [
  "-e HOME=/home/node",
  "-e MO_HOME=/home/node",
  "-e MO_STATE_DIR=/home/node/.mo",
  "-e MO_CONFIG_PATH=/home/node/.mo/mo.json",
  "-e MO_CONFIG_DIR=/home/node/.mo",
  "-e MO_WORKSPACE_DIR=/home/node/.mo/workspace",
].join(" ");
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    MO_GATEWAY_TOKEN: "test-token",
    MO_CONFIG_DIR: join(sandbox.rootDir, "config"),
    MO_WORKSPACE_DIR: join(sandbox.rootDir, "mo"),
    MO_AUTH_PROFILE_SECRET_DIR: join(sandbox.rootDir, "auth-profile-secrets"),
  };
...
function isGatewayStartLine(line: string) {
  return line.includes("compose") && line.includes(" up -d") && line.includes("mo-gateway");
}
...
async function runDockerSetupWithUnsetGatewayToken(
  ...
  const result = runDockerSetup(sandbox, {
    ...
    MO_CONFIG_DIR: configDir,
    MO_WORKSPACE_DIR: workspaceDir,
  });
...
describe("scripts/docker/setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and Docker build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      MO_DOCKER_APT_PACKAGES: "curl wget",
      MO_EXTRA_MOUNTS: undefined,
      MO_HOME_VOLUME: "mo-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("MO_IMAGE_APT_PACKAGES=curl wget");
    expect(envFile).toContain("MO_EXTRA_MOUNTS=");
    expect(envFile).toContain("MO_HOME_VOLUME=mo-home"); // pragma: allowlist secret
    expect(envFile).toContain("MO_DISABLE_BONJOUR=");
    expect(envFile).toContain(
      `MO_AUTH_PROFILE_SECRET_DIR=${join(activeSandbox.rootDir, "auth-profile-secrets")}`,
    );
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("mo-home:/home/node");
    expect(extraCompose).toContain(
      `${join(activeSandbox.rootDir, "auth-profile-secrets")}:/home/node/.config/mo`,
    );
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("mo-home:");
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg MO_IMAGE_APT_PACKAGES=curl wget");
    expect(log).toContain(
      `run --rm --no-deps ${prestartContainerEnvFlags} --entrypoint node mo-gateway dist/index.js onboard --mode local --no-install-daemon --gateway-auth token --gateway-token-ref-env MO_GATEWAY_TOKEN --skip-ui --suppress-gateway-token-output`,
    );
    ...
    expect(result.stdout).toContain("logs -f mo-gateway");
    expect(result.stdout).not.toContain("test-token");
    expect(result.stdout).not.toContain("#token=");
    expect(log).toContain(
      `run --rm --no-deps ${prestartContainerEnvFlags} --entrypoint node mo-gateway dist/index.js config set --batch-json [{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]`,
    );
    expect(log).not.toContain("run --rm mo-cli onboard --mode local --no-install-daemon");
  });
  ...
  it("allows ordinary spaces in host persistence paths and quotes generated mounts", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      MO_CONFIG_DIR: configDir,
      MO_WORKSPACE_DIR: workspaceDir,
      MO_AUTH_PROFILE_SECRET_DIR: authProfileSecretDir,
      MO_HOME_VOLUME: homeVolumeDir,
      MO_EXTRA_MOUNTS: `${extraMountSource}:/mnt/extra data:ro`,
    });

    expect(result.status).toBe(0);
    expect(result.stderr).not.toContain("cannot contain whitespace");
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain(`MO_CONFIG_DIR=${configDir}`);
    expect(envFile).toContain(`MO_WORKSPACE_DIR=${workspaceDir}`);
    expect(envFile).toContain(`MO_AUTH_PROFILE_SECRET_DIR=${authProfileSecretDir}`);
    ...
    expect(extraCompose).toContain(`"${configDir}:/home/node/.mo"`);
    expect(extraCompose).toContain(`"${workspaceDir}:/home/node/.mo/workspace"`);
    expect(extraCompose).toContain(`"${authProfileSecretDir}:/home/node/.config/mo"`);
  ...
  it("persists explicit Docker Bonjour opt-in overrides", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      MO_DISABLE_BONJOUR: "0",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("MO_DISABLE_BONJOUR=0");
  });

  it("normalizes legacy MO_DOCKER_APT_PACKAGES into MO_IMAGE_APT_PACKAGES", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("MO_IMAGE_APT_PACKAGES=curl wget");
    expect(envFile).not.toContain("MO_DOCKER_APT_PACKAGES");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg MO_IMAGE_APT_PACKAGES=curl wget");
    expect(log).not.toContain("--build-arg MO_DOCKER_APT_PACKAGES");
  });

  it("prefers MO_IMAGE_APT_PACKAGES over legacy MO_DOCKER_APT_PACKAGES", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      MO_IMAGE_APT_PACKAGES: "curl wget httpie",
      MO_DOCKER_APT_PACKAGES: "curl wget",
    });
    expect(result.status).toBe(0);

    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("MO_IMAGE_APT_PACKAGES=curl wget httpie");
    expect(envFile).not.toContain("MO_DOCKER_APT_PACKAGES");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg MO_IMAGE_APT_PACKAGES=curl wget httpie");
    expect(log).not.toMatch(/--build-arg MO_IMAGE_APT_PACKAGES=curl wget(?! httpie)/);
  });

  it("explicitly empty MO_IMAGE_APT_PACKAGES suppresses legacy fallback", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      MO_IMAGE_APT_PACKAGES: "",
      MO_DOCKER_APT_PACKAGES: "curl wget",
    });
    expect(result.status).toBe(0);

    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("MO_IMAGE_APT_PACKAGES=");
    expect(envFile).not.toContain("curl wget");

    const log = await readDockerLog(activeSandbox);
    expect(log).not.toContain("--build-arg MO_IMAGE_APT_PACKAGES=curl wget");
  });

  it("avoids shared-network mo-cli before the gateway is started", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    const prestartCliRunLines = collectMatchingLines(prestartLines, (line) =>
      /\bcompose\b.*\brun\b.*\bmo-cli\b/.test(line),
    );
  ...
  it("pins setup-time CLI state paths inside the container", async () => {
    ...
    await resetDockerLog(activeSandbox);
    const result = runDockerSetup(activeSandbox, {
      MO_HOME: "/mnt/c/Users/Trevor",
      MO_STATE_DIR: "/mnt/c/Users/Trevor/.mo",
      MO_CONFIG_PATH: "/mnt/c/Users/Trevor/.mo/mo.json",
      MO_SKIP_ONBOARDING: "1",
    });
    expect(result.status).toBe(0);
  ...
  it("offline mode reuses a preloaded local image without build or pull", async () => {
    ...
    expect(result.stdout).toContain(
      "Using preloaded Docker image: ghcr.io/mo/mo:latest",
    );

    const lines = await readDockerLogLines(activeSandbox);
    const log = lines.join("\n");
    expect(log).toContain("image inspect ghcr.io/mo/mo:latest");
  ...
  it("offline mode fails before setup when the main image is missing", async () => {
    ...
    const result = runDockerSetup(
      activeSandbox,
      {
        MO_IMAGE: "ghcr.io/mo/mo:offline",
        DOCKER_STUB_MISSING_IMAGES: "ghcr.io/mo/mo:offline",
      },
      ["--offline"],
    );
    ...
    expect(result.stderr).toContain(
      "Offline Docker setup requires preloaded image ghcr.io/mo/mo:offline",
    );

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("image inspect ghcr.io/mo/mo:offline");
    expect(log).not.toMatch(/^build /m);
    expect(log).not.toMatch(/^pull /m);
    expect(log).not.toContain("up -d mo-gateway");
  });
  ...
  it("offline sandbox stays disabled when its configured image is missing", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const defaultImage = "registry.example/mo-sandbox:approved";
      const agentImage = " registry.example/mo-sandbox:agent ";
      const result = runDockerSetup(
        activeSandbox,
        {
          MO_SANDBOX: "1",
  ...
  it("offline sandbox validates only effective Docker and browser images", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const defaultImage = "registry.example/mo-sandbox:default";
      const browserImage = "registry.example/mo-sandbox-browser:default";
      const ignoredImages = [
        "registry.example/mo-sandbox:ssh",
        "registry.example/mo-sandbox:shared-agent",
        "registry.example/mo-sandbox-browser:shared-agent",
        "registry.example/mo-sandbox:off",
        "registry.example/mo-sandbox-browser:denied",
      ];
      const result = runDockerSetup(
        activeSandbox,
        {
  ...
  it("offline sandbox rejects an incompatible browser image", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const browserImage = "registry.example/mo-sandbox-browser:stale";
      const result = runDockerSetup(
        activeSandbox,
        {
          MO_SANDBOX: "1",
  ...
  it("writes MO_TZ into .env when given a real IANA timezone", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("MO_TZ=Asia/Shanghai");
  });
  ...
  it("precreates agent data dirs to avoid EACCES in container", async () => {
    ...
    expect(log).toContain("run --rm --no-deps --user root --entrypoint sh mo-gateway -c");
  ...
  it("precreates auth profile secret key dir outside the mounted state dir", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      MO_CONFIG_DIR: configDir,
      MO_WORKSPACE_DIR: workspaceDir,
      MO_AUTH_PROFILE_SECRET_DIR: secretDir,
    });
    ...
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("find /home/node/.config/mo -xdev");
  });

  it("reuses existing config token when MO_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("MO_GATEWAY_TOKEN=config-token-123"); // pragma: allowlist secret
  });

  it("reuses existing .env token when MO_GATEWAY_TOKEN and config token are unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await writeFile(
      join(activeSandbox.rootDir, ".env"),
      "MO_GATEWAY_TOKEN=dotenv-token-123\nMO_GATEWAY_PORT=18789\n", // pragma: allowlist secret
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
      "dotenv-token-reuse",
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("MO_GATEWAY_TOKEN=dotenv-token-123"); // pragma: allowlist secret
  ...
  it("reuses the last non-empty .env token and strips CRLF without truncating '='", async () => {
    ...
    await writeFile(
      ...
      [
        "MO_GATEWAY_TOKEN=",
        "MO_GATEWAY_TOKEN=first-token",
        "MO_GATEWAY_TOKEN=last=token=value\r", // pragma: allowlist secret
      ].join("\n"),
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("MO_GATEWAY_TOKEN=last=token=value"); // pragma: allowlist secret
    expect(envFile).not.toContain("MO_GATEWAY_TOKEN=first-token");
    expect(envFile).not.toContain("\r");
  });

  it("treats MO_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("MO_SANDBOX=");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg MO_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg MO_INSTALL_DOCKER_CLI=1");
  ...
  it("resets stale sandbox mode and overlay when sandbox is not active", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        MO_SANDBOX: "1",
        MO_DOCKER_SOCKET: socketPath,
        DOCKER_STUB_FAIL_MATCH: "--entrypoint docker mo-gateway --version",
      });
  ...
  it("keeps offline policy when sandbox config writes fail and the gateway rolls back", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      ...
      expect(log).toContain(
        "run --pull never --rm --no-deps mo-cli config set agents.defaults.sandbox.mode non-main",
      );
      expect(log).toContain("config set agents.defaults.sandbox.mode off");
      const forceRecreateLine = log
        .split("\n")
        .find((line) => line.includes("--force-recreate mo-gateway"));
      expect(forceRecreateLine).toBe(
        `compose compose -f ${join(activeSandbox.rootDir, "docker-compose.yml")} up -d --pull never --no-build --force-recreate mo-gateway`,
      );
      expect(forceRecreateLine).not.toContain("docker-compose.sandbox.yml");
      expect(log).toContain(
        `image inspect mo-sandbox:bookworm-slim host=unix://${socketPath}`,
      );
      expectOfflineComposePolicy(lines);
      await expectMissingPath(join(activeSandbox.rootDir, "docker-compose.sandbox.yml"));
    });
  ...
  it("rejects injected multiline MO_EXTRA_MOUNTS values", () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      MO_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("MO_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid MO_EXTRA_MOUNTS mount format", () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      MO_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid MO_HOME_VOLUME names", () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      MO_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("MO_HOME_VOLUME must match");
  });

  it("rejects MO_TZ values that are not present in zoneinfo", () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("MO_TZ must match a timezone in /usr/share/zoneinfo");
  });

  it("skips onboarding when MO_SKIP_ONBOARDING is set", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("MO_SKIP_ONBOARDING=1");
  });

  it("treats MO_SKIP_ONBOARDING=0 as disabled and runs onboarding", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      MO_SKIP_ONBOARDING: "0",
    });

    expect(result.status).toBe(0);
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain(
      "onboard --mode local --no-install-daemon --gateway-auth token --gateway-token-ref-env MO_GATEWAY_TOKEN --skip-ui --suppress-gateway-token-output",
    );
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toMatch(/MO_SKIP_ONBOARDING=\n/);
  });
  ...
  it("keeps docker-compose gateway Bonjour advertising in auto mode by default", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(
      compose.match(/MO_DISABLE_BONJOUR: \$\{MO_DISABLE_BONJOUR:-\}/g),
    ).toHaveLength(1);
  });

  it("keeps docker-compose CLI network namespace settings in sync", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose).toContain('network_mode: "service:mo-gateway"');
    expect(compose).toContain("depends_on:\n      - mo-gateway");
  });

  it("keeps docker-compose gateway token env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/MO_GATEWAY_TOKEN: \$\{MO_GATEWAY_TOKEN:-\}/g)).toHaveLength(
      2,
    );
  });
  ...
  it("keeps docker-compose auth profile secret key source durable outside state", async () => {
    ...
    expect(
      compose.split(
        '"${MO_AUTH_PROFILE_SECRET_DIR:-${HOME:-/tmp}/.mo-auth-profile-secrets}:/home/node/.config/mo"',
      ),
    ).toHaveLength(3);
  });
  ...
  it("keeps docker-compose timezone env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/TZ: \$\{MO_TZ:-UTC\}/g)).toHaveLength(2);
  });
  ...
  it("pins container-side state, workspace, and config dirs on both services so host .env paths cannot leak (#77436)", async () => {
    ...
    expect(compose.match(/MO_HOME: \/home\/node$/gm)).toHaveLength(2);
    expect(compose.match(/MO_STATE_DIR: \/home\/node\/\.mo$/gm)).toHaveLength(2);
    expect(
      compose.match(/MO_CONFIG_PATH: \/home\/node\/\.mo\/mo\.json$/gm),
    ).toHaveLength(2);
    expect(compose.match(/MO_CONFIG_DIR: \/home\/node\/\.mo$/gm)).toHaveLength(2);
    expect(
      compose.match(/MO_WORKSPACE_DIR: \/home\/node\/\.mo\/workspace$/gm),
    ).toHaveLength(2);
  });

  it("Dockerfile ARG MO_IMAGE_APT_PACKAGES must not have a default value", async () => {
    // If the ARG has a default (e.g. ARG MO_IMAGE_APT_PACKAGES=""), Docker treats it as
    // "set" even when no --build-arg is passed. That breaks the RUN fallback expression
    // ${MO_IMAGE_APT_PACKAGES-$MO_DOCKER_APT_PACKAGES} because the variable is
    // never truly unset, so legacy-only callers using --build-arg MO_DOCKER_APT_PACKAGES
    ...
    const argLine = dockerfile
      .split("\n")
      .find((line) => line.startsWith("ARG MO_IMAGE_APT_PACKAGES"));
    expect(argLine).toBeDefined();
    // Must be bare `ARG MO_IMAGE_APT_PACKAGES` with no default assignment
    expect(argLine).toBe("ARG MO_IMAGE_APT_PACKAGES");
  });
});
