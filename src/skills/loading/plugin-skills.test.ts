    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "mo.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "mo.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "mo.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("mo-");
  const acpxRoot = await tempDirs.make("mo-acpx-plugin-");
  const helperRoot = await tempDirs.make("mo-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("mo-");
  const pluginRoot = await tempDirs.make("mo-plugin-");
  const outsideDir = await tempDirs.make("mo-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("mo-");
    const pluginRoot = await tempDirs.make("mo-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("mo-");
    const pluginRoot = await tempDirs.make("mo-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
