...
const fixtureSuite = createFixtureSuite("mo-skills-suite-");
let tempHome: TempHomeEnv | null = null;
let skillsHomeEnv: SkillsHomeEnvSnapshot | null = null;
const pluginEnvSnapshot = captureEnv(["MO_DISABLE_BUNDLED_PLUGINS"]);
...
beforeAll(async () => {
  ...
  tempHome = await createTempHomeEnv("mo-skills-home-");
  skillsHomeEnv = setMockSkillsHomeEnv(tempHome.home);
  await fs.mkdir(path.join(tempHome.home, ".mo", "agents", "main", "sessions"), {
    recursive: true,
  });
});
...
describe("buildWorkspaceSkillCommandSpecs", () => {
  ...
  it("includes enabled Claude bundle markdown commands as native OpenClaw slash commands", async () => {
    ...
    const pluginRoot = path.join(workspaceDir, ".mo", "extensions", "compound-bundle");
