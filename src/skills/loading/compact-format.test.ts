...
import { formatSkillsForPrompt as upstreamFormatSkillsForPrompt } from "mo/plugin-sdk/agent-sessions";
...
describe("applySkillsPromptLimits (via buildWorkspaceSkillsPrompt)", () => {
  ...
  beforeEach(() => {
    envSnapshot = setMockSkillsHomeEnv("/Users/mo-test-user");
  });
  ...
  it("budget check uses compacted home-dir paths, not canonical paths", () => {
    ...
    const skills = Array.from({ length: 30 }, (_, i) =>
      makeSkill(
        `skill-${i}`,
        "A".repeat(200),
        `${home}/.mo/workspace/skills/skill-${i}/SKILL.md`,
      ),
    );
  ...
  it("resolvedSkills in snapshot keeps canonical paths, not compacted", () => {
    ...
    const skills = Array.from({ length: 5 }, (_, i) =>
      makeSkill(`skill-${i}`, "A skill", `${home}/.mo/workspace/skills/skill-${i}/SKILL.md`),
    );
