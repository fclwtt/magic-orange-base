...
const CASES: GuidanceCase[] = [
  {
    ...
    required: ["MO_STATE_DIR"],
    forbidden: [
      "for f in ~/.mo/agents/<agentId>/sessions/*.jsonl",
      'rg -l "phrase" ~/.mo/agents/<agentId>/sessions/*.jsonl',
      "~/.mo/agents/<agentId>/sessions/<id>.jsonl",
    ],
  },
  {
    file: "skills/gh-issues/SKILL.md",
    required: ["MO_CONFIG_PATH"],
    forbidden: ["cat ~/.mo/mo.json"],
  },
  {
  ...
  },
  {
    ...
    required: [
      "MO_STATE_DIR",
      "MO_CONFIG_PATH",
      'STATE_DIR="${MO_STATE_DIR:-$HOME/.mo}"',
    ],
    forbidden: [
      'SHERPA_ONNX_RUNTIME_DIR: "~/.mo/tools/sherpa-onnx-tts/runtime"',
      'SHERPA_ONNX_MODEL_DIR: "~/.mo/tools/sherpa-onnx-tts/models/vits-piper-en_US-lessac-high"',
      "<state-dir>",
    ],
  },
  {
    ...
    required: ["MO_STATE_DIR"],
    forbidden: ["NEVER start Codex in ~/.mo/"],
  },
];
