...
const SKILL_SOURCE_ORIGIN_RELATIVE_PATH = path.join(".mo", "source-origin.json");
...
function isContainerStateHomeWherePromptTildeEscapes(home: string): boolean {
  ...
  return home === "/data" && (configDir === "/data/.mo" || isPathInside("/data/.mo", configDir));
}
...
function buildEscapedSkillPathReason(params: { source: string; candidatePath: string }): {
  ...
  if (params.source === "mo-bundled" && candidateIsSymlink) {
    return {
...
function shouldEnforceConfiguredSkillRootContainment(source: string): boolean {
  return source !== "mo-managed" && source !== "agents-skills-personal";
}
...
function loadGeneratedPluginSkillRecords(params: {
  ...
  for (const name of childDirs) {
    ...
    // Plugin skills live as symlinks under ~/.mo/plugin-skills/, so
...
function buildSkillsLimitNote(params: {
  ...
  if (params.truncated) {
    return `⚠️ Skills truncated: included ${params.included} of ${params.total}${params.compact ? " (compact format, descriptions omitted)" : ""}. Run \`mo skills check\` to audit.`;
  }
  if (params.compact) {
    return `⚠️ Skills catalog using compact format (descriptions omitted). Run \`mo skills check\` to audit.`;
  }
  return "";
}
