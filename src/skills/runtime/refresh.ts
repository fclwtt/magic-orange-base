...
function resolveWatchTargets(workspaceDir: string, config?: OpenClawConfig): WatchTarget[] {
  ...
  if (workspaceDir.trim()) {
    baseRoots.push({ path: path.join(workspaceDir, "skills"), source: "mo-workspace" });
    baseRoots.push({
      path: path.join(workspaceDir, ".agents", "skills"),
      source: "agents-skills-project",
    });
  }
  baseRoots.push({ path: path.join(CONFIG_DIR, "skills"), source: "mo-managed" });
  baseRoots.push({
...
function isTrustedSymlinkSkillTarget(
  ...
  if (source === "mo-managed" || source === "agents-skills-personal") {
    return true;
  }
  return (
