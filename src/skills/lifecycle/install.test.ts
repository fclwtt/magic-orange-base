...
metadata: {"mo":{"install":[{"id":"deps","kind":"node","package":"example-package"}]}}
---
...
const workspaceSuite = createFixtureSuite("mo-skills-install-");
...
describe("installSkill before_install hooks", () => {
  beforeEach(() => {
    ...
    skillsInstallTesting.setDepsForTest({
      ...
      resolveNodeInstallStateDir: () => {
        ...
        if (!stateDir) {
          throw new Error("MO_STATE_DIR missing in skills install test");
        }
        return stateDir;
      },
    });
  ...
  it("keeps the default npm prefix out of env-overridden state paths", () => {
    const envSnapshot = captureEnv(["MO_STATE_DIR", "MO_CONFIG_PATH"]);
    try {
      process.env.MO_STATE_DIR = "/tmp/untrusted-state";
      process.env.MO_CONFIG_PATH = "/tmp/untrusted-config/mo.json";
  ...
  it("surfaces plugin hook findings from before_install", async () => {
    ...
    await withWorkspaceCase(async ({ workspaceDir }) => {
      ...
      expect(payload?.origin).toBe("mo-workspace");
