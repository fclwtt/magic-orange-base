...
async function installArchiveResolution(params: {
  ...
  archivePath: string;
  registry: string;
  authority: "official" | "mo" | "third-party";
  force?: boolean;
  logger?: Logger;
  config?: OpenClawConfig;
}) {
  return await withExtractedArchiveRoot({
    ...
    tempDirPrefix: "mo-skill-clawhub-",
    timeoutMs: 120_000,
...
async function installGitHubResolution(params: {
  ...
  return await withExtractedArchiveRoot({
    archivePath: params.archivePath,
    tempDirPrefix: "mo-skill-clawhub-github-",
    timeoutMs: 120_000,
...
function assertInstallResolutionAllowed(
  ...
  if (resolution.reason === "ambiguous_slug") {
    ...
    throw new Error(
      `Skill "${resolution.slug}" is ambiguous on ClawHub. Install an owner-qualified skill, for example: mo skills install @owner/${resolution.slug}.${message}`,
    );
  }
...
async function performClawHubSkillInstall(
  ...
  try {
    ...
    const registry = resolveClawHubBaseUrl(params.baseUrl);
    const clawhubAuthority = isDefaultClawHubBaseUrl(params.baseUrl) ? "mo" : "third-party";
