...
function resolveDefaultNodeInstallStateDir({
  ...
  if (platform !== "win32" && getuid?.() === 0) {
    return path.join(path.parse(cwd).root, "var", "lib", "mo");
  }
  return path.join(homedir(), ".mo");
}
...
export async function installSkill(params: SkillInstallRequest): Promise<SkillInstallResult> {
  ...
  const scanResult = await evaluateSkillInstallPolicy({
    ...
    source:
      skillSource === "mo-bundled"
        ? { kind: "bundled", authority: "mo", mutable: false, network: false }
        : skillSource === "mo-managed" || skillSource === "mo-extra"
          ? { kind: "managed", authority: "mo", mutable: false, network: false }
  ...
  const trustedInstallSources = new Set(["mo-bundled", "mo-managed", "mo-extra"]);
