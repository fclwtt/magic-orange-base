...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "MO_ENV_TEST_A";
    const keyB = "MO_ENV_TEST_B";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "MO_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "MO_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "MO_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "MO_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    setTestEnvValue(key, "outer");
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    const homeDir = path.join(path.sep, "tmp", "mo-home");
    const resolvedHomeDir = path.resolve(homeDir);
    const previousOpenClawHome = process.env.MO_HOME;
    const previousStateDir = process.env.MO_STATE_DIR;
    const previousBundledDir = process.env.MO_BUNDLED_PLUGINS_DIR;
    setTestEnvValue("MO_HOME", "/srv/mo-home");
    setTestEnvValue("MO_STATE_DIR", "/srv/mo-state");
    setTestEnvValue("MO_BUNDLED_PLUGINS_DIR", "/srv/mo-bundled");
    ...
    try {
      ...
      expect(env.MO_HOME).toBeUndefined();
      expect(env.MO_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.MO_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("MO_HOME", previousOpenClawHome);
      restoreEnvKey("MO_STATE_DIR", previousStateDir);
      restoreEnvKey("MO_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
    ...
    try {
      const seen = withPathResolutionEnv(
        ...
        (env) => ({
          ...
          processOpenClawHome: process.env.MO_HOME,
          processBundledDir: process.env.MO_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.MO_BUNDLED_PLUGINS_DIR,
        }),
      );
      ...
      expect(process.env.MO_HOME).toBe("/srv/mo-home");
    } finally {
      restoreEnvKey("MO_HOME", previousOpenClawHome);
    }
  });
