...
function primeUpdateConfigSnapshot(params: {
  ...
  const configPath = params.configPath ?? path.join(process.cwd(), "mo.json5");
...
describe("plugins cli update", () => {
        ...
        internal: {
          installs: {
            "new-hooks": {
              ...
              installPath: "~/.mo/hooks/new-hooks",
            },
          },
        },
      },
          ...
          internal: {
            installs: {
              "new-hooks": {
                source: "npm",
                spec: "@acme/new-hooks@1.0.0",
                installPath: "/home/test/.mo/hooks/new-hooks",
              },
            },
          },
        },
  ...
  it.each([
    {
      ...
      record: {
        source: "clawhub",
        spec: "clawhub:@mo/voice-call",
        clawhubPackage: "@mo/voice-call",
        installPath: "/tmp/voice-call",
      },
    },
    {
      ...
      record: {
        source: "git",
        spec: "https://github.com/mo/voice-call.git",
        installPath: "/tmp/voice-call",
      },
    },
    {
  ...
  it("preserves an include-owned plugins section during legacy-record cleanup", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mo-plugin-update-"));
  ...
  it("passes dangerous force unsafe install to plugin updates", async () => {
    ...
    expect(updateParams.pluginIds).toEqual(["mo-codex-app-server"]);
  ...
  it("does not pass an interactive ClawHub risk prompt to dry-run plugin updates", async () => {
    ...
    const config = createTrackedPluginConfig({
      pluginId: "mo-codex-app-server",
      spec: "clawhub:mo-codex-app-server",
    });
    loadConfig.mockReturnValue(config);
    setInstalledPluginIndexInstallRecords(config.plugins?.installs ?? {});
    updateNpmInstalledPlugins.mockResolvedValue({
    ...
    await runPluginsCommand(["plugins", "update", "mo-codex-app-server", "--dry-run"]);
      ...
      outcomes: [
        {
          ...
          message:
            'Skipped demo ClawHub update: ClawHub security data for "@mo/plugin-demo@1.1.0" is unavailable, so OpenClaw left the existing installed plugin unchanged. Try again later or choose a different version.',
        },
      ],
      changed: false,
      config: cfg,
