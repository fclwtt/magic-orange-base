...
const installWriteOptions = {
  ...
  expectedConfigPath: "/tmp/mo.json",
  ownedConfigPathForWrite: "/tmp/mo.json",
};
...
describe("persistPluginInstall", () => {
  ...
  it("removes a replaced managed install directory before refreshing the registry", async () => {
    ...
    setInstalledPluginIndexInstallRecords({
      codex: {
        source: "clawhub",
        spec: "clawhub:@mo/codex",
        installPath: "/tmp/mo/extensions/codex",
      },
    });
    planPluginUninstall.mockReturnValueOnce({
      ...
      directoryRemoval: {
        target: "/tmp/mo/extensions/codex",
      },
    });
    ...
    await persistPluginInstall({
      ...
      install: {
        source: "npm",
        spec: "@mo/codex",
        installPath: "/tmp/mo/npm/node_modules/@mo/codex",
      },
    });
    ...
    expect(runtimeLogs.join("\n")).toContain(
      "Removed previous plugin install directory: /tmp/mo/extensions/codex",
    );
  });
  ...
  it("preserves replaced npm install directories across generation updates", async () => {
    ...
    enablePluginInConfig.mockReturnValue({ config: enabledConfig });
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mo-plugin-persist-"));
  ...
  it("warns when an installed npm plugin remains shadowed by a config-selected source", async () => {
    ...
    const next = await persistPluginInstall({
      ...
      install: {
        source: "npm",
        spec: "@mo/discord",
        installPath: "/tmp/mo/npm/node_modules/@mo/discord/index.ts",
      },
    });
    ...
    expect(runtimeLogs.join("\n")).toContain(
      "active config source: /tmp/mo-upstream/extensions/discord/index.ts",
    );
    expect(runtimeLogs.join("\n")).toContain(
      "installed npm source: /tmp/mo/npm/node_modules/@mo/discord/index.ts",
    );
    expect(runtimeLogs.join("\n")).toContain("mo plugins doctor");
  });
  ...
  it("does not warn when the config-selected source is inside the npm install path", async () => {
      ...
      plugins: [
        {
          id: "discord",
          origin: "config",
          source: "/tmp/mo/npm/node_modules/@mo/discord/dist/index.js",
          status: "loaded",
        },
      ],
      diagnostics: [],
    ...
    await persistPluginInstall({
      ...
      install: {
        source: "npm",
        spec: "@mo/discord",
        installPath: "/tmp/mo/npm/node_modules/@mo/discord",
      },
    });
