...
const buildGatewayInstallPlan = vi.fn(
  async (params: {
    ...
    environment: {
      MO_GATEWAY_PORT: String(params.port),
      ...(params.wrapperPath ? { MO_WRAPPER: params.wrapperPath } : {}),
      ...(params.token ? { MO_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-daemon-cli-"));
    ...
    setTestEnvValue("MO_STATE_DIR", tmpDir);
    setTestEnvValue("MO_CONFIG_PATH", path.join(tmpDir, "mo.json"));
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        MO_PROFILE: "dev",
        MO_STATE_DIR: "/tmp/mo-daemon-state",
        MO_CONFIG_PATH: "/tmp/mo-daemon-state/mo.json",
        MO_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.mo.gateway.plist",
    });
  ...
  it("passes the existing service environment into the install plan on forced reinstall", async () => {
    ...
    expect((installPlanParams.env as NodeJS.ProcessEnv).MO_WRAPPER).toBe(
      "/usr/local/bin/mo-doppler",
    );
  });
