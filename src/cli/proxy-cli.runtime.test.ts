...
describe("proxy cli runtime", () => {
  const envKeys = [
    "MO_STATE_DIR",
    "MO_DEBUG_PROXY_CERT_DIR",
    "MO_DEBUG_PROXY_SESSION_ID",
    "MO_DEBUG_PROXY_ENABLED",
    "FORCE_COLOR",
    "NO_COLOR",
  ] as const;
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(path.join(os.tmpdir(), "mo-proxy-cli-runtime-"));
    process.env.MO_STATE_DIR = tempDir;
    process.env.MO_DEBUG_PROXY_CERT_DIR = path.join(tempDir, "certs");
    delete process.env.MO_DEBUG_PROXY_ENABLED;
    delete process.env.MO_DEBUG_PROXY_SESSION_ID;
  ...
  afterEach(async () => {
    ...
    const { closeOpenClawStateDatabaseForTest } = await import("../state/mo-state-db.js");
  ...
  it("prints actionable disabled proxy config output", async () => {
    ...
    expect(process.stdout["write"]).toHaveBeenCalledWith(
      "Proxy validation failed\n\n" +
        ...
        "Next steps\n" +
        "  Enable proxy.enabled with proxy.proxyUrl or MO_PROXY_URL, or pass --proxy-url for an explicit one-off validation.\n",
    );
  });
  ...
  it("prints actionable output when proxy config is disabled and missing", async () => {
    runProxyValidationMock.mockResolvedValueOnce({
      ...
      config: {
        ...
        source: "disabled",
        errors: [
          "proxy validation requires proxy.enabled=true with proxy.proxyUrl or MO_PROXY_URL, or --proxy-url",
        ],
      },
      checks: [],
    });
  ...
  it("redacts malformed proxy URLs in text output", async () => {
    ...
    expect(process.stdout["write"]).toHaveBeenCalledWith(
      "Proxy validation failed\n\n" +
        ...
        "Next steps\n" +
        "  Fix proxy.proxyUrl, MO_PROXY_URL, or --proxy-url so it uses a reachable http:// or https:// proxy.\n",
    );
  });
  ...
  it("prints proxy validation JSON and sets exit code on failure", async () => {
    runProxyValidationMock.mockResolvedValueOnce({
      ...
      config: {
        enabled: true,
        source: "missing",
        errors: ["proxy validation requires proxy.proxyUrl, --proxy-url, or MO_PROXY_URL"],
      },
      checks: [],
    });
    const { runProxyValidateCommand } = await import("./proxy-cli.runtime.js");
      ...
      `${JSON.stringify(
        {
          ...
          config: {
            ...
            source: "missing",
            errors: [
              "proxy validation requires proxy.proxyUrl, --proxy-url, or MO_PROXY_URL",
            ],
          },
          checks: [],
        },
