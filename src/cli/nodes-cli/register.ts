    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mo nodes status", "List known nodes with live status."],
          ["mo nodes pairing pending", "Show pending node pairing requests."],
          ["mo nodes remove --node <id|name|ip>", "Remove a stale paired node entry."],
          [
            'mo nodes invoke --node <id> --command system.which --params \'{"name":"uname"}\'',
            "Invoke a node command directly.",
          ],
          ["mo nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.mo.ai/cli/nodes")}\n`,
    );
...
function shouldRegisterNodesPluginCommands(nodes: Command, argv: readonly string[]): boolean {
  ...
  if (!requestedSubcommand) {
    // Bare `mo nodes` listing should still surface plugin-provided subcommands.
    return true;
  }
