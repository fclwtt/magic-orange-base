...
export function buildToolPluginPackageManifest(params: {
  ...
  const mo =
    params.packageManifest.mo &&
    typeof params.packageManifest.mo === "object" &&
    !Array.isArray(params.packageManifest.mo)
      ? { ...(params.packageManifest.mo as JsonObject) }
      : {};
  const existingExtensions = Array.isArray(mo.extensions)
    ? mo.extensions.filter((entry): entry is string => typeof entry === "string")
    : [];
...
export function validateToolPluginProject(params: {
  ...
  if (JSON.stringify(params.manifest) !== JSON.stringify(expectedManifest)) {
    errors.push("mo.plugin.json generated metadata is stale. Run mo plugins build.");
  }
  if (params.manifest.id !== params.metadata.id) {
    errors.push(
      `mo.plugin.json id (${String(params.manifest.id)}) must match entry id (${params.metadata.id})`,
    );
  }
  if (!params.manifest.configSchema || typeof params.manifest.configSchema !== "object") {
    errors.push("mo.plugin.json must include object configSchema");
  }
  ...
  if (missing.length > 0) {
    errors.push(`mo.plugin.json contracts.tools is missing: ${missing.join(", ")}`);
  }
  if (extra.length > 0) {
    errors.push(
      `mo.plugin.json contracts.tools has no matching defineToolPlugin tool: ${extra.join(
        ", ",
      )}`,
    );
  }
  ...
  if (extensionResolution.status !== "ok") {
    errors.push(
      extensionResolution.status === "missing" || extensionResolution.status === "empty"
        ? "package.json must include mo.extensions"
        : extensionResolution.error,
    );
  } else if (!extensionResolution.entries.includes(params.entry)) {
    errors.push(`package.json mo.extensions must include ${params.entry}`);
  }
  return errors;
}
...
export async function runPluginsBuildCommand(opts: PluginsBuildOptions): Promise<void> {
  ...
  if (opts.check) {
    ...
    if (
      ...
      defaultRuntime.error("Generated plugin metadata is out of date. Run mo plugins build.");
...
function writeToolPluginScaffold(params: { rootDir: string; id: string; name: string }): void {
  const packageManifest = {
    name: `mo-plugin-${params.id}`,
    version: "0.1.0",
    type: "module",
    private: true,
    scripts: {
      build: "tsc -p tsconfig.json",
      "plugin:build": "npm run build && mo plugins build --entry ./dist/index.js",
      "plugin:validate": "npm run build && mo plugins validate --entry ./dist/index.js",
      test: "vitest run --config ./vitest.config.ts",
    },
    files: ["dist", "mo.plugin.json", "README.md"],
    peerDependencies: {
      mo: ">=2026.5.17",
    },
    dependencies: {
...
import { defineToolPlugin } from "mo/plugin-sdk/tool-plugin";
...
import { getToolPluginMetadata } from "mo/plugin-sdk/tool-plugin";
...
function writeProviderPluginScaffold(params: { rootDir: string; id: string; name: string }): void {
  const packageName = `mo-plugin-${params.id}`;
  ...
  const indexSource = `import { definePluginEntry } from "mo/plugin-sdk/plugin-entry";
import { createProviderApiKeyAuthMethod } from "mo/plugin-sdk/provider-auth-api-key";
import { buildSingleProviderApiKeyCatalog } from "mo/plugin-sdk/provider-catalog-shared";
...
`;
  const testSource = `import { describe, expect, it } from "vitest";
import type { OpenClawPluginApi, ProviderPlugin } from "mo/plugin-sdk/plugin-entry";
import entry from "./index.js";
...
jobs:
  publish:
    permissions:
      ...
      contents: read
      id-token: write
    uses: mo/clawhub/.github/workflows/package-publish.yml@${CLAWHUB_PACKAGE_PUBLISH_WORKFLOW_REF}
    with:
