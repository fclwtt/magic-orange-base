...
describe("shouldSkipRespawnForArgv", () => {
  it.each([
    ...
    { argv: ["node", "mo", "-V"] },
    { argv: ["node", "mo", "tui"] },
    { argv: ["node", "mo", "terminal"] },
    { argv: ["node", "mo", "chat"] },
    { argv: ["node", "mo", "gateway"] },
    { argv: ["node", "mo", "gateway", "--port", "14720", "--bind", "loopback"] },
    { argv: ["node", "mo", "gateway", "run", "--port=14720", "--bind", "loopback"] },
    {
      argv: ["node", "mo", "--profile", "server", "gateway", "run", "--allow-unconfigured"],
    },
  ...
  it.each([
    { argv: ["node", "mo", "status"] },
    { argv: ["node", "mo", "gateway", "status"] },
    { argv: ["node", "mo", "gateway", "call", "health"] },
...
describe("shouldSkipStartupEnvironmentRespawnForArgv", () => {
  it.each([
    { argv: ["node", "mo", "--help"] },
    { argv: ["node", "mo", "gateway"] },
    { argv: ["node", "mo", "gateway", "run", "--port=14720"] },
...
describe("dns cli", () => {
  it("prints setup info (no apply)", async () => {
    ...
    try {
      ...
      await program.parseAsync(["dns", "setup", "--domain", "mo.internal"], { from: "user" });
      const output = log.mock.calls.map((call) => call.join(" ")).join("\\n");
      expect(output).toContain("DNS setup");
      expect(output).toContain("mo.internal");
    } finally {
      log.mockRestore();
    }
  });
