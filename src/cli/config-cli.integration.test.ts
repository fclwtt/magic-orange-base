...
async function withExecDryRunConfigHarness(
  ...
  const envSnapshot = captureEnv(["MO_CONFIG_PATH", "MO_TEST_FAST"]);
  try {
    fs.writeFileSync(
      configPath,
      `${JSON.stringify(
    ...
    setTestEnvValue("MO_TEST_FAST", "1");
...
describe("config cli integration", () => {
  beforeAll(async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-config-cli-warmup-"));
    const configPath = path.join(tempDir, "mo.json");
    const envSnapshot = captureEnv(["MO_CONFIG_PATH", "MO_TEST_FAST"]);
    try {
  ...
  it("accepts plugin hook conversation-access policy via config set", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-config-cli-plugin-hooks-"));
    const configPath = path.join(tempDir, "mo.json");
    const envSnapshot = captureEnv(["MO_CONFIG_PATH", "MO_TEST_FAST"]);
    try {
      fs.writeFileSync(
      ...
      await runConfigSet({
        path: "plugins.entries.mo-mem0.hooks.allowConversationAccess",
        value: "true",
        cliOptions: {},
      ...
      expect(afterWrite.plugins?.entries?.["mo-mem0"]?.hooks).toEqual({
  ...
  it("supports batch-file dry-run and then writes real config changes", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-config-cli-int-"));
    const configPath = path.join(tempDir, "mo.json");
    const batchPath = path.join(tempDir, "batch.json");
  ...
  it("keeps file unchanged when real-file dry-run fails and reports JSON error payload", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-config-cli-int-fail-"));
    const configPath = path.join(tempDir, "mo.json");
    const envSnapshot = captureEnv([
      "MO_CONFIG_PATH",
      "MO_TEST_FAST",
  ...
  it("skips exec provider execution during dry-run by default", async () => {
    await withExecDryRunConfigHarness("mo-config-cli-int-exec-skip-", async (params) => {
  ...
  it("executes exec providers during dry-run when --allow-exec is set", async () => {
    await withExecDryRunConfigHarness("mo-config-cli-int-exec-allow-", async (params) => {
      const before = fs.readFileSync(params.configPath, "utf8");
      await runConfigSet({
        cliOptions: {
          batchFile: params.batchPath,
