...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-bundled-skills-test-"));
    process.env.MO_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
