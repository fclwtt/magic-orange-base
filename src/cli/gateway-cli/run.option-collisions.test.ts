...
const resolveShellEnvExpectedKeys = vi.fn((_env?: NodeJS.ProcessEnv) => ["MO_GATEWAY_TOKEN"]);
...
const writeDiagnosticStabilityBundleForFailureSync = vi.fn((_reason: string, _error: unknown) => ({
  ...
  message: "wrote stability bundle: /tmp/mo-stability.json",
  path: "/tmp/mo-stability.json",
}));
const controlUiState = vi.hoisted(() => ({
  root: "/tmp/mo-control-ui" as string | null,
}));
...
vi.mock("../../config/config.js", () => ({
  getConfigPath: () => "/tmp/mo-test-missing-config.json",
...
vi.mock("../../config/paths.js", () => ({
  CONFIG_PATH: "/tmp/mo-test-missing-config.json",
...
describe("gateway run option collisions", () => {
  ...
  beforeEach(() => {
    ...
    controlUiState.root = "/tmp/mo-control-ui";
  ...
  it("loads configured shell env fallback before final proxy refresh and gateway startup", async () => {
    await withEnvAsync({ MO_GATEWAY_TOKEN: undefined }, async () => {
      const finalConfig = {
        env: {
          shellEnv: { enabled: true, timeoutMs: 1234 },
          vars: { MO_GATEWAY_TOKEN: "config-token" },
        },
        gateway: {
          auth: { mode: "token", token: "${MO_GATEWAY_TOKEN}" },
          mode: "local",
        },
        proxy: { enabled: true, proxyUrl: "http://127.0.0.1:29876" },
      };
      ...
      readConfigFileSnapshotWithPluginMetadata
        ...
        .mockImplementationOnce(async (options) => {
          expect(options?.lowerPrecedenceEnv).toEqual({
            MO_GATEWAY_TOKEN: "shell-token",
          });
          expect(process.env.MO_GATEWAY_TOKEN).toBe("shell-token");
          return {
            snapshot: {
              ...configState.snapshot,
              config: {
      ...
      loadShellEnvFallback.mockImplementationOnce((opts?: unknown) => {
        callOrder.push("shell-env");
        (opts as { env: NodeJS.ProcessEnv }).env.MO_GATEWAY_TOKEN = "shell-token";
      });
      const uninstall = installGatewayRunRuntimeHooks({ refreshManagedProxy });
      try {
        await runGatewayCli(["gateway"]);
      ...
      expect(loadShellEnvFallback).toHaveBeenCalledWith({
        enabled: true,
        env: process.env,
        expectedKeys: ["MO_GATEWAY_TOKEN"],
        logger: expect.any(Object),
        timeoutMs: 1234,
      });
      expect(readConfigFileSnapshotWithPluginMetadata).toHaveBeenCalledWith(
        expect.objectContaining({
          lowerPrecedenceEnv: { MO_GATEWAY_TOKEN: "shell-token" },
        }),
      );
      expect(readConfigFileSnapshotWithPluginMetadata).toHaveBeenCalledTimes(2);
      expect(process.env.MO_GATEWAY_TOKEN).toBe("config-token");
      expect(clearShellEnvAppliedKeys).toHaveBeenCalledWith(["MO_GATEWAY_TOKEN"]);
  ...
  it("uses config env shell fallback controls without mutating the live env during planning", async () => {
    await withEnvAsync(
      {
        MO_GATEWAY_TOKEN: undefined,
        MO_LOAD_SHELL_ENV: undefined,
        MO_SHELL_ENV_TIMEOUT_MS: undefined,
      },
      async () => {
        const finalConfig = {
          env: {
        ...
        shouldEnableShellEnvFallback.mockImplementationOnce(
          (env?: NodeJS.ProcessEnv) => env?.MO_LOAD_SHELL_ENV === "1",
        );
        resolveShellEnvFallbackTimeoutMs.mockImplementationOnce((env?: NodeJS.ProcessEnv) =>
          Number(env?.MO_SHELL_ENV_TIMEOUT_MS),
        );
        ...
        expect(process.env.MO_LOAD_SHELL_ENV).toBe("1");
        expect(process.env.MO_SHELL_ENV_TIMEOUT_MS).toBe("4321");
      },
    );
  });
  ...
  it("honors config env shell fallback deferral", async () => {
    await withEnvAsync(
      {
        MO_DEFER_SHELL_ENV_FALLBACK: undefined,
        MO_LOAD_SHELL_ENV: undefined,
      },
      async () => {
        const finalConfig = {
        ...
        shouldDeferShellEnvFallback.mockImplementationOnce(
          (env?: NodeJS.ProcessEnv) => env?.MO_DEFER_SHELL_ENV_FALLBACK === "1",
        );
  ...
  it("ignores shell fallback controls from invalid config", async () => {
    ...
    await withEnvAsync(
      ...
      async () => {
        const invalidConfig = {
          env: { vars: { MO_DEFER_SHELL_ENV_FALLBACK: "1" } },
          gateway: { mode: "local" },
        };
        configState.snapshot = {
  ...
  it("rejects an invalid final config after a prepared config selected runtime paths", async () => {
    const selectedStateDir = "/tmp/mo-prepared-selected-state";
    await withEnvAsync({ MO_STATE_DIR: undefined }, async () => {
  ...
  it("replaces config-derived env when the final startup snapshot changes in place", async () => {
    await withEnvAsync(
      {
        MO_GATEWAY_TOKEN: undefined,
        MO_PROXY_URL: undefined,
        MO_RAW_STREAM: undefined,
      },
      async () => {
        const oldConfig = {
          env: {
            vars: {
              ...
              MO_PROXY_URL: "http://127.0.0.1:19876",
              MO_RAW_STREAM: "1",
            },
          },
          gateway: { mode: "local" },
        ...
        const newConfig = {
          env: { vars: { MO_GATEWAY_TOKEN: "new-token" } },
          gateway: { mode: "local" },
        };
        configState.snapshot = {
          config: oldConfig,
        ...
        expect(process.env.MO_GATEWAY_TOKEN).toBe("old-token");
        expect(process.env.MO_PROXY_URL).toBe("http://127.0.0.1:19876");
        ...
        readConfigFileSnapshotWithPluginMetadata.mockImplementationOnce(async () => {
          expect(process.env.MO_GATEWAY_TOKEN).toBeUndefined();
          expect(process.env.MO_PROXY_URL).toBeUndefined();
          return { snapshot: configState.snapshot };
        });
        await runGatewayCli(["gateway", "--raw-stream"]);

        expect(process.env.MO_GATEWAY_TOKEN).toBe("new-token");
        expect(process.env.MO_PROXY_URL).toBeUndefined();
        expect(process.env.MO_RAW_STREAM).toBe("1");
      },
    );
  });
  ...
  it("marks descendants when the final config supplies the service marker", async () => {
    await withEnvAsync(
      ...
      async () => {
        ...
        expect(process.env.MO_SERVICE_MARKER).toBe("mo");
        expect(process.env[GATEWAY_SERVICE_RUNTIME_PID_ENV]).toBe(String(process.pid));
      },
    );
  });
  ...
  it("blocks service-mode startup from an older binary with newer config", async () => {
    ...
    process.env.MO_SERVICE_MARKER = "gateway";
    try {
    ...
    await withEnvAsync(
      ...
      async () => {
        ...
        ensureDevGatewayConfig.mockImplementationOnce(async () => {
          ...
          expect(process.env.MO_HOME).toBeUndefined();
          expect(process.env.MO_PROFILE).toBe("dev");
          expect(process.env.MO_STATE_DIR).toBeUndefined();
          expect(process.env.MO_GATEWAY_TOKEN).toBeUndefined();
          expect(process.env.MO_WORKSPACE_DIR).toBe("/tmp/mo-reset-workspace");
          configState.snapshot = {
            exists: true,
            valid: true,
            config: { gateway: { mode: "local" } },
        ...
        loadGlobalRuntimeDotEnvFiles.mockImplementation(() => {
          process.env.MO_GATEWAY_TOKEN ??= "trusted-token";
          process.env.MO_PROFILE ??= "dev";
          if (process.env.MO_WORKSPACE_DIR === undefined) {
            setTestEnvValue("MO_WORKSPACE_DIR", "/tmp/mo-reset-workspace");
          }
        });
        ...
        expect(process.env.MO_GATEWAY_TOKEN).toBe("trusted-token");
  ...
  it("refuses dev reset if trusted dotenv retargets after pre-bootstrap", async () => {
    await withEnvAsync({ MO_STATE_DIR: "/tmp/mo-reset-original" }, async () => {
      configState.snapshot = {
        config: { gateway: { mode: "local" } },
        exists: true,
        path: "/tmp/mo-reset-original/mo.json",
        sourceConfig: { gateway: { mode: "local" } },
        valid: true,
      };
      await prepareGatewayReset();
      loadGlobalRuntimeDotEnvFiles.mockImplementation(() => {
        setTestEnvValue("MO_STATE_DIR", "/tmp/mo-reset-retargeted");
        return {
          gatewayEnvAppliedKeys: [],
          stateEnvAppliedKeys: ["MO_STATE_DIR"],
        };
      });
      ...
      expect(ensureDevGatewayConfig).not.toHaveBeenCalled();
      expect(process.env.MO_STATE_DIR).toBe("/tmp/mo-reset-original");
      expect(runtimeErrors.join("\n")).toContain(
  ...
  it.each([
    ...
    await withEnvAsync({ [selector]: "/tmp/mo-reset-value" }, async () => {
      loadGlobalRuntimeDotEnvFiles.mockImplementation(() => {
        setTestEnvValue(selector, "/tmp/mo-reset-retargeted");
      });
      ...
      expect(process.env[selector]).toBe("/tmp/mo-reset-value");
      expect(runtimeErrors.join("\n")).toContain(
  ...
  it("blocks a final startup snapshot that changes guarded config selection", async () => {
    await withEnvAsync({ MO_STATE_DIR: undefined }, async () => {
      configState.snapshot = {
        ...
        config: { gateway: { mode: "local" } },
        sourceConfig: {
          env: { vars: { MO_STATE_DIR: "/tmp/mo-late-selection" } },
          gateway: { mode: "local" },
        },
      };
  ...
  it("blocks a final startup snapshot that changes an already-selected config selector", async () => {
    await withEnvAsync({ MO_STATE_DIR: undefined }, async () => {
      const guardedConfig = {
        env: { vars: { MO_STATE_DIR: "/tmp/mo-guarded-state" } },
        gateway: { mode: "local" },
      };
      configState.snapshot = {
      ...
      await selectGatewayRunEnvironment({ opts: {}, runtime: defaultRuntime });
      await prepareGatewayRunBootstrap({ opts: {}, runtime: defaultRuntime });
      expect(process.env.MO_STATE_DIR).toBe("/tmp/mo-guarded-state");

      const finalConfig = {
        env: { vars: { MO_STATE_DIR: "/tmp/mo-final-state" } },
        gateway: { mode: "local" },
      };
      configState.snapshot = {
        config: finalConfig,
  ...
  it.each([
    ...
    delete process.env.MO_CLI_BACKEND_LOG_OUTPUT;
    ...
    expect(process.env.MO_CLI_BACKEND_LOG_OUTPUT).toBe("1");
  });
  ...
  it("blocks startup when the observed snapshot loses gateway.mode", async () => {
    ...
    expect(runtimeErrors).toContain(
      "Gateway start blocked: existing config is missing gateway.mode. Treat this as suspicious or clobbered config. Re-run `mo onboard --mode local` or `mo setup`, set gateway.mode=local manually, or pass --allow-unconfigured.",
    );
    expect(runtimeErrors).toContain(
  ...
  it("blocks invalid startup config without automatic recovery", async () => {
    ...
    configState.snapshot = {
      exists: true,
      valid: false,
      path: "/tmp/mo-test-missing-config.json",
      config: {},
      parsed: null,
  ...
  it("allows password mode preflight when password is configured via SecretRef", async () => {
    configState.cfg = {
      gateway: {
        auth: {
          mode: "password",
          password: { source: "env", provider: "default", id: "MO_GATEWAY_PASSWORD" },
        },
      },
      secrets: {
        defaults: {
  ...
  it("reads gateway password from --password-file", async () => {
    ...
    expect(runtimeErrors).not.toContain(
      "Warning: --password can be exposed via process listings. Prefer --password-file or MO_GATEWAY_PASSWORD.",
    );
  });
