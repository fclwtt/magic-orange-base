...
async function createTimelinePath(): Promise<string> {
  const dir = await mkdtemp(path.join(tmpdir(), "mo-marketplace-entries-"));
...
describe("plugins marketplace entries", () => {
  ...
  it("lists entries from the configured marketplace feed as JSON", async () => {
    ...
    mocks.loadConfiguredHostedOfficialExternalPluginCatalogEntries.mockResolvedValue({
      ...
      metadata: {
        url: "https://packages.acme.example/mo/feed",
        status: 200,
        checksum: "feed-sha",
      },
      snapshot: {
  ...
  it("prints bundled fallback entries without failing", async () => {
    ...
    expect(output).toContain("@mo/acpx");
    expect(output).not.toContain("clawhub:@mo/acpx");
  ...
  it("emits bounded diagnostics for feed entry listing", async () => {
    ...
    vi.stubEnv("MO_DIAGNOSTICS", "1");
    vi.stubEnv("MO_DIAGNOSTICS_TIMELINE_PATH", timelinePath);
    mocks.getRuntimeConfig.mockReturnValue({});
    mocks.loadConfiguredHostedOfficialExternalPluginCatalogEntries.mockResolvedValue({
      source: "hosted-snapshot",
      entries: [
        {
          name: "@acme/calendar",
          mo: { plugin: { id: "acme-calendar", label: "Acme Calendar" } },
        },
      ],
      feed: {
        schemaVersion: 1,
      ...
      metadata: {
        url: "https://user:secret@packages.acme.example/mo/feed?token=leak#frag",
        status: 200,
        checksum: "feed-sha",
      },
      snapshot: {
