...
import { installedPluginRoot } from "mo/plugin-sdk/test-fixtures";
...
const CLI_STATE_ROOT = "/tmp/mo-state";
const ORIGINAL_MO_STATE_DIR = process.env.MO_STATE_DIR;
const ORIGINAL_MO_NIX_MODE = process.env.MO_NIX_MODE;
const PROFILE_STATE_ROOT = "/tmp/mo-ledger-profile";
...
function useProfileExtensionsDir(): string {
  process.env.MO_STATE_DIR = PROFILE_STATE_ROOT;
...
function createNpmPackPluginInstallResult(
  ...
  return {
    ...
    manifestName: `@mo/${pluginId}`,
    npmTarballName: `mo-${pluginId}-1.2.3.tgz`,
    npmResolution: {
      name: `@mo/${pluginId}`,
      version: "1.2.3",
      resolvedSpec: `@mo/${pluginId}@1.2.3`,
...
function primeHookPackNpmFallback() {
  ...
  installPluginFromNpmSpec.mockResolvedValue({
    ok: false,
    error: "package.json missing mo.plugin.json",
  });
...
function primeBlockedPluginConfigMutation(
  ...
  const configPath = path.join(process.cwd(), "mo.json5");
...
function primeNestedPluginConfigMutation(tempRoot: string): void {
  const configPath = path.join(tempRoot, "mo.json5");
...
describe("plugins cli install", () => {
  ...
  afterEach(() => {
    if (ORIGINAL_MO_STATE_DIR === undefined) {
      delete process.env.MO_STATE_DIR;
    } else {
      process.env.MO_STATE_DIR = ORIGINAL_MO_STATE_DIR;
    }
    if (ORIGINAL_MO_NIX_MODE === undefined) {
      delete process.env.MO_NIX_MODE;
    } else {
      process.env.MO_NIX_MODE = ORIGINAL_MO_NIX_MODE;
    }
  });
  ...
  it.each(["@acme/demo-plugin", "npm:@acme/demo-plugin"])(
    ...
    async (spec) => {
      ...
      installHooksFromNpmSpec.mockResolvedValue({
        ok: false,
        error: "package.json missing mo.hooks",
      });
  ...
  it("blocks local package inspection when plugin and hook config are include-owned", async () => {
    const localPath = fs.mkdtempSync(path.join(os.tmpdir(), "mo-hook-pack-"));
    primeBlockedPluginConfigMutation({ blockHooks: true });
    installHooksFromPath.mockResolvedValue(createHookPackInstallResult(localPath));
    installPluginFromPath.mockResolvedValue({
      ok: false,
      error: "package.json missing mo.extensions",
      code: "missing_mo_extensions",
    });
  ...
  it("fails closed for ambiguous local plugins when the whole config is include-owned", async () => {
    const localPath = fs.mkdtempSync(path.join(os.tmpdir(), "mo-demo-plugin-"));
    primeBlockedRootConfigMutation();
  ...
  it("fails closed before installing a blocked ambiguous local plugin", async () => {
    const archivePath = path.join(os.tmpdir(), `mo-plugin-${process.pid}.tgz`);
  ...
  it("fails closed when a local hook probe finds a plugin-capable package", async () => {
    const localPath = fs.mkdtempSync(path.join(os.tmpdir(), "mo-dual-package-"));
    primeBlockedPluginConfigMutation();
    installHooksFromPath.mockResolvedValue({
  ...
  it("fails closed for a local bundle plugin instead of installing its hooks", async () => {
    const localPath = fs.mkdtempSync(path.join(os.tmpdir(), "mo-bundle-plugin-"));
    primeBlockedPluginConfigMutation();
    installHooksFromPath.mockResolvedValue({
      ...createHookPackInstallResult(localPath),
      packageKind: "plugin-capable",
  ...
  it("fails closed when a blocked-config local hook probe throws", async () => {
    const localPluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-local-plugin-"));
    primeBlockedPluginConfigMutation();
  ...
  it("blocks explicit plugins through nested include config before installer side effects", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mo-plugin-nested-"));
  ...
  it("fails closed for unrelated invalid config before installer side effects", async () => {
    ...
    expect(runtimeErrors.at(-1)).toContain(
      "Config invalid; run `mo doctor --fix` before installing plugins.",
    );
    expect(installPluginFromMarketplace).not.toHaveBeenCalled();
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(writeConfigFile).not.toHaveBeenCalled();
  ...
  it("resolves exact official external plugin ids through their npm package", async () => {
    ...
    expect(installPluginFromClawHub).not.toHaveBeenCalled();
    expect(npmInstallCall().spec).toBe("@mo/brave-plugin");
    ...
    expect(record.spec).toBe("@mo/brave-plugin");
  ...
  it("passes third-party external catalog integrity with catalog install trust", async () => {
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("wecom-mo-plugin");
    loadConfig.mockReturnValue(cfg);
    findBundledPluginSourceMock.mockReturnValue(undefined);
    installPluginFromNpmSpec.mockResolvedValue(
      createNpmPluginInstallResult("wecom-mo-plugin"),
    );
    enablePluginInConfig.mockReturnValue({ config: enabledCfg });
    applyExclusiveSlotSelection.mockReturnValue({
      config: enabledCfg,
    ...
    expect(npmInstallCall().spec).toBe("@wecom/wecom-mo-plugin@2026.5.7");
    expect(npmInstallCall().expectedPluginId).toBe("wecom-mo-plugin");
  ...
  it("passes third-party external catalog integrity to hook-pack fallback", async () => {
    ...
    installHooksFromNpmSpec.mockResolvedValue({
      ok: false,
      error:
        "aborted: npm package integrity drift detected for @wecom/wecom-mo-plugin@2026.5.7",
    });

    await expect(runPluginsCommand(["plugins", "install", "wecom"])).rejects.toThrow("__exit__:1");

    expect(npmInstallCall().trustedSourceLinkedOfficialInstall).toBe(true);
    expect(hookNpmInstallCall().spec).toBe("@wecom/wecom-mo-plugin@2026.5.7");
  ...
  it("installs npm-pack archives through npm install semantics", async () => {
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("demo");
    const archivePath = "/tmp/mo-demo-1.2.3.tgz";
    ...
    const record = persistedInstallRecord("demo");
    expect(record.source).toBe("npm");
    expect(record.spec).toBe("@mo/demo@1.2.3");
    ...
    expect(record.npmTarballName).toBe("mo-demo-1.2.3.tgz");
    expect(writeConfigFile).toHaveBeenCalledWith(enabledCfg);
  });
  ...
  it("marks explicit official npm package installs as trusted", async () => {
    ...
    await runPluginsCommand(["plugins", "install", "npm:@mo/discord"]);
  ...
  it("marks scoped official npm package installs as trusted", async () => {
    ...
    await runPluginsCommand(["plugins", "install", "@mo/discord"]);
  ...
  it("uses bundled OpenClaw package specs instead of pinning stale managed npm overrides", async () => {
    ...
    findBundledPluginSourceMock.mockImplementation((params: unknown) => {
      ...
      return lookup.kind === "npmSpec" && lookup.value === "@mo/discord"
        ? {
            pluginId: "discord",
    ...
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(findBundledPluginSourceMock).toHaveBeenCalledWith({
      lookup: { kind: "npmSpec", value: "@mo/discord@2026.5.20" },
    });
    expect(findBundledPluginSourceMock).toHaveBeenCalledWith({
      lookup: { kind: "npmSpec", value: "@mo/discord" },
    });
    const record = persistedInstallRecord("discord");
    expect(record.source).toBe("path");
    expect(record.spec).toBe("@mo/discord@2026.5.20");
    ...
    expect(runtimeLogsContain("npm:@mo/discord@2026.5.20")).toBe(true);
  });
  ...
  it("marks catalog npm package installs with alternate selectors as trusted", async () => {
    ...
    await runPluginsCommand(["plugins", "install", "@wecom/wecom-mo-plugin@latest"]);
    ...
    expect(npmInstallCall().spec).toBe("@wecom/wecom-mo-plugin@latest");
    expect(npmInstallCall().expectedPluginId).toBe("wecom-mo-plugin");
    expect(npmInstallCall().trustedSourceLinkedOfficialInstall).toBe(true);
    expect(npmInstallCall().expectedIntegrity).toBeUndefined();
    expect(installPluginFromClawHub).not.toHaveBeenCalled();
  ...
  it("adds a Git PATH hint when npm plugin dependency install cannot spawn git", async () => {
    ...
    await expect(
      runPluginsCommand(["plugins", "install", "npm:@mo/whatsapp"]),
    ).rejects.toThrow("__exit__:1");
  ...
  it("rejects --pin for git installs and points at git refs", async () => {
    ...
    expect(runtimeErrors.at(-1)).toContain("mo plugins install git:<repo>@<ref>");
  });
  ...
  it("passes dangerous force unsafe install to linked path probe installs", async () => {
    ...
    const enabledCfg = createEnabledPluginConfig("demo");
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mo-plugin-link-"));
  ...
  it("passes dangerous force unsafe install to linked hook-pack probe fallback", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mo-hook-link-"));
  ...
  it("does not fall back to hook pack for linked path when a no-flag security scan blocks", async () => {
    const localPluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-link-plugin-"));
  ...
  it("passes dangerous force unsafe install to local hook-pack fallback installs", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mo-hook-install-"));
    primeHookPackPathFallback({
      tmpRoot,
  ...
  it("suggests update or --force when npm plugin install target already exists", async () => {
    ...
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
      error:
        "plugin already exists: /home/mo/.mo/extensions/lossless-claw (delete it first)",
    });
    installHooksFromNpmSpec.mockResolvedValue({
      ok: false,
      error: "package.json missing mo.hooks",
    ...
    expect(runtimeErrors.at(-1)).toContain(
      "Use `mo plugins update <id-or-npm-spec>` to upgrade the tracked plugin, or rerun install with `--force` to replace it.",
    );
    expect(runtimeErrors.at(-1)).not.toContain("Also not a valid hook pack");
  });
  ...
  it("still falls back to local hook pack when dangerous force unsafe install is set for non-security errors", async () => {
    const localHookDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-local-hook-pack-"));
    const cfg = {} as OpenClawConfig;
    const installedCfg = {
      hooks: {
        internal: {
  ...
  it("does not fall back to npm when explicit ClawHub rejects a real package", async () => {
    ...
    installPluginFromClawHub.mockResolvedValue({
      ok: false,
      error: 'Use "mo skills install demo" instead.',
      code: "skill_package",
    });
    ...
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(runtimeErrors.at(-1)).toContain('Use "mo skills install demo" instead.');
  });
