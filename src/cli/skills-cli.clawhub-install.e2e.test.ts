...
describe("mo skills install ClawHub GitHub-backed E2E", () => {
  it("installs from the install resolver and reports install telemetry", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-clawhub-cli-e2e-"));
    try {
      const result = await spawnOpenClaw(["skills", "install", "aiq-deploy", "--global"], {
        ...
        env: {
          ...
          MO_CLAWHUB_URL: registry,
          MO_CLAWHUB_TOKEN: "test-token",
          MO_CLAWHUB_GITHUB_CODELOAD_BASE_URL: registry,
