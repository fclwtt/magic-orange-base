...
describe("pickTagline", () => {
  ...
  it("keeps MO_TAGLINE_INDEX behavior in random mode", () => {
    const value = pickTagline({
      mode: "random",
      env: { MO_TAGLINE_INDEX: "0" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(
  ...
  it("ignores partial MO_TAGLINE_INDEX values", () => {
    expect(
      pickTagline({
        mode: "random",
        env: { MO_TAGLINE_INDEX: "1abc" } as NodeJS.ProcessEnv,
        random: () => 0,
      }),
