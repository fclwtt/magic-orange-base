    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "mo node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["mo node status", "Check node host service status."],
          ["mo node install", "Install the node host service."],
          ["mo node start", "Start the installed node host service."],
          ["mo node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.mo.ai/cli/node")}\n`,
    );
