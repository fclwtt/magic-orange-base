...
describe("completion-runtime", () => {
  ...
  it("installs PowerShell completion into the concrete profile path", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-completion-home-"));
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-completion-state-bob's-"));

    try {
      await withEnvAsync({ HOME: homeDir, MO_STATE_DIR: stateDir }, async () => {
        const cachePath = resolveCompletionCachePath("powershell", "mo");
        await fs.mkdir(path.dirname(cachePath), { recursive: true });
        await fs.writeFile(cachePath, "# powershell completion\n", "utf-8");

        await installCompletion("powershell", true, "mo");
  ...
  it("rejects install when the completion cache is missing", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-completion-home-"));
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-completion-state-"));

    try {
      await withEnvAsync({ HOME: homeDir, MO_STATE_DIR: stateDir }, async () => {
        await expect(installCompletion("zsh", true, "mo")).rejects.toThrow(
