...
export async function registerChannelsCli(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mo channels list", "List configured channels."],
          ["mo channels list --all", "Show configured, bundled, and installable channels."],
          ["mo channels add", "Open guided channel setup."],
          ["mo channels status --probe", "Run channel status checks and probes."],
          [
            "mo channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["mo channels login --channel whatsapp", "Link a WhatsApp Web account."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink(
          "/cli/channels",
          "docs.mo.ai/cli/channels",
        )}\n`,
    );
  ...
  const addCommand = channels
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mo channels add", "Open guided setup for available chat channels."],
          [
            "mo channels add --channel telegram --token <token>",
            "Add or update Telegram non-interactively.",
          ],
          ["mo channels list --all", "Find channel ids before using --channel."],
        ])}\n`,
    )
    .option("--channel <name>", `Channel (${channelNames})`)
