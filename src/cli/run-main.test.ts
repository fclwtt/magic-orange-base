...
describe("isGatewayRunFastPathArgv", () => {
  it("matches only plain gateway foreground starts without root options or help", () => {
    expect(isGatewayRunFastPathArgv(["node", "mo", "gateway"])).toBe(true);
    expect(isGatewayRunFastPathArgv(["node", "mo", "gateway", "--force"])).toBe(true);
    expect(isGatewayRunFastPathArgv(["node", "mo", "gateway", "--port", "18789"])).toBe(true);
    expect(isGatewayRunFastPathArgv(["node", "mo", "gateway", "--auth=none"])).toBe(true);
    expect(
      isGatewayRunFastPathArgv(["node", "mo", "--no-color", "gateway", "--bind", "loopback"]),
    ).toBe(true);
    expect(isGatewayRunFastPathArgv(["node", "mo", "gateway", "run"])).toBe(true);
    expect(
      isGatewayRunFastPathArgv(["node", "mo", "gateway", "run", "--raw-stream-path", "x"]),
    ).toBe(true);
    expect(isGatewayRunFastPathArgv(["node", "mo", "gateway", "call", "health"])).toBe(false);
    expect(isGatewayRunFastPathArgv(["node", "mo", "gateway", "--help"])).toBe(false);
    expect(isGatewayRunFastPathArgv(["node", "mo", "gateway", "--port"])).toBe(false);
    expect(isGatewayRunFastPathArgv(["node", "mo", "gateway", "--unknown"])).toBe(false);
  });
});
...
describe("resolveGatewayRunPreBootstrapOptions", () => {
  it("resolves destructive gateway flags across fast and full Commander paths", () => {
    expect(
      resolveGatewayRunPreBootstrapOptions(["node", "mo", "gateway", "run", "--force"]),
  ...
  it("does not treat malformed required option values as destructive flags", () => {
    expect(
      resolveGatewayRunPreBootstrapOptions(["node", "mo", "gateway", "--token", "--force"]),
...
describe("shouldEnsureCliPath", () => {
  it("skips path bootstrap for help/version invocations", () => {
    expect(shouldEnsureCliPath(["node", "mo", "--help"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "mo", "-V"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "mo", "-v"])).toBe(false);
  });

  it("skips path bootstrap for read-only fast paths", () => {
    expect(shouldEnsureCliPath(["node", "mo"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "mo", "--profile", "work"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "mo", "approvals"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "mo", "channels"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "mo", "cron"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "mo", "devices"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "mo", "plugins"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "mo", "mcp"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "mo", "status"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "mo", "--log-level", "debug", "status"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "mo", "sessions", "--json"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "mo", "config", "get", "update"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "mo", "models", "status", "--json"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "mo", "tools", "effective"])).toBe(false);
  });

  it("keeps path bootstrap for mutating or unknown commands", () => {
    expect(shouldEnsureCliPath(["node", "mo", "message", "send"])).toBe(true);
    expect(shouldEnsureCliPath(["node", "mo", "voicecall", "status"])).toBe(true);
    expect(shouldEnsureCliPath(["node", "mo", "acp", "-v"])).toBe(true);
  });
});

describe("shouldStartCrestodianForBareRoot", () => {
  it("starts Crestodian for bare root invocations", () => {
    expect(shouldStartCrestodianForBareRoot(["node", "mo"])).toBe(true);
    expect(shouldStartCrestodianForBareRoot(["node", "mo", "--profile", "work"])).toBe(true);
    expect(shouldStartCrestodianForBareRoot(["node", "mo", "--dev"])).toBe(true);
  });

  it("does not start Crestodian for help, version, or commands", () => {
    expect(shouldStartCrestodianForBareRoot(["node", "mo", "--help"])).toBe(false);
    expect(shouldStartCrestodianForBareRoot(["node", "mo", "-V"])).toBe(false);
    expect(shouldStartCrestodianForBareRoot(["node", "mo", "status"])).toBe(false);
  });
});
...
describe("shouldStartCrestodianForModernOnboard", () => {
  ...
  it("keeps classic onboard and help on the normal command path", () => {
    expect(shouldStartCrestodianForModernOnboard(["node", "mo", "onboard"])).toBe(false);
    expect(
      shouldStartCrestodianForModernOnboard(["node", "mo", "onboard", "--modern", "--help"]),
    ).toBe(false);
  });
});
...
describe("shouldStartProxyForCli", () => {
  it("starts managed proxy routing for the --update shorthand", () => {
    expect(shouldStartProxyForCli(["node", "mo", "--update"])).toBe(true);
    expect(shouldStartProxyForCli(["node", "mo", "--profile", "p", "--update"])).toBe(true);
  });

  it("skips managed proxy routing for bare parent default help", () => {
    expect(shouldStartProxyForCli(["node", "mo", "plugins"])).toBe(false);
    expect(shouldStartProxyForCli(["node", "mo", "channels"])).toBe(false);
    expect(shouldStartProxyForCli(["node", "mo", "cron"])).toBe(false);
    expect(shouldStartProxyForCli(["node", "mo", "devices"])).toBe(false);
    expect(shouldStartProxyForCli(["node", "mo", "mcp"])).toBe(false);
  });
});

describe("shouldUseRootHelpFastPath", () => {
  it("uses the fast path for root help only", () => {
    expect(shouldUseRootHelpFastPath(["node", "mo", "--help"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "mo", "--profile", "work", "-h"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "mo", "help", "--help"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "mo", "tools", "--help"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "mo", "status", "--help"])).toBe(false);
    expect(shouldUseRootHelpFastPath(["node", "mo", "--help", "status"])).toBe(false);
    expect(shouldUseRootHelpFastPath(["node", "mo", "help", "gateway"])).toBe(false);
  });
});

describe("shouldUseBrowserHelpFastPath", () => {
  it("uses the fast path for browser command help only", () => {
    expect(shouldUseBrowserHelpFastPath(["node", "mo", "browser", "--help"])).toBe(true);
    expect(shouldUseBrowserHelpFastPath(["node", "mo", "browser", "-h"])).toBe(true);
    expect(
      shouldUseBrowserHelpFastPath(["node", "mo", "--profile", "work", "browser", "-h"]),
    ).toBe(true);
    expect(shouldUseBrowserHelpFastPath(["node", "mo", "browser", "status", "--help"])).toBe(
      false,
    );
    expect(shouldUseBrowserHelpFastPath(["node", "mo", "browser", "--version"])).toBe(false);
    expect(shouldUseBrowserHelpFastPath(["node", "mo", "status", "--help"])).toBe(false);
    expect(shouldUseBrowserHelpFastPath(["node", "mo", "browser", "--version"])).toBe(false);
  });
});
...
describe("parent command help fast paths", () => {
  it("use fast paths for secrets and nodes parent help only", () => {
    expect(shouldUseSecretsHelpFastPath(["node", "mo", "secrets", "--help"])).toBe(true);
    expect(shouldUseSecretsHelpFastPath(["node", "mo", "secrets", "-h"])).toBe(true);
    expect(shouldUseSecretsHelpFastPath(["node", "mo", "secrets", "--version"])).toBe(false);
    expect(shouldUseSecretsHelpFastPath(["node", "mo", "secrets", "audit", "--help"])).toBe(
      false,
    );

    expect(shouldUseNodesHelpFastPath(["node", "mo", "nodes", "--help"])).toBe(true);
    expect(shouldUseNodesHelpFastPath(["node", "mo", "nodes", "-h"])).toBe(true);
    expect(shouldUseNodesHelpFastPath(["node", "mo", "nodes", "--version"])).toBe(false);
    expect(shouldUseNodesHelpFastPath(["node", "mo", "nodes", "invoke", "--help"])).toBe(
      false,
    );
  });
});
...
describe("shouldUseSetupOnboardConfigureHelpFastPath", () => {
  it("uses the fast path only for setup, onboard, and configure help", () => {
    expect(
      shouldUseSetupOnboardConfigureHelpFastPath(["node", "mo", "setup", "--help"]),
    ).toBe(true);
    expect(shouldUseSetupOnboardConfigureHelpFastPath(["node", "mo", "onboard", "-h"])).toBe(
      true,
    );
    expect(
      shouldUseSetupOnboardConfigureHelpFastPath([
    ...
    expect(
      shouldUseSetupOnboardConfigureHelpFastPath(["node", "mo", "status", "--help"]),
    ).toBe(false);
  });
});
...
describe("resolvePrecomputedSubcommandHelpFastPath", () => {
  it("uses the fast path only for allowlisted parent command help", () => {
    expect(resolvePrecomputedSubcommandHelpFastPath(["node", "mo", "doctor", "--help"])).toBe(
      "doctor",
    );
    expect(resolvePrecomputedSubcommandHelpFastPath(["node", "mo", "gateway", "-h"])).toBe(
      "gateway",
    );
    expect(
      resolvePrecomputedSubcommandHelpFastPath([
    ...
    expect(
      resolvePrecomputedSubcommandHelpFastPath(["node", "mo", "plugins", "--help"]),
    ).toBe("plugins");
    expect(
      resolvePrecomputedSubcommandHelpFastPath(["node", "mo", "sessions", "--help"]),
    ).toBe("sessions");
    expect(resolvePrecomputedSubcommandHelpFastPath(["node", "mo", "tasks", "-h"])).toBe(
      "tasks",
    );
    expect(
      resolvePrecomputedSubcommandHelpFastPath(["node", "mo", "doctor", "--version"]),
    ).toBeNull();
    expect(
      resolvePrecomputedSubcommandHelpFastPath(["node", "mo", "gateway", "-V"]),
    ).toBeNull();
    expect(
      resolvePrecomputedSubcommandHelpFastPath([
        "node",
    ...
    expect(
      resolvePrecomputedSubcommandHelpFastPath(["node", "mo", "doctor", "--version", "-h"]),
    ).toBeNull();
    expect(
      resolvePrecomputedSubcommandHelpFastPath(["node", "mo", "--bogus", "doctor", "--help"]),
    ).toBeNull();
    expect(
      resolvePrecomputedSubcommandHelpFastPath(["node", "mo", "doctor", "--help", "--bogus"]),
    ).toBeNull();
    expect(
      resolvePrecomputedSubcommandHelpFastPath(["node", "mo", "doctor", "--help", "extra"]),
    ).toBeNull();
    expect(
      resolvePrecomputedSubcommandHelpFastPath(["node", "mo", "gateway", "status", "--help"]),
    ).toBeNull();
    expect(
      resolvePrecomputedSubcommandHelpFastPath(["node", "mo", "status", "--help"]),
    ).toBeNull();
    expect(
      resolvePrecomputedSubcommandHelpFastPath(["node", "mo", "doctor", "--help"], {
        MO_DISABLE_CLI_STARTUP_HELP_FAST_PATH: "1",
      }),
    ).toBeNull();
  });
});
...
describe("resolveMissingPluginCommandMessage", () => {
  ...
  it("explains that dreaming is a runtime slash command, not a CLI command", () => {
    ...
    expect(message).toContain("mo memory");
  });
  ...
  it("explains disabled-by-default parent plugins for CLI command aliases", () => {
    ...
    expect(message).toContain("mo plugins enable voice-call");
  });
