...
describe("npm-resolution helpers", () => {
  it("keeps the requested selector when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@mo/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@mo/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@mo/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@mo/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @mo/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@mo/plugin-alpha@latest",
        installPath: ALPHA_INSTALL_PATH,
        version: "1.2.3",
        resolution: {
          name: "@mo/plugin-alpha",
          version: "1.2.3",
          resolvedSpec: "@mo/plugin-alpha@1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @mo/plugin-alpha@1.2.3."]);
  ...
  it("keeps install record selector for CLI unless --pin is requested", () => {
    ...
    expect(record.spec).toBe("@mo/plugin-alpha");
    expect(logs).toEqual([]);
  });
});
