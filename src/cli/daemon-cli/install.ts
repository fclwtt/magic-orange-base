...
import { MO_WRAPPER_ENV_KEY, resolveOpenClawWrapperPath } from "../../daemon/program-args.js";
...
export function mergeInstallInvocationEnv(params: {
  ...
  for (const [rawKey, rawValue] of Object.entries(params.existingServiceEnv)) {
    ...
    if (upper === MO_WRAPPER_ENV_KEY) {
      const value = rawValue.trim();
      if (value) {
        preservedServiceEnv[normalizeInstallEnvKey(MO_WRAPPER_ENV_KEY)] = value;
      }
      continue;
    }
    if (
...
export async function runDaemonInstall(opts: DaemonInstallOptions) {
  ...
  if (!wrapperPath) {
    try {
      wrapperPath = await resolveOpenClawWrapperPath(installEnv[MO_WRAPPER_ENV_KEY]);
    } catch (err) {
      fail(`Invalid ${MO_WRAPPER_ENV_KEY}: ${String(err)}`);
      return;
    }
  }
  if (loaded) {
    if (!opts.force) {
      ...
      if (autoRefreshMessage) {
        ...
        if (!json) {
          ...
          defaultRuntime.log(
            `Reinstall with: ${formatCliCommand("mo gateway install --force")}`,
          );
        }
        return;
      }
  ...
  try {
    ...
    if (currentEmbeddedToken) {
      ...
      const plannedEmbeddedToken = normalizeOptionalString(
        plannedInstall.environment.MO_GATEWAY_TOKEN,
      );
      if (currentEmbeddedToken !== plannedEmbeddedToken) {
        return "Gateway service MO_GATEWAY_TOKEN differs from the current install plan; refreshing the install.";
      }
    }
    const wrapperRequested = Boolean(
      params.wrapperPath || normalizeOptionalString(params.installEnv[MO_WRAPPER_ENV_KEY]),
    );
    if (wrapperRequested) {
      const plannedInstall = await buildGatewayInstallPlan({
        env: params.installEnv,
      ...
      const plannedWrapperPath = normalizeOptionalString(
        plannedInstall.environment[MO_WRAPPER_ENV_KEY],
      );
      const currentWrapperPath = normalizeOptionalString(
        currentCommand.environment?.[MO_WRAPPER_ENV_KEY],
      );
      if (plannedWrapperPath !== currentWrapperPath) {
        return `Gateway service ${MO_WRAPPER_ENV_KEY} differs from the current wrapper install plan; refreshing the install.`;
      }
    }
