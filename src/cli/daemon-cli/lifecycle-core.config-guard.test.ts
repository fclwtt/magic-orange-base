...
const newerConfigHints = [
  "Run the newer mo binary on PATH, or reinstall the intended gateway service from the newer install.",
  "Set MO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 only for an intentional downgrade or recovery action.",
];
const newerConfigHintItems = newerConfigHints.map((text) => ({ kind: "generic", text }));
const invalidConfigRecoveryHint = [
  'Run "mo doctor --fix" to repair, then retry.',
