...
describe("printDaemonStatus", () => {
  ...
  it("prints stale gateway pid guidance when runtime does not own the listener", () => {
    ...
    expectMockLineContains(runtime.error, formatCliCommand("mo gateway restart"));
  });
  ...
  it("prints established gateway client guidance gathered by deep status", () => {
    ...
    expectMockLineContains(runtime.log, "newer-mo");
  ...
  it("prints stale updater launchd job guidance", () => {
    ...
    expectMockLineContains(runtime.error, "ai.mo.update.2026.5.12");
    expectMockLineContains(runtime.error, "ai.mo.manual-update.1717168800");
  ...
  it("prints macOS launchd stdout and suppressed stderr when gateway is not listening", () => {
    ...
    expectMockLineContains(runtime.error, "/Users/test/Library/Logs/mo/gateway.log");
  ...
  it("prints CLI and gateway versions with readable guidance when they differ", () => {
    ...
    expectMockLineContains(runtime.log, "CLI version: 2026.4.23 (/usr/local/bin/mo)");
    ...
    expectMockLineContains(
      runtime.error,
      "if this mismatch is unexpected, update PATH so `mo` points to the version you want",
    );
  });
    ...
    printDaemonStatus(
      {
        ...
        config: {
          cli: {
            path: "/tmp/mo-cli/mo.json",
            exists: true,
            valid: true,
          },
          daemon: {
            path: "/tmp/mo-daemon/mo.json",
            exists: true,
            valid: true,
    ...
    printDaemonStatus(
      {
        ...
        extraServices: [{ label: "ai.mo.gateway.rescue", scope: "user", detail: "loaded" }],
      },
      { json: false },
    );

    expectMockLineContains(runtime.log, "Other gateway-like services detected");
    expectMockLineContains(runtime.log, "ai.mo.gateway.rescue");
  ...
  it("prints a terse plugin drift warning outside deep mode", () => {
    ...
    expectMockLineContains(runtime.log, "mo gateway status --deep");
  ...
  it("prints detailed plugin drift entries in deep mode", () => {
    ...
    expectMockLineContains(runtime.log, "mo plugins update whatsapp");
    expectMockLineContains(runtime.log, "mo gateway restart");
  });
  ...
  it("prints exact package update commands for pinned npm plugin drift in deep mode", () => {
          ...
          drifts: [
            {
              ...
              gatewayVersion: "2026.6.10-beta.1",
              source: "npm",
              packageName: "@mo/brave-plugin",
              spec: "@mo/brave-plugin@2026.6.9",
            },
          ],
        },
        extraServices: [],
    ...
    expectMockLineContains(
      runtime.log,
      "mo plugins update @mo/brave-plugin@2026.6.10-beta.1",
    );
    expectMockLineContains(runtime.log, "mo gateway restart");
  });
