...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("mo-daemon-install-int-");
    configPath = path.join(tempHome, "mo.json");
    process.env.HOME = tempHome;
    process.env.MO_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.MO_GATEWAY_TOKEN).toBeUndefined();
  });
});
