...
describe("renderGatewayServiceStartHints", () => {
  ...
  it("resolves daemon container context from either env key", () => {
    expect(
      resolveDaemonContainerContext({
        MO_CONTAINER: "mo-demo-container",
      } as NodeJS.ProcessEnv),
    ).toBe("mo-demo-container");
    expect(
      resolveDaemonContainerContext({
        MO_CONTAINER_HINT: "mo-demo-container",
      } as NodeJS.ProcessEnv),
    ).toBe("mo-demo-container");
  });

  it("prepends a single container restart hint when MO_CONTAINER is set", () => {
    expect(
      renderGatewayServiceStartHints({
        MO_CONTAINER: "mo-demo-container",
      } as NodeJS.ProcessEnv),
    ).toContain(
      "Restart the container or the service that manages it for mo-demo-container.",
    );
  });

  it("prepends a single container restart hint when MO_CONTAINER_HINT is set", () => {
    expect(
      renderGatewayServiceStartHints({
        MO_CONTAINER_HINT: "mo-demo-container",
      } as NodeJS.ProcessEnv),
...
describe("filterContainerGenericHints", () => {
  it("drops the generic container foreground hint when MO_CONTAINER is set", () => {
    expect(
      filterContainerGenericHints(
        [
          "systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.",
          "If you're in a container, run the gateway in the foreground instead of `mo gateway`.",
        ],
        { MO_CONTAINER: "mo-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toStrictEqual([]);
  });

  it("drops the generic container foreground hint when MO_CONTAINER_HINT is set", () => {
    expect(
      filterContainerGenericHints(
        [
          "systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.",
        ...
        { MO_CONTAINER_HINT: "mo-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toStrictEqual([]);
  });
});
