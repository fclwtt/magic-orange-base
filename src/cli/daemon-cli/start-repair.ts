...
export async function repairLoadedGatewayServiceForStart(params: {
  ...
  const wrapperPath = await resolveOpenClawWrapperPath(installEnv[MO_WRAPPER_ENV_KEY]);
  ...
  return {
    result: "started",
    message:
      "Gateway service definition repaired and started. Reopen the Control UI with `mo dashboard` or copy a fresh auth URL with `mo dashboard --no-open`.",
