...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    MO_STATE_DIR: "/tmp/mo-daemon",
    MO_CONFIG_PATH: "/tmp/mo-daemon/mo.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.MO_STATE_DIR ?? "/tmp/mo-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.MO_CONFIG_PATH ?? `${stateDir}/mo.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({
    ...
    const isDaemon = configPath.includes("/mo-daemon/");
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    setTestEnvValue("MO_STATE_DIR", "/tmp/mo-cli");
    setTestEnvValue("MO_CONFIG_PATH", "/tmp/mo-cli/mo.json");
  ...
  it("forwards requireRpc and configPath to the daemon probe", async () => {
    ...
    expect(probeInput.configPath).toBe("/tmp/mo-daemon/mo.json");
  });
  ...
  it("reuses the shared CLI config snapshot when the daemon uses the same config path", async () => {
    ...
    expect(readConfigFileSnapshotCalls).toHaveBeenCalledWith("/tmp/mo-cli/mo.json");
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.MO_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.MO_GATEWAY_PORT ?? "missing-port",
    }));
    ...
    expect(
      serviceReadRuntime.mock.calls.some(([env]) => env?.MO_GATEWAY_PORT === "19001"),
    ).toBe(true);
  ...
  it("surfaces recent service restart handoffs only during deep status", async () => {
    ...
    expect(handoffInput.MO_STATE_DIR).toBe("/tmp/mo-daemon");
    expect(handoffInput.MO_CONFIG_PATH).toBe("/tmp/mo-daemon/mo.json");
    ...
    async () => {
      serviceReadCommand.mockResolvedValueOnce({
        ...
        environment: {
          MO_STATE_DIR: "/tmp/mo-daemon",
          MO_CONFIG_PATH: "/tmp/mo-daemon/mo.json",
          MO_LAUNCHD_LABEL: "ai.mo.manual-update.gateway",
        },
      });
      ...
      expect(staleScanEnv?.MO_STATE_DIR).toBe("/tmp/mo-daemon");
      expect(staleScanEnv?.MO_CONFIG_PATH).toBe("/tmp/mo-daemon/mo.json");
      expect(staleScanEnv?.MO_LAUNCHD_LABEL).toBe("ai.mo.manual-update.gateway");
      ...
      connections: [
        {
          ...
          ppid: 1,
          command: "node",
          commandLine: "node /tmp/newer-mo/dist/index.js logs --follow",
  ...
  it("uses the fast config path for plain same-file status reads", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "mo-status-config-"));
    const configPath = path.join(tmp, "mo.json");
    await fs.writeFile(
      configPath,
      JSON.stringify({
        gateway: {
    ...
    setTestEnvValue("MO_STATE_DIR", tmp);
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    setTestEnvValue("MO_GATEWAY_TOKEN", "env-token");
    setTestEnvValue("MO_GATEWAY_PASSWORD", "env-password"); // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection when probe fails", async () => {
    ...
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "mo-gateway" }],
        hints: [],
      },
      healthy: false,
  ...
  it("reads install records from the merged daemon service environment, not the CLI process env", async () => {
    ...
    // The mock daemon service command sets MO_STATE_DIR=/tmp/mo-daemon,
    // distinct from the CLI process MO_STATE_DIR=/tmp/mo-cli. Drift
