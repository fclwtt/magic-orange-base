...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean; deep?: boolean }) {
  ...
  if (service.configAudit?.issues.length) {
    ...
    defaultRuntime.error(
      warnText(
        `Recommendation: run "${formatCliCommand("mo doctor")}" (or "${formatCliCommand("mo doctor --repair")}").`,
      ),
    );
  }
  ...
  if (status.config) {
    ...
    if (status.config.mismatch) {
      ...
      defaultRuntime.error(
        errorText(
          `Fix: rerun \`${formatCliCommand("mo gateway install --force")}\` from the same --profile / MO_STATE_DIR you expect.`,
        ),
      );
    }
    spacer();
  ...
  if (gatewayVersion) {
    ...
    if (status.cli?.version && status.cli.version !== gatewayVersion) {
      ...
      defaultRuntime.error(
        warnText(
          "Check `mo --version`, `which mo`, and `mo gateway status --deep`; if this mismatch is unexpected, update PATH so `mo` points to the version you want, or reinstall the Gateway service from that same OpenClaw install.",
        ),
      );
    }
    spacer();
  ...
  if (
    ...
    defaultRuntime.error(
      errorText(
        `Fix: run ${formatCliCommand("mo gateway restart")} and re-check with ${formatCliCommand("mo gateway status --deep")}.`,
      ),
    );
    spacer();
  }
  ...
  if (service.runtime?.cachedLabel) {
    ...
    const labelValue = resolveGatewayLaunchAgentLabel(env.MO_PROFILE);
    defaultRuntime.error(
      errorText(
    ...
    defaultRuntime.error(
      errorText(`Then reinstall: ${formatCliCommand("mo gateway install")}`),
    );
    spacer();
  }
  ...
  if (service.staleUpdateLaunchdJobs?.length) {
    ...
    defaultRuntime.error(
      errorText(
        `Fix after confirming no update is running: launchctl remove <label>, then run ${formatCliCommand("mo gateway restart")}.`,
      ),
    );
    spacer();
  }
  ...
  if (
    ...
    if (process.platform === "linux") {
      const unit = resolveGatewaySystemdServiceName(serviceEnv.MO_PROFILE);
      defaultRuntime.error(
  ...
  if (drift && drift.drifts.length > 0) {
    ...
    if (opts.deep) {
      ...
      if (updateCommands.length === 1) {
        defaultRuntime.log(
          `${label("Fix:")} ${updateCommands[0]} && ${formatCliCommand("mo gateway restart")}.`,
        );
      } else {
        ...
        defaultRuntime.log(`Then run ${formatCliCommand("mo gateway restart")}.`);
      }
    } else {
      defaultRuntime.log(
        infoText(
          `Run ${formatCliCommand("mo gateway status --deep")} for affected plugin ids and fix commands.`,
        ),
      );
    }
    spacer();
  ...
  defaultRuntime.log(`${label("Troubles:")} run ${formatCliCommand("mo status")}`);
  defaultRuntime.log(`${label("Troubleshooting:")} https://docs.mo.ai/troubleshooting`);
}
