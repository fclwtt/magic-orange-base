...
import type { OpenClawConfig, ConfigFileSnapshot } from "../config/types.mo.js";
    ...
    if (process.env.MO_NIX_MODE === "1") {
      throw new Error(
        [
          ...
          "Agent-first Nix setup: https://github.com/mo/nix-mo#quick-start",
          "OpenClaw Nix overview: https://docs.mo.ai/install/nix",
        ].join("\n"),
      );
    }
  },
...
vi.mock("../utils.js", async (importOriginal) => {
  ...
  return {
    ...
    resolveConfigDir: () => "/tmp/mo-config",
  };
});
...
const { resolveOpenClawPackageRoot } = await import("../infra/mo-root.js");
const {
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/mo-update-tests";
  let fixtureCount = 0;
  ...
  const stripOpenClawPackageAlias = (spec: string) => {
    ...
    return trimmed.toLowerCase().startsWith("mo@")
      ? trimmed.slice("mo@".length)
      : trimmed;
  };
  ...
  const expectPackageInstallSpec = (spec: string) => {
    ...
    if (isNpmGitPackageSpec(spec)) {
      ...
      installSpec = path.join(packDir, "mo-9999.0.0.tgz");
    } else {
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("mo-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("mo-updated-root");
  ...
  beforeEach(() => {
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        if (destination) {
          await fs.writeFile(path.join(destination, "mo-9999.0.0.tgz"), "packed\n", "utf8");
        }
      }
      return {
        stdout: "",
    ...
    readPackageName.mockResolvedValue("mo");
    ...
    serviceReadCommand.mockImplementation(async () =>
      (await serviceLoaded()) ? { programArguments: ["mo", "gateway", "run"] } : null,
    );
    ...
    prepareRestartScript.mockResolvedValue("/tmp/mo-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "mo-gateway" }],
      hints: [],
    });
    ...
    checkShellCompletionStatus.mockResolvedValue({
      ...
      cacheExists: false,
      cachePath: "/tmp/mo-completion.zsh",
  ...
  it("bounds completion cache refresh during update follow-up", async () => {
    const root = createCaseDir("mo-completion-timeout");
    ...
    expect(call?.[1]).toEqual([path.join(root, "mo.mjs"), "completion", "--write-state"]);
    expect(call?.[2]?.env?.MO_COMPLETION_SKIP_PLUGIN_COMMANDS).toBe("1");
  ...
  it("disarms legacy launchd updater jobs before refusing mutating updates in Nix mode", async () => {
    await withEnvAsync({ MO_NIX_MODE: "1" }, async () => {
      await expect(updateCommand({ yes: true })).rejects.toThrow("MO_NIX_MODE=1");
    });
  ...
  it("logs friendly hint with manual refresh command when completion cache write times out", async () => {
    const root = createCaseDir("mo-completion-timeout-msg");
    pathExists.mockResolvedValue(true);
    ...
    expect(logOutput).toContain("mo completion --write-state");
  ...
  it("keeps update completion refresh best-effort when profile install fails", async () => {
    ...
    expect(installCompletion).toHaveBeenCalledWith("zsh", true, "mo");
    const logOutput = vi
      .mocked(runtimeCapture.log)
      .mock.calls.map((call) => String(call[0]))
      .join("\n");
  ...
  it("respawns into the updated package root before running post-update tasks", async () => {
    ...
    expect(call?.[2]?.env?.MO_UPDATE_IN_PROGRESS).toBe("1");
    expect(call?.[2]?.env?.MO_UPDATE_POST_CORE).toBe("1");
    expect(call?.[2]?.env?.MO_UPDATE_POST_CORE_CHANNEL).toBe("dev");
    expect(call?.[2]?.env?.MO_COMPATIBILITY_HOST_VERSION).toBe("1.0.0");
  ...
  it("finishes package updates when the post-core process writes a result but keeps handles open", async () => {
    ...
    spawn.mockImplementationOnce((_command: unknown, _argv: unknown, options: unknown) => {
      const resultPath = (options as { env?: NodeJS.ProcessEnv }).env
        ?.MO_UPDATE_POST_CORE_RESULT_PATH;
      if (!resultPath) {
  ...
  it("does not restart a stopped managed gateway after post-core plugin errors", async () => {
    const root = createCaseDir("mo-update");
            ...
            warnings: [
              {
                ...
                guidance: ["Run mo update repair to retry post-update plugin repair."],
              },
            ],
            sync: {
              changed: false,
  ...
  it("does not carry gateway service markers into the post-core update process", async () => {
    ...
    expect(spawnEnv?.MO_SERVICE_MARKER).toBeUndefined();
    expect(spawnEnv?.MO_SERVICE_KIND).toBeUndefined();
  ...
  it("passes pre-update plugin install records into the post-core update process", async () => {
    ...
    const pluginInstallRecords = {
      demo: {
        source: "npm",
        spec: "@mo/demo@1.0.0",
        installPath: "/tmp/mo-demo-plugin",
      },
    } as const;
    ...
    spawn.mockImplementationOnce((_node, _argv, options) => {
      ...
      const recordsPath = env?.MO_UPDATE_POST_CORE_INSTALL_RECORDS_PATH;
      const sourceConfigPath = env?.MO_UPDATE_POST_CORE_SOURCE_CONFIG_PATH;
      if (!recordsPath) {
  ...
  it("clears stale npm resolution metadata before post-core downgrade resume", async () => {
    ...
    const pluginInstallRecords = {
      msteams: {
        source: "npm",
        spec: "@mo/msteams",
        installPath: "/tmp/mo-msteams-plugin",
        version: "1.0.0",
        resolvedName: "@mo/msteams",
        resolvedVersion: "1.0.0",
        resolvedSpec: "@mo/msteams@1.0.0",
  ...
  it("respawns into the updated git root before requested channel persistence", async () => {
    ...
    expect(call?.[2]?.env?.MO_UPDATE_POST_CORE).toBe("1");
    expect(call?.[2]?.env?.MO_UPDATE_POST_CORE_CHANNEL).toBe("dev");
    expect(call?.[2]?.env?.MO_UPDATE_POST_CORE_REQUESTED_CHANNEL).toBe("dev");
    expect(replaceConfigFile).not.toHaveBeenCalled();
    expect(syncPluginsForUpdateChannel).not.toHaveBeenCalled();
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
  });
  ...
  it("keeps downgrade post-update work in the current process", async () => {
    const downgradedRoot = createCaseDir("mo-downgraded-root");
    setupUpdatedRootRefresh({
  ...
  it("pins the compatibility host version to the downgraded target during current-process post-core plugin convergence (#87914)", async () => {
    const downgradedRoot = createCaseDir("mo-downgraded-compat-root");
    setupUpdatedRootRefresh({
      gatewayUpdateImpl: async () =>
        makeOkUpdateResult({
          mode: "npm",
    ...
    delete process.env.MO_COMPATIBILITY_HOST_VERSION;
    let hostVersionDuringPluginUpdate: string | undefined = "unset";
    updateNpmInstalledPlugins.mockImplementation(async () => {
      hostVersionDuringPluginUpdate = process.env.MO_COMPATIBILITY_HOST_VERSION;
    ...
    try {
      ...
      expect(process.env.MO_COMPATIBILITY_HOST_VERSION).toBeUndefined();
    } finally {
      delete process.env.MO_COMPATIBILITY_HOST_VERSION;
    }
  });
  ...
  it("post-core resume mode skips the core update and only runs post-update tasks", async () => {
    await withEnvAsync(
      {
        MO_UPDATE_POST_CORE: "1",
        MO_UPDATE_POST_CORE_CHANNEL: "stable",
      },
      async () => {
  ...
  it("post-core resume children exit after writing a plugin update result", async () => {
    const resultDir = createCaseDir("mo-post-core-result");
    ...
    await withEnvAsync(
      {
        MO_UPDATE_POST_CORE: "1",
        MO_UPDATE_POST_CORE_CHANNEL: "stable",
        MO_UPDATE_POST_CORE_RESULT_PATH: resultPath,
      },
      async () => {
        await updateCommand({ restart: false });
      },
  ...
  it("post-core resume mode uses the parent install records snapshot for missing payload warnings", async () => {
    const resultDir = createCaseDir("mo-post-core-records");
    ...
    await withEnvAsync(
      {
        MO_UPDATE_POST_CORE: "1",
        MO_UPDATE_POST_CORE_CHANNEL: "stable",
        MO_UPDATE_POST_CORE_INSTALL_RECORDS_PATH: recordsPath,
      },
      async () => {
  ...
  it("post-core resume mode prefers post-doctor disk install records over the stale parent snapshot", async () => {
    const resultDir = createCaseDir("mo-post-core-disk-records");
    const recordsPath = path.join(resultDir, "plugin-install-records.json");
    await fs.mkdir(resultDir, { recursive: true });
    await fs.writeFile(
      recordsPath,
  ...
  it("post-core resume mode persists the requested update channel with the updated process", async () => {
    ...
    await withEnvAsync(
      {
        MO_UPDATE_POST_CORE: "1",
        MO_UPDATE_POST_CORE_CHANNEL: "dev",
        MO_UPDATE_POST_CORE_REQUESTED_CHANNEL: "dev",
      },
      async () => {
        await updateCommand({ restart: false });
      },
  ...
  it("prints plugin channel fallbacks near the post-core plugin summary", async () => {
    ...
    await withEnvAsync(
      {
        MO_UPDATE_POST_CORE: "1",
        MO_UPDATE_POST_CORE_CHANNEL: "beta",
      },
      async () => {
        await updateCommand({ restart: false });
      },
  ...
  it("keeps json update output successful when post-core plugin updates warn", async () => {
          ...
          outcomes: [
            {
              ...
              message:
                proceed === false
                  ? "Failed to update demo: aborted: npm package integrity drift detected for @mo/demo@1.0.0"
    ...
    expect(pluginWarning(jsonOutput)?.guidance).toEqual([
      "Run mo update repair to retry post-update plugin repair.",
      "Run mo plugins inspect demo --runtime --json for details.",
    ]);
  ...
  it("does not print duplicate ClawHub update trust warnings in human post-core output", async () => {
    ...
    expect(output).toContain("Run mo update repair to retry post-update plugin repair.");
    expect(output).toContain("Run mo plugins inspect demo --runtime --json for details.");
  });

  it("detects missing plugin payloads from persisted records before npm updates", async () => {
    const installPath = createCaseDir("mo-missing-plugin-payload");
  ...
  it("prints non-fatal plugin warnings in human update output", async () => {
    ...
    expect(logs).toContain("Run mo update repair to retry post-update plugin repair.");
    expect(logs).toContain("Run mo plugins inspect demo --runtime --json for details.");
  });
  ...
  it("marks blocked ClawHub update skips as post-update warnings", async () => {
    ...
    expect(pluginOutcome(jsonOutput)?.pluginId).toBe("demo");
    expect(pluginOutcome(jsonOutput)?.status).toBe("skipped");
    expect(pluginOutcome(jsonOutput)?.message).toContain("Run mo update repair");
  });
  ...
  it("preserves fresh-process plugin warning details in parent json output", async () => {
    ...
    spawn.mockImplementationOnce((_node, _argv, options) => {
      ...
      queueMicrotask(() => {
        void (async () => {
          const resultPath = env?.MO_UPDATE_POST_CORE_RESULT_PATH;
          if (resultPath) {
            await fs.writeFile(
              resultPath,
              JSON.stringify({
                ...
                warnings: [
                  {
                    ...
                    message:
                      'Plugin "demo" could not be processed after the core update: Failed to update demo: registry timeout Run mo update repair to retry post-update plugin repair. Run mo plugins inspect demo --runtime --json for details.',
                    guidance: [
                      "Run mo update repair to retry post-update plugin repair.",
                      "Run mo plugins inspect demo --runtime --json for details.",
                    ],
  ...
  it("parses update status --json as the subcommand option", async () => {
    ...
    await program.parseAsync(["node", "mo", "update", "status", "--json"]);
  ...
  it.each([
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expectPackageInstallSpec("mo@latest");
      }
  ...
  it("allows package updates from inherited gateway service env when the managed gateway is not running", async () => {
    mockPackageInstallStatus(createCaseDir("mo-update"));
    ...
    expect(defaultRuntime.error).not.toHaveBeenCalledWith(
      [
        ...
        "Run `mo update` from a shell outside the gateway service, or stop the gateway service first and then update.",
      ].join("\n"),
    );
    expectPackageInstallSpec("mo@latest");
  });
  ...
  it("refuses package updates from inherited gateway service env when --no-restart leaves the gateway running", async () => {
    ...
    serviceReadCommand.mockResolvedValue({
      programArguments: ["mo", "gateway", "run"],
      environment: {
        MO_SERVICE_MARKER: "mo",
        MO_SERVICE_KIND: "gateway",
      },
  ...
  it("refuses package updates from inside the active gateway process tree", async () => {
    ...
    const errors = vi.mocked(defaultRuntime.error).mock.calls.map((call) => String(call[0]));
    expect(errors.join("\n")).toContain(
      "mo update detected it is running inside the gateway process tree.",
    );
  ...
  it("blocks package updates when the target requires a newer Node runtime", async () => {
    ...
    expect(errors.join("\n")).toContain("Node ");
    expect(errors.join("\n")).toContain(
      "Bare `npm i -g mo` can silently install an older compatible release.",
    );
  });
  ...
  it.each([
    ...
    },
    {
      ...
      expectedSpec: "github:mo/mo#main",
    },
    {
      name: "explicit git package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("mo-update"));
        await updateCommand({ yes: true, tag: "github:mo/mo#main" });
      },
      expectedSpec: "github:mo/mo#main",
    },
    {
      ...
      run: async () => {
        mockPackageInstallStatus(createCaseDir("mo-update"));
        await updateCommand({ yes: true, tag: "OpenClaw@github:mo/mo#main" });
      },
      expectedSpec: "OpenClaw@github:mo/mo#main",
    },
    {
      name: "full git URL package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("mo-update"));
        await updateCommand({ yes: true, tag: "https://github.com/mo/mo.git#main" });
      },
      expectedSpec: "https://github.com/mo/mo.git#main",
    },
    {
      name: "hosted GitHub URL package spec without git suffix",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("mo-update"));
        await updateCommand({ yes: true, tag: "https://github.com/mo/mo#main" });
      },
      expectedSpec: "https://github.com/mo/mo#main",
    },
    {
      ...
      run: async () => {
        ...
        await updateCommand({
          yes: true,
          tag: "mo@https://github.com/mo/mo#main",
        });
      },
      expectedSpec: "https://github.com/mo/mo#main",
    },
    {
      ...
      run: async () => {
        mockPackageInstallStatus(createCaseDir("mo-update"));
        await updateCommand({ yes: true, tag: "mo/mo#main" });
      },
      expectedSpec: "mo/mo#main",
    },
    {
      name: "SCP-style SSH package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("mo-update"));
        await updateCommand({ yes: true, tag: "git@github.com:mo/mo.git#main" });
      },
      expectedSpec: "git@github.com:mo/mo.git#main",
    },
    {
      name: "MO_UPDATE_PACKAGE_SPEC override",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("mo-update"));
        await withEnvAsync(
          { MO_UPDATE_PACKAGE_SPEC: "http://10.211.55.2:8138/mo-next.tgz" },
          async () => {
            await updateCommand({ yes: true, tag: "latest" });
          },
        );
      },
      expectedSpec: "http://10.211.55.2:8138/mo-next.tgz",
    },
  ] as const)(
  ...
  it("fails package updates when the installed correction version does not match the requested target", async () => {
    ...
    const pkgRoot = path.join(nodeModules, "mo");
    mockPackageInstallStatus(tempDir);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "mo", version: "2026.3.23" }),
      "utf-8",
    );
  ...
  it("stops package post-update work when staged npm install verification fails", async () => {
    const tempDir = await createTrackedTempDir("mo-update-staged-fail-");
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "mo", version: "2026.4.20" }),
      "utf-8",
    );
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      ...
      if (
        ...
        const stageRoot = path.join(stagePrefix, "lib", "node_modules", "mo");
        await fs.mkdir(path.join(stageRoot, "dist"), { recursive: true });
        await fs.writeFile(
          path.join(stageRoot, "package.json"),
          JSON.stringify({ name: "mo", version: "2026.4.25" }),
          "utf-8",
        );
  ...
  it("marks package post-update doctor as update-in-progress", async () => {
    const tempDir = await createTrackedTempDir("mo-update-package-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "mo");
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "mo", version: "2026.4.21" }),
      "utf-8",
    );
    ...
    expect(
      (doctorCall?.[1].env as NodeJS.ProcessEnv | undefined)?.MO_UPDATE_IN_PROGRESS,
    ).toBe("1");
  ...
  it("continues package post-core work for explicit post-update doctor advisories", async () => {
    const tempDir = await createTrackedTempDir("mo-update-package-doctor-warning-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "mo");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    expect(postCoreCall?.[2]?.env?.MO_UPDATE_POST_CORE).toBe("1");
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
    expect(defaultRuntime.exit).not.toHaveBeenCalledWith(1);
    const jsonOutput = lastWriteJsonCall() as UpdateRunResult | undefined;
    const doctorStep = jsonOutput?.steps.find((step) => step.name === "mo doctor");
    expect(jsonOutput?.status).toBe("ok");
  ...
  it("fails package updates when the post-update doctor is killed after verification", async () => {
    const tempDir = await createTrackedTempDir("mo-update-package-doctor-timeout-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "mo");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
  ...
  it("runs package post-update doctor from the verified package root after a staged swap", async () => {
    const tempDir = await createTrackedTempDir("mo-update-staged-doctor-");
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      ...
      if (argv[0] === "npm" && argv[1] === "i" && argv.includes("--prefix")) {
        ...
        await fs.writeFile(
          ...
          JSON.stringify({ name: "mo", version: "2026.5.14" }),
          "utf-8",
        );
    ...
    expect(
      (doctorCall?.[1].env as NodeJS.ProcessEnv | undefined)?.MO_COMPATIBILITY_HOST_VERSION,
    ).toBe("2026.5.14");
    expect(defaultRuntime.exit).not.toHaveBeenCalledWith(1);
  });

  it("stops a running managed gateway before package replacement", async () => {
    const tempDir = await createTrackedTempDir("mo-update-stop-service-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "mo");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    const serviceStopCall = serviceStop.mock.calls[0]?.[0] as
      ...
      | undefined;
    expect(serviceStopCall?.env?.MO_SERVICE_MARKER).toBe("mo");
    expect(serviceStopCall?.env?.MO_SERVICE_KIND).toBe("gateway");
  ...
  it("stops a running managed git gateway when wrapper commands hide the service root", async () => {
    const wrapperPath = path.join(
      createCaseDir("mo-update-wrapper-service"),
      "gateway-wrapper",
    );
    serviceReadCommand.mockResolvedValue({
  ...
  it("stops a managed gateway rooted at the git checkout when switching package installs to dev", async () => {
    const packageRoot = createCaseDir("mo-update-package-root");
    const gitRoot = await createTrackedTempDir("mo-update-git-service-root-");
    ...
    await withEnvAsync({ MO_GIT_DIR: gitRoot }, async () => {
  ...
  it("stops a managed gateway rooted at the package install when switching package installs to dev", async () => {
    const packageRoot = await createTrackedTempDir("mo-update-package-service-root-");
  ...
  it("does not stop or restart a managed gateway owned by another git checkout", async () => {
    const otherRoot = await createTrackedTempDir("mo-update-other-service-root-");
  ...
  it("keeps managed service stop output off stdout during json package updates", async () => {
    const tempDir = await createTrackedTempDir("mo-update-json-stop-service-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "mo");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    ...
    serviceStop.mockImplementationOnce(async (params: { stdout?: NodeJS.WritableStream }) => {
      params.stdout?.write("Stopped systemd service: mo-gateway.service\n");
    });
    pathExists.mockImplementation(async (candidate: string) => {
      try {
        await fs.access(candidate);
  ...
  it("disarms legacy launchd updater jobs before stopping the gateway", async () => {
    const tempDir = await createTrackedTempDir("mo-update-launchd-loop-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "mo");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
  ...
  it("refreshes package installs even when the current version already matches the target", async () => {
    const tempDir = await createTrackedTempDir("mo-update-current-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "mo");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "mo", version: "2026.4.23" }),
      "utf-8",
    );
    await fs.writeFile(entryPath, "export {};\n", "utf-8");
    for (const relativePath of TEST_BUNDLED_RUNTIME_SIDECAR_PATHS) {
    ...
    expect(postCoreSpawn?.[2].env?.MO_UPDATE_POST_CORE).toBe("1");
    expect(postCoreSpawn?.[2].env?.MO_UPDATE_POST_CORE_CHANNEL).toBe("stable");
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
    expect(
      vi
        .mocked(defaultRuntime.log)
  ...
  it("retries package updates without optional deps when npm global update fails", async () => {
    const tempDir = await createTrackedTempDir("mo-update-optional-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "mo");
    mockPackageInstallStatus(pkgRoot);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "mo", version: "1.0.0" }),
      "utf-8",
    );
  ...
  it("uses the owning npm binary for package updates when PATH npm points elsewhere", async () => {
    ...
    const pkgRoot = path.join(brewRoot, "mo");
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("mo-update");
    const localAppData = createCaseDir("mo-localappdata");
  ...
  it("warns when a package update targets a managed service root outside the shell root", async () => {
    const shellRoot = createCaseDir("mo-shell-root");
    const serviceRoot = await createTrackedTempDir("mo-service-root-");
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "mo", version: "2026.5.18" }),
      "utf-8",
    );
    ...
    expect(logs).toContain("make sure `mo` on PATH resolves to the managed service root");
  ...
  it("runs managed service package follow-up commands with the service Node", async () => {
    const shellRoot = createCaseDir("mo-shell-root");
    const servicePrefix = await createTrackedTempDir("mo-service-prefix-");
    const nodeModules = path.join(servicePrefix, "lib", "node_modules");
    const serviceRoot = path.join(nodeModules, "mo");
  ...
  it("uses the managed service Node when package roots match but node binaries differ", async () => {
    const root = createCaseDir("mo-same-root");
  ...
  it("uses the managed service Node for follow-up commands when roots match but nodes differ", async () => {
    const servicePrefix = await createTrackedTempDir("mo-service-prefix-");
    const nodeModules = path.join(servicePrefix, "lib", "node_modules");
    const root = path.join(nodeModules, "mo");
    const serviceNode = path.join(servicePrefix, "bin", "node");
    const serviceNpm = path.join(servicePrefix, "bin", "npm");
    const entrypoint = path.join(root, "dist", "index.js");
    await fs.mkdir(path.dirname(entrypoint), { recursive: true });
  ...
  it("pins package install to the service root when nodes differ and no owning npm exists at the prefix", async () => {
    const servicePrefix = await createTrackedTempDir("mo-no-npm-prefix-");
    const nodeModules = path.join(servicePrefix, "lib", "node_modules");
    const root = path.join(nodeModules, "mo");
    const serviceNode = path.join(servicePrefix, "bin", "node");
    const entrypoint = path.join(root, "dist", "index.js");
  ...
  it("refreshes post-doctor config before post-update plugin sync", async () => {
    ...
    syncPluginsForUpdateChannel.mockImplementation(async ({ config }) => ({
      ...
      config: {
        ...
        plugins: {
          ...config.plugins,
          load: { paths: ["/tmp/mo-updated-plugin"] },
        },
      },
      summary: {
        switchedToBundled: [],
  ...
  it("restores pre-update channels when post-core resume sees post-doctor config without them", async () => {
    const tempDir = createCaseDir("mo-update");
    const configPath = path.join(tempDir, "mo.json");
    const preUpdateConfig = {
      update: { channel: "stable" },
      channels: {
        whatsapp: {
    ...
    await withEnvAsync(
      {
        MO_UPDATE_POST_CORE: "1",
        MO_UPDATE_POST_CORE_CHANNEL: "stable",
        MO_UPDATE_POST_CORE_STARTED_AT_MS: String(updateStartedAtMs),
      },
      async () => {
        await updateCommand({ yes: true, restart: false });
      },
    ...
    await withEnvAsync(
      {
        MO_UPDATE_POST_CORE: "1",
        MO_UPDATE_POST_CORE_CHANNEL: "stable",
        MO_UPDATE_POST_CORE_SOURCE_CONFIG_PATH: sourceConfigPath,
      },
      async () => {
        await updateCommand({ yes: true, restart: false });
      },
  ...
  it("persists channel and runs post-update work after switching from package to git", async () => {
    const tempDir = createCaseDir("mo-update");
    const gitRoot = path.join(tempDir, "..", "mo");
  ...
  it("explains why git updates cannot run with edited files", async () => {
    ...
    expect(logs.join("\n")).toContain(
      "Commit, stash, or discard the local changes, then rerun `mo update`.",
    );
    expect(serviceStop).not.toHaveBeenCalled();
    expect(defaultRuntime.exit).toHaveBeenCalledWith(0);
  });
  ...
  it("tries the updated install restart when package service refresh fails", async () => {
    const updatedRoot = createCaseDir("mo-updated-root");
  ...
  it("accepts same-version refresh failure recovery when the managed service restarts", async () => {
    ...
    await fs.writeFile(
      updatedPackageJson,
      JSON.stringify({ name: "mo", version: "2026.4.24" }),
      "utf8",
    );
    setupUpdatedRootRefresh({
      entrypoints: [updatedEntrypoint],
  ...
  it("rejects same-version refresh failure recovery from a stale service definition", async () => {
    const oldRoot = createCaseDir("mo-old-root");
    const updatedRoot = createCaseDir("mo-updated-root");
  ...
  it("writes the control-plane update sentinel after managed package restart health passes", async () => {
    const stateDir = await createTrackedTempDir("mo-update-sentinel-state-");
    const metaDir = await createTrackedTempDir("mo-update-sentinel-meta-");
  ...
  it.each([
    ...
    },
    {
      ...
      expectedEnv: () => ({
        MO_STATE_DIR: path.resolve("./state"),
        MO_CONFIG_PATH: path.resolve("./config/mo.json"),
      }),
      assertExtra: () => {
        expect(runDaemonInstall).not.toHaveBeenCalled();
      },
    },
    {
      ...
      expectedEnv: (context?: { originalCwd: string }) => ({
        MO_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
      }),
      assertExtra: () => {
        expect(runDaemonInstall).not.toHaveBeenCalled();
      },
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ MO_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.MO_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("marks the whole update command as update-in-progress", async () => {
    await withEnvAsync({ MO_UPDATE_IN_PROGRESS: undefined }, async () => {
      ...
      vi.mocked(runGatewayUpdate).mockImplementationOnce(async () => {
        observedUpdateEnv = process.env.MO_UPDATE_IN_PROGRESS;
  ...
  it("updateFinalizeCommand runs doctor and plugin convergence with full update env", async () => {
    await withEnvAsync(
      {
        MO_UPDATE_IN_PROGRESS: undefined,
        MO_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR: undefined,
        MO_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE: undefined,
      },
      async () => {
        ...
        expect(doctorEnv?.MO_UPDATE_IN_PROGRESS).toBe("1");
        expect(doctorEnv?.MO_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR).toBe("1");
        expect(doctorEnv?.MO_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE).toBe("1");
        expect(process.env.MO_UPDATE_IN_PROGRESS).toBeUndefined();
        expect(process.env.MO_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR).toBeUndefined();
        expect(process.env.MO_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE).toBeUndefined();
  ...
  it("updateFinalizeCommand restores channels from the RPC pre-update config payload", async () => {
    const tempDir = createCaseDir("mo-rpc-finalize");
    const sourceConfigPath = path.join(tempDir, "source-config.json");
    const preUpdateConfig = {
      channels: {
        whatsapp: {
  ...
  it("updateFinalizeCommand converges on the effective channel from env without persisting update.channel", async () => {
    ...
    const priorEffective = process.env.MO_UPDATE_EFFECTIVE_CHANNEL;
    // Simulate a no-config git/source update whose effective channel is dev.
    process.env.MO_UPDATE_EFFECTIVE_CHANNEL = "dev";
    try {
      await updateFinalizeCommand({ json: true, restart: false });
    } finally {
      if (priorEffective === undefined) {
        delete process.env.MO_UPDATE_EFFECTIVE_CHANNEL;
      } else {
        process.env.MO_UPDATE_EFFECTIVE_CHANNEL = priorEffective;
      }
    }
  ...
  it.each([
    ...
    },
    {
      ...
      requireTty: false,
      expectedError:
        "Update wizard requires a TTY. Use `mo update --channel <stable|beta|dev>` instead.",
    },
  ] as const)(
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("mo-update-wizard");
    await withEnvAsync({ MO_GIT_DIR: tempDir }, async () => {
      setTty(true);
  ...
  it("uses ~/mo as the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    try {
      await withEnvAsync(
        {
          HOME: undefined,
          MO_GIT_DIR: undefined,
          MO_HOME: undefined,
          USERPROFILE: undefined,
        },
        async () => {
          expect(resolveGitInstallDir()).toBe(path.posix.join("/tmp/oc-home", "mo"));
        },
      );
    } finally {
  ...
  it("uses MO_HOME for the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    try {
      await withEnvAsync(
        { MO_GIT_DIR: undefined, MO_HOME: "/srv/mo-home" },
        async () => {
          expect(resolveGitInstallDir()).toBe(path.posix.join("/srv/mo-home", "mo"));
        },
      );
    } finally {
      homedirSpy.mockRestore();
  ...
  it("creates the parent directory before cloning the default dev checkout", async () => {
    const root = await createTrackedTempDir("mo-update-home-");
    const home = path.join(root, "custom-mo-home");
    const checkoutDir = path.join(home, "mo");

    await withEnvAsync({ MO_GIT_DIR: undefined, MO_HOME: home }, async () => {
