...
function createCompletionProgram(): Command {
  ...
  program.option(
    "--status-json",
    "Output JSON (alias for `models status --json`) in $MO_STATE_DIR",
  );
...
describe("completion-cli", () => {
  it("generates zsh functions for nested subcommands", () => {
    ...
    expect(script).toContain("_mo_gateway()");
    expect(script).toContain("(status) _mo_gateway_status ;;");
    expect(script).toContain("(restart) _mo_gateway_restart ;;");
    expect(script).toContain("--force[Force the action]");
    expect(script).toContain("\\`models status --json\\`");
    expect(script).toContain("\\$MO_STATE_DIR");
  });

  it("escapes zsh option descriptions for double-quoted arguments specs", () => {
    const program = new Command()
      .name("mo")
      .option("--literal", "Use $MO_STATE_DIR with `model/list` and John's profile");

    const script = getCompletionScript("zsh", program);

    expect(script).toContain(
      "--literal[Use \\$MO_STATE_DIR with \\`model/list\\` and John's profile]",
    );
  ...
  it("defers zsh registration until compinit is available", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-zsh-completion-"));
    try {
      const scriptPath = path.join(tempDir, "mo.zsh");
      ...
      const result = spawnSync(
        ...
        [
          ...
          `
            ...
            [[ -z "\${_comps[mo]-}" ]] || exit 10
            [[ "\${precmd_functions[(r)_mo_register_completion]}" = "_mo_register_completion" ]] || exit 11
            autoload -Uz compinit
            compinit -C
            _mo_register_completion
            [[ -z "\${precmd_functions[(r)_mo_register_completion]}" ]] || exit 12
            [[ "\${_comps[mo]-}" = "_mo_root_completion" ]]
          `,
        ],
        {
          encoding: "utf8",
  ...
  it("generates PowerShell command paths without the executable prefix", () => {
    ...
    expect(script).not.toContain("if ($commandPath -eq 'mo gateway') {");
  ...
  it("generates valid PowerShell root arrays when commands or options are empty", () => {
    const commandsOnly = new Command().name("mo");
    commandsOnly.command("status");
    const optionsOnly = new Command().name("mo").option("--json", "JSON output");
    const empty = new Command().name("mo");
  ...
  it("generates fish completions for root and nested command contexts", () => {
    ...
    expect(script).toContain(
      'complete -c mo -n "__fish_use_subcommand" -a "gateway" -d \'Gateway commands\'',
    );
    expect(script).toContain(
      'complete -c mo -n "__mo_command_path_matches gateway -- -t --token" -a "status" -d \'Show gateway status\'',
    );
    expect(script).toContain(
      "complete -c mo -n \"__mo_command_path_matches gateway -- -t --token\" -l force -d 'Force the action'",
    );
    expect(script).toContain(
      "complete -c mo -n \"__mo_command_path_matches gateway status -- -t --token\" -l json -d 'JSON output'",
    );
    expect(script).toContain("__mo_command_path_matches gateway -- -t --token");
  ...
  it("scopes fish value-taking option skips to the active command path", () => {
    ...
    expect(script).toContain("__mo_command_path_matches agent -- --verbose");
    expect(script).toContain("__mo_command_path_matches sessions cleanup --");
    expect(script).not.toContain("__mo_command_path_matches sessions cleanup -- --verbose");
    expect(script).toContain(
      "complete -c mo -n \"__mo_command_path_matches sessions cleanup --\" -l dry-run -d 'Preview cleanup'",
    );
  });
