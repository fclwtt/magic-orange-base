...
describe("parseCliContainerArgs", () => {
  it("extracts a root --container flag before the command", () => {
    expect(
      parseCliContainerArgs(["node", "mo", "--container", "demo", "status", "--deep"]),
    ).toEqual({
      ok: true,
      container: "demo",
      argv: ["node", "mo", "status", "--deep"],
    });
  });

  it("accepts the equals form", () => {
    expect(parseCliContainerArgs(["node", "mo", "--container=demo", "health"])).toEqual({
      ok: true,
      container: "demo",
      argv: ["node", "mo", "health"],
    });
  ...
  it("rejects a missing container value", () => {
    expect(parseCliContainerArgs(["node", "mo", "--container"])).toEqual({
      ok: false,
  ...
  it("does not consume an adjacent flag as the container value", () => {
    expect(
      parseCliContainerArgs(["node", "mo", "--container", "--no-color", "status"]),
    ).toEqual({
      ok: false,
      error: "--container requires a value",
    });
  ...
  it("leaves argv unchanged when the flag is absent", () => {
    expect(parseCliContainerArgs(["node", "mo", "status"])).toEqual({
      ok: true,
      container: null,
      argv: ["node", "mo", "status"],
    });
  ...
  it("extracts --container after the command like other root options", () => {
    expect(
      parseCliContainerArgs(["node", "mo", "status", "--container", "demo", "--deep"]),
    ).toEqual({
      ok: true,
      container: "demo",
      argv: ["node", "mo", "status", "--deep"],
...
describe("resolveCliContainerTarget", () => {
  it("uses argv first and falls back to MO_CONTAINER", () => {
    expect(
      resolveCliContainerTarget(["node", "mo", "--container", "demo", "status"], {}),
    ).toBe("demo");
    expect(resolveCliContainerTarget(["node", "mo", "status"], {})).toBeNull();
    expect(
      resolveCliContainerTarget(["node", "mo", "status"], {
        MO_CONTAINER: "demo",
...
describe("maybeRunCliInContainer", () => {
  it("passes through when no container target is provided", () => {
    expect(maybeRunCliInContainer(["node", "mo", "status"], { env: {} })).toEqual({
      handled: false,
      argv: ["node", "mo", "status"],
    });
  });

  it("uses MO_CONTAINER when the flag is absent", () => {
    const spawnSync = vi
      .fn()
      .mockReturnValueOnce({
        status: 0,
    ...
    expect(
      maybeRunCliInContainer(["node", "mo", "status"], {
        env: { MO_CONTAINER: "demo" } as NodeJS.ProcessEnv,
        spawnSync,
      }),
    ).toEqual({
      handled: true,
  ...
  it("clears inherited host routing and gateway env before execing into the child CLI", () => {
    ...
    maybeRunCliInContainer(["node", "mo", "status"], {
      env: {
        ...
        MO_GATEWAY_TOKEN: "token",
        MO_GATEWAY_PASSWORD: "password",
      } as NodeJS.ProcessEnv,
      spawnSync,
    });
  ...
  it("passes the proxy URL env fallback into the child container CLI", () => {
    ...
    maybeRunCliInContainer(["node", "mo", "status"], {
      env: {
        MO_CONTAINER: "demo",
        MO_PROXY_URL: " http://proxy.internal:3128 ",
      } as NodeJS.ProcessEnv,
      spawnSync,
    });
  ...
  it("redacts proxy URL credentials and URL suffixes before rejecting loopback container proxy forwarding", () => {
    ...
    expect(message).toContain("MO_PROXY_URL=http://redacted:redacted@127.0.0.1:3128/");
  ...
  it("allows explicitly overridden loopback proxy URL forwarding into a child container CLI", () => {
    ...
    maybeRunCliInContainer(["node", "mo", "status"], {
      env: {
        ...
        MO_CONTAINER_ALLOW_LOOPBACK_PROXY_URL: "1",
      } as NodeJS.ProcessEnv,
      spawnSync,
    });
    ...
    expect(podmanCall[1]).toContain("MO_PROXY_URL=http://127.0.0.1:3128");
  ...
  it("executes through podman when the named container is running", () => {
    ...
    expect(
      maybeRunCliInContainer(["node", "mo", "--container", "demo", "status"], {
        env: {},
        spawnSync,
      }),
    ).toEqual({
  ...
  it("falls back to docker when podman does not have the container", () => {
    ...
    expect(
      maybeRunCliInContainer(["node", "mo", "--container", "demo", "health"], {
        env: { USER: "mo" } as NodeJS.ProcessEnv,
        spawnSync,
      }),
    ).toEqual({
      handled: true,
  ...
  it("checks docker after podman and before failing", () => {
    ...
    expect(spawnSync).toHaveBeenNthCalledWith(
      ...
      {
        stdio: "inherit",
        env: { USER: "somalley", MO_CONTAINER: "" },
      },
    );
  ...
  it("allocates a tty for interactive terminal sessions", () => {
    ...
    maybeRunCliInContainer(["node", "mo", "--container", "demo", "setup"], {
      env: {},
      spawnSync,
      stdinIsTTY: true,
      stdoutIsTTY: true,
  ...
  it("prefers --container over MO_CONTAINER", () => {
    const spawnSync = vi
      .fn()
      .mockReturnValueOnce({
        status: 0,
    ...
    expect(
      maybeRunCliInContainer(["node", "mo", "--container", "flag-demo", "health"], {
        env: { MO_CONTAINER: "env-demo" } as NodeJS.ProcessEnv,
        spawnSync,
      }),
    ).toEqual({
      handled: true,
  ...
  it("skips recursion when the bypass env is set", () => {
    expect(
      maybeRunCliInContainer(["node", "mo", "--container", "demo", "status"], {
        env: { MO_CLI_CONTAINER_BYPASS: "1" } as NodeJS.ProcessEnv,
      }),
    ).toEqual({
      handled: false,
      argv: ["node", "mo", "--container", "demo", "status"],
    });
  });
  ...
  it("blocks updater commands from running inside the container", () => {
    ...
    expect(() =>
      maybeRunCliInContainer(["node", "mo", "--container", "demo", "update"], {
        env: {},
        spawnSync,
      }),
    ).toThrow(
      "mo update is not supported with --container; rebuild or restart the container image instead.",
    );
  ...
  it("blocks update after interleaved root flags", () => {
    ...
    expect(() =>
      maybeRunCliInContainer(["node", "mo", "--container", "demo", "--no-color", "update"], {
        env: {},
        spawnSync,
      }),
    ).toThrow(
