...
export function rejectAgentScopedModelWrite(
  ...
  throw new Error(
    `mo models ${commandName} does not support --agent; it only updates global model defaults. Remove --agent, or run ${formatCliCommand("mo agents list")} and set the per-agent model in agent config.`,
  );
}
