...
describe("json output mode", () => {
  ...
  it("detects json output flags before argv terminators", () => {
    expect(hasJsonOutputFlag(["node", "mo", "nodes", "list", "--json"])).toBe(true);
    expect(hasJsonOutputFlag(["node", "mo", "nodes", "list", "--json=true"])).toBe(true);
    expect(hasJsonOutputFlag(["node", "mo", "nodes", "--", "--json"])).toBe(false);
  });
