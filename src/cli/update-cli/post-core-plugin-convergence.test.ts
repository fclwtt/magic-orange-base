...
describe("runPostCorePluginConvergence", () => {
  ...
  function makeTempDir(): string {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-post-core-convergence-"));
    tempDirs.push(dir);
    return dir;
  }
  ...
  it("calls repair with MO_UPDATE_POST_CORE_CONVERGENCE=1 set", async () => {
    const cfg = { plugins: { entries: {} } } as unknown as OpenClawConfig;
    await runPostCorePluginConvergence({
      cfg,
      env: { MO_UPDATE_IN_PROGRESS: "1" },
    });
    ...
    expect(mocks.repairMissingConfiguredPluginInstalls).toHaveBeenCalledWith({
      ...
      env: {
        ...
        MO_COMPATIBILITY_HOST_VERSION: VERSION,
  ...
  it("uses the candidate runtime version over a stale inherited host version", async () => {
    ...
    await runPostCorePluginConvergence({
      cfg,
      env: { MO_COMPATIBILITY_HOST_VERSION: "2026.5.12" },
    });
    expect(mocks.repairMissingConfiguredPluginInstalls).toHaveBeenCalledWith({
      cfg,
      env: {
  ...
  it("repairs managed npm mo peer links in every managed npm project before payload smoke checks", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      changes: [],
      warnings: [],
    ...
    mocks.listManagedPluginNpmRoots.mockResolvedValue([
      "/tmp/mo-state/npm",
      "/tmp/mo-state/npm/projects/codex",
    ]);
    mocks.relinkOpenClawPeerDependenciesInManagedNpmRoot
  ...
  it("keeps repair warnings nonblocking with actionable guidance", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      changes: [],
      warnings: [
        'Failed to install missing configured plugin "discord" from @mo/discord: ENETUNREACH.',
      ],
      records: {},
    });
    const result = await runPostCorePluginConvergence({
    ...
    expect(result.warnings).toStrictEqual([
      {
        ...
        message:
          'Failed to install missing configured plugin "discord" from @mo/discord: ENETUNREACH.',
        guidance: ["Run `mo update repair` to retry plugin repair."],
      },
    ]);
  });
  ...
  it("keeps failed configured-plugin repair fetches nonblocking", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      changes: [],
      warnings: [
        'Failed to install missing configured plugin "matrix" from clawhub:@mo/matrix@beta: ClawHub ClawPack download for @mo/matrix@2026.6.1-beta.1 body stalled after 30000ms.',
      ],
  ...
  it("surfaces repair notices without marking convergence errored", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      ...
      notices: [
        'ClawHub trust warning for "@mo/discord@1.2.3": ClawHub has not completed a fresh clean security check for this release. Status: security scan is pending. Review the package before enabling it.',
      ],
      warnings: [],
    ...
    expect(result.warnings).toStrictEqual([
      {
        ...
        guidance: [
          "Run `mo update repair` to retry plugin repair.",
          "Run `mo plugins inspect brave --runtime --json` for details.",
        ],
      },
    ]);
  });
