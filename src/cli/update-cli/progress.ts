...
const STEP_LABELS: Record<string, string> = {
  ...
  "mo doctor entry": "Checking doctor entrypoint",
  "mo doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (isGlobalPackageInstallStep && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g mo@latest");
    hints.push(
      "System install outline: mo gateway stop -> sudo <system-npm> i -g mo@latest -> mo gateway install --force -> mo gateway restart.",
    );
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g mo@latest --omit=optional");
  }
