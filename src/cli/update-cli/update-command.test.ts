...
describe("resolveGatewayInstallEntrypointCandidates", () => {
  it("prefers index.js before legacy entry.js", () => {
    expect(resolveGatewayInstallEntrypointCandidates("/tmp/mo-root")).toEqual([
      path.join("/tmp/mo-root", "dist", "index.js"),
      path.join("/tmp/mo-root", "dist", "index.mjs"),
      path.join("/tmp/mo-root", "dist", "entry.js"),
      path.join("/tmp/mo-root", "dist", "entry.mjs"),
    ]);
  });
});
  ...
  it("uses the managed service port ahead of the caller environment", () => {
    expect(
      resolveUpdatedGatewayRestartPort({
        ...
        processEnv: { MO_GATEWAY_PORT: "19001" },
        serviceEnv: { MO_GATEWAY_PORT: "19002" },
      }),
    ).toBe(19002);
  });
...
describe("resolvePostUpdateServiceStateReadEnv", () => {
  it("keeps package restart preparation anchored to the pre-update service env", () => {
    const processEnv = {
      MO_STATE_DIR: "/source/state",
      MO_CONFIG_PATH: "/source/mo.json",
    } as NodeJS.ProcessEnv;
    const prePackageServiceEnv = {
      MO_STATE_DIR: "/managed/state",
      MO_CONFIG_PATH: "/managed/mo.json",
    } as NodeJS.ProcessEnv;
  ...
  it("keeps git updates tied to the caller environment", () => {
    const processEnv = { MO_STATE_DIR: "/source/state" } as NodeJS.ProcessEnv;
    const prePackageServiceEnv = { MO_STATE_DIR: "/managed/state" } as NodeJS.ProcessEnv;
  ...
  it("uses the managed service environment for git updates stopped by this updater", () => {
    const processEnv = { MO_STATE_DIR: "/source/state" } as NodeJS.ProcessEnv;
    const preManagedServiceEnv = { MO_STATE_DIR: "/managed/state" } as NodeJS.ProcessEnv;
...
describe("resolvePostInstallDoctorEnv", () => {
  it("uses the managed service profile paths for post-install doctor", () => {
    const env = resolvePostInstallDoctorEnv({
      invocationCwd: "/srv/mo",
      baseEnv: {
        PATH: "/bin",
        MO_STATE_DIR: "/wrong/state",
        MO_CONFIG_PATH: "/wrong/mo.json",
        MO_PROFILE: "wrong",
      },
      serviceEnv: {
        MO_STATE_DIR: "daemon-state",
        MO_CONFIG_PATH: "daemon-state/mo.json",
        MO_PROFILE: "work",
      },
    });
    ...
    expect(env.MO_STATE_DIR).toBe(path.join("/srv/mo", "daemon-state"));
    expect(env.MO_CONFIG_PATH).toBe(
      path.join("/srv/mo", "daemon-state", "mo.json"),
    );
    expect(env.MO_PROFILE).toBe("work");
  });
  ...
  it("keeps the caller env when no managed service env is available", () => {
    ...
    expect(env.PATH).toBe("/bin");
    expect(env.NODE_DISABLE_COMPILE_CACHE).toBe("1");
    expect(env.MO_STATE_DIR).toBe("/caller/state");
    expect(env.MO_PROFILE).toBe("caller");
  });
});

describe("collectMissingPluginInstallPayloads", () => {
  it("reports tracked npm install records whose package payload is absent", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-update-plugin-payload-"));
    const presentDir = path.join(tmpDir, "state", "npm", "node_modules", "@mo", "present");
    const missingDir = path.join(tmpDir, "state", "npm", "node_modules", "@mo", "missing");
    ...
    try {
      ...
      await fs.writeFile(path.join(presentDir, "package.json"), '{"name":"@mo/present"}\n');
      ...
      await expect(
        collectMissingPluginInstallPayloads({
          ...
          records: {
            present: {
              source: "npm",
              spec: "@mo/present@beta",
              installPath: presentDir,
            },
            missing: {
              source: "npm",
              spec: "@mo/missing@beta",
              installPath: missingDir,
            },
            "no-package-json": {
              source: "npm",
              spec: "@mo/no-package-json@beta",
              installPath: noPackageJsonDir,
            },
            "missing-install-path": {
              source: "npm",
              spec: "@mo/missing-install-path@beta",
            },
            local: {
              source: "path",
    ...
    try {
      ...
      await fs.writeFile(
        ...
        JSON.stringify({
          ...
          mo: { extensions: ["./missing-extension.js"] },
        }),
        "utf8",
      );
      await expect(
  ...
  it("keeps disabled trusted official npm records eligible for payload repair when requested", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-update-plugin-payload-"));
    const missingDir = path.join(tmpDir, "state", "npm", "node_modules", "@mo", "codex");
    try {
      await expect(
        collectMissingPluginInstallPayloads({
          env: { HOME: tmpDir } as NodeJS.ProcessEnv,
...
describe("formatPostUpdateGatewayRecoveryInstructions", () => {
  ...
  it("uses systemd wording on Linux instead of macOS LaunchAgent instructions", () => {
    ...
    expect(line).toContain("mo gateway restart");
    expect(line).toContain("mo gateway install --force");
    expect(line).toContain("mo gateway status --deep");
...
describe("recoverInstalledLaunchAgentAfterUpdate", () => {
  it("re-bootstraps an installed-but-not-loaded macOS LaunchAgent after update", async () => {
    ...
    const serviceEnv = { MO_PROFILE: "stomme" } as NodeJS.ProcessEnv;
    const recoveredEnv = { ...serviceEnv, MO_PORT: "18790" } as NodeJS.ProcessEnv;
  ...
  it("does not recover a loaded LaunchAgent", async () => {
    const readState = vi.fn(async () => ({
      ...
      loaded: true,
      running: true,
      env: { MO_PROFILE: "stomme" } as NodeJS.ProcessEnv,
      command: null,
  ...
  it("does not report recovered update health until the gateway passes the post-recovery wait", async () => {
    ...
    await expect(
      recoverLaunchAgentAndRecheckGatewayHealth({
        ...
        env: { MO_PROFILE: "stomme", MO_PORT: "18790" },
...
describe("resolvePostCoreUpdateChildStdio", () => {
  it('returns "pipe" on Windows so the child never inherits the parent console handles', () => {
    ...
    // causing the terminal to hang after `mo update` completes (#78445).
