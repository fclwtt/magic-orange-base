  ...
  if (manifestDeclaresOpenClawPeer(params.manifest)) {
    ...
    if (peerIssue) {
      failures.push({
        ...
        detail: `Plugin declares peerDependency "mo" but peer link audit failed: ${peerIssue.reason}.`,
      });
    }
  }
    ...
    failures.push({
      ...
      detail: `Plugin extension entry validation failed: ${
        extensionResolution.status === "invalid"
          ...
          : "package.json mo.extensions is empty"
      }`,
    });
    return failures;
...
function manifestDeclaresOpenClawPeer(manifest: PackageManifest): boolean {
  ...
  return (
    ...
    typeof (peerDependencies as Record<string, unknown>).mo === "string"
  );
}
