...
vi.mock("node:child_process", async () => {
  const { mockNodeBuiltinModule } = await import("mo/plugin-sdk/test-node-mocks");
...
`,
  ...
  function expectWindowsRestartWaitOrdering(content: string, port = 18789) {
    ...
    const skipEndLog = "mo restart skipped schtasks end";
  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      expect(content).toContain("systemctl --user restart 'mo-gateway.service'");
    ...
    it("creates restart scripts in a private temp directory with exclusive creation", async () => {
      ...
      const oldCandidatePath = path.join(os.tmpdir(), `mo-restart-${timestamp}.sh`);
      const victimDir = await makeTempDir("mo-restart-helper-victim-");
      ...
      try {
        ...
        expect(path.basename(scriptDir)).toMatch(/^mo-restart-/);
    ...
    it("uses MO_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        MO_PROFILE: "default",
        MO_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("fails with sudo systemd guidance when the gateway unit is system-scoped", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const tmpDir = await makeTempDir("mo-restart-helper-");
...
`,
      ...
      const { scriptPath } = await prepareAndReadScript({
        ...
        MO_STATE_DIR: path.join(tmpDir, "state"),
      });
      const result = await executeScript(scriptPath, {
        PATH: `${fakeBinDir}:${process.env.PATH ?? ""}`,
        MO_SYSTEMCTL_CALLS: callsPath,
      });
      const calls = await fs.readFile(callsPath, "utf-8");

      expect(result.code).toBe(78);
      expect(result.stderr).toContain("system-scoped mo gateway unit detected");
      expect(result.stderr).toContain("sudo systemctl restart mo-gateway.service");
      expect(calls).toContain("--user is-active --quiet mo-gateway.service");
      expect(calls).toContain("is-active --quiet mo-gateway.service");
      expect(calls).not.toContain("--user restart mo-gateway.service");
    });
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.mo.gateway'");
      // Should clear disabled state and fall back to bootstrap when kickstart fails.
      expect(content).toContain("launchctl enable 'gui/501/ai.mo.gateway'");
    ...
    it("captures macOS launchctl stderr to ~/.mo/logs/gateway-restart.log (#68486)", async () => {
      ...
      expect(content).toContain("exec >>'/Users/testuser/.mo/logs/gateway-restart.log' 2>&1");
    ...
    it("uses MO_STATE_DIR for the macOS update restart log", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;
      ...
      expect(content).toContain(
        "if mkdir -p '/tmp/mo-state/logs' 2>/dev/null && : >>'/tmp/mo-state/logs/gateway-restart.log' 2>/dev/null; then",
      );
      expect(content).toContain("exec >>'/tmp/mo-state/logs/gateway-restart.log' 2>&1");
      await cleanupScript(scriptPath);
    });
...
`,
      ...
      expect(log).toContain("mo restart attempt source=update target=ai.mo.gateway");
      expect(log).toContain("launchctl kickstart -k gui/501/ai.mo.gateway");
      expect(log).toContain("mo restart failed source=update status=42");
      expect(log).not.toContain("mo restart done source=update");
    });
...
`,
    ...
    it("logs custom macOS launchd labels without shell expansion", async () => {
      ...
      const { scriptPath } = await prepareAndReadScript({
        MO_LAUNCHD_LABEL: "ai.mo.$(echo injected)",
        HOME: path.join(tmpDir, "home"),
        MO_STATE_DIR: stateDir,
      });
      ...
      expect(result.code).toBeNull();
      expect(log).toContain("target=ai.mo.$(echo injected)");
      expect(log).not.toContain("target=ai.mo.injected");
    });

    it("uses MO_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        MO_PROFILE: "default",
        MO_LAUNCHD_LABEL: "com.custom.mo",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.mo'");
      await cleanupScript(scriptPath);
    });
    ...
    it("creates a guarded schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain("mo restart skipped schtasks end");
      expect(content).toContain(
        '$launcherPath = Join-Path $env:USERPROFILE ".mo\\gateway.cmd"',
      );
      expect(content).toContain("mo restart launched startup fallback");
      expectWindowsRestartWaitOrdering(content);
      expect(content).toContain('del "%~f0" >nul 2>&1');
      expect(content).toContain('rmdir "%MO_RESTART_SCRIPT_DIR%" >nul 2>&1');
      await cleanupScript(scriptPath);
    });

    it("uses MO_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });

      const { scriptPath, content } = await prepareAndReadScript({
        MO_PROFILE: "default",
        MO_WINDOWS_TASK_NAME: "OpenClaw Gateway (custom)",
      });
    ...
    it("uses custom profile in service names", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        MO_PROFILE: "production",
      });
      expect(content).toContain("mo-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      expect(content).toContain("gui/502/ai.mo.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        MO_LAUNCHD_LABEL: "ai.mo.it's-a-test",
      });
      // The plist path must also shell-escape the label to prevent injection
      expect(content).toContain("ai.mo.it'\\''s-a-test.plist");
      await cleanupScript(scriptPath);
    });
