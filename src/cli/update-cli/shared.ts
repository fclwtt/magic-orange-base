...
import { resolveOpenClawPackageRoot } from "../../infra/mo-root.js";
...
const MO_REPO_URL = "https://github.com/mo/mo.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "mo";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);

/** Normalize a CLI tag/version/spec into the npm target form accepted by update flows. */
export function normalizeTag(value?: string | null): string | null {
  return normalizePackageTagInput(value, ["mo", DEFAULT_PACKAGE_NAME]);
}
...
export function resolveGitInstallDir(): string {
  const override = process.env.MO_GIT_DIR?.trim();
  if (override) {
...
function resolveDefaultGitDir(): string {
  ...
  if (home.startsWith("/")) {
    return path.posix.join(home, "mo");
  }
  return path.join(home, "mo");
}
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    ...
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", MO_REPO_URL, params.dir],
      env: gitEnv,
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `MO_GIT_DIR points at a non-git directory: ${params.dir}. Set MO_GIT_DIR to an empty folder or an mo checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`MO_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
const COMPLETION_CACHE_MANUAL_REFRESH_HINT =
  "Shell tab-completion may be stale; refresh manually with: mo completion --write-state";

/** Best-effort refresh of shell completion state after a successful update. */
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "mo.mjs");
