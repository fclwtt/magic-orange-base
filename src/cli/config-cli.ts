...
import { isRecord as isPlainRecord } from "@mo/normalization-core/record-coerce";
import { normalizeOptionalString } from "@mo/normalization-core/string-coerce";
import {
  normalizeStringEntries,
  uniqueValues,
} from "@mo/normalization-core/string-normalization";
...
const CONFIG_SET_EXAMPLE_VALUE = formatCliCommand(
  "mo config set gateway.port 19001 --strict-json",
);
const CONFIG_SET_EXAMPLE_REF = formatCliCommand(
  "mo config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN",
);
const CONFIG_SET_EXAMPLE_PROVIDER = formatCliCommand(
  "mo config set secrets.providers.vault --provider-source file --provider-path /etc/mo/secrets.json --provider-mode json",
);
const CONFIG_SET_EXAMPLE_BATCH = formatCliCommand(
  "mo config set --batch-file ./config-set.batch.json --dry-run",
);
const CONFIG_PATCH_EXAMPLE_FILE = formatCliCommand(
  "mo config patch --file ./mo.patch.json5 --dry-run",
);
const CONFIG_PATCH_EXAMPLE_STDIN = formatCliCommand("mo config patch --stdin");
...
function formatDoctorHint(message: string): string {
  return `Run \`${formatCliCommand("mo doctor --fix")}\` ${message}`;
}
...
function formatPluginInstallConfigSetError(): string {
  return [
    ...
    `  ${formatCliCommand("mo plugins install <spec>")}`,
    `  ${formatCliCommand("mo plugins update <plugin-id>")}`,
    `  ${formatCliCommand("mo plugins uninstall <plugin-id>")}`,
  ].join("\n");
}
...
export async function runConfigGet(opts: { path: string; json?: boolean; runtime?: RuntimeEnv }) {
  ...
  try {
    ...
    if (!res.found) {
      runtime.error(
        danger(
          `Config path not found: ${opts.path}. Run ${formatCliCommand("mo config validate")} to inspect config shape.`,
        ),
      );
      runtime.exit(1);
      return;
...
export async function runConfigUnset(opts: {
  ...
  try {
    ...
    if (!unsetResult.removed) {
      ...
      runtime.error(
        danger(
          `Config path not found: ${opts.path}. Nothing was changed. Run ${formatCliCommand("mo config get <path>")} first if you are unsure of the path.`,
        ),
      );
      runtime.exit(1);
      return;
...
export async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "mo.json";
  ...
  try {
    ...
    if (!snapshot.exists) {
      if (opts.json) {
        ...
        runtime.error(
          `Create one with ${formatCliCommand("mo onboard")} or run ${formatCliCommand("mo doctor --fix")}.`,
        );
      }
      runtime.exit(1);
      return;
    ...
    if (!snapshot.valid) {
      ...
      if (opts.json) {
        ...
        runtime.error(`Inspect with ${formatCliCommand("mo config validate")}.`);
      }
      runtime.exit(1);
      return;
    }
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.mo.ai/cli/config")}\n`,
    )
    .option(
      "--section <section>",
  ...
  setCommandJsonMode(cmd.command("set"), "parse-only")
    ...
    .option(
      "--dry-run",
      "Validate changes without writing mo.json (checks run in builder/json/batch modes; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    ...
    .option(
      "--dry-run",
      "Validate changes without writing mo.json (checks schema and SecretRef resolvability; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    .command("schema")
    .description("Print the JSON schema for mo.json")
    .action(async () => {
