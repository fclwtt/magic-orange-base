...
describe("runCli exit behavior", () => {
  ...
  beforeEach(() => {
    delete process.env.MO_SERVICE_MARKER;
    delete process.env.MO_SERVICE_KIND;
    ...
    delete process.env.MO_DISABLE_CLI_STARTUP_HELP_FAST_PATH;
    delete process.env.MO_HIDE_BANNER;
  ...
  it("does not force process.exit after successful routed command", async () => {
    ...
    expect(tryRouteCliMock).toHaveBeenCalledWith(["node", "mo", "status"]);
  ...
  it("disposes registered harnesses after full CLI command completion", async () => {
    ...
    await runCli(["node", "mo", "agent", "--local"]);

    expect(parseAsync).toHaveBeenCalledWith(["node", "mo", "agent", "--local"]);
  ...
  it("suppresses startup progress for json output commands before full CLI parsing", async () => {
    ...
    await runCli(["node", "mo", "sessions", "--json", "--limit", "all"]);
  ...
  it("pauses non-tty stdin after full CLI command completion", async () => {
    ...
    try {
      await runCli(["node", "mo", "channels"]);

      expect(parseAsync).toHaveBeenCalledWith(["node", "mo", "channels"]);
  ...
  it("emits the startup banner before gateway foreground fast-path startup", async () => {
    await runCli(["node", "mo", "gateway", "--force"]);

    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(emitCliBannerMock).toHaveBeenCalledWith("9.9.9-test", {
      argv: ["node", "mo", "gateway", "--force"],
    });
  ...
  it.each([
    ...
    const previousMarker = process.env.MO_SERVICE_MARKER;
    const previousOverride = process.env.MO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS;
    if (params.marker) {
      process.env.MO_SERVICE_MARKER = params.marker;
    } else {
      delete process.env.MO_SERVICE_MARKER;
    }
    if (params.override) {
      process.env.MO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = params.override;
    } else {
      delete process.env.MO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS;
    }
    const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
    const exitSpy = vi.spyOn(process, "exit").mockImplementation(((code?: number) => {
      throw new Error(`exit:${String(code)}`);
    ...
    try {
      await expect(runCli(["node", "mo", "gateway", ...params.flags])).rejects.toThrow(
      ...
      if (params.marker) {
        expect(process.env.MO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS).toBeUndefined();
      }
    } finally {
      exitSpy.mockRestore();
      errorSpy.mockRestore();
      if (previousMarker === undefined) {
        ...
        process.env.MO_SERVICE_MARKER = previousMarker;
      }
      if (previousOverride === undefined) {
        delete process.env.MO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS;
      } else {
        process.env.MO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = previousOverride;
      }
    }
  });
  ...
  it("blocks and revokes the destructive override when selected config declares service mode", async () => {
    readConfigFileSnapshotMock.mockResolvedValue({
      ...
      valid: true,
      sourceConfig: {
        env: { vars: { MO_SERVICE_MARKER: "gateway" } },
    ...
    try {
      await withEnvAsync(
        {
          MO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS: "1",
          MO_SERVICE_MARKER: undefined,
        },
        async () => {
          await expect(runCli(["node", "mo", "gateway"])).rejects.toThrow("exit:78");
          expect(process.env.MO_SERVICE_MARKER).toBeUndefined();
          expect(process.env.MO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS).toBeUndefined();
          expect(ensureCliExecutionBootstrapMock).not.toHaveBeenCalled();
        },
      );
  ...
  it("ignores service mode declared by an invalid selected config", async () => {
    ...
    await withEnvAsync(
      ...
      async () => {
        ...
        expect(process.env.MO_SERVICE_MARKER).toBeUndefined();
        expect(process.env.MO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS).toBe("1");
      },
    );
  });

  it("guards the config selected by trusted global dotenv before the default config", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-gateway-global-selection-"));
    const stateDir = path.join(homeDir, ".mo");
    const selectedConfigPath = path.join(stateDir, "selected.json");
    await fs.mkdir(stateDir, { recursive: true });
    await fs.writeFile(
      path.join(stateDir, ".env"),
      [
        `MO_CONFIG_PATH=${selectedConfigPath}`,
        "MO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1",
        "",
      ].join("\n"),
    );
    try {
      await withEnvAsync(
        {
          HOME: homeDir,
          MO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS: undefined,
        ...
        async () => {
          readConfigFileSnapshotMock.mockImplementation(async () =>
            process.env.MO_CONFIG_PATH === selectedConfigPath
              ? {
                  exists: true,
                  valid: true,
                  sourceConfig: { gateway: { mode: "local" } },
          ...
          expect(process.env.MO_CONFIG_PATH).toBe(selectedConfigPath);
          expect(process.env.MO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS).toBeUndefined();
  ...
  it("loads state dotenv before a custom config-root fallback", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-gateway-config-env-"));
    const stateDir = path.join(homeDir, ".mo");
    const configDir = path.join(homeDir, "profile");
    const configPath = path.join(configDir, "mo.json");
    await fs.mkdir(stateDir, { recursive: true });
    await fs.mkdir(configDir, { recursive: true });
    await fs.writeFile(path.join(stateDir, ".env"), "MO_GATEWAY_TOKEN=state-token\n");
    await fs.writeFile(
      path.join(configDir, ".env"),
      [
        "MO_GATEWAY_PASSWORD=config-root-password",
        "MO_GATEWAY_TOKEN=config-root-token",
        "",
      ].join("\n"),
    );
    try {
      await withEnvAsync(
        {
          HOME: homeDir,
          MO_CONFIG_PATH: configPath,
          MO_GATEWAY_PASSWORD: undefined,
          MO_GATEWAY_TOKEN: undefined,
          MO_HOME: homeDir,
          MO_STATE_DIR: undefined,
        },
        async () => {
          ...
          expect(process.env.MO_GATEWAY_TOKEN).toBe("state-token");
          expect(process.env.MO_GATEWAY_PASSWORD).toBe("config-root-password");
        },
      );
    } finally {
      await fs.rm(homeDir, { recursive: true, force: true });
  ...
  it("loads and repins a legacy state dotenv after automatic state migration", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-gateway-legacy-env-"));
    const legacyStateDir = path.join(homeDir, ".clawdbot");
    const newStateDir = path.join(homeDir, ".mo");
    await fs.mkdir(legacyStateDir, { recursive: true });
    await fs.writeFile(path.join(legacyStateDir, ".env"), "MO_GATEWAY_TOKEN=legacy-token\n");
    try {
      await withEnvAsync(
        {
          HOME: homeDir,
        ...
        async () => {
          ...
          expect(process.env.MO_GATEWAY_TOKEN).toBe("legacy-token");
  ...
  it("blocks a future-config recovery candidate before destructive gateway reset", async () => {
    ...
    try {
      await runCli(["node", "mo", "gateway", "--dev", "--reset"]);
      const hooks = addGatewayRunCommandMock.mock.calls[0]?.[1] as
        | { beforeRun?: (opts: { reset?: boolean }) => Promise<void> }
        | undefined;
  ...
  it("re-guards config env path selection until the gateway config is stable", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-gateway-selection-"));
    try {
      await withEnvAsync(
        {
          HOME: homeDir,
        ...
        async () => {
          readConfigFileSnapshotMock.mockImplementation(async () => {
            if (process.env.MO_CONFIG_PATH === "/tmp/mo-chain-c.json") {
              return {
                exists: true,
                valid: true,
                sourceConfig: { meta: { lastTouchedVersion: "9999.1.1" } },
            ...
            if (process.env.MO_STATE_DIR === "/tmp/mo-chain-b") {
              return {
                exists: true,
                valid: true,
                sourceConfig: {
                  env: { vars: { MO_CONFIG_PATH: "/tmp/mo-chain-c.json" } },
                  gateway: { mode: "local" },
                },
              };
            }
            return {
              ...
              valid: true,
              sourceConfig: {
                env: { vars: { MO_STATE_DIR: "/tmp/mo-chain-b" } },
                gateway: { mode: "local" },
              },
            };
          });
          ...
          try {
            await expect(runCli(["node", "mo", "gateway"])).rejects.toThrow("exit:1");
            expect(errorSpy).toHaveBeenCalledWith(
              expect.stringContaining("run automatic gateway startup migrations"),
            );
            expect(ensureCliExecutionBootstrapMock).not.toHaveBeenCalled();
  ...
  it("drops credentials from configs superseded during state selection", async () => {
    await withEnvAsync(
      ...
      async () => {
        readConfigFileSnapshotMock.mockImplementation(async () =>
          process.env.MO_STATE_DIR === "/tmp/mo-selected-state"
            ? {
                exists: true,
                valid: true,
                sourceConfig: {
                  env: { vars: { MO_GATEWAY_TOKEN: "selected-token" } },
                  gateway: { mode: "local" },
                },
              }
            : {
                ...
                sourceConfig: {
                  env: {
                    vars: {
                      MO_GATEWAY_TOKEN: "superseded-token",
                      MO_STATE_DIR: "/tmp/mo-selected-state",
                    },
                  },
                  gateway: { mode: "local" },
                },
        ...
        expect(process.env.MO_STATE_DIR).toBe("/tmp/mo-selected-state");
        expect(process.env.MO_GATEWAY_TOKEN).toBe("selected-token");
  ...
  it("re-guards config selection from a newly selected state dotenv", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-gateway-dotenv-"));
    const futureConfigPath = path.join(stateDir, "future.json");
    await fs.writeFile(
      path.join(stateDir, ".env"),
      [
        `MO_CONFIG_PATH=${futureConfigPath}`,
        "MO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1",
        "",
      ].join("\n"),
    );
    try {
      await withEnvAsync(
        ...
        async () => {
          readConfigFileSnapshotMock.mockImplementation(async () => {
            if (process.env.MO_CONFIG_PATH === futureConfigPath) {
              return {
                exists: true,
                valid: true,
                sourceConfig: { meta: { lastTouchedVersion: "9999.1.1" } },
            ...
            return {
              ...
              valid: true,
              sourceConfig: {
                env: { vars: { MO_STATE_DIR: stateDir } },
                gateway: { mode: "local" },
              },
            };
          });
  ...
  it("re-inspects recovery after recovery changes config selection", async () => {
    await withEnvAsync({ MO_CONFIG_PATH: undefined }, async () => {
      const selectedConfigPath = "/tmp/mo-recovered-selection.json";
      const currentSnapshot = {
        exists: true,
        valid: true,
        sourceConfig: { gateway: { mode: "local" } },
      ...
      readConfigFileSnapshotMock.mockImplementation(async (options) => {
        ...
        if (recoveryReads === 1) {
          const recoveredSnapshot = {
            ...
            valid: true,
            sourceConfig: {
              env: { vars: { MO_CONFIG_PATH: selectedConfigPath } },
              gateway: { mode: "local" },
            },
          };
          await options.allowSuspiciousRecovery?.(
  ...
  it("discards env from a config replaced by suspicious recovery", async () => {
    await withEnvAsync(
      { MO_GATEWAY_TOKEN: undefined, MO_PROXY_ACTIVE: undefined },
      async () => {
        const clobberedSnapshot = {
          exists: true,
          valid: true,
          sourceConfig: {
            env: { vars: { MO_GATEWAY_TOKEN: "discarded-token" } },
            gateway: { mode: "local" },
          },
          hash: "clobbered",
          path: "/tmp/mo.json",
        ...
        process.env.MO_PROXY_ACTIVE = "1";
        const hooks = addGatewayRunCommandMock.mock.calls[0]?.[1] as
          | { beforeRun?: (opts: { force?: boolean }) => Promise<void> }
          | undefined;
        await hooks?.beforeRun?.({});

        expect(process.env.MO_GATEWAY_TOKEN).toBeUndefined();
        expect(process.env.MO_PROXY_ACTIVE).toBe("1");
        expect(ensureCliExecutionBootstrapMock).toHaveBeenCalledOnce();
      },
    );
  });
  ...
  it("does not apply environment variables from invalid config snapshots", async () => {
    await withEnvAsync({ MO_INCLUDE_ROOTS: undefined }, async () => {
      readConfigFileSnapshotMock.mockResolvedValue({
        exists: true,
        valid: false,
        sourceConfig: {
          env: { vars: { MO_INCLUDE_ROOTS: "/tmp/mo-includes" } },
          gateway: { mode: "local" },
        },
      });
      ...
      expect(process.env.MO_INCLUDE_ROOTS).toBeUndefined();
      expect(readConfigFileSnapshotMock.mock.calls).toEqual([
        [{ isolateEnv: true, observe: false }],
        [{ isolateEnv: true, observe: false }],
      ]);
  ...
  it("loads selected state dotenv before config env and environment normalization", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-gateway-selected-env-"));
        ...
        async () => {
          readConfigFileSnapshotMock.mockResolvedValue({
            ...
            sourceConfig: {
              env: {
                vars: {
                  MO_GATEWAY_TOKEN: "config-token",
                  MO_STATE_DIR: stateDir,
                },
              },
              gateway: { mode: "local" },
            },
          ...
          normalizeEnvMock.mockImplementation(() => {
            tokenAtNormalize = process.env.MO_GATEWAY_TOKEN;
          });
  ...
  it("drops credentials from a trusted dotenv superseded by state selection", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-gateway-dotenv-hop-"));
    const defaultStateDir = path.join(homeDir, ".mo");
    ...
    await fs.writeFile(
      ...
      "MO_GATEWAY_TOKEN=selected-token\n",
    );
    try {
      await withEnvAsync(
        {
        ...
        async () => {
          ...
          expect(process.env.MO_STATE_DIR).toBe(selectedStateDir);
          expect(process.env.MO_GATEWAY_TOKEN).toBe("selected-token");
        },
      );
    } finally {
  ...
  it("drops gateway.env selectors when the default state dotenv selects a custom state", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-gateway-fallback-hop-"));
    const defaultStateDir = path.join(homeDir, ".mo");
    const selectedStateDir = path.join(homeDir, "selected-state");
    const gatewayEnvDir = path.join(homeDir, ".config", "mo");
    await fs.mkdir(defaultStateDir, { recursive: true });
    await fs.mkdir(selectedStateDir, { recursive: true });
    ...
    await fs.writeFile(
      ...
      [
        "MO_CONFIG_PATH=/tmp/wrong-mo.json",
        "MO_GATEWAY_TOKEN=fallback-token",
        "",
      ].join("\n"),
    );
    await fs.writeFile(
      ...
      [
        "MO_GATEWAY_TOKEN=selected-token",
        "MO_INCLUDE_ROOTS=/tmp/untrusted-include-root",
    ...
    try {
      await withEnvAsync(
        ...
        async () => {
          ...
          expect(process.env.MO_STATE_DIR).toBe(selectedStateDir);
          expect(process.env.MO_CONFIG_PATH).toBeUndefined();
          expect(process.env.MO_GATEWAY_TOKEN).toBe("selected-token");
        },
      );
    } finally {
  ...
  it("preserves gateway.env selectors when the compatibility fallback selects the target", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-gateway-fallback-select-"));
    const selectedStateDir = path.join(homeDir, "selected-state");
    const gatewayEnvDir = path.join(homeDir, ".config", "mo");
    await fs.mkdir(selectedStateDir, { recursive: true });
    await fs.mkdir(gatewayEnvDir, { recursive: true });
    await fs.writeFile(
      path.join(gatewayEnvDir, "gateway.env"),
      [`MO_STATE_DIR=${selectedStateDir}`, "MO_GATEWAY_TOKEN=fallback-token", ""].join(
        "\n",
      ),
    );
    await fs.writeFile(
  ...
  it("drops old state dotenv credentials when config selects another state", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-gateway-config-state-hop-"));
    const defaultStateDir = path.join(homeDir, ".mo");
    const selectedStateDir = path.join(homeDir, "selected-state");
    await fs.mkdir(defaultStateDir, { recursive: true });
    await fs.mkdir(selectedStateDir, { recursive: true });
      ...
      await withEnvAsync(
        ...
        async () => {
          readConfigFileSnapshotMock.mockImplementation(async () => ({
            ...
            valid: true,
            sourceConfig:
              process.env.MO_STATE_DIR === selectedStateDir
                ? { gateway: { mode: "local" } }
                : {
                    env: { vars: { MO_STATE_DIR: selectedStateDir } },
                    gateway: { mode: "local" },
                  },
          }));
  ...
  it("drops early target credentials when a later guard selects another state", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-gateway-late-state-hop-"));
    const defaultStateDir = path.join(homeDir, ".mo");
    const selectedStateDir = path.join(homeDir, "selected-state");
    await fs.mkdir(defaultStateDir, { recursive: true });
    await fs.mkdir(selectedStateDir, { recursive: true });
    await fs.writeFile(path.join(defaultStateDir, ".env"), "MO_GATEWAY_TOKEN=early-token\n");
    await fs.writeFile(
      path.join(selectedStateDir, ".env"),
      "MO_GATEWAY_TOKEN=selected-token\n",
    );
      ...
      await withEnvAsync(
        ...
        async () => {
          ...
          readConfigFileSnapshotMock.mockImplementation(async () => ({
            ...
            valid: true,
            sourceConfig:
              selectLateState && process.env.MO_STATE_DIR !== selectedStateDir
                ? {
                    env: { vars: { MO_STATE_DIR: selectedStateDir } },
                    gateway: { mode: "local" },
                  }
          ...
          await runCli(["node", "mo", "gateway"]);
          expect(process.env.MO_GATEWAY_TOKEN).toBe("early-token");
  ...
  it("does not let gateway.env authorize automatic mutations of a selected future config", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-gateway-global-env-"));
    const gatewayEnvDir = path.join(homeDir, ".config", "mo");
    ...
    try {
      await withEnvAsync(
        ...
        async () => {
          readConfigFileSnapshotMock.mockImplementation(async () =>
            process.env.MO_CONFIG_PATH === futureConfigPath
              ? {
                  exists: true,
                  valid: true,
                  sourceConfig: { meta: { lastTouchedVersion: "9999.1.1" } },
              ...
              : {
                  ...
                  valid: true,
                  sourceConfig: {
                    env: { vars: { MO_CONFIG_PATH: futureConfigPath } },
                    gateway: { mode: "local" },
                  },
                },
          );
  ...
  it("does not treat gateway option values as bootstrap command paths", async () => {
    await runCli(["node", "mo", "gateway", "--raw-stream-path", "status"]);
  ...
  it("retains selected config paths and invocation reset targets", async () => {
    await withEnvAsync(
      {
        MO_CONFIG_PATH: "/tmp/mo-invocation/mo.json",
        MO_GATEWAY_TOKEN: undefined,
        MO_HOME: "/tmp/mo-invocation-home",
        MO_INCLUDE_ROOTS: undefined,
        MO_PROFILE: undefined,
        MO_STATE_DIR: "/tmp/mo-invocation-state",
        MO_TEST_FAST: "1",
        MO_WORKSPACE_DIR: "/tmp/mo-invocation-workspace",
      },
      async () => {
        readConfigFileSnapshotMock.mockResolvedValue({
          exists: true,
          ...
          sourceConfig: {
            env: {
              vars: {
                MO_CONFIG_PATH: "/tmp/mo-reset/mo.json",
                MO_GATEWAY_TOKEN: "old-token",
                MO_HOME: "/tmp/mo-reset-home",
                MO_INCLUDE_ROOTS: "/tmp/mo-reset-includes",
                MO_PROFILE: "config-dev",
                MO_STATE_DIR: "/tmp/mo-reset",
                MO_TEST_FAST: "0",
                MO_WORKSPACE_DIR: "/tmp/mo-reset-workspace",
              },
            },
            gateway: { mode: "local" },
          },
        ...
        expect(process.env.MO_CONFIG_PATH).toBe("/tmp/mo-invocation/mo.json");
        expect(process.env.MO_HOME).toBe("/tmp/mo-invocation-home");
        expect(process.env.MO_PROFILE).toBeUndefined();
        expect(process.env.MO_STATE_DIR).toBe("/tmp/mo-invocation-state");
        expect(process.env.MO_TEST_FAST).toBe("1");
        expect(process.env.MO_WORKSPACE_DIR).toBe("/tmp/mo-invocation-workspace");
        expect(process.env.MO_GATEWAY_TOKEN).toBeUndefined();
        expect(process.env.MO_INCLUDE_ROOTS).toBeUndefined();
        expect(ensureCliExecutionBootstrapMock).not.toHaveBeenCalled();
      },
  ...
  it("does not let config env authorize or retarget an explicit reset", async () => {
    await withEnvAsync(
      { MO_PROFILE: undefined, MO_WORKSPACE_DIR: undefined },
      async () => {
        readConfigFileSnapshotMock.mockResolvedValue({
          exists: true,
          valid: true,
          sourceConfig: {
            env: {
              vars: {
                MO_PROFILE: "dev",
                MO_WORKSPACE_DIR: "/tmp/mo-config-workspace",
              },
            },
            gateway: { mode: "local" },
          },
        ...
        await runCli(["node", "mo", "gateway", "--reset"]);

        expect(process.env.MO_PROFILE).toBeUndefined();
        expect(process.env.MO_WORKSPACE_DIR).toBeUndefined();
      },
    );
  });

  it("honors banner suppression on the gateway foreground fast path", async () => {
    process.env.MO_HIDE_BANNER = "1";

    await runCli(["node", "mo", "gateway"]);

    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(emitCliBannerMock).not.toHaveBeenCalled();
    expect(commanderParseAsyncMock).toHaveBeenCalledWith(["node", "mo", "gateway"]);
  });
  ...
  it("renders browser help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "mo", "browser", "--help"]);
  ...
  it("renders secrets help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "mo", "secrets", "--help"]);
  ...
  it("renders nodes help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "mo", "nodes", "--help"]);
  ...
  it("renders selected subcommand help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "mo", "doctor", "--help"]);
  ...
  it("renders setup/onboard/configure help without building the full program", async () => {
    await runCli(["node", "mo", "setup", "--help"]);
  ...
  it("renders root help without building the full program", async () => {
    ...
    expect(maybeRunCliInContainerMock).toHaveBeenCalledWith(["node", "mo", "--help"]);
    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(loadRootHelpRenderOptionsForConfigSensitivePluginsMock).toHaveBeenCalledTimes(1);
    expect(outputPrecomputedRootHelpTextMock).toHaveBeenCalledTimes(1);
  ...
  it.each([
    ["gateway runtime", ["node", "mo", "gateway", "run"]],
    ["bare gateway runtime", ["node", "mo", "gateway"]],
    ["node runtime", ["node", "mo", "node", "run"]],
    ["local agent runtime", ["node", "mo", "agent", "--local"]],
    ["provider inference", ["node", "mo", "infer", "web", "fetch", "https://example.com"]],
    ["model command", ["node", "mo", "models", "auth", "login", "openai"]],
    ["plugin command", ["node", "mo", "plugins", "marketplace", "list"]],
    ["skill command", ["node", "mo", "skills", "search", "browser"]],
    ["update command", ["node", "mo", "update", "check"]],
    ["channel probe", ["node", "mo", "channels", "status", "--probe"]],
    ["channel capabilities probe", ["node", "mo", "channels", "capabilities"]],
    ["directory plugin command", ["node", "mo", "directory", "peers", "list"]],
    ["message plugin command", ["node", "mo", "message", "send", "--to", "demo"]],
    ["metadata-owned plugin command", ["node", "mo", "googlemeet", "login"]],
  ])("starts managed proxy routing for %s", (_name, argv) => {
    expect(shouldStartProxyForCli(argv)).toBe(true);
  });

  it.each([
    ["root help", ["node", "mo", "--help"]],
    ["root version", ["node", "mo", "--version"]],
    ["gateway help", ["node", "mo", "gateway", "--help"]],
    ["gateway run help", ["node", "mo", "gateway", "run", "--help"]],
    ["status", ["node", "mo", "status"]],
    ["health", ["node", "mo", "health"]],
    ["gateway status", ["node", "mo", "gateway", "status"]],
    ["gateway health", ["node", "mo", "gateway", "health"]],
    ["remote agent control-plane", ["node", "mo", "agent", "run"]],
    ["chat control-plane", ["node", "mo", "chat"]],
    ["terminal control-plane", ["node", "mo", "terminal"]],
    ["config", ["node", "mo", "config", "get", "proxy.enabled"]],
    ["channels parent help", ["node", "mo", "channels"]],
    ["completion", ["node", "mo", "completion", "zsh"]],
    ["debug proxy cli", ["node", "mo", "proxy", "start"]],
    ["agents list", ["node", "mo", "agents", "list"]],
    ["models list", ["node", "mo", "models", "list"]],
    ["models status without live probe", ["node", "mo", "models", "status"]],
    ["skills check", ["node", "mo", "skills", "check"]],
    ["skills info", ["node", "mo", "skills", "info", "weather"]],
    ["skills list", ["node", "mo", "skills", "list"]],
    ["tasks list", ["node", "mo", "tasks", "list"]],
    ["legacy singular tool namespace", ["node", "mo", "tool", "image_generate"]],
    ["gateway tools namespace typo", ["node", "mo", "tools", "effective"]],
    ["migrate", ["node", "mo", "migrate"]],
  ...
  it("starts the managed proxy for network-capable commands by default", async () => {
    ...
    await runCli(["node", "mo", "plugins", "marketplace", "list"]);

    expect(startProxyMock).toHaveBeenCalledWith(undefined);
  });

  it.each([
    ["fast path", ["node", "mo", "gateway", "run"]],
    [
      "full Commander path with root options",
      ["node", "mo", "--log-level", "debug", "gateway", "run"],
    ],
  ...
  it("validates the runtime before selecting gateway config", async () => {
    await runCli(["node", "mo", "gateway", "run"]);
  ...
  it("selects gateway config env before starting its managed proxy", async () => {
    await withEnvAsync({ MO_TEST_PROXY_SELECTION: undefined }, async () => {
      readConfigFileSnapshotMock.mockResolvedValue({
        exists: true,
        valid: true,
        sourceConfig: {
          env: { vars: { MO_TEST_PROXY_SELECTION: "selected" } },
          gateway: { mode: "local" },
        },
      });
      loadConfigMock.mockImplementationOnce(() => ({
        proxy: { selected: process.env.MO_TEST_PROXY_SELECTION },
      }));
  ...
  it("starts the managed proxy for metadata-owned plugin commands by default", async () => {
    ...
    await runCli(["node", "mo", "googlemeet", "login"]);

    expect(startProxyMock).toHaveBeenCalledWith(undefined);
  });

  it("rejects unowned command roots before proxy and plugin runtime registration", async () => {
    await expect(runCli(["node", "mo", "foo"])).rejects.toThrow(
  ...
  it("suggests close known commands for unowned command roots before proxy startup", async () => {
    await expect(runCli(["node", "mo", "upate"])).rejects.toThrow(
      "Did you mean this?\n  mo update",
    );
  ...
  it("rejects unowned command roots even when --help is appended (regression for #81077)", async () => {
    await expect(runCli(["node", "mo", "foo", "--help"])).rejects.toThrow(
      'No built-in command or plugin CLI metadata owns "foo"',
    );
  ...
  it("rejects unowned command roots even when --version is appended", async () => {
    await expect(runCli(["node", "mo", "foo", "--version"])).rejects.toThrow(
      'No built-in command or plugin CLI metadata owns "foo"',
    );
  ...
  it("does not suggest plugins.allow for unknown command roots before proxy startup", async () => {
    ...
    let error: unknown;
    try {
      await runCli(["node", "mo", "totally-unknown"]);
    } catch (caught) {
      error = caught;
    }
    expect(error).toBeInstanceOf(Error);
  ...
  it("preserves plugins.allow diagnostics for roots owned only by CLI metadata", async () => {
    ...
    await expect(runCli(["node", "mo", "qa"])).rejects.toThrow(
  ...
  it("reports plugin tool command mistakes before proxy startup", async () => {
    ...
    await expect(runCli(["node", "mo", "lcm_recent"])).rejects.toThrow(
  ...
  it("does not install the env proxy dispatcher for bypassed skills inspection commands", async () => {
    ...
    await runCli(["node", "mo", "skills", "check"]);
  ...
  it.each([
    ["auth", ["node", "mo", "auth", "--help"]],
    ["tool", ["node", "mo", "tool", "image_generate"]],
    ["tools", ["node", "mo", "tools", "effective"]],
  ...
  it("routes lazy plugin registration logs to stderr only during --json registration", async () => {
    ...
    await runCli(["node", "mo", "memory", "search", "query", "--json"]);
  ...
  it("does not route lazy plugin registration logs for pass-through --json after terminator", async () => {
    ...
    await runCli(["node", "mo", "memory", "--", "--json"]);
  ...
  it("fails protected commands when managed proxy activation fails", async () => {
    ...
    await expect(runCli(["node", "mo", "gateway", "run"])).rejects.toThrow(
  ...
  it("stops the managed proxy and exits after SIGINT", async () => {
    ...
    try {
      const runPromise = runCli(["node", "mo", "plugins", "marketplace", "list"]);
      await vi.waitFor(() => {
        expect(
  ...
  it("starts onboarding for bare root invocations when config only has metadata", async () => {
    readConfigFileSnapshotMock.mockResolvedValueOnce({
      ...
      valid: true,
      sourceConfig: {
        $schema: "https://mo.ai/config.json",
  ...
  it("points noninteractive fresh bare root invocations to onboarding automation", async () => {
    ...
    try {
      ...
      expect(errorSpy).toHaveBeenCalledWith(
        "Onboarding needs an interactive TTY. Use `mo onboard --non-interactive --accept-risk ...` for automation.",
      );
  ...
  it("starts the gateway-backed TUI for bare root invocations when config already exists", async () => {
    ...
    await withEnvAsync({ MO_GATEWAY_PASSWORD: "gateway-ref-password" }, async () => {
      await withInteractiveTty(async () => {
        await runCli(["node", "mo"]);
      });
    });
  ...
  it("uses gateway env credentials for bare root gateway preflight", async () => {
    ...
    await withEnvAsync({ MO_GATEWAY_TOKEN: "env-token" }, async () => {
      await withInteractiveTty(async () => {
        await runCli(["node", "mo"]);
      });
    });
  ...
  it("does not probe unsafe remote gateway URLs with configured credentials", async () => {
    ...
    await withEnvAsync({ MO_ALLOW_INSECURE_PRIVATE_WS: undefined }, async () => {
      await withInteractiveTty(async () => {
        await runCli(["node", "mo"]);
      });
    });
  ...
  it("rejects configured bare root TUI startup without an interactive TTY", async () => {
    ...
    try {
      ...
      expect(process.exitCode).toBe(1);
      expect(errorSpy).toHaveBeenCalledWith(
        "OpenClaw TUI needs an interactive TTY. Use `mo agent --local ...` for automation.",
      );
  ...
  it("bootstraps env proxy before modern onboard Crestodian startup", async () => {
    ...
    await runCli(["node", "mo", "onboard", "--modern", "--json"]);
  ...
  it("does not fail the command when memory cleanup is unavailable", async () => {
    ...
    await expect(runCli(["node", "mo", "status"])).resolves.toBeUndefined();
  ...
  it("returns after a handled container-target invocation", async () => {
    ...
    await runCli(["node", "mo", "--container", "demo", "status"]);
  ...
  it("loads the real primary command before rendering command help", async () => {
    ...
    expect(registerCoreCliByNameMock.mock.calls).toEqual([
      [program, ctx, "doctor", ["node", "mo", "doctor", "--help"]],
    ]);
    expect(registerSubCliByNameMock.mock.calls).toEqual([
      [program, "doctor", ["node", "mo", "doctor", "--help"]],
    ]);
  });
  ...
  it("restores terminal state before uncaught CLI exits", async () => {
    ...
    try {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        "[mo] OpenClaw hit an unexpected runtime error.",
      );
      expect(consoleErrorSpy).toHaveBeenCalledWith("[mo] Reason: boom");
  ...
  it("does not exit for transient uncaught CLI exceptions", async () => {
    ...
    try {
      ...
      expect(consoleWarnSpy.mock.calls).toEqual([
        ["[mo] Non-fatal uncaught exception (continuing):", hostUnreachable.stack],
      ]);
