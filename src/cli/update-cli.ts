// Commander wiring for `mo update`, its status/finalize subcommands, and help text.
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mo update repair", "Rerun post-update doctor and plugin convergence."],
          ["mo update repair --channel beta", "Repair against the beta update channel."],
          ["mo update repair --json", "JSON output for automation."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          ...
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.mo.ai/cli/update")}`,
    )
...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    ...
    .addHelpText("after", () => {
      const examples = [
        ["mo update", "Update a source checkout (git)"],
        ["mo update --channel beta", "Switch to beta channel (git + npm)"],
        ["mo update --channel dev", "Switch to dev channel (git + npm)"],
        ["mo update --tag beta", "One-off update to a dist-tag or version"],
        ["mo update --tag main", "One-off package update from GitHub main"],
        ["mo update --dry-run", "Preview actions without changing anything"],
        ["mo update --no-restart", "Update without restarting the service"],
        ["mo update --json", "Output result as JSON"],
        ["mo update --yes", "Non-interactive (accept downgrade prompts)"],
        ["mo update repair", "Repair stranded post-update plugin state"],
        ["mo update --acknowledge-clawhub-risk", "Acknowledge ClawHub plugin trust warnings"],
        ["mo update wizard", "Interactive update wizard"],
        ["mo --update", "Shorthand for mo update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run mo update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.mo.ai/cli/update")}`;
    })
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mo update status", "Show channel + version status."],
          ["mo update status --json", "JSON output."],
          ["mo update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
