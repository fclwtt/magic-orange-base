...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mo status", "Show channel health + session summary."],
          ["mo status --all", "Full diagnosis (read-only)."],
          ["mo status --json", "Machine-readable output."],
          ["mo status --usage", "Show model provider usage/quota snapshots."],
          [
            "mo status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["mo status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.mo.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.mo.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
  ...
  const sessionsCmd = addSessionsListOptions(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mo sessions", "List all sessions."],
          ["mo sessions --agent work", "List sessions for one agent."],
          ["mo sessions --all-agents", "Aggregate sessions across agents."],
          ["mo sessions --active 120", "Only last 2 hours."],
          ["mo sessions --limit 25", "Show the newest 25 sessions."],
          ["mo sessions --json", "Machine-readable output."],
          ["mo sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.mo.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mo sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "mo sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          [
            "mo sessions cleanup --dry-run --fix-dm-scope",
            "Preview stale direct-DM rows after returning dmScope to main.",
          ],
          ["mo sessions cleanup --enforce", "Apply maintenance now."],
          ["mo sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["mo sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "mo sessions cleanup --enforce --store ./tmp/sessions.json",
  ...
  sessionsCmd
    ...
    .option("--output <path>", "Output directory name inside .mo/trajectory-exports")
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            'mo sessions compact "agent:main:main"',
            "LLM-summarize a session to reclaim context budget.",
          ],
          [
            'mo sessions compact "agent:main:main" --max-lines 200',
            "Truncate to the last 200 transcript lines instead.",
          ],
          [
            'mo sessions compact "agent:work:main" --agent work --json',
    ...
    .action(async (key: string, opts, command) => {
      ...
      // command, e.g. `mo sessions --agent work compact <key>`. Merge those
  ...
  const commitmentsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mo commitments", "List pending inferred follow-ups."],
          ["mo commitments --all", "List all inferred follow-ups."],
          ["mo commitments --agent work", "List one agent's inferred follow-ups."],
          ["mo commitments dismiss cm_abc123", "Dismiss a follow-up."],
        ])}`,
    )
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
