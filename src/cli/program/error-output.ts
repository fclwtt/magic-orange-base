...
function resolveHelpCommand(argv: string[] | undefined, options?: { root?: boolean }): string {
  if (options?.root || !argv) {
    return formatCliCommand("mo --help");
  }
  const commandPath = getCommandPathWithRootOptions(argv, 2);
  if (commandPath.length === 0) {
    return formatCliCommand("mo --help");
  }
  return formatCliCommand(`mo ${commandPath.join(" ")} --help`);
}
...
function formatDocsHint(): string {
  return `${theme.muted("Docs:")} ${formatDocsLink("/cli", "docs.mo.ai/cli")}`;
}
...
export function formatCliParseErrorOutput(
  ...
  if (unknownCommand) {
    ...
    return lines(
      ...
      `${theme.muted("Plugin command?")} ${theme.command(formatCliCommand("mo plugins list"))}`,
      formatDocsHint(),
    );
  }
