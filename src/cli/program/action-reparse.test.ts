...
describe("reparseProgramFromActionArgs", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    buildParseArgvMock.mockReturnValue(["node", "mo", "status"]);
  ...
  it("uses action command name + args as fallback argv", async () => {
    ...
    setRawArgs(program, ["node", "mo", "status", "--json"]);
    ...
    expect(buildParseArgvMock).toHaveBeenCalledWith({
      programName: "mo",
      rawArgs: ["node", "mo", "status", "--json"],
      fallbackArgv: ["status", "--json"],
    });
    expect(parseAsync).toHaveBeenCalledWith(["node", "mo", "status"]);
  });

  it("falls back to action args without command name when action has no name", async () => {
    const program = new Command().name("mo");
    setRawArgs(program, ["node", "mo"]);
    const parseAsync = vi.spyOn(program, "parseAsync").mockResolvedValue(program);
    const actionCommand = {
      name: () => "",
      parent: program,
  ...
  it("uses root raw args and reparses the root for nested lazy commands", async () => {
    const root = new Command().name("mo");
    setRawArgs(root, ["node", "mo", "workspaces", "audit", "export", "--since", "1"]);
    ...
    expect(buildParseArgvMock).toHaveBeenCalledWith({
      programName: "mo",
      rawArgs: ["node", "mo", "workspaces", "audit", "export", "--since", "1"],
