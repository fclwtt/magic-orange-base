...
/** Parse `mo health` flags for the route-first status family. */
...
/** Parse `mo status` flags without registering the full command tree. */
...
/** Parse `mo gateway status` RPC-only flags accepted by the fast route. */
...
/** Parse `mo sessions` filters for JSON/list route execution. */
...
/** Parse `mo agents list` display switches for route-first execution. */
...
/** Parse `mo config get <path>` while preserving root option handling. */
...
/** Parse `mo config unset <path>` and its mutation guard flags. */
...
/** Parse `mo models list` filters for the lightweight model catalog route. */
...
/** Parse `mo models status` probe controls for the route-first status path. */
...
/** Parse `mo channels list` display flags for the route-first list path. */
...
/** Parse `mo channels status` probe flags without full CLI registration. */
...
/** Parse JSON-only `mo plugins list` flags for plugin inventory output. */
...
/** Parse both `mo tasks --json` and `mo tasks list --json` aliases. */
...
/** Parse JSON-only `mo tasks audit` filters for the route-first audit path. */
