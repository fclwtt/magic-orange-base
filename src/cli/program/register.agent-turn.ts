...
/** Register `mo agent` for one Gateway-backed agent turn. */
...
${formatHelpExamples([
  ['mo agent --to +15555550123 --message "status update"', "Start a new session."],
  ['mo agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  ["mo agent --agent ops --message-file ./task.md", "Read a multiline message file."],
  [
    'mo agent --session-key agent:ops:incident-42 --message "Summarize status"',
    "Target an exact session key.",
  ],
  [
    'mo agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'mo agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['mo agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'mo agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.mo.ai/cli/agent")}`,
    )
