...
export function registerAgentsCommands(program: Command): void {
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.mo.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ['mo agents set-identity --agent main --name "OpenClaw" --emoji "🦞"', "Set name + emoji."],
  ["mo agents set-identity --agent main --avatar avatars/mo.png", "Set avatar path."],
  [
    "mo agents set-identity --workspace ~/.mo/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "mo agents set-identity --identity-file ~/.mo/workspace/IDENTITY.md --agent main",
