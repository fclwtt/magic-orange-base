...
export function registerBackupCommand(program: Command) {
  const backup = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/backup", "docs.mo.ai/cli/backup")}\n`,
    );
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mo backup create", "Create a timestamped backup in the current directory."],
          [
            "mo backup create --output ~/Backups",
            "Write the archive into an existing backup directory.",
          ],
          [
            "mo backup create --dry-run --json",
            "Preview the archive plan without writing any files.",
          ],
          [
            "mo backup create --verify",
            "Create the archive and immediately validate its manifest and payload layout.",
          ],
          [
            "mo backup create --no-include-workspace",
            "Back up state/config without agent workspace files.",
          ],
          ["mo backup create --only-config", "Back up only the active JSON config file."],
        ])}`,
    )
    .action(async (opts) => {
  ...
  backup
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "mo backup verify ./2026-03-09T08-00-00.000+08-00-mo-backup.tar.gz",
            "Check that the archive structure and manifest are intact.",
          ],
          [
            "mo backup verify ~/Backups/latest.tar.gz --json",
