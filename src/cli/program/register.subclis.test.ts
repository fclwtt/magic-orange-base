...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.MO_DISABLE_LAZY_SUBCOMMANDS;
  const originalEnablePrivateQaCli = process.env.MO_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.MO_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.MO_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
    process.env.MO_ENABLE_PRIVATE_QA_CLI = "1";
  ...
  afterEach(() => {
    ...
    if (originalEnablePrivateQaCli === undefined) {
      delete process.env.MO_ENABLE_PRIVATE_QA_CLI;
    } else {
      process.env.MO_ENABLE_PRIVATE_QA_CLI = originalEnablePrivateQaCli;
    }
  });

  it("registers the primary placeholder plus completion and dispatches", async () => {
    const program = createRegisteredProgram(["node", "mo", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "mo"]);
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "mo", "nodes", "list"], "mo");
  ...
  it("registers the infer placeholder and dispatches through the capability registrar", async () => {
    const program = createRegisteredProgram(["node", "mo", "infer"], "mo");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "mo", "acp", "--help"], "mo");
  ...
  it("registers only the gateway run surface for gateway startup", async () => {
    const argv = ["node", "mo", "gateway", "--force"];
    process.argv = argv;
    const program = new Command().name("mo");
  ...
  it("keeps the full gateway CLI for non-run gateway subcommands", async () => {
    const argv = ["node", "mo", "gateway", "call", "health"];
    process.argv = argv;
    const program = new Command().name("mo");
  ...
  it("passes completion context to channel registration", async () => {
    const argv = ["node", "mo", "completion", "--write-state"];
    const program = new Command().name("mo");
