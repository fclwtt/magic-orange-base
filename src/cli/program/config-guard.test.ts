...
describe("ensureConfigReady", () => {
  ...
  function useTempOpenClawHome(): string {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "mo-config-guard-"));
    tempRoots.push(root);
    setTestEnvValue("MO_HOME", root);
    deleteTestEnvValue("MO_STATE_DIR");
    return root;
  }

  function writeLegacyTaskSidecarMarker(root: string): void {
    const markerPath = path.join(root, ".mo", "tasks", "runs.sqlite");
  ...
  function writeStateMarker(root: string, relativePath: string): void {
    const markerPath = path.join(root, ".mo", relativePath);
    fs.mkdirSync(path.dirname(markerPath), { recursive: true });
    fs.writeFileSync(markerPath, "{}");
  }
  ...
  it("runs doctor flow for legacy sessions without task sidecars", async () => {
    const root = useTempOpenClawHome();
    fs.mkdirSync(path.join(root, ".mo", "sessions"), { recursive: true });
  ...
  it("uses shared tilde expansion for MO_HOME in the startup detector", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "mo-config-guard-home-"));
    tempRoots.push(root);
    setTestEnvValue("HOME", root);
    setTestEnvValue("MO_HOME", "~/svc");
    deleteTestEnvValue("MO_STATE_DIR");
  ...
  it("exits for invalid config on non-allowlisted commands", async () => {
    ...
    expect(plainErrorCalls(runtime)).toEqual([
      ...
      "",
      `Fix: ${formatCliCommand("mo doctor --fix")}`,
      `Inspect: ${formatCliCommand("mo config validate")}`,
  ...
  it("replaces doctor fix advice for plugin packaging-only invalid config", async () => {
    ...
    expect(calls).not.toContain(`Fix: ${formatCliCommand("mo doctor --fix")}`);
    expect(runtime.exit).toHaveBeenCalledWith(1);
  });
