...
const EXAMPLES = [
  ["mo onboard", "Run guided setup for a local Gateway, workspace, auth, and channels."],
  ["mo setup", "Create the baseline config, workspace, and session folders."],
  ["mo configure", "Change models, Gateway, channels, plugins, skills, and health checks."],
  ["mo status", "Check Gateway, channel, model, and recent-session status."],
  ["mo doctor --fix", "Repair common config, service, plugin, and channel problems."],
  ["mo channels add", "Add or update a chat channel account with guided prompts."],
  ["mo channels status", "See connected messaging accounts and login state."],
  ["mo --dev gateway", "Run a dev Gateway (isolated state/config) on ws://127.0.0.1:19001."],
  ["mo gateway run --force", "Start the Gateway and replace anything bound to its port."],
  ["mo models status", "Show model/provider auth health before running agents."],
  ["mo plugins list", "Inspect enabled, disabled, and installed plugins."],
  [
    'mo agent --to +15555550123 --message "Run summary" --deliver',
    "Run one agent turn through the Gateway and optionally deliver the reply.",
  ],
  [
    'mo message send --channel telegram --target @mychat --message "Hi"',
...
export function configureProgramHelp(
  ...
  program
    ...
    .option(
      "--container <name>",
      "Run the CLI inside a running Podman/Docker container named <name> (default: env MO_CONTAINER)",
    )
    .option(
      "--dev",
      "Dev profile: isolate state under ~/.mo-dev, default gateway port 19001, and shift derived ports (browser/canvas)",
    )
    .option(
      "--profile <name>",
      "Use a named profile (isolates MO_STATE_DIR/MO_CONFIG_PATH under ~/.mo-<name>)",
    )
    .option(
      "--log-level <level>",
  ...
  program.addHelpText("beforeAll", () => {
    if (hasEmittedCliBanner() || process.env.MO_SUPPRESS_HELP_BANNER === "1") {
      return "";
    }
    const rich = isRich();
  ...
  program.addHelpText("afterAll", ({ command }) => {
    ...
    const docs = formatDocsLink("/cli", "docs.mo.ai/cli");
