...
function buildNodeApproveCommand(opts: DevicesRpcOpts, requestId: string): string {
  const args = ["mo", "nodes", "approve", requestId];
...
function buildFallbackStateMismatchError(details: ConnectPairingRequiredDetails): Error {
  return new Error(
    [
      ...
      "The running gateway is probably using a different MO_PROFILE or MO_STATE_DIR than this CLI.",
...
function buildExplicitApproveCommand(opts: DevicesRpcOpts, requestId: string): string {
  const args = ["mo", "devices", "approve", requestId];
...
function resolveRequiredDeviceRole(
  ...
  defaultRuntime.error(
    `--device and --role are required. Run ${formatCliCommand("mo devices list")} to choose a paired device.`,
  );
...
export async function runDevicesRemoveCommand(
  ...
  if (!trimmed) {
    defaultRuntime.error(
      `deviceId is required. Run ${formatCliCommand("mo devices list")} to choose a paired device.`,
    );
    defaultRuntime.exit(1);
    return;
  }
...
export async function runDevicesApproveCommand(
  ...
  if (!result) {
    defaultRuntime.error(
      `No pending device request matches ${sanitizeForLog(resolvedRequestId)}. Run ${formatCliCommand("mo devices list")} and retry with the current request ID.`,
    );
