...
function shouldShowStack(argv: string[] | undefined, env: NodeJS.ProcessEnv): boolean {
  return hasDebugArg(argv) || isTruthyEnvValue(env.MO_DEBUG);
}
...
export function formatCliFailureLines(options: FormatCliFailureOptions): string[] {
  ...
  const lines = [
    `[mo] ${options.title}`,
    `[mo] Reason: ${formatErrorMessage(options.error)}`,
  ];
  ...
  if (shouldShowStack(options.argv, env)) {
    ...
    lines.push("[mo] Debug: set MO_DEBUG=1 to include the stack trace.");
  }

  if (options.includeDoctorHint !== false) {
    lines.push(`[mo] Try: ${formatCliCommand("mo doctor", env)}`);
  }
  lines.push(`[mo] Help: ${formatCliCommand("mo --help", env)}`);
  return lines;
}
