...
async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("mo-cli-mcp-home-", async (home) => {
  ...
  it("adds a configured MCP server from flags without replacing operator knobs", async () => {
    await withTempHome("mo-cli-mcp-home-", async () => {
      const workspaceDir = await createWorkspace();
      vi.spyOn(process, "cwd").mockReturnValue(workspaceDir);
  ...
  it("fails named probes for disabled MCP servers", async () => {
    await withTempHome("mo-cli-mcp-home-", async (home) => {
      ...
      expect(lastErrorLine()).toBe(
        `MCP server "docs" is disabled in ${configPath}. Run mo mcp configure docs --enable before probing it.`,
      );
    });
  });
  ...
  it("fails when removing an unknown MCP server", async () => {
    await withTempHome("mo-cli-mcp-home-", async (home) => {
      ...
      expect(lastErrorLine()).toBe(
        `No MCP server named "missing" in ${configPath}. Run mo mcp list to see configured servers.`,
      );
    });
  });
