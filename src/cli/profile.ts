...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.mo${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  const existingStateDir = normalizeOptionalString(env.MO_STATE_DIR);
  const stateDir = existingStateDir || resolveProfileStateDir(profile, env, homedir);
  if (!existingStateDir) {
    env.MO_STATE_DIR = stateDir;
  }

  if (!normalizeOptionalString(env.MO_CONFIG_PATH)) {
    env.MO_CONFIG_PATH = path.join(stateDir, "mo.json");
  }

  if (profile === "dev" && !env.MO_GATEWAY_PORT?.trim()) {
    env.MO_GATEWAY_PORT = "19001";
  }
}
