...
import type { ConfigFileSnapshot, OpenClawConfig } from "../config/types.mo.js";
...
async function resolveGatewayProbeAuth(
  ...
  const token = configToken ?? normalizeOptionalString(process.env.MO_GATEWAY_TOKEN);
  const password = configPassword ?? normalizeOptionalString(process.env.MO_GATEWAY_PASSWORD);
  if (token) {
...
function isSafeGatewayProbeTarget(target: GatewayProbeTarget): boolean {
  ...
  return isSecureWebSocketUrl(target.url, {
    allowPrivateWs: process.env.MO_ALLOW_INSECURE_PRIVATE_WS === "1",
  });
}
...
function shouldBootstrapCliProxyBeforeFastPath(env: NodeJS.ProcessEnv = process.env): boolean {
  if (
    isTruthyEnvValue(env.MO_DEBUG_PROXY_ENABLED) ||
    isTruthyEnvValue(env.MO_DEBUG_PROXY_REQUIRE)
  ) {
    return true;
  }
...
async function resolveUnownedCliPrimaryMessage(params: {
  ...
  return [
    `Unknown command: mo ${params.primary}. No built-in command or plugin CLI metadata owns "${params.primary}".`,
    suggestion,
  ]
    .filter(Boolean)
    .join("\n");
...
export async function runCli(argv: string[] = process.argv) {
  ...
  const containerTargetName =
    parsedContainer.container ?? normalizeOptionalString(process.env.MO_CONTAINER) ?? null;
  ...
  try {
    ...
    // `mo <typo> --help` surfaces the same Unknown command error as
    // `mo <typo>` instead of silently showing generic top-level help.
    ...
    if (bareRootLaunchTarget) {
      ...
      if (!process.stdin.isTTY || !process.stdout.isTTY) {
        console.error(
          'Crestodian needs an interactive TTY. Use `mo crestodian --message "status"` for one command.',
        );
        process.exitCode = 1;
        return;
      }
    ...
    try {
      ...
      process.on("uncaughtException", (error) => {
        ...
        if (isBenignUncaughtExceptionError(error)) {
          console.warn(
            "[mo] Non-fatal uncaught exception (continuing):",
