...
function shouldResolveLocalGatewayPasswordSecret(
  ...
  if (trimToUndefined(env.MO_GATEWAY_PASSWORD)) {
    return false;
  }
...
export function registerQrCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/qr", "docs.mo.ai/cli/qr")}\n`,
    )
    .option(
      "--remote",
    ...
    .action(async (opts: QrCliOptions) => {
      try {
        ...
        lines.push(
          ...
          `  ${theme.command("mo devices list")}`,
          `  ${theme.command("mo devices approve <requestId>")}`,
        );
