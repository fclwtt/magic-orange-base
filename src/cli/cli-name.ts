...
const DEFAULT_CLI_NAME = "mo";

const KNOWN_CLI_NAMES = new Set([DEFAULT_CLI_NAME]);
const CLI_PREFIX_RE = /^(?:((?:pnpm|npm|bunx|npx)\s+))?(mo)\b/;

/** Resolve the displayed CLI binary name from argv, falling back to `mo`. */
...
/** Replace a leading `mo` command prefix with the active CLI name. */
