...
vi.mock("./overview.js", () => ({
  ...
  loadCrestodianOverview: vi.fn(async () => ({
    ...
    config: { path: "/tmp/mo.json", exists: true, valid: true, issues: [], hash: null },
    tools: {
    ...
    references: {
      docsUrl: "https://docs.mo.ai",
      sourceUrl: "https://github.com/mo/mo",
    },
  })),
}));
...
describe("parseCrestodianOperation", () => {
  ...
  beforeEach(() => {
    mockConfig.reset();
    stateDirSnapshot = captureEnv(["MO_STATE_DIR"]);
  ...
  it("parses plugin management operations", () => {
    ...
    expect(parseCrestodianOperation("install npm plugin @mo/demo")).toEqual({
      kind: "plugin-install",
      spec: "npm:@mo/demo",
    });
    expect(parseCrestodianOperation("plugin install clawhub:mo-demo")).toEqual({
      kind: "plugin-install",
      spec: "clawhub:mo-demo",
    });
    expect(parseCrestodianOperation("plugin uninstall mo-demo")).toEqual({
  ...
  it("validates missing config without exiting the process", async () => {
    mockConfig.missing("/tmp/mo.json");
    const { runtime, lines } = createCrestodianTestRuntime();
  ...
  it("applies config set through typed deps and writes an audit entry", async () => {
    ...
    setTestEnvValue("MO_STATE_DIR", tempDir);
    const { runtime, lines } = createCrestodianTestRuntime();
  ...
  it("installs plugins only after approval and audits the write", async () => {
    ...
    const plan = await executeCrestodianOperation(
      { kind: "plugin-install", spec: "clawhub:mo-demo" },
      runtime,
      { deps: { runPluginInstall } },
    );
    expectRecordFields(plan as unknown as Record<string, unknown>, {
      applied: false,
      message: "Plan: install plugin clawhub:mo-demo. Say yes to apply.",
    });
    ...
    expect(installCall[0]).toBe("clawhub:mo-demo");
    ...
    expectAuditRecord(
      audit,
      {
        ...
        summary: "Installed plugin clawhub:mo-demo",
      },
      { rescue: true, spec: "clawhub:mo-demo" },
    );
  });
  ...
  it("uninstalls plugins only after approval and audits the write", async () => {
    ...
    const plan = await executeCrestodianOperation(
      { kind: "plugin-uninstall", pluginId: "mo-demo" },
      runtime,
      { deps: { runPluginUninstall } },
    );
    expectRecordFields(plan as unknown as Record<string, unknown>, {
      applied: false,
      message: "Plan: uninstall plugin mo-demo. Say yes to apply.",
    });
    ...
    expect(uninstallCall[0]).toBe("mo-demo");
    ...
    expectAuditRecord(
      audit,
      {
        ...
        summary: "Uninstalled plugin mo-demo",
      },
      { rescue: true, pluginId: "mo-demo" },
    );
  });
