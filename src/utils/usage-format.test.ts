...
describe("usage-format", () => {
  ...
  beforeEach(async () => {
    envSnapshot = captureEnv(["MO_AGENT_DIR", "MO_STATE_DIR"]);
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-usage-format-"));
  ...
  it("prefers models.json pricing over mo config and cached pricing", async () => {
    const config = {
      models: {
        providers: {
          "demo-preferred": {
  ...
  it("falls back to mo config pricing when models.json is absent", () => {
    const config = {
      models: {
        providers: {
