 ...
 * `MO_STATE_DIR`, which prevents parallel Vitest workers from sharing WAL files.
 */
function resolveOpenClawStateRootDir(env: NodeJS.ProcessEnv): string {
  if (env.MO_STATE_DIR?.trim()) {
  ...
  if (env.VITEST || env.NODE_ENV === "test") {
    ...
    return path.join(os.tmpdir(), "mo-test-state", shardSuffix);
  }
  return resolveStateDir(env);
}
...
export function resolveOpenClawStateSqlitePath(env: NodeJS.ProcessEnv = process.env): string {
  return path.join(resolveOpenClawStateSqliteDir(env), "mo.sqlite");
}
