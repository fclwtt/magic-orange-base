...
function createTempStateDir(): string {
  return makeTempDir(agentDbTempDirs, "mo-agent-db-");
}
...
describe("mo agent database", () => {
  it("resolves under the per-agent state directory", () => {
    ...
    ).toBe(path.join(stateDir, "agents", "worker-1", "agent", "mo-agent.sqlite"));
  });
  ...
  it("creates the per-agent schema and registers it globally", () => {
    ...
    expect(collectSqliteSchemaShape(database.db)).toEqual(
      createSqliteSchemaShapeFromSql(new URL("./mo-agent-schema.sql", import.meta.url)),
    );
    expect(database.agentId).toBe("worker-1");
    expect(database.path).toBe(
      path.join(stateDir, "agents", "worker-1", "agent", "mo-agent.sqlite"),
    );
  ...
  it("rejects the legacy agent registry primary key with a doctor repair hint", () => {
    ...
    ).toThrow(/run mo doctor --fix/);
  });

  it("keys explicit relative paths by resolved database pathname", () => {
    const agentModuleUrl = new URL("./mo-agent-db.ts", import.meta.url).href;
    const stateModuleUrl = new URL("./mo-state-db.ts", import.meta.url).href;
    const output = execFileSync(
      ...
      [
        ...
        `
          ...
          const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-agent-db-state-"));
          const env = { MO_STATE_DIR: stateDir };
          const root = fs.mkdtempSync(path.join(os.tmpdir(), "mo-agent-db-relative-"));
  ...
  it("does not chmod shared parent directories for explicit database paths", () => {
    ...
    const env = { MO_STATE_DIR: parentDir };
