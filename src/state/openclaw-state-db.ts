...
} from "./mo-state-db.paths.js";
import { MO_STATE_SCHEMA_SQL } from "./mo-state-schema.generated.js";
 ...
 */
const MO_STATE_SCHEMA_VERSION = 1;
/** Shared timeout used by state and agent SQLite handles before surfacing busy errors. */
export const MO_SQLITE_BUSY_TIMEOUT_MS = 30_000;
const MO_STATE_DIR_MODE = 0o700;
const MO_STATE_FILE_MODE = 0o600;
...
function assertSupportedSchemaVersion(db: DatabaseSync, pathname: string): void {
  ...
  if (userVersion > MO_STATE_SCHEMA_VERSION) {
    throw new Error(
      `OpenClaw state database ${pathname} uses newer schema version ${userVersion}; this OpenClaw build supports ${MO_STATE_SCHEMA_VERSION}.`,
    );
  }
}
...
function ensureOpenClawStatePermissions(pathname: string, env: NodeJS.ProcessEnv): void {
  ...
  mkdirSync(dir, { recursive: true, mode: MO_STATE_DIR_MODE });
  ...
  if (isDefaultStateDatabase || !dirExisted) {
    bestEffortChmodSync(dir, MO_STATE_DIR_MODE);
  }
  for (const candidate of resolveSqliteDatabaseFilePaths(pathname)) {
    if (existsSync(candidate)) {
      bestEffortChmodSync(candidate, MO_STATE_FILE_MODE);
    }
  }
}
...
function assertCanonicalStateSchemaShape(db: DatabaseSync, pathname: string): void {
  ...
  throw new Error(
    `OpenClaw state database ${pathname} has a legacy agent database registry schema; run mo doctor --fix to migrate it.`,
  );
}
...
export function repairOpenClawStateDatabaseSchema(options: OpenClawStateDatabaseOptions = {}): {
  ...
  db.exec(`PRAGMA busy_timeout = ${MO_SQLITE_BUSY_TIMEOUT_MS};`);
  try {
...
function ensureSchema(db: DatabaseSync, pathname: string): void {
  ...
  db.exec(MO_STATE_SCHEMA_SQL);
  ensureAdditiveStateColumns(db);
  db.exec(`PRAGMA user_version = ${MO_STATE_SCHEMA_VERSION};`);
  const now = Date.now();
  ...
  executeSqliteQuerySync(
    ...
    kysely
      ...
      .values({
        meta_key: "primary",
        role: "global",
        schema_version: MO_STATE_SCHEMA_VERSION,
        agent_id: null,
        app_version: null,
        created_at: now,
        updated_at: now,
  ...
  const walMaintenance = (() => {
    ...
    try {
      maintenance = configureSqliteConnectionPragmas(db, {
        busyTimeoutMs: MO_SQLITE_BUSY_TIMEOUT_MS,
        databaseLabel: "mo-state",
