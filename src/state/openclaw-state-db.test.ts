...
import type { DB as OpenClawStateKyselyDatabase } from "./mo-state-db.generated.js";
import {
  closeOpenClawStateDatabaseForTest,
  openOpenClawStateDatabase,
  runOpenClawStateWriteTransaction,
} from "./mo-state-db.js";
import { resolveOpenClawStateSqlitePath } from "./mo-state-db.paths.js";
import {
...
function createTempStateDir(): string {
  return makeTempDir(stateDbTempDirs, "mo-state-db-");
}
...
describe("mo state database", () => {
  it("resolves under the shared state database directory", () => {
    const stateDir = createTempStateDir();

    expect(resolveOpenClawStateSqlitePath({ MO_STATE_DIR: stateDir })).toBe(
      path.join(stateDir, "state", "mo.sqlite"),
    );
  });
  ...
  it("keeps test default state under a worker-sharded temp directory", () => {
    expect(
      ...
      path.join(os.tmpdir(), "mo-test-state", `${process.pid}-7`, "state", "mo.sqlite"),
    );
  });
  ...
  it("creates the shared state schema from the committed SQL shape", () => {
    ...
    const database = openOpenClawStateDatabase({
      env: { MO_STATE_DIR: stateDir },
    });

    expect(collectSqliteSchemaShape(database.db)).toEqual(
      createSqliteSchemaShapeFromSql(new URL("./mo-state-schema.sql", import.meta.url)),
    );
    expect(database.path).toBe(path.join(stateDir, "state", "mo.sqlite"));
  });

  it.runIf(process.platform === "linux")("closes the database when initialization fails", () => {
    const databasePath = path.join(createTempStateDir(), "mo.sqlite");
  ...
  it("opens databases with early cron tables before creating cron indexes", () => {
    const stateDir = createTempStateDir();
    const databasePath = path.join(stateDir, "state", "mo.sqlite");
  ...
  it("does not chmod shared parent directories for explicit database paths", () => {
    const databasePath = path.join(
      os.tmpdir(),
      `mo-explicit-state-${process.pid}-${Date.now()}.sqlite`,
    );
  ...
  it("keys explicit relative paths by resolved database pathname", () => {
    const moduleUrl = new URL("./mo-state-db.ts", import.meta.url).href;
    const output = execFileSync(
      ...
      [
        ...
        `
          ...
          const root = fs.mkdtempSync(path.join(os.tmpdir(), "mo-state-db-relative-"));
  ...
  it("uses savepoints for nested write transaction rollback", () => {
    const stateDir = createTempStateDir();
    const options = { env: { MO_STATE_DIR: stateDir } };
