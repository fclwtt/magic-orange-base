...
import type { DB as OpenClawAgentKyselyDatabase } from "./mo-agent-db.generated.js";
import { resolveOpenClawAgentSqlitePath } from "./mo-agent-db.paths.js";
import { MO_AGENT_SCHEMA_SQL } from "./mo-agent-schema.generated.js";
import type { DB as OpenClawStateKyselyDatabase } from "./mo-state-db.generated.js";
import {
  MO_SQLITE_BUSY_TIMEOUT_MS,
  runOpenClawStateWriteTransaction,
  type OpenClawStateDatabaseOptions,
} from "./mo-state-db.js";
export { resolveOpenClawAgentSqlitePath } from "./mo-agent-db.paths.js";
 ...
 */
const MO_AGENT_SCHEMA_VERSION = 1;
const MO_AGENT_DB_DIR_MODE = 0o700;
const MO_AGENT_DB_FILE_MODE = 0o600;
...
function assertSupportedAgentSchemaVersion(db: DatabaseSync, pathname: string): void {
  ...
  if (userVersion > MO_AGENT_SCHEMA_VERSION) {
    throw new Error(
      `OpenClaw agent database ${pathname} uses newer schema version ${userVersion}; this OpenClaw build supports ${MO_AGENT_SCHEMA_VERSION}.`,
    );
  }
}
...
function ensureOpenClawAgentDatabasePermissions(
  ...
  mkdirSync(dir, { recursive: true, mode: MO_AGENT_DB_DIR_MODE });
  ...
  if (isDefaultAgentDatabase || !dirExisted) {
    chmodSync(dir, MO_AGENT_DB_DIR_MODE);
  }
  for (const candidate of resolveSqliteDatabaseFilePaths(pathname)) {
    if (existsSync(candidate)) {
      chmodSync(candidate, MO_AGENT_DB_FILE_MODE);
    }
  }
}
...
function ensureAgentSchema(db: DatabaseSync, agentId: string, pathname: string): void {
  ...
  db.exec(MO_AGENT_SCHEMA_SQL);
  const kysely = getNodeSqliteKysely<OpenClawAgentMetadataDatabase>(db);
  db.exec(`PRAGMA user_version = ${MO_AGENT_SCHEMA_VERSION};`);
  const now = Date.now();
  executeSqliteQuerySync(
    db,
    kysely
      ...
      .values({
        meta_key: "primary",
        role: "agent",
        schema_version: MO_AGENT_SCHEMA_VERSION,
        agent_id: agentId,
        app_version: null,
        created_at: now,
        updated_at: now,
  ...
  const walMaintenance = (() => {
    ...
    try {
      maintenance = configureSqliteConnectionPragmas(db, {
        ...
        databaseLabel: `mo-agent:${agentId}`,
