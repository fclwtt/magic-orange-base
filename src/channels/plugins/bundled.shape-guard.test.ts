...
function restoreBundledPluginsDir(previousBundledPluginsDir: string | undefined) {
  if (previousBundledPluginsDir === undefined) {
    ...
    process.env.MO_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
  }
}
  ...
  return fs
    ...
    .filter(
      (entryPath) =>
        ...
        fs.existsSync(path.join(entryPath, "mo.plugin.json")),
    );
}
...
function listGitSourceBundledPluginRoots(extensionsDir: string): string[] | null {
  const result = spawnSync(
    "git",
    ["ls-files", "--", "extensions/*/package.json", "extensions/*/mo.plugin.json"],
    {
      cwd: process.cwd(),
      encoding: "utf8",
...
afterEach(() => {
  delete (globalThis as { __moBundledChannelReenter?: () => void })[
    "__moBundledChannelReenter"
  ];
  vi.resetModules();
...
describe("bundled channel entry shape guards", () => {
  ...
  it("fills sparse bundled channel plugin metadata from package metadata", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mo-bundled-metadata-"));
    const previousBundledPluginsDir = process.env.MO_BUNDLED_PLUGINS_DIR;
    ...
    try {
      process.env.MO_BUNDLED_PLUGINS_DIR = path.join(tempRoot, "dist", "extensions");
  ...
  it("uses the active bundled plugin root override for channel entry loading", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mo-bundled-override-"));
    const previousBundledPluginsDir = process.env.MO_BUNDLED_PLUGINS_DIR;
    const pluginDir = path.join(tempRoot, "dist", "extensions", "alpha");
    fs.mkdirSync(pluginDir, { recursive: true });
    fs.writeFileSync(
  ...
  it("falls back through the cached loader for package-local dist entries needing SDK aliases", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "mo-bundled-package-dist-"));
    ...
    fs.writeFileSync(
      path.join(pluginDir, "index.js"),
      [
        'import { defineBundledChannelEntry } from "mo/plugin-sdk/channel-entry-contract";',
  ...
  it("falls back through the cached loader for direct override dist entries needing SDK aliases", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "mo-bundled-direct-dist-"));
    const previousBundledPluginsDir = process.env.MO_BUNDLED_PLUGINS_DIR;
    ...
    try {
      process.env.MO_BUNDLED_PLUGINS_DIR = pluginsRoot;
  ...
  it("treats direct bundled plugin-tree overrides as scan roots", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mo-bundled-direct-override-"));
    const previousBundledPluginsDir = process.env.MO_BUNDLED_PLUGINS_DIR;
  ...
  it("partitions bundled channel lazy caches by active bundled root without re-importing", async () => {
    const rootA = fs.mkdtempSync(path.join(os.tmpdir(), "mo-bundled-root-a-"));
    const rootB = fs.mkdtempSync(path.join(os.tmpdir(), "mo-bundled-root-b-"));
    const previousBundledPluginsDir = process.env.MO_BUNDLED_PLUGINS_DIR;
    const testGlobal = globalThis as typeof globalThis & {
    ...
    try {
      ...
      process.env.MO_BUNDLED_PLUGINS_DIR = path.join(rootA, "dist", "extensions");
      ...
      process.env.MO_BUNDLED_PLUGINS_DIR = path.join(rootB, "dist", "extensions");
  ...
  it("uses dist-runtime as the boundary root for packaged setup entries", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "mo-bundled-runtime-root-"));
  ...
  it("loads setup-entry feature plugins without loading the main channel entry", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "mo-bundled-setup-only-"));
    const previousBundledPluginsDir = process.env.MO_BUNDLED_PLUGINS_DIR;
    ...
    try {
      process.env.MO_BUNDLED_PLUGINS_DIR = path.join(root, "dist", "extensions");
  ...
  it("swallows and caches bundled plugin and setup load failures", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "mo-bundled-load-failure-"));
    const previousBundledPluginsDir = process.env.MO_BUNDLED_PLUGINS_DIR;
    const pluginDir = path.join(root, "dist", "extensions", "alpha");
    const testGlobal = globalThis as typeof globalThis & {
  ...
  it("caches undefined bundled plugin loads as unavailable", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "mo-bundled-null-load-"));
    const previousBundledPluginsDir = process.env.MO_BUNDLED_PLUGINS_DIR;
    const pluginDir = path.join(root, "dist", "extensions", "alpha");
    const testGlobal = globalThis as typeof globalThis & {
  ...
  it("keeps channel entrypoints on the dedicated entry-contract SDK surface", () => {
    const offenders = collectBundledChannelEntrypointOffenders(
      bundledPluginRoots,
      (source) =>
        !source.includes('from "mo/plugin-sdk/channel-entry-contract"') ||
        source.includes('from "mo/plugin-sdk/core"') ||
        source.includes('from "mo/plugin-sdk/channel-core"'),
    );
  ...
  it("keeps setup-entry legacy feature hints mirrored in package metadata", () => {
    ...
    for (const extensionDir of bundledPluginRoots) {
      ...
      for (const feature of ["legacyStateMigrations", "legacySessionSurfaces"]) {
        ...
        const hasHint = packageJson.mo?.setupFeatures?.[feature] === true;
  ...
  it("keeps channel implementations off the broad core SDK surface", () => {
    ...
    for (const extensionDir of bundledPluginRoots) {
      for (const relativePath of ["src/channel.ts", "src/plugin.ts"]) {
        ...
        if (source.includes('from "mo/plugin-sdk/core"')) {
          offenders.push(path.relative(process.cwd(), filePath));
        }
      }
    }
  ...
  it("keeps bundled hot runtime barrels off the broad core SDK surface", () => {
    const offenders = [
      ...
      fs.readFileSync(path.resolve(filePath), "utf8").includes("mo/plugin-sdk/core"),
    );
  ...
  it("keeps bundled doctor surfaces off the broad runtime barrel", () => {
    const offenders = [
      ...
      fs
        ...
        .includes('from "mo/plugin-sdk/runtime"'),
    );
  ...
  it("breaks reentrant bundled channel discovery cycles with an empty fallback", async () => {
    const pluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-bundled-reentrant-"));
    const modulePath = path.join(pluginDir, "index.cjs");
    fs.writeFileSync(
      modulePath,
      `
const reenter = globalThis["__moBundledChannelReenter"];
...
`,
    ...
    let reentered = false;
    (globalThis as { __moBundledChannelReenter?: () => void })[
      "__moBundledChannelReenter"
    ] = () => {
      if (!reentered) {
        reentered = true;
