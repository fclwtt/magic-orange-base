...
describe("channel registry helpers", () => {
  ...
  it("formats Telegram selection lines without a docs prefix and with website extras", () => {
    ...
    expect(line).toContain("https://mo.ai");
  });

  it("prefers the pinned channel registry when resolving registered plugin channels", () => {
    const startupRegistry = createRegistryWithRegisteredChannel("mo-weixin", ["weixin"]);
    ...
    expect(listRegisteredChannelPluginIds()).toEqual(["mo-weixin"]);
    expect(normalizeAnyChannelId("weixin")).toBe("mo-weixin");
