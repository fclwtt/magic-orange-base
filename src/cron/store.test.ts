...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mo-cron-store-"));
});
...
describe("resolveCronStorePath", () => {
  const envSnapshot = captureEnv(["MO_HOME", "HOME"]);

  afterEach(() => {
    envSnapshot.restore();
  });

  it("uses MO_HOME for tilde expansion", () => {
    setTestEnvValue("MO_HOME", "/srv/mo-home");
    setTestEnvValue("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/mo-home", "cron", "jobs.json"));
  });
});
