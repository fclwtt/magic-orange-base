...
describe("dispatchCronDelivery — double-announce guard", () => {
  ...
  it("keeps same-recipient message-tool awareness separate across channels", async () => {
    vi.mocked(resolveOutboundSessionRoute)
      ...
      .mockResolvedValueOnce({
        sessionKey: "agent:main:mo-weixin:direct:123456",
        baseSessionKey: "agent:main:mo-weixin:direct:123456",
        peer: { kind: "direct", id: "123456" },
        chatType: "direct",
        from: "mo-weixin:123456",
        to: "123456",
      });
    ...
    expect(enqueueSystemEvent).toHaveBeenCalledWith(
      ...
      {
        sessionKey: "agent:main:mo-weixin:direct:123456",
        contextKey: "cron-direct-delivery:v1:cron:test-job:1000:mo-weixin::123456:",
      },
    );
  });
  ...
  it("queues message-tool awareness for explicit off-plan message-tool deliveries", async () => {
    mockResolvedOutboundRoute({
      sessionKey: "agent:main:mo-weixin:direct:user-123",
      baseSessionKey: "agent:main:mo-weixin:direct:user-123",
      to: "user-123",
    });
    ...
    expect(enqueueSystemEvent).toHaveBeenCalledExactlyOnceWith(
      ...
      {
        sessionKey: "agent:main:mo-weixin:direct:user-123",
        contextKey: "cron-direct-delivery:v1:cron:test-job:1000:mo-weixin::user-123:",
      },
    );
  });
