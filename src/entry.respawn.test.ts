...
const MO_NODE_EXTRA_CA_CERTS_READY = "MO_NODE_EXTRA_CA_CERTS_READY";
const MO_NODE_OPTIONS_READY = "MO_NODE_OPTIONS_READY";
...
describe("buildCliRespawnPlan", () => {
  ...
  it("adds NODE_EXTRA_CA_CERTS and warning suppression in one respawn", () => {
    ...
    expect(respawnPlan.env[MO_NODE_EXTRA_CA_CERTS_READY]).toBe("1");
    expect(respawnPlan.env[MO_NODE_OPTIONS_READY]).toBe("1");
  ...
  it.each(["tui", "terminal", "chat"] as const)(
    ...
    (command) => {
      ...
      const respawnPlan = expectCliRespawnPlan(plan);
      expect(respawnPlan.argv).toEqual(["mo", command]);
      expect(respawnPlan.env.NODE_EXTRA_CA_CERTS).toBe("/etc/ssl/certs/ca-certificates.crt");
      expect(respawnPlan.env[MO_NODE_EXTRA_CA_CERTS_READY]).toBe("1");
      expect(respawnPlan.env[MO_NODE_OPTIONS_READY]).toBeUndefined();
  ...
  it("keeps bare-root startup respawns attached to the terminal", () => {
    ...
    const respawnPlan = expectCliRespawnPlan(plan);
    expect(respawnPlan.argv).toEqual([EXPERIMENTAL_WARNING_FLAG, "mo"]);
    expect(respawnPlan.detachForProcessTree).toBe(false);
  });
  ...
  it("does not respawn interactive commands for warning suppression only", () => {
    expect(
      buildCliRespawnPlan({
        argv: ["node", "mo", "tui"],
        env: { [MO_NODE_EXTRA_CA_CERTS_READY]: "1" },
        execArgv: [],
  ...
  it("returns null when both respawn guards are already satisfied", () => {
    expect(
      buildCliRespawnPlan({
        argv: ["node", "mo", "status"],
        env: {
          [MO_NODE_EXTRA_CA_CERTS_READY]: "1",
          [MO_NODE_OPTIONS_READY]: "1",
        },
  ...
  it("adds a larger V8 stack size on Windows", () => {
    ...
    expect(respawnPlan.env[MO_NODE_EXTRA_CA_CERTS_READY]).toBeUndefined();
    expect(respawnPlan.env[MO_NODE_OPTIONS_READY]).toBeUndefined();
    expect(respawnPlan.detachForProcessTree).toBe(false);
  });
  ...
  it("respawns Volta shims through node so the shim is not called directly", () => {
    const plan = buildCliRespawnPlan({
      argv: ["/home/alice/.volta/bin/volta-shim", "/usr/local/bin/mo", "status"],
    ...
    runCliRespawnPlan(
      {
        ...
        argv: ["/repo/mo/dist/entry.js", "status"],
        env: { MO_NODE_OPTIONS_READY: "1" },
  ...
  it("force-kills a signaled respawn child that does not exit", () => {
    ...
    try {
      runCliRespawnPlan(
        {
          command: "/usr/bin/node",
          argv: ["/repo/mo/dist/entry.js", "tui"],
          env: {},
