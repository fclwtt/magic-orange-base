...
vi.mock("../../config/config.js", () => ({
  ...
  transformConfigFileWithRetry: async (params: {
    ...
    return {
      path: snapshot.path ?? "/tmp/mo.json",
      previousHash,
...
describe("command gating", () => {
  ...
  it("redacts secret-shaped fields from full /config show replies", async () => {
    readConfigFileSnapshotMock.mockResolvedValueOnce({
      ...
      parsed: {
        gateway: {
          auth: {
            mode: "token",
            token: "MO_CONFIG_SHOW_CANARY_TOKEN_65623",
            password: "MO_CONFIG_SHOW_CANARY_PASSWORD_65623",
          },
          bind: "127.0.0.1",
          port: 3210,
        },
        models: {
          providers: {
            openai: {
              apiKey: "MO_CONFIG_SHOW_CANARY_API_KEY_65623",
        ...
        browser: {
          cdpUrl:
            "wss://chrome.example.test/devtools?token=MO_CONFIG_SHOW_CANARY_CDP_TOKEN_65623&apiKey=MO_CONFIG_SHOW_CANARY_CDP_API_KEY_65623",
          profiles: {
            local: {
            ...
            remote: {
              cdpUrl:
                "wss://chrome.remote.example.test/devtools?apiKey=MO_CONFIG_SHOW_CANARY_CDP_PROFILE_API_KEY_65623",
            },
          },
        },
        talk: {
    ...
    expect(output).not.toContain("MO_CONFIG_SHOW_CANARY_TOKEN_65623");
    expect(output).not.toContain("MO_CONFIG_SHOW_CANARY_PASSWORD_65623");
    expect(output).not.toContain("MO_CONFIG_SHOW_CANARY_API_KEY_65623");
    expect(output).not.toContain("MO_CONFIG_SHOW_CANARY_CDP_TOKEN_65623");
    expect(output).not.toContain("MO_CONFIG_SHOW_CANARY_CDP_API_KEY_65623");
    expect(output).not.toContain("MO_CONFIG_SHOW_CANARY_CDP_PROFILE_API_KEY_65623");
  ...
  it("redacts secret-shaped values from /config set acknowledgements", async () => {
    ...
    const params = buildParams(
      '/config set gateway.auth.token="MO_CONFIG_SET_CANARY_TOKEN_65623"',
      {
        commands: { config: true, text: true },
        channels: { whatsapp: { allowFrom: ["*"] } },
      } as OpenClawConfig,
    ...
    expect(output).toContain(REDACTED_SENTINEL);
    expect(output).not.toContain("MO_CONFIG_SET_CANARY_TOKEN_65623");
  });
  ...
  it("redacts secret-shaped fields from /debug show replies", async () => {
    getConfigOverridesMock.mockReturnValueOnce({
      gateway: {
        auth: {
          token: "MO_DEBUG_SHOW_CANARY_TOKEN_65623",
        },
      },
      channels: {
        telegram: {
          botToken: "MO_DEBUG_SHOW_CANARY_BOT_TOKEN_65623",
        },
      },
      messages: {
        ackReaction: ":)",
    ...
    expect(output).not.toContain("MO_DEBUG_SHOW_CANARY_TOKEN_65623");
    expect(output).not.toContain("MO_DEBUG_SHOW_CANARY_BOT_TOKEN_65623");
  });

  it("redacts secret-shaped values from /debug set acknowledgements", async () => {
    const params = buildParams(
      '/debug set gateway.auth.token="MO_DEBUG_SET_CANARY_TOKEN_65623"',
      {
        commands: { debug: true, text: true },
        channels: { whatsapp: { allowFrom: ["*"] } },
      } as OpenClawConfig,
    ...
    expect(output).toContain(REDACTED_SENTINEL);
    expect(output).not.toContain("MO_DEBUG_SET_CANARY_TOKEN_65623");
  });
