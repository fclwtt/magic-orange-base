...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-export-command-"));
  tempDirs.push(dir);
  return dir;
}
    ...
    execute: vi.fn(async (_toolCallId: string, params: unknown) => {
      ...
      return {
        details: {
          ...
          command: "mo sessions export-trajectory --session-key agent:target:session",
          cwd: "/tmp",
        },
      };
    }),
...
describe("buildExportTrajectoryCommandReply", () => {
  ...
  it("requests per-run exec approval for trajectory exports", async () => {
    ...
    expect(reply.text).toContain("https://docs.mo.ai/tools/trajectory");
    ...
    expect(execCall.defaults.sessionStore).toBe("/tmp/mo-sessions.json");
    ...
    expect(command).not.toContain("mo sessions export-trajectory");
    ...
    expect(String(request.workspace)).toContain("mo-export-command-");
  });
  ...
  it("routes group trajectory export approval privately", async () => {
    ...
    expect(privateReplies[0]?.text).toContain("mo sessions export-trajectory");
