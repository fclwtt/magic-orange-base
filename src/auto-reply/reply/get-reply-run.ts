...
import { asDateTimestampMs } from "@mo/normalization-core/number-coercion";
import { type FastMode, normalizeOptionalString } from "@mo/normalization-core/string-coerce";
import {
...
export async function runPreparedReply(
  ...
  const useFastReplyRuntime = shouldUseReplyFastTestRuntime({
    cfg,
    isFastTestEnv: process.env.MO_TEST_FAST === "1",
  });
  ...
  const userTurnInput =
    userTurnTranscriptText !== undefined || userTurnMediaForPersistence.length > 0
      ? {
          ...
          // replayed as history. See: https://github.com/mo/mo/issues/3658
