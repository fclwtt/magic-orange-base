  ...
  const lines = [
    ...
    ...report.plugins.map((plugin) => {
      const format = plugin.bundleFormat
        ? `${plugin.format ?? "mo"}/${plugin.bundleFormat}`
        : (plugin.format ?? "mo");
  ...
  if (clawhubSpec) {
    ...
    if (!result.ok) {
      ...
      if (result.code === CLAWHUB_INSTALL_ERROR_CODE.CLAWHUB_RISK_ACKNOWLEDGEMENT_REQUIRED) {
        return {
          ok: false,
          error: `${warningPrefix}${result.error} The /plugins chat command cannot acknowledge ClawHub risk; run the local mo plugins install command with --acknowledge-clawhub-risk from a trusted shell after reviewing the warning.`,
        };
      }
