...
const MO_CLI_ENTRY_BASENAMES = new Set(["mo", "mo.mjs"]);
const MO_PACKAGE_ENTRY_PATHS = new Set([
...
const TEST_RUNNER_ENV_PREFIXES = ["VITEST_", "MO_VITEST_"];
...
function isOpenClawCliLauncherEntry(entry: string): boolean {
  return MO_CLI_ENTRY_BASENAMES.has(path.basename(entry));
}
...
function isOpenClawPackageEntry(entry: string, packageRoot: string): boolean {
  ...
  return MO_PACKAGE_ENTRY_PATHS.has(relativeEntry);
}
  ...
  if (fs.existsSync(sourceEntry)) {
    ...
    return isBunRuntime(process.execPath)
      ...
      : tsxLoader
        ...
        : [process.execPath, path.join(packageRoot, "mo.mjs")];
  }
  return [process.execPath, path.join(packageRoot, "mo.mjs")];
}
