...
const COMPLETE_REPLY_CONFIG_SYMBOL = Symbol.for("mo.reply.complete-config");
const FULL_REPLY_RUNTIME_SYMBOL = Symbol.for("mo.reply.full-runtime");
...
function isSlowReplyTestAllowed(env: NodeJS.ProcessEnv = process.env): boolean {
  return (
    env.MO_ALLOW_SLOW_REPLY_TESTS === "1" || env.MO_STRICT_FAST_REPLY_CONFIG === "0"
  );
}
...
export function resolveGetReplyConfig(params: {
  ...
  if (params.isFastTestEnv && !isCompleteReplyConfig(configOverride) && !isSlowReplyTestAllowed()) {
    throw new Error(
      "Fast reply tests must pass with withFastReplyConfig()/markCompleteReplyConfig(); set MO_ALLOW_SLOW_REPLY_TESTS=1 to opt out.",
    );
  }
