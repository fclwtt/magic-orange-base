...
// See: https://github.com/mo/mo/issues/58348
...
function buildMissingApiKeyFailureText(input: { message: string; error?: unknown }): string | null {
  ...
  if (provider === "openai") {
    return '⚠️ Missing API key for provider "openai". Run `mo doctor --fix` to repair stale OpenAI model/session routes, restart the gateway if doctor asks, then try again. If doctor has nothing to repair or the error persists, re-auth with `mo models auth login --provider openai` or run `mo configure`.';
  }
        ...
        runWithModelFallback<EmbeddedAgentRunResult>({
          ...
          run: async (provider, model, runOptions) => {
            ...
            const embeddedRunHarnessOverride =
              ...
              (agentHarnessPolicy.runtime === "mo" && embeddedRunProvider !== provider
                ? "mo"
                : undefined);
            return (async () => {
                  ...
                  runEmbeddedAgent({
                    ...
                    onToolResult: onToolResult
                      ? (() => {
                          ...
                          // See: https://github.com/mo/mo/issues/11044
