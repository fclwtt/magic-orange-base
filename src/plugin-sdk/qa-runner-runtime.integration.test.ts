...
const ORIGINAL_ENV = {
  MO_DISABLE_BUNDLED_PLUGINS: process.env.MO_DISABLE_BUNDLED_PLUGINS,
  MO_CONFIG_PATH: process.env.MO_CONFIG_PATH,
  MO_STATE_DIR: process.env.MO_STATE_DIR,
  MO_TEST_FAST: process.env.MO_TEST_FAST,
} as const;
...
describe("plugin-sdk qa-runner-runtime linked plugin smoke", () => {
  ...
  it("loads an activated qa runner from a linked plugin path without a bundled install fallback", async () => {
    const stateDir = makeTempDir("mo-qa-runner-state-");
