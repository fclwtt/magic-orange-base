...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("MO_HOME", snapshot.moHome);
  restoreKey("MO_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked MO_HOME.
  deleteTestEnvValue("MO_HOME");
  setTestEnvValue("MO_STATE_DIR", path.join(base, ".mo"));
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "mo-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".mo", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
