...
describe("plugin-sdk qa-runner-runtime", () => {
  ...
  const originalPrivateQaCli = process.env.MO_ENABLE_PRIVATE_QA_CLI;
  ...
  it("loads bundled plugin test APIs with the private QA source tree override", async () => {
    const sourceRoot = makePrivateQaSourceRoot(tempDirs, "mo-qa-test-api-root-");
    ...
    expect(testApiCall?.env?.MO_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(testApiCall?.env?.MO_BUNDLED_PLUGINS_DIR).toBe(
  ...
  it("prefers the source bundled tree for private qa discovery in repo checkouts", async () => {
    const sourceRoot = makePrivateQaSourceRoot(tempDirs, "mo-qa-runner-root-");
    resolveOpenClawPackageRootSync.mockReturnValue(sourceRoot);
    ...
    expect(manifestCall?.env?.MO_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(manifestCall?.env?.MO_BUNDLED_PLUGINS_DIR).toBe(
      path.join(sourceRoot, "extensions"),
    );
    ...
    expect(publicSurfaceCall?.env?.MO_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(publicSurfaceCall?.env?.MO_BUNDLED_PLUGINS_DIR).toBe(
      path.join(sourceRoot, "extensions"),
    );
  });
  ...
  it("fails when runtime registrations include an undeclared command", async () => {
    ...
    expect(() => module.listQaRunnerCliContributions()).toThrow(
      'QA runner plugin "qa-matrix" exported "extra" from runtime-api.js but did not declare it in mo.plugin.json',
    );
  });
});
