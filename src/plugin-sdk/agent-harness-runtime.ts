...
export { VERSION as MO_VERSION } from "../version.js";
...
export async function detectAndLoadAgentHarnessPromptImages(params: {
  ...
  config?: import("../config/types.mo.js").OpenClawConfig;
  ...
  const [{ resolveImageSanitizationLimits }, { detectAndLoadPromptImages }, { MAX_IMAGE_BYTES }] =
    await Promise.all([
      ...
      import("@mo/media-core/constants"),
    ]);
 ...
 * mo/plugin-sdk/session-transcript-runtime so transcript writes target a
 ...
 * mo/plugin-sdk/session-transcript-runtime so transcript updates target
