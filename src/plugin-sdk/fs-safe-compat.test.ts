...
import { loadSecretFileSync as loadSecretFileSyncFromCore } from "mo/plugin-sdk/core";
import { readFileWithinRoot, writeFileWithinRoot } from "mo/plugin-sdk/file-access-runtime";
import {
  loadSecretFileSync,
  type SecretFileReadResult,
} from "mo/plugin-sdk/secret-file-runtime";
...
describe("plugin SDK fs-safe compatibility exports", () => {
  it("keeps deprecated secret-file result helpers on public SDK subpaths", async () => {
    await withTempDir({ prefix: "mo-sdk-secret-compat-" }, async (root) => {
  ...
  it("keeps deprecated root-bounded read/write helpers on file-access-runtime", async () => {
    await withTempDir({ prefix: "mo-sdk-file-access-compat-" }, async (root) => {
