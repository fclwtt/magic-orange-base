...
const versionFixtureRoot = createSuiteTempRootTracker({ prefix: "mo-version-" });
...
describe("version resolution", () => {
  it("keeps bundled version injection as a direct define identifier", async () => {
    ...
    expect(source).toContain("typeof __MO_VERSION__");
    expect(source).toContain("? __MO_VERSION__");
  });

  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "mo", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "mo", version: "2.3.4" });
  ...
  it("ignores non-mo package and blank build-info versions", async () => {
    await withVersionFixtureDir(async (root) => {
  ...
  it("prefers MO_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        MO_VERSION: "9.9.9",
        MO_SERVICE_VERSION: "2.2.2",
  ...
  it("prefers runtime VERSION over stale MO_VERSION for compatibility checks", () => {
    const previous = process.env.MO_VERSION;
    const previousService = process.env.MO_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.MO_VERSION = "2026.3.25";
      process.env.MO_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      restoreEnvValue("MO_VERSION", previous);
      restoreEnvValue("MO_SERVICE_VERSION", previousService);
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        MO_VERSION: "2026.3.99",
        MO_SERVICE_VERSION: "2026.3.98",
  ...
  it("prefers explicit compatibility host overrides over runtime and stale env versions", () => {
    expect(
      resolveCompatibilityHostVersion({
        MO_COMPATIBILITY_HOST_VERSION: "2026.4.8",
        MO_VERSION: "2026.3.99",
        MO_SERVICE_VERSION: "2026.3.98",
        npm_package_version: "2026.3.97",
      }),
