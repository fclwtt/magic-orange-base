...
function writeConfig(source: string): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-redact-config-"));
  tempDirs.push(dir);
  const configPath = path.join(dir, "mo.json");
...
describe("redactSensitiveText", () => {
  ...
  it("masks URL userinfo and database connection-string passwords", () => {
    const input = [
      ...
      "postgres://dbuser:database-password-1234567890@db.example.test/mo",
      "postgres://secret:secret@db.example.test/mo",
    ...
    expect(output).toContain("postgres://dbuser:databa…7890@db.example.test/mo");
    expect(output).toContain("postgres://secret:***@db.example.test/mo");
  ...
  it("does not redact ordinary identifiers containing short token-prefix substrings", () => {
    const input = [
      "npm_telegram_package_spec ask_mo_query_patterns team_management risk_assessment glpat-docs dapi-example sbp_short nfp_site CCIPAT_docs ATATT-example fw-tooshort fw_tooshort fpk_tooshort",
  ...
  it("masks connection-string passwords through the default options path", () => {
    expect(
      redactSensitiveText("postgres://dbuser:opaquepw12345@db.example.test/mo", {
        mode: "tools",
      }),
    ).toBe("postgres://dbuser:***@db.example.test/mo");
  });
  ...
  it("honors logging redaction settings from the active config path", () => {
    ...
    withEnv({ MO_CONFIG_PATH: configPath }, () =>
