...
const logPathTracker = createSuiteLogPathTracker("mo-log-redaction-");
...
describe("file log redaction", () => {
  ...
  it("uses logging.file from the active config path", () => {
    ...
    withEnv({ MO_CONFIG_PATH: configPath, MO_TEST_FILE_LOG: "1" }, () => {
  ...
  it("expands leading tilde in logging.file", () => {
    ...
    withEnv({ HOME: home }, () => {
      expect(loggerTest.resolveActiveLogFile("~/custom-mo.log")).toBe(
        path.join(home, "custom-mo.log"),
      );
    });
  });
