...
describe("shouldSkipMutatingLoggingConfigRead", () => {
  it("matches config schema and validate invocations", () => {
    expect(
      testApi.shouldSkipMutatingLoggingConfigRead(["node", "mo", "config", "schema"]),
    ).toBe(true);
    expect(
      testApi.shouldSkipMutatingLoggingConfigRead(["node", "mo", "config", "validate"]),
    ).toBe(true);
  });
  ...
  it("does not match other commands", () => {
    expect(
      testApi.shouldSkipMutatingLoggingConfigRead(["node", "mo", "config", "get", "foo"]),
    ).toBe(false);
    expect(testApi.shouldSkipMutatingLoggingConfigRead(["node", "mo", "status"])).toBe(false);
  });
});
