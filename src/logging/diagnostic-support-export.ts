...
const SUPPORT_EXPORT_PREFIX = "mo-diagnostics-";
...
function resolveBonjourEnvOverride(
  ...
  const raw = env.MO_DISABLE_BONJOUR?.trim().toLowerCase();
  if (!raw) {
    return "unset";
  }
  switch (raw) {
...
function readStabilityBundle(
  ...
  if (target === false) {
    return { status: "missing", dir: "$MO_STATE_DIR/logs/stability" };
  }
...
function renderSummary(params: {
  ...
  return [
    ...
    "- `logs/mo-sanitized.jsonl`: sanitized log summaries and metadata",
