...
describe("diagnostic support export", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-support-export-"));
  ...
  it("writes a shareable zip without raw chats, webhook bodies, or secrets", async () => {
    ...
    const logTail: LogTailPayload = {
      file: path.join(tempDir, "logs", "mo.log"),
      cursor: 200,
      size: 200,
      truncated: false,
      reset: false,
    ...
    const result = await writeDiagnosticSupportExport({
      ...
      readStatusSnapshot: async () => ({
        service: {
          loaded: true,
          command: {
            programArguments: ["mo", "gateway", "run", "--token", fakeToken],
            environment: {
              HOME: tempDir,
              MO_GATEWAY_TOKEN: fakeToken,
            },
          },
        },
        gateway: {
    ...
    expect(combined).toContain("$MO_STATE_DIR");
    ...
    const sanitizedLogs = entries["logs/mo-sanitized.jsonl"];
    ...
    expect(status.data?.service?.command?.environment?.MO_GATEWAY_TOKEN).toBe("<redacted>");
  ...
  it("redacts Windows USERPROFILE paths when HOME is unset", () => {
    ...
    const stateDir = `${userProfile}\\AppData\\Roaming\\mo`;
    const redaction = {
      env: {
    ...
    const status = sanitizeSupportSnapshotValue(
      {
        service: {
          command: {
            programArguments: [
              "node",
              `${userProfile}\\mo\\dist\\index.js`,
              "--config",
              `${stateDir}\\mo.json`,
            ],
            sourcePath: "c:\\users\\support-user\\AppData\\Local\\mo\\gateway-service.json",
          },
        },
      },
      redaction,
    ...
    expect(serialized).toContain("~\\\\mo\\\\dist\\\\index.js");
    expect(serialized).toContain("$MO_STATE_DIR\\\\mo.json");
    expect(serialized).toContain("~\\\\AppData\\\\Local\\\\mo\\\\gateway-service.json");
  });
  ...
  it("keeps writing when log tail collection fails", async () => {
    ...
    await writeDiagnosticSupportExport({
      ...
      readLogTail: async () => {
        throw new Error(`log tail failed at ${tempDir}/mo.log with token ${fakeToken}`);
      },
    });

    const entries = await readZipTextEntries(outputPath);
    expect(Object.keys(entries).toSorted()).toContain("logs/mo-sanitized.jsonl");
