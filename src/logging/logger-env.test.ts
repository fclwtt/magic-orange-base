...
const logPathTracker = createSuiteLogPathTracker("mo-test-env-log-level-");

describe("MO_LOG_LEVEL", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["MO_LOG_LEVEL"]);
  ...
  it("applies a valid env override to both file and console levels", () => {
    ...
    process.env.MO_LOG_LEVEL = "debug";
  ...
  it("warns once and ignores invalid env values", () => {
    ...
    const warnings = stderrSpy.mock.calls
      ...
      .filter((line) => line.includes("MO_LOG_LEVEL"));
    expect(warnings).toHaveLength(1);
    expect(warnings[0]).toContain('Ignoring invalid MO_LOG_LEVEL="nope"');
  });
});
