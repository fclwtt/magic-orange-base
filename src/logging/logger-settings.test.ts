...
beforeEach(() => {
  envSnapshot = captureEnv(["MO_TEST_FILE_LOG", "MO_LOG_LEVEL"]);
  delete process.env.MO_TEST_FILE_LOG;
...
describe("getResolvedLoggerSettings", () => {
  ...
  it("reads logging config when test file logging is explicitly enabled", () => {
    ...
    expect(settings.file).toBe("/tmp/mo-configured.log");
  ...
  it("uses defaults when no logging config is available", () => {
    ...
    expect(path.basename(settings.file)).toMatch(/^mo-vitest-\d+-\d{4}-\d{2}-\d{2}\.log$/);
    expect(settings.file).not.toBe(
      `/tmp/mo/mo-${new Date().toISOString().slice(0, 10)}.log`,
    );
  });
});
