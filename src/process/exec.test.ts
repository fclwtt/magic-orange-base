...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", () => {
    ...
    expect(resolved.MO_BASE_ENV).toBe("base");
    expect(resolved.MO_TEST_ENV).toBe("ok");
    expect(resolved.MO_TO_REMOVE).toBeUndefined();
    expect(resolved.MO_CLI).toBe(MO_CLI_ENV_VALUE);
  });
