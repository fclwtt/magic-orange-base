...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "mo-bundled": {
    ...
    canOverride: ["mo-bundled"],
    canBeOverriddenBy: ["mo-managed", "mo-plugin"],
  },
  "mo-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["mo-bundled", "mo-plugin"],
    canBeOverriddenBy: ["mo-managed"],
  },
  "mo-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["mo-bundled", "mo-managed", "mo-plugin"],
    canBeOverriddenBy: ["mo-managed"],
  },
  "mo-workspace": {
    precedence: 40,
    ...
    canOverride: ["mo-workspace"],
    canBeOverriddenBy: ["mo-workspace"],
  },
};
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "mo-plugin") {
    return { enabled: true };
  }
