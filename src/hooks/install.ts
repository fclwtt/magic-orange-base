...
async function ensureOpenClawHooks(manifest: HookPackageManifest) {
  ...
  if (!Array.isArray(hooks)) {
    throw new Error("package.json missing mo.hooks");
  }
  const list = normalizeTrimmedStringList(hooks);
  if (list.length === 0) {
    throw new Error("package.json mo.hooks is empty");
  }
  return list;
}
...
async function installHookPackageFromDir(
  ...
  for (const entry of hookEntries) {
    ...
    if (!runtime.isPathInside(params.packageDir, hookDir)) {
      return {
        ok: false,
        error: `mo.hooks entry escapes package directory: ${entry}`,
      };
    }
    ...
    if (
      ...
      return {
        ok: false,
        error: `mo.hooks entry resolves outside package directory: ${entry}`,
      };
    }
