...
const fixtureRoot = path.join(process.cwd(), ".tmp", `mo-hook-install-${randomUUID()}`);
...
function writeHookPackFiles(params: {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      'metadata: {"mo":{"events":["command:new"]}}',
      "---",
      "",
      `# ${params.heading}`,
    ].join("\n"),
...
async function createZipHookPackBuffer(params: {
  ...
  const packageJson = JSON.stringify({
    ...
    version: "0.0.1",
    mo: { hooks: [`./hooks/${params.hookName}`] },
  });
...
describe("installHooksFromPath", () => {
  ...
  it("blocks a staged single hook before publishing the target", async () => {
    ...
    expect(scanCall.packageDir).toContain(".mo-install-stage-");
  ...
  it.each([".codex-plugin/plugin.json", "hooks/hooks.json", "mo.plugin.json"])(
    ...
    const cases = [
      {
        hooks: ["../outside"],
        setupLink: false,
        expected: "mo.hooks entry escapes package directory",
      },
      {
        hooks: ["./linked"],
        setupLink: true,
        expected: "mo.hooks entry resolves outside package directory",
      },
    ] as const;
...
describe("installHooksFromNpmSpec", () => {
          ...
          expect(params.archiveInstallParams).toEqual(
            expect.objectContaining({
              installPolicyRequest: {
                kind: "plugin-npm",
                requestedSpecifier: "@mo/test-hooks@0.0.1",
                source: {
                  kind: "npm",
  ...
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(result.hookPackId).toBe("test-hooks");
    expect(result.packageKind).toBe("hook-only");
    expect(result.npmResolution?.resolvedSpec).toBe("@mo/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@mo/test-hooks@0.0.1",
    });
  ...
  it("rejects invalid npm spec shapes", async () => {
    ...
    if (!result.ok) {
      ...
      expect(result.error).toContain('"@mo/test-hooks@beta"');
    }
  });
});
