...
function requireOpenClawMetadata(metadata: OpenClawHookMetadata | undefined): OpenClawHookMetadata {
  if (!metadata) {
    throw new Error("expected mo metadata");
  }
  return metadata;
}
...
`;
    ...
    expect(parsed.mo.emoji).toBe("💾");
    expect(parsed.mo.events).toEqual(["command:new"]);
  });
...
`;
    ...
    expect(parsed.mo.emoji).toBe("📝");
    expect(parsed.mo.events).toEqual(["command"]);
    expect(parsed.mo.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.mo.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"mo": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"mo": {"events": ["test"]}}');
  });
...
describe("resolveOpenClawMetadata", () => {
  it("extracts mo metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const mo = requireOpenClawMetadata(result);
    expect(mo.emoji).toBe("🔥");
    expect(mo.events).toEqual(["command:new", "command:reset"]);
    expect(mo.requires?.config).toEqual(["workspace.dir"]);
    expect(mo.requires?.bins).toEqual(["git"]);
  });
  ...
  it("returns undefined when mo key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
      ...
      metadata: JSON.stringify({
        mo: {
          ...
          install: [
            ...
            { id: "npm", kind: "npm", package: "@mo/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@mo/hook");
  });
...
homepage: https://docs.mo.ai/automation/hooks#session-memory
metadata:
  {
    "mo":
      {
...
`;
    ...
    const mo = requireOpenClawMetadata(resolveOpenClawMetadata(frontmatter));
    expect(mo.emoji).toBe("💾");
    expect(mo.events).toEqual(["command:new", "command:reset"]);
    expect(mo.requires?.config).toEqual(["workspace.dir"]);
    expect(mo.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const mo = resolveOpenClawMetadata(frontmatter);
    expect(mo?.emoji).toBe("disk");
    expect(mo?.events).toEqual(["command:new"]);
  });
});
