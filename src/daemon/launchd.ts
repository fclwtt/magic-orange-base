...
const MO_UPDATE_LAUNCHD_LABEL_PREFIX = "ai.mo.update.";
const MO_MANUAL_UPDATE_LAUNCHD_LABEL_PATTERN = /^ai\.mo\.manual-update\.\d+$/;
...
function normalizeOpenClawUpdateLaunchdLabel(label: unknown): string | null {
  ...
  if (trimmed.startsWith(MO_UPDATE_LAUNCHD_LABEL_PREFIX)) {
    return trimmed;
  }
  // Manual update jobs include a timestamp-like suffix and should be cleaned up
  // without matching arbitrary ai.mo labels.
  return MO_MANUAL_UPDATE_LAUNCHD_LABEL_PATTERN.test(trimmed) ? trimmed : null;
}

function isCurrentGatewayLaunchdLabel(label: string, env: NodeJS.ProcessEnv): boolean {
  const gatewayProfileLabel = resolveGatewayLaunchAgentLabel(env.MO_PROFILE);
  if (label === gatewayProfileLabel) {
    return true;
  }
  if (
    env.MO_SERVICE_MARKER?.trim() !== GATEWAY_SERVICE_MARKER ||
    env.MO_SERVICE_KIND?.trim() !== GATEWAY_SERVICE_KIND
  ) {
    return false;
  }
  const configuredLabel = env.MO_LAUNCHD_LABEL?.trim();
...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  const envLabel = args?.env?.MO_LAUNCHD_LABEL?.trim();
  if (envLabel) {
    return assertValidLaunchAgentLabel(envLabel);
  }
  return assertValidLaunchAgentLabel(resolveGatewayLaunchAgentLabel(args?.env?.MO_PROFILE));
}
...
async function resolveLaunchAgentEnvironmentWrapperOverwriteWarnings(params: {
  ...
  return [
    `Existing generated LaunchAgent env wrapper at ${params.wrapperPath} contains custom behavior and will be overwritten; move custom behavior to mo gateway install --wrapper <path> or MO_WRAPPER.`,
  ];
}
...
async function resolveLaunchAgentGatewayPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  const fromServiceEnv = parseTcpPort(command?.environment?.MO_GATEWAY_PORT ?? "");
  if (fromServiceEnv !== null) {
    return fromServiceEnv;
  }
  return parseTcpPort(env.MO_GATEWAY_PORT ?? "");
}
...
export function formatLaunchAgentGuiSessionError(params: {
  ...
  return [
    ...
    "Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.mo.ai/gateway",
  ].join("\n");
}
...
export async function stopLaunchAgent({
  ...
  if (!persistDisable) {
    ...
    // `mo gateway start` re-enables cleanly without a manual `launchctl enable`.
...
async function writeLaunchAgentPlist({
  ...
  const domain = resolveGuiDomain();
  const label = resolveLaunchAgentLabel({ env });
  for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.MO_PROFILE)) {
...
async function activateLaunchAgent(params: { env: GatewayServiceEnv; plistPath: string }) {
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    actionHint: "mo gateway install --force",
  });
}
...
export async function restartLaunchAgent({
  ...
  // `mo gateway restart` is an explicit operator request to bring the
