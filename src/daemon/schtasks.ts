...
async function writeTaskXmlTempFile(xml: string): Promise<string> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-task-xml-"));
...
function shouldUseHiddenWindowsTaskLauncher(env: GatewayServiceEnv): boolean {
  const value = normalizeLowercaseStringOrEmpty(env.MO_WINDOWS_TASK_HIDDEN_LAUNCHER);
...
async function resolveScheduledTaskNodeHostProcess(env: GatewayServiceEnv): Promise<{
  ...
  const port =
    ...
    parsePositivePort(command?.environment?.MO_GATEWAY_PORT) ??
...
function shouldManageGatewayListenerPort(env: GatewayServiceEnv): boolean {
  return normalizeLowercaseStringOrEmpty(env.MO_SERVICE_KIND) !== NODE_SERVICE_KIND;
}
...
const SCHEDULED_TASK_ACTIVATION_KEYS = [
  "MO_WINDOWS_TASK_HIDDEN_LAUNCHER",
  "MO_TASK_SCRIPT_NAME",
  "MO_TASK_SCRIPT",
  "MO_SERVICE_KIND",
  "MO_GATEWAY_PORT",
