...
export const MO_WRAPPER_ENV_KEY = "MO_WRAPPER";
...
async function resolveCliEntrypointPathForService(): Promise<string> {
  ...
  if (looksLikeDist) {
    ...
    // since symlinks like node_modules/mo -> .pnpm/mo@X.Y.Z/...
...
function appendNodeModulesBinCandidates(
  ...
  // mo from node_modules/.bin points at the package root sibling.
...
export async function resolveOpenClawWrapperPath(
  ...
  try {
    ...
    throw new Error(
      `${MO_WRAPPER_ENV_KEY} must point to an executable file: ${resolved}${detail}`,
      { cause: error },
    );
  }
  return resolved;
