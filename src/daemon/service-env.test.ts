...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.MO_GATEWAY_PORT).toBe("18789");
    expect(env.MO_GATEWAY_TOKEN).toBeUndefined();
    expect(env.MO_SERVICE_MARKER).toBe("mo");
    expect(env.MO_SERVICE_KIND).toBe("gateway");
    expect(typeof env.MO_SERVICE_VERSION).toBe("string");
    expect(env.MO_SYSTEMD_UNIT).toBe("mo-gateway.service");
    expect(env.MO_WINDOWS_TASK_NAME).toBe("OpenClaw Gateway");
    expect(env.MO_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
    if (process.platform === "darwin") {
      expect(env.MO_LAUNCHD_LABEL).toBe("ai.mo.gateway");
    }
  });
  ...
  it("passes through MO_WRAPPER for gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        MO_WRAPPER: " /usr/local/bin/mo-doppler ",
      },
      port: 18789,
    });

    expect(env.MO_WRAPPER).toBe("/usr/local/bin/mo-doppler");
  });
  ...
  it("uses a durable state temp directory for macOS LaunchAgents", () => {
    ...
    expect(env.TMPDIR).toBe(path.join("/Users/user", ".mo", "tmp"));
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", MO_PROFILE: "work" },
      port: 18789,
    });
    expect(env.MO_SYSTEMD_UNIT).toBe("mo-gateway-work.service");
    expect(env.MO_WINDOWS_TASK_NAME).toBe("OpenClaw Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.MO_LAUNCHD_LABEL).toBe("ai.mo.work");
    }
  });
  ...
  it("preserves explicit systemd unit overrides", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        MO_PROFILE: "work",
        MO_SYSTEMD_UNIT: "mo-gateway-maintenance",
      },
      port: 18789,
      platform: "linux",
    });

    expect(env.MO_SYSTEMD_UNIT).toBe("mo-gateway-maintenance.service");
  });
  ...
  it("preserves explicit systemd unit overrides with service suffix", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        MO_SYSTEMD_UNIT: "mo-gateway-maintenance.service",
      },
      port: 18789,
      platform: "linux",
    });
  ...
  it("sets a profile-specific launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user", MO_PROFILE: "work" },
      port: 18789,
      platform: "darwin",
    });
  ...
  it("forwards proxy URL env fallback for installed gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        MO_PROXY_URL: " http://127.0.0.1:3128 ",
      },
      port: 18789,
    });

    expect(env.MO_PROXY_URL).toBe("http://127.0.0.1:3128");
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("sets the OpenClaw-owned launchd marker for macOS node services", () => {
    ...
    expect(env.MO_LAUNCHD_LABEL).toBe("ai.mo.node");
  });

  it("passes through MO_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", MO_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.MO_GATEWAY_TOKEN).toBe("node-token");
  });

  it("passes through MO_ALLOW_INSECURE_PRIVATE_WS for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", MO_ALLOW_INSECURE_PRIVATE_WS: " 1 " },
    });
    expect(env.MO_ALLOW_INSECURE_PRIVATE_WS).toBe("1");
  });

  it("omits MO_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        MO_GATEWAY_TOKEN: "   ",
  ...
  it("marks Windows node tasks for hidden launcher startup", () => {
    ...
    expect(env.MO_WINDOWS_TASK_NAME).toBe("OpenClaw Node");
    expect(env.MO_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
    expect(env.MO_TASK_SCRIPT_NAME).toBe("node.cmd");
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".mo"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", MO_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".mo-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", MO_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".mo"));
  });

  it("uses MO_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", MO_STATE_DIR: "/var/lib/mo" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/mo"));
  });

  it("expands ~ in MO_STATE_DIR", () => {
    const env = { HOME: "/Users/test", MO_STATE_DIR: "~/mo-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/mo-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { MO_STATE_DIR: "C:\\State\\mo" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\mo");
  });
});
