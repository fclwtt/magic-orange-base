...
function readServiceProxyEnvironment(
  ...
  const proxyUrl = normalizeOptionalString(env.MO_PROXY_URL);
  return proxyUrl ? { MO_PROXY_URL: proxyUrl } : {};
}
...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.MO_PROFILE;
  const wrapperPath = normalizeOptionalString(env.MO_WRAPPER);
  ...
  return {
    ...
    MO_GATEWAY_PORT: String(port),
    MO_LAUNCHD_LABEL: resolvedLaunchdLabel,
    MO_SYSTEMD_UNIT: systemdUnit,
    MO_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    MO_WINDOWS_TASK_HIDDEN_LAUNCHER: "1",
    MO_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    MO_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    MO_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = normalizeOptionalString(env.MO_GATEWAY_TOKEN);
  const allowInsecurePrivateWs = normalizeOptionalString(env.MO_ALLOW_INSECURE_PRIVATE_WS);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    MO_GATEWAY_TOKEN: gatewayToken,
    MO_ALLOW_INSECURE_PRIVATE_WS: allowInsecurePrivateWs,
    MO_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    MO_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    MO_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    MO_WINDOWS_TASK_HIDDEN_LAUNCHER: "1",
    MO_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    MO_LOG_PREFIX: "node",
    MO_SERVICE_MARKER: NODE_SERVICE_MARKER,
    MO_SERVICE_KIND: NODE_SERVICE_KIND,
    MO_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    MO_STATE_DIR: sharedEnv.stateDir,
    MO_CONFIG_PATH: sharedEnv.configPath,
    ...sharedEnv.proxyEnv,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.MO_STATE_DIR;
  const configPath = env.MO_CONFIG_PATH;
