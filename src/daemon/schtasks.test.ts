...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when MO_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".mo", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when MO_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", MO_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".mo-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers MO_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        MO_PROFILE: "rescue",
        MO_STATE_DIR: "C:\\State\\mo",
      },
      expected: path.join("C:\\State\\mo", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", MO_PROFILE: "default" },
      expected: path.join("/home/test", ".mo", "gateway.cmd"),
    },
    {
      name: "uses a custom task script file name inside the state directory",
      env: {
        USERPROFILE: "C:\\Users\\test",
        MO_TASK_SCRIPT_NAME: "gateway-node.cmd",
      },
      expected: path.join("C:\\Users\\test", ".mo", "gateway-node.cmd"),
    },
  ])("$name", ({ env, expected }) => {
  ...
  it.each([
    ...
    expect(() =>
      resolveTaskScriptPath({
        USERPROFILE: "C:\\Users\\test",
        MO_TASK_SCRIPT_NAME: scriptName,
      }),
    ).toThrow("MO_TASK_SCRIPT_NAME must be a file name only");
  });
});
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\mo",
          environment: {
            NODE_ENV: "production",
            MO_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\mo\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
  ...
  it("reads script from MO_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ MO_STATE_DIR: path.join(tmpDir, "custom-state") }),
