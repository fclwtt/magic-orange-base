...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds MO_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `mo gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service MO_GATEWAY_TOKEN does not match gateway.auth.token in mo.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (isEnvironmentFileOnlySource(command.environmentValueSources?.MO_GATEWAY_TOKEN)) {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.MO_GATEWAY_TOKEN);
}
...
function auditGatewayServiceVersion(command: GatewayServiceCommand, issues: ServiceConfigIssue[]) {
  const serviceVersion = command?.environment?.MO_SERVICE_VERSION?.trim();
