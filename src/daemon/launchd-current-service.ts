...
export function isCurrentProcessLaunchdServiceLabel(
  ...
  const configuredLabel = normalizeOptionalString(env.MO_LAUNCHD_LABEL);
  if (!configuredLabel || configuredLabel !== label) {
    return false;
  }
  if (
    normalizeOptionalString(env.MO_SERVICE_MARKER) === "mo" &&
    Boolean(normalizeOptionalString(env.MO_SERVICE_KIND))
  ) {
