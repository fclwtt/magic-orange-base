...
const EXTRA_MARKERS = ["mo", "clawdbot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  ...
  const markerKeys = ["mo_service_marker"];
  const kindKeys = ["mo_service_kind"];
...
function isOpenClawGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  if (detectLaunchdGatewayExecutionMarker(contents) !== "mo") {
    return false;
  }
  return label.startsWith("ai.mo.");
}
...
function isOpenClawGatewayTaskName(name: string): boolean {
  ...
  return stripped === defaultName || /^mo gateway \(.+\)$/.test(stripped);
}
...
async function scanLaunchdDir(params: {
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    const marker =
      hasGatewayServiceMarker(contents) || executionMarker === "mo"
        ? "mo"
    ...
    if (marker === "mo" && isOpenClawGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "mo" || isLegacyLabel(label),
    });
  }
