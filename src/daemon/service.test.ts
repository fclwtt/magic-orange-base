...
describe("resolveGatewayService", () => {
  ...
  it("guards mutating service adapters when config was written by a newer OpenClaw", async () => {
    const tempHome = await makeTempWorkspace("mo-service-future-config-");
    const stateDir = path.join(tempHome, ".mo");
    const configPath = path.join(stateDir, "mo.json");
    const envSnapshot = captureEnv(["HOME", "MO_STATE_DIR", "MO_CONFIG_PATH"]);
    try {
      ...
      process.env.MO_CONFIG_PATH = configPath;
      clearConfigCache();
...
describe("readGatewayServiceState", () => {
  it("tracks installed, loaded, and running separately", async () => {
    ...
    const state = await readGatewayServiceState(service, {
      env: { MO_GATEWAY_PORT: "1" },
    });

    expect(state.installed).toBe(true);
    expect(state.loaded).toBe(true);
    expect(state.running).toBe(true);
    expect(state.env.MO_GATEWAY_PORT).toBe("18789");
  });
  ...
  it("keeps the caller-selected service identity when merging persisted env", async () => {
    ...
    const state = await readGatewayServiceState(service, {
      env: { MO_SYSTEMD_UNIT: "mo-gateway-maintenance.service" },
    });

    expect(state.env.MO_SYSTEMD_UNIT).toBe("mo-gateway-maintenance.service");
...
describe("startGatewayService", () => {
  ...
  it("requests repair before start when the loaded service version is stale", async () => {
    const service = createService({
      readCommand: vi.fn(async () => ({
        programArguments: ["mo", "gateway", "run"],
        environment: { MO_SERVICE_VERSION: "2026.4.24" },
      })),
      isLoaded: vi.fn(async () => true),
  ...
  it("requests repair before start when the loaded service points at temporary install paths", async () => {
    const service = createService({
      readCommand: vi.fn(async () => ({
        programArguments: [
          "/private/tmp/mo-ai-install-cli-pr118/tools/node/bin/node",
          "/tmp/mo-ai-install-cli-pr118/lib/node_modules/mo/dist/index.js",
          "gateway",
        ],
        environment: {},
      })),
