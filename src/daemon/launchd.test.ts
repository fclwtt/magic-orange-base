...
describe("launchctl list detection", () => {
  it("parses stale OpenClaw updater jobs from launchctl list", () => {
    const jobs = parseLaunchctlListOpenClawUpdateJobs(
      [
        "123 0 ai.mo.gateway",
        "- 127 ai.mo.update.2026.5.12",
        "- 0 ai.mo.manual-update.1717168800",
        "8142 0 ai.mo.update.2026.5.13-beta.1",
        "- 0 ai.mo.manual-updater.1717168800",
        "- 0 com.example.other",
      ].join("\n"),
    );
    ...
    expect(jobs).toEqual([
      {
        label: "ai.mo.manual-update.1717168800",
        lastExitStatus: 0,
      },
      {
        label: "ai.mo.update.2026.5.12",
        lastExitStatus: 127,
      },
      {
        label: "ai.mo.update.2026.5.13-beta.1",
        pid: 8142,
        lastExitStatus: 0,
      },
    ]);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      state.listOutput = "- 127 ai.mo.update.2026.5.12\n";
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      state.listOutput = [
        "- 0 ai.mo.manual-update.1717168800",
        "812 0 ai.mo.manual-update.profile",
        "913 0 ai.mo.manual-update.custom-label",
      ].join("\n");

      const jobs = await findStaleOpenClawUpdateLaunchdJobs({
        MO_PROFILE: "manual-update.profile",
        MO_LAUNCHD_LABEL: "ai.mo.manual-update.custom-label",
        MO_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
        MO_SERVICE_KIND: GATEWAY_SERVICE_KIND,
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentOpenClawUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.mo.update.2026.5.12",
        }),
      ).resolves.toBe(true);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentOpenClawUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.mo.manual-update.1717168800",
        }),
      ).resolves.toBe(true);
      ...
      expect(state.launchctlCalls).toContainEqual([
        "disable",
        `${domain}/ai.mo.manual-update.1717168800`,
      ]);
      expect(launchctlCommandNames()).not.toContain("remove");
    },
  );
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentOpenClawUpdateLaunchdJob({
          MO_LAUNCHD_LABEL: "ai.mo.update.2026.5.12",
        }),
      ).resolves.toBe(true);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentOpenClawUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.mo.gateway",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentOpenClawUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.mo.manual-update.1717168800",
          MO_PROFILE: "manual-update.1717168800",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentOpenClawUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.mo.manual-update.gateway",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")("disables explicit legacy updater jobs", async () => {
    await expect(disableOpenClawUpdateLaunchdJob("ai.mo.update.2026.5.12")).resolves.toBe(
      true,
    );
  ...
  it.runIf(process.platform === "darwin")("disables explicit manual updater jobs", async () => {
    await expect(
      disableOpenClawUpdateLaunchdJob("ai.mo.manual-update.1717168800"),
    ).resolves.toBe(true);
...
describe("launchd install", () => {
  ...
  it("writes LaunchAgent environment to an owner-only env file when provided", async () => {
    const env = createDefaultLaunchdEnv();
    const tmpDir = "/Users/test/.mo/tmp";
    ...
    const plistPath = resolveLaunchAgentPlistPath(env);
    const envFilePath = "/Users/test/.mo/service-env/ai.mo.gateway.env";
    const wrapperPath = "/Users/test/.mo/service-env/ai.mo.gateway-env-wrapper.sh";
    ...
    expect(state.dirModes.get("/Users/test/.mo/service-env")).toBe(0o700);
  ...
  it("warns before overwriting a customized generated LaunchAgent env wrapper", async () => {
    ...
    expect(output).toContain("mo gateway install --wrapper <path>");
    expect(output).toContain("MO_WRAPPER");
  ...
  it("repairs a mangled label-derived service-env wrapper path on restart", async () => {
    ...
    const serviceEnv = {
      ...callerEnv,
      MO_STATE_DIR: "/Users/test/service-env/custom-state",
    };
    await installLaunchAgent({
      env: serviceEnv,
      stdout: new PassThrough(),
      ...
      environment: {
        ...
        MO_STATE_DIR: serviceEnv.MO_STATE_DIR,
      },
    });

    const plistPath = resolveLaunchAgentPlistPath(callerEnv);
    const envFilePath = "/Users/test/service-env/custom-state/service-env/ai.mo.gateway.env";
    const wrapperPath =
      "/Users/test/service-env/custom-state/service-env/ai.mo.gateway-env-wrapper.sh";
    const callerEnvFilePath = "/Users/test/.mo/service-env/ai.mo.gateway.env";
    const callerWrapperPath =
      "/Users/test/.mo/service-env/ai.mo.gateway-env-wrapper.sh";
    const mangledEnvFilePath =
      "/Users/test/service-env/custom-state/service-env/[ai.mo.gateway.env](http:/ai.mo.gateway.env)";
    const mangledWrapperPath =
      "/Users/test/service-env/custom-state/service-env/[ai.mo.gateway-env-wrapper.sh](http:/ai.mo.gateway-env-wrapper.sh)";
    state.files.set(
      plistPath,
    ...
    expect(command?.programArguments).toEqual(defaultProgramArguments);
    expect(command?.environment?.MO_GATEWAY_PORT).toBe("18789");
    expect(command?.environment?.MO_STATE_DIR).toBe(serviceEnv.MO_STATE_DIR);
    expect(command?.environmentValueSources?.MO_GATEWAY_PORT).toBe("file");
    ...
    expect(rewrittenEnv).toContain("export MO_GATEWAY_PORT='18789'");
    expect(rewrittenEnv).toContain(
      "export MO_STATE_DIR='/Users/test/service-env/custom-state'",
    );
  });
  ...
  it("writes KeepAlive=true policy with shutdown and throttle limits", async () => {
    ...
    expect(plist).toContain("<string>/Users/test/Library/Logs/mo/gateway.log</string>");
  ...
  it("stops LaunchAgent via bootout by default, preserving KeepAlive for future crashes", async () => {
    ...
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const serviceId = `${domain}/ai.mo.gateway`;
    ...
    await withProcessEnv(
      ...
      async () => {
        await expect(stopLaunchAgent({ env, stdout: new PassThrough() })).rejects.toThrow(
          "Refusing to stop LaunchAgent ai.mo.gateway from inside the same launchd service",
        );
      },
    );
    ...
    await withProcessEnv(
      {
        ...
        MO_SERVICE_MARKER: "mo",
        MO_SERVICE_KIND: "gateway",
        MO_LAUNCHD_LABEL: "ai.mo.gateway",
      },
      async () => {
        await expect(stopLaunchAgent({ env, stdout: new PassThrough() })).rejects.toThrow(
          "Refusing to stop LaunchAgent ai.mo.gateway from inside the same launchd service",
  ...
  it("allows external LaunchAgent label overrides to stop the selected target", async () => {
    const env = {
      ...
      MO_LAUNCHD_LABEL: "com.example.mo.gateway",
    };
    const stdout = new PassThrough();
    let output = "";
    stdout.on("data", (chunk: Buffer) => {
    ...
    await withProcessEnv(
      {
        ...
        MO_LAUNCHD_LABEL: undefined,
        MO_SERVICE_MARKER: undefined,
        MO_SERVICE_KIND: undefined,
      },
      async () => {
        await stopLaunchAgent({ env, stdout });
      },
    ...
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const serviceId = `${domain}/com.example.mo.gateway`;
    expect(state.launchctlCalls).toEqual([["bootout", serviceId]]);
    expect(output).toContain("Stopped LaunchAgent");
  });
  ...
  it("resolves the stop postcondition port from the stored LaunchAgent environment", async () => {
    ...
    await installLaunchAgent({
      ...
      stdout: new PassThrough(),
      programArguments: defaultProgramArguments,
      environment: { MO_GATEWAY_PORT: "19006" },
    });
    state.launchctlCalls.length = 0;
  ...
  it("reloads launchd after rewriting an existing plist", async () => {
    ...
    state.files.set(
      ...
      [
        ...
        "    <string>/Users/test/.mo-default/logs/gateway.log</string>",
        "  </dict>",
        "</plist>",
      ].join("\n"),
    );
  ...
  it("uses the stored LaunchAgent environment port for restart stale cleanup", async () => {
    ...
    await installLaunchAgent({
      ...
      stdout: new PassThrough(),
      programArguments: defaultProgramArguments,
      environment: { MO_GATEWAY_PORT: "19007" },
    });
    state.launchctlCalls.length = 0;
  ...
  it("ignores invalid stored LaunchAgent environment ports for stale cleanup", async () => {
    ...
    await installLaunchAgent({
      ...
      stdout: new PassThrough(),
      programArguments: defaultProgramArguments,
      environment: { MO_GATEWAY_PORT: "65536" },
    });
    state.launchctlCalls.length = 0;
  ...
  it("hands restart off to a detached helper when invoked from the current LaunchAgent", async () => {
    ...
    const result = await withProcessEnv({ LAUNCH_JOB_LABEL: "ai.mo.gateway" }, async () =>
      restartLaunchAgent({
        env,
        stdout: new PassThrough(),
      }),
  ...
  it("hands plist reload off when current LaunchAgent needs rewritten paths", async () => {
    ...
    expect(state.files.get(plistPath)).toContain("/Users/test/Library/Logs/mo/gateway.log");
    expect(state.launchctlCalls).toStrictEqual([]);
  });
  ...
  it("surfaces detached handoff failures", async () => {
    ...
    await expect(
      withProcessEnv({ LAUNCH_JOB_LABEL: "ai.mo.gateway" }, async () =>
        restartLaunchAgent({
          env,
          stdout: new PassThrough(),
        }),
  ...
  it("shows actionable guidance when launchctl gui domain does not support bootstrap", async () => {
    ...
    expect(message).toContain("https://docs.mo.ai/gateway");
  });
...
describe("resolveLaunchAgentPlistPath", () => {
  it.each([
    {
      name: "uses default label when MO_PROFILE is unset",
      env: { HOME: "/Users/test" },
      expected: "/Users/test/Library/LaunchAgents/ai.mo.gateway.plist",
    },
    {
      name: "uses profile-specific label when MO_PROFILE is set to a custom value",
      env: { HOME: "/Users/test", MO_PROFILE: "jbphoenix" },
      expected: "/Users/test/Library/LaunchAgents/ai.mo.jbphoenix.plist",
    },
    {
      name: "prefers MO_LAUNCHD_LABEL over MO_PROFILE",
      env: {
        HOME: "/Users/test",
        MO_PROFILE: "jbphoenix",
        MO_LAUNCHD_LABEL: "com.custom.label",
      },
      expected: "/Users/test/Library/LaunchAgents/com.custom.label.plist",
    },
    {
      name: "trims whitespace from MO_LAUNCHD_LABEL",
      env: {
        HOME: "/Users/test",
        MO_LAUNCHD_LABEL: "  com.custom.label  ",
      },
    ...
    {
      name: "ignores empty MO_LAUNCHD_LABEL and falls back to profile",
      env: {
        HOME: "/Users/test",
        MO_PROFILE: "myprofile",
        MO_LAUNCHD_LABEL: "   ",
      },
      expected: "/Users/test/Library/LaunchAgents/ai.mo.myprofile.plist",
    },
  ])("$name", ({ env, expected }) => {
