...
// Real content from the mo-gateway.service unit file (the canonical gateway unit).
...
[Service]
ExecStart=/usr/bin/node /home/mo/.npm-global/lib/node_modules/mo/dist/entry.js gateway --port 18789
Restart=always
Environment=MO_SERVICE_MARKER=mo
Environment=MO_SERVICE_KIND=gateway
Environment=MO_SERVICE_VERSION=2026.3.8

[Install]
WantedBy=default.target
`;

// Real content from the mo-test.service unit file (a non-gateway mo service).
...
After=mo-gateway.service
Requires=mo-gateway.service

[Service]
ExecStart=/usr/bin/node /opt/mo-worker/dist/index.js worker
`;

const CUSTOM_MO_GATEWAY_CONTENTS = `\
[Unit]
Description=Custom OpenClaw gateway

[Service]
ExecStart=/usr/bin/node /opt/mo/dist/entry.js gateway --port 18888
`;

describe("detectMarkerLineWithGateway", () => {
  it("returns null for mo-test.service (mo only in description, no gateway on same line)", () => {
    expect(detectMarkerLineWithGateway(TEST_SERVICE_CONTENTS)).toBeNull();
  });

  it("returns mo for the canonical gateway unit (ExecStart has both mo and gateway)", () => {
    expect(detectMarkerLineWithGateway(GATEWAY_SERVICE_CONTENTS)).toBe("mo");
  });
  ...
  it("handles line continuations — marker and gateway split across physical lines", () => {
    const contents = `[Service]\nExecStart=/usr/bin/node /opt/mo/dist/entry.js \\\n  gateway --port 18789\n`;
    expect(detectMarkerLineWithGateway(contents)).toBe("mo");
  });
  ...
  it("ignores non-gateway ExecStart commands that only pass gateway-named options", () => {
    const contents = `[Service]\nExecStart=/usr/bin/mo-helper --gateway-url http://127.0.0.1:18789 sync\n`;
...
describe("findExtraGatewayServices (linux / scanSystemdDir) — real filesystem", () => {
  ...
  it.skipIf(!isLinux)("does not report mo-test.service as a gateway service", async () => {
    const tmpHome = await fs.mkdtemp(path.join(os.tmpdir(), "mo-test-"));
    const systemdDir = path.join(tmpHome, ".config", "systemd", "user");
    try {
      await fs.mkdir(systemdDir, { recursive: true });
      await fs.writeFile(path.join(systemdDir, "mo-test.service"), TEST_SERVICE_CONTENTS);
  ...
  it.skipIf(!isLinux)(
    "does not report the canonical mo-gateway.service as an extra service",
    async () => {
      const tmpHome = await fs.mkdtemp(path.join(os.tmpdir(), "mo-test-"));
      const systemdDir = path.join(tmpHome, ".config", "systemd", "user");
      try {
        await fs.mkdir(systemdDir, { recursive: true });
        await fs.writeFile(
          path.join(systemdDir, "mo-gateway.service"),
  ...
  it.skipIf(!isLinux)(
    ...
    async () => {
      ...
      try {
        await fs.mkdir(systemdDir, { recursive: true });
        await fs.writeFile(
          path.join(systemdDir, "mo-companion.service"),
  ...
  it.skipIf(!isLinux)(
    "reports custom-named gateway units that execute mo gateway",
    async () => {
      const tmpHome = await fs.mkdtemp(path.join(os.tmpdir(), "mo-test-"));
      const systemdDir = path.join(tmpHome, ".config", "systemd", "user");
      const unitPath = path.join(systemdDir, "custom-mo.service");
      try {
        await fs.mkdir(systemdDir, { recursive: true });
        await fs.writeFile(unitPath, CUSTOM_MO_GATEWAY_CONTENTS);
        const result = await findExtraGatewayServices({ HOME: tmpHome });
        expect(result).toEqual([
          {
            platform: "linux",
            label: "custom-mo.service",
            detail: `unit: ${unitPath}`,
            scope: "user",
            marker: "mo",
            legacy: false,
...
<key>KeepAlive</key><dict><key>OtherJobEnabled</key><dict><key>ai.mo.gateway</key><true/></dict></dict>
<key>ProgramArguments</key><array><string>/usr/local/bin/mo-helper</string><string>sync</string></array>
</dict></plist>`,
      );
      const result = await findExtraGatewayServices({ HOME: tmpHome });
      expect(result).toStrictEqual([]);
...
<key>ProgramArguments</key><array><string>/usr/local/bin/mo-helper</string><string>--gateway-url</string><string>http://127.0.0.1:18789</string><string>sync</string></array>
</dict></plist>`,
      );
      const result = await findExtraGatewayServices({ HOME: tmpHome });
      expect(result).toStrictEqual([]);
...
<key>ProgramArguments</key><array><string>/opt/homebrew/bin/watchman</string><string>--foreground</string></array>
  ...
  it("reports custom LaunchAgents that execute mo gateway", async () => {
    const tmpHome = await fs.mkdtemp(path.join(os.tmpdir(), "mo-test-"));
    const launchdDir = path.join(tmpHome, "Library", "LaunchAgents");
    const plistPath = path.join(launchdDir, "com.example.mo-gateway.plist");
    try {
      await fs.mkdir(launchdDir, { recursive: true });
      await fs.writeFile(
        plistPath,
        `<?xml version="1.0" encoding="UTF-8"?>
<plist version="1.0"><dict>
<key>Label</key><string>com.example.mo-gateway</string>
<key>ProgramArguments</key><array><string>/usr/local/bin/mo</string><string>gateway</string><string>--port</string><string>18888</string></array>
</dict></plist>`,
      );
      const result = await findExtraGatewayServices({ HOME: tmpHome });
      expect(result).toEqual([
...
describe("findExtraGatewayServices (win32)", () => {
  ...
  it("collects only non-mo marker tasks from schtasks output", async () => {
    ...
    execSchtasksMock.mockResolvedValueOnce({
      ...
      stdout: [
        ...
        "Task To Run: C:\\Program Files\\OpenClaw\\mo.exe gateway run",
        "",
  ...
  it("reports duplicate root tasks that only share the gateway task prefix", async () => {
    execSchtasksMock.mockResolvedValueOnce({
      ...
      stdout: [
        ...
        "Task To Run: C:\\Program Files\\OpenClaw\\mo.exe gateway run --profile dev",
        "",
    ...
    expect(result).toEqual([
      {
        ...
        detail:
          "task: \\OpenClaw Gateway Backup, run: C:\\Program Files\\OpenClaw\\mo.exe gateway run",
        scope: "system",
        marker: "mo",
        legacy: false,
      },
