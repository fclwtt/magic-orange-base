...
function createAgentDir(agentId: string, includeNestedAgentDir = true) {
  const stateDir = process.env.MO_STATE_DIR;
  if (!stateDir) {
    throw new Error("MO_STATE_DIR is not set");
  }
...
describe("structured state integrity findings", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["HOME", "MO_HOME", "MO_STATE_DIR"]);
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "mo-doctor-state-integrity-"));
    setTestEnvValue("HOME", tempHome);
    setTestEnvValue("MO_HOME", tempHome);
  ...
  it("reports permissive state and config file permissions as structured findings", () => {
    ...
    const stateDir = path.join(tempHome, ".mo");
    const configPath = path.join(tempHome, "mo.json");
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    deleteTestEnvValue("MO_OAUTH_DIR");
    deleteTestEnvValue("MO_AGENT_DIR");
  ...
  it("prompts for oauth dir when MO_OAUTH_DIR is explicitly configured", async () => {
    process.env.MO_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: OpenClawConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(hasRepairPromptMessage(confirmRuntimeRepair, "Create OAuth dir at")).toBe(true);
  ...
  it("does not warn when MO_AGENT_DIR points at the live compatibility agent dir", async () => {
    ...
    setTestEnvValue("MO_AGENT_DIR", legacyAgentDir);
  ...
  it("warns about tombstoned subagent restart recovery sessions", async () => {
    ...
    expect(text).toContain("mo tasks maintenance --apply");
  ...
  it("does not warn when a case-mismatched dir resolves to the configured agent path", async () => {
    ...
    const resolvedResearchAgentDir = realpathNative(
      path.join(process.env.MO_STATE_DIR ?? "", "agents", "Research", "agent"),
    );
    const realpathSpy = vi
      .spyOn(fs.realpathSync, "native")
      .mockImplementation((target, options) => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      try {
        const symlinkStateDir = path.join(symlinkHome, ".mo");
        setTestEnvValue("HOME", symlinkHome);
        setTestEnvValue("MO_HOME", symlinkHome);
        setTestEnvValue("MO_STATE_DIR", symlinkStateDir);
  ...
  it("prints mo-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: OpenClawConfig = {};
    writeSessionStore(cfg, {
      "agent:main:main": {
    ...
    expect(text).toMatch(/mo sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/mo sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /mo sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
  ...
  it("does not let synthetic heartbeat metadata override mixed transcript history", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-heartbeat-main-mixed-"));
    try {
  ...
  it("does not let heartbeat-looking routing metadata skip mixed transcript checks", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-heartbeat-main-route-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
      fs.writeFileSync(
        transcriptPath,
  ...
  it("does not classify transcripts with real user activity after 400 heartbeat messages", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-heartbeat-main-cap-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
  ...
  it("keeps the heartbeat main-session helper conservative", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-heartbeat-main-helper-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
      fs.writeFileSync(
        transcriptPath,
  ...
  it("moves store entries and clears matching TUI pointers without touching others", () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-tui-pointer-clear-"));
    try {
