...
describe("status.command-sections", () => {
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:mo security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:mo security audit --deep)");
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:mo status --all",
      "  Need to debug live? cmd:mo logs --follow",
      "  Fix reachability first: cmd:mo gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:mo devices approve req-123)",
      "muted(Fallback: cmd:mo devices approve --latest)",
      "muted(Inspect: cmd:mo devices list)",
    ]);
  });
