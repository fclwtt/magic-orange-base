 ...
 * Mirrors the real `mo config <sub>` surface so the message only ever
...
const CONFIGURE_NON_TTY_HINT = [
  ...
  `  ${formatCliCommand("mo config set <path> <value>")}  write a config entry`,
  `  ${formatCliCommand("mo config get <path>")}          read a config entry`,
  `  ${formatCliCommand("mo config patch")}              apply a JSON patch`,
  `  ${formatCliCommand("mo config validate")}           validate configuration`,
].join("\n");
...
export async function configureCommandFromSectionsArg(
  ...
  // Fail closed once at the shared entry: both `mo configure` and the
  // no-subcommand `mo config` route here, so a single guard keeps them
  ...
  if (invalid.length > 0) {
    runtime.error(
      `Invalid --section: ${invalid.join(", ")}. Expected one of: ${CONFIGURE_WIZARD_SECTIONS.join(", ")}. Run ${formatCliCommand("mo configure")} without --section to use the full wizard.`,
    );
    runtime.exit(1);
    return;
  }
