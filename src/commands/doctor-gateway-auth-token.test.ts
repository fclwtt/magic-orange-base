...
describe("resolveGatewayAuthTokenForService", () => {
  ...
  it("skips exec SecretRefs by default for service token checks", async () => {
    const tmp = await fs.mkdtemp(join(tmpdir(), "mo-service-token-exec-ref-"));
  ...
  it("falls back to MO_GATEWAY_TOKEN when SecretRef is unresolved", async () => {
    const resolved = await resolveGatewayAuthTokenForService(
      {
        gateway: {
          auth: {
      ...
      {
        MO_GATEWAY_TOKEN: "env-fallback-token",
      } as NodeJS.ProcessEnv,
    );
  ...
  it("falls back to MO_GATEWAY_TOKEN when SecretRef resolves to empty", async () => {
    const resolved = await resolveGatewayAuthTokenForService(
      {
        gateway: {
          auth: {
...
describe("shouldRequireGatewayTokenForInstall", () => {
    ...
    const required = shouldRequireGatewayTokenForInstall(
      {
        ...
        env: {
          vars: {
            MO_GATEWAY_PASSWORD: "configured-password", // pragma: allowlist secret
          },
        },
      } as OpenClawConfig,
      {} as NodeJS.ProcessEnv,
  ...
  it("does not require token in inferred mode when password env exists in state-dir .env", async () => {
    await withTempHome(async (_home) => {
      await writeStateDirDotEnv("MO_GATEWAY_PASSWORD=dotenv-password\n", {
        env: process.env,
      });
