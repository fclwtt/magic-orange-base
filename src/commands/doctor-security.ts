  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("mo approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectPlaintextConfigSecretWarnings(cfg: OpenClawConfig): string[] {
  ...
  return [
    "- WARNING: mo.json contains plaintext secret-bearing config fields.",
    `  Paths: ${pathLine}`,
    "  Agents or workspace tools that can read config files may see these API keys/tokens.",
    `  Migrate them to SecretRefs with ${formatCliCommand("mo secrets configure")} or ${formatCliCommand("mo secrets apply")}, then verify with ${formatCliCommand("mo secrets audit --check")}.`,
  ];
}
...
export async function collectSecurityWarnings(
  ...
  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      ...
      `  Check local policy with: ${formatCliCommand("mo approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.mo.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("mo configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("mo config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("mo doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "mo config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("mo config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('mo config set session.dmScope "per-channel-peer"') +
...
export async function noteSecurityWarnings(cfg: OpenClawConfig) {
  ...
  const auditHint = `- Run: ${formatCliCommand("mo security audit --deep")}`;
