...
/** Prompt for configured channel sections to remove from mo.json. */
export async function removeChannelConfigWizard(
  ...
  while (true) {
    ...
    if (configured.length === 0) {
      note(
        [
          "No channel config found in mo.json.",
          `Tip: \`${formatCliCommand("mo channels status")}\` shows what is configured and enabled.`,
        ].join("\n"),
        "Remove channel",
      );
      return next;
