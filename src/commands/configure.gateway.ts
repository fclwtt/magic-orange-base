    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "MO_GATEWAY_TOKEN",
          placeholder: "MO_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = normalizeOptionalString(value) ?? "";
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like MO_GATEWAY_TOKEN.";
            }
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        ...
        "Docs: https://docs.mo.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
