...
describe("readServiceStatusSummary", () => {
  it("marks OpenClaw-managed services as installed", async () => {
    const summary = await readServiceStatusSummary(
      createService({
        ...
        readCommand: vi.fn(async () => ({ programArguments: ["mo", "gateway", "run"] })),
  ...
  it("passes command environment to runtime and loaded checks", async () => {
    const isLoaded = vi.fn(async ({ env }: GatewayServiceEnvArgs) => {
      return env?.MO_GATEWAY_PORT === "18789";
    });
    const readRuntime = vi.fn(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.MO_GATEWAY_PORT === "18789" ? ("running" as const) : ("unknown" as const),
    }));
    ...
    const summary = await readServiceStatusSummary(
      createService({
        ...
        readCommand: vi.fn(async () => ({
          programArguments: ["mo", "gateway", "run", "--port", "18789"],
    ...
    expect(loadedArgs?.env?.MO_GATEWAY_PORT).toBe("18789");
    const runtimeEnv = requireMockArg(readRuntime, "readRuntime") as NodeJS.ProcessEnv;
    expect(runtimeEnv?.MO_GATEWAY_PORT).toBe("18789");
    expect(summary.installed).toBe(true);
  ...
  it("includes service layout diagnostics and flags source checkout entrypoints", async () => {
    await withTempDir({ prefix: "mo-status-service-layout-" }, async (root) => {
      ...
      const serviceFile = path.join(root, "mo-gateway.service");
      ...
      expect(layout.sourcePathReal).toBe(path.join(realRoot, "mo-gateway.service"));
