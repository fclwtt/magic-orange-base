...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/mo-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/mo-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi.fn(
      (agentDir?: string) => `${agentDir ?? "/tmp/mo-agent"}/auth-profiles.json`,
    ),
...
vi.mock("../../agents/workspace.js", () => ({
  resolveDefaultAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/mo-agent/workspace"),
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/mo-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.configPath).toBe("/tmp/mo-dev/mo.json");
    expect(payload.auth.storePath).toBe("/tmp/mo-agent/auth-profiles.json");
  ...
  it("honors MO_AGENT_DIR when no --agent override is provided", async () => {
    const localRuntime = createRuntime();
    mocks.resolveAgentDir.mockClear();
    await withEnvAsync({ MO_AGENT_DIR: "/tmp/mo-isolated-agent" }, async () => {
      await modelsStatusCommand({ json: true }, localRuntime as never);
    });

    expect(mocks.resolveAgentDir).not.toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/mo-isolated-agent");
    const payload = parseFirstJsonLog(localRuntime);
    expect(payload.agentDir).toBe("/tmp/mo-isolated-agent");
    expect(payload.auth.storePath).toBe("/tmp/mo-isolated-agent/auth-profiles.json");
  });

  it("honors deprecated PI_CODING_AGENT_DIR when MO_AGENT_DIR is unset", async () => {
    const localRuntime = createRuntime();
    mocks.resolveAgentDir.mockClear();
    await withEnvAsync(
      {
        MO_AGENT_DIR: undefined,
    ...
    expect(mocks.resolveAgentDir).not.toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/mo-legacy-agent");
    const payload = parseFirstJsonLog(localRuntime);
    expect(payload.agentDir).toBe("/tmp/mo-legacy-agent");
  });
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    await withAgentScopeOverrides(
      ...
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/mo-agent-custom");
        ...
        expect(openAiCodex?.effective).toEqual({
          kind: "profiles",
          detail: "/tmp/mo-agent-custom/auth-profiles.json",
        });
      },
    );
  });
  ...
  it("reports Gemini CLI OAuth for canonical Google text routed through the CLI runtime", async () => {
    ...
    try {
      ...
      expect(
        ...
        kind: "profiles",
        detail: "/tmp/mo-agent/auth-profiles.json",
      });
      expect(payload.auth.runtimeAuthRoutes).toEqual([
        {
          provider: "google",
  ...
  it("includes auth-evidence-only providers in the auth overview", async () => {
    ...
    mocks.resolveEnvApiKey.mockImplementation(
      (provider: string, _env?: NodeJS.ProcessEnv, options?: { workspaceDir?: string }) =>
        provider === "workspace-cloud" && options?.workspaceDir === "/tmp/mo-agent/workspace"
          ? {
