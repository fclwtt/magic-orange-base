...
vi.mock("../channels/plugins/bundled.js", async () => {
  ...
  function detectTelegramAllowFromMigration(params: {
    ...
    const root = params.env.MO_STATE_DIR;
    if (!root) {
      return [];
    }
...
async function makeTempRoot() {
  const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "mo-doctor-"));
  tempRoots.push(root);
  return root;
}
...
async function runTelegramAllowFromMigration(params: { root: string; cfg: OpenClawConfig }) {
  ...
  const detected = await detectLegacyStateMigrations({
    cfg: params.cfg,
    env: { MO_STATE_DIR: params.root } as NodeJS.ProcessEnv,
  });
...
function createLegacyAgentDatabaseRegistry(stateDir: string): string {
  const stateDatabasePath = path.join(stateDir, "state", "mo.sqlite");
  ...
  try {
    db.exec(`
      ...
      INSERT INTO agent_databases (
        ...
        'worker-1',
        '/legacy/worker-1/mo-agent.sqlite',
        1,
        10,
        20
      );
...
async function runLegacyStateMigrationsForRoot(root: string) {
  const detected = await detectLegacyStateMigrations({
    cfg: {},
    env: { MO_STATE_DIR: root } as NodeJS.ProcessEnv,
  });
...
describe("doctor legacy state migrations", () => {
      ...
      expect(() =>
        db.exec(`
          INSERT INTO agent_databases (
            ...
            'worker-1',
            '/relocated/worker-1/mo-agent.sqlite',
            1,
            20,
            30
          )
  ...
  it("migrates legacy ACP metadata from sessions.json into shared SQLite", async () => {
    ...
    const sqlite = requireNodeSqlite();
    const db = new sqlite.DatabaseSync(path.join(root, "state", "mo.sqlite"));
    try {
      const row = db
        .prepare(
  ...
  it("imports debug proxy capture storage from shipped environment overrides", async () => {
    ...
    const env = {
      MO_STATE_DIR: root,
      MO_DEBUG_PROXY_DB_PATH: sourcePath,
      MO_DEBUG_PROXY_BLOB_DIR: blobDir,
  ...
  it("ignores a legacy debug proxy override that points at shared state", async () => {
    const root = await makeTempRoot();
    const sharedStatePath = path.join(root, "state", "mo.sqlite");
    const state = openOpenClawStateDatabase({
      env: { MO_STATE_DIR: root } as NodeJS.ProcessEnv,
    });
    state.db
      ...
      .run("shared-session", 100, "proxy-run", "mo", "mo");
    const detected = await detectLegacyStateMigrations({
      cfg: {},
      env: {
        MO_STATE_DIR: root,
        MO_DEBUG_PROXY_DB_PATH: sharedStatePath,
      } as NodeJS.ProcessEnv,
    });
  ...
  it("leaves debug proxy sources in place when a session id conflicts", async () => {
    ...
    state.db
      ...
      .run("legacy-session", 999, "different", "mo", "mo");
  ...
  it("archives exact legacy npm install record when SQLite has authoritative resolved metadata", async () => {
    ...
    await writeExistingPluginInstallIndex(root, {
      discord: {
        source: "npm",
        spec: "@mo/discord@latest",
        resolvedName: "@mo/discord",
  ...
  it("auto-migrates the plugin-state sidecar when custom agent dirs skip session migration", async () => {
    ...
    const result = await autoMigrateLegacyState({
      ...
      env: {
        MO_STATE_DIR: root,
        MO_AGENT_DIR: path.join(root, "custom-agent"),
      } as NodeJS.ProcessEnv,
      log: { info: vi.fn(), warn: vi.fn() },
    });
  ...
  it("migrates default exec approvals into an explicit state dir", async () => {
    ...
    const sourcePath = path.join(root, ".mo", "exec-approvals.json");
    const legacySocketPath = path.join(root, ".mo", "exec-approvals.sock");
    ...
    const detected = await detectLegacyStateMigrations({
      cfg: {},
      env: { MO_STATE_DIR: stateDir } as NodeJS.ProcessEnv,
      homedir: () => root,
    });
  ...
  it("keeps task sidecars when shared state already has conflicting task rows", async () => {
    ...
    await withStateDir(root, async () => {
      const sqlite = requireNodeSqlite();
      const sharedPath = path.join(root, "state", "mo.sqlite");
