...
describe("noteWorkspaceStatus", () => {
  ...
  it("collects plugin version drift as structured findings", async () => {
    ...
    expect(findings).toEqual([
      expect.objectContaining({
        ...
        fixHint: expect.stringContaining("mo plugins update codex"),
      }),
    ]);
  });
  ...
  it("collects compatibility warnings, plugin diagnostics, and TaskFlow recovery findings", async () => {
    ...
    expect(findings).toEqual([
      ...
      expect.objectContaining({
        ...
        fixHint: expect.stringContaining("mo tasks flow show flow-123"),
      }),
    ]);
  });
  ...
  it("surfaces active official managed plugin version drift", async () => {
    ...
    try {
      ...
      expect(body).toContain("mo plugins update codex");
      expect(body).toContain("mo gateway restart");
    } finally {
      noteSpy.mockRestore();
    }
  });
  ...
  it("uses package-version update commands for exact npm plugin drift", async () => {
    ...
    try {
      ...
      expect(driftCalls).toHaveLength(1);
      const [[body]] = driftCalls;
      expect(body).toContain("mo plugins update @mo/brave-plugin@2026.6.10-beta.1");
      expect(body).not.toContain("mo plugins update brave");
      expect(body).toContain("mo gateway restart");
    } finally {
      noteSpy.mockRestore();
    }
  ...
  it("adds TaskFlow recovery hints for broken blocked flows", async () => {
    ...
    try {
      ...
      expect(body).toContain("mo tasks flow show <flow-id>");
    } finally {
      noteSpy.mockRestore();
    }
  });
