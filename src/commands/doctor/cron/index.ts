  ...
  if (state.legacyStoreDetected) {
    ...
    if (archiveResult.ok) {
      ...
      for (const failure of archiveResult.failures) {
        warnings.push(
          `Migrated cron jobs to SQLite but could not archive the legacy cron file at ${shortenHomePath(failure.path)}: ${failure.reason}. Remove it manually or rerun ${formatCliCommand("mo doctor --fix")} to retry.`,
        );
      }
    }
...
export async function maybeRepairLegacyCronStore(params: {
  ...
  try {
    ...
    note(
      [
        ...
        `- ${reason}`,
        `Fix the file's permissions or contents and re-run ${formatCliCommand("mo doctor")}; later health checks will continue.`,
      ].join("\n"),
      "Cron",
    );
    return;
  ...
  if (rawJobs.length === 0) {
    ...
    note(
      [
        ...
        ...previewLines,
        `Repair with ${formatCliCommand("mo doctor --fix")} to finish the migration.`,
      ].join("\n"),
      "Cron",
    );
  ...
  note(
    [
      noteHeading,
      ...previewLines,
      `Repair with ${formatCliCommand("mo doctor --fix")} to normalize the store before the next scheduler run.`,
    ].join("\n"),
    "Cron",
  );
