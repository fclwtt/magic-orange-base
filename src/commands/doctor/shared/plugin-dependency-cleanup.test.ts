...
describe("cleanupLegacyPluginDependencyState", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-plugin-deps-cleanup-"));
  });
  ...
  it("collects and removes legacy plugin dependency state roots", async () => {
    ...
    const explicitStageDir = path.join(stateDir, ".mo-install-stage-explicit");
    ...
    const legacyExtensionStamp = path.join(
      ...
      "extensions",
      "demo",
      ".mo-runtime-deps-stamp.json",
    );
    const legacyManifest = path.join(
      packageRoot,
      "extensions",
      "demo",
      ".mo-runtime-deps.json",
    );
    ...
    const env = {
      ...
      MO_PLUGIN_STAGE_DIR: explicitStageDir,
  ...
  it("removes configured plugin stage roots outside OpenClaw roots", async () => {
    ...
    await fs.writeFile(
      path.join(stageRoot, "node_modules", "ansi-escapes", ".mo-rename-tmp"),
  ...
  it("refuses explicit plugin stage paths with parent segments", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
    const dotDotStage = `${stateDir}${path.sep}..${path.sep}.mo-install-stage-dotdot`;
    ...
    const result = await cleanupLegacyPluginDependencyState({
      env: {
        MO_STATE_DIR: stateDir,
        MO_PLUGIN_STAGE_DIR: dotDotStage,
      },
      packageRoot,
    });
  ...
  it("does not follow symlinked extension roots outside OpenClaw roots", async () => {
    ...
    await fs.writeFile(path.join(externalPlugin, ".mo-runtime-deps.json"), "{}");
  ...
  it("does not unlink global runtime symlinks through unsafe cleanup roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "prefix", "lib", "node_modules", "mo");
