...
import { isRecord as hasRecord } from "@mo/normalization-core/record-coerce";
...
export async function collectCodexNativeAssetInfoNotes(params: {
  ...
  return [
    [
      ...
      "- If the Codex plugin is not installed, run `mo plugins install npm:@mo/codex` first. Then run `mo migrate plan codex` to inventory them. Applying that migration copies skills into the current OpenClaw agent workspace; Codex plugins, hooks, and config stay manual-review only.",
    ].join("\n"),
  ];
}
