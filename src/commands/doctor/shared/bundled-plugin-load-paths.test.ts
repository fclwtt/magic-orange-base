...
import { bundledDistPluginRootAt, bundledPluginRootAt } from "mo/plugin-sdk/test-fixtures";
...
describe("bundled plugin load path repair", () => {
  beforeEach(() => {
    const packageRoot = "/app/node_modules/mo";
  ...
  it("removes stale bundled paths from old versioned OpenClaw package roots", () => {
    const currentPackageRoot = path.resolve("node_modules", "mo");
    const stalePackageRoot = path.resolve(
      "pnpm-global",
      ".pnpm",
      "mo@2026.3.28_@napi-rs+canvas@0.1.97",
      "node_modules",
      "mo",
    );
  ...
  it("formats a doctor hint for legacy bundled plugin paths", () => {
    ...
    expect(warnings).toEqual([
      ...
      '- Run "mo doctor --fix" to remove these redundant bundled plugin paths.',
    ]);
  });

  it("ignores bundled plugins that already resolve to source extensions", () => {
    const sourcePath = path.resolve("repo", "mo", "extensions", "feishu");
    vi.spyOn(bundledSources, "resolveBundledPluginSources").mockReturnValue(
