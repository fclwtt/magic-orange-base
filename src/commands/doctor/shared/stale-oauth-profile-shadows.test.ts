...
describe("stale OAuth profile shadow doctor repair", () => {
  const envSnapshot = captureEnv(["MO_STATE_DIR", "MO_HOME"]);
  let tempRoot = "";
  let stateDir = "";

  beforeEach(async () => {
    clearRuntimeAuthProfileStoreSnapshots();
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mo-stale-oauth-shadow-"));
    stateDir = path.join(tempRoot, "state");
    process.env.MO_STATE_DIR = stateDir;
    process.env.MO_HOME = stateDir;
  });
  ...
  it("warns about stale local OAuth shadows without modifying the child store", async () => {
    ...
    expect(warnings[0]).toContain("mo doctor --fix");
  ...
  it("uses the injected env for the main auth store", async () => {
    ...
    const injectedEnv = {
      ...process.env,
      MO_STATE_DIR: injectedStateDir,
      MO_HOME: injectedStateDir,
    };
    saveAuthProfileStore(
      storeWith(
        profileId,
