...
import { AGENT_MODEL_CONFIG_KEYS } from "@mo/model-catalog-core/configured-model-refs";
import { normalizeProviderId } from "@mo/model-catalog-core/provider-id";
import { asOptionalRecord as asMutableRecord } from "@mo/normalization-core/record-coerce";
import { normalizeOptionalLowercaseString as normalizeString } from "@mo/normalization-core/string-coerce";
...
function formatLegacyLosslessCompactionWarning(params: {
  ...
  return [
    ...
    ...configLines,
    params.canAutoFix
      ? "- Run `mo doctor --fix`: it migrates legacy Lossless compaction config to the Lossless context-engine slot."
...
function formatDisabledCodexPluginWarning(params: {
  ...
  const fixHint = params.blockedOutsideEntry
    ...
    : "- Run `mo doctor --fix`: it enables plugins.entries.codex, or set the affected OpenAI models to an OpenClaw runtime policy.";
  return [
  ...
  if (hits.length > 0) {
    warnings.push(
      [
        ...
        "- Run `mo doctor --fix`: it rewrites configured model refs and stale sessions to `openai/*`, moves Codex intent to provider/model runtime policy, and clears old whole-agent runtime pins.",
      ].join("\n"),
    );
  }
  ...
  if (fixableHits.length > 0) {
    warnings.push(
      formatUnsupportedCompactionWarning({
        hits: fixableHits,
        fixHint:
          "- Run `mo doctor --fix`: it removes unsupported Codex compaction overrides.",
      }),
    );
  }
  return warnings;
...
function clearStaleSessionRuntimePins(entry: SessionEntry): boolean {
  ...
  let changed = false;
  if (entry.agentHarnessId !== undefined && harnessRuntime !== "mo") {
    delete entry.agentHarnessId;
    changed = true;
  }
  if (entry.agentRuntimeOverride !== undefined && overrideRuntime !== "mo") {
        ...
        stale.length > 0
          ? [
              [
                ...
                "- Run `mo doctor --fix` to rewrite stale session model/provider pins across all agent session stores.",
              ].join("\n"),
            ]
          : [],
      changes: [],
