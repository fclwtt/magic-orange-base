...
const MO_BETA_COMPANION_VERSION_RE = /^(\d{4}\.[1-9]\d?\.[1-9]\d?)-beta\.[1-9]\d*$/;
const MO_STABLE_OR_BETA_COMPANION_VERSION_RE =
...
function appendClawHubRiskAcknowledgementGuidance(params: {
  ...
  return `${params.message} To review and acknowledge this ClawHub package, run \`mo plugins install ${shellSpec} --acknowledge-clawhub-risk\` from a trusted shell, then rerun repair.`;
}
...
function betaCompanionMatchesCurrentStableVersion(params: {
  ...
  const installedBase = MO_BETA_COMPANION_VERSION_RE.exec(params.installedVersion)?.[1];
  const currentBase = MO_STABLE_OR_BETA_COMPANION_VERSION_RE.exec(params.currentVersion)?.[1];
  ...
  switch (issue.kind) {
    case "missing-install-record":
      return {
        ...
        target,
        fixHint: `Run \`mo doctor --fix\` to install ${issue.installSpec}.`,
      };
    case "missing-installed-payload":
      return {
        ...
        fixHint: "Run `mo doctor --fix` to reinstall the configured plugin package.",
      };
    case "repairable-installed-plugin":
      return {
        ...
        target,
        ...(issue.installPath ? { path: issue.installPath } : {}),
        fixHint: "Run `mo doctor --fix` to repair the configured plugin package.",
      };
    case "stale-version-bound-runtime":
      return {
        ...
        target,
        ...(issue.installPath ? { path: issue.installPath } : {}),
        fixHint: "Run `mo doctor --fix` to refresh the configured runtime plugin.",
      };
    case "stale-channel-config-descriptor":
      return {
        ...
        target,
        ...(issue.installPath ? { path: issue.installPath } : {}),
        fixHint: "Run `mo doctor --fix` to repair the configured plugin install metadata.",
      };
    case "deferred-package-manager-repair":
      return {
        ...
        target,
        ...(issue.installPath ? { path: issue.installPath } : {}),
        fixHint: "Rerun `mo doctor --fix` after the package update completes.",
      };
  }
...
async function repairMissingPluginInstalls(params: {
  ...
  if (shouldDeferConfiguredPluginInstallRepair(env)) {
    ...
    for (const pluginId of updateDeferredPluginIds) {
      ...
      const detail = `Skipped package-manager repair for configured plugin "${pluginId}" during package update; rerun "mo doctor --fix" after the update completes.`;
      changes.push(detail);
