...
function isRuntimeDependencyMarkerName(name: string): boolean {
  return (
    name === ".mo-runtime-deps.json" ||
    name === ".mo-runtime-deps-stamp.json" ||
    name.startsWith(".mo-runtime-deps-")
  );
}

function isInstallStageDebrisName(name: string): boolean {
  return /^\.mo-install-stage(?:-.+)?$/u.test(name);
}
...
function collectExplicitStageTargets(env: NodeJS.ProcessEnv): CleanupTarget[] {
  return splitPathList(env.MO_PLUGIN_STAGE_DIR).map((entry) => ({
    kind: "explicit-stage",
...
async function hasOpenClawRenameResidue(root: string): Promise<boolean> {
  ...
  if (await isFile(path.join(nodeModulesRoot, ".mo-rename-tmp"))) {
    return true;
  }
  ...
  for (const entry of entries) {
    ...
    if (await isFile(path.join(entryPath, ".mo-rename-tmp"))) {
      return true;
    }
    ...
    for (const scopedEntry of scopedEntries) {
      ...
      if (await isFile(path.join(entryPath, scopedEntry.name, ".mo-rename-tmp"))) {
        return true;
      }
    }
  }
