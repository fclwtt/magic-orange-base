...
const LEGACY_OAUTH_SECRET_KEY_ENV = "MO_AUTH_PROFILE_SECRET_KEY";
...
function buildLegacyOAuthSecretKey(seed: string): Buffer {
  ...
  return hash("sha256", `mo:auth-profile-oauth:${seed}`, "buffer");
}
...
function resolveLegacyOAuthSecretKeyFileCandidates(env: NodeJS.ProcessEnv): string[] {
  if (process.platform === "win32") {
    ...
    return uniquePaths([
      ...
      home
        ? path.join(home, ".mo-auth-profile-secrets", LEGACY_OAUTH_SECRET_KEY_FILE_NAME)
        : undefined,
    ]);
  }
  ...
  return uniquePaths([
    root ? path.join(root, "mo", LEGACY_OAUTH_SECRET_KEY_FILE_NAME) : undefined,
    home
      ? path.join(home, ".mo-auth-profile-secrets", LEGACY_OAUTH_SECRET_KEY_FILE_NAME)
      : undefined,
  ]);
...
function resolveLegacyOAuthSecretKeySeeds(env: NodeJS.ProcessEnv): string[] {
  ...
  if (env.NODE_ENV === "test" && env.VITEST === "true") {
    addSeed("mo-test-oauth-profile-secret-key");
  }
...
function emitKeychainOnlyMigrationHintOnce(profileId: string): void {
  ...
  log.warn(
    "Legacy Codex OAuth credentials are stored only in macOS Keychain on this host. " +
      "Headless paths cannot prompt for Keychain access; run `mo doctor --fix` " +
