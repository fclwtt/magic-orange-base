...
function resolveTestConfigPath() {
  const override = process.env.MO_CONFIG_PATH?.trim();
  if (override) {
    return override;
  }
  const stateDir = process.env.MO_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("MO_STATE_DIR must be set before config IO in this test");
  }
  return path.join(stateDir, "mo.json");
}
...
vi.mock("./onboard-helpers.js", () => {
  ...
  return {
    DEFAULT_WORKSPACE: "/tmp/mo-workspace",
    ...
    runNonInteractiveSetup(
      {
        nonInteractive: true,
        mode: "local",
        workspace: path.join(stateDir, "mo"),
        authChoice: "skip",
        skipSkills: true,
        skipHealth: false,
        installDaemon: true,
...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    setTestEnvValue("MO_SKIP_CHANNELS", "1");
    setTestEnvValue("MO_SKIP_GMAIL_WATCHER", "1");
    setTestEnvValue("MO_SKIP_CRON", "1");
    setTestEnvValue("MO_SKIP_CANVAS_HOST", "1");
    setTestEnvValue("MO_SKIP_BROWSER_CONTROL_SERVER", "1");
    deleteTestEnvValue("MO_GATEWAY_TOKEN");
    deleteTestEnvValue("MO_GATEWAY_PASSWORD");

    tempHome = await makeTempWorkspace("mo-onboard-");
  ...
  it("preserves existing agents.list and bindings on onboard rerun (mo#84692)", async () => {
    await withStateDir("state-preserve-agents-", async (stateDir) => {
      const workspace = path.join(stateDir, "mo");
      const seededAgents = [
  ...
  it("preserves existing agents.list and bindings on remote onboard rerun (mo#84692)", async () => {
  ...
  it("explains local health failure when no daemon was requested", async () => {
    await withStateDir("state-local-health-hint-", async (stateDir) => {
      ...
      await expect(
        ...
        /only waits for an already-running gateway unless you pass `--install-daemon` to `mo onboard`[\s\S]*mo onboard --install-daemon[\s\S]*mo onboard --skip-health/,
      );
    });
  }, 60_000);
  ...
  it("emits structured JSON diagnostics when daemon health fails", async () => {
    await withStateDir("state-local-daemon-health-json-fail-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Run `mo gateway status --deep` for more detail.");
  ...
  it("classifies daemon health ECONNREFUSED failures with a recovery command", async () => {
    await withStateDir("state-local-daemon-health-refused-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Fix: run `mo gateway restart`.");
    });
  }, 60_000);
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    await withStateDir("state-lan-", async (stateDir) => {
      setTestEnvValue("MO_STATE_DIR", stateDir);
      setTestEnvValue("MO_CONFIG_PATH", path.join(stateDir, "mo.json"));
