...
vi.mock("node:child_process", async () => {
  const { mockNodeChildProcessSpawnSync } = await import("mo/plugin-sdk/test-node-mocks");
...
describe("doctor WhatsApp responsiveness", () => {
  ...
  it("lists only verified local TUI processes", () => {
    spawnSyncMock.mockReturnValue({
      ...
      stdout: [
        " 101 mo-tui",
        " 102 /usr/bin/node /usr/lib/node_modules/mo/dist/index.js gateway --port 18789",
        " 103 mo channels",
        " 104 mo tui --local",
        " 105 /usr/bin/mo chat",
        " 106 helper --note 'mo tui'",
        " 107 mo-helper mo terminal",
        " 108 mo --flag tui",
      ].join("\n"),
    });

    expect(listLocalTuiProcesses()).toEqual([
      { pid: 101, command: "mo-tui" },
      { pid: 104, command: "mo tui --local" },
      { pid: 105, command: "/usr/bin/mo chat" },
    ]);
  });
  ...
  it("terminates stale local TUI processes with a kill fallback", async () => {
    ...
    await expect(
      terminateLocalTuiProcesses({
        processes: [{ pid: 101, command: "mo-tui" }],
        controller,
        graceMs: 0,
      }),
  ...
  it("warns and repairs local TUI pressure when WhatsApp is enabled and the gateway is degraded", async () => {
    ...
    await noteWhatsappResponsivenessHealth({
      ...
      shouldRepair: true,
      listLocalTuiProcesses: () => [{ pid: 101, command: "mo-tui" }],
