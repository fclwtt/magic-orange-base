...
vi.mock("mo/plugin-sdk/provider-auth", () => ({
  isNonSecretApiKeyMarker: () => false,
}));

vi.mock("mo/plugin-sdk/provider-model-shared", () => ({
  DEFAULT_CONTEXT_TOKENS: 32768,
  normalizeProviderId: (value: string) => normalizeLowercaseStringOrEmpty(value),
}));

vi.mock("mo/plugin-sdk/provider-stream-shared", () => ({
...
beforeEach(() => {
  ...
  setStdinTty(true);
  originalStateDir = process.env.MO_STATE_DIR;
  originalUpdateInProgress = process.env.MO_UPDATE_IN_PROGRESS;
  process.env.MO_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-doctor-state-"));
  process.env.MO_STATE_DIR = tempStateDir;
