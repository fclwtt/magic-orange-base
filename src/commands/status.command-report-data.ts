// Builds the data model for the standard `mo status` text report.
...
export async function buildStatusCommandReportData(
  ...
  const securityAuditLines = params.securityAudit
    ...
    : [
        params.theme.muted(
          `Skipped in fast status. Full report: ${params.formatCliCommand("mo security audit")}`,
        ),
        params.theme.muted(`Deep probe: ${params.formatCliCommand("mo status --deep")}`),
      ];
  ...
  return {
    ...
    taskMaintenanceHint: `Task maintenance: ${params.formatCliCommand("mo tasks maintenance --apply")}`,
