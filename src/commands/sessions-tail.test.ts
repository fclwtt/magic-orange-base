...
describe("sessionsTailCommand", () => {
  ...
  beforeEach(() => {
    ...
    previousTrajectoryDir = process.env.MO_TRAJECTORY_DIR;
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-sessions-tail-"));
    process.env.MO_STATE_DIR = path.join(tmpDir, "state");
    delete process.env.MO_TRAJECTORY_DIR;
  ...
  afterEach(() => {
    ...
    if (previousTrajectoryDir === undefined) {
      ...
      process.env.MO_TRAJECTORY_DIR = previousTrajectoryDir;
    }
  ...
  it("uses a session trajectory pointer for relocated runtime files", async () => {
    ...
    fs.writeFileSync(
      ...
      `${JSON.stringify({
        traceSchema: "mo-trajectory-pointer",
        schemaVersion: 1,
        sessionId: "session-one",
  ...
  it("resolves the target store from a fully qualified non-default agent session key", async () => {
    ...
    const opsSessionsDir = path.join(process.env.MO_STATE_DIR!, "agents", "ops", "sessions");
