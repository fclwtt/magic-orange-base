...
describe("buildCleanupPlan", () => {
  test("resolves inside-state flags and workspace dirs", () => {
    ...
    const defaultWorkspace = path.join(tmpRoot, "mo-workspace-default");
    const opsWorkspace = path.join(tmpRoot, "mo-workspace-ops");
    const cfg = {
      agents: {
    ...
    const plan = buildCleanupPlan({
      ...
      stateDir: path.join(tmpRoot, "mo-state"),
      configPath: path.join(tmpRoot, "mo-state", "mo.json"),
      oauthDir: path.join(tmpRoot, "mo-oauth"),
    });
  ...
  test("includes implicit per-agent workspaces under the state dir", () => {
    const tmpRoot = path.join(path.parse(process.cwd()).root, "tmp", "mo-cleanup-plan");
    ...
    return withEnvAsync(
      ...
      async () => {
        const plan = buildCleanupPlan({
          cfg: cfg as unknown as OpenClawConfig,
          stateDir,
          configPath: path.join(stateDir, "mo.json"),
...
describe("cleanup path removals", () => {
  ...
  it("removes state and only linked paths outside state", async () => {
    ...
    const tmpRoot = path.join(path.parse(process.cwd()).root, "tmp", "mo-cleanup");
    await removeStateAndLinkedPaths(
      {
        stateDir: path.join(tmpRoot, "state"),
        configPath: path.join(tmpRoot, "state", "mo.json"),
    ...
    expect(runtime.log.mock.calls.map(([line]) => line.replaceAll("\\", "/"))).toEqual([
      "[dry-run] remove /tmp/mo-cleanup/state",
      "[dry-run] remove /tmp/mo-cleanup/oauth",
    ]);
  });

  it("preserves nested workspace paths during state-only removal", async () => {
    const runtime = createRuntimeMock();
    const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mo-cleanup-"));
    const stateDir = path.join(tmpRoot, ".mo");
  ...
  it("removes every workspace directory", async () => {
    const runtime = createRuntimeMock();
    const workspaces = ["/tmp/mo-workspace-1", "/tmp/mo-workspace-2"];

    await removeWorkspaceDirs(workspaces, runtime, { dryRun: true });

    const logs = runtime.log.mock.calls.map(([line]) => line);
    expect(logs).toEqual([
      "[dry-run] remove /tmp/mo-workspace-1",
      "[dry-run] remove /tmp/mo-workspace-2",
    ]);
  });

  it("removes owned legacy workspace attestations", async () => {
    const runtime = createRuntimeMock();
    const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mo-cleanup-attest-"));
  ...
  it("refuses to remove a directory containing the current working directory", async () => {
    const runtime = createRuntimeMock();
    const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mo-cleanup-cwd-"));
