...
describe("normalizeCompatibilityConfigValues", () => {
  ...
  beforeAll(() => {
    previousOauthDir = process.env.MO_OAUTH_DIR;
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-oauth-"));
    process.env.MO_OAUTH_DIR = tempOauthDir;
  });
  ...
  afterAll(() => {
    if (previousOauthDir === undefined) {
      ...
      process.env.MO_OAUTH_DIR = previousOauthDir;
    }
    fs.rmSync(tempOauthDir, { recursive: true, force: true });
  });
        ...
        list: [
          {
            ...
            models: {
              "anthropic/claude-opus-4-7": { agentRuntime: { id: "mo" } },
            },
          },
        ],
      },
