...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("collects clear unsetenv instructions for token override", async () => {
    const getenv = vi.fn(async (name: string) =>
      name === "MO_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(warning).toContain("MO_GATEWAY_TOKEN");
    expect(warning).toContain("launchctl unsetenv MO_GATEWAY_TOKEN");
    expect(warning).not.toContain("MO_GATEWAY_PASSWORD");
  });
  ...
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    expect(message).toContain("MO_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv MO_GATEWAY_TOKEN");
    expect(message).not.toContain("MO_GATEWAY_PASSWORD");
  });
  ...
  it("treats SecretRef-backed credentials as configured", async () => {
    const noteFn = vi.fn();
    const getenv = vi.fn(async (name: string) =>
      name === "MO_GATEWAY_PASSWORD" ? "launchctl-password" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("MO_GATEWAY_PASSWORD");
  });
...
describe("noteMacStaleOpenClawUpdateLaunchdJobs", () => {
  it("collects stale updater job cleanup guidance on macOS", async () => {
    ...
    expect(warning).toContain("ai.mo.update.2026.5.12");
    expect(warning).toContain("ai.mo.manual-update.1717168800");
    expect(warning).toContain("launchctl remove <label>");
    expect(warning).toContain("mo gateway restart");
  });
  ...
  it("prints stale updater job cleanup guidance on macOS", async () => {
    ...
    expect(message).toContain("ai.mo.update.2026.5.12");
    expect(message).toContain("ai.mo.manual-update.1717168800");
    expect(message).toContain("launchctl remove <label>");
    expect(message).toContain("mo gateway restart");
  });
