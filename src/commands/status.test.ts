...
beforeAll(() => {
  envSnapshot = captureEnv(["MO_PROFILE"]);
  process.env.MO_PROFILE = "isolated";
});
...
async function createStatusServiceSummary(
  ...
  return {
    ...
    wrapperPath: command?.environment?.MO_WRAPPER?.trim() || undefined,
  };
}
  ...
  return {
    ...
    update: {
      ...
      deps: {
        manager: "pnpm",
        status: "ok",
        lockfilePath: "/tmp/mo/pnpm-lock.yaml",
        markerPath: "/tmp/mo/node_modules/.modules.yaml",
      },
    ...
    gatewayProbeAuth: process.env.MO_GATEWAY_TOKEN
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.mo.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.mo.node.plist",
    }),
  }),
}));
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.MO_GATEWAY_TOKEN?.trim();
...
vi.mock("../infra/mo-root.js", () => ({
  resolveOpenClawPackageRoot: vi.fn().mockResolvedValue("/tmp/mo"),
  resolveOpenClawPackageRootSync: vi.fn(() => "/tmp/mo"),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines with verbose cache details", async () => {
    ...
    expectLogsMatch(logs, /mo (?:--profile isolated )?status --all/);
  ...
  it("shows node-only gateway info when no local gateway service is installed", async () => {
    ...
    expect(joined).toContain("mo --profile isolated node status");
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("MO_GATEWAY_TOKEN", "abcd1234", async () => {
  ...
  it("notes when secret diagnostics may come from a CLI process outside the service wrapper context", async () => {
    ...
    mocks.resolveGatewayService.mockReturnValue({
      ...
      readCommand: async () => ({
        ...
        environment: { MO_WRAPPER: wrapperPath },
        sourcePath: "/tmp/Library/LaunchAgents/ai.mo.gateway.plist",
      }),
    });
    mocks.loadConfig.mockReturnValue({
    ...
    await withOptionalEnvVar("MO_WRAPPER", undefined, async () => {
      const logs = await runStatusAndGetLogs();
      expectLogsInclude(logs, "Secret diagnostics:");
      expectLogsInclude(logs, "installed gateway service uses MO_WRAPPER");
    ...
    await withEnvVar("MO_WRAPPER", wrapperPath, async () => {
      const logs = await runStatusAndGetLogs();
      expectLogsInclude(logs, "Secret diagnostics:");
