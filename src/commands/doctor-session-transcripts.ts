...
export function sessionTranscriptIssueToHealthFinding(
  ...
  return {
    ...
    path: issue.filePath,
    fixHint:
      "To clean up the advisory artifact, run `mo doctor --fix` to rewrite affected transcripts to their active branch.",
  };
}
...
export async function noteSessionTranscriptHealth(params?: {
  ...
  if (!shouldRepair) {
    lines.push('- Run "mo doctor --fix" to rewrite affected files to their active branch.');
