// Implements docs link/search output for `mo docs`.
...
const SEARCH_API = "https://docs.mo.ai/api/search";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.mo.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('mo docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.mo.ai/");
      runtime.log(`Search: ${formatCliCommand('mo docs "your query"')}`);
    }
    return;
  }
