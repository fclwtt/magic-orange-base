...
export function formatUnavailableSkillDoctorLines(skills: SkillStatusEntry[]): string[] {
  ...
  lines.push(`Disable unused skills: ${formatCliCommand("mo doctor --fix")}`);
  lines.push(
    `Inspect details: ${formatCliCommand("mo skills check --agent <id>")} or ${formatCliCommand("mo skills info <name> --agent <id>")}`,
  );
  return lines;
}
