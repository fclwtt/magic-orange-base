...
function readPluginManifestId(packageDir: string): string | undefined {
  const manifest = readJsonObject(path.join(packageDir, "mo.plugin.json"));
  ...
  if (!params.prompter.shouldRepair) {
    note(
      [
        ...
        `Repair with ${formatCliCommand("mo doctor --fix")} to remove stale managed npm packages and rebuild the plugin registry.`,
      ].join("\n"),
      "Plugin registry",
    );
    return false;
  ...
  if (!params.prompter.shouldRepair) {
    note(
      [
        ...
        `Repair with ${formatCliCommand("mo doctor --fix")} to remove stale local install records and rebuild the plugin registry.`,
      ].join("\n"),
      "Plugin registry",
    );
    return [];
    ...
    if (issues.length > 0) {
      note(
        [
          ...
          `Repair with ${formatCliCommand("mo doctor --fix")} to relink managed npm plugin packages.`,
        ].join("\n"),
        "Plugin registry",
      );
    }
  ...
  switch (issue.kind) {
    case "registry-missing-or-stale":
      return {
        ...
        path: issue.path,
        fixHint: "Run `mo doctor --fix` to rebuild the plugin registry from enabled plugins.",
      };
    case "stale-managed-npm-bundled-plugin":
      return {
        ...
        fixHint:
          "Run `mo doctor --fix` to remove stale managed npm packages and rebuild the plugin registry.",
      };
    case "stale-local-bundled-plugin-install-record":
      return {
        ...
        target: issue.pluginId,
        fixHint:
          "Run `mo doctor --fix` to remove stale local install records and rebuild the plugin registry.",
      };
    case "managed-npm-mo-peer-link":
      return {
        checkId: PLUGIN_REGISTRY_CHECK_ID,
        severity: "warning",
        ...
        fixHint: "Run `mo doctor --fix` to relink managed npm plugin packages.",
      };
  }
...
export function pluginRegistryIssueToRepairEffect(
  ...
  switch (issue.kind) {
    ...
    case "managed-npm-mo-peer-link":
      return {
        kind: "package",
        action: "would-relink-managed-npm-mo-peer",
        target: issue.packageDir,
        dryRunSafe: false,
      };
  }
    ...
    if (preflight.action === "migrate") {
      note(
        [
          ...
          `Repair with ${formatCliCommand("mo doctor --fix")} to rebuild ${shortenHomePath(preflight.filePath)} from enabled plugins.`,
        ].join("\n"),
        "Plugin registry",
      );
    }
