...
describe("status-all diagnosis port checks", () => {
  ...
  it("treats same-process dual-stack loopback listeners as healthy", async () => {
    const params = createBaseParams([
      { pid: 5001, commandLine: "mo-gateway", address: "127.0.0.1:18789" },
      { pid: 5001, commandLine: "mo-gateway", address: "[::1]:18789" },
    ]);
  ...
  it("keeps warning for multi-process listener conflicts", async () => {
    const params = createBaseParams([
      { pid: 5001, commandLine: "mo-gateway", address: "127.0.0.1:18789" },
      { pid: 5002, commandLine: "mo-gateway", address: "[::1]:18789" },
    ]);
  ...
  it("adds direct update restart guidance for failed update sentinels", async () => {
    ...
    expect(output).toContain(
      "Update restart: failed · managed-service-handoff-failed · run mo gateway status --deep",
    );
    expect(output).toContain("Update restart failed; run mo gateway status --deep.");
    expect(output).toContain(
      "If the service is down, run mo gateway restart or mo gateway install --force.",
    );
  });
  ...
  it("adds direct update restart guidance for pending update sentinels", async () => {
    ...
    const output = params.lines.join("\n");
    expect(output).toContain(
      "Update restart: restart pending health verification · run mo gateway status --deep",
    );
    expect(output).toContain(
      "Update restart is still pending; run mo update status --json for handoff state.",
    );
  });
  ...
  it("does not read or display stale stderr tails on Darwin", async () => {
    ...
    try {
      ...
      restartLogMocks.resolveGatewayRestartLogPath.mockReturnValue(
        "/tmp/mo/logs/gateway-restart.log",
      );
      ...
      expect(gatewayMocks.readFileTailLines).not.toHaveBeenCalledWith(
        "/Users/test/Library/Logs/mo/gateway.err.log",
        40,
      );
      expect(output).toContain("# stdout: /Users/test/Library/Logs/mo/gateway.log");
