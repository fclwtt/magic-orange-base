...
function printGatewayNotReadyHints(runtime: RuntimeEnv, reason: string): void {
  runtime.log(reason);
  runtime.log("Run `mo gateway status --deep` for details.");
  runtime.log("Run `mo gateway start` to start a managed gateway.");
  runtime.log("Run `mo gateway run` for a foreground gateway.");
}
