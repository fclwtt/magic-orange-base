...
function isTempBackedOpenClawRoot(segments: readonly string[]): boolean {
  ...
  const moIndex = lower.lastIndexOf("mo");
  if (moIndex < 1) {
    return false;
  }
  return lower[moIndex - 1] === "tmp" || lower[moIndex - 1] === "temp";
}
...
function isBundledRuntimeSkillsPath(cachedPath: string, skillRootIndex: number): boolean {
  ...
  return (
    lower.some(
      (segment) =>
        segment === "dist-runtime" || segment === "node_modules" || segment.startsWith("mo@"),
...
export function sessionSnapshotIssueToHealthFinding(
  ...
  return {
    ...
    fixHint:
      "To clean up the advisory artifact, run `mo doctor --fix` to rewrite stale cached session metadata paths, or start a fresh session after confirming history can be retired.",
  };
}
