...
import {
  ...
  normalizeProviderId,
} from "@mo/model-catalog-core/provider-id";
...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("mo doctor --fix")} or ${formatCliCommand("mo memory status --fix")}`
    : `Verify: ${formatCliCommand("mo memory status --deep")}`;
  return [
 ...
 * Runs as part of `mo doctor` — config-only checks where possible;
...
export async function noteMemorySearchHealth(
        ...
        noteFn(
          [
            ...
            workspaceProbeFailed
              ...
              : `- Set an explicit binary path: ${formatCliCommand("mo config set memory.qmd.command /absolute/path/to/qmd")}`,
            `- Or switch back to builtin memory: ${formatCliCommand("mo config set memory.backend builtin")}`,
            "",
            `Verify: ${formatCliCommand("mo memory status --deep")}`,
          ]
            .filter(Boolean)
      ...
      noteFn(
        [
          ...
          `- Enable QMD session export: ${formatCliCommand(
            "mo config set memory.qmd.sessions.enabled true",
          )}`,
  ...
  if (provider === "local") {
    ...
    noteFn(
      [
        ...
        "",
        "Fix (pick one):",
        `- Install the llama.cpp provider plugin: ${formatCliCommand("mo plugins install @mo/llama-cpp-provider")}`,
        `- Set a local GGUF model path in config`,
        suggestedRemoteProvider
          ? `- Switch to a remote provider: ${formatCliCommand(`mo config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
  ...
  if (
    ...
    noteFn(
      [
        ...
        "",
        "Fix:",
        `- ${formatCliCommand("mo config set agents.defaults.memorySearch.remote.baseUrl http://127.0.0.1:1234/v1")}`,
        "",
        `Verify: ${formatCliCommand("mo memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  if (isOpenAICompatibleMemoryProvider(provider, cfg) && !normalizeOptionalString(resolved.model)) {
    noteFn(
      [
        ...
        "",
        "Fix:",
        `- ${formatCliCommand("mo config set agents.defaults.memorySearch.model text-embedding-bge-m3")}`,
        "",
        `Verify: ${formatCliCommand("mo memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  if (isKeyOptionalMemoryProvider(provider, cfg)) {
    ...
    // not warn. A skipped probe means the user ran `mo doctor` without
  ...
  noteFn(
    [
      ...
      `- Configure credentials: ${formatCliCommand("mo configure --section model")}`,
      `- To disable: ${formatCliCommand("mo config set agents.defaults.memorySearch.enabled false")}`,
      "",
      `Verify: ${formatCliCommand("mo memory status --deep")}`,
    ].join("\n"),
    "Memory search",
