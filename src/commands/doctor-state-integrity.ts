...
import { asNullableObjectRecord } from "@mo/normalization-core/record-coerce";
import { normalizeOptionalLowercaseString } from "@mo/normalization-core/string-coerce";
...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidates = [".mo"].map((dir) => path.resolve(root, entry.name, dir));
...
export function formatLinuxSdBackedStateDirWarning(
  ...
  return [
    ...
    "- For better startup and state durability, prefer SSD/NVMe (or USB SSD on Raspberry Pi) for MO_STATE_DIR.",
  ].join("\n");
}
...
export function formatLinuxVolatileStateDirWarning(
  ...
  return [
    ...
    "- Move MO_STATE_DIR to a persistent filesystem to avoid data loss.",
  ].join("\n");
}
...
export function detectMacCloudSyncedStateDir(
  ...
  // MO_HOME can relocate app data defaults, but iCloud/CloudStorage remain under the OS home.
  ...
  switch (issue.kind) {
    case "mac-cloud-state-dir":
      return {
        ...
        path: issue.path,
        fixHint: "Move MO_STATE_DIR to local non-synced storage such as ~/.mo.",
      };
    case "linux-sd-state-dir":
      return {
        ...
        fixHint: "Move MO_STATE_DIR to SSD/NVMe-backed storage.",
      };
    case "linux-volatile-state-dir":
      return {
        ...
        path: issue.path,
        target: issue.mountPoint,
        fixHint: "Move MO_STATE_DIR to persistent local storage.",
      };
    ...
    case "state-dir-not-writable":
      return {
        ...
        path: issue.path,
        fixHint: "Run `mo doctor --fix` to repair state directory permissions.",
      };
    case "state-dir-too-open":
      return {
        ...
        path: issue.path,
        fixHint: "Run `mo doctor --fix` to tighten state directory permissions.",
      };
    case "config-file-too-open":
      return {
        ...
        path: issue.path,
        fixHint: "Run `mo doctor --fix` to tighten config file permissions.",
      };
    case "missing-runtime-dir":
      return {
        ...
        path: issue.path,
        fixHint: "Run `mo doctor --fix` to create missing runtime state directories.",
      };
    case "runtime-dir-not-writable":
      return {
        ...
        path: issue.path,
        fixHint: "Run `mo doctor --fix` to repair runtime state directory permissions.",
      };
  }
  ...
  if (cloudSyncedStateDir) {
    warnings.push(
      [
        ...
        "- Prefer a local non-synced state dir (for example: ~/.mo).",
        `  Set locally: MO_STATE_DIR=~/.mo ${formatCliCommand("mo doctor")}`,
      ].join("\n"),
    );
  }
  if (linuxSdBackedStateDir) {
    ...
    if (missing.length > 0) {
      warnings.push(
        [
          ...
          `  Verify sessions in store: ${formatCliCommand(`mo sessions --store "${absoluteStorePath}"`)}`,
          `  Preview cleanup impact: ${formatCliCommand(`mo sessions cleanup --store "${absoluteStorePath}" --dry-run`)}`,
          `  Prune missing entries: ${formatCliCommand(`mo sessions cleanup --store "${absoluteStorePath}" --enforce --fix-missing`)}`,
        ].join("\n"),
      );
    }
      ...
      warnings.push(
        [
          ...
          `  Examples: ${wedgedSubagentSessions
            ...
            .map(([key]) => key)
            .join(", ")}`,
          `  Fix: ${formatCliCommand("mo tasks maintenance --apply")}`,
        ].join("\n"),
      );
...
export function collectWorkspaceBackupTip(workspaceDir: string): string | null {
  ...
  return [
    ...
    "- Keep ~/.mo out of git; it contains credentials and session history.",
