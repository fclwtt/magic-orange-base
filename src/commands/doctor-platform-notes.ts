...
export function collectMacLaunchAgentOverrideWarning(deps?: {
  ...
  const markerCandidates = [path.join(home, ".mo", "disable-launchagent")];
...
export async function collectMacStaleOpenClawUpdateLaunchdJobsWarning(deps?: {
  ...
  return [
    ...
    `  ${formatCliCommand("mo gateway restart")}`,
  ].join("\n");
}
...
export async function collectMacLaunchctlGatewayEnvOverrideWarning(
  ...
  const tokenEntries = [
    ["MO_GATEWAY_TOKEN", await getenv("MO_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["MO_GATEWAY_PASSWORD", await getenv("MO_GATEWAY_PASSWORD")],
  ] as const;
  ...
  return [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "MO_GATEWAY_PASSWORD"}\` is set; it can make local clients use a different password than gateway.auth.password.`
      : undefined,
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = normalizeOptionalString(env.MO_NO_RESPAWN) ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- MO_NO_RESPAWN is not set to 1; set it when you want routine gateway restarts to stay in-process instead of handing off to a managed supervisor.",
    );
  }
  ...
  const suggestions = [
    ...
    "  export NODE_COMPILE_CACHE=/var/tmp/mo-compile-cache",
    "  mkdir -p /var/tmp/mo-compile-cache",
    "  export MO_NO_RESPAWN=1",
