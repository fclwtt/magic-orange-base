...
describe("status.scan.config-shared", () => {
  beforeEach(() => {
    ...
    mocks.resolveConfigPath.mockReturnValue(
      `/tmp/mo-status-scan-config-shared-missing-${process.pid}.json`,
    );
  });
  ...
  it("adds a status diagnostic for gateway token source conflicts", async () => {
    ...
    const result = await loadStatusScanCommandConfig({
      ...
      readConfigSnapshot,
      resolveConfig,
      env: { VITEST: "true", MO_GATEWAY_TOKEN: "env-token" },
      allowMissingConfigFastPath: true,
    });

    expect(result.secretDiagnostics).toEqual([
      "MO_GATEWAY_TOKEN conflicts with gateway.auth.token: Remove MO_GATEWAY_TOKEN from the shell, ~/.mo/.env, or launchctl env if gateway.auth.token is intended, or point gateway.auth.token at ${MO_GATEWAY_TOKEN} if the env var should be canonical.",
    ]);
  });
  ...
  it("does not add a status diagnostic when config uses MO_GATEWAY_TOKEN", async () => {
    const sourceConfig = {
