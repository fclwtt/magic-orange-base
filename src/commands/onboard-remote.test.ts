...
const INSECURE_WS_URL_MESSAGE =
  ...
  "Break-glass: MO_ALLOW_INSECURE_PRIVATE_WS=1 for trusted private networks.";
...
describe("promptRemoteGatewayConfig", () => {
  const envSnapshot = captureEnv(["MO_ALLOW_INSECURE_PRIVATE_WS"]);
  ...
  it("allows ws:// hostname remote URLs when MO_ALLOW_INSECURE_PRIVATE_WS=1", async () => {
    process.env.MO_ALLOW_INSECURE_PRIVATE_WS = "1";
    const text: WizardPrompter["text"] = vi.fn(async (params) => {
      if (params.message === "Gateway WebSocket URL") {
        expect(params.validate?.("ws://mo-gateway.ai:18789")).toBeUndefined();
        expect(params.validate?.("ws://1.1.1.1:18789")).toBe(INSECURE_WS_URL_MESSAGE);
        return "ws://mo-gateway.ai:18789";
      }
      return "";
    }) as WizardPrompter["text"];
    ...
    expect(next.gateway?.mode).toBe("remote");
    expect(next.gateway?.remote?.url).toBe("ws://mo-gateway.ai:18789");
  });

  it("supports storing remote auth as an external env secret ref", async () => {
    process.env.MO_GATEWAY_TOKEN = "remote-token-value";
    const text: WizardPrompter["text"] = vi.fn(async (params) => {
      if (params.message === "Gateway WebSocket URL") {
        return "wss://remote.example.com:18789";
      }
      if (params.message === "Environment variable name") {
        return "MO_GATEWAY_TOKEN";
      }
      return "";
    }) as WizardPrompter["text"];
