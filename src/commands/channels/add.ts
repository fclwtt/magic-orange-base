// Implements guided and non-interactive `mo channels add` account setup.
...
async function channelsAddCommandImpl(
  ...
  if (!channel) {
    const hint = catalogEntry
      ? `Plugin ${catalogEntry.meta.label} could not be loaded after install. Run mo doctor --fix, then retry mo channels add.`
  ...
  if (!plugin?.setup?.applyAccountConfig) {
    runtime.error(
      ...
      })} Run ${formatCliCommand("mo channels add")} with no flags for guided setup.`,
    );
    runtime.exit(1);
    return;
  }
