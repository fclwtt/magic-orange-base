 ...
 *
 *   - RED: ACP-keyed session resolves to `id: "mo"`, `source: "implicit"`.
 ...
 *
 * Result: the old metadata resolver fell through to the implicit "mo"
...
describe("sessions --json agentRuntime classifier (catalog #18)", () => {
  it("RED→GREEN: ACP session key is no longer misclassified (overlay applies)", () => {
    ...
    // resolved metadata said id="mo", source="implicit".
    ...
    expect(
      agentRuntime.id,
      `ACP session ${ACP_SESSION_KEY} should no longer be misclassified as "auto" or "mo". ` +
  ...
  it("FIX-SHAPE expectation: ACP session should resolve to 'acpx'", () => {
    ...
    // from "mo" and reflects the actual runtime driving the session.
