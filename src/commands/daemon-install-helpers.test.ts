...
describe("resolveGatewayDevMode", () => {
  it("detects dev mode for src ts entrypoints", () => {
    expect(resolveGatewayDevMode(["node", "/Users/me/mo/src/cli/index.ts"])).toBe(true);
    expect(resolveGatewayDevMode(["node", "C:\\Users\\me\\mo\\src\\cli\\index.ts"])).toBe(
      true,
    );
    expect(resolveGatewayDevMode(["node", "/Users/me/mo/dist/cli/index.js"])).toBe(false);
  });
});
...
function mockNodeGatewayPlanFixture(
  ...
  const {
    ...
    supported = true,
    warning,
    serviceEnvironment = { MO_PORT: "3000" },
  } = params;
...
describe("buildGatewayInstallPlan", () => {
  // Prevent tests from reading the developer's real ~/.mo/.env when
  ...
  it("uses provided nodePath and returns plan", async () => {
    ...
    expect(plan.environment).toEqual({ MO_PORT: "3000" });
  ...
  it("adds the active mo command bin directory to the managed service PATH", async () => {
    mockNodeGatewayPlanFixture();
    const originalArgv = process.argv;
    const moBinPath = path.join(isolatedHome, ".npm-global", "bin", "mo");
    process.argv = ["node", moBinPath, "gateway", "install"];
    ...
    ).toStrictEqual(["/opt/homebrew/opt/node/bin", path.dirname(moBinPath)]);
  });
  ...
  it("uses the state dir as the default macOS launchd working directory", async () => {
    ...
    expect(plan.workingDirectory).toBe(path.join(isolatedHome, ".mo"));
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledOnce();
  ...
  it("passes MO_WRAPPER through program args and managed service env", async () => {
    const wrapperPath = path.resolve("/usr/local/bin/mo-doppler");
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        MO_PORT: "3000",
        MO_WRAPPER: wrapperPath,
      },
    });
    ...
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledOnce();
    expect(
      firstMockArg(mocks.buildServiceEnvironment, "buildServiceEnvironment").env?.MO_WRAPPER,
    ).toBe(wrapperPath);
    expect(plan.environment.MO_WRAPPER).toBe(wrapperPath);
  });

  it("clears a Windows wrapper env that points at the generated gateway.cmd script", async () => {
    const selfWrapperPath = path.join(isolatedHome, ".mo", "gateway.cmd");
    const warn = vi.fn();
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        MO_PORT: "3000",
    ...
    expect(plan.environment.MO_WRAPPER).toBeUndefined();
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining(
        "Ignoring MO_WRAPPER because it points to the Windows task script",
      ),
    );
  });
  ...
  it("tracks safe config env keys without embedding literal values", async () => {
    ...
    expect(plan.environment.MO_PORT).toBe("3000");
    expect(plan.environment.MO_SERVICE_MANAGED_ENV_KEYS).toBe(
  ...
  it("includes env SecretRef values from config into the service environment", async () => {
    ...
    expect(plan.environment.MO_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
  ...
  it("allows safe inherited passEnv names while blocking dangerous exec SecretRef env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        BASH_ENV: "/tmp/mo-test-bashenv",
        XDG_CONFIG_HOME: "/tmp/mo-test-xdg-home",
  ...
  it("does not embed gateway auth SecretRef values into the service environment", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        MO_GATEWAY_TOKEN: "gateway-test-token",
      }),
      port: 3000,
      runtime: "node",
      config: {
        gateway: {
          auth: {
            token: { source: "env", provider: "default", id: "MO_GATEWAY_TOKEN" },
          },
        },
      },
    });

    expect(plan.environment.MO_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environment.MO_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
  ...
  it("does not inline config env SecretRef values already backed by state-dir dotenv", async () => {
    await writeStateDirDotEnv("DISCORD_BOT_TOKEN=discord-dotenv-token\n", {
      stateDir: path.join(isolatedHome, ".mo"),
    });
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        MO_PORT: "3000",
    ...
    expect(plan.environment.MO_SERVICE_MANAGED_ENV_KEYS).toBe("DISCORD_BOT_TOKEN");
  });
...
describe("buildGatewayInstallPlan — dotenv merge", () => {
  ...
  it("retains .env values for macOS LaunchAgent env SecretRefs", async () => {
    ...
    expect(plan.environment.MO_SERVICE_MANAGED_ENV_KEYS).toBe("MINIMAX_API_KEY");
  });
  ...
  it("works when .env file does not exist", async () => {
    mockNodeGatewayPlanFixture({ serviceEnvironment: { MO_PORT: "3000" } });
  ...
  it("preserves safe custom vars from an existing service env and merges PATH", async () => {
    ...
    expect(plan.environment.MO_SERVICE_MARKER).toBeUndefined();
  });
  ...
  it("drops keys that were previously tracked as managed service env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        BLOGWATCHER_HOME: "/Users/test/.blogwatcher",
        GOPATH: "/Users/test/.local/gopath",
        MO_SERVICE_MANAGED_ENV_KEYS: "GOBIN,GOPATH",
      },
    });
  ...
  it("drops legacy inline env values when the key is now managed by .env", async () => {
    ...
    expect(plan.environment.MO_SERVICE_MANAGED_ENV_KEYS).toBe("TAVILY_API_KEY");
  ...
  it("keeps source metadata for EnvironmentFile-backed preserved vars", async () => {
    ...
    expect(plan.environment.MO_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environmentValueSources?.MO_GATEWAY_TOKEN).toBeUndefined();
  });
  ...
  it("does not embed auth-profile env refs when the key is already durable", async () => {
    ...
    expect(plan.environment.MO_SERVICE_MANAGED_ENV_KEYS).toBe("OPENAI_API_KEY");
  });
});
...
describe("gatewayInstallErrorHint", () => {
  it("returns platform-specific hints", () => {
    ...
    expect(gatewayInstallErrorHint("linux")).toMatch(
      /(?:mo|mo)( --profile isolated)? gateway install/,
    );
  });
});
...
describe("collectPreservedExistingServiceEnvVars — operator opt-in allowlist", () => {
  ...
  it("continues to drop stale MO_ALLOW_ROOT", () => {
    const result = collectPreservedExistingServiceEnvVars(
      { MO_ALLOW_ROOT: "1" },
      managedKeys,
    );
    expect(result.MO_ALLOW_ROOT).toBeUndefined();
  });

  it("preserves MO_CLI_CONTAINER_BYPASS and MO_CONTAINER_HINT", () => {
    const result = collectPreservedExistingServiceEnvVars(
      {
        MO_CLI_CONTAINER_BYPASS: "1",
        MO_CONTAINER_HINT: "ci",
      },
      managedKeys,
    );
    expect(result.MO_CLI_CONTAINER_BYPASS).toBe("1");
    expect(result.MO_CONTAINER_HINT).toBe("ci");
  });

  it("still drops arbitrary MO_FOO", () => {
    const result = collectPreservedExistingServiceEnvVars({ MO_FOO: "bar" }, managedKeys);
    expect(result.MO_FOO).toBeUndefined();
  });

  it("preserves container opt-ins while dropping unrelated MO_* keys", () => {
    const result = collectPreservedExistingServiceEnvVars(
      {
        MO_CLI_CONTAINER_BYPASS: "1",
        MO_CONTAINER_HINT: "ci",
    ...
    expect(result.MO_CLI_CONTAINER_BYPASS).toBe("1");
    expect(result.MO_CONTAINER_HINT).toBe("ci");
    expect(result.MO_BAZ).toBeUndefined();
  });
});
