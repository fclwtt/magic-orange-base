...
const mocks = vi.hoisted(() => ({
  ...
  readConfigFileSnapshot: vi.fn(async () => ({ path: "/tmp/mo.json" })),
  ...
  resolveMissingOfficialExternalChannelPluginRepairHint: ({ channelId }: { channelId: string }) =>
    mocks.missingOfficialExternalChannels.has(channelId)
      ? {
          ...
          channelId,
          label: "Feishu",
          installSpec: "@mo/feishu",
          installCommand: "mo plugins install @mo/feishu",
          doctorFixCommand: "mo doctor --fix",
          repairHint:
            "Install the official external plugin with: mo plugins install @mo/feishu, or run: mo doctor --fix.",
        }
      : null,
}));
...
describe("channelsStatusCommand SecretRef fallback flow", () => {
  ...
  it("shows missing official external plugin repair hints in config-only output", async () => {
    ...
    expect(joined).toContain(
      "Feishu: Install the official external plugin with: mo plugins install @mo/feishu, or run: mo doctor --fix.",
    );
  });
