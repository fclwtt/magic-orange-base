...
let replyPayloadModulePromise:
  | Promise<typeof import("mo/plugin-sdk/reply-payload")>
  | undefined;
...
function loadReplyPayloadModule() {
  replyPayloadModulePromise ??= import("mo/plugin-sdk/reply-payload");
...
function missingAgentMessageError(): Error {
  return new Error(
    `Missing message. Use ${formatCliCommand('mo agent --message "..." --agent <id>')} or ${formatCliCommand("mo agent --message-file <path> --agent <id>")}.`,
  );
}
...
async function agentViaGatewayCommand(
  ...
  if (!opts.to && !opts.sessionId && !opts.agent && !explicitSessionKey) {
    throw new Error(
      `No target session selected. Use --agent <id>, --session-key <key>, --session-id <id>, or --to <E.164>. Run ${formatCliCommand("mo agents list")} to see agents.`,
    );
  }
  ...
  if (agentId) {
    ...
    if (!knownAgents.includes(agentId)) {
      throw new Error(
        `Unknown agent id "${agentIdRaw}". Use "${formatCliCommand("mo agents list")}" to see configured agents.`,
      );
    }
  }
...
export async function agentCliCommand(
  ...
  if (isCompactControlCommand(messageOpts.message)) {
    runtime.error?.(
      "Slash commands cannot be executed via --message from the CLI. Use: mo sessions compact <key>",
    );
    runtime.exit(1);
    return undefined;
  }
