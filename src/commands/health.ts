...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.MO_DEBUG_HEALTH)) {
...
/** Runs the `mo health` command against the gateway and renders JSON or text. */
export async function healthCommand(
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.MO_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
