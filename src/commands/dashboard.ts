// Implements `mo dashboard` URL resolution, readiness check, clipboard, and browser launch.
...
export async function dashboardCommand(
  ...
  if (resolvedToken.unresolvedRefReason) {
    ...
    runtime.log(
      "Set MO_GATEWAY_TOKEN in this shell or resolve your secret provider, then rerun `mo dashboard`.",
    );
  }
  ...
  if (fallbackToManualAuth) {
    runtime.log(
      "Token auto-auth not delivered. Append your gateway token (from MO_GATEWAY_TOKEN or gateway.auth.token) as a URL fragment with key `token` to authenticate.",
    );
  }
}
