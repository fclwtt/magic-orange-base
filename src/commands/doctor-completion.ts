...
export type ShellCompletionStatus = {
  ...
  cacheExists: boolean;
  cachePath: string;
  /** True if profile uses slow dynamic pattern like `source <(mo completion ...)` */
    ...
    return [
      {
        ...
        path: pathLocal,
        fixHint: "Run `mo doctor --fix` to upgrade to cached completion.",
      },
    ];
  }
    ...
    return [
      {
        ...
        path: pathLocal,
        fixHint: `Run \`mo completion --write-state\` or \`mo doctor --fix\` to regenerate ${status.cachePath}.`,
      },
    ];
  }
  return [];
