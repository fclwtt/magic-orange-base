...
export async function runNonInteractiveLocalSetup(params: {
        ...
        hints:
          daemonInstall.skippedReason === "systemd-user-unavailable"
            ? [
                ...
                "If your auth profile uses env-backed refs, keep those env vars set in the shell that runs `mo gateway run` or `mo agent --local`.",
              ]
            : [`Run \`${formatCliCommand("mo gateway status --deep")}\` for more detail.`],
      });
      runtime.exit(1);
      return;
    }
    ...
    if (!probe.ok) {
      ...
      logNonInteractiveOnboardingFailure({
        ...
        hints: !opts.installDaemon
          ? [
              "Non-interactive local setup only waits for an already-running gateway unless you pass `--install-daemon` to `mo onboard`.",
              `Fix: start \`${formatCliCommand("mo gateway run")}\`, re-run \`${formatCliCommand("mo onboard --install-daemon")}\`, or use \`${formatCliCommand("mo onboard --skip-health")}\`.`,
  ...
  if (!opts.json) {
    runtime.log(
      `Tip: run \`${formatCliCommand("mo configure --section web")}\` to store your Brave API key for web_search. Docs: https://docs.mo.ai/tools/web`,
    );
  }
}
