...
import { normalizeUniqueSingleOrTrimmedStringList } from "@mo/normalization-core/string-normalization";
...
function resolvePendingPairingIssue(
  ...
  const approveCommand = formatCliArgs(["mo", "devices", "approve", pending.requestId]);
  const inspectCommand = formatCliArgs(["mo", "devices", "list"]);
  if (!paired) {
    return {
      kind: "first-time",
      pending,
  ...
  if (paired.publicKey !== pending.publicKey) {
    return {
      ...
      approveCommand,
      inspectCommand,
      removeCommand: formatCliArgs(["mo", "devices", "remove", pending.deviceId]),
    };
  }
