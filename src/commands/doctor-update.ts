...
export async function maybeOfferUpdateBeforeDoctor(params: {
  ...
  const updateInProgress = isTruthyEnvValue(process.env.MO_UPDATE_IN_PROGRESS);
  ...
  if (git === "not-git") {
    note(
      [
        ...
        `Run \`${formatCliCommand("mo update")}\` to update via your package manager (npm/pnpm), then rerun doctor.`,
      ].join("\n"),
      "Update",
    );
  }
