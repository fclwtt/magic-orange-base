...
describe("doctor config analysis helpers", () => {
  ...
  describe("stripUnknownConfigKeys during update", () => {
    const originalEnv = process.env.MO_UPDATE_IN_PROGRESS;
    ...
    afterEach(() => {
      if (originalEnv !== undefined) {
        process.env.MO_UPDATE_IN_PROGRESS = originalEnv;
      } else {
        delete process.env.MO_UPDATE_IN_PROGRESS;
      }
    });

    it("returns input unchanged when MO_UPDATE_IN_PROGRESS=1", () => {
    ...
    it("returns input unchanged when MO_UPDATE_IN_PROGRESS=true", () => {
      process.env.MO_UPDATE_IN_PROGRESS = "true";
      const input = { hooks: {}, unexpected: true } as never;
      const result = stripUnknownConfigKeys(input);
      expect(result.config).toBe(input);
      expect(result.removed).toEqual([]);
