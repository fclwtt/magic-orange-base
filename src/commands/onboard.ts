/**
 * Top-level `mo onboard` command entrypoint.
 *
...
export async function setupWizardCommand(
  ...
  if (
    ...
    runtime.error(
      `Invalid --secret-input-mode. Use "plaintext" or "ref", or run ${formatCliCommand("mo onboard")} for the interactive setup.`,
    );
    runtime.exit(1);
    return;
  }
  ...
  if (normalizedOpts.resetScope && !VALID_RESET_SCOPES.has(normalizedOpts.resetScope)) {
    runtime.error(
      `Invalid --reset-scope. Use "config", "config+creds+sessions", or "full". Run ${formatCliCommand("mo onboard --reset --reset-scope config")} for a config-only reset.`,
    );
    runtime.exit(1);
    return;
  }
  ...
  if (normalizedOpts.nonInteractive && normalizedOpts.acceptRisk !== true) {
    ...
    runtime.error(
      [
        ...
        "Read: https://docs.mo.ai/security",
        `Re-run with: ${formatCliCommand("mo onboard --non-interactive --accept-risk ...")}`,
      ].join("\n"),
    );
    runtime.exit(1);
    return;
  ...
  if (process.platform === "win32") {
    runtime.log(
      [
        ...
        "Guide: https://docs.mo.ai/windows",
      ].join("\n"),
    );
  }
