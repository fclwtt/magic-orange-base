...
describe("noteSecurityWarnings gateway exposure", () => {
  ...
  beforeEach(() => {
    ...
    pluginRegistry.list = [];
    prevToken = process.env.MO_GATEWAY_TOKEN;
    prevPassword = process.env.MO_GATEWAY_PASSWORD;
    prevHome = process.env.HOME;
    prevServiceKind = process.env.MO_SERVICE_KIND;
  ...
  afterEach(() => {
    ...
    if (prevPassword === undefined) {
      ...
      process.env.MO_GATEWAY_PASSWORD = prevPassword;
    }
    ...
    if (prevServiceKind === undefined) {
      ...
      process.env.MO_SERVICE_KIND = prevServiceKind;
    }
  });
  ...
  async function withExecApprovalsFile(
    ...
    await withTempDir({ prefix: "mo-doctor-security-" }, async (home) => {
      process.env.HOME = home;
      await fs.mkdir(path.join(home, ".mo"), { recursive: true });
      await fs.writeFile(
        path.join(home, ".mo", "exec-approvals.json"),
  ...
  it("uses env token to avoid critical warning", async () => {
    process.env.MO_GATEWAY_TOKEN = "token-123";
    const cfg = { gateway: { bind: "lan" } } as OpenClawConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("warns when MO_GATEWAY_TOKEN env conflicts with gateway.auth.token config (#74271)", async () => {
    process.env.MO_GATEWAY_TOKEN = "env-token-123";
    const cfg = {
      gateway: {
        auth: {
    ...
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).toContain("MO_GATEWAY_TOKEN conflicts with gateway.auth.token");
    expect(message).toContain("Direct local Gateway clients commonly prefer the env token");
    expect(message).toContain("~/.mo/.env");
  });

  it("does not warn when only env token is set without config token", async () => {
    process.env.MO_GATEWAY_TOKEN = "env-token-only";
    const cfg = { gateway: { bind: "lan" } } as OpenClawConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).not.toContain("MO_GATEWAY_TOKEN overrides");
  });

  it("does not warn inside the managed gateway service credential context", async () => {
    process.env.MO_GATEWAY_TOKEN = "env-token-123";
    process.env.MO_SERVICE_KIND = "gateway";
    const cfg = {
      gateway: {
        auth: {
          token: "config-token-456",
    ...
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).not.toContain("MO_GATEWAY_TOKEN conflicts");
  });

  it("does not warn when config token uses MO_GATEWAY_TOKEN SecretRef", async () => {
    process.env.MO_GATEWAY_TOKEN = "env-token-123";
    const cfg = {
      gateway: { auth: { token: "${MO_GATEWAY_TOKEN}" } },
  ...
  it("clarifies approvals.exec forwarding-only behavior", async () => {
    ...
    expect(message).toContain("mo approvals get --gateway");
  });
  ...
  it("warns when model provider API keys are stored as plaintext in config", async () => {
    ...
    expect(message).toContain("mo secrets audit --check");
  });
  ...
  it("degrades safely when channel account resolution fails in read-only security checks", async () => {
    ...
    expect(message).toContain("Run: mo security audit --deep");
  });
