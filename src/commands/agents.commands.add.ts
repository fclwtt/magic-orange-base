// Implements `mo agents add`, including config mutation, workspace setup, auth copy, and route binding setup.
...
export async function agentsAddCommand(
  ...
  if (nonInteractive) {
    if (!nameInput) {
      runtime.error(
        `Agent name is required in non-interactive mode. Run ${formatCliCommand("mo agents add <id> --workspace <path>")}.`,
      );
      runtime.exit(1);
      return;
    }
    ...
    if (agentId === DEFAULT_AGENT_ID) {
      runtime.error(
        `"${DEFAULT_AGENT_ID}" is reserved. Choose another name, or run ${formatCliCommand("mo agents list")} to inspect the default agent.`,
      );
      runtime.exit(1);
      return;
    }
    ...
    if (findAgentEntryIndex(listAgentEntries(cfg), agentId) >= 0) {
      runtime.error(
        `Agent "${agentId}" already exists. Run ${formatCliCommand("mo agents list")} to inspect configured agents.`,
      );
      runtime.exit(1);
      return;
    }
      ...
      if (wantsBindings) {
        ...
        await prompter.note(
          [
            ...
            "Docs: https://docs.mo.ai/concepts/multi-agent",
          ].join("\n"),
          "Routing",
        );
      }
