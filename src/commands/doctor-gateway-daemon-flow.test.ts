...
vi.mock("../daemon/constants.js", () => ({
  resolveGatewayLaunchAgentLabel: vi.fn(() => "ai.mo.gateway"),
  resolveNodeLaunchAgentLabel: vi.fn(() => "ai.mo.node"),
}));
...
describe("maybeRepairGatewayDaemon", () => {
  ...
  it("reports recent restart handoffs during deep doctor", async () => {
    ...
    service.readCommand.mockResolvedValueOnce({
      ...
      environment: {
        MO_STATE_DIR: "/tmp/mo-service",
        MO_CONFIG_PATH: "/tmp/mo-service/mo.json",
      },
    });
    ...
    const [handoffEnv] = readGatewayRestartHandoffSync.mock.calls[0] as unknown as [
      { MO_STATE_DIR?: string; MO_CONFIG_PATH?: string },
    ];
    expect(handoffEnv?.MO_STATE_DIR).toBe("/tmp/mo-service");
    expect(handoffEnv?.MO_CONFIG_PATH).toBe("/tmp/mo-service/mo.json");
    expect(note).toHaveBeenCalledWith(
      ...
      connections: [
        {
          pid: 4242,
          command: "node",
          commandLine: "/tmp/newer-mo/bin/mo logs --follow",
  ...
  it("suppresses busy-port note for expected Gateway listeners", async () => {
    setPlatform("linux");
    const listeners = [{ pid: 5001, commandLine: "mo-gateway", address: "0.0.0.0:18789" }];
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners,
  ...
  it("keeps busy-port note for unexpected Gateway listeners", async () => {
    ...
    inspectPortUsage.mockResolvedValue({
      ...
      status: "busy",
      listeners: [
        { pid: 5001, commandLine: "mo-gateway", address: "0.0.0.0:18789" },
        { pid: 5002, commandLine: "mo-gateway", address: "127.0.0.1:18789" },
      ],
  ...
  it("skips gateway install during non-interactive doctor repairs", async () => {
    ...
    expect(service.restart).not.toHaveBeenCalled();
    expect(note).toHaveBeenCalledWith(
      `Run ${formatCliCommand("mo gateway install")} when you want to install the gateway service.`,
      "Gateway",
    );
  });
  ...
  it("skips gateway service install when a system OpenClaw gateway service exists", async () => {
    ...
    findSystemGatewayServices.mockResolvedValue([
      {
        ...
        detail: "unit: /etc/systemd/system/mo-gateway.service",
        scope: "system",
        marker: "mo",
        legacy: false,
      },
    ...
    expect(note).toHaveBeenCalledWith(
      [
        ...
        "- mo-gateway.service (unit: /etc/systemd/system/mo-gateway.service)",
        "OpenClaw will not install a second user-level gateway service automatically.",
        "Run `mo gateway status --deep` or `mo doctor --deep` to inspect duplicate services.",
