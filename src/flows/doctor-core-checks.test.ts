...
function createSkill(overrides: Partial<SkillStatusEntry> = {}): SkillStatusEntry {
  return {
    ...
    source: "workspace",
    bundled: false,
    filePath: "/tmp/mo-test-workspace/skills/missing-tool/SKILL.md",
    baseDir: "/tmp/mo-test-workspace/skills/missing-tool",
    ...
    requirements: {
      bins: ["mo-test-missing-skill-bin"],
      anyBins: [],
      env: [],
      config: [],
      os: [],
...
describe("CORE_HEALTH_CHECKS", () => {
  ...
  it("converts unavailable skills into repair-capable health findings", async () => {
    ...
    const cfg: OpenClawConfig = {
      agents: {
        defaults: {
          workspace: "/tmp/mo-test-workspace",
          skills: ["missing-tool"],
        },
      },
    };
  ...
  it("skips gateway auth warning when SecretRef-managed token resolves in lint checks", async () => {
    ...
    await withEnvAsync({ MO_TEST_GATEWAY_TOKEN: "resolved-test-token" }, async () => {
      const findings = await check?.detect({
        mode: "lint",
  ...
  it("reports unresolved SecretRefs even when MO_GATEWAY_TOKEN is set", async () => {
    const check = CORE_HEALTH_CHECKS.find((entry) => entry.id === "core/doctor/gateway-auth");
    await withEnvAsync(
      {
        MO_GATEWAY_TOKEN: "fallback-token",
        MO_MISSING_GATEWAY_REF_TOKEN: undefined,
      },
      async () => {
        const findings = await check?.detect({
          mode: "lint",
  ...
  it("does not execute or warn for valid exec SecretRefs during default gateway auth lint checks", async () => {
    tmp = await fs.mkdtemp(join(tmpdir(), "mo-health-exec-ref-"));
  ...
  it("reports exec SecretRef failures when gateway auth lint explicitly allows exec checks", async () => {
    ...
    const findings = await withEnvAsync({ MO_GATEWAY_TOKEN: "fallback-token" }, async () => {
    ...
    expect(findings).toContainEqual(
      expect.objectContaining({
        ...
        message: expect.stringContaining("Gateway token SecretRef could not be resolved:"),
        fixHint:
          "Run `mo doctor --allow-exec` to verify exec SecretRefs during doctor, or `mo secrets audit --allow-exec` to audit all exec SecretRefs.",
      }),
    );
  });
