...
import { redactSensitiveUrlLikeString } from "@mo/net-policy/redact-sensitive-url";
    ...
    return [
      {
        ...
        fixHint:
          "Fix Gateway connection configuration, then rerun `mo doctor --lint --only core/doctor/gateway-health`.",
      },
    ];
  }
  ...
  return [
    {
      ...
      fixHint:
        mode === "remote"
          ...
          : "Start the Gateway service or run `mo doctor --fix` for service repair prompts.",
    },
  ];
}
...
export async function collectGatewayDaemonFindings(
  ...
  if (!state.installed) {
    findings.push({
      ...
      fixHint: "Run `mo doctor --fix` or `mo gateway install` to install it.",
    });
    return findings;
  }
  if (!state.loaded) {
    findings.push({
      ...
      target: service.label,
      fixHint: "Run `mo doctor --fix` or `mo gateway start` to load it.",
    });
  }
  ...
  if (state.loaded && !state.running) {
    findings.push({
      ...
      path: state.command?.sourcePath,
      target: service.label,
      fixHint: "Run `mo gateway status --deep` or `mo doctor --fix` for repair hints.",
    });
  }
