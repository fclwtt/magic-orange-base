...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn(() => ({ label: "mo-gateway" })),
...
describe("doctor runtime tool schema checks", () => {
  beforeEach(() => {
    ...
    mocks.readGatewayServiceState.mockReset().mockResolvedValue({
      ...
      running: true,
      env: {},
      command: { programArguments: ["mo", "gateway"], sourcePath: "/tmp/gateway.service" },
...
describe("doctor gateway runtime checks", () => {
  beforeEach(() => {
    ...
    mocks.resolveGatewayService.mockReset().mockReturnValue({ label: "mo-gateway" });
  });
