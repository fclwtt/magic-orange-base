...
describe("resolveChannelSetupSelectionContributions", () => {
  ...
  it("localizes channel status note labels", async () => {
    ...
    await withEnvAsync({ MO_LOCALE: "zh-CN" }, async () => {
      const summary = await collectChannelStatus({
        cfg: {} as never,
        accountOverrides: {},
        installedPlugins: [],
  ...
  it("sanitizes channel metadata before primer notes", async () => {
    ...
    expect(note).toHaveBeenCalledWith(
      [
        ...
        "Approve with: mo pairing approve <channel> <code>",
        'Open/public DMs require dmPolicy="open" plus allowFrom=["*"].',
        'For multi-user DMs, isolate sessions with: mo config set session.dmScope "per-channel-peer" (or "per-account-channel-peer" for multi-account channels).',
        "Docs: https://docs.mo.ai/channels/pairing",
        "",
        "bad\\nid: Blurb\\nline",
      ].join("\n"),
      "How channels work",
  ...
  it("sanitizes channel metadata before selection notes", () => {
    ...
    expect(docsLink("/channels/zalo", "Docs")).toBe("https://docs.mo.ai/channels/zalo");
