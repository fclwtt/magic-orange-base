...
import { markOpenClawExecEnv } from "../../infra/mo-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "mo.sandbox=1");
  args.push("--label", `mo.sessionKey=${params.scopeKey}`);
  args.push("--label", `mo.createdAtMs=${createdAtMs}`);
  args.push("--label", `mo.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `mo.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "mo.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`mo sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`mo sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("mo sandbox recreate --all");
}
