...
const {
  ...
  const baseDir = mkdtempSync(nodePath.join(tmpdir(), "mo-sandbox-registry-"));
  const previousStateDir = process.env.MO_STATE_DIR;
  Reflect.set(process.env, "MO_STATE_DIR", baseDir);

  return {
    TEST_STATE_DIR: baseDir,
    PREVIOUS_MO_STATE_DIR: previousStateDir,
...
afterAll(async () => {
  ...
  if (PREVIOUS_MO_STATE_DIR === undefined) {
    deleteTestEnvValue("MO_STATE_DIR");
  } else {
    setTestEnvValue("MO_STATE_DIR", PREVIOUS_MO_STATE_DIR);
  }
});
