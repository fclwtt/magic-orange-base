...
function createSession() {
  return {
    command: "ssh",
    configPath: path.join(os.tmpdir(), "mo-test-ssh-config"),
    host: "mo-sandbox",
  };
}
...
describe("ssh sandbox backend", () => {
  ...
  it("describes runtimes via the configured ssh target", async () => {
    const result = await sshSandboxBackendManager.describeRuntime({
      entry: {
        containerName: "mo-ssh-worker-abcd1234",
        backendId: "ssh",
        runtimeLabel: "mo-ssh-worker-abcd1234",
    ...
    expect(sessionSettings.workspaceRoot).toBe("/remote/mo");
    const commandParams = requireSshRunCommandParams();
    expect(commandParams.remoteCommand).toContain("/remote/mo/mo-ssh-agent-worker");
  });
  ...
  it("creates a remote-canonical backend that seeds once and reuses ssh exec", async () => {
    ...
    const skillsWorkspaceDir = await createTempDir("mo-ssh-skills-");
    ...
    expect(execSpec.argv.at(-1)).toContain("/remote/mo/mo-ssh-agent-worker");
    ...
    expect(skillsUploadParams.remoteDir).toContain("/workspace/.mo/sandbox-skills");
  ...
  it("validates remote workdirs before exec accepts backend-owned cwd", async () => {
    sshMocks.runSshSandboxCommand
      ...
      .mockResolvedValueOnce({
        stdout: Buffer.from("/remote/mo/mo-ssh-agent-worker-abcd1234/workspace/src\n"),
        stderr: Buffer.alloc(0),
        code: 0,
      })
      .mockResolvedValueOnce({
      ...
      .mockResolvedValueOnce({
        stdout: Buffer.from("/remote/mo/mo-ssh-agent-worker-abcd1234/agent/src\n"),
        stderr: Buffer.alloc(0),
        code: 0,
      });
    ...
    await expect(
      backend.validateWorkdir?.(
        "/remote/mo/mo-ssh-agent-worker-abcd1234/workspace/src",
      ),
    ).resolves.toBe("/remote/mo/mo-ssh-agent-worker-abcd1234/workspace/src");
    await expect(
      backend.validateWorkdir?.(
        "/remote/mo/mo-ssh-agent-worker-abcd1234/workspace/missing",
      ),
    ).resolves.toBeNull();
    await expect(
      backend.validateWorkdir?.("/remote/mo/mo-ssh-agent-worker-abcd1234/agent/src"),
    ).resolves.toBe("/remote/mo/mo-ssh-agent-worker-abcd1234/agent/src");

    const validationCommand = String(requireSshRunCommandParams(1).remoteCommand);
    expect(validationCommand).toContain("mo-validate-workdir");
    ...
    expect(agentValidationCommand).toContain(
      "/remote/mo/mo-ssh-agent-worker-abcd1234/agent",
    );
  });
  ...
  it("refreshes materialized skills before validating a skills workdir", async () => {
    ...
    const runtimePaths = resolveSshRuntimePaths("/remote/mo", "agent:worker");
  ...
  it("clears stale remote materialized skills when the local copy is missing", async () => {
    const tmpDir = await createTempDir("mo-ssh-skills-");
    ...
    expect(commandParams.remoteCommand).toContain("mo-sandbox-clear");
    expect(commandParams.remoteCommand).toContain("/workspace/.mo/sandbox-skills");
    await backend.finalizeExec?.({
      status: "completed",
      exitCode: 0,
      timedOut: false,
