...
describe("sanitizeUserFacingText", () => {
  ...
  it("strips internal tool trace warning lines from error-context delivery text", () => {
    const input = [
      ...
      "⚠️ 🛠️ gh search issues --repo mo/mo --state open --no-search-pages.jsonl /tmp/mo_open_unlabeled_current.json (agent) failed",
      "⚠️ 🛠️ gh search issues --repo mo/mo --state open (agent) failed: command timed out",
      "🛠️ run git status",
