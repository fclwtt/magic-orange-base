 ...
 *
 * @see https://github.com/mo/mo/issues/47711
 */
...
export async function recoverOrphanedSubagentSessions(params: {
  ...
  const configChangePattern = /mo\.json|mo gateway restart|config\.patch/i;
      ...
      try {
        ...
        const resumeResult = await resumeOrphanedSession({
          ...
          configChangeHint: configChangeDetected
            ? "\n\n[config changes from your previous run were already applied — do not re-modify mo.json or restart the gateway]"
            : undefined,
          originalRunId: runId,
