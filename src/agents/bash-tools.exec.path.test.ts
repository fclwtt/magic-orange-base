...
const ENV_KEYS = ["MO_EXEC_SHELL_SNAPSHOT", "PATH", "SHELL", "SSLKEYLOGFILE"] as const;
...
vi.mock("../process/supervisor/index.js", () => ({
  getProcessSupervisor: () => ({
    spawn: async (input: {
      ...
      const env = input.env ?? {};
      if (command.includes("MO_SHELL")) {
        input.onStdout?.(env.MO_SHELL ?? "");
...
describe("exec PATH login shell merge", () => {
  ...
  beforeEach(() => {
    ...
    process.env.MO_EXEC_SHELL_SNAPSHOT = "0";
  ...
  it("strips malformed XML arg-value suffixes from exec command and routing options", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-exec-xml-"));
    try {
  ...
  it("fails without running when an explicit workdir is unavailable", async () => {
    const missingWorkdir = path.join(
      os.tmpdir(),
      `mo-missing-workdir-${process.pid}-${Date.now()}`,
    );
  ...
  it("sets MO_SHELL for host=gateway commands", async () => {
    if (isWin) {
      return;
    }

    const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
    const result = await tool.execute("call-mo-shell", {
      command: 'printf "%s" "${MO_SHELL:-}"',
      yieldMs: FOREGROUND_TEST_YIELD_MS,
    });
    const value = normalizeText(result.content.find((c) => c.type === "text")?.text);
  ...
  it("does not apply login-shell PATH when probe rejects unregistered absolute SHELL", async () => {
    ...
    process.env.PATH = "/usr/bin";
    const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-shell-env-"));
...
describe("exec host env validation", () => {
  ...
  it("strips dangerous inherited env vars from host execution", async () => {
    ...
    process.env.SSLKEYLOGFILE = "/tmp/mo-ssl-keys.log";
    try {
      const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
      const result = await tool.execute("call1", {
      ...
      expect(output).not.toContain("/tmp/mo-ssl-keys.log");
    } finally {
  ...
  it.each([
    ...
    "sudo MO_APPROVE=1 /approve abc123 allow-once",
    "sudo -uroot bash -lc '/approve abc123 allow-once'",
    "sudo -u root MO_APPROVE=1 bash -lc '/approve abc123 allow-once'",
