...
describe("resolveMediaToolLocalRoots", () => {
  it("does not widen default local roots from media sources", () => {
    const stateDir = path.join("/tmp", "mo-media-tool-roots-state");
    const picturesDir =
    ...
    const roots = withEnv({ MO_STATE_DIR: stateDir }, () =>
  ...
  it("keeps channel inbound attachment roots separate from local roots", () => {
    ...
    const accountRoot = path.join("/tmp", "mo-imessage-work");
    const sharedRoot = path.join("/tmp", "mo-imessage-shared");
    const cfg = {
      channels: {
        imessage: {
