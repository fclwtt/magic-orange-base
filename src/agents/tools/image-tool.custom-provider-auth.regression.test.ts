...
import { resolveImageToolFactoryAvailable } from "../mo-tools.media-factory-plan.js";
...
async function withEmptyAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-image-auth-regression-"));
  try {
