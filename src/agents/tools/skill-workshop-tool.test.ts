...
describe("skill_workshop tool", () => {
  it("is exposed in the OpenClaw tool set", async () => {
    const workspaceDir = await tempDirs.make("mo-skill-workshop-tool-");
  ...
  it("scopes proposal discovery to the tool workspace", async () => {
    const firstWorkspaceDir = await tempDirs.make("mo-skill-workshop-tool-first-");
    const secondWorkspaceDir = await tempDirs.make("mo-skill-workshop-tool-second-");
