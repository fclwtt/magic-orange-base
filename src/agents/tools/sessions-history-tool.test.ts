...
function readMessageSeq(message: unknown): number | undefined {
  ...
  const meta = (message as Record<string, unknown>)["__mo"];
...
describe("sessions_history redaction", () => {
  beforeAll(async () => {
    previousConfigPath = process.env.MO_CONFIG_PATH;
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-sessions-history-redact-"));
  ...
  afterAll(() => {
    if (previousConfigPath === undefined) {
      ...
      setTestEnvValue("MO_CONFIG_PATH", previousConfigPath);
    }
    if (tempDir) {
  ...
  it("uses the oldest visible message for pagination after tool messages are filtered", async () => {
    const tool = createSessionsHistoryTool({
      ...
      callGateway: async <T = Record<string, unknown>>(): Promise<T> =>
        ({
          messages: [
            { role: "tool", content: "hidden", __mo: { seq: 6 } },
            { role: "assistant", content: "visible", __mo: { seq: 7 } },
            { role: "assistant", content: "latest", __mo: { seq: 8 } },
          ],
          offset: 0,
          nextOffset: 5,
          hasMore: true,
