...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses MO_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      MO_HOME: "/srv/mo-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/mo-home"), ".mo", "workspace"));
  });

  it("prefers MO_WORKSPACE_DIR for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      MO_WORKSPACE_DIR: "/srv/mo-workspace",
      MO_HOME: "/srv/mo-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.resolve("/srv/mo-workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = ["mo-workspace-state.json"] as const;
const LEGACY_WORKSPACE_STATE_PATH_SEGMENTS = [".mo", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("mo-workspace-");
  ...
  it("does not overwrite a foreign root workspace-state.json file", async () => {
    ...
    const foreignState = "not mo state\n";
  ...
  it("refuses to re-seed a recently attested workspace after only generated git metadata survives", async () => {
    ...
    await fs.mkdir(path.join(tempDir, ".mo"), { recursive: true });
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-workspace-hardlink-"));
    try {
