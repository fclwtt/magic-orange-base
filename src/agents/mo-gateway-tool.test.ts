...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-test-"));

    try {
      await withEnvAsync(
        { MO_STATE_DIR: stateDir, MO_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
  ...
  it("rejects config.apply when it changes safe bin trusted directories", async () => {
    ...
    await expect(
      tool.execute("call-protected-safe-bin-trust-apply", {
        action: "config.apply",
        raw: '{ tools: { exec: { ask: "on-miss", security: "allowlist", safeBinTrustedDirs: ["/tmp/mo-bin"] } } }',
      }),
    ).rejects.toThrow(
