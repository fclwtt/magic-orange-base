...
function buildWebchatCanvasSection(params: {
  ...
  return [
    ...
    '- You may also use an explicit hosted URL: `[embed url="/__mo__/canvas/documents/cv_123/index.html" title="Status" height="320" /]`.',
    '- Never use local filesystem paths or `file://...` URLs in `[embed ...]`. Hosted embeds must point at `/__mo__/canvas/...` URLs or use `ref="..."`.',
    ...
    "- Quote all attribute values. Prefer `ref` for hosted documents unless you already have the full `/__mo__/canvas/documents/<id>/index.html` URL.",
    "",
  ];
}
...
function buildDocsSection(params: {
  ...
  const lines = [
    "## Documentation",
    docsPath ? `Docs: ${docsPath}` : "Docs: https://docs.mo.ai",
    docsPath ? "Mirror: https://docs.mo.ai" : undefined,
    sourcePath ? `Source: ${sourcePath}` : "Source: https://github.com/mo/mo",
    docsPath
    ...
    "Diagnosing issues: run `mo status` when possible; ask user only if blocked.",
    "",
  ];
...
export function buildAgentSystemPrompt(params: {
  ...
  const promptSurface = params.promptSurface ?? "mo_main";
  ...
  const workspaceOnlyGuidance =
    params.fsWorkspaceOnly === true
      ? "tools.fs.workspaceOnly is enabled: scratch/temp/meta files that file tools must later read/write/edit must stay inside the workspace, preferably as relative paths under `.mo/tmp/`. Do not write files to `/tmp/...` with exec if a later read/write/edit/apply_patch tool needs them; use `.mo/tmp/...` instead."
      : "";
  const safetySection = [
    "## Safety",
  ...
  const stablePrefix = cacheStablePromptPrefix(stablePrefixCacheKey, () => {
    const lines = [
      ...
      "CLI lifecycle only on explicit user request: `mo gateway status|restart|start|stop`.",
