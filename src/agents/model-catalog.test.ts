...
describe("loadModelCatalog", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./agent-scope.js", () => ({
      resolveAgentWorkspaceDir: (cfg: OpenClawConfig, agentId: string) => {
        ...
        return entry?.workspace ?? cfg.agents?.defaults?.workspace ?? "/tmp/mo-workspace";
      },
      resolveDefaultAgentDir: () => "/tmp/mo",
  ...
  it("loads generated plugin catalog rows in read-only mode", async () => {
    const catalogPath = "/tmp/mo/plugins/read-only-shard/catalog.json";
    mkdirSync("/tmp/mo/plugins/read-only-shard", { recursive: true });
    ...
    try {
      ...
      rmSync("/tmp/mo/plugins/read-only-shard", { recursive: true, force: true });
    }
  });
  ...
  it("passes explicit env when checking current manifest catalog snapshot compatibility", () => {
    const env = { HOME: "/tmp/mo-model-catalog-env" } as NodeJS.ProcessEnv;
