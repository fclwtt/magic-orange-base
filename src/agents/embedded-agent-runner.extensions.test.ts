import { wrapToolWithBeforeToolCallHook } from "mo/plugin-sdk/agent-harness-runtime";
import {
...
import {
  ...
  ModelRegistry,
  SessionManager,
} from "mo/plugin-sdk/agent-sessions";
...
describe("buildEmbeddedExtensionFactories", () => {
  it("bridges middleware mutations with unique fallback tool call ids", async () => {
    ...
    expect(seenToolCallIds[0]).toMatch(/^mo-/);
    expect(seenToolCallIds[1]).toMatch(/^mo-/);
