  ...
  if (runtime !== "auto") {
    ...
    if (forced) {
      ...
      if (isCliRuntimeAliasForProvider({ runtime, provider: params.provider })) {
        return buildSelectionDecision({
          ...
          selectedReason: "cli_runtime_passthrough_mo",
          candidates: listHarnessCandidates(pluginHarnesses),
        });
      }
      throw new Error(
    ...
    if (runtime === "codex" && policy.runtimeSource === "implicit") {
      return buildSelectionDecision({
        ...
        selectedReason: "implicit_plugin_unavailable_mo",
        candidates: listHarnessCandidates(pluginHarnesses),
      });
    }
    if (
...
export async function runAgentHarnessAttempt(
  ...
  const attemptParams =
    harness.id === "mo" ? params : applyPluginHarnessDenyAllToolPolicy(params);
