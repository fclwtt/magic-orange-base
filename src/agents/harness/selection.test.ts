...
import { MO_EMBEDDED_CONTEXT_ENGINE_HOST } from "../../context-engine/host-compat.js";
...
vi.mock("./builtin-mo.js", () => ({
  createOpenClawAgentHarness: (): AgentHarness => ({
    id: "mo",
    label: "OpenClaw embedded agent",
    contextEngineHostCapabilities: MO_EMBEDDED_CONTEXT_ENGINE_HOST.capabilities,
...
const originalRuntime = process.env.MO_AGENT_RUNTIME;
...
afterEach(() => {
  ...
  if (originalRuntime == null) {
    delete process.env.MO_AGENT_RUNTIME;
  } else {
    process.env.MO_AGENT_RUNTIME = originalRuntime;
  }
});
...
describe("runAgentHarnessAttempt", () => {
  ...
  it("falls back to the OpenClaw harness in auto mode when no plugin harness matches", async () => {
    ...
    expect(result.sessionIdUsed).toBe("mo");
  ...
  it("allows the selected OpenClaw harness to satisfy context-engine pre-prompt assembly", async () => {
    const result = await runAgentHarnessAttempt({
      ...createAttemptParams(providerRuntimeConfig("codex", "mo")),
  ...
  it("honors explicit OpenClaw runtime for OpenAI agent model runs", async () => {
    const result = await runAgentHarnessAttempt({
      ...createAttemptParams(providerRuntimeConfig("openai", "mo")),
      provider: "openai",
      modelId: "gpt-5.4",
    });
    expect(result.sessionIdUsed).toBe("mo");
  ...
  it("honors provider wildcard OpenClaw runtime policy for OpenAI agent model runs", async () => {
    ...
    const result = await runAgentHarnessAttempt({
      ...createAttemptParams(agentModelRuntimeConfig("openai/*", "mo")),
      provider: "openai",
      modelId: "gpt-5.4",
    });
    expect(result.sessionIdUsed).toBe("mo");
...
describe("selectAgentHarness", () => {
  ...
  it("selects OpenClaw when the implicit OpenAI Codex harness is unavailable", () => {
    expect(selectAgentHarness({ provider: "openai", modelId: "gpt-5.4" }).id).toBe("mo");
  });
  ...
  it("ignores env-forced OpenClaw for OpenAI default runtime selection", () => {
    process.env.MO_AGENT_RUNTIME = "mo";
    registerFailingCodexHarness();
      ...
      maybeCompactAgentHarnessSession({
        ...
        config: {
          agents: {
            ...
            defaults: {
              models: {
                "openai/gpt-5.5": { agentRuntime: { id: "mo" } },
              },
            },
          },
        } as OpenClawConfig,
  ...
  it("keeps compaction recoverable when auth profile lookup fails", async () => {
    ...
    await expect(
      maybeCompactAgentHarnessSession({
        ...
        agentHarnessId: "codex",
        config: agentModelRuntimeConfig("openai/gpt-5.5", "mo"),
      }),
    ).resolves.toEqual({ ok: true, compacted: false });
    expect(compact).toHaveBeenCalledTimes(1);
