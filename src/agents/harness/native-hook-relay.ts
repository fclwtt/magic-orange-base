...
const NATIVE_HOOK_RELAY_STATE_SYMBOL = Symbol.for("mo.nativeHookRelay.state");
...
function nativeHookRelayBridgeDir(): string {
  ...
  return path.join(tmpdir(), `mo-native-hook-relays-${uid}`);
}
...
function nativeHookRelayPermissionAllowAlwaysKey(params: {
  ...
  hash.update("mo:native-hook-relay:permission-allow-always:v2");
  hash.update("\0");
...
function readNativeHookRelayApprovalMode(rawPayload: JsonValue): "report" | undefined {
  const payload = isJsonObject(rawPayload) ? rawPayload : {};
  return payload.mo_approval_mode === "report" ? "report" : undefined;
}
...
async function requestNativeHookRelayPermissionApproval(
  ...
  const requestResult: {
    ...
    {
      pluginId: `mo-native-hook-relay-${request.provider}`,
      title: truncateText(
...
function resolveOpenClawCliExecutable(): string {
  const envPath = process.env.MO_CLI_PATH?.trim();
