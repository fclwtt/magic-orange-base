...
async function withTempSessionLockFile(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "mo-lock-"));
  try {
...
describe("acquireSessionWriteLock", () => {
  ...
  it("reports live OpenClaw-owned stale locks without removing them", async () => {
    await withTempSessionLockFile(async ({ sessionFile, lockPath }) => {
      const owner = spawn(process.execPath, ["-e", "setInterval(() => {}, 1000)", "mo"], {
        stdio: "ignore",
      });
      if (!owner.pid) {
  ...
  it("lets session write-lock env override config for emergency tuning", () => {
    expect(
      resolveSessionWriteLockOptions(
        ...
        {
          env: {
            MO_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS: "120000",
            MO_SESSION_WRITE_LOCK_STALE_MS: "60000",
            MO_SESSION_WRITE_LOCK_MAX_HOLD_MS: "50000",
          },
        },
      ),
    ).toEqual({
  ...
  it("ignores non-decimal and unsafe session write-lock env values", () => {
    expect(
      resolveSessionWriteLockOptions(
        ...
        {
          env: {
            MO_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS: "1e3",
            MO_SESSION_WRITE_LOCK_STALE_MS: "0x1000",
            MO_SESSION_WRITE_LOCK_MAX_HOLD_MS: "9007199254740993",
          },
        },
      ),
    ).toEqual({
  ...
  it("allows explicit infinite acquire timeout but not infinite stale policy", () => {
    expect(
      resolveSessionWriteLockOptions(undefined, {
        env: {
          MO_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS: "Infinity",
          MO_SESSION_WRITE_LOCK_STALE_MS: "Infinity",
        },
      }),
    ).toMatchObject({
  ...
  it("uses resolved stale policy when cleaning stale lock files", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "mo-lock-policy-"));
    ...
    try {
      ...
      const configOnly = await cleanStaleLockFiles({
        ...
        nowMs,
        removeStale: false,
        readOwnerProcessArgs: () => ["node", "/opt/mo/mo.mjs", "doctor"],
      });
      ...
      const envOverride = await cleanStaleLockFiles({
        sessionsDir,
        config: { session: { writeLock: { staleMs: 30_000 } } },
        env: { MO_SESSION_WRITE_LOCK_STALE_MS: "60000" },
        nowMs,
        removeStale: false,
        readOwnerProcessArgs: () => ["node", "/opt/mo/mo.mjs", "doctor"],
      });
  ...
  it("does not clean live OpenClaw locks just because holder max hold expired", async () => {
    ...
    try {
      ...
      const result = await cleanStaleLockFiles({
        ...
        nowMs,
        removeStale: true,
        readOwnerProcessArgs: () => ["node", "/opt/mo/mo.mjs", "agent"],
      });
  ...
  it("cleans old live .jsonl lock files owned by non-OpenClaw processes", async () => {
    ...
    await fs.mkdir(sessionsDir, { recursive: true });
    const nowMs = Date.now();
    const lockPath = path.join(sessionsDir, "old-non-mo.jsonl.lock");
    ...
    try {
      ...
      expect(lockCleanupRecords(result.cleaned)).toEqual([
        {
          name: "old-non-mo.jsonl.lock",
          removed: true,
          stale: true,
          staleReasons: ["too-old", "non-mo-owner"],
        },
      ]);
      await expectPathMissing(lockPath);
    } finally {
  ...
  it("keeps fresh live .jsonl lock files with OpenClaw or unknown owners", async () => {
    ...
    const nowMs = Date.now();
    const moLock = path.join(sessionsDir, "mo-live.jsonl.lock");
    ...
    try {
      ...
      const moResult = await cleanStaleLockFiles({
        sessionsDir,
        staleMs: 30_000,
        nowMs,
        removeStale: true,
      ...
      expect(moResult.cleaned).toEqual([]);
      await expect(fs.access(moLock)).resolves.toBeUndefined();

      await fs.rm(moLock, { force: true });
      await fs.writeFile(
        gatewayLock,
        JSON.stringify({
          pid: process.pid,
  ...
  it("removes held locks on termination signals", async () => {
    ...
    try {
      for (const signal of signals) {
        const root = await fs.mkdtemp(path.join(os.tmpdir(), "mo-lock-cleanup-"));
        try {
          const sessionFile = path.join(root, "sessions.json");
          const lockPath = `${sessionFile}.lock`;
