...
const LIVE = isLiveTestEnabled() && isTruthyEnvValue(process.env.MO_LIVE_SUBAGENT_E2E);
...
function resolveLiveSubagentModelConfig(): LiveSubagentModelConfig {
  const modelKey = process.env.MO_LIVE_SUBAGENT_E2E_MODEL?.trim() || "openai/gpt-5.5";
  ...
  return {
    ...
    agents: {
      defaults: {
        ...
        models: { [modelKey]: { agentRuntime: { id: "mo" }, params: { maxTokens: 1024 } } },
        sandbox: { mode: "off" },
        subagents: {
          allowAgents: ["*"],
...
describeLive("subagent announce live", () => {
  ...
  it(
    ...
    async () => {
      if (!isTruthyEnvValue(process.env.MO_SUBAGENT_ISSUE_82913_REPRO)) {
        console.warn(
          "[issue-82913] skip: set MO_SUBAGENT_ISSUE_82913_REPRO=1 to run this focused repro",
        );
        return;
      }
      ...
      state = await createOpenClawTestState({
        ...
        env: {
          ...
          MO_DISABLE_PERSISTED_PLUGIN_REGISTRY: "1",
          MO_BUNDLED_PLUGINS_DIR: path.resolve("extensions"),
          MO_TEST_TRUST_BUNDLED_PLUGINS_DIR: "1",
          MO_PLUGIN_CATALOG_PATHS: undefined,
          MO_PLUGINS_PATHS: undefined,
        },
      });
  ...
  it(
    ...
    async () => {
      ...
      if (!modelConfig.modelKey.startsWith("google/")) {
        console.warn(
          "[subagent-stress] skip: set MO_LIVE_SUBAGENT_E2E_MODEL=google/gemini-3.1-pro-preview",
        );
        return;
      }
      requireLiveSubagentAuth(modelConfig);
      ...
      state = await createOpenClawTestState({
        ...
        env: {
          ...
          MO_PLUGIN_CATALOG_PATHS: undefined,
          MO_PLUGINS_PATHS: undefined,
          MO_DEBUG_MODEL_TRANSPORT: "1",
          MO_DEBUG_MODEL_PAYLOAD: "tools",
          MO_DEBUG_SSE: "events",
        },
      });
      await fs.writeFile(
        path.join(state.workspaceDir, "package.json"),
        `${JSON.stringify({ name: "mo-gemini-stress-live", private: true }, null, 2)}\n`,
        "utf8",
      );
      await fs.writeFile(
              ...
              return `Call sessions_spawn for child ${childNumber} with exactly this JSON input: ${JSON.stringify(
                {
                  task: [
                    ...
                    "Then run a tiny shell command if the bash tool is available: printf mo.",
