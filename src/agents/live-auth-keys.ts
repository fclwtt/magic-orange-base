...
const GOOGLE_LIVE_SINGLE_KEY = "MO_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "MO_LIVE_ANTHROPIC_KEY",
    listVar: "MO_LIVE_ANTHROPIC_KEYS",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `MO_LIVE_${base}_KEY`;
...
export function collectProviderApiKeys(
  ...
  const forcedSingle = config.liveSingle
    ...
    : undefined;
  if (forcedSingle) {
    // MO_LIVE_*_KEY pins a single key so retries do not rotate fixtures.
    return [forcedSingle];
  }
