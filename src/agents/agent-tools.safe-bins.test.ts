...
beforeAll(async () => {
  await withEnvAsync(
    {
      MO_BUNDLED_PLUGINS_DIR: path.join(os.tmpdir(), "mo-test-no-bundled-extensions"),
    },
    async () => {
...
vi.mock("mo/plugin-sdk/agent-sessions", () => ({
  AuthStorage: vi.fn(),
...
describe("createOpenClawCodingTools safeBins", () => {
  ...
  it("rejects unprofiled custom safe-bin entries", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "mo-safe-bins-unprofiled-",
        safeBins: ["echo"],
      },
      async ({ tmpDir, execTool }) => {
        await expect(
  ...
  it("blocks shell redirection metacharacters in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "mo-safe-bins-redirect-",
        safeBins: ["head"],
