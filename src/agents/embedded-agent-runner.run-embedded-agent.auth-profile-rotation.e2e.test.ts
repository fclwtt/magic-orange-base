...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-workspace-"));
    const now = Date.now();
...
describe("runEmbeddedAgent auth profile rotation", () => {
  ...
  it("logs structured failover decision metadata for overloaded assistant rotation", async () => {
    ...
    setLoggerOverrideFn({
      level: "trace",
      consoleLevel: "silent",
      file: path.join(os.tmpdir(), `mo-auth-rotation-${Date.now()}.log`),
    });
