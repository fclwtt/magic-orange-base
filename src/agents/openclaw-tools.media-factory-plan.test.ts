...
import { resolveOptionalMediaToolFactoryPlan } from "./mo-tools.media-factory-plan.js";
...
type CreateOpenClawToolsOptions = Parameters<
  typeof import("./mo-tools.js").createOpenClawTools
>[0];
let createOpenClawToolsForTestModule: typeof import("./mo-tools.js").createOpenClawTools;
...
function createPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/plugins/${params.id}/mo.plugin.json`,
    channels: [],
    providers: [],
    cliBackends: [],
    skills: [],
...
describe("optional media tool factory planning", () => {
  ...
  it("does not register the image tool without cheap vision availability evidence", async () => {
    const config: OpenClawConfig = {};
    const workspaceDir = "/tmp/mo-workspace";
