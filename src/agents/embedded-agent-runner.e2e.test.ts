...
vi.mock("mo/plugin-sdk/llm", async () => {
  const actual =
    await vi.importActual<typeof import("mo/plugin-sdk/llm")>("mo/plugin-sdk/llm");
...
let SessionManager: typeof import("mo/plugin-sdk/agent-sessions").SessionManager;
...
beforeAll(async () => {
  ...
  ({ SessionManager } = await import("mo/plugin-sdk/agent-sessions"));
  e2eWorkspace = await createEmbeddedAgentRunnerTestWorkspace("mo-embedded-agent-");
...
describe("runEmbeddedAgent", () => {
  ...
  it("resolves explicit OpenAI OpenClaw runs through Codex when auth order starts with Codex OAuth", async () => {
    ...
    await runEmbeddedAgent({
      ...
      timeoutMs: 5_000,
      agentDir,
      runId: nextRunId("codex-first-mo"),
      enqueue: immediateEnqueue,
    });
