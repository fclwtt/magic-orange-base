...
const ORIGINAL_BUILD_PRIVATE_QA = process.env.MO_BUILD_PRIVATE_QA;
const ORIGINAL_QA_FORCE_RUNTIME = process.env.MO_QA_FORCE_RUNTIME;
...
function restoreEnv(
  name: "MO_BUILD_PRIVATE_QA" | "MO_QA_FORCE_RUNTIME",
...
afterEach(() => {
  restoreEnv("MO_BUILD_PRIVATE_QA", ORIGINAL_BUILD_PRIVATE_QA);
  restoreEnv("MO_QA_FORCE_RUNTIME", ORIGINAL_QA_FORCE_RUNTIME);
});

describe("resolveModelRuntimePolicy", () => {
  it("ignores the QA force-runtime override when the private QA gate is unset", () => {
    deleteTestEnvValue("MO_BUILD_PRIVATE_QA");
    setTestEnvValue("MO_QA_FORCE_RUNTIME", "mo");
  ...
  it("respects the QA force-runtime override when the private QA gate is set", () => {
    ...
    setTestEnvValue("MO_BUILD_PRIVATE_QA", "1");
    setTestEnvValue("MO_QA_FORCE_RUNTIME", "mo");
  ...
  it("ignores invalid QA force-runtime values even when the private QA gate is set", () => {
    setTestEnvValue("MO_BUILD_PRIVATE_QA", "1");
    setTestEnvValue("MO_QA_FORCE_RUNTIME", "bogus");
  ...
  it("prefers provider-qualified agent entries over bare entries for inferred providers", () => {
    const config = {
      agents: {
        defaults: {
          models: {
            "claude-opus-4-7": { agentRuntime: { id: "mo" } },
