...
import { resolveOpenClawPluginToolsForOptions } from "./mo-plugin-tools.js";
import {
  isToolExplicitlyAllowedByFactoryPolicy,
  mergeFactoryPolicyList,
  resolveImageToolFactoryAvailable,
  resolveOptionalMediaToolFactoryPlan,
} from "./mo-tools.media-factory-plan.js";
import { applyNodesToolWorkspaceGuard } from "./mo-tools.nodes-workspace-guard.js";
import {
  collectPresentOpenClawTools,
  shouldIncludeUpdatePlanToolForOpenClawTools,
} from "./mo-tools.registration.js";
...
export function createOpenClawTools(
  ...
  options?.recordToolPrepStage?.("mo-tools:session-workspace");
  ...
  const imageTool = resolveImageToolFactoryAvailable({
    ...
    : null;
  options?.recordToolPrepStage?.("mo-tools:image-tool");
  const imageGenerateTool = optionalMediaTools.imageGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("mo-tools:image-generate-tool");
  const videoGenerateTool = optionalMediaTools.videoGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("mo-tools:video-generate-tool");
  const musicGenerateTool = optionalMediaTools.musicGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("mo-tools:music-generate-tool");
  const pdfTool =
    optionalMediaTools.pdf && options?.agentDir?.trim()
      ...
      : null;
  options?.recordToolPrepStage?.("mo-tools:pdf-tool");
  ...
  options?.recordToolPrepStage?.("mo-tools:web-search-tool");
  ...
  options?.recordToolPrepStage?.("mo-tools:web-fetch-tool");
  ...
  options?.recordToolPrepStage?.("mo-tools:message-tool");
  ...
  options?.recordToolPrepStage?.("mo-tools:nodes-tool");
  ...
  options?.recordToolPrepStage?.("mo-tools:core-tool-list");
  let allTools = tools;
  if (!options?.disablePluginTools) {
    ...
    options?.recordToolPrepStage?.("mo-tools:plugin-tools");
  }
  ...
  options?.recordToolPrepStage?.("mo-tools:tool-hooks");
  return allTools
    .map((tool) =>
