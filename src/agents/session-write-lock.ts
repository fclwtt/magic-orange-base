...
const CLEANUP_STATE_KEY = Symbol.for("mo.sessionWriteLockCleanupState");
const WATCHDOG_STATE_KEY = Symbol.for("mo.sessionWriteLockWatchdogState");
...
const SESSION_LOCKS = createFileLockManager("mo.session-write-lock");
...
const SESSION_WRITE_LOCK_ENV: Record<SessionWriteLockMsKey, string> = {
  acquireTimeoutMs: "MO_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS",
  staleMs: "MO_SESSION_WRITE_LOCK_STALE_MS",
  maxHoldMs: "MO_SESSION_WRITE_LOCK_MAX_HOLD_MS",
};
...
function shouldStartBackgroundWatchdog(): boolean {
  return process.env.VITEST !== "true" || process.env.MO_TEST_SESSION_LOCK_WATCHDOG === "1";
}
...
function isOpenClawSessionOwnerArgv(args: string[]): boolean {
  ...
  if (exe === "mo" || exe.endsWith("/mo") || exe.endsWith("/mo-gateway")) {
    return true;
  }
  if (
    normalized.some(
...
function inspectLockPayloadForSession(params: {
  ...
  if (
    ...
    return {
      ...inspected,
      stale: true,
      staleReasons: [...inspected.staleReasons, "non-mo-owner"],
    };
  }
