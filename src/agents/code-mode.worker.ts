...
// (e.g. "at mo-code-mode:user.js:2:37"). Lead with name+message so the
...
(() => {
  ...
  const catalog = Array.isArray(globalThis.__moCatalog) ? globalThis.__moCatalog : [];
  const apiFiles = Array.isArray(globalThis.__moApiFiles) ? globalThis.__moApiFiles : [];
  const namespaceDescriptors = Array.isArray(globalThis.__moNamespaces) ? globalThis.__moNamespaces : [];
  ...
  function request(method, args) {
    const id = String(globalThis.__moHostRequest(String(method), JSON.stringify(safe(args ?? []))));
  ...
  Object.defineProperties(globalThis, {
    ...
    __moSettleBridge: { value: settle },
    __moTakeOutput: { value: () => output.splice(0) },
  });
})();
`;

function buildUserSource(code: string): string {
  return `globalThis.__moResult = (async () => {\n${code}\n})()`;
}
...
async function createVm(params: {
  ...
  try {
    vm.setProp(vm.global, "__moHostRequest", hostRequest);
  } finally {
    hostRequest.dispose();
  }
  vm.evalCode(CONTROLLER_SOURCE, "mo-code-mode:controller.js").dispose();
...
function takeOutput(vm: QuickJS): unknown[] {
  const take = vm.global.getProp("__moTakeOutput");
  try {
...
function getResultHandle(vm: QuickJS): JSValueHandle {
  return vm.global.getProp("__moResult");
}
...
async function runResume(input: Extract<CodeModeWorkerInput, { kind: "resume" }>) {
  ...
  return runVmExecution({
    ...
    config: input.config,
    prepare: () => {
      const settle = vm.global.getProp("__moSettleBridge");
      try {
