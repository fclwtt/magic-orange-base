...
import { normalizeTrimmedStringList } from "@mo/normalization-core/string-normalization";
...
const MO_ATTRIBUTION_PRODUCT = "OpenClaw";
const MO_ATTRIBUTION_ORIGINATOR = "mo";
...
function formatOpenClawUserAgent(version: string): string {
  return `${MO_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
export function resolveProviderAttributionIdentity(
  ...
  return {
    product: MO_ATTRIBUTION_PRODUCT,
...
function buildNvidiaAttributionPolicy(
  ...
  return {
    ...
    headers: {
      "X-BILLING-INVOKE-ORIGIN": MO_ATTRIBUTION_PRODUCT,
    },
  };
}
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: MO_ATTRIBUTION_ORIGINATOR,
...
function buildXaiAttributionPolicy(
  ...
  return {
    ...
    hook: "request-headers",
    reviewNote:
      "xAI api.x.ai accepts a standard mo User-Agent. Companion originator/version headers mirror the OpenAI attribution shape for consistency; they are not validated against an xAI-specific spec and are expected to be ignored by xAI's OpenAI-compatible surface.",
    ...identity,
    headers: {
      originator: MO_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
