...
export const INTERNAL_RUNTIME_CONTEXT_BEGIN = "<<<BEGIN_MO_INTERNAL_CONTEXT>>>";
/** Closing delimiter for protected OpenClaw runtime context blocks. */
export const INTERNAL_RUNTIME_CONTEXT_END = "<<<END_MO_INTERNAL_CONTEXT>>>";

const ESCAPED_INTERNAL_RUNTIME_CONTEXT_BEGIN = "[[MO_INTERNAL_CONTEXT_BEGIN]]";
const ESCAPED_INTERNAL_RUNTIME_CONTEXT_END = "[[MO_INTERNAL_CONTEXT_END]]";

/** Notice inserted into runtime-generated context blocks. */
export const MO_RUNTIME_CONTEXT_NOTICE =
  "This context is runtime-generated, not user-authored. Keep internal details private.";
/** Header for context attached to the immediately preceding user message. */
export const MO_NEXT_TURN_RUNTIME_CONTEXT_HEADER =
  "OpenClaw runtime context for the immediately preceding user message.";
/** Header for runtime events passed as prompt context. */
export const MO_RUNTIME_EVENT_HEADER = "OpenClaw runtime event.";
/** Custom message type used for structured runtime-context messages. */
export const MO_RUNTIME_CONTEXT_CUSTOM_TYPE = "mo.runtime-context";

const LEGACY_INTERNAL_CONTEXT_HEADER =
  ["OpenClaw runtime context (internal):", MO_RUNTIME_CONTEXT_NOTICE, ""].join("\n") + "\n";
...
function isRuntimeContextPromptHeader(line: string): boolean {
  return (
    line === MO_NEXT_TURN_RUNTIME_CONTEXT_HEADER || line === MO_RUNTIME_EVENT_HEADER
  );
}
...
function stripRuntimeContextPromptPreface(text: string): string {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (
      ...
      nextLine.trim() === MO_RUNTIME_CONTEXT_NOTICE
    ) {
      changed = true;
      index += 1;
...
export function hasInternalRuntimeContext(text: string): boolean {
  ...
  return (
    ...
    text.includes(
      `${MO_NEXT_TURN_RUNTIME_CONTEXT_HEADER}\n${MO_RUNTIME_CONTEXT_NOTICE}`,
    ) ||
    text.includes(`${MO_RUNTIME_EVENT_HEADER}\n${MO_RUNTIME_CONTEXT_NOTICE}`)
  );
}
...
function isOpenClawRuntimeContextCustomMessage(message: unknown): boolean {
  ...
  return (
    candidate.role === "custom" && candidate.customType === MO_RUNTIME_CONTEXT_CUSTOM_TYPE
  );
}
