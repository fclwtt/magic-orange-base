...
import type { AssistantMessage, Message, Tool } from "mo/plugin-sdk/llm";
...
describeCacheLive("embedded agent runner prompt caching (live)", () => {
  beforeAll(async () => {
    liveRunnerRootDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-live-cache-"));
    ...
    previousCacheTraceEnv = {
      enabled: process.env.MO_CACHE_TRACE,
      file: process.env.MO_CACHE_TRACE_FILE,
      messages: process.env.MO_CACHE_TRACE_MESSAGES,
      prompt: process.env.MO_CACHE_TRACE_PROMPT,
      system: process.env.MO_CACHE_TRACE_SYSTEM,
    };
    setTestEnvValue("MO_CACHE_TRACE", "1");
    setTestEnvValue("MO_CACHE_TRACE_FILE", liveCacheTraceFile);
    setTestEnvValue("MO_CACHE_TRACE_MESSAGES", "0");
    setTestEnvValue("MO_CACHE_TRACE_PROMPT", "0");
    setTestEnvValue("MO_CACHE_TRACE_SYSTEM", "0");
  }, 120_000);
  ...
  afterAll(async () => {
    if (previousCacheTraceEnv) {
      ...
      restore("MO_CACHE_TRACE", previousCacheTraceEnv.enabled);
      restore("MO_CACHE_TRACE_FILE", previousCacheTraceEnv.file);
      restore("MO_CACHE_TRACE_MESSAGES", previousCacheTraceEnv.messages);
      restore("MO_CACHE_TRACE_PROMPT", previousCacheTraceEnv.prompt);
      restore("MO_CACHE_TRACE_SYSTEM", previousCacheTraceEnv.system);
    }
  ...
  describe("openai", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "MO_LIVE_OPENAI_CACHE_MODEL",
  ...
  describe("anthropic", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "MO_LIVE_ANTHROPIC_CACHE_MODEL",
