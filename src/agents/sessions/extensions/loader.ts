...
import * as bundledLlm from "mo/plugin-sdk/llm";
...
const VIRTUAL_MODULES: Record<string, unknown> = {
  ...
  "mo/plugin-sdk/agent-core": bundledAgentCore,
  "@mo/plugin-sdk/agent-core": bundledAgentCore,
  "mo/plugin-sdk/llm": bundledLlm,
  "@mo/plugin-sdk/llm": bundledLlm,
  "mo/plugin-sdk/agent-sessions": bundledAgentSessions,
  "@mo/plugin-sdk/agent-sessions": bundledAgentSessions,
};
...
const EXTENSION_LOADER_ALIAS_IMPORT_PATTERN =
  /(?:@mo\/plugin-sdk|mo\/plugin-sdk|@sinclair\/typebox|typebox)(?:\/[A-Za-z0-9_-]+)?/u;
...
function getExtensionLoaderAliases(): Record<string, string> {
  ...
  aliases = {
    ...
    "mo/plugin-sdk/agent-sessions": agentSessionsEntry,
    "@mo/plugin-sdk/agent-sessions": agentSessionsEntry,
...
function readResourceManifest(packageJsonPath: string): ResourceManifest | null {
  try {
    ...
    if (pkg.mo && typeof pkg.mo === "object") {
      return pkg.mo as ResourceManifest;
    }
    return null;
  } catch {
    return null;
 ...
 *
 * Checks for:
 * 1. package.json with "mo.extensions" field -> returns declared paths
 * 2. index.ts or index.js -> returns the index file
 *
 * Returns resolved paths or null if no entry points found.
 */
function resolveExtensionEntries(dir: string): string[] | null {
  // Check for package.json with "mo" field first
 ...
 * 3. Subdirectory with package.json: `extensions/* /package.json` with "mo" field → load what it declares
 *
