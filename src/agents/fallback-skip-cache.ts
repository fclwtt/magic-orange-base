 ...
 * MO_FALLBACK_SKIP_TTL_MS.
 */
const DEFAULT_FALLBACK_SKIP_TTL_MS = 0;
const FALLBACK_SKIP_TTL_ENV = "MO_FALLBACK_SKIP_TTL_MS";
...
function getState(): SkipCacheState {
  const globalStore = globalThis as typeof globalThis & {
    moFallbackSkipCache?: SkipBySession;
    moFallbackSkipCacheState?: SkipCacheState;
  };
  if (!globalStore.moFallbackSkipCacheState) {
    ...
    const buckets = globalStore.moFallbackSkipCache ?? new Map();
    globalStore.moFallbackSkipCacheState = {
      buckets,
      lastGlobalPruneAtMs: 0,
    };
    globalStore.moFallbackSkipCache = buckets;
  }
  return globalStore.moFallbackSkipCacheState;
}
