...
type AcpSessionIdentifiersRuntime = typeof import("@mo/acp-core/runtime/session-identifiers");
...
async function prepareAgentCommandExecution(opts: AgentCommandOpts, runtime: RuntimeEnv) {
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("mo agents list")}" to see configured agents.`,
      );
    }
  }
 ...
 * this every diagnostics consumer (Langfuse bridge, @mo/diagnostics-otel,
