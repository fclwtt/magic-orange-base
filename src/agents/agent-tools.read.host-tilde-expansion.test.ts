...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("mo-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("mo-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("mo-tilde-test-mkdir-");
  ...
  it("ignores MO_HOME for write operations", async () => {
    const moHome = await createTempDir("mo-home-override-", os.tmpdir());
    const dir = await createTempDir("mo-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");

    await withEnvAsync({ MO_HOME: moHome }, async () => {
      createHostWorkspaceWriteTool(moHome, { workspaceOnly: false });
      ...
      await expectMissingPath(fs.access(path.join(moHome, path.basename(testFile))));
    });
  });

  it("ignores MO_HOME for mkdir operations", async () => {
    const moHome = await createTempDir("mo-home-override-", os.tmpdir());
    const dir = await createTempDir("mo-tilde-test-mkdir-");
    ...
    await withEnvAsync({ MO_HOME: moHome }, async () => {
      ...
      expect((await fs.stat(newDir)).isDirectory()).toBe(true);
      await expectMissingPath(fs.access(path.join(moHome, path.basename(newDir))));
    });
  });

  it("ignores MO_HOME for readFile operations", async () => {
    const moHome = await createTempDir("mo-home-override-", os.tmpdir());
    const dir = await createTempDir("mo-tilde-test-edit-");
    ...
    await withEnvAsync({ MO_HOME: moHome }, async () => {
      createHostWorkspaceEditTool(moHome, { workspaceOnly: false });
      const content = await readEditOps().readFile(toTildePath(testFile));
  ...
  it("ignores MO_HOME for access operations", async () => {
    const moHome = await createTempDir("mo-home-override-", os.tmpdir());
    const dir = await createTempDir("mo-tilde-test-edit-");
