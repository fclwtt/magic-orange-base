...
describe("runCliTurnCompactionLifecycle", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-cli-compaction-"));
  ...
  it("ignores stale native harness ids when the active provider no longer matches", async () => {
    const sessionKey = "agent:main:mo-after-codex";
    const sessionId = "session-mo-after-codex";
    const sessionFile = path.join(tmpDir, "session-mo-after-codex.jsonl");
    const storePath = path.join(tmpDir, "sessions-mo-after-codex.json");
    await writeSessionFile({ sessionFile, sessionId });
