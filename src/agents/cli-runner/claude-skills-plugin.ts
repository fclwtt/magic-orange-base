...
const MO_CLAUDE_PLUGIN_NAME = "mo-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredOpenClawTmpDir(), "mo-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, MO_CLAUDE_PLUGIN_NAME);
