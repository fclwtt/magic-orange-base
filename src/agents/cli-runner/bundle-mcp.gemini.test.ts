...
describe("prepareCliBundleMcpConfig gemini", () => {
  it("writes Gemini system settings for bundle MCP servers", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/mo-bundle-mcp-gemini",
    ...
    expect(prepared.env?.MO_MCP_TOKEN).toBe("loopback-token-123");
    ...
    expect(raw.mcp?.allowed).toEqual(["mo"]);
  ...
  it("writes a unique capture token into per-attempt Gemini settings", async () => {
    ...
    try {
      ...
      expect(raw.mcpServers?.mo?.headers?.["x-mo-cli-capture-key"]).toBe("attempt-123");
  ...
  it("preserves inherited Gemini auth selection in generated system settings", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-gemini-settings-"));
