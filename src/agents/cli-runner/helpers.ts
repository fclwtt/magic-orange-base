...
import { extensionForMime } from "@mo/media-core/mime";
import {
  normalizeLowercaseStringOrEmpty,
  normalizeOptionalLowercaseString,
} from "@mo/normalization-core/string-coerce";
import { KeyedAsyncQueue } from "mo/plugin-sdk/keyed-async-queue";
...
function resolveCliImagePath(image: ImageContent): string {
  ...
  const digest = crypto
    ...
    .update(image.data)
    .digest("hex");
  return path.join(resolvePreferredOpenClawTmpDir(), "mo-cli-images", `${digest}${ext}`);
}
...
function resolveCliImageRoot(params: { backend: CliBackendConfig; workspaceDir: string }): string {
  if (params.backend.imagePathScope === "workspace") {
    return path.join(params.workspaceDir, ".mo-cli-images");
  }
  return path.join(resolvePreferredOpenClawTmpDir(), "mo-cli-images");
}
