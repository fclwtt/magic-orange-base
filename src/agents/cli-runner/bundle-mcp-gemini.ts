...
export async function writeGeminiSystemSettings(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-gemini-mcp-"));
...
export async function writeGeminiMcpCaptureSettings(params: {
  ...
  const mo = isRecord(mcpServers.mo) ? mcpServers.mo : {};
  const headers = normalizeStringRecord(mo.headers) ?? {};
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-gemini-mcp-attempt-"));
  const settingsPath = path.join(tempDir, "settings.json");
  await writeJson(
    settingsPath,
    {
      ...
      mcpServers: {
        ...
        mo: {
          ...
          headers: {
            ...headers,
            "x-mo-cli-capture-key": params.captureKey,
          },
        },
      },
    },
