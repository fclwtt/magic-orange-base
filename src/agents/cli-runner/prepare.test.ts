  ...
  return {
    mcpServers: {
      mo: {
        ...
        alwaysLoad: true,
        headers: {
          Authorization: "Bearer ${MO_MCP_TOKEN}",
          "x-session-key": "${MO_MCP_SESSION_KEY}",
          "x-mo-session-id": "${MO_MCP_SESSION_ID}",
          "x-mo-agent-id": "${MO_MCP_AGENT_ID}",
          "x-mo-account-id": "${MO_MCP_ACCOUNT_ID}",
          "x-mo-message-channel": "${MO_MCP_MESSAGE_CHANNEL}",
          "x-mo-current-channel-id": "${MO_MCP_CURRENT_CHANNEL_ID}",
          "x-mo-current-thread-ts": "${MO_MCP_CURRENT_THREAD_TS}",
          "x-mo-current-message-id": "${MO_MCP_CURRENT_MESSAGE_ID}",
          "x-mo-current-inbound-audio": "${MO_MCP_CURRENT_INBOUND_AUDIO}",
          "x-mo-inbound-event-kind": "${MO_MCP_INBOUND_EVENT_KIND}",
          "x-mo-source-reply-delivery-mode": "${MO_MCP_SOURCE_REPLY_DELIVERY_MODE}",
          "x-mo-require-explicit-message-target":
            "${MO_MCP_REQUIRE_EXPLICIT_MESSAGE_TARGET}",
          "x-mo-cli-capture-key": "${MO_MCP_CLI_CAPTURE_KEY}",
        },
      },
    },
  };
...
function createSessionFile() {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-cli-prepare-"));
  sessionFileEnvSnapshot ??= captureEnv(["MO_STATE_DIR"]);
  setTestEnvValue("MO_STATE_DIR", dir);
...
describe("shouldSkipLocalCliCredentialEpoch", () => {
  ...
  it("lets Gemini CLI preparation override generated MCP system settings auth", async () => {
    ...
    try {
      ...
      expect(generatedSettings.mcp?.allowed).toEqual(["mo"]);
      expect(generatedSettings.mcpServers?.mo?.url).toBe("http://127.0.0.1:31783/mcp");
  ...
  it("uses cwd for CLI system prompt workspace guidance", async () => {
    const { dir, sessionFile } = createSessionFile();
    const taskDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-cli-task-"));
    try {
      const context = await prepareCliRunContext({
        sessionId: "session-test",
        sessionFile,
  ...
  it("passes current turn kind into bundle MCP loopback env", async () => {
    ...
    try {
      ...
      expect(context.preparedBackend.env).toMatchObject({
        MO_MCP_SESSION_ID: "session-test",
        MO_MCP_MESSAGE_CHANNEL: "telegram",
        MO_MCP_CURRENT_CHANNEL_ID: "telegram:-100123:topic:42",
        MO_MCP_CURRENT_THREAD_TS: "42",
        MO_MCP_CURRENT_MESSAGE_ID: "reply-message-1",
        MO_MCP_CURRENT_INBOUND_AUDIO: "true",
        MO_MCP_INBOUND_EVENT_KIND: "room_event",
        MO_MCP_SOURCE_REPLY_DELIVERY_MODE: "message_tool_only",
        MO_MCP_REQUIRE_EXPLICIT_MESSAGE_TARGET: "true",
  ...
  it("renders CLI skills from sandbox-readable paths instead of persisted host snapshots", async () => {
    const { dir, sessionFile } = createSessionFile();
    const hostSkillDir = "/home/tzdai/.npm-global/lib/node_modules/mo/skills/gog";
    ...
    try {
      ...
      expect(context.systemPrompt).toContain(
        "/workspace/.mo/sandbox-skills/skills/gog/SKILL.md",
      );
  ...
  it("omits Claude CLI prompt skills when the native skills plugin can carry them", async () => {
    ...
    try {
      ...
      setCliRunnerPrepareTestDeps({
        prepareClaudeCliSkillsPlugin: vi.fn(async () => ({
          args: ["--plugin-dir", path.join(dir, "mo-skills")],
          cleanup: vi.fn(async () => undefined),
          pluginDir: path.join(dir, "mo-skills"),
        })),
      });
