...
const MO_MCP_TOOL_PREFIX = "mcp__mo__";

function normalizeCliMessagingToolName(toolName: string): string {
  return toolName.startsWith(MO_MCP_TOOL_PREFIX)
    ? toolName.slice(MO_MCP_TOOL_PREFIX.length)
    : toolName;
}
...
const CLI_BACKEND_PRESERVE_ENV = "MO_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.MO_MCP_TOKEN ? "set" : "missing"}`,
    `sessionKey=${childEnv.MO_MCP_SESSION_KEY ? "set" : "<empty>"}`,
    `agentId=${childEnv.MO_MCP_AGENT_ID || "<empty>"}`,
    `accountId=${childEnv.MO_MCP_ACCOUNT_ID || "<empty>"}`,
    `messageChannel=${childEnv.MO_MCP_MESSAGE_CHANNEL || "<empty>"}`,
  ].join(" ");
}
    ...
    completedOutput = await enqueueCliRun(queueKey, async () => {
      ...
      try {
        ...
        if (logOutputText) {
          ...
          if (env.MO_MCP_TOKEN || env.MO_MCP_SESSION_KEY || env.MO_MCP_AGENT_ID) {
