...
describe("prepareCliBundleMcpConfig", () => {
  ...
  it("loads workspace bundle MCP plugins from the configured workspace root", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "mo-cli-bundle-mcp-workspace-root-",
    );
    const pluginRoot = path.join(workspaceDir, ".mo", "extensions", "workspace-probe");
  ...
  it("merges loopback overlay config with bundle MCP servers", async () => {
    ...
    const prepared = await prepareBundleProbeCliConfig({
      additionalConfig,
      env: {
        MO_MCP_TOKEN: "loopback-token-123",
    ...
    const otherEnvPrepared = await prepareBundleProbeCliConfig({
      additionalConfig,
      env: {
        MO_MCP_TOKEN: "other-loopback-token",
        MO_MCP_CLI_CAPTURE_KEY: "",
      },
    });
    ...
    expect(Object.keys(raw.mcpServers ?? {}).toSorted()).toEqual(["bundleProbe", "mo"]);
    expect(raw.mcpServers?.mo?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(raw.mcpServers?.mo?.headers?.Authorization).toBe("Bearer loopback-token-123");
    expect(raw.mcpServers?.mo?.headers?.["x-mo-cli-capture-key"]).toBe("");
    ...
    expect(attemptRaw.mcpServers?.mo?.headers?.Authorization).toBe(
      "Bearer loopback-token-123",
    );
    expect(attemptRaw.mcpServers?.mo?.headers?.["x-mo-cli-capture-key"]).toBe(
      "attempt-123",
    );
  ...
  it("preserves extra env values alongside generated MCP config", async () => {
    ...
    const prepared = await prepareCliBundleMcpConfig({
      ...
      env: {
        MO_MCP_TOKEN: "loopback-token-123",
        MO_MCP_SESSION_KEY: "agent:main:telegram:group:chat123",
      },
    });
  ...
  it("leaves args untouched when bundle MCP is disabled", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/mo-bundle-mcp-disabled",
    });
