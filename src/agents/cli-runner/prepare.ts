...
export async function prepareCliRunContext(
  ...
  const preparedBackend = await prepareCliBundleMcpConfig({
    ...
    env: mcpLoopbackRuntime
      ? {
          MO_MCP_TOKEN: prepareDeps.resolveMcpLoopbackBearerToken(
            mcpLoopbackRuntime,
            params.senderIsOwner === true,
          ),
          MO_MCP_AGENT_ID: sessionAgentId ?? "",
          MO_MCP_ACCOUNT_ID: params.agentAccountId ?? "",
          MO_MCP_SESSION_KEY: params.sessionKey ?? "",
          MO_MCP_SESSION_ID: params.sessionId,
          MO_MCP_MESSAGE_CHANNEL: params.messageChannel ?? params.messageProvider ?? "",
          MO_MCP_CURRENT_CHANNEL_ID: params.currentChannelId ?? "",
          MO_MCP_CURRENT_THREAD_TS: params.currentThreadTs ?? "",
          MO_MCP_CURRENT_MESSAGE_ID:
            params.currentMessageId != null ? String(params.currentMessageId) : "",
          MO_MCP_CURRENT_INBOUND_AUDIO: params.currentInboundAudio === true ? "true" : "",
          MO_MCP_INBOUND_EVENT_KIND: params.currentInboundEventKind ?? "",
          MO_MCP_SOURCE_REPLY_DELIVERY_MODE: params.sourceReplyDeliveryMode ?? "",
          MO_MCP_REQUIRE_EXPLICIT_MESSAGE_TARGET: requireExplicitMessageTarget ? "true" : "",
