...
const LIVE = isLiveTestEnabled(["MO_LIVE_CLI_MCP_GEMINI"]);
...
async function startLocalStreamableHttpMcpServer(): Promise<{
  ...
  const mcpServer = new McpServer({ name: "mo-gemini-live-probe", version: "1.0.0" });
  mcpServer.tool("mo_live_probe", "OpenClaw Gemini MCP live probe", async () => ({
...
describeLive("Gemini CLI MCP settings smoke", () => {
  it("connects to an OpenClaw-configured streamable-http server", async () => {
    const geminiCommand = process.env.MO_LIVE_GEMINI_COMMAND ?? "gemini";
    ...
    try {
      ...
      expect(output).toContain("moLiveProbe");
