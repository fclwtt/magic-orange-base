...
export function setupCliBundleMcpTestHarness(): void {
  beforeAll(async () => {
    ...
    envSnapshot = captureEnv(["MO_BUNDLED_PLUGINS_DIR"]);
    bundleProbeHomeDir = await tempHarness.createTempDir("mo-cli-bundle-mcp-home-");
    bundleProbeWorkspaceDir = await tempHarness.createTempDir("mo-cli-bundle-mcp-workspace-");
    const emptyBundledDir = await tempHarness.createTempDir("mo-cli-bundle-mcp-bundled-");
    setTestEnvValue("MO_BUNDLED_PLUGINS_DIR", emptyBundledDir);
