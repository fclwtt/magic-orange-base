...
const EXEC_SHELL_SNAPSHOT_ENV = "MO_EXEC_SHELL_SNAPSHOT";
...
describe("exec shell snapshots", () => {
  ...
  it("leaves commands unchanged when trusted process env disables snapshots", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-disabled-state-"));
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-disabled-home-"));
    ...
    expect(wrapped).toBe(command);
    expect(fs.existsSync(resolveShellSnapshotDir({ MO_STATE_DIR: stateDir }))).toBe(false);
  });
  ...
  it("does not honor per-call env for selecting the snapshot state dir", async () => {
    ...
    const trustedStateDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "mo-snapshot-trusted-state-"),
    );
    const untrustedStateDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "mo-snapshot-untrusted-state-"),
    );
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-state-home-"));
    const untrustedHome = fs.mkdtempSync(
      path.join(os.tmpdir(), "mo-snapshot-untrusted-home-"),
    );
    ...
    const wrapped = await maybeWrapCommandWithShellSnapshot({
      ...
      env: {
        ...
        MO_STATE_DIR: untrustedStateDir,
    ...
    expect(fs.existsSync(resolveShellSnapshotDir({ MO_STATE_DIR: untrustedStateDir }))).toBe(
      false,
    );
    expect(fs.existsSync(resolveShellSnapshotDir({ MO_STATE_DIR: trustedStateDir }))).toBe(
      true,
    );
  ...
  it("captures bash startup aliases, functions, and safe environment without secrets", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-cwd-"));
    ...
    fs.writeFileSync(
      ...
      [
        ...
        '[ "$MO_SHELL" = exec ] && alias oc_snap_exec_alias="printf marker-ok"',
  ...
  it("captures bash aliases behind common interactive-only guards", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-interactive-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-interactive-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-interactive-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
    fs.writeFileSync(
      path.join(home, ".bashrc"),
  ...
  it("preserves per-call safe env overrides after trusted capture", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-env-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-env-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-env-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
  ...
  it("preserves per-call env outside the snapshot allowlist", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-plugin-env-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-plugin-env-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-plugin-env-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
  ...
  it("does not let non-fingerprinted env change captured shell state", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-branch-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-branch-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-branch-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
    fs.writeFileSync(
      path.join(home, ".bashrc"),
  ...
  it("refreshes stale snapshot files when startup files source alias fragments", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-refresh-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-refresh-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-refresh-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
  ...
  it("refreshes fresh cached snapshots that no longer parse", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-corrupt-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-corrupt-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-corrupt-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
  ...
  it("refuses to persist aliases or functions with literal secret-looking values", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-secret-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-secret-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-secret-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
    fs.writeFileSync(
      path.join(home, ".bashrc"),
  ...
  it("captures zsh aliases in sourceable form", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-zsh-home-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-zsh-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-zsh-cwd-"));
    tempDirs.push(home, stateDir, cwd);
    setSnapshotStateForTest(stateDir, { home });
    fs.writeFileSync(
  ...
  it("captures zsh startup state from ZDOTDIR", async () => {
    ...
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-zdot-home-"));
    const zdotdir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-zdot-dir-"));
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-zdot-state-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "mo-snapshot-zdot-cwd-"));
