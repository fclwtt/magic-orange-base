...
describe("createCliJsonlStreamingParser", () => {
  ...
  it("streams Gemini message deltas and tool events", () => {
    ...
    expect(starts).toEqual([
      {
        toolCallId: "tool-1",
        name: "mcp_mo_create_goal",
        args: { objective: "Update files" },
      },
    ]);
    expect(results).toEqual([
      { toolCallId: "tool-1", name: "mcp_mo_create_goal", isError: false, result: "created" },
    ]);
  ...
  it.each(["server_tool_use", "mcp_tool_use"])("recognizes %s blocks", (type) => {
      ...
      [
        ...
        JSON.stringify({
          ...
          event: {
            type: "content_block_delta",
            index: 0,
            delta: { type: "input_json_delta", partial_json: '{"query":"mo"}' },
          },
        }),
        JSON.stringify({
          type: "stream_event",
    ...
    expect(starts).toEqual([
      { toolCallId: "toolu_hosted", name: "web_search", args: { query: "mo" } },
    ]);
  });
