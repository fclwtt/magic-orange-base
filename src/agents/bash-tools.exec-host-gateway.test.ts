...
describe("processGatewayAllowlist", () => {
  ...
  it("requires approval for security audit suppression edits unless yolo mode is active", async () => {
    ...
    const result = await runGatewayAllowlist({
      command: "mo config set security.audit.suppressions '[]'",
      security: "full",
      ask: "on-miss",
    });
  ...
  it("does not require suppression edit approval for read-only suppression inspection", async () => {
    evaluateShellAllowlistWithAuthorizationMock.mockReturnValue({
      ...
      allowlistSatisfied: true,
      segments: [
        { resolution: null, argv: ["mo", "config", "get", "security.audit.suppressions"] },
      ],
      segmentAllowlistEntries: [],
    });
    resolveExecHostApprovalContextMock.mockReturnValue({
    ...
    await runGatewayAllowlist({
      command: "mo config get security.audit.suppressions",
      security: "full",
      ask: "on-miss",
    });
  ...
  it("does not require suppression edit approval for profile-scoped read-only inspection", async () => {
    evaluateShellAllowlistWithAuthorizationMock.mockReturnValue({
      ...
      segments: [
        {
          resolution: null,
          argv: ["mo", "--profile", "rescue", "config", "get", "security.audit.suppressions"],
        },
      ],
      segmentAllowlistEntries: [],
    });
    ...
    await runGatewayAllowlist({
      command: "mo --profile rescue config get security.audit.suppressions",
      security: "full",
      ask: "on-miss",
    });
  ...
  it("requires suppression edit approval when a mutating segment follows read-only inspection", async () => {
    evaluateShellAllowlistWithAuthorizationMock.mockReturnValue({
      ...
      segments: [
        ...
        {
          resolution: null,
          argv: ["mo", "config", "set", "security.audit.suppressions", "[]"],
        },
      ],
      segmentAllowlistEntries: [],
    });
    ...
    const result = await runGatewayAllowlist({
      command:
        "mo config get security.audit.suppressions; mo config set security.audit.suppressions '[]'",
      security: "full",
      ask: "on-miss",
    });
  ...
  it("requires suppression edit approval when a heredoc patch follows read-only inspection", async () => {
    evaluateShellAllowlistWithAuthorizationMock.mockReturnValue({
      ...
      segments: [
        {
          raw: "mo config get security.audit.suppressions",
          resolution: null,
          argv: ["mo", "config", "get", "security.audit.suppressions"],
        },
        {
          raw: "mo config patch --stdin <<'EOF'",
          resolution: null,
          argv: ["mo", "config", "patch", "--stdin"],
        },
      ],
      segmentAllowlistEntries: [],
    });
    ...
    const result = await runGatewayAllowlist({
      command: `mo config get security.audit.suppressions; mo config patch --stdin <<'EOF'
...
EOF`,
  ...
  it("keeps webchat diagnostics approvals as direct pasteable followups", async () => {
    ...
    const result = await runGatewayAllowlist({
      command: "mo gateway diagnostics export --json",
      trigger: "diagnostics",
    ...
    expect(followupText).toContain("Path: /tmp/mo-diagnostics.zip");
  ...
  it("uses async agent followups for explicit webchat approval mode", async () => {
    ...
    const result = await runGatewayAllowlist({
      command: "mo sessions export-trajectory --json",
      approvalFollowupMode: "agent",
      sessionId: "approval-session",
      sessionStore: "/tmp/mo-sessions.json",
      turnSourceChannel: "webchat",
    });
