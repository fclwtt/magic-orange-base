...
async function withTempExecApprovalsFile(
  ...
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "mo-cli-exec-approvals-"));
...
async function withTempOpenClawHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "mo-cli-home-"));
  try {
    await withEnvAsync({ MO_HOME: home }, async () => run(home));
  } finally {
    await fs.rm(home, { recursive: true, force: true });
  }
}
...
describe("runCliAgent spawn path", () => {
  ...
  it("passes Claude system prompts through a file instead of argv", async () => {
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      expect(systemPromptPath).toContain("mo-cli-system-prompt-");
  ...
  it("passes prepared backend env to the spawned CLI process", async () => {
    ...
    await executePreparedCliRun(
      buildPreparedCliRunContext({
        ...
        preparedEnv: {
          GEMINI_CLI_HOME: "/tmp/mo-gemini-profile-home",
          GEMINI_CLI_SYSTEM_SETTINGS_PATH: "/tmp/mo-gemini-system-settings.json",
        },
      }),
    );
    ...
    expect(input.env?.GEMINI_CLI_HOME).toBe("/tmp/mo-gemini-profile-home");
    expect(input.env?.GEMINI_CLI_SYSTEM_SETTINGS_PATH).toBe(
      "/tmp/mo-gemini-system-settings.json",
    );
  });

  it("passes OpenClaw skills to Claude as a session plugin", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-cli-skills-"));
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      expect(manifest.name).toBe("mo-skills");
  ...
  it("keeps captured live prepared backend cleanup with the whole-run owner", async () => {
    const mcpConfigDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "mo-cli-captured-mcp-config-"),
    );
  ...
  it("preserves completed output when system prompt cleanup fails after delivery", async () => {
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      const captureHandle = markMcpLoopbackToolCallStarted({
        captureKey: input.env?.MO_MCP_CLI_CAPTURE_KEY ?? "",
        toolName: "message",
  ...
  it("preserves Claude resume args when building live session argv", () => {
    ...
    expect(args).not.toContain("mo-session");
...
`,
  ...
  it("does not create exec approvals file while resolving Claude live policy", async () => {
    await withTempOpenClawHome(async (home) => {
      const approvalsPath = path.join(home, ".mo", "exec-approvals.json");
      let stdoutListener: ((chunk: string) => void) | undefined;
      const stdin = {
...
`,
  ...
  it("restarts Claude live sessions when selected skills change", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-live-skills-"));
  ...
  it("can preserve selected clearEnv keys for live CLI backend probes", async () => {
    try {
      process.env.MO_LIVE_CLI_BACKEND_PRESERVE_ENV = '["SAFE_CLEAR"]';
      process.env.SAFE_CLEAR = "from-base";
      mockSuccessfulCliRun();
      await executePreparedCliRun(
        buildPreparedCliRunContext({
      ...
      delete process.env.MO_LIVE_CLI_BACKEND_PRESERVE_ENV;
  ...
  it("loads workspace bootstrap files into the Claude CLI system prompt", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "mo-cli-bootstrap-context-"),
    );
