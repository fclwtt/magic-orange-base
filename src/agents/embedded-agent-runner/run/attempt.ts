...
import {
  ...
  MO_EMBEDDED_CONTEXT_ENGINE_HOST,
  ...
  try {
    ...
    if (activeContextEngine && activeContextEngine.info.id !== "legacy") {
      assertContextEngineHostSupport({
        ...
        operation: "agent-run",
        host: MO_EMBEDDED_CONTEXT_ENGINE_HOST,
      });
    }
    ...
    const toolsRaw = !shouldConstructTools
      ...
      : (() => {
          ...
          corePluginToolStages.mark("attempt:create-mo-coding-tools");
    ...
    try {
      ...
      await withOwnedSessionWriteLock(async () => {
        await runAttemptContextEngineBootstrap({
          ...
          contextEngineHostSupport: MO_EMBEDDED_CONTEXT_ENGINE_HOST,
      ...
      // rehydration also restores OpenClaw runtime's auto-compaction (mo#75799), so re-apply
      // both guards.
      applyAgentCompactionSettingsFromConfig({
        settingsManager,
        cfg: params.config,
      ...
      if (activeContextEngine?.info.ownsCompaction === true) {
        ...
        const contextEngineLoopRuntimeSettings = buildContextEngineRuntimeSettings({
          contextEngineHost: MO_EMBEDDED_CONTEXT_ENGINE_HOST,
          provider: params.provider,
      ...
      toolSearchCatalogExecutor = async (toolParams) => {
        try {
          if (toolParams.source === "mo" && toolParams.sourceName === "core") {
      ...
      try {
        ...
        await withOwnedSessionWriteLock(async () => {
          ...
          // double-compaction. See: https://github.com/mo/mo/issues/9282
          ...
          if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
            try {
              activeSessionManager.appendCustomEntry("mo:prompt-error", {
                timestamp: Date.now(),
                runId: params.runId,
                sessionId: params.sessionId,
                provider: params.provider,
      ...
      // See: https://github.com/mo/mo/issues/8643
