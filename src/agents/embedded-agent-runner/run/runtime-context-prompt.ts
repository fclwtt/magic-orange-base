...
import {
  ...
  MO_RUNTIME_CONTEXT_CUSTOM_TYPE,
  MO_RUNTIME_CONTEXT_NOTICE,
  MO_RUNTIME_EVENT_HEADER,
} from "../../internal-runtime-context.js";
import type { CurrentInboundPromptContext } from "./params.js";

const MO_RUNTIME_EVENT_USER_PROMPT = "Continue the OpenClaw runtime event.";
...
export type RuntimeContextCustomMessage = {
  ...
  content: string;
  display: false;
  details: { source: "mo-runtime-context" };
  timestamp: number;
};
...
export function resolveRuntimeContextPromptParts(params: {
  ...
  if (!prompt.trim()) {
    return runtimeContext
      ? {
          prompt: MO_RUNTIME_EVENT_USER_PROMPT,
          ...(modelPromptText.trim() && modelPromptText !== MO_RUNTIME_EVENT_USER_PROMPT
            ? { modelPrompt: modelPromptText }
            : {}),
          runtimeContext,
          runtimeOnly: true,
...
export function buildRuntimeContextCustomMessage(
  ...
  return {
    role: "custom",
    customType: MO_RUNTIME_CONTEXT_CUSTOM_TYPE,
