...
describe("buildAttemptSystemPrompt", () => {
  it("injects workspace identity context", () => {
    ...
    const result = buildAttemptSystemPrompt({
      ...
      embeddedSystemPrompt: {
        ...
        userTimezone: "UTC",
        contextFiles: [
          { path: "/tmp/mo/SOUL.md", content: "SOUL_CONTEXT_MARKER" },
          { path: "/tmp/mo/IDENTITY.md", content: "IDENTITY_CONTEXT_MARKER" },
          { path: "/tmp/mo/USER.md", content: "USER_CONTEXT_MARKER" },
        ],
      },
    ...
    expect(result.systemPrompt).toContain("## /tmp/mo/SOUL.md");
    expect(result.systemPrompt).toContain("SOUL_CONTEXT_MARKER");
    expect(result.systemPrompt).toContain("## /tmp/mo/IDENTITY.md");
    expect(result.systemPrompt).toContain("IDENTITY_CONTEXT_MARKER");
    expect(result.systemPrompt).toContain("## /tmp/mo/USER.md");
  ...
  it("preserves bootstrap Project Context", () => {
    ...
    expect(result.systemPrompt).toContain("## /tmp/mo/USER.md");
    expect(result.systemPrompt).toContain("USER_CONTEXT_MARKER");
    expect(result.systemPrompt).toContain("## /tmp/mo/BOOTSTRAP.md");
