...
type AbortSettleTimeoutEnv = Partial<
  Pick<NodeJS.ProcessEnv, "MO_EMBEDDED_ABORT_SETTLE_TIMEOUT_MS" | "MO_TEST_FAST">
>;
...
export function resolveEmbeddedAbortSettleTimeoutMs(
  ...
  const override = parseStrictPositiveInteger(env.MO_EMBEDDED_ABORT_SETTLE_TIMEOUT_MS);
  if (override !== undefined) {
    return override;
  }
  return env.MO_TEST_FAST === "1" ? 250 : 2_000;
}
