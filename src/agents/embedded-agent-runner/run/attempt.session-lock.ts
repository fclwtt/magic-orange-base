...
import { clampTimerTimeoutMs } from "@mo/normalization-core/number-coercion";
...
type PromptReleaseStreamFn = ((...args: unknown[]) => unknown) & {
  __moSessionLockPromptReleaseInstalled?: boolean;
};
...
const EMBEDDED_ATTEMPT_SESSION_FILE_OWNER_STATE_KEY = Symbol.for(
  "mo.embeddedAttemptSessionFileOwnerState",
);
...
export function installPromptSubmissionLockRelease(params: {
  ...
  if (currentStreamFn["__moSessionLockPromptReleaseInstalled"] === true) {
    return;
  }
  ...
  wrappedStreamFn["__moSessionLockPromptReleaseInstalled"] = true;
