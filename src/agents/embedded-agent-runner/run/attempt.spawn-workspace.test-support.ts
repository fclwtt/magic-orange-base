...
vi.mock("../cache-ttl.js", () => ({
  appendCacheTtlTimestamp: (
    ...
    data: unknown,
  ) => sessionManager.appendCustomEntry?.("mo.cache-ttl", data),
...
export async function createContextEngineAttemptRunner(params: {
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-ctx-engine-workspace-"));
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-ctx-engine-agent-"));
  ...
  const previousTrajectoryEnv = process.env.MO_TRAJECTORY;
  if (params.trajectory !== true) {
    process.env.MO_TRAJECTORY = "0";
  }
  try {
    return await (
      await loadRunEmbeddedAttempt()
    ...
    if (previousTrajectoryEnv === undefined) {
      delete process.env.MO_TRAJECTORY;
    } else {
      process.env.MO_TRAJECTORY = previousTrajectoryEnv;
    }
  }
}
