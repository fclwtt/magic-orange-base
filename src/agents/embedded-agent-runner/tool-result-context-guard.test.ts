...
describe("installContextEngineLoopHook", () => {
  ...
  it("projects marked model prompts for ingest without leaking the marker to assembly", async () => {
    ...
    expect(JSON.stringify(afterTurnMessage)).not.toContain("__moTranscriptPromptText");
    expect(assembleMessage).toMatchObject({
      role: "user",
      content: "model-only hook context\n\nvisible prompt",
    });
    expect(JSON.stringify(assembleMessage)).not.toContain("__moTranscriptPromptText");
    expect(transformedMessage).toMatchObject({
      role: "user",
      content: "model-only hook context\n\nvisible prompt",
    });
    expect(JSON.stringify(transformedMessage)).not.toContain("__moTranscriptPromptText");
  });
