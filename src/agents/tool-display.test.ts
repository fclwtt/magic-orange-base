...
describe("tool display details", () => {
  it("summarizes tool-search code targets from described tool ids", () => {
    expect(
      resolveToolSearchCodeDisplayTarget({
        code: "const tool = await mo.tools.describe('mo:core:exec'); return await mo.tools.call(tool.id, { command: 'echo hi' });",
      }),
    ).toEqual({
      toolName: "mo:core:exec",
  ...
  it("normalizes direct tool-search catalog ids to native display names and args", () => {
    expect(
      resolveToolSearchCodeDisplayTarget({
        code: 'return await mo.tools.call("mo:core:exec", { command: "echo hi" });',
      }),
    ).toEqual({
      toolName: "mo:core:exec",
      displayToolName: "exec",
  ...
  it("preserves JS numeric literals in tool-search call args", () => {
    expect(
      resolveToolSearchCodeDisplayTarget({
        code: 'return await mo.tools.call("web_search", { query: "OpenClaw", count: 1e3, limit: +3, threshold: .5 });',
      })?.displayArgs,
    ).toEqual({
      query: "OpenClaw",
      count: 1000,
      ...
      formatToolDetail(
        resolveToolDisplay({
          ...
          args: {
            ...
            search_queries: ["mo github", "mo repository"],
            count: 5,
          },
        }),
      ),
    ).toBe(
      'for "Find the OpenClaw repository on GitHub", "mo github", "mo repository" (top 5)',
    );
  });
  ...
  it("summarizes exec commands with context", () => {
    const detail = formatToolDetail(
      resolveToolDisplay({
        ...
        args: {
          command:
            "set -euo pipefail\ngit -C /Users/adityasingh/.mo/workspace status --short | head -n 3",
          workdir: "/Users/adityasingh/.mo/workspace",
        },
      }),
    );
  ...
  it("uses compact workspace markers for common workspace paths", () => {
    expect(
      formatToolDetail(
        resolveToolDisplay({
          name: "bash",
          args: { command: "git fetch", workdir: "/Users/peter/mantis-workspace/mo" },
          detailMode: "explain",
        }),
      ),
    ...
    expect(
      formatToolDetail(
        resolveToolDisplay({
          name: "bash",
          args: { command: "git status", workdir: "/Users/peter/Projects/mo" },
          detailMode: "explain",
        }),
      ),
      ...
      formatToolDetail(
        resolveToolDisplay({
          ...
          args: {
            ...
            workdir: "/root/.mo/sandboxes/agent-clawsweeper-sandbox-discor-766423d0",
          },
          detailMode: "explain",
        }),
      ),
