...
type CatalogSource = "mo" | "mcp" | "client";
...
async function runModelCode(code, timeoutMs) {
  ...
  Object.defineProperties(sandbox, {
    ...
    mo: { value: controller.mo, enumerable: true },
  });
...
const SESSION_CATALOGS_KEY = Symbol.for("mo.toolSearch.sessionCatalogs");
...
function classifyTool(tool: CatalogTool): {
  ...
  if (pluginId) {
    return { source: "mo", sourceName: pluginId };
  }
  return { source: "mo", sourceName: "core" };
}
...
function formatUnknownToolIdError(
  ...
  const recoveryText =
    options.recoverySurface === "code-mode"
      ? "Use mo.tools.search to find a tool, mo.tools.describe to inspect it, then mo.tools.call with the exact id or name."
  ...
  return [
    {
      ...
      description:
        "Run JavaScript in an isolated Node subprocess with mo.tools.search, mo.tools.describe, and mo.tools.call for large tool catalogs.",
      parameters: Type.Object({
        code: Type.String({
          description:
            "JavaScript body for an async function. Use return to return the final value. The mo.tools bridge is available.",
        }),
      }),
      execute: async (
        toolCallId: string,
