...
async function withManagedMediaRoot<T>(run: (ctx: { stateDir: string }) => Promise<T>) {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-managed-media-"));
  try {
    return await withEnvAsync({ MO_STATE_DIR: stateDir }, async () => {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside mo tmp root",
      media: path.join(os.tmpdir(), "outside-mo", "passwd"),
      expected: /sandbox/i,
    },
    {
