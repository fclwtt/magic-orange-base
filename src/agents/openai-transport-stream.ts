...
const OPENAI_RESPONSES_REASONING_REPLAY_META_KEY = "__mo_replay";
const OPENAI_RESPONSES_REASONING_REPLAY_BLOCK_META_KEY = "moReasoningReplay";
...
function createResponsesFirstEventTimeoutError(model: Model, timeoutMs: number): Error {
  return new Error(
    `Azure OpenAI Responses stream did not deliver a first event within ${timeoutMs}ms after HTTP streaming headers. ` +
      ...
      "The provider may be stalled while parsing the tool payload; retry with a smaller tool surface or enable MO_DEBUG_MODEL_PAYLOAD=tools to inspect exposed tools.",
  );
}
  ...
  return (model, context, options) => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (
          (options as { moCodeModeToolSurface?: unknown } | undefined)
            ?.moCodeModeToolSurface === true
        ) {
...
function resolveCacheRetention(cacheRetention: string | undefined): "short" | "long" | "none" {
  ...
  if (typeof process !== "undefined" && process.env.MO_CACHE_RETENTION === "long") {
    return "long";
  }
  return "short";
}
...
function isOpenAICodexResponsesModel(model: Model): boolean {
  return (
    ...
    (model.api === "openai-chatgpt-responses" ||
      model.api === "mo-openai-responses-transport")
  );
}
