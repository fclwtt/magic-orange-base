...
export const DEFAULT_MAX_OUTPUT = clampWithDefault(
  readEnvInt("MO_BASH_MAX_OUTPUT_CHARS", "PI_BASH_MAX_OUTPUT_CHARS"),
  200_000,
  1_000,
  200_000,
);
...
export const DEFAULT_PENDING_MAX_OUTPUT = clampWithDefault(
  readEnvInt("MO_BASH_PENDING_MAX_OUTPUT_CHARS"),
  30_000,
  1_000,
  200_000,
);
...
function wrapPosixCommandWithPathPrepend(
  ...
  env.MO_PREPEND_PATH = pathPrepend.join(path.delimiter);

  return `export PATH="\${MO_PREPEND_PATH}\${PATH:+:$PATH}"; unset MO_PREPEND_PATH; ${command}`;
}
