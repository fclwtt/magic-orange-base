...
describe("buildAgentSystemPrompt", () => {
  it("resolves helper session keys to scoped prompt surfaces", () => {
    ...
    expect(resolveAgentPromptSurfaceForSessionKey("agent:main")).toBe("mo_main");
    expect(resolveAgentPromptSurfaceForSessionKey(undefined)).toBe("mo_main");
  });
  ...
  it("includes an OpenClaw control section", () => {
    ...
    expect(prompt).toContain("mo gateway status|restart|start|stop");
  ...
  it("includes embed guidance only for webchat sessions", () => {
    ...
    expect(prompt).toContain(
      '[embed url="/__mo__/canvas/documents/cv_123/index.html" title="Status" height="320" /]',
    );
    expect(prompt).toContain(
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("Docs: /tmp/mo/docs");
    expect(prompt).toContain(
  ...
  it("includes docs guidance when docsPath is provided", () => {
    ...
    expect(prompt).toContain("Docs: /tmp/mo/docs");
    expect(prompt).toContain("Source: /tmp/mo");
    expect(prompt).toContain(
      "Docs are authoritative for OpenClaw self-knowledge: before understanding how OpenClaw works (memory/daily notes, sessions, tools, Gateway, config, commands, project context), use `read` or search local docs first; treat AGENTS.md/project context, workspace/profile/memory notes, and `memory_search` as instruction context or user memory, not OpenClaw design/implementation knowledge.",
    );
  ...
  it("falls back to public docs and GitHub source guidance when local docs are unavailable", () => {
    ...
    expect(prompt).toContain("Docs: https://docs.mo.ai");
    expect(prompt).toContain("Source: https://github.com/mo/mo");
    expect(prompt).toContain(
  ...
  it("includes bootstrap instructions in system prompt when bootstrap is pending", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/mo",
      bootstrapMode: "full",
      contextFiles: [{ path: "/tmp/mo/BOOTSTRAP.md", content: "Ask who I am." }],
    });
    ...
    expect(prompt).toContain("## /tmp/mo/BOOTSTRAP.md");
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/mo. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildAgentBootstrapSystemPromptSections", () => {
  it("can render bootstrap guidance without duplicating Project Context", () => {
    ...
    expect(sections).not.toContain("## /tmp/mo/BOOTSTRAP.md");
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`mo ...`, `acpx ...`)");
