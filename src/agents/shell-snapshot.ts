...
const CAPTURE_MARKER = "__MO_SHELL_SNAPSHOT_CAPTURE__";
const ENV_MARKER = "__MO_SHELL_SNAPSHOT_ENV__";
...
async function captureShellSnapshot(opts: ShellSnapshotWrapOptions): Promise<string | null> {
  ...
  const captureOutputDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-shell-snapshot-"));
...
function buildTrustedSnapshotCaptureEnv(
  ...
  // MO_SHELL is injected by the exec runtime, so startup files can keep
  // their documented exec-specific branches without trusting model input.
  if (runtimeEnv.MO_SHELL === "exec") {
    env.MO_SHELL = "exec";
  }
  return env;
}
