...
describe("resolveAgentConfig", () => {
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/mo");
  });

  it("uses MO_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "mo-home");
    withEnv({ MO_HOME: home }, () => {
      const workspace = resolveAgentWorkspaceDir({} as OpenClawConfig, "main");
      expect(workspace).toBe(path.join(path.resolve(home), ".mo", "workspace"));
    });
  });

  it("uses MO_WORKSPACE_DIR for default agent workspace", () => {
    const workspaceDir = path.join(path.sep, "srv", "mo-workspace");
    withEnv(
      {
        MO_WORKSPACE_DIR: workspaceDir,
        MO_HOME: path.join(path.sep, "srv", "mo-home"),
      },
      () => {
        const workspace = resolveAgentWorkspaceDir({} as OpenClawConfig, "main");
  ...
  it("uses MO_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "mo-home");
    withEnv({ MO_HOME: home, MO_STATE_DIR: "" }, () => {
      const agentDir = resolveAgentDir({} as OpenClawConfig, "main");
      expect(agentDir).toBe(path.join(path.resolve(home), ".mo", "agents", "main", "agent"));
    });
  });
  ...
  it("resolves default agentDir from the configured default agent", () => {
    ...
    const agentDir = withEnv({ MO_STATE_DIR: stateDir }, () => resolveDefaultAgentDir(cfg));
  ...
  it("non-default agent without defaults.workspace falls back to stateDir", () => {
    ...
    const workspace = withEnv({ MO_STATE_DIR: stateDir }, () =>
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/mo-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/mo-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mo-agent-scope-"));
