...
import { completeSimple, type Model } from "mo/plugin-sdk/llm";
...
const SETUP_TOKEN_MODEL = process.env.MO_LIVE_SETUP_TOKEN_MODEL?.trim() ?? "";
...
async function resolveTokenSource(): Promise<TokenSource> {
  ...
  if (explicitToken) {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-setup-token-"));
  ...
  if (SETUP_TOKEN_RAW && SETUP_TOKEN_RAW !== "1" && SETUP_TOKEN_RAW !== "auto") {
    throw new Error(
      "MO_LIVE_SETUP_TOKEN did not look like a setup-token. Use MO_LIVE_SETUP_TOKEN_VALUE for raw tokens.",
    );
  }

  if (candidates.length === 0) {
    throw new Error(
      "No Anthropics setup-token profiles found. Set MO_LIVE_SETUP_TOKEN_VALUE or MO_LIVE_SETUP_TOKEN_PROFILE.",
    );
  }
