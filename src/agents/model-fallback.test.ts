    ...
    plugins: [
      {
        ...
        manifestPath: `/tmp/fallback-normalizer-${params.manifestHash}/mo.plugin.json`,
...
async function makeAuthTempDir(): Promise<string> {
  authTempRoot ||= path.join("/tmp", "mo-auth-suite-mock");
...
describe("runWithModelFallback", () => {
  it("uses the opt-in auth skip cache on the second turn for the same session", async () => {
    const previous = process.env.MO_FALLBACK_SKIP_TTL_MS;
    process.env.MO_FALLBACK_SKIP_TTL_MS = "60000";
    try {
      const cfg = makeCfg({
        agents: {
          defaults: {
      ...
      if (previous === undefined) {
        delete process.env.MO_FALLBACK_SKIP_TTL_MS;
      } else {
        process.env.MO_FALLBACK_SKIP_TTL_MS = previous;
      }
    }
  });
  ...
  it("warns when falling back due to model_not_found", async () => {
    const warnLogs = createWarnLogCapture("mo-model-fallback-test");
    try {
      const cfg = makeCfg();
      const run = vi
        .fn()
