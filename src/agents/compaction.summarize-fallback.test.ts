...
import type { UserMessage } from "mo/plugin-sdk/llm";
...
vi.mock("mo/plugin-sdk/agent-sessions", async () => {
  const actual = await vi.importActual<typeof import("mo/plugin-sdk/agent-sessions")>(
    "mo/plugin-sdk/agent-sessions",
  );
  return {
    ...actual,
