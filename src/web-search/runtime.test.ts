...
describe("web search runtime", () => {
  ...
  it("auto-detects a provider from a model-provider auth profile", async () => {
    const agentDir = mkdtempSync(path.join(os.tmpdir(), "mo-web-search-auth-"));
  ...
  it("auto-detects a provider from the active agent auth profile", async () => {
    const defaultAgentDir = mkdtempSync(path.join(os.tmpdir(), "mo-web-search-default-"));
    const activeAgentDir = mkdtempSync(path.join(os.tmpdir(), "mo-web-search-active-"));
  ...
  it("passes the active agentDir into selected provider tools", async () => {
    const activeAgentDir = mkdtempSync(path.join(os.tmpdir(), "mo-web-search-tool-agent-"));
