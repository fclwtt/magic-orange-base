...
describe("resolveInitialTuiAgentId", () => {
  const cfg: OpenClawConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/mo" },
        { id: "ops", workspace: "/tmp/mo/projects/ops" },
      ],
    },
  };
...
describe("resolveGatewayDisconnectState", () => {
  it("returns scope-upgrade recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.pairingHint).toContain("mo devices approve --latest");
    expect(state.pairingHint).toContain("mo devices approve <requestId>");
    expect(state.pairingHint).toContain("--token");
    // Must steer users to `devices`, not the unrelated chat-DM `pairing` command.
    expect(state.pairingHint).not.toContain("mo pairing");
  });
...
describe("TUI shutdown safety", () => {
  ...
  it("schedules a process-exit guard after standalone TUI return", () => {
    ...
    callback?.();
    expect(writeStderr).toHaveBeenCalledWith("mo tui forcing process exit after return\n");
...
describe("resolveLocalAuthCliInvocation", () => {
  ...
  it("uses the packaged wrapper when dist is available", () => {
    expect(
      ...
      command: "/usr/bin/node",
      args: ["/repo/mo.mjs", "models", "auth", "login"],
    });
  });
});
