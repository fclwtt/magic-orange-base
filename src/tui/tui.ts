...
const MO_CLI_WRAPPER_PATH = fileURLToPath(new URL("../../mo.mjs", import.meta.url));
const MO_RUN_NODE_SCRIPT_PATH = fileURLToPath(
  new URL("../../scripts/run-node.mjs", import.meta.url),
);
const MO_DIST_ENTRY_JS_PATH = fileURLToPath(new URL("../../dist/entry.js", import.meta.url));
const MO_DIST_ENTRY_MJS_PATH = fileURLToPath(
...
export function resolveLocalAuthCliInvocation(params?: {
  ...
  const hasDistEntry =
    params?.hasDistEntry ??
    (existsSync(MO_DIST_ENTRY_JS_PATH) || existsSync(MO_DIST_ENTRY_MJS_PATH));
  const hasRunNodeScript = params?.hasRunNodeScript ?? existsSync(MO_RUN_NODE_SCRIPT_PATH);
  const command = params?.execPath ?? process.execPath;
  const wrapperPath = params?.wrapperPath ?? MO_CLI_WRAPPER_PATH;
  const runNodePath = params?.runNodePath ?? MO_RUN_NODE_SCRIPT_PATH;
...
export function resolveGatewayDisconnectState(reason?: string): {
  ...
  if (/pairing required|scope upgrade/i.test(reasonLabel)) {
    return {
      ...
      pairingHint:
        "Device approval needed. Run `mo devices approve --latest` to preview the pending request, " +
        "then rerun the printed `mo devices approve <requestId>` command " +
...
export function scheduleProcessExitAfterTuiReturn(
  ...
  const timer = setTimeoutFn(() => {
    try {
      writeStderr("mo tui forcing process exit after return\n");
    } catch {
...
export async function runTui(opts: RunTuiOptions): Promise<TuiResult> {
  ...
  const previousConsoleSubsystemFilter = isLocalMode
    ...
    : null;
  if (isLocalMode) {
    setConsoleSubsystemFilter(["__mo_tui_quiet__"]);
  }
  ...
  const forceExit = () => {
    try {
      process.stderr.write("mo tui forcing exit\n");
    } catch {
    ...
    void Promise.resolve()
      ...
      .catch((err: unknown) => {
        if (!isTuiTerminalLossError(err)) {
          try {
            process.stderr.write(`mo tui shutdown failed: ${String(err)}\n`);
          } catch {
