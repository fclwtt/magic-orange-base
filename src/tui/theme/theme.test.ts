...
describe("light background detection", () => {
  ...
  it("selects light palette when MO_THEME=light", async () => {
    const mod = await importThemeWithEnv({ MO_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when MO_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ MO_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats MO_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ MO_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("MO_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      MO_THEME: "dark",
      COLORFGBG: "0;15",
    });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ MO_THEME: "light" });
    const darkMod = await importThemeWithEnv({ MO_THEME: "dark" });
