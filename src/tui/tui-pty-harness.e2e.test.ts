...
async function writeTuiPtyFixtureScript(dir: string) {
  ...
  await writeFile(
    ...
    `
      ...
      const actionLogPath = process.env.MO_TUI_PTY_LOG_PATH;
      const gatewayStatus = process.env.MO_TUI_PTY_GATEWAY_STATUS ?? "fixture gateway ok";
      ...
      let fastMode = process.env.MO_TUI_PTY_FAST_MODE === "true";
...
async function startTuiFixture(opts: { env?: NodeJS.ProcessEnv } = {}) {
  const tempDir = await mkdtemp(path.join(tmpdir(), "mo-tui-pty-"));
