...
/** Resolve raw home with MO_HOME tilde expansion. */
function resolveRawHomeDir(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = os.homedir,
): string | undefined {
  const explicitHome = normalize(env.MO_HOME);
  if (explicitHome) {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  return explicitHome ? { home, prefix: "$MO_HOME" } : { home, prefix: "~" };
}

/** Replace the effective home path with "~" or "$MO_HOME" for terminal display. */
