...
import type {
  ...
  RequestFrame,
  ResponseFrame,
} from "@mo/gateway-protocol";
import {
  GATEWAY_CLIENT_MODES,
  GATEWAY_CLIENT_NAMES,
  type GatewayClientMode,
  type GatewayClientName,
} from "@mo/gateway-protocol/client-info";
import {
...
} from "@mo/gateway-protocol/connect-error-details";
import { resolveGatewayStartupRetryAfterMs } from "@mo/gateway-protocol/startup-unavailable";
import { MIN_CLIENT_PROTOCOL_VERSION, PROTOCOL_VERSION } from "@mo/gateway-protocol/version";
...
export class GatewayClient {
  ...
  start() {
    ...
    const allowPrivateWs =
      (this.opts.env ?? process.env).MO_ALLOW_INSECURE_PRIVATE_WS === "1";
    ...
    if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
      ...
      const error = new Error(
        `SECURITY ERROR: Cannot connect to "${displayHost}" over plaintext ws://. ` +
          ...
          (allowPrivateWs
            ? ""
            : "Break-glass (trusted private networks only): set MO_ALLOW_INSECURE_PRIVATE_WS=1. ") +
          "Run `mo doctor --fix` for guidance.",
      );
