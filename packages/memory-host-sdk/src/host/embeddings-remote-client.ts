...
import { requireApiKey, resolveApiKeyForProvider } from "./mo-runtime-auth.js";
...
function resolveOpenClawAttributionHeaders(): Record<string, string> {
  const version = typeof process !== "undefined" ? process.env.MO_VERSION?.trim() : undefined;
  return {
    originator: "mo",
    ...(version ? { version } : {}),
    "User-Agent": version ? `mo/${version}` : "mo",
  };
}
