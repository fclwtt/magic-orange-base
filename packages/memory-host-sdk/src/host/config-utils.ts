...
export { splitShellArgs } from "./mo-runtime-io.js";
...
/** Resolve MO_HOME or the OS home, falling back to cwd for hermetic tests. */
function resolveRequiredHomeDir(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = os.homedir,
): string {
  const explicitHome = normalizeHomeValue(env.MO_HOME);
...
export function resolveStateDir(
  ...
  if (env.MO_TEST_FAST === "1" || fs.existsSync(nextDir)) {
    return nextDir;
  }
  // Existing legacy state remains authoritative until an explicit migration creates .mo.
...
/** Resolve the default agent workspace, partitioned by MO_PROFILE when set. */
function resolveDefaultAgentWorkspaceDir(env: NodeJS.ProcessEnv = process.env): string {
  const home = resolveRequiredHomeDir(env, os.homedir);
  const profile = env.MO_PROFILE?.trim();
  if (profile && normalizeLowercaseStringOrEmpty(profile) !== "default") {
    return path.join(home, ".mo", `workspace-${profile}`);
  }
  return path.join(home, ".mo", "workspace");
}
