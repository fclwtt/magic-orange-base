...
function readOpenClawBlock(packageJson: unknown) {
  ...
  const mo = isRecord(root?.mo) ? root.mo : undefined;
  const compat = isRecord(mo?.compat) ? mo.compat : undefined;
  const build = isRecord(mo?.build) ? mo.build : undefined;
  const install = isRecord(mo?.install) ? mo.install : undefined;
  return { root, mo, compat, build, install };
}
...
export function normalizeExternalPluginCompatibility(
  ...
  const builtWithOpenClawVersion = normalizeOptionalString(build?.moVersion) ?? version;
...
export function listMissingExternalCodePluginFieldPaths(packageJson: unknown): string[] {
  ...
  if (!normalizeOptionalString(compat?.pluginApi)) {
    missing.push("mo.compat.pluginApi");
  }
  if (!normalizeOptionalString(build?.moVersion)) {
    missing.push("mo.build.moVersion");
  }
  return missing;
}
