...
import { MAX_TIMER_TIMEOUT_MS } from "mo/plugin-sdk/number-runtime";
import type { ReplyPayload } from "mo/plugin-sdk/reply-payload";
import {
...
} from "mo/plugin-sdk/speech-core";
...
vi.mock("mo/plugin-sdk/media-runtime", () => ({
  transcodeAudioBuffer: transcodeAudioBufferMock,
}));

vi.mock("mo/plugin-sdk/channel-targets", () => ({
...
describe("speech-core native voice-note routing", () => {
  ...
  it("tells generic TTS guidance to defer to MEMORY voice-delivery instructions", () => {
    const hint = buildTtsSystemPromptHint(createTtsConfig("mo-speech-core-tts-hint-test"));
  ...
  it("marks Discord auto TTS replies as native voice messages", async () => {
    await expectTtsPayloadResult({
      channel: "discord",
      prefsName: "mo-speech-core-tts-test",
  ...
  it("keeps compatible audio-file synthesis deliverable as a voice memo", async () => {
    await expectTtsPayloadResult({
      ...
      prefsName: "mo-speech-core-tts-voice-memo-mp3-test",
  ...
  it("does not mark unsupported audio-file output as a voice memo", async () => {
    await expectTtsPayloadResult({
      channel: "voice-memo-chat",
      prefsName: "mo-speech-core-tts-voice-memo-ogg-test",
  ...
  it("pre-transcodes synthesized mp3 to opus-in-CAF when the host can satisfy preferAudioFileFormat", async () => {
    ...
    await expectTtsPayloadResult({
      channel: "voice-memo-chat",
      prefsName: "mo-speech-core-tts-voice-memo-caf-transcode-test",
  ...
  it("falls back to the original mp3 buffer when the host transcoder fails", async () => {
    ...
    await expectTtsPayloadResult({
      channel: "voice-memo-chat",
      prefsName: "mo-speech-core-tts-voice-memo-caf-fallback-test",
    ...
    const result = await synthesizeSpeech({
      ...
      cfg: {
        ...
        messages: {
          tts: {
            enabled: true,
            prefsPath: "/tmp/mo-speech-core-voice-model-default-test.json",
          },
        },
      } as OpenClawConfig,
      disableFallback: true,
      ...
      cfg: {
        ...
        messages: {
          tts: {
            enabled: true,
            provider: "openrouter",
            prefsPath: "/tmp/mo-speech-core-explicit-model-alias-test.json",
            providers: {
              openrouter: {
    ...
    const result = await synthesizeSpeech({
      ...
      cfg: {
        ...
        messages: {
          tts: {
            enabled: true,
            prefsPath: "/tmp/mo-speech-core-voice-model-fallback-test.json",
          },
        },
      } as OpenClawConfig,
    });
    ...
    const result = await synthesizeSpeech({
      ...
      cfg: {
        ...
        messages: {
          tts: {
            enabled: true,
            prefsPath: "/tmp/mo-speech-core-same-provider-voice-model-fallback-test.json",
          },
        },
      } as OpenClawConfig,
    });
      ...
      cfg: {
        ...
        messages: {
          tts: {
            enabled: true,
            provider: "openai",
            prefsPath: "/tmp/mo-speech-core-realtime-voice-model-ignored-test.json",
          },
        },
      } as OpenClawConfig,
      disableFallback: true,
    ...
    const result = await synthesizeSpeech({
      ...
      cfg: {
        ...
        messages: {
          tts: {
            enabled: true,
            prefsPath: "/tmp/mo-speech-core-supported-voice-model-provider-test.json",
          },
        },
      } as OpenClawConfig,
    });
  ...
  it.each(["feishu", "whatsapp"] as const)(
    ...
    async (channel) => {
      ...
      await expectTtsPayloadResult({
        channel,
        prefsName: `mo-speech-core-tts-${channel}-mp3-test`,
  ...
  it("keeps non-native voice-note channels as regular audio files", async () => {
    await expectTtsPayloadResult({
      channel: "slack",
      prefsName: "mo-speech-core-tts-slack-test",
  ...
  it("synthesizes explicitly tagged short hidden TTS text", async () => {
    const cfg = createTtsConfig("mo-speech-core-short-hidden-tts-test");
    let mediaDir: string | undefined;
    try {
      const result = await maybeApplyTtsToPayload({
        payload: {
  ...
  it("skips block delivery kind in final mode (accumulated final tail synthesizes instead)", async () => {
    synthesizeMock.mockClear();
    const cfg = createTtsConfig("mo-speech-core-block-kind-tts-test");
    const result = await maybeApplyTtsToPayload({
  ...
  it("skips tool delivery kind in final mode", async () => {
    synthesizeMock.mockClear();
    const cfg = createTtsConfig("mo-speech-core-tool-kind-tts-test");
    const result = await maybeApplyTtsToPayload({
  ...
  it("keeps skipping untagged short TTS text", async () => {
    const cfg = createTtsConfig("mo-speech-core-short-plain-tts-test");
    const result = await maybeApplyTtsToPayload({
      payload: {
        text: "hello",
        audioAsVoice: true,
  ...
  it("skips auto TTS for legacy final media directives", async () => {
    synthesizeMock.mockClear();
    const cfg = createTtsConfig("mo-speech-core-media-directive-tts-test");
    const result = await maybeApplyTtsToPayload({
  ...
  it("keeps skipping explicit tagged TTS text that strips to empty markdown", async () => {
    const cfg = createTtsConfig("mo-speech-core-empty-hidden-tts-test");
    const result = await maybeApplyTtsToPayload({
      payload: {
  ...
  it("selects persona preferred provider before config fallback", () => {
    ...
    const prefsPath = "/tmp/mo-speech-core-persona-provider.json";
    ...
    const cfg: OpenClawConfig = {
      messages: {
        tts: {
          enabled: true,
          provider: "mock",
          prefsPath: "/tmp/mo-speech-core-persona-merge.json",
          providers: {
            mock: {
              model: "base-model",
              voice: "base-voice",
