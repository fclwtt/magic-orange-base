...
function normalizeWorkspaceDependencies(
  ...
  for (const [name, spec] of Object.entries(dependencies)) {
    normalized[name] =
      name.startsWith("@mo/") && spec === "workspace:*" ? "0.0.0-private" : spec;
  }
  return normalized;
}
...
describe("OpenClaw SDK package e2e", () => {
  ...
  it("packs and imports from an external temp consumer", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-sdk-consumer-"));
    ...
    const sdkTarball =
      packedPackages.find((pkg) => pkg.manifest.name === "@mo/sdk")?.tarball ?? "";
    ...
    await fs.writeFile(path.join(tempDir, ".npmrc"), `@mo:registry=${registry.registryUrl}`);
    try {
    ...
    const importScript = `
      import { GatewayClientTransport, OpenClaw, normalizeGatewayEvent } from "@mo/sdk";
