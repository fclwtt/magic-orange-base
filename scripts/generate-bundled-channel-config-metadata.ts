...
function resolvePackageChannelMeta(source: BundledPluginSource) {
  const moMeta =
    ...
    "mo" in source.packageJson
      ? (source.packageJson.mo as Record<string, unknown> | undefined)
      : undefined;
  const channelMeta =
    moMeta &&
    typeof moMeta.channel === "object" &&
    moMeta.channel &&
    !Array.isArray(moMeta.channel)
      ? (moMeta.channel as Record<string, unknown>)
      : undefined;
  return channelMeta;
}
