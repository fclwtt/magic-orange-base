...
function resolveOpenClawLaunchArgs(repoRoot, sourceEntryExists = existsSync) {
  ...
  return [path.join(repoRoot, "mo.mjs")];
}
    ...
    child = spawnImpl(
      ...
      ],
      {
        ...
        env: {
          ...
          MO_STATE_DIR: path.join(tempRoot, "state"),
...
async function main() {
  ...
  const repoRoot = path.resolve(args.repoRoot ?? process.env.MO_REPO_ROOT ?? process.cwd());
  ...
  try {
    ...
    const connect = await client.request(
      "connect",
      {
        ...
        client: {
          id: "gateway-client",
          displayName: "mo-rtt rpc probe",
          version: "rtt",
          platform: process.platform,
          mode: "backend",
          instanceId: `mo-rtt-rpc-${randomUUID()}`,
        },
        locale: "en-US",
        userAgent: "mo-rtt-rpc",
        role: "operator",
