...
const DISABLE_POSTINSTALL_ENV = "MO_DISABLE_BUNDLED_PLUGIN_POSTINSTALL";
const DISABLE_PLUGIN_REGISTRY_MIGRATION_ENV = "MO_DISABLE_PLUGIN_REGISTRY_MIGRATION";
...
function resolvePostinstallOpenClawHomeDir(env, getHomedir = homedir) {
  ...
  const override = env?.MO_HOME?.trim();
...
function isLegacyInstalledPluginDependencyDirName(name) {
  return name === "node_modules" || /^\.mo-install-stage(?:-[^/]+)?$/iu.test(name);
}
...
export function collectLegacyPluginRuntimeDepsStateRoots(params = {}) {
  ...
  const configPath = env?.MO_CONFIG_PATH?.trim();
  if (configPath) {
    addStateRoot(dirname(resolvePostinstallUserPath(configPath, openClawHome)));
  }
  addStateRoot(join(openClawHome, ".mo"));
  ...
  const createTempPath =
    ...
    ((unsafeTargetPath) =>
      join(
        ...
        `.${basename(unsafeTargetPath)}.mo-hotfix-${randomUUID()}`,
      ));
  const writeFile =
