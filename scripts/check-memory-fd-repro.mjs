...
const SKIP_GATEWAY_ENV = {
  ...
  MO_NO_RESPAWN: "1",
  MO_SKIP_ACPX_RUNTIME: "1",
  MO_SKIP_ACPX_RUNTIME_PROBE: "1",
...
Options:
  ...
  --keep                         Keep the synthetic MO_HOME and workspace after the run.
...
export function parseArgs(argv) {
  ...
  const options = {
    fileCount: undefined,
    mode: process.env.MO_MEMORY_FD_REPRO_MODE || "fixed",
    ...
    keep: process.env.MO_MEMORY_FD_REPRO_KEEP === "1",
    allowNonDarwin: process.env.MO_MEMORY_FD_REPRO_ALLOW_NON_DARWIN === "1",
  };
  ...
  options.fileCount ??= readPositiveNumberEnv("MO_MEMORY_FD_REPRO_FILES", DEFAULT_FILE_COUNT);
  options.maxWorkspaceRegFds ??= readNumberEnv(
    "MO_MEMORY_FD_REPRO_MAX_WORKSPACE_REG_FDS",
    DEFAULT_MAX_WORKSPACE_REG_FDS,
  );
  options.invokeTimeoutMs ??= readTimerTimeoutNumberEnv("MO_MEMORY_FD_REPRO_TIMEOUT_MS", 30_000);
  options.sampleDelayMs ??= readTimerTimeoutNumberEnv(
    "MO_MEMORY_FD_REPRO_SAMPLE_DELAY_MS",
    1_000,
    0,
  );
  options.settleDelayMs ??= readTimerTimeoutNumberEnv(
    "MO_MEMORY_FD_REPRO_SETTLE_DELAY_MS",
    5_000,
    0,
  );
...
export function writeConfig({ homeDir, workspaceDir, port, token }) {
  const configDir = path.join(homeDir, ".mo");
...
async function main() {
  ...
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-memory-fd-repro-"));
  ...
  const env = {
    ...
    ...SKIP_GATEWAY_ENV,
    HOME: homeDir,
    MO_STATE_DIR: path.join(homeDir, ".mo"),
