...
function shouldCopyBundledPluginMetadata(id, env, buildablePluginDirs) {
  ...
  return env.MO_BUILD_PRIVATE_QA === "1";
}
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "mo.plugin.json");
    ...
    if (packageJson.mo && "extensions" in packageJson.mo) {
      packageJson.mo = {
        ...packageJson.mo,
        extensions: rewritePackageExtensions(packageJson.mo.extensions),
        ...(typeof packageJson.mo.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.mo.setupEntry) }
          : {}),
      };
    }
