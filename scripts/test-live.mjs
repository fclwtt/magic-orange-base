...
export function testLiveUsage() {
  return [
    ...
    "",
    "Runs live Vitest suites with MO_LIVE_TEST=1.",
    "",
    "Options:",
...
export function buildTestLiveEnv(args, baseEnv = process.env) {
  return {
    ...
    pnpm_config_verify_deps_before_run: baseEnv.pnpm_config_verify_deps_before_run || "false",
    MO_LIVE_TEST: baseEnv.MO_LIVE_TEST || "1",
    MO_LIVE_TEST_QUIET: args.quietOverride ?? baseEnv.MO_LIVE_TEST_QUIET ?? "1",
    ...(args.forceCodexHarness ? { MO_LIVE_CODEX_HARNESS: "1" } : {}),
  };
}
...
export function resolveTestLiveHeartbeatMs(baseEnv = process.env) {
  const value = baseEnv.MO_LIVE_WRAPPER_HEARTBEAT_MS;
  if (value === undefined || value === "") {
    return 20_000;
  }
  const text = value.trim();
  if (!/^\d+$/u.test(text)) {
    throw new Error(`invalid MO_LIVE_WRAPPER_HEARTBEAT_MS: ${text}`);
  }
