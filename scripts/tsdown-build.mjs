...
const logLevel = process.env.MO_BUILD_VERBOSE ? "info" : "warn";
const INEFFECTIVE_DYNAMIC_IMPORT_MARKER = "[INEFFECTIVE_DYNAMIC_IMPORT]";
const UNRESOLVED_IMPORT_RE = /\[UNRESOLVED_IMPORT\]/;
const ANSI_ESCAPE_RE = new RegExp(String.raw`\u001B\[[0-9;]*m`, "g");
const DEPENDENCY_PATH_MARKERS = ["node_modules/", "mo-pnpm-node-modules/"];
...
const TSDOWN_MAX_OLD_SPACE_MB_ENV = "MO_TSDOWN_MAX_OLD_SPACE_MB";
...
const PRESERVE_CLI_STARTUP_METADATA_ENV = "MO_PRESERVE_CLI_STARTUP_METADATA";
...
export async function runTsdownBuildInvocation(invocation, params = {}) {
  ...
  const timeoutMs = parsePositiveIntegerEnv(
    env.MO_TSDOWN_TIMEOUT_MS,
    "MO_TSDOWN_TIMEOUT_MS",
  );
  const heartbeatMs =
    parseNonNegativeIntegerEnv(env.MO_TSDOWN_HEARTBEAT_MS, "MO_TSDOWN_HEARTBEAT_MS") ??
