...
const EXPECTED_REPOSITORY_URL = "https://github.com/mo/mo";
const OPTIONAL_LOCAL_EMBEDDING_RUNTIME_PACKAGE = "node-llama-cpp";
const FS_SAFE_PACKAGE = "@mo/fs-safe";
...
const skipPackValidationEnv = "MO_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function resolveNpmReleaseCheckCommandTimeoutMs(
  ...
  return positiveEnvInt(
    "MO_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS",
    env,
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "mo") {
    errors.push(`package.json name must be "mo"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.mo !== "mo.mjs") {
    errors.push(
      `package.json bin.mo must be "mo.mjs"; found "${pkg.bin?.mo ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`mo-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `mo-npm-release-check: validated ${channel} release ${pkg.version} (monthly patch version${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
