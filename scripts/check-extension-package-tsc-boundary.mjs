...
export function resolveCompileConcurrency(
  ...
  const raw = env.MO_EXTENSION_BOUNDARY_CONCURRENCY?.trim();
  if (raw) {
    return parsePositiveInt(raw, "MO_EXTENSION_BOUNDARY_CONCURRENCY");
  }
...
async function runCompileCheck(extensionIds) {
  ...
  const verboseFreshLogs = process.env.MO_EXTENSION_BOUNDARY_VERBOSE_FRESH === "1";
