...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.mo) ? packageJson.mo : null;
...
function getCatalogChannelId(entry) {
  return trimString(entry?.mo?.channel?.id) || trimString(entry?.name);
}
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.mo?.channel?.id) || left.name;
    const rightId = trimString(right.mo?.channel?.id) || right.name;
