...
export function resolveWindowsExtensionChunkSize(env = process.env) {
  return resolvePositiveEnvIntWithFallback(
    env,
    "MO_OXLINT_WINDOWS_EXTENSION_CHUNK_SIZE",
...
export function shouldRunOxlintShardsSerial({
  ...
  const explicitMode = env.MO_OXLINT_SHARDS_SERIAL?.trim();
  if (explicitMode === "1") {
    return true;
  }
  if (platform === "win32") {
  ...
  const localCheckMode = env.MO_LOCAL_CHECK_MODE?.trim().toLowerCase();
...
function isRemoteChangedGateEnv(env) {
  return (
    env.MO_CHECK_CHANGED_REMOTE_CHILD === "1" || env.MO_CHANGED_LANES_RAW_SYNC === "1"
  );
}
...
export function resolveOxlintShardConcurrency({
  ...
  const explicitConcurrency = resolvePositiveEnvInt(env, "MO_OXLINT_SHARD_CONCURRENCY");
...
export async function runShard({ env, extraArgs, runner, shard }) {
  ...
  const child = spawn(process.execPath, [runner, ...shard.args, ...extraArgs], {
    ...
    env: {
      ...env,
      MO_OXLINT_SKIP_LOCK: "1",
      MO_OXLINT_SKIP_PREPARE: "1",
    },
  });
