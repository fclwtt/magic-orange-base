...
export function isLocalCheckEnabled(env) {
  const raw = env.MO_LOCAL_CHECK?.trim().toLowerCase();
...
export function applyLocalTsgoPolicy(args, env, hostResources) {
  ...
  if (defaultProjectRun) {
    ...
    insertBeforeSeparator(
      nextArgs,
      "--tsBuildInfoFile",
      nextEnv.MO_TSGO_BUILD_INFO_FILE ?? DEFAULT_LOCAL_TSGO_BUILD_INFO_FILE,
    );
  }
  ...
  if (nextEnv.MO_TSGO_PPROF_DIR && !hasFlag(nextArgs, "--pprofDir")) {
    insertBeforeSeparator(nextArgs, "--pprofDir", nextEnv.MO_TSGO_PPROF_DIR);
  }
...
export function acquireLocalHeavyCheckLockSync(params) {
  ...
  const timeoutMs = readPositiveInt(
    env.MO_HEAVY_CHECK_LOCK_TIMEOUT_MS,
    DEFAULT_LOCK_TIMEOUT_MS,
    "MO_HEAVY_CHECK_LOCK_TIMEOUT_MS",
  );
  const pollMs = readPositiveInt(
    env.MO_HEAVY_CHECK_LOCK_POLL_MS,
    DEFAULT_LOCK_POLL_MS,
    "MO_HEAVY_CHECK_LOCK_POLL_MS",
  );
  const progressMs = readPositiveInt(
    env.MO_HEAVY_CHECK_LOCK_PROGRESS_MS,
    DEFAULT_LOCK_PROGRESS_MS,
    "MO_HEAVY_CHECK_LOCK_PROGRESS_MS",
  );
  const staleLockMs = readPositiveInt(
    env.MO_HEAVY_CHECK_STALE_LOCK_MS,
    DEFAULT_STALE_LOCK_MS,
    "MO_HEAVY_CHECK_STALE_LOCK_MS",
  );
...
function resolveHeavyCheckLocksDir(cwd, env) {
  const lockScope = env.MO_HEAVY_CHECK_LOCK_SCOPE?.trim().toLowerCase();
  if (lockScope === "worktree") {
    return path.join(resolveGitWorktreeRoot(cwd), ".artifacts", "mo-local-checks");
  }

  return path.join(resolveGitCommonDir(cwd), "mo-local-checks");
}
...
function readLocalCheckMode(env, defaultMode) {
  const raw = env.MO_LOCAL_CHECK_MODE?.trim().toLowerCase();
