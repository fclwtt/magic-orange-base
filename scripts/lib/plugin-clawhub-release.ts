...
const MO_PLUGIN_CLAWHUB_REPOSITORY = "mo/mo";
const MO_PLUGIN_CLAWHUB_WORKFLOW_FILENAME = "plugin-clawhub-release.yml";
...
const CLAWHUB_SHARED_RELEASE_INPUT_PATHS = [
  ...
  "scripts/mo-npm-release-check.ts",
...
export function collectClawHubPublishablePluginPackages(
  ...
  for (const candidate of collectExtensionPackageJsonCandidates(rootDir)) {
    ...
    if (packageJson.mo?.release?.publishToClawHub !== true) {
      continue;
    }
...
export function collectClawHubVersionGateErrors(params: {
  ...
  for (const plugin of changedPlugins) {
    ...
    if (baseManifest?.mo?.release?.publishToClawHub !== true) {
      continue;
    }
    const baseVersion =
...
function isOpenClawPluginTrustedPublisher(value: unknown): boolean {
  ...
  return (
    trustedPublisher.repository === MO_PLUGIN_CLAWHUB_REPOSITORY &&
    trustedPublisher.workflowFilename === MO_PLUGIN_CLAWHUB_WORKFLOW_FILENAME &&
