...
function usage() {
  return `Usage:
  node scripts/lib/mo-test-state.mjs -- create [--label <name>] [--scenario <name>] [--env-file <path>] [--json]
  node scripts/lib/mo-test-state.mjs shell [--label <name>] [--scenario <name>]
  node scripts/lib/mo-test-state.mjs shell-function
...
function scenarioConfig(scenario, options = {}) {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      skills: {
        allowBundled: ["memory", "mo-testing"],
        limits: {
          maxSkillsInPrompt: 8,
  ...
  if (scenario === "gateway-loopback") {
    return {
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.token || "mo-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function scenarioEnv(scenario) {
  if (scenario === "external-service") {
    return {
      MO_SERVICE_REPAIR_POLICY: "external",
    };
  }
  return {};
}
...
function renderAuthProfileSecretKeyExport() {
  return [
    'MO_AUTH_PROFILE_SECRET_KEY_FILE="$MO_TEST_STATE_HOME/.mo-test-auth-profile-secret-key"',
    'if [ -s "$MO_AUTH_PROFILE_SECRET_KEY_FILE" ]; then',
    '  MO_AUTH_PROFILE_SECRET_KEY="$(cat "$MO_AUTH_PROFILE_SECRET_KEY_FILE")"',
    "else",
    '  MO_AUTH_PROFILE_SECRET_KEY="$(od -An -N 32 -tx1 /dev/urandom | tr -d " \\n")"',
    '  ( umask 077; printf "%s\\n" "$MO_AUTH_PROFILE_SECRET_KEY" > "$MO_AUTH_PROFILE_SECRET_KEY_FILE" )',
    "fi",
    'if [ -z "$MO_AUTH_PROFILE_SECRET_KEY" ]; then',
    '  echo "failed to generate MO_AUTH_PROFILE_SECRET_KEY" >&2',
    "  return 1 2>/dev/null || exit 1",
    "fi",
    "export MO_AUTH_PROFILE_SECRET_KEY",
  ];
}
...
function renderConfigWrite(configPathExpression, config) {
  ...
  return [
    `cat > ${configPathExpression} <<'MO_TEST_STATE_JSON'`,
    json,
    "MO_TEST_STATE_JSON",
  ].join("\n");
}
...
function buildCreatePlan(options = {}) {
  ...
  const env = {
    ...
    MO_AUTH_PROFILE_SECRET_KEY: generateAuthProfileSecretKey(),
...
export async function createState(options = {}) {
  const label = normalizeLabel(options.label);
  const root = await fs.mkdtemp(path.join(os.tmpdir(), `mo-${label}-`));
...
export function renderShellSnippet(options = {}) {
  ...
  const homeTemplate = `mo-${label}-${scenario}-home.XXXXXX`;
  const lines = [
    'MO_TEST_STATE_TMP_ROOT="${MO_TEST_STATE_TMPDIR:-${TMPDIR:-/tmp}}"',
    'MO_TEST_STATE_TMP_ROOT="${MO_TEST_STATE_TMP_ROOT%/}"',
    '[ -n "$MO_TEST_STATE_TMP_ROOT" ] || MO_TEST_STATE_TMP_ROOT="/tmp"',
    "export MO_TEST_STATE_TMP_ROOT",
    'mkdir -p "$MO_TEST_STATE_TMP_ROOT"',
    `MO_TEST_STATE_HOME="$(mktemp -d "$MO_TEST_STATE_TMP_ROOT/${homeTemplate}")"`,
    'export HOME="$MO_TEST_STATE_HOME"',
    'export USERPROFILE="$MO_TEST_STATE_HOME"',
    'export MO_HOME="$MO_TEST_STATE_HOME"',
    'export MO_STATE_DIR="$MO_TEST_STATE_HOME/.mo"',
    'export MO_CONFIG_PATH="$MO_STATE_DIR/mo.json"',
    ...renderAuthProfileSecretKeyExport(),
    'export MO_TEST_WORKSPACE_DIR="$MO_TEST_STATE_HOME/workspace"',
    'mkdir -p "$MO_STATE_DIR" "$MO_TEST_WORKSPACE_DIR"',
  ];
  for (const [key, value] of Object.entries(env)) {
    lines.push(`export ${key}=${shellQuote(value)}`);
  }
  const configWrite = renderConfigWrite('"$MO_CONFIG_PATH"', config);
  if (configWrite) {
...
export function renderShellFunction() {
  ...
  case "$raw_label" in
    /*)
      MO_TEST_STATE_HOME="$raw_label"
      mkdir -p "$MO_TEST_STATE_HOME"
      ;;
    *)
      label="$(printf "%s" "$label" | tr -cs "A-Za-z0-9_.-" "-" | sed -e "s/^-*//" -e "s/-*$//")"
      [ -n "$label" ] || label="state"
      local tmp_root="\${MO_TEST_STATE_TMPDIR:-\${TMPDIR:-/tmp}}"
      tmp_root="\${tmp_root%/}"
      [ -n "$tmp_root" ] || tmp_root="/tmp"
      mkdir -p "$tmp_root"
      MO_TEST_STATE_HOME="$(mktemp -d "$tmp_root/mo-$label-$scenario-home.XXXXXX")"
      ;;
  esac
  export HOME="$MO_TEST_STATE_HOME"
  export USERPROFILE="$MO_TEST_STATE_HOME"
  ...
  export MO_TEST_WORKSPACE_DIR="$MO_TEST_STATE_HOME/workspace"
  unset MO_AGENT_DIR
  unset MO_SERVICE_REPAIR_POLICY
  mkdir -p "$MO_STATE_DIR" "$MO_TEST_WORKSPACE_DIR"
  case "$scenario" in
    minimal)
      cat > "$MO_CONFIG_PATH" <<'MO_TEST_STATE_JSON'
{}
MO_TEST_STATE_JSON
      ;;
    update-stable)
...
MO_TEST_STATE_JSON
      ;;
    external-service)
      export MO_SERVICE_REPAIR_POLICY="external"
      cat > "$MO_CONFIG_PATH" <<'MO_TEST_STATE_JSON'
{}
MO_TEST_STATE_JSON
      ;;
