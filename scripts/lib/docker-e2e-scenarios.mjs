...
const LIVE_DOCKER_DEFAULT_HARNESS_DIR =
  ...
  process.env.MO_DOCKER_E2E_REPO_ROOT
    ? ".release-harness"
    : ".";
...
const RELEASE_OPENWEBUI_COMMAND =
  "MO_OPENWEBUI_MODEL=openai/gpt-5.4-mini MO_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS=300 MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:openwebui";
export const BUNDLED_PLUGIN_INSTALL_UNINSTALL_SHARDS = 24;
const upgradeSurvivorCommand = "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:upgrade-survivor";
const rootManagedVpsUpgradeCommand =
  "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:root-managed-vps-upgrade";
const updateRestartAuthCommand =
  "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-restart-auth";
const CODEX_HARNESS_API_KEY_ENV = "MO_LIVE_CODEX_HARNESS_AUTH=api-key";
...
function liveDockerScriptCommand(script, envPrefix = "", options = {}) {
  ...
  const skipBuild = options.skipBuild === false ? "" : "MO_SKIP_DOCKER_BUILD=1 ";
  return `${prefix}${skipBuild}bash -c 'harness="\${MO_DOCKER_E2E_TRUSTED_HARNESS_DIR:-${LIVE_DOCKER_DEFAULT_HARNESS_DIR}}"; MO_LIVE_DOCKER_REPO_ROOT="\${MO_DOCKER_E2E_REPO_ROOT:-$PWD}" bash "$harness/scripts/${script}"'`;
}
...
function createPackageUpdateMaintenanceLanes() {
  return [
    npmLane("doctor-switch", "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:doctor-switch", {
      stateScenario: "empty",
      weight: 3,
    }),
    npmLane(
      "update-channel-switch",
      "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-channel-switch",
      {
    ...
    npmLane("skill-install", "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:skill-install", {
    ...
    npmLane(
      ...
      "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:published-upgrade-survivor",
      {
        stateScenario: "upgrade-survivor",
...
const bundledPluginInstallUninstallLanes = Array.from(
  ...
  (_, index) =>
    lane(
      ...
      `MO_BUNDLED_PLUGIN_SWEEP_TOTAL=${BUNDLED_PLUGIN_INSTALL_UNINSTALL_SHARDS} MO_BUNDLED_PLUGIN_SWEEP_INDEX=${index} MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:bundled-plugin-install-uninstall`,
      {
        estimateSeconds: 120,
        resources: ["npm"],
        stateScenario: "empty",
...
function livePluginToolLane() {
  return liveLane(
    "live-plugin-tool",
    "MO_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS=300 MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:live-plugin-tool",
    {
...
function liveOpenAiChatToolsLane() {
  return liveLane(
    "openai-chat-tools",
    "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:openai-chat-tools",
    {
...
function liveMcpCodeModeGatewayLane() {
  return liveLane(
    ...
    "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:live-mcp-code-mode-gateway",
    {
...
function kitchenSinkRpcLane() {
  return serviceLane(
    "kitchen-sink-rpc",
    "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:kitchen-sink-rpc",
    {
      resources: ["npm"],
      stateScenario: "empty",
      timeoutMs: 25 * 60 * 1000,
...
export const mainLanes = [
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      ...
      "MO_IMAGE=mo:local-live-gateway MO_DOCKER_BUILD_EXTENSIONS=matrix MO_LIVE_GATEWAY_PROVIDERS=claude-cli,google-gemini-cli",
      { skipBuild: false },
    ),
    {
      providers: ["claude-cli", "google-gemini-cli"],
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      ...
      "MO_LIVE_CLI_BACKEND_MODEL=claude-cli/claude-sonnet-4-6",
    ),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      "test-live-cli-backend-docker.sh",
      "MO_LIVE_CLI_BACKEND_ADVISORY=1 MO_LIVE_CLI_BACKEND_ALLOW_PROVIDER_SKIP=1 MO_LIVE_CLI_BACKEND_MODEL=google-gemini-cli/gemini-3-flash-preview",
    ),
    {
  ...
  liveLane("openwebui", "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:openwebui", {
    e2eImageKind: "functional",
    needsLiveImage: false,
    provider: "openai",
    resources: ["service"],
  ...
  serviceLane("onboard", "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:onboard", {
    stateScenario: "empty",
    weight: 2,
  }),
  npmLane("codex-on-demand", "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:codex-on-demand", {
    resources: ["service"],
    stateScenario: "empty",
    weight: 3,
  }),
  serviceLane(
    "codex-media-path",
    "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:codex-media-path",
    {
      resources: ["npm"],
      stateScenario: "empty",
      weight: 3,
  ...
  npmLane(
    ...
    "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-onboard-channel-agent",
    { resources: ["service"], stateScenario: "empty", weight: 3 },
  ),
  npmLane(
    "npm-onboard-discord-channel-agent",
    "MO_NPM_ONBOARD_CHANNEL=discord MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-onboard-channel-agent",
    { resources: ["service"], stateScenario: "empty", weight: 3 },
  ),
  npmLane(
    "npm-onboard-slack-channel-agent",
    "MO_NPM_ONBOARD_CHANNEL=slack MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-onboard-channel-agent",
    { resources: ["service"], stateScenario: "empty", weight: 3 },
  ),
  npmLane(
    "release-user-journey",
    "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-user-journey",
    {
      resources: ["npm", "service"],
      stateScenario: "empty",
      timeoutMs: 20 * 60 * 1000,
  ...
  npmLane(
    ...
    "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-typed-onboarding",
    {
      resources: ["npm", "service"],
      stateScenario: "empty",
      timeoutMs: 20 * 60 * 1000,
  ...
  npmLane(
    ...
    "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-media-memory",
    {
      resources: ["npm", "service"],
      stateScenario: "empty",
      timeoutMs: 20 * 60 * 1000,
  ...
  npmLane(
    ...
    "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-upgrade-user-journey",
    {
      resources: ["npm", "service"],
      stateScenario: "empty",
      timeoutMs: 30 * 60 * 1000,
  ...
  npmLane(
    ...
    "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-plugin-marketplace",
    {
      resources: ["npm"],
      stateScenario: "empty",
      timeoutMs: 20 * 60 * 1000,
  ...
  serviceLane("gateway-network", "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:gateway-network"),
  ...
  serviceLane(
    ...
    "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:agents-delete-shared-workspace",
    { stateScenario: "empty" },
  ),
  serviceLane("mcp-channels", "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:mcp-channels", {
    resources: ["npm"],
    stateScenario: "empty",
    weight: 3,
  }),
  serviceLane(
    ...
    "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:mcp-code-mode-gateway",
    {
      resources: ["npm"],
      stateScenario: "empty",
      weight: 3,
  ...
  lane(
    ...
    "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:agent-bundle-mcp-tools",
    {
      stateScenario: "empty",
    },
  ),
  lane("crestodian-rescue", "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:crestodian-rescue", {
    stateScenario: "empty",
  }),
  lane("crestodian-planner", "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:crestodian-planner", {
    stateScenario: "empty",
  }),
  serviceLane(
    "cron-mcp-cleanup",
    "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:cron-mcp-cleanup",
    { resources: ["npm"], stateScenario: "empty", weight: 3 },
  ),
  ...createPackageUpdateMaintenanceLanes(),
  npmLane("update-migration", "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-migration", {
    stateScenario: "upgrade-survivor",
    timeoutMs: 30 * 60 * 1000,
    weight: 3,
  }),
  lane("plugins", "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugins", {
    resources: ["npm", "service"],
    stateScenario: "empty",
    weight: 6,
  }),
  lane("kitchen-sink-plugin", "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:kitchen-sink-plugin", {
    resources: ["npm"],
    stateScenario: "empty",
    weight: 3,
  }),
  ...
  lane(
    "plugins-offline",
    "MO_PLUGINS_E2E_CLAWHUB=0 MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugins",
    {
      resources: ["npm", "service"],
      stateScenario: "empty",
      weight: 6,
  ...
  npmLane("plugin-update", "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugin-update", {
    stateScenario: "empty",
  }),
  npmLane(
    "update-corrupt-plugin",
    "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-corrupt-plugin",
    {
      stateScenario: "empty",
      timeoutMs: 30 * 60 * 1000,
      weight: 3,
  ...
  npmLane(
    ...
    "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugin-lifecycle-matrix",
    {
      stateScenario: "empty",
      timeoutMs: 12 * 60 * 1000,
    },
  ),
  serviceLane("config-reload", "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:config-reload", {
    stateScenario: "empty",
  }),
  npmLane("multi-node-update", "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:multi-node-update", {
    stateScenario: "empty",
    timeoutMs: 15 * 60 * 1000,
    weight: 3,
  }),
  lane("openai-image-auth", "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:openai-image-auth", {
    stateScenario: "empty",
  }),
  lane(
    "crestodian-first-run",
    "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:crestodian-first-run",
    { stateScenario: "empty" },
  ),
  lane(
    "session-runtime-context",
    "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:session-runtime-context",
  ),
  lane(
    "plugin-binding-command-escape",
    "MO_SKIP_DOCKER_BUILD=0 pnpm test:docker:plugin-binding-command-escape",
    {
      e2eImageKind: false,
      resources: ["npm"],
      stateScenario: "empty",
  ...
  lane("commitments-safety", "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:commitments-safety", {
    stateScenario: "empty",
  }),
  liveLane("npm-telegram-live", "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-telegram-live", {
    e2eImageKind: "bare",
    provider: "openai",
...
export const tailLanes = [
  serviceLane(
    ...
    "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:openai-web-search-minimal",
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      ...
      "MO_LIVE_CODEX_HARNESS_AUTH=api-key MO_LIVE_CODEX_HARNESS_CHAT_IMAGE_PROBE=1 MO_LIVE_CODEX_HARNESS_IMAGE_PROBE=0 MO_LIVE_CODEX_HARNESS_MCP_PROBE=0 MO_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=0 MO_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=0",
    ),
    {
      cacheKey: "codex-harness",
      provider: "openai",
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      "test-live-codex-harness-docker.sh",
      `${CODEX_HARNESS_API_KEY_ENV} MO_LIVE_CODEX_BIND=1 MO_LIVE_CODEX_TEST_FILES=src/gateway/gateway-codex-bind.live.test.ts`,
    ),
    {
      cacheKey: "codex-harness",
      provider: "openai",
  ...
  liveLane(
    ...
    liveDockerScriptCommand("test-live-acp-bind-docker.sh", "MO_LIVE_ACP_BIND_AGENT=claude"),
    {
  ...
  liveLane(
    "live-acp-bind-codex",
    liveDockerScriptCommand("test-live-acp-bind-docker.sh", "MO_LIVE_ACP_BIND_AGENT=codex"),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      ...
      "MO_LIVE_ACP_BIND_AGENT=droid MO_LIVE_ACP_BIND_REQUIRE_TRANSCRIPT=1",
    ),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand("test-live-acp-bind-docker.sh", "MO_LIVE_ACP_BIND_AGENT=gemini"),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      "test-live-acp-bind-docker.sh",
      "MO_LIVE_ACP_BIND_AGENT=opencode MO_LIVE_ACP_BIND_REQUIRE_TRANSCRIPT=1",
    ),
    {
...
const releasePathPackageInstallOpenAiLanes = [
  liveLane(
    "install-e2e-openai",
    "MO_INSTALL_TAG=beta MO_E2E_MODELS=openai MO_INSTALL_E2E_IMAGE=mo-install-e2e-openai:local MO_INSTALL_E2E_AGENT_TOOL_SMOKE=0 MO_INSTALL_E2E_OPENAI_MODEL=openai/gpt-5.4-mini MO_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS=120 MO_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS=120 pnpm test:install:e2e",
    {
      e2eImageKind: "bare",
      needsLiveImage: false,
      provider: "openai",
...
const releasePathPackageInstallAnthropicLanes = [
  liveLane(
    ...
    "MO_INSTALL_TAG=beta MO_E2E_MODELS=anthropic MO_INSTALL_E2E_IMAGE=mo-install-e2e-anthropic:local pnpm test:install:e2e",
    {
      e2eImageKind: "bare",
      needsLiveImage: false,
      provider: "claude",
...
const primaryReleasePathChunks = {
  core: [
    lane("qr", "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:qr"),
    serviceLane("onboard", "MO_SKIP_DOCKER_BUILD=1 pnpm test:docker:onboard", {
      stateScenario: "empty",
      weight: 2,
    }),
...
export function releasePathChunkLanes(chunk, options = {}) {
  ...
  if (!base) {
    throw new Error(
      `MO_DOCKER_ALL_CHUNK must be one of: ${[
