...
export function isPublishablePluginPackage(packageJson) {
  return (
    packageJson.mo?.release?.publishToNpm === true ||
    packageJson.mo?.release?.publishToClawHub === true
  );
}
...
export function resolvePluginNpmRuntimePackageFiles(plan) {
  ...
  merged.add("dist/**");
  if (packageRelativePathExists(plan.packageDir, "mo.plugin.json")) {
    merged.add("mo.plugin.json");
  }
...
function resolveOpenClawPeerRange(packageJson, rootPackageJson) {
  return (
    normalizeOpenClawPeerRange(packageJson.mo?.compat?.pluginApi) ||
    normalizeOpenClawPeerRange(packageJson.peerDependencies?.mo) ||
    normalizeOpenClawPeerRange(packageJson.mo?.build?.moVersion) ||
...
export function resolvePluginNpmRuntimePackagePeerMetadata(plan) {
  const moPeerRange = resolveOpenClawPeerRange(plan.packageJson, plan.rootPackageJson);
  if (!moPeerRange) {
    throw new Error(
      `cannot infer mo peerDependency range for ${plan.pluginDir}; set mo.compat.pluginApi or package version`,
    );
  }
  ...
  const existingOpenClawMeta = getRecord(existingPeerDependenciesMeta.mo);
  return {
    peerDependencies: {
...
export function resolvePluginNpmRuntimeBuildPlan(params) {
  ...
  const plan = {
    ...
    runtimeExtensions: (Array.isArray(packageJson.mo?.extensions)
      ? packageJson.mo.extensions
      : []
    )
      .map(normalizePackageEntry)
      .filter(Boolean)
      .map(toPackageRuntimeEntry),
    runtimeSetupEntry: normalizePackageEntry(packageJson.mo?.setupEntry)
      ? toPackageRuntimeEntry(packageJson.mo.setupEntry)
      : undefined,
  };
  return {
    ...plan,
