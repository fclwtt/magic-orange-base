...
export function parseReleaseVerifyBetaArgs(argv: string[]): ReleaseVerifyBetaArgs {
  ...
  if (!version || version.startsWith("-")) {
    throw new Error(
      "Usage: pnpm release:verify-beta -- <version> [--workflow-ref REF] [--clawhub-workflow-ref REF] [--full-release-validation-run ID] [--mo-npm-run ID] [--plugin-npm-run ID] [--plugin-clawhub-run ID] [--plugin-clawhub-bootstrap-run ID] [--npm-telegram-run ID] [--skip-github-release] [--skip-clawhub]",
    );
  }
  ...
  for (let index = 0; index < values.length; index += 1) {
    ...
    switch (arg) {
      ...
      case "--full-release-validation-run":
        ...
        break;
      case "--mo-npm-run":
        parsed.workflowRuns.moNpm = next();
        break;
      case "--plugin-npm-run":
...
export async function verifyBetaRelease(
  ...
  const moNpm = await verifyNpmPackage("mo", args.version, args.distTag);
  lines.push(`mo npm OK: ${args.version} (${args.distTag})`);

  if (!args.skipPostpublish) {
    runCommandInherited("node", [
      "--import",
      "tsx",
      "scripts/mo-npm-postpublish-verify.ts",
      args.version,
    ]);
    lines.push("mo postpublish verifier OK");
  }
  ...
  if (args.workflowRuns.moNpm !== undefined) {
    workflowRuns.push(
      verifyWorkflowRun({
        id: args.workflowRuns.moNpm,
      ...
      `${JSON.stringify(
        {
          ...
          moNpmIntegrity: moNpm.integrity,
          moNpmTarball: moNpm.tarball,
