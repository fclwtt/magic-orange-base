...
export const DEFAULT_E2E_BARE_IMAGE = "mo-docker-e2e-bare:local";
export const DEFAULT_E2E_FUNCTIONAL_IMAGE = "mo-docker-e2e-functional:local";
...
export function normalizeUpgradeSurvivorBaselineSpec(raw) {
  ...
  const spec = value.startsWith("mo@") ? value : `mo@${value}`;
  if (
    !/^mo@(?:alpha|beta|latest|[0-9]{4}\.[0-9]+\.[0-9]+(?:-(?:[0-9]+|alpha\.[0-9]+|beta\.[0-9]+))?)$/u.test(
      spec,
    )
  ) {
    throw new Error(
      `invalid published upgrade survivor baseline: ${JSON.stringify(
        value,
      )}. Expected mo@latest, mo@beta, mo@alpha, or mo@YYYY.M.PATCH.`,
    );
  }
  return spec;
}
...
function parsePublishedReleaseVersion(spec) {
  const match = /^mo@([0-9]{4})\.([0-9]+)\.([0-9]+)/u.exec(String(spec ?? ""));
  if (!match) {
    return null;
  }
  return {
    ...
    return matrixBaselines.flatMap((baselineSpec) =>
      matrixScenarios
        ...
        .map((scenario) => {
          ...
          const commandPrefix = [
            `MO_UPGRADE_SURVIVOR_ARTIFACT_DIR="$PWD/.artifacts/upgrade-survivor/${name}"`,
            baselineSpec
              ? `MO_UPGRADE_SURVIVOR_BASELINE_SPEC=${shellQuote(baselineSpec)}`
              : "",
            scenario ? `MO_UPGRADE_SURVIVOR_SCENARIO=${shellQuote(scenario)}` : "",
          ]
            .filter(Boolean)
            .join(" ");
...
export function parseLiveMode(raw) {
  ...
  throw new Error(
    `MO_DOCKER_ALL_LIVE_MODE must be one of: all, skip, only. Got: ${JSON.stringify(raw)}`,
  );
}
...
export function parseProfile(raw) {
  ...
  throw new Error(
    `MO_DOCKER_ALL_PROFILE must be one of: ${DEFAULT_PROFILE}, ${RELEASE_PATH_PROFILE}. Got: ${JSON.stringify(raw)}`,
  );
}
...
export function findLaneByName(name) {
  return dedupeLanes(
    expandUpgradeSurvivorBaselineLanes(
      ...
      process.env.MO_UPGRADE_SURVIVOR_BASELINE_SPECS,
      process.env.MO_UPGRADE_SURVIVOR_SCENARIOS,
    ),
  ...
  const selectedLanes =
    options.selectedLaneNames.length > 0
      ? options.selectedLaneNames.flatMap((selectedName) => {
          ...
          selectNamedLanes(selectableLanes, [selectedName], "MO_DOCKER_ALL_LANES");
          return [];
        })
      : undefined;
