...
const PLUGIN_SDK_ENTRY_DTS_CACHE_ENV = ["MO_BUILD_PRIVATE_QA"];
...
export const BUILD_ALL_STEPS = [
  ...
  },
  {
    ...
    // from package.json, git HEAD, and MO_CONTROL_UI_BUILD_ID env, so a
...
export const BUILD_ALL_PROFILE_STEP_ENV = {
  full: {
    tsdown: {
      MO_PRESERVE_CLI_STARTUP_METADATA: "1",
    },
  },
  ciArtifacts: {
    tsdown: {
      MO_RUN_NODE_SKIP_DTS_BUILD: "1",
      MO_PRESERVE_CLI_STARTUP_METADATA: "1",
    },
  },
  gatewayWatch: {
...
export function resolveBuildAllStep(step, params = {}) {
  ...
  if (step.kind === "pnpm") {
    const nodeFallbackArgs =
      env.MO_BUILD_ALL_NO_PNPM === "1" ? PNPM_STEP_NODE_FALLBACKS.get(step.label) : undefined;
    if (nodeFallbackArgs) {
      return {
...
if (isMainModule()) {
  ...
  if (args?.help) {
    ...
    for (const step of resolveBuildAllSteps(args.profile)) {
      ...
      if (process.env.MO_BUILD_CACHE !== "0" && cacheState.fresh) {
