...
const WATCH_GATEWAY_SKIP_ENV = {
  ...
  MO_RUNTIME_POSTBUILD_STATIC_ASSETS: "0",
...
function formatCapturedWatchLog(text, truncated) {
  return truncated
    ? `[mo] log truncated to last ${WATCH_LOG_CAPTURE_MAX_CHARS} chars\n${text}`
    : text;
}
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const isolatedStateDir = path.join(isolatedHomeDir, ".mo");
  const isolatedConfigPath = path.join(isolatedStateDir, "mo.json");
  const shellSource = [
    'echo "$$" > "$MO_WATCH_PID_FILE"',
    'mkdir -p "$MO_STATE_DIR"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}},"plugins":{"enabled":false}}' > "$MO_CONFIG_PATH"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    MO_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    MO_HOME: isolatedHomeDir,
    MO_CONFIG_PATH: isolatedConfigPath,
    MO_STATE_DIR: isolatedStateDir,
...
export async function runTimedWatch(options, outputDir, deps = {}) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-gateway-watch-"));
