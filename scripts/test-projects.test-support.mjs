...
const TOOLING_ISOLATED_TEST_TARGET = "test/scripts/mo-e2e-instance.test.ts";
...
const INCLUDE_FILE_ENV_KEY = "MO_VITEST_INCLUDE_FILE";
const FS_MODULE_CACHE_PATH_ENV_KEY = "MO_VITEST_FS_MODULE_CACHE_PATH";
...
const BROAD_CHANGED_FALLBACK_PATTERNS = [
  /^package\.json$/u,
  /^pnpm-lock\.yaml$/u,
  /^test\/setup(?:\.shared|\.extensions|-mo-runtime)?\.ts$/u,
...
const RUNTIME_SIDECAR_PATH_CONSUMER_TEST_TARGETS = [
  ...
  "test/mo-npm-postpublish-verify.test.ts",
];
...
const GITHUB_WORKFLOW_OWNER_TEST_TARGETS = new Map([
  ...
  [
    ".github/workflows/mo-cross-os-release-checks-reusable.yml",
    [
      "test/scripts/mo-cross-os-release-checks.test.ts",
      "test/scripts/mo-cross-os-release-workflow.test.ts",
      "test/scripts/package-acceptance-workflow.test.ts",
    ],
  ],
  [
    ".github/workflows/mo-live-and-e2e-checks-reusable.yml",
    [
      "test/scripts/package-acceptance-workflow.test.ts",
  ...
  [
    ".github/workflows/mo-npm-release.yml",
    [
      "test/mo-npm-postpublish-verify.test.ts",
      "test/scripts/package-acceptance-workflow.test.ts",
    ],
  ...
  [
    ".github/workflows/mo-performance.yml",
    ["test/scripts/mo-performance-workflow.test.ts"],
  ],
  [
    ".github/workflows/mo-release-checks.yml",
    [
      "test/scripts/mo-cross-os-release-checks.test.ts",
      "test/scripts/package-acceptance-workflow.test.ts",
      "test/scripts/plugin-prerelease-test-plan.test.ts",
  ...
  [
    ".github/workflows/mo-release-publish.yml",
    ["test/scripts/package-acceptance-workflow.test.ts"],
  ],
  [
    ".github/workflows/mo-scheduled-live-checks.yml",
    ["test/scripts/package-acceptance-workflow.test.ts"],
  ],
  [
    ".github/workflows/mo-stable-main-closeout.yml",
    ["test/scripts/package-acceptance-workflow.test.ts"],
  ],
  [
...
const TOOLING_SOURCE_TEST_TARGETS = new Map([
  ...
  ["scripts/github/resolve-mo-ref.sh", ["test/scripts/resolve-mo-ref.test.ts"]],
  ["scripts/ci-hydrate-testbox-env.sh", ["test/scripts/ci-hydrate-testbox-env.test.ts"]],
  [
    "scripts/github/run-mo-cross-os-release-checks.sh",
    ["test/scripts/mo-cross-os-release-workflow.test.ts"],
  ],
  ...
  [
    "scripts/mo-release-clawhub-runtime-state.ts",
    ["test/scripts/mo-release-clawhub-runtime-state.test.ts"],
  ],
  ["scripts/mo-release-clawhub-plan.ts", ["test/scripts/release-wrapper-scripts.test.ts"]],
  [
  ...
  [
    ...
    [
      ...
      "test/release-check.test.ts",
      "test/mo-npm-release-check.test.ts",
      "test/scripts/check-gateway-watch-regression.test.ts",
      "test/scripts/check-mo-package-tarball.test.ts",
      "test/scripts/mo-cross-os-release-checks.test.ts",
    ],
  ],
  [
  ...
  ["scripts/lib/mo-test-state.mjs", ["test/scripts/mo-test-state.test.ts"]],
  [
    "scripts/lib/workspace-bootstrap-smoke.mjs",
    ["test/release-check.test.ts", "test/mo-npm-release-check.test.ts"],
  ],
  [
  ...
  ["scripts/lib/mo-release-clawhub-plan.ts", ["test/plugin-clawhub-release.test.ts"]],
  [
  ...
  ["scripts/systemd/mo-auth-monitor.service", ["test/scripts/auth-monitor.test.ts"]],
  ["scripts/systemd/mo-auth-monitor.timer", ["test/scripts/auth-monitor.test.ts"]],
  ...
  ["scripts/podman/mo.container.in", ["test/scripts/test-install-sh-docker.test.ts"]],
  ...
  ["scripts/mo-npm-prepublish-verify.ts", ["test/mo-npm-prepublish-verify.test.ts"]],
  ["scripts/mo-npm-postpublish-verify.ts", ["test/mo-npm-postpublish-verify.test.ts"]],
  ["scripts/mo-npm-release-check.ts", ["test/mo-npm-release-check.test.ts"]],
  ["scripts/mo-prepack.ts", ["test/mo-prepack.test.ts"]],
  [
    "scripts/check-mo-package-tarball.mjs",
    ["test/scripts/check-mo-package-tarball.test.ts"],
  ],
  ...
  [
    "scripts/package-mo-for-docker.mjs",
    ["test/e2e/qa-lab/runtime/package-mo-for-docker.e2e.test.ts"],
  ],
  ...
  [
    ...
    ["test/scripts/docker-build-helper.test.ts", "test/scripts/mo-test-state.test.ts"],
  ],
  [
  ...
  [
    ...
    [
      ...
      "test/scripts/mo-test-state.test.ts",
    ],
  ],
  ...
  [
    ...
    ["test/scripts/e2e-shell-tempfiles.test.ts", "test/scripts/mo-test-state.test.ts"],
  ],
  ...
  [
    "scripts/mo-performance-source-summary.mjs",
    ["test/scripts/mo-performance-source-summary.test.ts"],
  ],
...
const TOOLING_TEST_TARGETS = new Map([
  ...
  ["test/scripts/mo-test-state.test.ts", ["test/scripts/mo-test-state.test.ts"]],
...
const SOURCE_TEST_TARGETS = new Map([
  ...
  ["src/test-utils/mo-test-state.ts", ["src/test-utils/mo-test-state.test.ts"]],
  [
...
const BROAD_CHANGED_ENV_KEY = "MO_TEST_CHANGED_BROAD";
const VITEST_NO_OUTPUT_TIMEOUT_ENV_KEY = "MO_VITEST_NO_OUTPUT_TIMEOUT_MS";
const VITEST_NO_OUTPUT_HEARTBEAT_ENV_KEY = "MO_VITEST_NO_OUTPUT_HEARTBEAT_MS";
const VITEST_NO_OUTPUT_RETRY_ENV_KEY = "MO_VITEST_NO_OUTPUT_RETRY";
...
export function formatNoChangedTestTargetLines(skippedBroadFallbackPaths) {
  ...
  return [
    ...
    "[test] run `MO_TEST_CHANGED_BROAD=1 pnpm test:changed` for broad coverage.",
  ];
}
...
export function buildFullSuiteVitestRunPlans(args, cwd = process.cwd()) {
  ...
  const parallelShardCount = parsePositiveInt(
    process.env.MO_TEST_PROJECTS_PARALLEL,
    "MO_TEST_PROJECTS_PARALLEL",
  );
  const expandToProjectConfigs =
    process.env.MO_TEST_PROJECTS_LEAF_SHARDS === "1" ||
    (Number.isFinite(parallelShardCount) && parallelShardCount > 1) ||
    shouldExpandLocalFullSuiteShardsByDefault(process.env);
  return fullSuiteVitestShards.flatMap((shard) => {
    if (
      process.env.MO_TEST_SKIP_FULL_EXTENSIONS_SHARD === "1" &&
...
export function shouldUseLocalFullSuiteParallelByDefault(env = process.env) {
  ...
  return (
    env.MO_TEST_PROJECTS_SERIAL !== "1" && env.CI !== "true" && env.GITHUB_ACTIONS !== "true"
  );
}
...
function hasConservativeVitestWorkerBudget(env) {
  const workerBudget = parsePositiveInt(
    env.MO_VITEST_MAX_WORKERS ?? env.MO_TEST_WORKERS,
    env.MO_VITEST_MAX_WORKERS === undefined
      ? "MO_TEST_WORKERS"
      : "MO_VITEST_MAX_WORKERS",
  );
...
export function resolveParallelFullSuiteConcurrency(specCount, envInput, hostInfo) {
  ...
  const override = parsePositiveInt(
    env.MO_TEST_PROJECTS_PARALLEL,
    "MO_TEST_PROJECTS_PARALLEL",
  );
  if (override !== null) {
    return Math.min(override, specCount);
  }
  if (env.MO_TEST_PROJECTS_SERIAL === "1") {
    return 1;
  }
  if (isCiLikeEnv(env)) {
    return 1;
  ...
  if (
    env.MO_TEST_PROJECTS_LEAF_SHARDS !== "1" &&
  ...
  return plans.map((plan, index) => {
    const includeFilePath = plan.includePatterns
      ? path.join(
          ...
          `mo-vitest-include-${randomUUID()}-${index}.json`,
        )
      : null;
    return {
      config: plan.config,
...
export function shouldAcquireLocalHeavyCheckLock(runSpecs, env = process.env) {
  if (env.MO_TEST_HEAVY_CHECK_LOCK_HELD === "1") {
    return false;
  }

  if (env.MO_TEST_PROJECTS_FORCE_LOCK === "1") {
    return true;
  }
