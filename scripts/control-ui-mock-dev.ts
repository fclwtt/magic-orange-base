...
function createChatPickerScenario(): ControlUiMockGatewayScenario {
  ...
  const workspaceListCases = ["main", "alpha", "mo-mock"].map((agentId) => ({
    match: { agentId },
    response: {
      agentId,
  ...
  const workspaceFileCases = ["main", "alpha", "mo-mock"].flatMap((agentId) =>
  ...
  <text x="320" y="326" text-anchor="middle" font-family="ui-sans-serif, system-ui" font-size="24" fill="#f6f7f9">mo session artifact</text>
</svg>`;
  const lobsterArtifact = {
    id: "artifact-mo-lobster",
    type: "image",
    title: "mo-lobster-preview.svg",
  ...
  return {
    assistantAgentId: "mo-mock",
    assistantName: "OpenClaw mock",
    defaultAgentId: "mo-mock",
...
function createMockGatewayPlugin(scenario: ControlUiMockGatewayScenario): Plugin {
  ...
  return {
    ...
    name: "mo-control-ui-mock-gateway",
    transformIndexHtml(html) {
      return html.replace(
        "</head>",
        `    <script data-mo-control-ui-mock-gateway>\n${initScript}\n    </script>\n  </head>`,
      );
    },
  };
}
...
const server = await createServer({
  ...
  define: {
    MO_CONTROL_UI_BUILD_ID: JSON.stringify("mock"),
  },
  logLevel: "error",
  optimizeDeps: {
