...
const requiredPropertyNames = [
  "MO_ANDROID_STORE_FILE",
  "MO_ANDROID_STORE_PASSWORD",
  "MO_ANDROID_KEY_ALIAS",
  "MO_ANDROID_KEY_PASSWORD",
];
const sourceRequiredPropertyNames = requiredPropertyNames.filter(
  (name) => name !== "MO_ANDROID_STORE_FILE",
);
...
function parseArgs(argv) {
  const options = {
    ...
    workspace: "",
    materializedDir: "",
    keystorePath: process.env.MO_ANDROID_UPLOAD_KEYSTORE || "",
    propertiesPath: process.env.MO_ANDROID_SIGNING_PROPERTIES || "",
  };
...
function writeMaterializedProperties(materializedDir, sourceProperties) {
  ...
  properties.set("MO_ANDROID_STORE_FILE", keystorePath);
...
function validateMaterializedSigning(materializedDir) {
  ...
  requireProperties(properties, requiredPropertyNames, propertiesPath);
  if (properties.get("MO_ANDROID_STORE_FILE") !== keystorePath) {
    throw new Error(
      `${relativePath(propertiesPath)} must point MO_ANDROID_STORE_FILE at ${relativePath(keystorePath)}.`,
    );
  }
}
...
function requirePushSources(options) {
  if (!options.keystorePath) {
    throw new Error(
      "Missing Android upload keystore source. Pass --keystore or set MO_ANDROID_UPLOAD_KEYSTORE.",
    );
  }
  if (!options.propertiesPath) {
    throw new Error(
      "Missing Android signing properties source. Pass --properties or set MO_ANDROID_SIGNING_PROPERTIES.",
    );
  }
