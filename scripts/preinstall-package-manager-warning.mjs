...
export function createPackageManagerWarningMessage(packageManager) {
  ...
  return [
    `[mo] warning: detected ${packageManager} for install lifecycle.`,
    "[mo] this repo works best with pnpm; npm-compatible installs are slower and much larger here.",
    "[mo] prefer: corepack pnpm install",
  ].join("\n");
}
