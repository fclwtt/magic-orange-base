...
const TOOLING_EXCLUDED_TESTS = new Set([
  ...
  "test/scripts/mo-e2e-instance.test.ts",
]);
...
export function resolveVitestNodeArgs(env = process.env) {
  if (isTruthyEnvValue(env.MO_VITEST_ENABLE_MAGLEV)) {
    return [];
  }
...
export function resolveVitestCliEntry({
  ...
  try {
    ...
    if (isMissingVitestResolveError(error)) {
      ...
      wrappedError.code = "MO_MISSING_VITEST";
      throw wrappedError;
    }
    throw error;
  }
...
function shouldApplyNativeWorkerBudget(env) {
  ...
  return (
    env.MO_TEST_PROJECTS_SERIAL === "1" || resolveExplicitVitestWorkerBudget(env) !== null
  );
}
...
function resolveExplicitVitestWorkerBudget(env) {
  return parsePositiveInt(env.MO_VITEST_MAX_WORKERS ?? env.MO_TEST_WORKERS);
}
  ...
  try {
    return fsImpl.readdirSync(extensionsDir, { withFileTypes: true }).some((entry) => {
      ...
      return (
        ...
        fsImpl.existsSync(path.join(pluginDir, "mo.plugin.json"))
      );
    });
  } catch {
    return false;
...
export function resolveLinkedSourceBundledPluginsEnv(
  ...
  if (env.MO_BUNDLED_PLUGINS_DIR?.trim()) {
    return {};
  }
  ...
  return {
    MO_BUNDLED_PLUGINS_DIR: extensionsDir,
    MO_TEST_TRUST_BUNDLED_PLUGINS_DIR:
      env.MO_TEST_TRUST_BUNDLED_PLUGINS_DIR?.trim() || "1",
  };
}
...
function main(argv = process.argv.slice(2), env = process.env) {
  ...
  try {
    ...
    if (error instanceof Error && error.code === "MO_MISSING_VITEST") {
