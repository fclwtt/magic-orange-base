...
const loadAuthProfiles = (agentId: string) => {
  const stateRoot = process.env.MO_STATE_DIR?.trim() || path.join(os.homedir(), ".mo");
...
const resolveFetchTimeoutMs = (raw = process.env.MO_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS) => {
  return parseStrictIntegerOption({
    fallback: DEFAULT_FETCH_TIMEOUT_MS,
    label: "MO_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS",
    min: 1,
    raw,
  });
};
