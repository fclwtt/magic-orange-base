...
const DEFAULT_WORKFLOW = "mo-live-and-e2e-checks-reusable.yml";
...
function ghWorkflowCommand(lanes, ref, workflow, reuseInputs = {}) {
  const workflowRef =
    ...
    process.env.MO_DOCKER_E2E_WORKFLOW_REF ||
...
function defaultOutputDir(input) {
  return fs.mkdtempSync(path.join(os.tmpdir(), `mo-docker-e2e-rerun-${safePathSegment(input)}-`));
}
