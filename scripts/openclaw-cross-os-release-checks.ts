...
const PUBLISHED_INSTALLER_BASE_URL = "https://mo.ai";
...
export const CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS = parsePositiveIntegerEnv(
  "MO_CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS",
  600,
);
...
const CROSS_OS_PROCESS_TREE_KILL_AFTER_MS = parsePositiveIntegerEnv(
  "MO_CROSS_OS_PROCESS_TREE_KILL_AFTER_MS",
  15_000,
);
...
export function resolveProviderConfig(provider, env = process.env) {
  ...
  const providerEnvKey = `MO_CROSS_OS_${provider.toUpperCase().replace(/[^A-Z0-9]+/gu, "_")}_MODEL`;
  const model = env[providerEnvKey]?.trim() || env.MO_CROSS_OS_MODEL?.trim() || config.model;
  return { ...config, model };
}
...
function buildReleaseProviderConfigOverride(providerMeta) {
  ...
  return {
    ...
    ...(providerMeta.extensionId === "openai" ? { agentRuntime: { id: "mo" } } : {}),
    models: [],
...
const INSTALL_STAGE_DEBRIS_DIR_PATTERN = /^\.mo-install-stage(?:-[^/]+)?$/iu;
...
export const CROSS_OS_DISCORD_FETCH_TIMEOUT_MS = parsePositiveIntegerEnv(
  "MO_CROSS_OS_DISCORD_FETCH_TIMEOUT_MS",
  10_000,
);
...
export const CROSS_OS_COMMAND_HEARTBEAT_SECONDS = parsePositiveIntegerEnv(
  "MO_CROSS_OS_COMMAND_HEARTBEAT_SECONDS",
  60,
);
...
export function resolveCrossOsAgentTurnOptional(env = process.env) {
  return parseBooleanEnv("MO_CROSS_OS_AGENT_TURN_OPTIONAL", false, env);
}
...
async function main(argv) {
  ...
  const baselineSpec =
    ...
    (previousVersion ? `mo@${previousVersion}` : "mo@latest");
  ...
  const summary = {
    ...
    runnerOs: process.env.MO_RELEASE_CHECK_OS ?? "",
    runnerLabel: process.env.MO_RELEASE_CHECK_RUNNER ?? "",
    provider,
    mode,
    suite,
    ref: inputRef || null,
  ...
  try {
    ...
    await runInstalledCli({
      ...
      env: {
        ...
        MO_UPDATE_DEV_TARGET_REF: verificationRef,
      },
      cwd: lane.homeDir,
...
function createLaneState(name) {
  const rootDir = mkdtempSync(join(tmpdir(), `mo-${name}-`));
  const prefixDir = join(rootDir, "prefix");
  const homeDir = join(rootDir, "home");
  const stateDir = join(homeDir, ".mo");
...
function buildLaneEnv(lane, providerMeta, providerSecretValue) {
  ...
  return {
    ...
    MO_HOME: lane.homeDir,
    MO_STATE_DIR: lane.stateDir,
    MO_CONFIG_PATH: join(lane.stateDir, "mo.json"),
    MO_DISABLE_BONJOUR: "1",
    MO_DISABLE_BUNDLED_PLUGIN_POSTINSTALL: "1",
...
export function buildRealUpdateEnv(env) {
  ...
  delete updateEnv.MO_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function isRecoverableWindowsPackagedUpgradeTimeoutError(
  ...
  return (
    ...
    /[/\\]mo\.mjs update --tag http:\/\/127\.0\.0\.1:\d+\/mo[^/\s]*\.tgz --yes --json(?: --no-restart)? --timeout \d+/u.test(
      message,
    )
  );
}
...
export function resolveExplicitBaselineVersion(baselineSpec) {
  ...
  if (!trimmed || trimmed === "mo@latest") {
    return "";
  }
  if (trimmed.startsWith("mo@")) {
    return trimmed.slice("mo@".length);
  }
  return trimmed;
}
...
async function resolveInstallerTargetVersion(params) {
  ...
  const latestResult = await runCommand(npmCommand(), ["view", "mo@latest", "version"], {
    logPath: join(params.logsDir, `${params.suiteName}-latest-version.log`),
    timeoutMs: 2 * 60 * 1000,
  });
  const latestVersion = latestResult.stdout.trim();
  if (!latestVersion) {
    throw new Error("npm view mo@latest version did not return a version.");
  }
...
if ($null -ne $npmCommand) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($npmPrefix)) {
    ...
    foreach ($candidate in @(
      (Join-Path $npmPrefix 'mo.cmd'),
      (Join-Path $npmPrefix 'mo.ps1')
    )) {
...
if ([string]::IsNullOrWhiteSpace($commandPath)) {
  $cmd = Get-Command mo -ErrorAction Stop
...
Write-Output "__MO_PATH__=$commandPath"
...
async function verifyFreshShellCommand(params) {
  if (process.platform === "win32") {
    ...
    const cliPath = normalizeWindowsInstalledCliPath(
      parseMarkerLine(result.stdout, "__MO_PATH__="),
    );
    if (!cliPath) {
      throw new Error("Failed to resolve installed mo path from fresh Windows shell.");
    }
    return {
      cliPath,
  ...
  const cliPath = parseMarkerLine(result.stdout, "__MO_PATH__=");
  const versionOutput = `${result.stdout}\n${result.stderr}`.trim();
  if (!cliPath) {
    throw new Error("Failed to resolve installed mo path from fresh POSIX shell.");
  }
...
async function ensureDevUpdateGitInstall(params) {
  ...
  // The dev-update lane must prove that `mo update --channel dev` landed on
...
async function configureDiscordSmoke(params) {
  ...
  if (!shouldUseManagedGatewayService()) {
    ...
    const gateway = await startManualGatewayFromInstalledCli({
      ...
      cliPath: params.cliPath,
      env: gatewayEnv,
      logPath: join(params.cwd, `.mo/logs/${params.lane.name}-discord-gateway.log`),
    });
...
export function dashboardHtmlMarkerStatus(html: string): {
  ...
  const app = html.includes("<mo-app></mo-app>");
...
async function maybeRunDiscordRoundtrip(params) {
  const token =
    process.env.MO_DISCORD_SMOKE_BOT_TOKEN?.trim() ||
    process.env.DISCORD_BOT_TOKEN?.trim() ||
    "";
  const guildId = process.env.MO_DISCORD_SMOKE_GUILD_ID?.trim() || "";
  const channelId = process.env.MO_DISCORD_SMOKE_CHANNEL_ID?.trim() || "";
...
async function runBundledPluginPostinstall(params) {
  ...
  delete installEnv.MO_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function buildInstalledBrowserOverrideImportProbeScript(
  runtimeModuleSpecifier = "mo/plugin-sdk/plugin-runtime",
) {
  return `
import { existsSync } from "node:fs";
import { startLazyPluginServiceModule } from ${JSON.stringify(runtimeModuleSpecifier)};

const startedPath = process.env.MO_BROWSER_OVERRIDE_STARTED_PATH;
const stoppedPath = process.env.MO_BROWSER_OVERRIDE_STOPPED_PATH;

if (!process.env.MO_BROWSER_CONTROL_MODULE) {
  throw new Error("Missing MO_BROWSER_CONTROL_MODULE.");
}
...
const handle = await startLazyPluginServiceModule({
  overrideEnvVar: "MO_BROWSER_CONTROL_MODULE",
...
export async function startBrowserControlService() {
  writeFileSync(process.env.MO_BROWSER_OVERRIDE_STARTED_PATH, "started\\n", "utf8");
}

export async function stopBrowserControlService() {
  writeFileSync(process.env.MO_BROWSER_OVERRIDE_STOPPED_PATH, "stopped\\n", "utf8");
}
`.trim();
}
...
async function runInstalledBrowserOverrideImportSmoke(params) {
  ...
  await runCommand(process.execPath, [probePath], {
    ...
    env: {
      ...params.env,
      MO_BROWSER_CONTROL_MODULE: pathToFileURL(overridePath).href,
      MO_BROWSER_OVERRIDE_STARTED_PATH: startedPath,
      MO_BROWSER_OVERRIDE_STOPPED_PATH: stoppedPath,
    },
    logPath: params.logPath,
    timeoutMs: 60_000,
  });
...
function installedPackageRoot(prefixDir, platform = process.platform) {
  return platform === "win32"
    ? join(prefixDir, "node_modules", "mo")
    : join(prefixDir, "lib", "node_modules", "mo");
}

function installedEntryPath(prefixDir) {
  return join(installedPackageRoot(prefixDir), "mo.mjs");
}
