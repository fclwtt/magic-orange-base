...
export function parseArgs(argv) {
  ...
  const options = {
    ...
    pluginIds: [],
    shardTotal: readOptionalPositiveIntEnv("MO_PLUGIN_GATEWAY_GAUNTLET_TOTAL") ?? 1,
    shardIndex: readOptionalNonNegativeIntEnv("MO_PLUGIN_GATEWAY_GAUNTLET_INDEX") ?? 0,
    limit: undefined,
    skipPrebuild: false,
    skipLifecycle: false,
    skipQa: false,
    ...
    qaTimeoutMs: 900_000,
    allowEmpty: false,
    failOnObservation: process.env.MO_PLUGIN_GATEWAY_GAUNTLET_FAIL_ON_OBSERVATION === "1",
    keepRunRoot: process.env.MO_PLUGIN_GATEWAY_GAUNTLET_KEEP_RUN_ROOT === "1",
  };
  const envIds = normalizeCsv(process.env.MO_PLUGIN_GATEWAY_GAUNTLET_IDS);
...
Environment:
  MO_PLUGIN_GATEWAY_GAUNTLET_IDS   Comma-separated plugin ids to include
  MO_PLUGIN_GATEWAY_GAUNTLET_TOTAL Total plugin shards
  MO_PLUGIN_GATEWAY_GAUNTLET_INDEX Zero-based shard index
  MO_PLUGIN_GATEWAY_GAUNTLET_FAIL_ON_OBSERVATION=1
  MO_PLUGIN_GATEWAY_GAUNTLET_KEEP_RUN_ROOT=1
  MO_PLUGIN_GATEWAY_GAUNTLET_QA_SUMMARY_MAX_BYTES  QA summary read ceiling
`);
}
...
function moCommand(repoRoot, args) {
  return {
    command: process.execPath,
...
function createIsolatedEnv(repoRoot, runRoot) {
  ...
  return {
    ...
    MO_CONFIG_PATH: path.join(stateDir, "mo.json"),
    MO_LOG_DIR: path.join(runRoot, "logs"),
    MO_QA_SUITE_PROGRESS: process.env.MO_QA_SUITE_PROGRESS ?? "1",
...
function buildSlashHelpProbe(params) {
  ...
  return {
    ...
    ...moCommand(params.repoRoot, [command, "--help"]),
...
async function runPluginLifecycleCommand(params) {
  ...
  params.rows.push(
    await runMeasuredCommand({
      ...
      ...moCommand(params.repoRoot, ["plugins", ...params.args]),
      label: params.label,
...
async function main() {
  ...
  const runRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mo-plugin-gauntlet-"));
