...
const DEFAULT_OUTPUT_NAME = "mo-current.tgz";
...
const TRUSTED_PACKAGE_SOURCE_TOKEN_ENV = "MO_TRUSTED_PACKAGE_TOKEN";
...
export const MO_PACKAGE_SPEC_RE =
  /^mo@(alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;

function usage() {
  return `Usage: node scripts/resolve-mo-package-candidate.mjs --source <ref|npm|url|trusted-url|artifact> --output-dir <dir> [options]
...
function resolvePackedOpenClawTarballFilename(value) {
  ...
  if (
    !/^mo-[A-Za-z0-9._-]+\.tgz$/u.test(filename) ||
    filename.includes("\0") ||
...
export function validateOpenClawPackageSpec(spec) {
  if (!MO_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `package_spec must be mo@alpha, mo@beta, mo@latest, or an exact OpenClaw release version; got: ${spec}`,
    );
  }
}
...
async function resolveCandidate(options) {
  ...
  await run("node", ["scripts/check-mo-package-tarball.mjs", target], {
    timeoutMs: 5 * 60 * 1000,
  });
  console.error(
  ...
  if (pkg.name !== "mo") {
    throw new Error(`package candidate must be named "mo"; got: ${pkg.name || "<missing>"}`);
  }
  if (!pkg.version) {
