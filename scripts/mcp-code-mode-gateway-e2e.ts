...
export async function main() {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-mcp-code-mode-"));
  const keep = process.env.MO_MCP_CODE_MODE_GATEWAY_E2E_KEEP === "1";
  const previousEnv = {
  ...
  try {
    ...
    await provider?.stop();
    resetConfigRuntimeState();
    restoreEnvValue("MO_STATE_DIR", previousEnv.stateDir);
    restoreEnvValue("MO_CONFIG_PATH", previousEnv.configPath);
    restoreEnvValue("MO_TEST_FAST", previousEnv.testFast);
    if (!keep) {
