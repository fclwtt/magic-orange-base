...
// Usage:
// MO_PROMPT_TRANSPORT=direct|gateway
// MO_PROMPT_MODE=extra
// MO_PROMPT_TEXT='...'
// MO_PROMPT_CAPTURE=1
...
const TRANSPORT = process.env.MO_PROMPT_TRANSPORT?.trim() === "direct" ? "direct" : "gateway";
const GATEWAY_PROMPT_MODE = "extra";
const PROMPT_TEXT = process.env.MO_PROMPT_TEXT?.trim() ?? "";
const PROMPT_LIST_JSON = process.env.MO_PROMPT_LIST_JSON?.trim() ?? "";
const USER_PROMPT = process.env.MO_USER_PROMPT?.trim() || "is clawd here?";
const ENABLE_CAPTURE = parseBooleanEnv({
  fallback: false,
  name: "MO_PROMPT_CAPTURE",
  raw: process.env.MO_PROMPT_CAPTURE,
});
const INCLUDE_RAW = parseBooleanEnv({
  fallback: false,
  name: "MO_PROMPT_INCLUDE_RAW",
  raw: process.env.MO_PROMPT_INCLUDE_RAW,
});
const KEEP_TMP = parseBooleanEnv({
  fallback: false,
  name: "MO_PROMPT_KEEP_TMP",
  raw: process.env.MO_PROMPT_KEEP_TMP,
});
const CLAUDE_BIN = process.env.CLAUDE_BIN?.trim() || "claude";
const NODE_BIN = process.env.MO_NODE_BIN?.trim() || process.execPath;
const TIMEOUT_MS = parseStrictIntegerOption({
  fallback: 45_000,
  label: "MO_PROMPT_TIMEOUT_MS",
  min: 1,
  raw: process.env.MO_PROMPT_TIMEOUT_MS,
});
const GATEWAY_TIMEOUT_MS = parseStrictIntegerOption({
  fallback: 120_000,
  label: "MO_PROMPT_GATEWAY_TIMEOUT_MS",
  min: 1,
  raw: process.env.MO_PROMPT_GATEWAY_TIMEOUT_MS,
});
...
const CAPTURE_PROXY_MAX_BODY_BYTES = parseStrictIntegerOption({
  ...
  label: "MO_PROMPT_CAPTURE_MAX_BODY_BYTES",
  min: 1,
  raw: process.env.MO_PROMPT_CAPTURE_MAX_BODY_BYTES,
});
const GATEWAY_LOG_TAIL_BYTES = 256 * 1024;
const SETUP_TOKEN_RAW = process.env.MO_LIVE_SETUP_TOKEN?.trim() ?? "";
const SETUP_TOKEN_VALUE = process.env.MO_LIVE_SETUP_TOKEN_VALUE?.trim() ?? "";
const SETUP_TOKEN_PROFILE = process.env.MO_LIVE_SETUP_TOKEN_PROFILE?.trim() ?? "";
...
function resolveSetupTokenSource(): TokenSource {
  ...
  if (!match) {
    throw new Error(
      "no Anthropics setup-token profile found; set MO_LIVE_SETUP_TOKEN_VALUE or MO_LIVE_SETUP_TOKEN_PROFILE",
    );
  }
  return { profileId: match.id, token: validateSetupToken(match.token) };
}
...
async function runDirectPrompt(prompt: string): Promise<PromptResult> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-direct-prompt-probe-"));
...
async function startGatewayProcess(params: {
  ...
  const child = spawn(
    NODE_BIN,
    ["mo.mjs", "gateway", "--port", String(params.port), "--bind", "loopback", "--force"],
    {
      cwd: process.cwd(),
      env: {
        ...process.env,
        MO_CONFIG_PATH: params.configPath,
        MO_STATE_DIR: params.stateDir,
        MO_AGENT_DIR: params.agentDir,
        MO_GATEWAY_TOKEN: params.gatewayToken,
        MO_SKIP_CHANNELS: "1",
        MO_SKIP_GMAIL_WATCHER: "1",
        MO_SKIP_CANVAS_HOST: "1",
        MO_SKIP_BROWSER_CONTROL_SERVER: "1",
        MO_DISABLE_BONJOUR: "1",
        MO_SKIP_CRON: "1",
        MO_TEST_MINIMAL_GATEWAY: "1",
        MO_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir,
        ANTHROPIC_API_KEY: "",
        ANTHROPIC_API_KEY_OLD: "",
      },
      detached: process.platform !== "win32",
...
async function runGatewayPrompt(prompt: string): Promise<PromptResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-gateway-prompt-probe-"));
  const stateDir = path.join(tmpDir, "state");
  const agentDir = path.join(stateDir, "agents", "main", "agent");
  const bundledPluginsDir = path.join(tmpDir, "bundled-plugins-empty");
  const configPath = path.join(tmpDir, "mo.json");
...
async function main() {
  if (!PROMPT_TEXT && !PROMPT_LIST_JSON) {
    throw new Error("missing MO_PROMPT_TEXT or MO_PROMPT_LIST_JSON");
  }
