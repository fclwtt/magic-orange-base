...
function isBareImportSpecifier(id: string): boolean {
  if (
    id === "@mo/llm-core" ||
    id.startsWith("@mo/llm-core/") ||
    id === "@mo/model-catalog-core/model-catalog-types" ||
    id.startsWith("@mo/normalization-core/") ||
    id.startsWith("@mo/media-core/") ||
    id.startsWith("@mo/acp-core/")
  ) {
    return false;
  }
...
const flatDeclarationTempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-plugin-sdk-dts-"));
const shouldBuildPrivateQaEntries = process.env.MO_BUILD_PRIVATE_QA === "1";
