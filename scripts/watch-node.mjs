...
const shouldRunAutoDoctor = (deps, autoDoctorAttempted) =>
  ...
  !AUTO_DOCTOR_DISABLE_VALUES.has(
    String(deps.env.MO_GATEWAY_WATCH_AUTO_DOCTOR ?? "").toLowerCase(),
  );
...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[mo] ${message}\n`);
};
...
const printFriendlyWatchStartupError = (err) => {
  ...
  if (packageConfigPath) {
    console.error(`[mo] Invalid package config: ${packageConfigPath}`);
  }
  console.error("[mo] This usually means a file in node_modules is empty or truncated.");
  console.error("[mo] Recommended recovery:");
  console.error("[mo]   rm -rf node_modules");
  console.error("[mo]   pnpm store prune");
  console.error("[mo]   pnpm install");
  console.error("");
  console.error("[mo] Original error:");
  console.error(err);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.MO_WATCH_MODE = "1";
  childEnv.MO_WATCH_SESSION = watchSession;
  ...
  childEnv.MO_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.MO_WATCH_COMMAND = deps.args.join(" ");
  }
  ...
  return await new Promise((resolve, reject) => {
    ...
    const runAutoDoctorAndRestart = () => {
      ...
      logWatcher(
        "Gateway exited early; running `mo doctor --fix --non-interactive` once.",
        deps,
      );
