...
const DEFAULT_ENTRY = "mo.mjs";
const MAX_RSS_MARKER = "__MO_MAX_RSS_KB__=";
...
function parseGatewayPortEnv(raw: string | undefined): number {
  ...
  if (bracketHostMatch) {
    return parsePositiveInt(bracketHostMatch[1], 32123, "MO_GATEWAY_PORT");
  }
  ...
  return parsePositiveInt(portRaw, 32123, "MO_GATEWAY_PORT");
}
...
function buildConfigFixture(commandCase: CommandCase): Record<string, unknown> | null {
  ...
  const port = parseGatewayPortEnv(process.env.MO_GATEWAY_PORT);
  return {
    gateway: {
      auth: { mode: "none" },
      bind: "loopback",
...
async function runSample(params: {
  ...
  const runRoot = mkdtempSync(path.join(os.tmpdir(), "mo-cli-bench-home-"));
  const stateDir = path.join(runRoot, ".mo");
...
Options:
  ...
  --entry <path>               Primary entry file (default: mo.mjs)
...
async function main(): Promise<void> {
  ...
  const tmpDir = mkdtempSync(path.join(os.tmpdir(), "mo-cli-bench-"));
