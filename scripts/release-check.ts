...
} from "./mo-npm-postpublish-verify.ts";
...
export const CRITICAL_PLUGIN_SDK_SIZE_CHECK_SPECIFIERS = [
  "mo/plugin-sdk/core",
  "mo/plugin-sdk/provider-entry",
  "mo/plugin-sdk/runtime",
] as const;
export const CRITICAL_PLUGIN_SDK_IMPORT_SMOKE_SPECIFIERS = ["mo/plugin-sdk/core"] as const;
...
export function runReleaseCheckCommand(
  ...
  const output = execFileSync(invocation.command, invocation.args, {
    ...
    maxBuffer:
      options.maxBuffer ??
      positiveEnvInt(
        "MO_RELEASE_CHECK_COMMAND_MAX_BUFFER_BYTES",
    ...
    timeout:
      options.timeoutMs ??
      positiveEnvInt(
        "MO_RELEASE_CHECK_COMMAND_TIMEOUT_MS",
...
export function createPackedBundledPluginPostinstallEnv(
  ...
  return {
    ...env,
    MO_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK: "1",
  };
}
...
export function collectPackedInstalledPackageVerificationErrors(params: {
  ...
  if (
    ...
    errors.push(
      `installed mo binary version mismatch: expected ${params.expectedVersion}, found ${params.installedBinaryVersion || "<missing>"}.`,
    );
  }
...
export function createPackedPluginSdkTypescriptSmokeProject(params: {
  ...
  writeFileSync(
    ...
    `${JSON.stringify(
      {
        name: "mo-plugin-sdk-type-smoke",
        private: true,
        type: "module",
        dependencies: {
          mo: params.packageSpec,
        },
      },
      null,
      2,
...
function runPackedPluginSdkTypescriptSmoke(tarballPath: string, tmpRoot: string): void {
  ...
  const installedOpenClawRoot = join(consumerDir, "node_modules", "mo");
  const tscPath = [
...
export function writePackedBundledPluginActivationConfig(homeDir: string): void {
  const configPath = join(homeDir, ".mo", "mo.json");
  mkdirSync(join(homeDir, ".mo"), { recursive: true });
  writeFileSync(
    configPath,
    `${JSON.stringify(
      {
...
function runPackedBundledPluginActivationSmoke(packageRoot: string, tmpRoot: string): void {
  ...
  const env = createPackedCliSmokeEnv(process.env, {
    HOME: homeDir,
    OPENAI_API_KEY: "sk-mo-release-check",
  });
  ...
  runReleaseCheckCommand(
    {
      command: process.execPath,
      args: [join(packageRoot, "mo.mjs"), ...PACKED_BUNDLED_RUNTIME_DEPS_REPAIR_ARGS],
    },
    {
      cwd: packageRoot,
      stdio: "inherit",
  ...
  runReleaseCheckCommand(
    { command: process.execPath, args: [join(packageRoot, "mo.mjs"), "plugins", "doctor"] },
    {
      cwd: packageRoot,
      stdio: "inherit",
      env,
...
function runPackedBundledChannelEntrySmoke(): void {
  const tmpRoot = mkdtempSync(join(tmpdir(), "mo-release-pack-smoke-"));
  try {
    ...
    const packageRoot = join(resolveGlobalRoot(prefixDir, tmpRoot), "mo");
    ...
    runReleaseCheckCommand(
      {
        command: process.execPath,
        args: [join(packageRoot, "mo.mjs"), ...PACKED_COMPLETION_SMOKE_ARGS],
      },
      {
        cwd: packageRoot,
        stdio: "inherit",
  ...
  return [...paths]
    .filter(
      (path) =>
        ...
        /(^|\/)\.mo-runtime-deps-[^/]+(\/|$)/u.test(path) ||
        path.endsWith("/.mo-runtime-deps-stamp.json") ||
...
// Critical functions that channel extension plugins import from mo/plugin-sdk.
...
export function collectCriticalPluginSdkEntrypointSizeErrors(rootDir = process.cwd()): string[] {
  ...
  for (const specifier of CRITICAL_PLUGIN_SDK_SIZE_CHECK_SPECIFIERS) {
    const subpath = specifier.slice("mo/plugin-sdk/".length);
