...
function applyDefaultParallelVitestWorkerBudget(specs, env) {
  if (env.MO_VITEST_MAX_WORKERS || env.MO_TEST_WORKERS || isCiLikeEnv(env)) {
    return specs;
  }
  ...
  return specs.map((spec) => ({
    ...
    env: {
      ...spec.env,
      MO_VITEST_MAX_WORKERS: String(vitestMaxWorkers),
    },
  }));
}
...
async function main() {
  ...
  const isExplicitParallelMultiConfigRun =
    Boolean(baseEnv.MO_TEST_PROJECTS_PARALLEL) &&
    runSpecs.length > 1 &&
    !runSpecs.some((spec) => spec.watchMode);
  ...
  if (isParallelShardRun) {
    ...
    if (concurrency > 1) {
      ...
      if (
        !isCiLikeEnv(baseEnv) &&
        !baseEnv.MO_TEST_PROJECTS_PARALLEL &&
        !baseEnv.MO_VITEST_MAX_WORKERS &&
        !baseEnv.MO_TEST_WORKERS &&
