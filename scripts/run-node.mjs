...
const listRequiredBundledPluginMetadataOutputs = (pluginEntries, deps) =>
  pluginEntries.flatMap(({ id, hasManifest, hasPackageJson }) => {
    ...
    if (hasManifest) {
      requiredPaths.push(path.join(builtPluginDir, "mo.plugin.json"));
    }
...
const listRequiredOpenClawExtensionAliasOutputs = (deps) => {
  ...
  const aliasDir = path.join(distRoot, "extensions", "node_modules", "mo");
  return [
...
const listRequiredStaticExtensionAssetOutputs = (deps) => {
  if (deps.env.MO_RUNTIME_POSTBUILD_STATIC_ASSETS === "0") {
    return [];
  }
  const distRoot = resolveRuntimePostBuildDistRoot(deps);
  const runtimeRoot = resolveRuntimePostBuildRuntimeRoot(deps);
...
export const resolveBuildRequirement = (deps) => {
  if (deps.env.MO_FORCE_BUILD === "1") {
    return { shouldBuild: true, reason: "force_build" };
  }
  if (
    deps.env.MO_BUILD_PRIVATE_QA === "1" &&
...
export const resolveRuntimePostBuildRequirement = (deps) => {
  if (deps.env.MO_FORCE_RUNTIME_POSTBUILD === "1") {
...
const BUILD_REASON_LABELS = {
  force_build: "forced by MO_FORCE_BUILD",
...
const RUNTIME_POSTBUILD_REASON_LABELS = {
  force_runtime_postbuild: "forced by MO_FORCE_RUNTIME_POSTBUILD",
...
const RUN_NODE_OUTPUT_LOG_ENV = "MO_RUN_NODE_OUTPUT_LOG";
const RUN_NODE_CPU_PROF_DIR_ENV = "MO_RUN_NODE_CPU_PROF_DIR";
const RUN_NODE_CPU_PROF_MAX_FILES_ENV = "MO_RUN_NODE_CPU_PROF_MAX_FILES";
const RUN_NODE_FILTER_SYNC_IO_STDERR_ENV = "MO_RUN_NODE_FILTER_SYNC_IO_STDERR";
const RUN_NODE_BUILD_LOCK_TIMEOUT_ENV = "MO_RUN_NODE_BUILD_LOCK_TIMEOUT_MS";
const RUN_NODE_BUILD_LOCK_POLL_ENV = "MO_RUN_NODE_BUILD_LOCK_POLL_MS";
const RUN_NODE_BUILD_LOCK_STALE_ENV = "MO_RUN_NODE_BUILD_LOCK_STALE_MS";
const RUN_NODE_SKIP_DTS_BUILD_ENV = "MO_RUN_NODE_SKIP_DTS_BUILD";
...
const logRunner = (message, deps) => {
  if (deps.env.MO_RUNNER_LOG === "0") {
    return;
  }
  const line = `[mo] ${message}\n`;
...
const createRunNodeProgress = (label, deps) => {
  ...
  const render = () => {
    ...
    frameIndex += 1;
    deps.stderr.write(`\r[mo] ${frame} ${label} (${elapsedSeconds}s)`);
    visible = true;
  };
...
const resolveRunNodeCpuProfileArgs = (deps) => {
  ...
  const profileName = `mo-${commandName}-${pid}-${timestamp}.cpuprofile`;
...
const resolveRunNodeDiagnosticArgs = (deps) => {
  ...
  if (deps.env.MO_TRACE_SYNC_IO === "1") {
...
const runOpenClaw = async (deps) => {
  ...
  const useProcessGroup = shouldUseRunNodeChildProcessGroup(deps);
  const nodeProcess = deps.spawn(deps.execPath, [...diagnosticArgs, "mo.mjs", ...deps.args], {
    cwd: deps.cwd,
...
const closeRunNodeOutputTee = async (deps, exitCode) => {
  ...
  try {
    ...
    deps.stderr.write(
      `[mo] Failed to write output log: ${error?.message ?? "unknown error"}\n`,
    );
...
const canUseStampedGatewayClientDist = (deps) => {
  ...
  if (
    ...
    deps.env.MO_FORCE_RUNTIME_POSTBUILD === "1"
  ) {
    return false;
  }
...
export async function runNodeMain(params = {}) {
  ...
  if (deps.args[0] === "qa") {
    deps.env.MO_BUILD_PRIVATE_QA = "1";
    deps.env.MO_ENABLE_PRIVATE_QA_CLI = "1";
    deps.env.MO_DISABLE_BUNDLED_PLUGINS ??= "0";
  }
