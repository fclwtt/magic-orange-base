...
function buildFullSuiteLeafRunPlans() {
  const previousLeafShards = process.env.MO_TEST_PROJECTS_LEAF_SHARDS;
  ...
  try {
    ...
    if (previousLeafShards === undefined) {
      ...
      process.env.MO_TEST_PROJECTS_LEAF_SHARDS = previousLeafShards;
    }
  }
}
...
export function resolveFullSuiteVitestEnv(args, env = process.env, label = "") {
  if (
    !args.fullSuite ||
    env.MO_VITEST_MAX_WORKERS?.trim() ||
    env.MO_TEST_WORKERS?.trim()
  ) {
    return {};
  }

  return {
    MO_VITEST_MAX_WORKERS: label === "commands" ? "1" : "2",
  };
}
