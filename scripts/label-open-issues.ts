...
const STATE_FILE_PATH = join(CONFIG_BASE_DIR, "mo", STATE_FILE_NAME);
...
function resolveOpenAITimeoutMs(raw = process.env.MO_LABEL_OPEN_ISSUES_OPENAI_TIMEOUT_MS) {
  return parseStrictIntegerOption({
    fallback: DEFAULT_OPENAI_TIMEOUT_MS,
    label: "MO_LABEL_OPEN_ISSUES_OPENAI_TIMEOUT_MS",
    min: 1,
    raw,
  });
}
