...
export async function buildPackageArtifacts(sourceDir, options = {}) {
  ...
  for (const step of PACKAGE_ARTIFACT_BUILD_STEPS) {
    ...
    await runImpl(step.command, step.args, sourceDir, {
      env: {
        ...process.env,
        MO_BUILD_ALL_NO_PNPM: "1",
        MO_RUN_NODE_SKIP_DTS_BUILD: "1",
      },
      timeoutMs: resolveTimeoutMs(
        "MO_DOCKER_PACKAGE_BUILD_TIMEOUT_MS",
  ...
  try {
    ...
    packOutput = await runCaptureImpl(
      ...
      sourceDir,
      {
        ...
        timeoutMs: resolveTimeoutMs(
          "MO_DOCKER_PACKAGE_PACK_TIMEOUT_MS",
...
async function main() {
  ...
  await run(
    ...
    {
      timeoutMs: resolveTimeoutMs(
        "MO_DOCKER_PACKAGE_INVENTORY_TIMEOUT_MS",
  ...
  await run(
    "node",
    [path.join(ROOT_DIR, "scripts/check-mo-package-tarball.mjs"), tarball],
    sourceDir,
    {
      timeoutMs: resolveTimeoutMs(
        "MO_DOCKER_PACKAGE_TARBALL_CHECK_TIMEOUT_MS",
