...
const ignoreRepoBinary = process.env.MO_CRABBOX_WRAPPER_IGNORE_REPO_BINARY === "1";
...
const remoteChangedGateEnv = [
  "MO_CHECK_CHANGED_REMOTE_CHILD=1",
  "MO_CHANGED_LANES_RAW_SYNC=1",
  "CI=1",
];
...
function shouldRequireBrokeredAws(commandArgs, providerName) {
  if (process.env.MO_CRABBOX_ALLOW_DIRECT_AWS === "1") {
    return false;
  }
...
function enforceBrokeredAws(commandArgs, providerName) {
  ...
  console.error(
    [
      ...
      "[crabbox] run `crabbox login --url https://crabbox.mo.ai --provider aws`, then retry.",
      "[crabbox] for intentional direct AWS provider debugging, set MO_CRABBOX_ALLOW_DIRECT_AWS=1.",
    ].join("\n"),
  );
  process.exit(2);
}
...
function remoteGitBootstrapForChangedGate(changedGateBase) {
  ...
  return [
    "mo_changed_gate_base=${MO_CHANGED_GATE_BASE:-" + quotedBase + "};",
    'if ! command -v git >/dev/null 2>&1; then echo "git is required for OpenClaw remote changed-gate sync" >&2; exit 2; fi;',
    'mo_changed_gate_remote_base="$(git rev-parse --verify refs/remotes/origin/main 2>/dev/null || true)";',
    'if ! git status --short >/dev/null 2>&1 || [ "$mo_changed_gate_remote_base" != "$mo_changed_gate_base" ]; then',
    "rm -rf .git;",
    "git init -q;",
    "git remote add origin https://github.com/mo/mo.git 2>/dev/null || git remote set-url origin https://github.com/mo/mo.git;",
    'git fetch -q --depth=1 origin "$mo_changed_gate_base:refs/remotes/origin/main";',
    "git reset --mixed --quiet refs/remotes/origin/main;",
    "git add -A;",
    "if ! git diff --cached --quiet; then git -c user.name=OpenClaw -c user.email=ci@mo.local commit -q --no-gpg-sign -m remote-changed-gate-tree; fi;",
    "fi",
  ].join(" ");
}
...
function remoteWindowsHydratedNodeModulesBootstrap() {
  return [
    "$moModulesDir = $env:PNPM_CONFIG_MODULES_DIR",
    "if ($moModulesDir) {",
    'if (-not (Test-Path $moModulesDir)) { throw "PNPM_CONFIG_MODULES_DIR does not exist: $moModulesDir" }',
    '$moWorkspaceModules = Join-Path (Get-Location).Path "node_modules"',
    '$moSelfModules = Join-Path $moModulesDir "node_modules"',
    'if (-not (Test-Path $moSelfModules)) { cmd /c mklink /J "$moSelfModules" "$moModulesDir" | Out-Host; if ($LASTEXITCODE -ne 0) { throw "failed to link hydrated pnpm node_modules" } }',
    'if (-not (Test-Path $moWorkspaceModules)) { cmd /c mklink /J "$moWorkspaceModules" "$moModulesDir" | Out-Host; if ($LASTEXITCODE -ne 0) { throw "failed to link workspace node_modules" } }',
    "}",
  ].join("; ");
}
...
function remotePosixJsEnvBootstrap() {
  return [
    ...
    'while [ "$#" -gt 0 ]; do',
    'case "$1" in',
    '-i|--ignore-environment) mo_env_ignore=1; mo_env_args+=("$1"); shift ;;',
    '-S|--split-string|-S*|--split-string=*) command env "${mo_env_args[@]}" "$@"; return ;;',
    '-[!-]*i*) mo_env_ignore=1; mo_env_args+=("$1"); shift ;;',
    '-u|--unset|-C|--chdir) mo_env_args+=("$1"); shift; if [ "$#" -gt 0 ]; then mo_env_args+=("$1"); shift; fi ;;',
    '--unset=*|--chdir=*) mo_env_args+=("$1"); shift ;;',
    'PATH=*) if [ "$mo_env_ignore" = "1" ]; then mo_env_args+=("PATH=${MO_CRABBOX_BOOTSTRAP_PATH:-$PATH}:${1#PATH=}"); else mo_env_args+=("$1"); fi; mo_env_path_seen=1; shift ;;',
    '[A-Za-z_]*=*) mo_env_args+=("$1"); shift ;;',
    '--) mo_env_args+=("--"); shift; break ;;',
    "*) break ;;",
    "esac;",
    "done;",
    'if [ "$mo_env_ignore" = "1" ] && [ "$mo_env_path_seen" = "0" ]; then mo_env_args+=("PATH=${MO_CRABBOX_BOOTSTRAP_PATH:-$PATH}"); fi;',
    'command env "${mo_env_args[@]}" "$@";',
    "};",
  ];
}

function remoteAwsMacosJsBootstrap({ packageManager = false, bun = false } = {}) {
  const nodeVersion = process.env.MO_CRABBOX_MACOS_NODE_VERSION?.trim() || "24.15.0";
  const bootstrap = [
    "mo_crabbox_bootstrap_macos_js() {",
    'tool_root="${MO_CRABBOX_MACOS_TOOLCHAIN_DIR:-$HOME/.mo-crabbox-toolchain}";',
    ...
    'macos_locale="${MO_CRABBOX_MACOS_LOCALE:-en_US.UTF-8}";',
    ...
    'ready_marker="$node_dir/.mo-crabbox-node-ready";',
  ...
  if (bun) {
    bootstrap.push(
      ...
      'bun_ready_marker="$bun_root/.mo-crabbox-bun-ready";',
  ...
  bootstrap.push('export MO_CRABBOX_BOOTSTRAP_PATH="$PATH";');
  bootstrap.push("};", "mo_crabbox_bootstrap_macos_js");
  return bootstrap.join(" ");
}

function remoteWsl2JsBootstrap({ packageManager = false } = {}) {
  const nodeVersion = process.env.MO_CRABBOX_WSL2_NODE_VERSION?.trim() || "24.15.0";
  const bootstrap = [
    "mo_crabbox_bootstrap_wsl2_js() {",
    'tool_root="${MO_CRABBOX_WSL2_TOOLCHAIN_DIR:-$HOME/.mo-crabbox-toolchain}";',
    `node_version=${shellQuote(nodeVersion)};`,
    'arch="$(uname -m)";',
  ...
  bootstrap.push('export MO_CRABBOX_BOOTSTRAP_PATH="$PATH";');
  bootstrap.push("};", "mo_crabbox_bootstrap_wsl2_js");
  return bootstrap.join(" ");
}
...
function scopedAwsMacosEnvCommand(commandArgs) {
  ...
  return {
    ...
    bun: needsBun,
    shellCommand: `mo_crabbox_env ${shellJoin(commandArgs.slice(1))}`,
  };
}
...
function prepareRemoteWsl2JsBootstrapScript(commandArgs, providerName) {
  ...
  const scriptRoot = mkdtempSync(resolve(tmpdir(), "mo-crabbox-wsl2-script-"));
...
function remoteAwsMacosSwiftBootstrap() {
  return [
    "mo_crabbox_require_macos_swift_62() {",
    'mo_xcode="";',
    'for mo_candidate in /Applications/Xcode_26.1.app /Applications/Xcode_26*.app /Applications/Xcode-26*.app; do if [ -d "$mo_candidate" ]; then mo_xcode="$mo_candidate"; fi; done;',
    'if [ -n "$mo_xcode" ]; then mo_developer="$mo_xcode/Contents/Developer"; if [ ! -d "$mo_developer" ]; then mo_developer="$mo_xcode"; fi; sudo xcode-select -s "$mo_developer" || return 1; fi;',
    'mo_swift_version="$(swift --version 2>&1)" || { status=$?; printf "%s\\n" "$mo_swift_version" >&2; return "$status"; };',
    'printf "%s\\n" "$mo_swift_version" >&2;',
    'mo_swift_major_minor="$(printf "%s\\n" "$mo_swift_version" | sed -nE "s/.*Apple Swift version ([0-9]+)\\.([0-9]+).*/\\1 \\2/p" | head -n 1)";',
    'if [ -z "$mo_swift_major_minor" ]; then echo "[crabbox] OpenClaw macOS app proof requires Swift tools 6.2+; unable to parse swift --version." >&2; return 2; fi;',
    "set -- $mo_swift_major_minor;",
    ...
    "return 2;",
    "fi;",
    'mo_xcodebuild_version="$(xcodebuild -version 2>&1)" || { printf "%s\\n" "$mo_xcodebuild_version" >&2; echo "[crabbox] OpenClaw macOS app proof requires Xcode 26.x; active developer directory does not provide usable xcodebuild." >&2; return 2; };',
    'printf "%s\\n" "$mo_xcodebuild_version" >&2;',
    'mo_xcode_major="$(printf "%s\\n" "$mo_xcodebuild_version" | sed -nE "s/^Xcode ([0-9]+)(\\..*)?$/\\1/p" | head -n 1)";',
    'if [ "$mo_xcode_major" != "26" ]; then echo "[crabbox] OpenClaw macOS app proof requires Xcode 26.x; current xcodebuild is ${mo_xcode_major:-unknown}." >&2; return 2; fi;',
    "};",
    "mo_crabbox_require_macos_swift_62",
  ].join(" ");
}
...
function prepareAwsMacosScriptStdinBootstrap(commandArgs, providerName) {
  ...
  const scriptRoot = mkdtempSync(resolve(tmpdir(), "mo-crabbox-macos-script-"));
  const scriptPath = resolve(scriptRoot, "script.sh");
...
function createAwsMacosScriptStdinWrapper(script) {
  ...
  return [
    ...
    'tmp_script="$(mktemp "${TMPDIR:-/tmp}/mo-crabbox-script.XXXXXX")" || exit $?',
    'cleanup_mo_crabbox_script() { rm -f "$tmp_script"; }',
    "trap cleanup_mo_crabbox_script EXIT",
...
function uniqueHereDocDelimiter(script) {
  let index = 0;
  for (;;) {
    const delimiterLocal = `MO_CRABBOX_SCRIPT_${index}`;
...
function defaultFullCheckoutSyncRoot() {
  const home = homedir();
  if (home) {
    return resolve(home, ".cache", "mo", "crabbox-sync");
  }
  return resolve(tmpdir(), "mo-crabbox-sync");
}

function fullCheckoutSyncRoot() {
  const configured = process.env.MO_CRABBOX_SYNC_TMPDIR?.trim();
...
function assertFullCheckoutSyncDisk(root) {
  const requiredBytes = parseNonNegativeIntegerEnv(
    "MO_CRABBOX_SYNC_MIN_FREE_BYTES",
    1024 * 1024 * 1024,
    "byte count",
  );
  if (requiredBytes === 0) {
  ...
  throw new Error(
    [
      ...
      "set MO_CRABBOX_SYNC_TMPDIR to a roomier filesystem or lower MO_CRABBOX_SYNC_MIN_FREE_BYTES if you know this checkout fits",
    ].join("; "),
  );
}
...
function prepareFullCheckoutForSync(options = {}) {
  ...
  const dir = mkdtempSync(resolve(syncRoot, "mo-crabbox-sync-"));
  let active = false;
  ...
  if (!satisfiesMinimumCrabboxVersion(version.text, minimumBlacksmithCrabboxVersion)) {
    console.error(
      [
        ...
        "[crabbox] if using ../crabbox, rebuild it: version=$(git -C ../crabbox describe --tags --always --dirty | sed 's/^v//') && go build -C ../crabbox -trimpath -ldflags \"-s -w -X github.com/mo/crabbox/internal/cli.version=${version}\" -o bin/crabbox ./cmd/crabbox",
      ].join("\n"),
    );
    process.exit(2);
  }
...
if (
  ...
  childEnv.CRABBOX_LOCAL_CONTAINER_WORK_ROOT = "/tmp/mo-crabbox-docker-work";
  console.error(
