...
async function withFreshBundleCheckSource(sourceDir, run) {
  const tempDir = await fs.mkdtemp(path.join(tmpdir(), "mo-a2ui-native-check-"));
  try {
    ...
    bundleA2ui(rootDir, {
      ...process.env,
      MO_A2UI_BUNDLE_OUT: path.join(checkSourceDir, "a2ui.bundle.js"),
      MO_A2UI_BUNDLE_HASH_FILE: path.join(tempDir, ".bundle.hash"),
    });
