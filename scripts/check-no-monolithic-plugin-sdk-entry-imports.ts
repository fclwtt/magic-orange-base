...
const ROOT_IMPORT_PATTERN = /["']mo\/plugin-sdk["']/;
const LEGACY_COMPAT_IMPORT_PATTERN = /["']mo\/plugin-sdk\/compat["']/;
const LEGACY_BROAD_SUBPATH_PATTERNS = [
  {
    pattern: /["']mo\/plugin-sdk\/channel-runtime["']/,
    label: "mo/plugin-sdk/channel-runtime",
  },
  {
    pattern: /["']mo\/plugin-sdk\/config-runtime["']/,
    label: "mo/plugin-sdk/config-runtime",
  },
  {
    pattern: /["']mo\/plugin-sdk\/infra-runtime["']/,
    label: "mo/plugin-sdk/infra-runtime",
  },
] as const;
...
function main() {
  ...
  if (
    ...
    if (monolithicOffenders.length > 0) {
      console.error("Bundled plugin source files must not import monolithic mo/plugin-sdk.");
    ...
    if (legacyCompatOffenders.length > 0) {
      console.error(
        "Bundled plugin source files must not import legacy mo/plugin-sdk/compat.",
      );
    ...
    if (
      ...
      console.error(
        "Use focused mo/plugin-sdk/<domain> subpaths for bundled plugins; root, compat, and broad runtime barrels are legacy surfaces only.",
      );
    }
    process.exit(1);
  }
