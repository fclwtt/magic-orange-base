 ...
 * `mo update`. Use it to verify the completion installation flow
...
\`mo update\`, \`mo doctor\`, and \`mo onboard\`.
...
async function main() {
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`mo completion --install\` later to enable.`));
    return;
  }
