...
function parseArgs(argv, env = process.env) {
  const opts = parseFlagArgs(
    argv,
    {
      ...
      maxWallMs: readBudgetEnvNumber("MO_TEST_PERF_MAX_WALL_MS", env),
      baselineWallMs: readBudgetEnvNumber("MO_TEST_PERF_BASELINE_WALL_MS", env),
      maxRegressionPct: readBudgetEnvNumber("MO_TEST_PERF_MAX_REGRESSION_PCT", env) ?? 10,
      reportOnly: readBooleanEnv("MO_TEST_PERF_REPORT_ONLY", env),
    },
    [
