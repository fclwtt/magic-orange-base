...
const MANUAL_SECTION_START = "<!-- mo-plugin-reference:manual-start -->";
const MANUAL_SECTION_END = "<!-- mo-plugin-reference:manual-end -->";
...
function resolveDocs({ dirName, manifest, packageJson }) {
  ...
  const channelDoc = normalizeDocPath(packageJson.mo?.channel?.docsPath);
  if (channelDoc) {
...
function resolveInstallRoute(packageJson, status) {
  ...
  if (status === "core") {
    const release = packageJson.mo?.release;
  ...
  const install = packageJson.mo?.install;
  const release = packageJson.mo?.release;
  const clawhubSpec =
...
function resolveStatus({ dirName, packageJson, excludedDirs }) {
  ...
  const hasInstallSpec =
    typeof packageJson.mo?.install?.clawhubSpec === "string" ||
    typeof packageJson.mo?.install?.npmSpec === "string";
...
\`mo.plugin.json\`. Regenerate it with:
...
function collectPluginSourceEntries() {
  ...
  for (const dirName of fs
    ...
    const manifestPath = path.join(EXTENSIONS_DIR, dirName, "mo.plugin.json");
...
This page is generated from \`extensions/*/package.json\`, \`mo.plugin.json\`,
...
- **Core npm package:** built into the \`mo\` npm package and available without a separate plugin install.
...
\`\`\`bash
mo plugins install @mo/discord
mo gateway restart
mo plugins inspect discord --runtime --json
\`\`\`

During the launch cutover, ordinary bare package specs still install from npm.
Use \`clawhub:@mo/discord\` or \`npm:@mo/discord\` when you need an
