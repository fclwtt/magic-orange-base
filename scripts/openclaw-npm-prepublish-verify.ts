...
import { resolveNpmCommandInvocation } from "./mo-npm-release-check.ts";
...
export function openClawNpmPrepublishVerifyUsage(): string {
  return "Usage: node --import tsx scripts/mo-npm-prepublish-verify.ts <tarball.tgz> [expected-version]";
}
...
export function parseOpenClawNpmPrepublishVerifyArgs(
  ...
  if (tarballPath.startsWith("-")) {
    throw new Error(`Unknown mo npm prepublish verifier option: ${tarballPath}`);
  }

  const expectedVersion = args[1]?.trim();
  if (expectedVersion?.startsWith("-")) {
    throw new Error(`Unknown mo npm prepublish verifier option: ${expectedVersion}`);
  }
  const extraArg = args[2]?.trim();
  if (extraArg) {
    throw new Error(`Unexpected mo npm prepublish verifier argument: ${extraArg}`);
  }
...
function main(argv = process.argv.slice(2)): void {
  ...
  const workingDir = mkdtempSync(join(tmpdir(), "mo-prepublish-"));
  ...
  try {
    ...
    if (normalizeInstalledBinaryVersion(installedBinaryVersion) !== resolvedExpectedVersion) {
      errors.push(
        `installed mo binary version mismatch: expected ${resolvedExpectedVersion}, found ${installedBinaryVersion || "<missing>"}.`,
      );
    }
    if (errors.length === 0) {
    ...
    console.log(
      `mo-npm-prepublish-verify: prepared tarball install OK (${resolvedExpectedVersion}).`,
    );
  } finally {
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(`mo-npm-prepublish-verify: ${formatErrorMessage(error)}`);
    process.exitCode = 1;
  }
}
