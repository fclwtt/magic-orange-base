...
function usage() {
  return "Usage: node scripts/check-mo-package-tarball.mjs <mo.tgz>";
}
...
const phaseTimingsEnabled = process.env.MO_PACKAGE_TARBALL_CHECK_TIMINGS !== "0";
function runPhase(label, action) {
  ...
  try {
    ...
    if (phaseTimingsEnabled) {
      ...
      console.error(`check-mo-package-tarball: ${label} completed in ${durationMs}ms`);
    }
  }
}
...
const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-package-tarball-"));
try {
...
if (!entrySet.has("npm-shrinkwrap.json")) {
  ...
  try {
    ...
    if (shrinkwrap.name !== "mo") {
      errors.push("npm-shrinkwrap.json root name must be mo");
    }
    ...
    if (!rootPackage || rootPackage.name !== "mo") {
      errors.push("npm-shrinkwrap.json packages root must name mo");
    }
