...
function parseArgs(argv) {
  const options = {
    jsonPath:
      readNonEmptyEnv("MO_STARTUP_MEMORY_JSON_PATH") ??
      path.join(repoRoot, ".artifacts", "startup-memory", "startup-memory.json"),
    summaryPath:
      readNonEmptyEnv("MO_STARTUP_MEMORY_SUMMARY_PATH") ??
...
const cases = [
  {
    ...
    label: "--help",
    args: ["mo.mjs", "--help"],
    limitMb: readPositiveNumberEnv("MO_STARTUP_MEMORY_HELP_MB", DEFAULT_LIMITS_MB.help),
  },
  {
    id: "statusJson",
    label: "status --json",
    args: ["mo.mjs", "status", "--json"],
    limitMb: readPositiveNumberEnv(
      "MO_STARTUP_MEMORY_STATUS_JSON_MB",
  ...
  },
  {
    ...
    args: ["mo.mjs", "gateway", "status"],
    limitMb: readPositiveNumberEnv(
      "MO_STARTUP_MEMORY_GATEWAY_STATUS_MB",
...
function runStartupMemoryCheck(argv = process.argv.slice(2), params = {}) {
  ...
  tmpHome = mkdtempSync(path.join(os.tmpdir(), "mo-startup-memory-"));
