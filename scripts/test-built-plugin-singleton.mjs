...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mo-build-smoke-"));
...
fs.writeFileSync(
  path.join(distPluginDir, "mo.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
const registry = loadOpenClawPlugins({
  ...
  env: {
    ...process.env,
    MO_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "dist-runtime", "extensions"),
  },
  config: {
    plugins: {
      enabled: true,
