...
const TMUX_CWD_ENV_KEY = "MO_GATEWAY_WATCH_CWD";
const TMUX_CWD_OPTION_KEY = "@mo.gateway_watch.cwd";
const TMUX_CHILD_ENV_KEYS = [
  "NODE_OPTIONS",
  "MO_CONFIG_PATH",
  "MO_DIAGNOSTICS",
  "MO_DIAGNOSTICS_EVENT_LOOP",
  "MO_DIAGNOSTICS_TIMELINE_PATH",
  "MO_GATEWAY_PORT",
  "MO_GATEWAY_RESTART_TRACE",
  "MO_GATEWAY_STARTUP_TRACE",
  "MO_HOME",
  "MO_PROFILE",
...
const resolveGatewayWatchBenchmarkArgs = ({ args = [], env = process.env } = {}) => {
  ...
  if (benchmarkFlagSeen) {
    ...
    nextEnv.MO_TRACE_SYNC_IO ??= "0";
    if (nextEnv.MO_TRACE_SYNC_IO === "1") {
...
export const resolveGatewayWatchTmuxSessionName = ({ args = [], env = process.env } = {}) => {
  ...
  const port = env.MO_GATEWAY_PORT || readArgValue(args, "--port");
  const parts = [
    "mo",
    "gateway",
    "watch",
...
export const buildGatewayWatchTmuxCommand = ({
  ...
  const childEnv = [
    "env",
    ...colorEnv.options,
    `MO_GATEWAY_WATCH_TMUX_CHILD=1`,
    `MO_GATEWAY_WATCH_SESSION=${sessionName}`,
...
const shouldAttachTmux = ({ env, stdinIsTTY, stdoutIsTTY }) => {
  const raw = String(env.MO_GATEWAY_WATCH_ATTACH ?? "").toLowerCase();
...
export const runGatewayWatchTmuxMain = (params = {}) => {
  ...
  if (TMUX_DISABLE_VALUES.has((deps.env.MO_GATEWAY_WATCH_TMUX ?? "").toLowerCase())) {
  ...
  if (deps.env.MO_GATEWAY_WATCH_TMUX_CHILD === "1") {
    return runForegroundWatcher({
      args: deps.args,
      cwd: deps.cwd,
      env: deps.env,
