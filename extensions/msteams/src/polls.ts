...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["moPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["mo", "pollId"]) ??
    readNestedString(value, ["mo", "poll", "id"]) ??
    readNestedString(value, ["data", "moPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "mo", "pollId"]) ??
    // Action.Execute (Universal Action Model) payload shape: value.action.data
    readNestedString(value, ["action", "data", "moPollId"]) ??
