...
} from "mo/plugin-sdk/json-schema-runtime";
import type { RealtimeTranscriptionProviderPlugin } from "mo/plugin-sdk/realtime-transcription";
import type { RealtimeVoiceProviderPlugin } from "mo/plugin-sdk/realtime-voice";
...
vi.mock("mo/plugin-sdk/ssrf-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("mo/plugin-sdk/ssrf-runtime")>();
  return {
    ...actual,
...
describe("google-meet plugin", () => {
  ...
  afterAll(() => {
    vi.doUnmock("mo/plugin-sdk/ssrf-runtime");
  ...
  it("defaults to chrome agent mode with safe read-only tools", () => {
    ...
    expect(config.realtime.instructions).toContain("mo_agent_consult");
  ...
  it("declares advanced config metadata in the plugin entry and manifest", () => {
    const manifest = JSON.parse(
      readFileSync(new URL("./mo.plugin.json", import.meta.url), "utf8"),
    ) as {
  ...
  it("uses env fallbacks for OAuth, preview, and default meeting values", () => {
    const config = resolveGoogleMeetConfigWithEnv(
      {},
      {
        MO_GOOGLE_MEET_CLIENT_ID: "client-id",
        GOOGLE_MEET_CLIENT_SECRET: "client-secret",
        MO_GOOGLE_MEET_REFRESH_TOKEN: "refresh-token",
        GOOGLE_MEET_ACCESS_TOKEN: "access-token",
        MO_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT: "123456",
        GOOGLE_MEET_DEFAULT_MEETING: "https://meet.google.com/abc-defg-hij",
        MO_GOOGLE_MEET_PREVIEW_ACK: "true",
      },
    );
  ...
  it.each(["0x10", "1e3"])("ignores non-decimal env numeric fallbacks: %s", (expiresAt) => {
    const config = resolveGoogleMeetConfigWithEnv(
      {},
      {
        MO_GOOGLE_MEET_ACCESS_TOKEN: "access-token",
        MO_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT: expiresAt,
      },
    );
  ...
  it("reports setup status through the tool", async () => {
    ...
    try {
      const { tools } = setup({
        chrome: {
          audioInputCommand: ["mo-audio-bridge", "capture"],
          audioOutputCommand: ["mo-audio-bridge", "play"],
        },
      });
      const tool = tools[0] as {
  ...
  it("writes export bundles through the tool", async () => {
    stubMeetArtifactsApi();
    const tempDir = mkdtempSync(path.join(tmpdir(), "mo-google-meet-tool-export-"));
    const { tools } = setup();
    const tool = tools[0] as {
      execute: (
        id: string,
  ...
  it("dry-runs export bundles through the tool", async () => {
    stubMeetArtifactsApi();
    const parentDir = mkdtempSync(path.join(tmpdir(), "mo-google-meet-tool-dry-run-"));
  ...
  it("retries caption enable until the captions button is available", async () => {
    ...
    const captionsStateKey = "__moMeetCaptions";
  ...
  it("explains when chrome-node has no capable paired node", async () => {
    ...
    expect(result.details.error).toContain("mo node run");
  });
  ...
  it("pipes Chrome command-pair audio through the realtime provider", async () => {
    ...
    expect(progressPayload.tool).toBe("mo_agent_consult");
    ...
    expect(callbacks.tools?.map((tool) => tool.name)).toContain("mo_agent_consult");
    await vi.waitFor(() => {
