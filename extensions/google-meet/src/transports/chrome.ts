...
import { callGatewayFromCli } from "mo/plugin-sdk/gateway-runtime";
...
import type { RuntimeLogger } from "mo/plugin-sdk/plugin-runtime";
...
function meetStatusScript(params: {
  ...
  const captionState = (() => {
    ...
    const w = window;
    if (!inCall && !w.__moMeetCaptions) return undefined;
    if (!w.__moMeetCaptions) {
      w.__moMeetCaptions = {
