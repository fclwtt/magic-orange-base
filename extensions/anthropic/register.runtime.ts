 ...
 */
import { formatCliCommand, parseDurationMs } from "mo/plugin-sdk/cli-runtime";
import { resolveExpiresAtMsFromDurationMs } from "mo/plugin-sdk/number-runtime";
import type {
  OpenClawPluginApi,
  ProviderAuthContext,
...
} from "mo/plugin-sdk/provider-model-shared";
import { fetchClaudeUsage } from "mo/plugin-sdk/provider-usage";
...
const ANTHROPIC_SETUP_TOKEN_NOTE_LINES = [
  ...
  `If you want a direct API billing path instead, use ${formatCliCommand("mo models auth login --provider anthropic --method api-key --set-default")} or ${formatCliCommand("mo models auth login --provider anthropic --method cli --set-default")}.`,
] as const;
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("mo doctor --yes")}"`,
  ].join("\n");
}
