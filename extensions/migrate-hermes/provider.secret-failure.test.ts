...
import { resolveAuthStorePathForDisplay } from "mo/plugin-sdk/agent-runtime";
...
vi.mock("mo/plugin-sdk/provider-auth", async (importOriginal) => ({
  ...(await importOriginal<typeof import("mo/plugin-sdk/provider-auth")>()),
...
async function makeTempRoot() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "mo-hermes-secret-failure-"));
  tempRoots.add(root);
  return root;
}
