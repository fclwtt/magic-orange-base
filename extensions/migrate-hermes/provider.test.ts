...
import { createCapturedPluginRegistration } from "mo/plugin-sdk/plugin-test-runtime";
...
describe("Hermes migration provider", () => {
  ...
  it("resolves tilde source paths against the OS home when MO_HOME is set", () => {
    const previous = process.env.MO_HOME;
    process.env.MO_HOME = path.join(path.sep, "tmp", "mo-home");
    try {
