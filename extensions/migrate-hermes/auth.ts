...
import type { MigrationItem, MigrationProviderContext } from "mo/plugin-sdk/plugin-entry";
import {
  ...
  if (await hasLegacyHermesAuthJson(params.source.authPath)) {
    items.push(
      createMigrationManualItem({
        ...
        recommendation:
          "Run mo models auth login --provider openai after migration, or run mo doctor --fix for existing OpenClaw legacy auth state.",
      }),
    );
  }
