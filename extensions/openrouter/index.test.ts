...
import { createAssistantMessageEventStream } from "mo/plugin-sdk/llm";
import {
...
} from "mo/plugin-sdk/provider-test-contracts";
...
vi.mock("mo/plugin-sdk/provider-stream-family", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("mo/plugin-sdk/provider-stream-family")>();
  return {
    ...actual,
...
function readManifest(): OpenRouterManifest {
  return JSON.parse(readFileSync(new URL("./mo.plugin.json", import.meta.url), "utf8"));
}
...
describe("openrouter provider hooks", () => {
  ...
  it("injects provider routing into compat before applying stream wrappers", async () => {
    ...
    const baseStreamFn = vi.fn(
      (
        ...args: Parameters<import("mo/plugin-sdk/agent-core").StreamFn>
      ): ReturnType<import("mo/plugin-sdk/agent-core").StreamFn> => {
  ...
  it("forwards resolved API keys as explicit OpenRouter auth headers", async () => {
    ...
    expect(headers.get("http-referer")).toBe("https://mo.ai");
