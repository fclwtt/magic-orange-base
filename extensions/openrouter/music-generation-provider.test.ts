...
import { expectExplicitMusicGenerationCapabilities } from "mo/plugin-sdk/provider-test-contracts";
...
vi.mock("mo/plugin-sdk/provider-http", async (importOriginal) => {
  const original = await importOriginal<typeof import("mo/plugin-sdk/provider-http")>();
  return {
    ...original,
