...
describe("browser doctor readiness", () => {
  ...
  it("warns when managed browser profiles have no local executable", async () => {
    ...
    expect(noteFn).toHaveBeenCalledWith(
      [
        "- OpenClaw-managed browser profile(s) are configured: mo.",
  ...
  it("warns about legacy clawd managed browser profile residue", async () => {
    ...
    expect(note).toContain("/tmp/mo-home/browser/clawd");
    expect(note).toContain("/tmp/mo-home/browser/mo/user-data");
    expect(note).toContain("mo doctor --fix");
  });
...
describe("legacy clawd browser profile cleanup", () => {
  it("archives stale clawd residue with the safe trash mover", async () => {
    const movePathToTrash = vi.fn(async () => "/tmp/mo-home/browser/.trash/clawd");
    ...
    expect(movePathToTrash).toHaveBeenCalledWith("/tmp/mo-home/browser/clawd");
    expect(result.warnings).toStrictEqual([]);
    expect(result.changes.join("\n")).toContain(
      "Archived legacy clawd managed browser profile residue.",
    );
    expect(result.changes.join("\n")).toContain("/tmp/mo-home/browser/mo/user-data");
  });
