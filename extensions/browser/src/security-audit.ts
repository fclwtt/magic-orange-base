 ...
 */
import type { OpenClawPluginSecurityAuditContext } from "mo/plugin-sdk/plugin-entry";
import { hasConfiguredSecretInput } from "mo/plugin-sdk/secret-input";
import { formatCliCommand } from "mo/plugin-sdk/setup-tools";
import { isPrivateNetworkOptInEnabled, isPrivateIpAddress } from "mo/plugin-sdk/ssrf-policy";
...
export function collectBrowserSecurityAuditFindings(ctx: OpenClawPluginSecurityAuditContext) {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${ctx.configPath} and re-run "${formatCliCommand("mo security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.MO_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.MO_GATEWAY_PASSWORD) ||
