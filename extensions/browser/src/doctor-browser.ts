...
import { DEFAULT_MO_BROWSER_PROFILE_NAME, resolveBrowserConfig } from "./browser/config.js";
...
function collectManagedProfiles(cfg: OpenClawConfig): ManagedProfile[] {
  ...
  for (const [profileName, rawProfile] of Object.entries(configuredProfiles)) {
    const profile = asRecord(rawProfile);
    const driver = normalizeOptionalString(profile?.driver) ?? "mo";
    if (driver !== "existing-session") {
...
export function detectLegacyClawdBrowserProfileResidue(
  ...
  if (
    resolved.defaultProfile !== DEFAULT_MO_BROWSER_PROFILE_NAME ||
...
function formatLegacyClawdBrowserProfileResidueNote(
  ...
  return [
    ...
    `- If no browser is using the legacy profile, run ${formatCliCommand("mo doctor --fix")} to archive it safely instead of deleting it in place.`,
  ].join("\n");
}
