 ...
 */
import { parseBrowserHttpUrl } from "mo/plugin-sdk/browser-config";
import { resolveTimerTimeoutMs } from "mo/plugin-sdk/number-runtime";
import { readResponseWithLimit } from "mo/plugin-sdk/response-limit-runtime";
...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-mo-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-mo-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-mo-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function resolveBrowserFetchOperatorHint(
  ...
  return isLocal
    ? `Restart the OpenClaw gateway (OpenClaw.app menubar, or \`${formatCliCommand("mo gateway")}\`).`
