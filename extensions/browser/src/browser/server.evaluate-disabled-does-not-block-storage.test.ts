...
vi.mock("../config/config.js", async () => {
  ...
  const loadConfig = () => ({
    browser: {
      ...
      defaultProfile: "mo",
      profiles: {
        mo: { cdpPort: testPort + 1, color: "#FF4500" },
      },
    },
  });
  return {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.MO_GATEWAY_PORT;
    process.env.MO_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.MO_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.MO_GATEWAY_PASSWORD;
