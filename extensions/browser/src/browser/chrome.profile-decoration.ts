...
import { loadJsonFile, saveJsonFile } from "mo/plugin-sdk/json-store";
import {
...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".mo-profile-decorated");
}
...
export function decorateOpenClawProfile(
  ...
  const desiredName = opts?.name ?? DEFAULT_MO_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_MO_BROWSER_COLOR).toUpperCase();
