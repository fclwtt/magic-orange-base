...
import { clearRuntimeConfigSnapshot } from "mo/plugin-sdk/runtime-config-snapshot";
...
describe("browser client fetch attachOnly diagnostics", () => {
  ...
  it("does not suggest gateway restart when an attachOnly CDP endpoint hangs", async () => {
    tempHome = await createTempHomeEnv("mo-browser-client-fetch-live-");
    ...
    const configPath = path.join(tempHome.home, ".mo", "mo.json");
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
