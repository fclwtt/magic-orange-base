...
function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.MO_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "mo",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/mo" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    mo: { cdpPort: defaultBrowserCdpPortForState(testPort), color: "#FF4500" },
  };
}
...
vi.mock("../config/config.js", async () => {
  ...
  const mutateConfigFile = vi.fn(
    async (params: {
      ...
      const result = await params.mutate(draft, { snapshot: { path: "/tmp/mo.json" } });
      await writeConfigFile(draft);
      return {
        path: "/tmp/mo.json",
        previousHash: "test-hash",
        persistedHash: "test-hash",
        snapshot: { path: "/tmp/mo.json" },
        nextConfig: draft,
        result,
        attempts: 1,
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.MO_GATEWAY_PORT;
  process.env.MO_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.MO_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.MO_GATEWAY_PASSWORD;
...
function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.MO_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.MO_GATEWAY_PASSWORD;
  } else {
    process.env.MO_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
