...
describe("browser client", () => {
  ...
  it("uses the expected endpoints + methods for common calls", async () => {
    ...
    expect(doctorResult.profile).toBe("mo");
    ...
    expect(deepDoctorResult.profile).toBe("mo");
    ...
    expect(urls.some((url) => url.endsWith("/doctor?profile=mo&deep=true"))).toBe(true);
    ...
    const deepDoctor = calls.find((c) => c.url.endsWith("/doctor?profile=mo&deep=true"));
