...
import { finiteSecondsToTimerSafeMilliseconds } from "mo/plugin-sdk/number-runtime";
import type {
...
const ENABLE_STARTUP_PROBE_ENV = "MO_ACPX_RUNTIME_STARTUP_PROBE";
const SKIP_RUNTIME_PROBE_ENV = "MO_SKIP_ACPX_RUNTIME_PROBE";
  ...
  async function resolveRuntime(): Promise<AcpxRuntimeLike> {
    ...
    runtimePromise ??= loadRuntimeModule().then((module) => {
      runtime = new module.AcpxRuntime({
        ...
        moGatewayInstanceId: params.gatewayInstanceId,
        moProcessLeaseStore: params.processLeaseStore,
        moWrapperRoot: params.wrapperRoot,
        ...
        moToolsMcpBridgeEnabled: params.pluginConfig.openClawToolsMcpBridge,
...
export function createAcpxRuntimeService(
  ...
  return {
    ...
    async start(ctx: OpenClawPluginServiceContext): Promise<void> {
      if (process.env.MO_SKIP_ACPX_RUNTIME === "1") {
        ctx.logger.info("skipping embedded acpx runtime backend (MO_SKIP_ACPX_RUNTIME=1)");
        return;
      }
