...
describe("embedded acpx plugin config", () => {
  it("resolves workspace stateDir and cwd by default", () => {
    const workspaceDir = path.resolve("/tmp/mo-acpx");
  ...
  it("injects the built-in plugin-tools MCP server only when explicitly enabled", () => {
    ...
    const server = resolved.mcpServers["mo-plugin-tools"];
  ...
  it("injects the built-in OpenClaw tools MCP server only when explicitly enabled", () => {
    ...
    const server = resolved.mcpServers["mo-tools"];
    expect(server).toEqual({
      command: process.execPath,
      args: expectedMcpServerArgs({
        sourceEntry: "src/mcp/mo-tools-serve.ts",
        distEntry: "dist/mcp/mo-tools-serve.js",
      }),
    });
  });
  ...
  it("keeps the runtime json schema in sync with the manifest config schema", () => {
    ...
    const manifest = JSON.parse(
      fs.readFileSync(path.join(pluginRoot, "mo.plugin.json"), "utf8"),
