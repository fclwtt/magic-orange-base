...
import { parseStrictPositiveInteger } from "mo/plugin-sdk/number-runtime";
import { redactSensitiveText } from "mo/plugin-sdk/security-runtime";
import { normalizeStringEntries } from "mo/plugin-sdk/string-coerce-runtime";
...
type OpenClawAcpxRuntimeOptions = AcpRuntimeOptions & {
  moWrapperRoot?: string;
  moGatewayInstanceId?: string;
  moProcessLeaseStore?: AcpxProcessLeaseStore;
  moToolsMcpBridgeEnabled?: boolean;
};
type AcpxRuntimeTestOptions = Record<string, unknown> & {
  moProcessCleanup?: AcpxProcessCleanupDeps;
};
...
const ACPX_MO_TOOLS_MCP_SERVER_NAME = "mo-tools";
const MO_TOOLS_MCP_AGENT_SESSION_KEY_ENV = "MO_TOOLS_MCP_AGENT_SESSION_KEY";
...
function withOpenClawLeaseSessionMetadata<T extends object>(
  ...
  return {
    ...record,
    moLeaseId: metadata.moLeaseId,
    moGatewayInstanceId: metadata.moGatewayInstanceId,
  };
}
...
function readOpenClawLeaseIdFromRecord(record: unknown): string | undefined {
  ...
  const { moLeaseId } = record as { moLeaseId?: unknown };
  return typeof moLeaseId === "string" ? moLeaseId.trim() || undefined : undefined;
}
...
function readOpenClawGatewayInstanceIdFromRecord(record: unknown): string | undefined {
  ...
  const { moGatewayInstanceId } = record as { moGatewayInstanceId?: unknown };
  return typeof moGatewayInstanceId === "string"
    ? moGatewayInstanceId.trim() || undefined
    : undefined;
}
  ...
  return {
    async load(sessionId: string): Promise<AcpLoadedSessionRecord> {
      ...
      return withOpenClawLeaseSessionMetadata(record, {
        moLeaseId: lease.leaseId,
        moGatewayInstanceId: lease.gatewayInstanceId,
      });
    },
    async save(record: AcpSessionRecord): Promise<void> {
      ...
      if (
        ...
        recordToSave = withOpenClawLeaseSessionMetadata(
          ...
          {
            moLeaseId: launch.leaseId,
            moGatewayInstanceId: launch.gatewayInstanceId,
          },
        );
      }
...
const MO_BRIDGE_EXECUTABLE = "mo";
const MO_BRIDGE_SUBCOMMAND = "acp";
const CODEX_ACP_AGENT_ID = "codex";
const CODEX_ACP_MO_PREFIX = "openai/";
const CLAUDE_ACP_MO_PREFIX = "anthropic/";
...
function isOpenClawBridgeCommand(command: string | undefined): boolean {
  ...
  const parts = unwrapEnvCommand(splitCommandParts(command.trim()));
  if (basename(parts[0] ?? "") === MO_BRIDGE_EXECUTABLE) {
    return parts[1] === MO_BRIDGE_SUBCOMMAND;
  }
  if (basename(parts[0] ?? "") !== "node") {
    return false;
  }
  const scriptName = basename(parts[1] ?? "");
  return /^mo(?:\.[cm]?js)?$/i.test(scriptName) && parts[2] === MO_BRIDGE_SUBCOMMAND;
}
...
function normalizeCodexAcpModelOverride(
  ...
  let value = raw;
  if (value.toLowerCase().startsWith(CODEX_ACP_MO_PREFIX)) {
    value = value.slice(CODEX_ACP_MO_PREFIX.length);
  }
...
function normalizeClaudeAcpModelOverride(rawModel: string | undefined): string | undefined {
  ...
  if (!raw.toLowerCase().startsWith(CLAUDE_ACP_MO_PREFIX)) {
    return raw;
  }
  return raw.slice(CLAUDE_ACP_MO_PREFIX.length).trim() || undefined;
}
...
function withOpenClawToolsMcpSessionEnv(params: {
  ...
  const nextServers = params.mcpServers.map((server): AcpxMcpServer => {
    if (server.name !== ACPX_MO_TOOLS_MCP_SERVER_NAME || !("command" in server)) {
      return server;
    }
    changed = true;
    const env = [
      ...server.env.filter((entry) => entry.name !== MO_TOOLS_MCP_AGENT_SESSION_KEY_ENV),
      {
        name: MO_TOOLS_MCP_AGENT_SESSION_KEY_ENV,
        value: sessionKey,
      },
    ];
    return { ...server, env };
...
export class AcpxRuntime implements AcpRuntime {
  ...
  private readonly moToolsMcpBridgeEnabled: boolean;
  private readonly moToolsSessionDelegates = new Map<string, BaseAcpxRuntime>();
  ...
  constructor(options: OpenClawAcpxRuntimeOptions, testOptions?: AcpxRuntimeTestOptions) {
    const { moProcessCleanup, ...delegateTestOptions } = testOptions ?? {};
    this.processCleanupDeps = moProcessCleanup;
    this.wrapperRoot = options.moWrapperRoot;
    this.gatewayInstanceId = options.moGatewayInstanceId;
    this.processLeaseStore = options.moProcessLeaseStore;
    this.moToolsMcpBridgeEnabled = options.moToolsMcpBridgeEnabled === true;
    ...
    this.bridgeSafeDelegate = shouldUseDistinctBridgeDelegate(options)
      ...
      : this.delegate;
    this.probeDelegate = this.moToolsMcpBridgeEnabled
  ...
  private resolveOpenClawToolsDelegateForSession(sessionKey: string): BaseAcpxRuntime {
    if (!this.moToolsMcpBridgeEnabled) {
    ...
    const cached = this.moToolsSessionDelegates.get(normalizedSessionKey);
    if (cached) {
      return cached;
    }
    ...
    const delegate = new BaseAcpxRuntime(
      {
        ...
        mcpServers: withOpenClawToolsMcpSessionEnv({
          enabled: this.moToolsMcpBridgeEnabled,
    ...
    this.moToolsSessionDelegates.set(normalizedSessionKey, delegate);
    return delegate;
  }
  ...
  private releaseOpenClawToolsDelegateForSession(sessionKey: string): void {
    ...
    this.moToolsSessionDelegates.delete(normalizedSessionKey);
  }
