...
import { formatPluginConfigIssue } from "mo/plugin-sdk/extension-shared";
...
const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "mo-plugin-tools";
const ACPX_MO_TOOLS_MCP_SERVER_NAME = "mo-tools";
const requireFromHere = createRequire(import.meta.url);

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "mo.plugin.json")) &&
...
function resolveOpenClawToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  const pluginRoot = resolveAcpxPluginRoot(moduleUrl);
  const openClawRoot = resolveOpenClawRoot(pluginRoot);
  const distEntry = path.join(openClawRoot, "dist", "mcp", "mo-tools-serve.js");
  if (fs.existsSync(distEntry)) {
    return {
      command: process.execPath,
      args: [distEntry],
  ...
  const sourceEntry = path.join(openClawRoot, "src", "mcp", "mo-tools-serve.ts");
  return {
    command: process.execPath,
    args: ["--import", resolveTsxImportSpecifier(), sourceEntry],
  };
...
function resolveConfiguredMcpServers(params: {
  ...
  if (params.openClawToolsMcpBridge && resolved[ACPX_MO_TOOLS_MCP_SERVER_NAME]) {
    throw new Error(
      `mcpServers.${ACPX_MO_TOOLS_MCP_SERVER_NAME} is reserved when openClawToolsMcpBridge=true`,
    );
  }
  ...
  if (params.openClawToolsMcpBridge) {
    resolved[ACPX_MO_TOOLS_MCP_SERVER_NAME] = resolveOpenClawToolsMcpServerConfig(
      params.moduleUrl,
    );
  }
  return resolved;
