...
const MO_PLUGIN_DEPS_MARKER = "/plugin-runtime-deps/";
...
export type AcpxProcessCleanupResult = {
  ...
  skippedReason?: "missing-root" | "not-mo-owned" | "unverified-root";
};
...
function liveCommandMatchesLeaseIdentity(params: {
  ...
  return (
    commandOptionEquals(parts, MO_ACPX_LEASE_ID_ARG, params.expectedLeaseId) &&
    commandOptionEquals(parts, MO_GATEWAY_INSTANCE_ID_ARG, params.expectedGatewayInstanceId)
  );
}
...
export function isOpenClawOwnedAcpxProcessCommand(params: {
  ...
  if (!normalized.includes(MO_PLUGIN_DEPS_MARKER)) {
    return false;
  }
