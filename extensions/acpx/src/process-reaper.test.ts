...
const WRAPPER_ROOT = "/tmp/mo-state/acpx";
const CODEX_WRAPPER_COMMAND = `node ${WRAPPER_ROOT}/codex-acp-wrapper.mjs`;
const CODEX_WRAPPER_COMMAND_WITH_LEASE = `${CODEX_WRAPPER_COMMAND} ${MO_ACPX_LEASE_ID_ARG} lease-1 ${MO_GATEWAY_INSTANCE_ID_ARG} gateway-1`;
...
describe("process reaper", () => {
    ...
    const { deps, killed } = cleanupDeps([
      {
        pid: 113,
        ppid: 1,
        command: `${CODEX_WRAPPER_COMMAND} ${MO_ACPX_LEASE_ID_ARG} other-lease ${MO_GATEWAY_INSTANCE_ID_ARG} gateway-1`,
      },
    ]);
  ...
  it("skips non-owned recorded process trees", async () => {
    ...
    expect(result.skippedReason).toBe("not-mo-owned");
    expect(killed).toStrictEqual([]);
  });
