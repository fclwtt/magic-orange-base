...
import { MO_ACPX_LEASE_ID_ARG, MO_GATEWAY_INSTANCE_ID_ARG } from "./process-lease.js";
...
const DOCUMENTED_MO_BRIDGE_COMMAND =
  "env MO_HIDE_BANNER=1 MO_SUPPRESS_NOTES=1 mo acp --url ws://127.0.0.1:18789 --token-file ~/.mo/gateway.token --session agent:main:main";
const CODEX_ACP_COMMAND = "npx @zed-industries/codex-acp@0.13.0";
const CODEX_ACP_WRAPPER_COMMAND = `node "/tmp/mo/acpx/codex-acp-wrapper.mjs"`;
const CODEX_ACP_WRAPPER_COMMAND_WITH_LEASE = `${CODEX_ACP_WRAPPER_COMMAND} ${MO_ACPX_LEASE_ID_ARG} lease-close ${MO_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
  ...
  const runtime = new AcpxRuntime(
    {
      ...
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "mo" ? "mo acp" : agentName),
        list: () => ["codex", "mo"],
      },
...
describe("AcpxRuntime fresh reset wrapper", () => {
  ...
  it("adds the OpenClaw session key to the managed OpenClaw tools MCP bridge", () => {
    ...
    const { runtime } = makeRuntime(baseStore, {
      moToolsMcpBridgeEnabled: true,
      mcpServers: [
        {
          name: "mo-tools",
          command: "node",
          args: ["dist/mcp/mo-tools-serve.js"],
          env: [],
        },
      ],
    });
    ...
    const readScopedMcpEnv = (sessionKey: string) => {
      ...
      return delegate.options.mcpServers?.find((server) => server.name === "mo-tools")?.env;
    };

    expect(readScopedMcpEnv("agent:worker:main")).toContainEqual({
      name: "MO_TOOLS_MCP_AGENT_SESSION_KEY",
  ...
  it("keeps managed OpenClaw tools MCP delegates reachable for fresh sessions", async () => {
    ...
    const exposedRuntime = runtime as unknown as {
      moToolsSessionDelegates: Map<string, unknown>;
      resolveOpenClawToolsDelegateForSession(sessionKey: string): unknown;
    };

    const firstDelegate =
      exposedRuntime.resolveOpenClawToolsDelegateForSession("agent:worker:main");
    expect(exposedRuntime.moToolsSessionDelegates.has("agent:worker:main")).toBe(true);
  ...
  it("adds Codex wrapper stderr tail to generic session initialization failures", async () => {
    const wrapperRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mo-acpx-runtime-"));
    ...
    const { runtime, delegate } = makeRuntime(baseStore, {
      moGatewayInstanceId: "gateway-test",
      moProcessLeaseStore: leaseStore.store,
      moWrapperRoot: wrapperRoot,
      agentRegistry: {
  ...
  it("adds Codex wrapper stderr tail when startTurn creation throws", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
        agentCommand: CODEX_ACP_WRAPPER_COMMAND,
        moLeaseId: "lease-start-turn-create",
      })),
      save: vi.fn(async () => {}),
    };
    const { runtime, delegate } = makeRuntime(baseStore, {
  ...
  it("injects Codex ACP startup config into the scoped registry", () => {
    ...
    expect(testing.isCodexAcpCommand("mo acp")).toBe(false);
  });
  ...
  it("recognizes claude-agent-acp commands", () => {
    ...
    expect(
      testing.isClaudeAcpCommand(`node "/tmp/mo/acpx/claude-agent-acp-wrapper.mjs"`),
    ).toBe(true);
    expect(
      testing.isClaudeAcpCommand(
        `node.exe "C:/Users/runner/AppData/Local/Temp/mo/acpx/claude-agent-acp-wrapper.mjs"`,
      ),
    ).toBe(true);
    expect(
      testing.isClaudeAcpCommand(
    ...
    expect(testing.isClaudeAcpCommand("mo acp")).toBe(false);
  ...
  it("releases managed OpenClaw tools MCP delegates after close", async () => {
    ...
    const exposedRuntime = runtime as unknown as {
      moToolsSessionDelegates: Map<string, { close: AcpRuntime["close"] }>;
    ...
    expect(exposedRuntime.moToolsSessionDelegates.has("agent:codex:main")).toBe(false);
  });
  ...
  it("cleans up OpenClaw-owned ACPX process trees after close", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        ...
        agentCommand: 'node "/tmp/mo/acpx/codex-acp-wrapper.mjs"',
        pid: 900,
      })),
      save: vi.fn(async () => {}),
    };
        ...
        moProcessCleanup: {
          listProcesses: vi.fn(async () => [
            ...
            },
            {
              ...
              ppid: 900,
              command:
                "node /tmp/mo/plugin-runtime-deps/node_modules/@zed-industries/codex-acp/bin/codex-acp.js",
            },
          ]),
  ...
  it("records ACPX process leases without persisting lease-specific agent commands", async () => {
    ...
    expect(lease?.wrapperPath).toBe("/tmp/mo/acpx/codex-acp-wrapper.mjs");
    expect(launchCommands[0]).toContain("MO_ACPX_LEASE_ID=");
    expect(launchCommands[0]).toContain("MO_GATEWAY_INSTANCE_ID=gateway-test");
    expect(savedRecords[0]?.agentCommand).toBe(CODEX_ACP_WRAPPER_COMMAND);
    expect(savedRecords[0]?.moGatewayInstanceId).toBe("gateway-test");
    expect(savedRecords[0]?.moLeaseId).toBe(lease?.leaseId);
  });
  ...
  it("merges sidecar lease ids into loaded ACPX session records", async () => {
    ...
    expect(loadedRecord?.moGatewayInstanceId).toBe("gateway-test");
    expect(loadedRecord?.moLeaseId).toBe("lease-loaded");
  });
  ...
  it("merges the lease for the current ACPX session process when old leases exist", async () => {
    ...
    const loadedRecord = await wrappedStore.load("agent:codex:acp:binding:test");
    expect(loadedRecord?.moGatewayInstanceId).toBe("gateway-test");
    expect(loadedRecord?.moLeaseId).toBe("lease-current");
  });
          ...
          listProcesses: vi.fn(async () => [
            {
              pid: 930,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${MO_ACPX_LEASE_ID_ARG} lease-old ${MO_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
            {
              pid: 940,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${MO_ACPX_LEASE_ID_ARG} lease-current ${MO_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
          ...
          listProcesses: vi.fn(async () => [
            {
              pid: 920,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${MO_ACPX_LEASE_ID_ARG} other-lease ${MO_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
          ]),
          killProcess: vi.fn((pid, signal) => {
            killed.push({ pid, signal });
  ...
  it("routes mo ensureSession through the bridge-safe delegate when MCP servers are configured", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("routes non-mo sessions through the default delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("routes handle-based follow-up calls for mo sessions through the bridge-safe delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const handle: Parameters<NonNullable<AcpRuntime["getStatus"]>>[0]["handle"] = {
      sessionKey: "agent:mo:acp:test",
      backend: "acpx",
      runtimeSessionName: "mo-session-handle",
    };
  ...
  it("keeps MCP-enabled routing when the mo agent is overridden to a non-bridge adapter", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "mo" ? "codex" : agentName),
        list: () => ["codex", "mo"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for any agent mapped to the mo bridge command", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "codex" ? "mo acp" : agentName),
        list: () => ["codex", "mo"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for documented env-wrapped mo bridge commands", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "mo" ? DOCUMENTED_MO_BRIDGE_COMMAND : agentName,
        list: () => ["codex", "mo"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for local node mo entrypoints", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "mo" ? "env MO_HIDE_BANNER=1 node mo.mjs acp" : agentName,
        list: () => ["codex", "mo"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("routes follow-up calls by persisted agent command before current config", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:mo:acp:test",
        agentCommand: DOCUMENTED_MO_BRIDGE_COMMAND,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const status = await runtime.getStatus({
      handle: {
        sessionKey: "agent:mo:acp:test",
        backend: "acpx",
        runtimeSessionName: "agent:mo:acp:test",
      },
    });
  ...
  it("probes through the bridge-safe delegate when probeAgent resolves to mo bridge", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
