...
import { readJsonFileWithFallback } from "mo/plugin-sdk/json-store";
import {
...
const RUN_CONFIGURED_COMMAND_SENTINEL = "--mo-run-configured";
...
function readManifestDependencyVersion(packageName: string): string {
  ...
  if (typeof version !== "string" || version.trim() === "") {
    throw new Error(`Missing ${packageName} dependency version in @mo/acpx manifest`);
  }
  return version;
}
...
const openClawWrapperArgs = new Set([
  ${quoteCommandPart(MO_ACPX_LEASE_ID_ARG)},
  ${quoteCommandPart(MO_GATEWAY_INSTANCE_ID_ARG)},
]);
...
function resolveStderrLogPath(args) {
  ...
  const leaseId =
    process.env[${JSON.stringify(MO_ACPX_LEASE_ID_ENV)}] ||
    readOpenClawWrapperArg(args, ${quoteCommandPart(MO_ACPX_LEASE_ID_ARG)}) ||
    "pid-" + process.pid;
...
if (!command) {
  console.error("[mo] missing configured ${params.displayName} ACP command");
  process.exit(1);
}
...
child.on("error", (error) => {
  console.error(\`[mo] failed to launch ${params.displayName} ACP wrapper: \${error.message}\`);
  process.exit(1);
});
