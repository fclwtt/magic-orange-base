...
import type { OpenKeyedStoreOptions } from "mo/plugin-sdk/plugin-state-runtime";
import {
...
const previousEnv = {
  MO_ACPX_RUNTIME_STARTUP_PROBE: process.env.MO_ACPX_RUNTIME_STARTUP_PROBE,
  MO_SKIP_ACPX_RUNTIME: process.env.MO_SKIP_ACPX_RUNTIME,
  MO_SKIP_ACPX_RUNTIME_PROBE: process.env.MO_SKIP_ACPX_RUNTIME_PROBE,
};
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-acpx-service-"));
  tempDirs.push(dir);
  return dir;
}
...
afterEach(async () => {
  ...
  restoreEnv("MO_ACPX_RUNTIME_STARTUP_PROBE");
  restoreEnv("MO_SKIP_ACPX_RUNTIME");
  restoreEnv("MO_SKIP_ACPX_RUNTIME_PROBE");
...
function createServiceContext(workspaceDir: string): OpenClawPluginServiceContext {
  return {
    workspaceDir,
    stateDir: path.join(workspaceDir, ".mo-plugin-state"),
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
...
function createOpenKeyedStore(ctx: OpenClawPluginServiceContext) {
  const env = { ...process.env, MO_STATE_DIR: ctx.stateDir };
...
describe("createAcpxRuntimeService", () => {
  ...
  it("skips the startup probe and does not advertise backend health when explicitly disabled", async () => {
    process.env.MO_ACPX_RUNTIME_STARTUP_PROBE = "0";
    delete process.env.MO_SKIP_ACPX_RUNTIME_PROBE;
    const workspaceDir = await makeTempDir();
  ...
  it("waits for the embedded runtime startup probe before resolving by default", async () => {
    delete process.env.MO_ACPX_RUNTIME_STARTUP_PROBE;
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
  ...
  it("runs the embedded runtime probe at startup when explicitly enabled and reports health", async () => {
    process.env.MO_ACPX_RUNTIME_STARTUP_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("lets the skip env override the opt-in embedded runtime startup probe without advertising health", async () => {
    process.env.MO_ACPX_RUNTIME_STARTUP_PROBE = "1";
    process.env.MO_SKIP_ACPX_RUNTIME_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("can skip the embedded runtime backend via env", async () => {
    process.env.MO_SKIP_ACPX_RUNTIME = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith(
      "skipping embedded acpx runtime backend (MO_SKIP_ACPX_RUNTIME=1)",
    );
  });
});
