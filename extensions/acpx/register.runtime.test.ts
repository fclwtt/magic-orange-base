...
vi.mock("mo/plugin-sdk/acp-runtime-backend", () => ({
...
const previousSkipRuntime = process.env.MO_SKIP_ACPX_RUNTIME;

function restoreEnv(): void {
  if (previousSkipRuntime === undefined) {
    delete process.env.MO_SKIP_ACPX_RUNTIME;
  } else {
    process.env.MO_SKIP_ACPX_RUNTIME = previousSkipRuntime;
  }
}

function createServiceContext() {
  return {
    workspaceDir: "/tmp/mo-acpx-register-test",
    stateDir: "/tmp/mo-acpx-register-test/state",
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
