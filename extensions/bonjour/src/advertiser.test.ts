...
function enableAdvertiserUnitMode(hostname = "test-host") {
  ...
  process.env.MO_MDNS_HOSTNAME = hostname;
}
...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    const started = await startAdvertiser({
      ...
      cliPath: "/opt/homebrew/bin/mo",
      minimal: false,
    });

    expect(createService).toHaveBeenCalledTimes(1);
    const [gatewayCall] = createService.mock.calls as Array<[Record<string, unknown>]>;
    expect(gatewayCall?.[0]?.type).toBe("mo-gw");
  ...
  it("honors truthy MO_DISABLE_BONJOUR values", async () => {
    enableAdvertiserUnitMode();
    process.env.MO_DISABLE_BONJOUR = "true";
  ...
  it("auto-disables Bonjour on Fly Machines without Docker sentinel files", async () => {
    ...
    process.env.FLY_APP_NAME = "mo-clawcks-test";
  ...
  it("honors explicit Bonjour opt-in inside detected containers", async () => {
    enableAdvertiserUnitMode();
    process.env.MO_DISABLE_BONJOUR = "0";
    vi.spyOn(fs, "existsSync").mockImplementation((filePath) => String(filePath) === "/.dockerenv");
  ...
  it("logs ciao handler classifications at the bonjour caller", async () => {
    ...
    expect(
      handler?.(
        new Error(
          "Can't probe for a service which is announced already. Received announcing for service OpenClaw Gateway._mo._tcp.local.",
        ),
      ),
    ).toBe(true);
  ...
  it("suppresses ciao self-probe retry console noise while advertising", async () => {
    ...
    try {
      ...
      console.log(
        "[test._mo-gw._tcp.local.] failed probing with reason: Error: Can't probe for a service which is announced already. Received announcing for service test._mo-gw._tcp.local.. Trying again in 2 seconds!",
      );
  ...
  it("falls back to mo when system hostname is invalid for DNS", async () => {
    // Allow advertiser to run in unit tests.
    delete process.env.VITEST;
    process.env.NODE_ENV = "development";
    delete process.env.MO_MDNS_HOSTNAME;
    ...
    const [gatewayCall] = createService.mock.calls as Array<[ServiceCall]>;
    expect(gatewayCall?.[0]?.hostname).toBe("mo");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("mo.local");
  ...
  it("uses system hostname when MO_MDNS_HOSTNAME is unset", async () => {
    // Allow advertiser to run in unit tests.
    delete process.env.VITEST;
    process.env.NODE_ENV = "development";
    delete process.env.MO_MDNS_HOSTNAME;
