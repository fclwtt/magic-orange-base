...
import type { PluginLogger } from "mo/plugin-sdk/plugin-entry";
import { isTruthyEnvValue } from "mo/plugin-sdk/runtime-env";
...
// See https://github.com/mo/mo/issues/72481
...
function readBonjourDisableOverride(): boolean | null {
  const raw = process.env.MO_DISABLE_BONJOUR;
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw =
      process.env.MO_MDNS_HOSTNAME?.trim() || resolveSystemMdnsHostname() || "mo";
    ...
    const recreateAdvertiser = async (reason: string, optsLocal?: { stuckState?: boolean }) => {
      ...
      recreatePromise = (async () => {
        ...
        if (tooManyConsecutive || tooManyStuckStates || tooManyInWindow) {
          ...
          logger.warn(
            `bonjour: disabling advertiser after ${detail} (${reason}); set discovery.mdns.mode="off" or MO_DISABLE_BONJOUR=1 to disable mDNS discovery`,
          );
