...
import type { MemoryEmbeddingProbeResult } from "mo/plugin-sdk/memory-core-host-engine-storage";
import {
  resolveMemoryDreamingConfig,
  resolveMemoryLightDreamingConfig,
  resolveMemoryRemDreamingConfig,
} from "mo/plugin-sdk/memory-core-host-status";
import { buildAgentSessionKey } from "mo/plugin-sdk/routing";
import { isUsageCountedSessionTranscriptFileName } from "mo/plugin-sdk/memory-core-host-engine-qmd";
...
function formatMemoryIndexIdentityWarning(
  ...
  return {
    reason,
    fix: `Run: mo memory status --index --agent ${agentId}`,
  };
}
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredOpenClawTmpDir(), "mo-rem-harness-"),
  );
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: mo memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredOpenClawTmpDir(), "mo-rem-backfill-"),
      );
      try {
