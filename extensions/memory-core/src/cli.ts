...
import {
  ...
  formatHelpExamples,
  theme,
} from "mo/plugin-sdk/memory-core-host-runtime-cli";
import {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mo memory status", "Show index and provider status."],
          [
            "mo memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["mo memory status --deep", "Probe embedding provider readiness."],
          ["mo memory index --force", "Force a full reindex."],
          ['mo memory search "meeting notes"', "Quick search using positional query."],
          [
            'mo memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `mo memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
            "Review weighted short-term candidates for long-term memory.",
          ],
          [
            "mo memory promote --apply",
            "Append top-ranked short-term candidates into MEMORY.md.",
          ],
          [
            'mo memory promote-explain "router vlan"',
            "Explain why a specific candidate would or would not promote.",
          ],
          [
            "mo memory rem-harness --json",
            "Preview REM reflections, candidate truths, and deep promotion output.",
          ],
          [
            "mo memory rem-backfill --path ./memory",
            "Write grounded historical REM entries into DREAMS.md for UI review.",
          ],
          [
            "mo memory rem-backfill --path ./memory --stage-short-term",
            "Also seed durable grounded candidates into the live short-term promotion store.",
          ],
          ["mo memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.mo.ai/cli/memory")}\n`,
    );
