...
} from "mo/plugin-sdk/error-runtime";
import { resolveGlobalMap } from "mo/plugin-sdk/global-singleton";
import * as memoryCoreHostRuntimeCoreModule from "mo/plugin-sdk/memory-core-host-runtime-core";
import * as runtimeConfigSnapshotModule from "mo/plugin-sdk/runtime-config-snapshot";
import * as sessionStoreRuntimeModule from "mo/plugin-sdk/session-store-runtime";
...
const DREAMS_FILE_LOCKS_KEY = Symbol.for("mo.memoryCore.dreamingNarrative.fileLocks");
const NARRATIVE_SESSION_LOCKS_KEY = Symbol.for(
  "mo.memoryCore.dreamingNarrative.sessionLocks",
);
const EXPECTS_POSIX_PRIVATE_FILE_MODE = process.platform !== "win32";
const originalNarrativeStateDir = process.env.MO_STATE_DIR;

function setNarrativeTestEnv(stateDir: string): void {
  Reflect.set(process.env, "MO_STATE_DIR", stateDir);
}

function restoreNarrativeTestEnv(): void {
  if (originalNarrativeStateDir === undefined) {
    Reflect.deleteProperty(process.env, "MO_STATE_DIR");
  } else {
    Reflect.set(process.env, "MO_STATE_DIR", originalNarrativeStateDir);
  }
}
...
describe("backfill diary entries", () => {
  ...
  it("builds a marked backfill diary entry", () => {
    ...
    expect(entry).toContain("mo:dreaming:backfill-entry");
    expect(entry).toContain("What Happened");
  });

  it("writes and replaces backfill diary entries", async () => {
    const workspaceDir = await createTempWorkspace("mo-dreaming-backfill-");
    ...
    expect(content.match(/mo:dreaming:backfill-entry/g)?.length).toBe(1);
  });
...
describe("appendNarrativeEntry", () => {
  it("creates DREAMS.md with diary header on fresh workspace", async () => {
    const workspaceDir = await createTempWorkspace("mo-dreaming-narrative-");
    ...
    expect(content).toContain("<!-- mo:dreaming:diary:start -->");
    expect(content).toContain("<!-- mo:dreaming:diary:end -->");
  });
  ...
  it("appends a second entry within the diary markers", async () => {
    ...
    const start = content.indexOf("<!-- mo:dreaming:diary:start -->");
    const end = content.indexOf("<!-- mo:dreaming:diary:end -->");
  ...
  it("prepends diary before existing managed blocks", async () => {
    ...
    await fs.writeFile(
      dreamsPath,
      "## Light Sleep\n<!-- mo:dreaming:light:start -->\n- Candidate: test\n<!-- mo:dreaming:light:end -->\n",
      "utf-8",
    );
    await appendNarrativeEntry({
      workspaceDir,
  ...
  it("dedupes only exact diary duplicates while keeping distinct timestamps", async () => {
    const workspaceDir = await createTempWorkspace("mo-dreaming-dedupe-");
    const dreamsPath = path.join(workspaceDir, "DREAMS.md");
    await fs.writeFile(
      dreamsPath,
      [
...
describe("generateAndAppendDreamNarrative", () => {
  ...
  it("scrubs stale dreaming entries and orphan transcripts after cleanup", async () => {
    const workspaceDir = await createTempWorkspace("mo-dreaming-narrative-");
    const stateDir = await createTempWorkspace("mo-dreaming-state-");
  ...
  it("isolates narrative sessions across workspaces even at the same timestamp", async () => {
    const firstWorkspaceDir = await createTempWorkspace("mo-dreaming-narrative-");
    const secondWorkspaceDir = await createTempWorkspace("mo-dreaming-narrative-");
...
describe("runDetachedDreamNarrative", () => {
  ...
  it("caps the number of in-flight detached narratives at 3", async () => {
    ...
    const workspaceDirs = await Promise.all(
      Array.from({ length: 5 }, () => createTempWorkspace("mo-dreaming-detach-")),
    );
    const logger = createMockLogger();
  ...
  it("serializes detached narratives that reuse a workspace and phase session", async () => {
    ...
    const workspaceDir = await createTempWorkspace("mo-dreaming-detach-");
    const logger = createMockLogger();
