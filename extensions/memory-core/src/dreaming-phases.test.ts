...
import { RequestScopedSubagentRuntimeError } from "mo/plugin-sdk/error-runtime";
import { resolveSessionTranscriptsDirForAgent } from "mo/plugin-sdk/memory-core-host-runtime-core";
import {
  resolveMemoryCorePluginConfig,
  resolveMemoryLightDreamingConfig,
  resolveMemoryRemDreamingConfig,
} from "mo/plugin-sdk/memory-core-host-status";
import { saveSessionStore } from "mo/plugin-sdk/session-store-runtime";
...
const originalDreamingTestFast = process.env.MO_TEST_FAST;
const originalDreamingStateDir = process.env.MO_STATE_DIR;
...
function setDreamingTestEnv(stateDir: string): void {
  Reflect.set(process.env, "MO_TEST_FAST", "1");
  Reflect.set(process.env, "MO_STATE_DIR", stateDir);
}

function restoreDreamingTestEnv(): void {
  if (originalDreamingTestFast === undefined) {
    Reflect.deleteProperty(process.env, "MO_TEST_FAST");
  } else {
    Reflect.set(process.env, "MO_TEST_FAST", originalDreamingTestFast);
  }
  if (originalDreamingStateDir === undefined) {
    Reflect.deleteProperty(process.env, "MO_STATE_DIR");
  } else {
    Reflect.set(process.env, "MO_STATE_DIR", originalDreamingStateDir);
  }
}
...
async function createDreamingWorkspace(): Promise<string> {
  const workspaceDir = await createTempWorkspace("mo-dreaming-phases-");
...
async function triggerLightDreaming(
  ...
  await beforeAgentReply(
    { cleanedBody: "__mo_memory_core_light_sleep__" },
...
describe("memory-core dreaming phases", () => {
      ...
      await beforeAgentReply(
        {
          cleanedBody: [
            "System: rotate logs",
            "System: __mo_memory_core_light_sleep__",
            "",
            "A scheduled reminder has been triggered. The reminder content is:",
            "",
            "rotate logs",
            "__mo_memory_core_light_sleep__",
            "",
  ...
  it("skips dreaming-generated narrative transcripts during session ingestion", async () => {
    ...
    await fs.writeFile(
      ...
      [
        JSON.stringify({
          type: "custom",
          customType: "mo:bootstrap-context:full",
          data: {
  ...
  it("records light/rem signals that reinforce deep promotion ranking", async () => {
    ...
    await withDreamingTestClock(async () => {
      ...
      await beforeAgentReply(
        { cleanedBody: "__mo_memory_core_rem_sleep__" },
        { trigger: "heartbeat", workspaceDir },
      );
    });
