...
import type { checkQmdBinaryAvailability as checkQmdBinaryAvailabilityFn } from "mo/plugin-sdk/memory-core-host-engine-qmd";
...
describe("getMemorySearchManager caching", () => {
  it("repairs an invalid shared singleton cache shape before using qmd cache maps", async () => {
    await closeAllMemorySearchManagers();
    vi.resetModules();
    const cacheKey = Symbol.for("mo.memorySearchManagerCache");
  ...
  it("creates a missing agent workspace before probing qmd availability", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mo-qmd-workspace-"));
