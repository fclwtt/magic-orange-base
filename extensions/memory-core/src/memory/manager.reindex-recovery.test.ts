...
const originalReindexStateDir = process.env.MO_STATE_DIR;
...
function restoreReindexStateDir(): void {
  if (originalReindexStateDir === undefined) {
    ...
    Reflect.set(process.env, "MO_STATE_DIR", originalReindexStateDir);
  }
}
...
describe("memory manager reindex recovery", () => {
  ...
  beforeEach(async () => {
    resetEmbeddingMocks();
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mo-mem-reindex-recovery-"));
