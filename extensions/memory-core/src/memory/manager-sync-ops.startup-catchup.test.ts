...
import { emitSessionTranscriptUpdate } from "mo/plugin-sdk/agent-harness-runtime";
import {
...
const MEMORY_CORE_TRANSCRIPT_UPDATE_SUBSCRIBER_KEY = Symbol.for(
  "mo.memoryCore.sessionTranscriptUpdateSubscriber",
);
const originalStartupStateDir = process.env.MO_STATE_DIR;
const originalStartupConfigPath = process.env.MO_CONFIG_PATH;
...
function setStartupConfigPath(configPath: string): void {
  Reflect.set(process.env, "MO_CONFIG_PATH", configPath);
}
...
function restoreStartupEnv(): void {
  if (originalStartupStateDir === undefined) {
    ...
    Reflect.set(process.env, "MO_STATE_DIR", originalStartupStateDir);
  }
  if (originalStartupConfigPath === undefined) {
    Reflect.deleteProperty(process.env, "MO_CONFIG_PATH");
  } else {
    Reflect.set(process.env, "MO_CONFIG_PATH", originalStartupConfigPath);
  }
}
...
class SessionStartupCatchupHarness extends MemoryManagerSyncOps {
  ...
  protected readonly workspaceDir = "/tmp/mo-test-workspace";
...
describe("session startup catch-up", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-session-startup-"));
