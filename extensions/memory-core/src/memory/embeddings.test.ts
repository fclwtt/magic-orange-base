...
import type { EmbeddingProviderAdapter } from "mo/plugin-sdk/embedding-providers";
import type { MemoryEmbeddingProviderAdapter } from "mo/plugin-sdk/memory-core-host-engine-embeddings";
...
vi.mock("mo/plugin-sdk/embedding-providers", () => ({
...
vi.mock("mo/plugin-sdk/memory-core-host-engine-embeddings", () => ({
...
describe("createEmbeddingProvider", () => {
  ...
  it("reports the llama.cpp plugin install command when local is unregistered", async () => {
    await expect(createEmbeddingProvider(createOptions("local"))).rejects.toThrow(
      "mo plugins install @mo/llama-cpp-provider",
    );
  });
