...
import type { MemorySource } from "mo/plugin-sdk/memory-core-host-engine-storage";
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";

const { buildSessionEntryMock } = vi.hoisted(() => ({
  buildSessionEntryMock: vi.fn(),
}));
const originalSyncYieldStateDir = process.env.MO_STATE_DIR;

function setSyncYieldStateDir(): void {
  Reflect.set(
    process.env,
    "MO_STATE_DIR",
    path.join(os.tmpdir(), "mo-session-sync-yield"),
  );
}
...
function restoreSyncYieldStateDir(): void {
  if (originalSyncYieldStateDir === undefined) {
    ...
    Reflect.set(process.env, "MO_STATE_DIR", originalSyncYieldStateDir);
  }
}
...
vi.mock("mo/plugin-sdk/memory-core-host-engine-qmd", () => {
