...
import { hashText } from "mo/plugin-sdk/memory-core-host-engine-storage";
import { resolveSessionTranscriptsDirForAgent } from "mo/plugin-sdk/memory-core-host-runtime-core";
import { resolveOpenClawAgentSqlitePath } from "mo/plugin-sdk/sqlite-runtime";
import {
  closeOpenClawAgentDatabasesForTest,
  closeOpenClawStateDatabaseForTest,
  openOpenClawAgentDatabase,
} from "mo/plugin-sdk/sqlite-runtime-testing";
...
const originalMemoryIndexStateDir = process.env.MO_STATE_DIR;
...
function restoreMemoryIndexStateDir(): void {
  if (originalMemoryIndexStateDir === undefined) {
    ...
    Reflect.set(process.env, "MO_STATE_DIR", originalMemoryIndexStateDir);
  }
}
...
describe("memory index", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mo-mem-fixtures-"));
  ...
  it("status-purpose manager detects unindexed session transcripts as dirty", async () => {
    // Regression test for #97814: plain mo memory status (purpose: status)
