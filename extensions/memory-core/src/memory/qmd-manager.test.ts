...
import { withMockedWindowsPlatform } from "mo/plugin-sdk/test-node-mocks";
...
const MEMORY_EMBEDDING_PROVIDERS_KEY = Symbol.for("mo.memoryEmbeddingProviders");
const MCPORTER_STATE_KEY = Symbol.for("mo.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("mo.qmdEmbedQueueTail");
...
vi.mock("mo/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("mo/plugin-sdk/memory-core-host-engine-foundation")
  >("mo/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
vi.mock("mo/plugin-sdk/file-lock", async () => {
  const actual = await vi.importActual<typeof import("mo/plugin-sdk/file-lock")>(
    "mo/plugin-sdk/file-lock",
  );
  return {
    ...actual,
...
import type { OpenClawConfig } from "mo/plugin-sdk/memory-core-host-engine-foundation";
import {
  type MemorySearchRuntimeDebug,
  requireNodeSqlite,
  resolveMemoryBackendConfig,
} from "mo/plugin-sdk/memory-core-host-engine-storage";
import { MAX_TIMER_TIMEOUT_MS } from "mo/plugin-sdk/number-runtime";
import { formatSessionTranscriptMemoryHitKey } from "mo/plugin-sdk/session-transcript-hit";
import {
...
const originalQmdStateDir = process.env.MO_STATE_DIR;
...
function restoreQmdStateDir(): void {
  if (originalQmdStateDir === undefined) {
    ...
    Reflect.set(process.env, "MO_STATE_DIR", originalQmdStateDir);
  }
}
...
describe("QmdMemoryManager", () => {
  ...
  it("rebinds a stale in-container collection root to the host workspace (sandbox-mode transition)", async () => {
    ...
    // /home/node/.mo/... stored that in-container path as the collection root. Resolved
    ...
    const containerRoot = "/home/node/.mo/teams/x/workspace";
  ...
  it("parseShownCollection extracts path and pattern from qmd collection show output", async () => {
    ...
    const sampleOutput = [
      ...
      "  Path:     /home/node/.mo/teams/example-team/workspace-example/memory",
      "  Pattern:  **/*.md",
      "  Include:  yes (default)",
    ].join("\n");

    const result = parser(sampleOutput);
    expect(result.path).toBe("/home/node/.mo/teams/example-team/workspace-example/memory");
