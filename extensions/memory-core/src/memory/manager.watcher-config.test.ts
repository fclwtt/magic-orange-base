...
import type {
  MemorySearchConfig,
  OpenClawConfig,
} from "mo/plugin-sdk/memory-core-host-engine-foundation";
...
const {
  ...
  const chokidarKey = Symbol.for("mo.test.memoryWatchFactory");
  const nativeKey = Symbol.for("mo.test.memoryNativeWatchFactory");
...
const CHOKIDAR_FACTORY_KEY = Symbol.for("mo.test.memoryWatchFactory");
const NATIVE_FACTORY_KEY = Symbol.for("mo.test.memoryNativeWatchFactory");
const originalWatcherStateDir = process.env.MO_STATE_DIR;
...
function restoreWatcherStateDir(): void {
  if (originalWatcherStateDir === undefined) {
    ...
    Reflect.set(process.env, "MO_STATE_DIR", originalWatcherStateDir);
  }
}

vi.mock("mo/plugin-sdk/memory-core-host-engine-foundation", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("mo/plugin-sdk/memory-core-host-engine-foundation")>();
  return {
    ...actual,
...
import { clearMemoryEmbeddingProviders as clearRegistry } from "mo/plugin-sdk/memory-core-host-engine-embeddings";
import {
...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-memory-watch-"));
