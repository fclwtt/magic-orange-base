...
import { resolveUserPath } from "mo/plugin-sdk/memory-core-host-engine-foundation";
import {
...
import { resolveMemoryDreamingWorkspaces } from "mo/plugin-sdk/memory-core-host-status";
import { normalizeAgentId } from "mo/plugin-sdk/routing";
import type { PluginDoctorStateMigration } from "mo/plugin-sdk/runtime-doctor";
import {
  ensureOpenClawAgentDatabaseSchema,
  resolveOpenClawAgentSqlitePath,
} from "mo/plugin-sdk/sqlite-runtime";
import {
...
async function collectLegacyMemorySidecarSources(params: {
  ...
  const migrationEnv = { ...params.env, MO_STATE_DIR: params.stateDir };
...
async function migrateLegacyMemorySidecarSource(params: {
  ...
  try {
    const migrationEnv = {
      ...params.env,
      MO_STATE_DIR: params.source.stateDir,
    };
