 ...
 * `__MO_REDACTED__` literal anywhere in the output throws
...
export function emitMd(ast: MdAst, opts: EmitOptions = {}): string {
  ...
  if (mode === "roundtrip") {
    ...
    if (!acceptPreExisting && ast.raw.includes("__MO_REDACTED__")) {
      guardSentinel("__MO_REDACTED__", `${guardPath}/[raw]`);
    }
    return ast.raw;
  }
