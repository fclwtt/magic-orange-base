/**
 * Smoke tests for the `mo path` CLI handlers.
 *
...
describe("mo path CLI", () => {
  ...
  describe("set", () => {
    ...
    it("CLI-S08 sets slash-deep JSONC paths and parsed JSON values", async () => {
      const filePath = join(workspaceDir, "mo.json");
      writeFileSync(
        filePath,
      ...
      await pathSetCommand(
        "oc://mo.json/agents/list/0/tools/exec/security",
        "allowlist",
        { cwd: workspaceDir, json: true },
        rt2,
      );
