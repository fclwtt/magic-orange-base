...
import { buildModelAliasIndex, resolveModelRefFromString } from "mo/plugin-sdk/agent-runtime";
import {
...
export const llmTaskToolDefinition = {
  ...
  label: "LLM Task",
  description:
    "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via mo.invoke.",
  ...
  return {
    ...
    async execute(_id: string, params: LlmTaskParams) {
      ...
      return await withTempWorkspace(
        { rootDir: resolvePreferredOpenClawTmpDir(), prefix: "mo-llm-task-" },
        async ({ dir: tmpDir }) => {
