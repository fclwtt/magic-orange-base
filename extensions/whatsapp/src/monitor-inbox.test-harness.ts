...
const pluginRuntimeMocks = vi.hoisted(() => {
  ...
  let stateDir = `/tmp/mo-whatsapp-ingress-${Date.now()}-${Math.random()}`;
  ...
  return {
    ...
    reset: () => {
      ...
      stateDir = `/tmp/mo-whatsapp-ingress-${Date.now()}-${Math.random()}`;
    },
  };
});
...
vi.mock("mo/plugin-sdk/channel-activity-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("mo/plugin-sdk/channel-activity-runtime")
  >("mo/plugin-sdk/channel-activity-runtime");
  return {
    ...actual,
...
export function installWebMonitorInboxUnitTestHooks(opts?: { authDir?: boolean }) {
  ...
  beforeEach(async () => {
    ...
    resetWebInboundDedupe();
    if (createAuthDir) {
      authDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "mo-auth-"));
    } else {
      authDir = undefined;
    }
  });
