...
vi.mock("mo/plugin-sdk/channel-pairing", async () => {
  const actual = await vi.importActual<typeof import("mo/plugin-sdk/channel-pairing")>(
    "mo/plugin-sdk/channel-pairing",
  );
  return {
    ...actual,
    readChannelAllowFromStore(...args: unknown[]) {
...
vi.mock("mo/plugin-sdk/media-store", async () => {
  const actual = await vi.importActual<typeof import("mo/plugin-sdk/media-store")>(
    "mo/plugin-sdk/media-store",
  );
  return {
    ...actual,
...
vi.mock("./runtime.js", async () => {
  const { createChannelIngressQueueForTests: createChannelIngressQueue } = await Promise.resolve(
    vi.importActual<typeof import("mo/plugin-sdk/plugin-state-test-runtime")>(
      "mo/plugin-sdk/plugin-state-test-runtime",
    ),
  );
  const stateDir = `/tmp/mo-whatsapp-inbound-media-${Date.now()}-${Math.random()}`;
  return {
...
const HOME = path.join(os.tmpdir(), `mo-inbound-media-${crypto.randomUUID()}`);
