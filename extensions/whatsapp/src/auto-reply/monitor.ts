...
import { resolveAccountEntry } from "mo/plugin-sdk/account-core";
import { CHANNEL_APPROVAL_NATIVE_RUNTIME_CONTEXT_CAPABILITY } from "mo/plugin-sdk/approval-handler-runtime";
import { resolveInboundDebounceMs } from "mo/plugin-sdk/channel-inbound-debounce";
import { registerChannelRuntimeContext } from "mo/plugin-sdk/channel-runtime-context";
import { formatCliCommand } from "mo/plugin-sdk/cli-runtime";
import { isControlCommandMessage } from "mo/plugin-sdk/command-detection";
import { drainPendingDeliveries } from "mo/plugin-sdk/delivery-queue-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "mo/plugin-sdk/reply-history";
import { resolveAgentRoute } from "mo/plugin-sdk/routing";
import { logVerbose } from "mo/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "mo/plugin-sdk/runtime-env";
import { getChildLogger } from "mo/plugin-sdk/runtime-env";
import {
  defaultRuntime,
  formatDurationPrecise,
  warn,
  type RuntimeEnv,
} from "mo/plugin-sdk/runtime-env";
import { enqueueSystemEvent } from "mo/plugin-sdk/system-event-runtime";
    ...
    while (true) {
        ...
        if (setupDecision) {
          ...
          if (setupDecision.action === "stop") {
            ...
            if (setupDecision.healthState === "logged-out") {
              runtime.error(
                `WhatsApp session logged out during setup. Run \`${formatCliCommand("mo channels login --channel whatsapp")}\` to relink.`,
              );
            } else if (setupDecision.healthState === "conflict") {
              runtime.error(
                `WhatsApp Web connection closed during setup (status ${setupDecision.normalized.statusLabel}: session conflict). Resolve conflicting WhatsApp Web sessions, then restart the channel. To force a fresh QR, run \`${formatCliCommand("mo channels logout --channel whatsapp")}\` before \`${formatCliCommand("mo channels login --channel whatsapp")}\`. Stopping web monitoring.`,
              );
            } else {
              runtime.error(
                `WhatsApp Web connection closed during setup (status ${setupDecision.normalized.statusLabel}) after ${setupDecision.reconnectAttempts}/${reconnectPolicy.maxAttempts} attempts. Relink with \`${formatCliCommand("mo channels login --channel whatsapp")}\` if the issue persists.`,
              );
            }
            await controller.shutdown();
            break;
      ...
      if (decision.action === "stop") {
        ...
        if (decision.healthState === "logged-out") {
          runtime.error(
            `WhatsApp session logged out. Run \`${formatCliCommand("mo channels login --channel whatsapp")}\` to relink.`,
          );
