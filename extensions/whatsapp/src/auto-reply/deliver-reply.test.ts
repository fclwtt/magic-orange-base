...
import { sleep } from "mo/plugin-sdk/text-utility-runtime";
...
vi.mock("mo/plugin-sdk/text-utility-runtime", async () => {
  const actual = await vi.importActual<typeof import("mo/plugin-sdk/text-utility-runtime")>(
    "mo/plugin-sdk/text-utility-runtime",
  );
  return {
    ...actual,
    sleep: vi.fn(async () => {}),
