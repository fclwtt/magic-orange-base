...
import { createQueuedWizardPrompter } from "mo/plugin-sdk/plugin-test-runtime";
...
vi.mock("mo/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<typeof import("mo/plugin-sdk/setup")>(
    "mo/plugin-sdk/setup",
  );
...
describe("whatsapp setup wizard", () => {
  beforeEach(() => {
    ...
    hoisted.resolveWhatsAppAuthDir.mockReturnValue({ authDir: "/tmp/mo-whatsapp-test" });
  });
