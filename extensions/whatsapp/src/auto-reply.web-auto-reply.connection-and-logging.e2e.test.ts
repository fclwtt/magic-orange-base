...
import { setLoggerOverride } from "mo/plugin-sdk/runtime-env";
...
vi.mock("mo/plugin-sdk/delivery-queue-runtime", () => ({
...
describe("web auto-reply connection", () => {
  ...
  it("treats status 440 as non-retryable and stops without retrying", async () => {
    ...
    expectErrorContaining(runtime.error, "mo channels logout --channel whatsapp");
    ...
    await withEnvAsync({ TZ: "Europe/Vienna" }, async () => {
      ...
      try {
        ...
        expect(firstArgs.Body).toMatch(
          new RegExp(
            `\\[WhatsApp \\+1 (\\+\\d+[smhd] )?${firstPattern}\\] \\+1: \\[mo\\] first`,
          ),
        );
        expect(firstArgs.Body).not.toContain("second");
        expect(secondArgs.Body).toMatch(
          new RegExp(
            `\\[WhatsApp \\+1 (\\+\\d+[smhd] )?${secondPattern}\\] \\+1: \\[mo\\] second`,
          ),
        );
  ...
  it("emits heartbeat logs with connection metadata", async () => {
    vi.useFakeTimers();
    const logPath = `/tmp/mo-heartbeat-${crypto.randomUUID()}.log`;
  ...
  it("logs outbound replies to file", async () => {
    const logPath = `/tmp/mo-log-test-${crypto.randomUUID()}.log`;
    setLoggerOverride({ level: "trace", file: logPath });
