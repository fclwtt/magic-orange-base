...
import { captureEnv } from "mo/plugin-sdk/test-env";
...
import { withMockedWindowsPlatform, withRestoredMocks } from "mo/plugin-sdk/test-node-mocks";
import { optimizeImageToPng } from "mo/plugin-sdk/web-media";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredOpenClawTmpDir(), "mo-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub MO_STATE_DIR.
    ...
    process.env.MO_STATE_DIR = path.join(
...
describe("local media root guard", () => {
  ...
  it("accepts win32 dev=0 stat mismatch for local file loads", async () => {
    ...
    // os.tmpdir() fallback rather than the POSIX `/tmp/mo` root that
