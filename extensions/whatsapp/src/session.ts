...
import { VERSION } from "mo/plugin-sdk/cli-runtime";
import {
  createHttp1EnvHttpProxyAgent,
  createHttp1ProxyAgent,
  createNodeProxyAgent,
} from "mo/plugin-sdk/fetch-runtime";
import { danger, success } from "mo/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "mo/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "mo/plugin-sdk/text-utility-runtime";
import {
  readCredsJsonRaw,
...
export const MO_WHATSAPP_WEB_SOCKET_URL_ENV = "MO_WHATSAPP_WEB_SOCKET_URL";
...
function resolveEnvWaWebSocketUrl(): string | undefined {
  const value = resolveWaWebSocketUrl(process.env[MO_WHATSAPP_WEB_SOCKET_URL_ENV]);
  if (!value) {
    return undefined;
  }
  let url: URL;
  try {
    ...
    throw new Error(`${MO_WHATSAPP_WEB_SOCKET_URL_ENV} must be a valid URL.`);
  }
  if (url.protocol !== "ws:" && url.protocol !== "wss:") {
    throw new Error(`${MO_WHATSAPP_WEB_SOCKET_URL_ENV} must use ws:// or wss://.`);
  }
  return url.toString();
}
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("mo channels login")}`,
              ),
            );
          }
        }
