...
import { generateSecureUuid } from "mo/plugin-sdk/core";
import { redactIdentifier } from "mo/plugin-sdk/logging-core";
import {
  convertMarkdownTables,
  resolveMarkdownTableMode,
} from "mo/plugin-sdk/markdown-table-runtime";
import { requireRuntimeConfig } from "mo/plugin-sdk/plugin-config-runtime";
import { normalizePollInput, type PollInput } from "mo/plugin-sdk/poll-runtime";
import { createSubsystemLogger, getChildLogger } from "mo/plugin-sdk/runtime-env";
import {
...
function requireOutboundActiveWebListener(params: { cfg: OpenClawConfig; accountId?: string }): {
  ...
  if (!listener) {
    throw new Error(
      `No active WhatsApp Web listener (account: ${resolvedAccountId}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`mo channels login --channel whatsapp --account ${resolvedAccountId}`)}.`,
    );
  }
