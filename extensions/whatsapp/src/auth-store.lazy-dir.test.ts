...
describe("WhatsApp auth dir profile resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["MO_STATE_DIR", "MO_OAUTH_DIR"]);
    tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-wa-profile-"));
    delete process.env.MO_STATE_DIR;
    delete process.env.MO_OAUTH_DIR;
    vi.resetModules();
  });
  ...
  it("resolves the default web auth dir from MO_STATE_DIR at call time", async () => {
