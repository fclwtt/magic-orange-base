...
import { disposeRegisteredAgentHarnesses } from "mo/plugin-sdk/agent-harness";
...
function shouldLogQaSuiteProgress(env: NodeJS.ProcessEnv = process.env) {
  const override = parseQaSuiteBooleanEnv(env.MO_QA_SUITE_PROGRESS);
...
function buildRuntimeParityScenarioResult(params: {
  ...
  const moCell = params.result.cells.mo;
  return {
    ...
    steps: [
      {
        name: moCell.runtime,
        status:
          moCell.runtimeErrorClass || moCell.transportErrorClass ? "fail" : "pass",
        details: formatRuntimeParityCellDetails(moCell),
      },
      {
...
function buildQaRuntimeEnvPatch(params: {
  ...
  if (params.forcedRuntime) {
    patch.MO_BUILD_PRIVATE_QA = "1";
    patch.MO_QA_FORCE_RUNTIME = params.forcedRuntime;
  }
  if (params.forcedRuntime !== "codex" || params.providerMode !== "mock-openai") {
  ...
  patch.MO_CODEX_APP_SERVER_ARGS = `app-server -c openai_base_url=${mockBaseUrl}/v1 --listen stdio://`;
...
function shouldCaptureGatewayHeapCheckpoints(env: NodeJS.ProcessEnv = process.env) {
  return parseQaSuiteBooleanEnv(env.MO_QA_GATEWAY_HEAP_CHECKPOINTS) === true;
}
...
export async function runQaFlowSuite(params?: QaSuiteRunParams): Promise<QaSuiteResult> {
  ...
  const enabledPluginIds = [
    ...new Set([
      ...
      ...(params?.forcedRuntime && params.forcedRuntime !== "mo"
  ...
  try {
    ...
    const keepTemp = process.env.MO_QA_KEEP_TEMP === "1" || false;
    await gateway.stop({
      keepTemp,
