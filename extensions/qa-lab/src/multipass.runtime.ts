...
import type { OpenClawCrablineChannelDriverSelection } from "@mo/crabline";
import { sleep } from "mo/plugin-sdk/runtime-env";
import { appendRegularFile } from "mo/plugin-sdk/security-runtime";
...
const MULTIPASS_MOUNTED_REPO_PATH = "/workspace/mo-host";
const MULTIPASS_GUEST_REPO_PATH = "/workspace/mo";
const MULTIPASS_GUEST_CODEX_HOME_PATH = "/workspace/mo-codex-home";
...
export function createQaMultipassPlan(params: {
  ...
  const hostLiveProviderConfigPath =
    liveProviderConfig && fs.existsSync(liveProviderConfig.path)
      ...
      : undefined;
  const vmName = `mo-qa-${createVmSuffix()}`;
...
export function renderQaMultipassGuestScript(
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          key !== "CODEX_HOME" &&
          key !== "MO_CONFIG_PATH" &&
          key !== "MO_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `MO_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `MO_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
...
export async function runQaMultipass(params: {
  ...
  try {
    ...
    throw new Error(
      `Multipass is not installed on this host. Install it with '${resolveMultipassInstallHint()}', then rerun 'pnpm mo qa suite --runner multipass'.`,
      { cause: error },
    );
  }
