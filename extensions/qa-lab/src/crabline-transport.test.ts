...
import { MO_CRABLINE_MANIFEST_PATH } from "@mo/crabline";
    ...
    await withTempDir("qa-crabline-transport-", async (outputDir) => {
      ...
      try {
        ...
        const manifest = JSON.parse(
          await fs.readFile(path.join(outputDir, MO_CRABLINE_MANIFEST_PATH), "utf8"),
        ) as {
          provider?: string;
        };
    ...
    await withTempDir("qa-crabline-transport-", async (outputDir) => {
      ...
      try {
        ...
        expect(env).toMatchObject({
          ...
          MO_WHATSAPP_WEB_SOCKET_URL: expect.stringMatching(
