...
import { formatMemoryDreamingDay } from "mo/plugin-sdk/memory-core-host-status";
import { resolveSessionTranscriptsDirForAgent } from "mo/plugin-sdk/memory-host-core";
...
function setActiveMemorySessionDisabled(
  ...
  const store = createPluginStateSyncKeyedStore<{
    ...
    env: {
      ...process.env,
      MO_STATE_DIR: path.join(env.gateway.tempRoot, "state"),
    },
  });
