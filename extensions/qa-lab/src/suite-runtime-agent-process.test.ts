...
describe("qa suite runtime agent process helpers", () => {
  ...
  it("merges isolated env overrides into qa cli runs", async () => {
    ...
    const pending = runQaCli(
      {
        ...
        gateway: {
          tempRoot: "/tmp/runtime",
          runtimeEnv: { PATH: "/usr/bin", MO_STATE_DIR: "/tmp/default-state" },
        },
        primaryModel: "openai/gpt-5.5",
        alternateModel: "openai/gpt-5.5-mini",
        providerMode: "mock-openai",
      ...
      {
        env: {
          MO_STATE_DIR: "/tmp/isolated-state",
          MO_CONFIG_PATH: "/tmp/isolated-state/mo.json",
        },
      },
    );
    ...
    expect(spawnEnv?.MO_STATE_DIR).toBe("/tmp/isolated-state");
    expect(spawnEnv?.MO_CONFIG_PATH).toBe("/tmp/isolated-state/mo.json");
  });
  ...
  it("finds managed dreaming cron jobs across legacy and current payload contracts", () => {
    ...
    const current = {
      ...
      payload: {
        kind: "agentTurn",
        message: "__mo_memory_core_short_term_promotion_dream__",
        lightContext: true,
      },
