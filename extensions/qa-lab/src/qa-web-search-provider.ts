...
export const QA_LAB_WEB_SEARCH_DENIED_INPUT_QUERY = "MO_QA_WEB_SEARCH_DENIED_INPUT";
...
function buildQaLabSearchResult(query: string, index: number) {
  const url = `https://docs.mo.ai/qa-lab/search-fixture/${index + 1}`;
  return {
    ...
    siteName: resolveSiteName(url) || "docs.mo.ai",
  };
}
...
export function createQaLabWebSearchProvider(): WebSearchProviderPlugin {
  return {
    ...
    signupUrl: "https://docs.mo.ai/concepts/qa-e2e-automation",
    docsUrl: "https://docs.mo.ai/concepts/qa-e2e-automation",
    credentialPath: "",
    inactiveSecretPaths: [],
