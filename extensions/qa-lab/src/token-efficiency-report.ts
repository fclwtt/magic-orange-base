...
export type TokenEfficiencyRow = {
  ...
  mo: TokenEfficiencyRuntimeUsage;
...
export type TokenEfficiencyReport = {
  ...
  aggregate: {
    mo: { totalTokens: number; p50PerScenario: number; p90PerScenario: number };
...
const ZERO_AGGREGATE: TokenEfficiencyReport["aggregate"] = {
  mo: { totalTokens: 0, p50PerScenario: 0, p90PerScenario: 0 },
...
function deltaPercent(moTotalTokens: number, codexTotalTokens: number): number {
  if (moTotalTokens === 0) {
    return codexTotalTokens === 0 ? 0 : 100;
  }
  return ((codexTotalTokens - moTotalTokens) / moTotalTokens) * 100;
}
...
function toolNamesForCells(mo: RuntimeParityCell, codex: RuntimeParityCell): string[] {
  return [
    ...new Set([...mo.toolCalls, ...codex.toolCalls].map((call) => call.tool)),
...
function buildRow(params: {
  ...
  const mo = runtimeUsage(params.result.cells.mo);
  const codex = runtimeUsage(params.result.cells.codex);
  const delta = deltaPercent(mo.totalTokens, codex.totalTokens);
  ...
  return {
    ...
    classification,
    flagged,
    toolsUsed: toolNamesForCells(params.result.cells.mo, params.result.cells.codex),
  };
}

function buildAggregate(rows: readonly TokenEfficiencyRow[]): TokenEfficiencyReport["aggregate"] {
  const moTotals = rows.map((row) => row.mo.totalTokens);
  const codexTotals = rows.map((row) => row.codex.totalTokens);
  const moTotalTokens = moTotals.reduce((sum, value) => sum + value, 0);
  const codexTotalTokens = codexTotals.reduce((sum, value) => sum + value, 0);
  return {
    mo: {
      totalTokens: moTotalTokens,
      p50PerScenario: percentile(moTotals, 50),
      p90PerScenario: percentile(moTotals, 90),
    },
    codex: {
    ...
    deltaPercent: deltaPercent(moTotalTokens, codexTotalTokens),
...
function liveEvidenceFailures(row: TokenEfficiencyRow): string[] {
  ...
  if (row.mo.totalTokens <= 0) {
    failures.push(`${row.scenarioId} mo live usage totalTokens=${row.mo.totalTokens}`);
  }
...
export function buildTokenEfficiencyReport(
  ...
  const failures = rows.flatMap((row, index) => {
    ...
    const rowFailures =
      liveUsage && result
        ? [
            ...liveUsageShapeFailures(row.scenarioId, "mo", result.cells.mo.usage),
...
export function renderTokenEfficiencyMarkdownReport(report: TokenEfficiencyReport): string {
  ...
  lines.push(
    ...
    "| --- | ---: | ---: | ---: |",
    `| mo | ${report.aggregate.mo.totalTokens} | ${report.aggregate.mo.p50PerScenario} | ${report.aggregate.mo.p90PerScenario} |`,
  ...
  if (report.rows.length > 0) {
    ...
    for (const row of report.rows) {
      lines.push(
        `| ${row.scenarioId} | ${row.usageSource} | ${row.mo.inputTokens}/${row.mo.outputTokens}/${row.mo.totalTokens}/${row.mo.toolCallCount} | ${row.codex.inputTokens}/${row.codex.outputTokens}/${row.codex.totalTokens}/${row.codex.toolCallCount} | ${formatPercent(row.deltaPercent)} | ${row.classification} | ${row.flagged ? "yes" : "no"} | ${row.toolsUsed.join(", ")} |`,
      );
    }
    lines.push("");
  }
