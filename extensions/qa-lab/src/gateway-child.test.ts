...
vi.mock("mo/plugin-sdk/ssrf-runtime", () => ({
  fetchWithSsrFGuard: fetchWithSsrFGuardMock,
}));

vi.mock("mo/plugin-sdk/temp-path", () => ({
...
function createParams(baseEnv?: NodeJS.ProcessEnv) {
  return {
    configPath: "/tmp/mo-qa/mo.json",
    gatewayToken: "qa-token",
    homeDir: "/tmp/mo-qa/home",
    stateDir: "/tmp/mo-qa/state",
    tempRoot: "/tmp/mo-qa",
    xdgConfigHome: "/tmp/mo-qa/xdg-config",
    xdgDataHome: "/tmp/mo-qa/xdg-data",
    xdgCacheHome: "/tmp/mo-qa/xdg-cache",
    bundledPluginsDir: "/tmp/mo-qa/bundled-plugins",
    stagedBundledPluginsRoot: "/repo/.artifacts/qa-runtime/mo-qa-suite-test",
...
describe("buildQaRuntimeEnv", () => {
  ...
  it("reports command spawn errors instead of leaking unhandled child errors", async () => {
    ...
    qaTempPathState.preferredTmpDir = tempParent;
    const missingExecutable = path.join(tempParent, "missing-mo-node");
  ...
  it("keeps the slow-reply QA opt-out enabled under fast mode", () => {
    ...
    expect(env.MO_TEST_FAST).toBe("1");
    expect(env.MO_EMBEDDED_ABORT_SETTLE_TIMEOUT_MS).toBe("2000");
    expect(env.MO_QA_PARENT_PID).toBe(String(process.pid));
    expect(env.MO_QA_TEMP_ROOT).toBe("/tmp/mo-qa");
    expect(env.MO_QA_STAGED_RUNTIME_ROOT).toBe(
      "/repo/.artifacts/qa-runtime/mo-qa-suite-test",
    );
    expect(env.MO_QA_ALLOW_LOCAL_IMAGE_PROVIDER).toBe("1");
    expect(env.MO_ALLOW_SLOW_REPLY_TESTS).toBe("1");
    expect(env.MO_BUNDLED_PLUGINS_DIR).toBe("/tmp/mo-qa/bundled-plugins");
    expect(env.MO_COMPATIBILITY_HOST_VERSION).toBe("2026.4.8");
  });

  it("maps live frontier key aliases into provider env vars", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        MO_LIVE_OPENAI_KEY: "openai-live",
        MO_LIVE_ANTHROPIC_KEY: "anthropic-live",
        MO_LIVE_GEMINI_KEY: "gemini-live",
      }),
  ...
  it("preserves Codex CLI auth home for live frontier runs while sandboxing OpenClaw home", async () => {
    ...
    expect(env.HOME).toBe("/tmp/mo-qa/home");
    expect(env.MO_HOME).toBe("/tmp/mo-qa/home");
  ...
  it("forwards host HOME for live Claude CLI runs while keeping OpenClaw home sandboxed", async () => {
    ...
    expect(env.MO_HOME).toBe("/tmp/mo-qa/home");
    expect(env.MO_STATE_DIR).toBe("/tmp/mo-qa/state");
  });
  ...
  it("preserves the live Anthropic key for live Claude CLI runs without writing it into config", async () => {
    ...
    const env = buildQaRuntimeEnv({
      ...createParams({
        HOME: hostHome,
        MO_LIVE_ANTHROPIC_KEY: "anthropic-live",
        MO_LIVE_CLI_BACKEND_PRESERVE_ENV: '["SAFE_KEEP"]',
      }),
      providerMode: "live-frontier",
      forwardHostHomeForClaudeCli: true,
    ...
    expect(env.ANTHROPIC_API_KEY).toBe("anthropic-live");
    expect(env.MO_LIVE_CLI_BACKEND_PRESERVE_ENV).toBe('["SAFE_KEEP","ANTHROPIC_API_KEY"]');
    expect(env.MO_LIVE_CLI_BACKEND_AUTH_MODE).toBe("api-key");
  });
  ...
  it("removes preserved Anthropic keys for live Claude CLI subscription runs", async () => {
    ...
    const env = buildQaRuntimeEnv({
      ...createParams({
        ...
        MO_LIVE_CLI_BACKEND_PRESERVE_ENV: '["SAFE_KEEP","ANTHROPIC_API_KEY"]',
      }),
      providerMode: "live-frontier",
      forwardHostHomeForClaudeCli: true,
    ...
    expect(env.ANTHROPIC_API_KEY).toBe("anthropic-live");
    expect(env.MO_LIVE_CLI_BACKEND_PRESERVE_ENV).toBe('["SAFE_KEEP"]');
    expect(env.MO_LIVE_CLI_BACKEND_AUTH_MODE).toBe("subscription");
  });

  it("does not pass QA setup-token values to the gateway child env", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        MO_LIVE_SETUP_TOKEN_VALUE: `sk-ant-oat01-${"a".repeat(80)}`,
        MO_QA_LIVE_ANTHROPIC_SETUP_TOKEN: `sk-ant-oat01-${"b".repeat(80)}`,
      }),
      providerMode: "live-frontier",
    });

    expect(env.MO_LIVE_SETUP_TOKEN_VALUE).toBeUndefined();
    expect(env.MO_QA_LIVE_ANTHROPIC_SETUP_TOKEN).toBeUndefined();
  });

  it("does not pass Convex credential broker secrets to the gateway child env", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        MO_QA_CONVEX_SECRET_CI: "convex-ci-secret",
        MO_QA_CONVEX_SECRET_MAINTAINER: "convex-maintainer-secret",
      }),
      providerMode: "live-frontier",
    });

    expect(env.MO_QA_CONVEX_SECRET_CI).toBeUndefined();
    expect(env.MO_QA_CONVEX_SECRET_MAINTAINER).toBeUndefined();
  });
    ...
    (providerMode) => {
      const env = buildQaRuntimeEnv({
        ...createParams({
          ...
          MO_LIVE_ANTHROPIC_KEY: "anthropic-live",
          MO_LIVE_ANTHROPIC_KEYS: "anthropic-a,anthropic-b",
          MO_LIVE_CODEX_API_KEY: "codex-live",
          MO_LIVE_GEMINI_KEY: "gemini-live",
          MO_LIVE_OPENAI_KEY: "openai-live",
        }),
        providerMode,
      ...
      expect(env.MO_LIVE_OPENAI_KEY).toBeUndefined();
      expect(env.MO_LIVE_ANTHROPIC_KEY).toBeUndefined();
      expect(env.MO_LIVE_ANTHROPIC_KEYS).toBeUndefined();
      expect(env.MO_LIVE_CODEX_API_KEY).toBeUndefined();
      expect(env.MO_LIVE_GEMINI_KEY).toBeUndefined();
    },
  );
  ...
  it("stages a live Anthropic setup-token profile for isolated QA workers", async () => {
    ...
    const cfg = await testing.stageQaLiveAnthropicSetupToken({
      ...
      stateDir,
      env: {
        MO_LIVE_SETUP_TOKEN_VALUE: token,
      },
    });
  ...
  it("stages the OpenAI API-key fallback for live OpenAI QA workers", async () => {
    ...
    const cfg = await testing.stageQaLiveApiKeyProfiles({
      ...
      providerIds: ["openai"],
      env: {
        MO_LIVE_OPENAI_KEY: "qa-live-codex-fallback-key",
      },
    });
  ...
  it("stages direct live OpenAI API-key aliases for isolated QA workers", async () => {
    ...
    const cfg = await testing.stageQaLiveApiKeyProfiles({
      ...
      providerIds: ["openai"],
      env: {
        MO_LIVE_CODEX_API_KEY: "qa-live-direct-codex-key",
      },
    });
  ...
  it("fails fast when live OpenAI runs have no portable QA auth", () => {
    expect(() =>
      testing.assertQaLiveCodexAuthAvailable({
        ...
        providerIds: ["openai"],
        env: {
          CODEX_HOME: path.join(os.tmpdir(), "missing-mo-codex-home"),
        },
        readCodexCredentials: () => null,
      }),
  ...
  it("stages configured OpenAI env secret refs for default OpenAI live QA runs", async () => {
    ...
    const env = {
      MO_LIVE_CODEX_API_KEY: "qa-configured-env-ref-not-a-real-key",
    };
    const cfg = await testing.stageQaLiveApiKeyProfiles({
      cfg: {
        models: {
  ...
  it("stages configured OpenAI env markers for live QA runs", async () => {
    ...
    const cfg = await testing.stageQaLiveApiKeyProfiles({
      ...
      providerIds: ["openai"],
      env: {
        MO_LIVE_CODEX_API_KEY: "qa-configured-marker-not-a-real-key",
      },
    });
  ...
  it("preserves only sanitized gateway debug artifacts", async () => {
    ...
    await writeFile(
      stdoutLogPath,
      [
        "MO_GATEWAY_TOKEN=qa-suite-token",
        'OPENAI_API_KEY="openai-live"',
        "MO_QA_CONVEX_SECRET_CI=convex-ci-secret",
        "MO_QA_CONVEX_SECRET_MAINTAINER=convex-maintainer-secret",
        "MO_LIVE_CODEX_API_KEY=codex-live-secret",
    ...
    await expect(readFile(path.join(artifactDir, "gateway.stdout.log"), "utf8")).resolves.toBe(
      [
        "MO_GATEWAY_TOKEN=<redacted>",
        "OPENAI_API_KEY=<redacted>",
        "MO_QA_CONVEX_SECRET_CI=<redacted>",
        "MO_QA_CONVEX_SECRET_MAINTAINER=<redacted>",
        "MO_LIVE_CODEX_API_KEY=<redacted>",
        "botToken=<redacted>",
        "--botToken=<redacted>",
  ...
  it("rejects preserved gateway artifacts outside the repo root", async () => {
    await expect(
      testing.assertQaArtifactDirWithinRepo("/tmp/mo-repo", "/tmp/outside"),
  ...
  it("cleans startup temp roots when they are not preserved", async () => {
    ...
    await writeFile(path.join(tempRoot, "mo.json"), "{}", "utf8");
...
describe("qa bundled plugin dir", () => {
  ...
  it("resolves bundled plugins by manifest id when the directory name differs", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "kimi-coding", "mo.plugin.json"),
  ...
  it("uses a source bundled plugin when the built copy is missing CLI metadata", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "memory-core", "mo.plugin.json"),
    ...
    await writeFile(
      path.join(repoRoot, "extensions", "memory-core", "mo.plugin.json"),
      JSON.stringify({ id: "memory-core", kind: "memory" }),
      "utf8",
    );
    await writeFile(
  ...
  it("creates a scoped bundled plugin tree for allowed plugins plus always-allowed runtime facades", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "qa-channel", "package.json"),
      JSON.stringify({ name: "@mo/qa-channel", type: "module" }, null, 2),
      "utf8",
    );
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "qa-channel", "index.js"),
      [
        'import { normalizeAccountId } from "mo/plugin-sdk/account-id";',
  ...
  it("preserves dist-runtime-only root chunks when dist also exists", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ name: "@mo/runtime-only", type: "module" }, null, 2),
      "utf8",
    );
    await writeFile(
  ...
  it("maps cli backend provider ids to their owning bundled plugin ids", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "openai", "mo.plugin.json"),
      JSON.stringify({
        id: "openai",
  ...
  it("copies selected live provider configs from the host config", async () => {
    ...
    const overrides = await testing.readQaLiveProviderConfigOverrides({
      providerIds: ["custom-openai"],
      env: { MO_QA_LIVE_PROVIDER_CONFIG_PATH: configPath },
    });
  ...
  it("raises the QA runtime host version to the highest allowed plugin floor", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ mo: { install: { minHostVersion: ">=2026.4.8" } } }),
      "utf8",
    );
    ...
    await writeFile(
      ...
      JSON.stringify({ mo: { install: { minHostVersion: ">=2026.4.7" } } }),
      "utf8",
    );
  ...
  it("includes always-allowed runtime facade plugins when raising the QA runtime host version", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ mo: { install: { minHostVersion: ">=2026.4.9" } } }),
      "utf8",
    );
