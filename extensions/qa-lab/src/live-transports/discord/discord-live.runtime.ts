...
import {
  ...
  requestDiscord,
} from "@mo/discord/api.js";
import { DEFAULT_EMOJIS } from "mo/plugin-sdk/channel-feedback";
import type { OpenClawConfig } from "mo/plugin-sdk/config-contracts";
import { formatErrorMessage } from "mo/plugin-sdk/error-runtime";
import { writeExternalFileWithinRoot } from "mo/plugin-sdk/security-runtime";
import { uniqueStrings } from "mo/plugin-sdk/string-coerce-runtime";
...
const DISCORD_QA_CAPTURE_CONTENT_ENV = "MO_QA_DISCORD_CAPTURE_CONTENT";
const DISCORD_QA_CAPTURE_UI_METADATA_ENV = "MO_QA_DISCORD_CAPTURE_UI_METADATA";
const DISCORD_QA_KEEP_THREADS_ENV = "MO_QA_DISCORD_KEEP_THREADS";
const QA_REDACT_PUBLIC_METADATA_ENV = "MO_QA_REDACT_PUBLIC_METADATA";
const DISCORD_QA_ENV_KEYS = [
  "MO_QA_DISCORD_GUILD_ID",
  "MO_QA_DISCORD_CHANNEL_ID",
  "MO_QA_DISCORD_DRIVER_BOT_TOKEN",
  "MO_QA_DISCORD_SUT_BOT_TOKEN",
  "MO_QA_DISCORD_SUT_APPLICATION_ID",
] as const;
...
function resolveDiscordQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): DiscordQaRuntimeEnv {
  const voiceChannelId = env.MO_QA_DISCORD_VOICE_CHANNEL_ID?.trim();
  const runtimeEnv = {
    guildId: resolveEnvValue(env, "MO_QA_DISCORD_GUILD_ID"),
    channelId: resolveEnvValue(env, "MO_QA_DISCORD_CHANNEL_ID"),
    driverBotToken: resolveEnvValue(env, "MO_QA_DISCORD_DRIVER_BOT_TOKEN"),
    sutBotToken: resolveEnvValue(env, "MO_QA_DISCORD_SUT_BOT_TOKEN"),
    sutApplicationId: resolveEnvValue(env, "MO_QA_DISCORD_SUT_APPLICATION_ID"),
    ...(voiceChannelId ? { voiceChannelId } : {}),
  };
  validateDiscordQaRuntimeEnv(runtimeEnv, "MO_QA_DISCORD");
  return runtimeEnv;
}
...
async function resolveDiscordQaVoiceChannel(params: {
  ...
  if (!first) {
    throw new Error(
      "Discord voice auto-join scenario could not find a visible voice/stage channel for the SUT bot. Add voiceChannelId to the Convex discord credential payload or set MO_QA_DISCORD_VOICE_CHANNEL_ID.",
    );
  }
  return first;
}
