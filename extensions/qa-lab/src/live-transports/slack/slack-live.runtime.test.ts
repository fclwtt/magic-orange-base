...
describe("Slack live QA runtime helpers", () => {
  ...
  it("resolves env credential payloads", () => {
    expect(
      testing.resolveSlackQaRuntimeEnv({
        MO_QA_SLACK_CHANNEL_ID: "C123456789",
        MO_QA_SLACK_DRIVER_BOT_TOKEN: "xoxb-driver",
        MO_QA_SLACK_SUT_BOT_TOKEN: "xoxb-sut",
        MO_QA_SLACK_SUT_APP_TOKEN: "xapp-sut",
      }),
    ).toEqual({
  ...
  it("rejects malformed Slack channel ids", () => {
    expect(() =>
      testing.resolveSlackQaRuntimeEnv({
        MO_QA_SLACK_CHANNEL_ID: "qa-channel",
        MO_QA_SLACK_DRIVER_BOT_TOKEN: "xoxb-driver",
        MO_QA_SLACK_SUT_BOT_TOKEN: "xoxb-sut",
        MO_QA_SLACK_SUT_APP_TOKEN: "xapp-sut",
      }),
    ).toThrow("MO_QA_SLACK channelId must be a Slack id like C123 or U123.");
  });
  ...
  it("resolves Slack approval checkpoint configuration from env", () => {
    expect(
      testing.resolveSlackApprovalCheckpointConfig({
        MO_QA_SLACK_APPROVAL_CHECKPOINT_DIR: "/tmp/checkpoints",
        MO_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS: "5000",
      }),
    ).toEqual({
  ...
  it("writes artifacts when Convex credential acquisition fails", async () => {
    const outputDir = await fs.mkdtemp(path.join(tmpdir(), "mo-slack-qa-"));
    ...
    expect(result.scenarios[0]?.details).toContain("Missing MO_QA_CONVEX_SITE_URL");
    ...
    expect(summary.entries[0]).toMatchObject({
      ...
      result: {
        status: "fail",
        failure: {
          reason: expect.stringContaining("Missing MO_QA_CONVEX_SITE_URL"),
        },
      },
    });
  });
