...
import { resolveTimerTimeoutMs } from "mo/plugin-sdk/number-runtime";
import { z } from "zod";
import {
...
const CHUNKED_PAYLOAD_MAX_BYTES_ENV = "MO_QA_CREDENTIAL_PAYLOAD_MAX_BYTES";
const CHUNKED_PAYLOAD_MAX_CHUNKS_ENV = "MO_QA_CREDENTIAL_PAYLOAD_MAX_CHUNKS";
...
const CHUNKED_PAYLOAD_MARKER = "__moQaCredentialPayloadChunksV1";
...
function normalizeEndpointPrefix(value: string | undefined): string {
  return normalizeQaCredentialEndpointPrefix({
    ...
    invalidAbsoluteMessage:
      "MO_QA_CONVEX_ENDPOINT_PREFIX must be an absolute path like /qa-credentials/v1.",
    invalidSegmentsMessage:
      "MO_QA_CONVEX_ENDPOINT_PREFIX must not contain backslashes or .. path segments.",
  });
}
...
function resolveConvexAuthToken(env: NodeJS.ProcessEnv, role: QaCredentialRole): string {
  const roleToken =
    role === "ci"
      ? env.MO_QA_CONVEX_SECRET_CI?.trim()
      : env.MO_QA_CONVEX_SECRET_MAINTAINER?.trim();
  const token = roleToken;
  if (token) {
    return token;
  }
  if (role === "ci") {
    throw new Error("Missing MO_QA_CONVEX_SECRET_CI for CI credential access.");
  }
  throw new Error("Missing MO_QA_CONVEX_SECRET_MAINTAINER for maintainer credential access.");
}
...
function resolveConvexCredentialBrokerConfig(params: {
  ...
  const siteUrl = params.env.MO_QA_CONVEX_SITE_URL?.trim();
  if (!siteUrl) {
    throw new Error("Missing MO_QA_CONVEX_SITE_URL for --credential-source convex.");
  }
  const baseUrl = normalizeConvexSiteUrl(siteUrl, params.env);
  const endpointPrefix = normalizeEndpointPrefix(params.env.MO_QA_CONVEX_ENDPOINT_PREFIX);
  const ownerId =
    params.ownerId?.trim() ||
    params.env.MO_QA_CREDENTIAL_OWNER_ID?.trim() ||
  ...
  return {
    ...
    leaseTtlMs: parsePositiveIntegerEnv(
      params.env,
      "MO_QA_CREDENTIAL_LEASE_TTL_MS",
      DEFAULT_LEASE_TTL_MS,
    ),
    heartbeatIntervalMs: parsePositiveIntegerEnv(
      params.env,
      "MO_QA_CREDENTIAL_HEARTBEAT_INTERVAL_MS",
      DEFAULT_HEARTBEAT_INTERVAL_MS,
    ),
    acquireTimeoutMs: parsePositiveIntegerEnv(
      params.env,
      "MO_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS",
      DEFAULT_ACQUIRE_TIMEOUT_MS,
    ),
    httpTimeoutMs: parsePositiveIntegerEnv(
      params.env,
      "MO_QA_CREDENTIAL_HTTP_TIMEOUT_MS",
...
export async function acquireQaCredentialLease<TPayload>(
  ...
  const source = normalizeQaCredentialSource(opts.source ?? env.MO_QA_CREDENTIAL_SOURCE);
  if (source === "env") {
    return {
      source: "env",
      kind: opts.kind,
  ...
  const role = normalizeQaCredentialRole(opts.role ?? env.MO_QA_CREDENTIAL_ROLE, env);
