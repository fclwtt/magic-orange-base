...
import { isRecord, uniqueStrings } from "mo/plugin-sdk/string-coerce-runtime";
import { z } from "zod";
...
const TELEGRAM_QA_ENV_KEYS = [
  "MO_QA_TELEGRAM_GROUP_ID",
  "MO_QA_TELEGRAM_DRIVER_BOT_TOKEN",
  "MO_QA_TELEGRAM_SUT_BOT_TOKEN",
] as const;
const QA_REDACT_PUBLIC_METADATA_ENV = "MO_QA_REDACT_PUBLIC_METADATA";
const QA_SUITE_PROGRESS_ENV = "MO_QA_SUITE_PROGRESS";
...
function resolveTelegramQaCanaryTimeoutMs(env: NodeJS.ProcessEnv = process.env) {
  return parsePositiveTelegramQaEnvMs(
    env,
    "MO_QA_TELEGRAM_CANARY_TIMEOUT_MS",
...
function resolveTelegramQaScenarioTimeoutMs(
  ...
  return parsePositiveTelegramQaEnvMs(env, "MO_QA_TELEGRAM_SCENARIO_TIMEOUT_MS", fallbackMs);
}
...
function resolveTelegramQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): TelegramQaRuntimeEnv {
  const groupId = resolveEnvValue(env, "MO_QA_TELEGRAM_GROUP_ID");
  if (!/^-?\d+$/u.test(groupId)) {
    throw new Error("MO_QA_TELEGRAM_GROUP_ID must be a numeric Telegram chat id.");
  }
  return {
    groupId,
    driverToken: resolveEnvValue(env, "MO_QA_TELEGRAM_DRIVER_BOT_TOKEN"),
    sutToken: resolveEnvValue(env, "MO_QA_TELEGRAM_SUT_BOT_TOKEN"),
  };
}
