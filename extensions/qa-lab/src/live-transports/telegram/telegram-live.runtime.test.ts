...
vi.mock("mo/plugin-sdk/ssrf-runtime", async () => {
  const actual = await vi.importActual<typeof import("mo/plugin-sdk/ssrf-runtime")>(
    "mo/plugin-sdk/ssrf-runtime",
  );
  return {
    ...actual,
...
describe("telegram live qa runtime", () => {
  ...
  it("resolves required Telegram QA env vars", () => {
    expect(
      testing.resolveTelegramQaRuntimeEnv({
        MO_QA_TELEGRAM_GROUP_ID: "-100123",
        MO_QA_TELEGRAM_DRIVER_BOT_TOKEN: "driver",
        MO_QA_TELEGRAM_SUT_BOT_TOKEN: "sut",
      }),
    ).toEqual({
      groupId: "-100123",
      driverToken: "driver",
  ...
  it("fails when a required Telegram QA env var is missing", () => {
    ...
    ).toThrow("MO_QA_TELEGRAM_SUT_BOT_TOKEN");
  });

  it("fails when the Telegram group id is not numeric", () => {
    expect(() =>
      testing.resolveTelegramQaRuntimeEnv({
        MO_QA_TELEGRAM_GROUP_ID: "qa-group",
        MO_QA_TELEGRAM_DRIVER_BOT_TOKEN: "driver",
        MO_QA_TELEGRAM_SUT_BOT_TOKEN: "sut",
      }),
    ).toThrow("MO_QA_TELEGRAM_GROUP_ID must be a numeric Telegram chat id.");
  });
  ...
  it("applies MO_QA_SUITE_PROGRESS override to Telegram live logging", () => {
    expect(
      testing.shouldLogTelegramQaLiveProgress({
        CI: "false",
        MO_QA_SUITE_PROGRESS: "true",
      }),
    ).toBe(true);
    expect(
      testing.shouldLogTelegramQaLiveProgress({
        CI: "true",
        MO_QA_SUITE_PROGRESS: "false",
      }),
    ).toBe(false);
    expect(
      testing.shouldLogTelegramQaLiveProgress({
        CI: "true",
        MO_QA_SUITE_PROGRESS: "definitely",
      }),
    ).toBe(true);
  });
  ...
  it("normalizes the Telegram QA transport ready timeout env", () => {
    ...
    expect(
      testing.resolveTelegramQaReadyTimeoutMs({
        MO_QA_TRANSPORT_READY_TIMEOUT_MS: "180000",
      }),
    ).toBe(180_000);
    expect(
      testing.resolveTelegramQaReadyTimeoutMs({
        MO_QA_TRANSPORT_READY_TIMEOUT_MS: "bad",
      }),
    ).toBe(45_000);
    for (const value of ["0x10", "1e3", "10.5"]) {
      expect(
        testing.resolveTelegramQaReadyTimeoutMs({
          MO_QA_TRANSPORT_READY_TIMEOUT_MS: value,
        }),
      ).toBe(45_000);
    }
  });
  ...
  it("normalizes the Telegram QA canary timeout env", () => {
    ...
    expect(
      testing.resolveTelegramQaCanaryTimeoutMs({
        MO_QA_TELEGRAM_CANARY_TIMEOUT_MS: "90000",
      }),
    ).toBe(90_000);
    expect(
      testing.resolveTelegramQaCanaryTimeoutMs({
        MO_QA_TELEGRAM_CANARY_TIMEOUT_MS: "nope",
      }),
    ).toBe(30_000);
    for (const value of ["0x10", "1e3", "10.5"]) {
      expect(
        testing.resolveTelegramQaCanaryTimeoutMs({
          MO_QA_TELEGRAM_CANARY_TIMEOUT_MS: value,
        }),
      ).toBe(30_000);
    }
  });
  ...
  it("normalizes the Telegram QA scenario timeout env", () => {
    ...
    expect(
      testing.resolveTelegramQaScenarioTimeoutMs(45_000, {
        MO_QA_TELEGRAM_SCENARIO_TIMEOUT_MS: "180000",
      }),
    ).toBe(180_000);
    expect(
      testing.resolveTelegramQaScenarioTimeoutMs(45_000, {
        MO_QA_TELEGRAM_SCENARIO_TIMEOUT_MS: "nope",
      }),
    ).toBe(45_000);
    for (const value of ["0x10", "1e3", "10.5"]) {
      expect(
        testing.resolveTelegramQaScenarioTimeoutMs(45_000, {
          MO_QA_TELEGRAM_SCENARIO_TIMEOUT_MS: value,
        }),
      ).toBe(45_000);
    }
  });
  ...
  it("lists default status and regression refs in the Telegram scenario catalog", () => {
    ...
    expect(status.regressionRefs).toEqual(["mo/mo#74698"]);
    ...
    expect(usageFooter.regressionRefs).toEqual(["mo/mo#87392"]);
    ...
    expect(streamSingle.regressionRefs).toEqual(["mo/mo#39905"]);
