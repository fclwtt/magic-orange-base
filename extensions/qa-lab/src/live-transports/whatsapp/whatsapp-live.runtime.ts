...
} from "@mo/whatsapp/api.js";
import { normalizeE164 } from "mo/plugin-sdk/account-resolution";
import type { OpenClawConfig } from "mo/plugin-sdk/config-contracts";
import { formatErrorMessage } from "mo/plugin-sdk/error-runtime";
import { normalizeStringEntries, uniqueStrings } from "mo/plugin-sdk/string-coerce-runtime";
...
const WHATSAPP_QA_CAPTURE_CONTENT_ENV = "MO_QA_WHATSAPP_CAPTURE_CONTENT";
const QA_REDACT_PUBLIC_METADATA_ENV = "MO_QA_REDACT_PUBLIC_METADATA";
...
const WHATSAPP_QA_ENV_KEYS = [
  "MO_QA_WHATSAPP_DRIVER_PHONE_E164",
  "MO_QA_WHATSAPP_SUT_PHONE_E164",
  "MO_QA_WHATSAPP_DRIVER_AUTH_ARCHIVE_BASE64",
  "MO_QA_WHATSAPP_SUT_AUTH_ARCHIVE_BASE64",
] as const;
...
function buildWhatsAppQuoteReplyRun(target: "dm" | "group"): WhatsAppQaMessageScenarioRun {
  ...
  const input =
    target === "group"
      ? `moqa reply with only this exact marker: ${token}`
  ...
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        configMode: "allowlist",
        expectReply: true,
        input: `moqa reply with only this exact marker: ${replyToken}`,
  ...
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        ...
        input:
          `moqa pending history context check ${triggerMarker}. ` +
  ...
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        ...
        configMode: "open",
        expectReply: true,
        input: `moqa broadcast fanout check ${token}`,
  ...
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        ...
        configMode: "allowlist",
        expectReply: true,
        input: `moqa Mentioned group seed marker ${seedMarker}`,
  ...
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        ...
        expectReply: false,
        input:
          `moqa react to this WhatsApp group message with thumbs up for QA action check ${token}. ` +
          "Do not send any visible text reply after the reaction.",
        matchText: token,
        quietWindowMs: 8_000,
        target: "group",
  ...
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        ...
        expectReply: false,
        input:
          `moqa use the WhatsApp message tool upload-file action to send a PNG with caption ${token}. ` +
          "Do not send any visible text reply after the upload.",
        matchText: token,
        quietWindowMs: 8_000,
        target: "group",
  ...
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        ...
        configMode: "allowlist",
        expectReply: true,
        input: `moqa reply with only this exact marker before group outbound media checks: ${token}`,
        matchText: token,
        target: "group",
      };
    },
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        ...
        configMode: "allowlist",
        expectReply: true,
        input: `moqa reply with only this exact marker before group outbound audio check: ${token}`,
        matchText: token,
        target: "group",
      };
    },
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        ...
        configMode: "allowlist",
        expectReply: true,
        input: `moqa reply with only this exact marker before group outbound poll check: ${token}`,
        matchText: token,
        target: "group",
      };
    },
  ...
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        configMode: "allowlist",
        expectReply: true,
        input: `moqa reply with only this exact marker under groupPolicy open: ${token}`,
        matchText: token,
        target: "group",
      };
    },
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        configMode: "allowlist",
        expectReply: false,
        input: `moqa groupPolicy disabled must not reply with ${token}`,
        matchText: token,
        target: "group",
      };
    },
  ...
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        configMode: "allowlist",
        expectReply: false,
        input: `moqa blocked group should not reply with ${quietToken}`,
...
function resolveWhatsAppQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): WhatsAppQaRuntimeEnv {
  return validateWhatsAppQaRuntimeEnv(
    {
      driverPhoneE164: resolveEnvValue(env, "MO_QA_WHATSAPP_DRIVER_PHONE_E164"),
      sutPhoneE164: resolveEnvValue(env, "MO_QA_WHATSAPP_SUT_PHONE_E164"),
      driverAuthArchiveBase64: resolveEnvValue(
        env,
        "MO_QA_WHATSAPP_DRIVER_AUTH_ARCHIVE_BASE64",
      ),
      sutAuthArchiveBase64: resolveEnvValue(env, "MO_QA_WHATSAPP_SUT_AUTH_ARCHIVE_BASE64"),
      groupJid: env.MO_QA_WHATSAPP_GROUP_JID?.trim() || undefined,
    },
    "MO_QA_WHATSAPP",
  );
}
...
async function writeWhatsAppQaWorkspaceFixture(
  ...
  const fixtureDir = path.join(context.gatewayWorkspaceDir, ".mo", "qa-whatsapp-media");
...
export async function runWhatsAppQaLive(params: {
  ...
  try {
    ...
    tempAuthRoot = await fs.mkdtemp(
      path.join(resolvePreferredOpenClawTmpDir(), "mo-whatsapp-qa-"),
    );
