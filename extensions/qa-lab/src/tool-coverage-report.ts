...
function countRuntimeToolCalls(
  ...
  const cell = runtime === "mo" ? result.cells.mo : result.cells.codex;
...
function buildRow(params: {
  ...
  return {
    ...
    mo: result ? cellStatus(result.cells.mo) : "not-run",
    codex: result ? cellStatus(result.cells.codex) : "not-run",
    drift: result?.drift ?? "not-run",
    moToolCalls: countRuntimeToolCalls(result, "mo", runtimeToolName),
...
function coverageFailureForRow(row: QaToolCoverageRow): string | undefined {
  ...
  if (row.mo !== "pass" || row.codex !== "pass") {
    return `${row.tool} status mo=${row.mo} codex=${row.codex}`;
  }
  if (row.drift === "failure-mode") {
    return `${row.tool} drift=failure-mode${row.details ? ` (${row.details})` : ""}`;
  }
  if (row.runtimeToolName && row.moToolCalls === 0) {
    return `${row.tool} missing mo tool call ${row.runtimeToolName}`;
  }
...
export function buildQaToolCoverageReport(params: {
  ...
  return {
    ...
    dynamicIntegrationTools: rows.filter((row) => row.bucket === "mo-dynamic-integration")
      .length,
    searchableDynamicTools: rows.filter(
      (row) => row.capabilityLayer === "mo-dynamic-searchable",
    ).length,
