...
describe("qa docker harness", () => {
  it("writes compose, env, config, and workspace scaffold files", async () => {
    ...
    for (const expectedFile of [
      ...
      path.join(outputDir, "state", "mo.json"),
    ...
    expect(compose).toContain("--ui-dist-dir /opt/mo-qa-lab-ui");
    expect(compose).toContain(":/opt/mo-repo:ro");
    expect(compose).toContain("./state:/opt/mo-scaffold:ro");
    expect(compose).toContain(
      "cp -R /opt/mo-scaffold/seed-workspace/. /tmp/mo/workspace/ && rm -rf /tmp/mo/workspace/repo && ln -s /opt/mo-repo /tmp/mo/workspace/repo",
    );
    expect(compose).toContain("MO_CONFIG_PATH: /tmp/mo/mo.json");
    expect(compose).toContain("MO_STATE_DIR: /tmp/mo/state");
    expect(compose).toContain('MO_NO_RESPAWN: "1"');

    const envExample = await readFile(path.join(outputDir, ".env.example"), "utf8");
    expect(envExample).toContain("MO_GATEWAY_TOKEN=qa-token");
    ...
    const configText = await readFile(path.join(outputDir, "state", "mo.json"), "utf8");
    ...
    expect(configText).toContain('"/tmp/mo/workspace"');
  ...
  it("builds the reusable QA image with bundled QA extensions", async () => {
    ...
    expect(result.imageName).toBe("mo:qa-local-prebaked");
    expect(calls).toEqual([
      "docker build -t mo:qa-local-prebaked --build-arg MO_EXTENSIONS=qa-channel qa-lab -f Dockerfile . @/repo/mo",
    ]);
  });
  ...
  it("quotes generated compose paths so shell-sensitive repo paths survive YAML parsing", async () => {
    ...
    expect(services["qa-lab"]?.volumes).toContain(
      "../repo #hash/extensions/qa-lab/web/dist:/opt/mo-qa-lab-ui:ro",
    );
    expect(services["mo-qa-gateway"]?.volumes).toContain(
      "../repo #hash:/opt/mo-repo:ro",
    );
  });
});
