...
describe("qa scenario catalog", () => {
  ...
  it("loads runtime tool fixture metadata for standard and optional lanes", () => {
    ...
    expect(readQaScenarioExecutionConfig(webSearch.id)).toMatchObject({
      toolName: "web_search",
      toolCoverage: {
        bucket: "mo-dynamic-integration",
        expectedLayer: "mo-dynamic",
        capabilityLayer: "mo-dynamic-direct",
        required: true,
      },
    });
  ...
  it("loads the opt-in update.run package self-upgrade sentinel", () => {
    ...
    expect(config?.allowEnv).toBe("MO_QA_ALLOW_UPDATE_RUN_SELF");
  ...
  it("includes the Kitchen Sink live OpenAI plugin gauntlet", () => {
    ...
    expect(config?.requiredProviderMode).toBe("live-frontier");
    expect(config?.requiredProvider).toBe("openai");
    expect(config?.pluginSpec).toBe("npm:@mo/kitchen-sink@latest");
    expect(config?.pluginId).toBe("mo-kitchen-sink-fixture");
