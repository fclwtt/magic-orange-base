...
export const QA_EVIDENCE_SUMMARY_KIND = "mo.qa.evidence-summary";
...
export function resolveQaEvidenceProfile(params: {
  ...
  const envProfiles = [
    ["MO_E2E_PROFILE", params.env?.MO_E2E_PROFILE],
    ["MO_QA_PROFILE", params.env?.MO_QA_PROFILE],
  ] as const;
...
function resolveQaEvidenceRunner(params: { env?: NodeJS.ProcessEnv; fallback?: string }) {
  return params.env?.MO_QA_RUNNER?.trim() || params.fallback || "host";
}

function resolveQaEvidenceChannelDriver(params: { env?: NodeJS.ProcessEnv; fallback?: string }) {
  const id =
    params.fallback?.trim() ||
    params.env?.MO_QA_CHANNEL_DRIVER?.trim() ||
    params.env?.MO_E2E_CHANNEL_DRIVER?.trim();
...
function resolveQaEvidencePackageSource(env: NodeJS.ProcessEnv | undefined) {
  const spec = env?.MO_QA_PACKAGE_SOURCE?.trim() || undefined;
  const sha = env?.MO_QA_PACKAGE_SOURCE_SHA?.trim() || undefined;
  const explicitKind = env?.MO_QA_PACKAGE_SOURCE_KIND?.trim();
  const kind =
    explicitKind ||
