...
import type { QaRunnerCliContribution } from "mo/plugin-sdk/qa-runner-runtime";
...
vi.mock("mo/plugin-sdk/qa-runner-runtime", () => ({
...
describe("qa cli registration", () => {
  ...
  it.each([
    ...
    await expect(
      program.parseAsync(["node", "mo", "qa", "run", flag, ...values]),
  ...
  it("rejects an empty qa run --qa-profile instead of falling back to self-check", async () => {
    await expect(
      program.parseAsync(["node", "mo", "qa", "run", "--qa-profile", ""]),
  ...
  it("registers standalone provider server commands from the provider registry", async () => {
    ...
    await program.parseAsync(["node", "mo", "qa", "aimock", "--port", "44080"]);
  ...
  it("normalizes signed decimal QA numeric option values through the shared parser", async () => {
    await program.parseAsync(["node", "mo", "qa", "aimock", "--port", "+044080"]);
  ...
  it.each([
    ...
    await expect(invalidProgram.parseAsync(["node", "mo", ...args])).rejects.toThrow(message);
  });
  ...
  it("shows an enable hint when a discovered runner plugin is installed but blocked", async () => {
    ...
    await expect(
      blockedProgram.parseAsync(["node", "mo", "qa", TEST_QA_RUNNER.commandName]),
  ...
  it("routes telegram CLI defaults into the lane runtime", async () => {
    await program.parseAsync(["node", "mo", "qa", "telegram"]);
  ...
  it("forwards --list-scenarios for telegram runs", async () => {
    await program.parseAsync(["node", "mo", "qa", "telegram", "--list-scenarios"]);
  ...
  it("forwards --allow-failures for telegram runs", async () => {
    await program.parseAsync(["node", "mo", "qa", "telegram", "--allow-failures"]);
  ...
  it("forwards --allow-failures for suite runs", async () => {
    await program.parseAsync(["node", "mo", "qa", "suite", "--allow-failures"]);
  ...
  it("forwards --pack for suite runs", async () => {
    await program.parseAsync(["node", "mo", "qa", "suite", "--pack", "personal-agent"]);
