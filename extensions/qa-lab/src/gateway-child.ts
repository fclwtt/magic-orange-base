...
const QA_GATEWAY_CHILD_BLOCKED_SECRET_ENV_VARS = Object.freeze([
  "MO_QA_CONVEX_SECRET_CI",
  "MO_QA_CONVEX_SECRET_MAINTAINER",
]);
...
export function buildQaRuntimeEnv(params: {
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ...(provider?.appliesLiveEnvAliases
      ...
      : {}),
    MO_HOME: params.homeDir,
    MO_CONFIG_PATH: params.configPath,
    MO_STATE_DIR: params.stateDir,
    MO_OAUTH_DIR: path.join(params.stateDir, "credentials"),
    ...
    MO_NO_RESPAWN: "1",
    MO_TEST_FAST: "1",
    MO_EMBEDDED_ABORT_SETTLE_TIMEOUT_MS: "2000",
    MO_QA_PARENT_PID: String(process.pid),
    MO_QA_TEMP_ROOT: params.tempRoot,
    ...(params.stagedBundledPluginsRoot
      ? { MO_QA_STAGED_RUNTIME_ROOT: params.stagedBundledPluginsRoot }
      : {}),
    MO_QA_ALLOW_LOCAL_IMAGE_PROVIDER: "1",
    ...
    MO_ALLOW_SLOW_REPLY_TESTS: "1",
    XDG_CONFIG_HOME: params.xdgConfigHome,
    XDG_DATA_HOME: params.xdgDataHome,
    XDG_CACHE_HOME: params.xdgCacheHome,
    ...(params.bundledPluginsDir ? { MO_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir } : {}),
    ...(params.compatibilityHostVersion
      ? { MO_COMPATIBILITY_HOST_VERSION: params.compatibilityHostVersion }
      : {}),
  };
...
export async function startQaGatewayChild(params: {
  ...
  const tempRoot = await fs.mkdtemp(
    path.join(resolvePreferredOpenClawTmpDir(), "mo-qa-suite-"),
  );
  ...
  const configPath = path.join(tempRoot, "mo.json");
  ...
  const keepTemp = process.env.MO_QA_KEEP_TEMP === "1";
  let gatewayPort = 0;
  let baseUrl = "";
  let wsUrl = "";
