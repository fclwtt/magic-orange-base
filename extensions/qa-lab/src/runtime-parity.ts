...
import { fetchWithSsrFGuard } from "mo/plugin-sdk/ssrf-runtime";
import {
...
export type RuntimeId = "mo" | "codex";
...
export function isRuntimeParityResultPass(result: RuntimeParityResult) {
  return (
    ...
    isRuntimeParityCellPassable(result.cells.mo) &&
...
function classifyRuntimeParityCells(params: {
  mo: RuntimeParityCell;
  codex: RuntimeParityCell;
  moScenarioStatus: "pass" | "fail";
  codexScenarioStatus: "pass" | "fail";
}): Pick<RuntimeParityResult, "drift" | "driftDetails"> {
  if (
    isHardFailureRuntimeError(params.mo.runtimeErrorClass) ||
    isHardFailureRuntimeError(params.codex.runtimeErrorClass) ||
    params.mo.transportErrorClass ||
    params.codex.transportErrorClass
  ) {
    return {
      drift: "failure-mode",
      driftDetails:
        params.mo.transportErrorClass || params.codex.transportErrorClass
  ...
  if (
    hasMissingToolResult(params.mo.toolCalls) ||
  ...
  if (
    params.moScenarioStatus === "fail" ||
    params.codexScenarioStatus === "fail" ||
    !isRuntimeParityCellPassable(params.mo) ||
    !isRuntimeParityCellPassable(params.codex)
  ) {
    return {
      drift: "failure-mode",
      driftDetails:
        params.moScenarioStatus === params.codexScenarioStatus
          ? "at least one runtime failed"
          : `scenario status differs (${params.moScenarioStatus} vs ${params.codexScenarioStatus})`,
    };
  }
  ...
  const moTranscriptLines = params.mo.transcriptBytes.trim().length
    ? params.mo.transcriptBytes.trim().split(/\r?\n/u).length
    : 0;
  const codexTranscriptLines = params.codex.transcriptBytes.trim().length
    ? params.codex.transcriptBytes.trim().split(/\r?\n/u).length
    : 0;
  if (
    moTranscriptLines !== codexTranscriptLines ||
    (!params.mo.finalText && Boolean(params.codex.finalText)) ||
    (Boolean(params.mo.finalText) && !params.codex.finalText)
  ) {
    return {
      drift: "structural",
      driftDetails: `transcript/final-text structure differs (${moTranscriptLines} lines vs ${codexTranscriptLines})`,
    };
  }

  if (
    normalizeTextForParity(params.mo.finalText) ===
