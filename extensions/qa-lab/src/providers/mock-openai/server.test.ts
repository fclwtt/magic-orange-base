...
const WHATSAPP_GROUP_AGENT_REACT_PROMPT =
  "moqa react to this WhatsApp group message with thumbs up for QA action check WHATSAPP_QA_GROUP_AGENT_REACT_TEST.";
...
const WHATSAPP_GROUP_AGENT_UPLOAD_PROMPT =
  `moqa use the WhatsApp message tool upload-file action to send a PNG with caption ${WHATSAPP_GROUP_AGENT_UPLOAD_TOKEN}. ` +
  "Do not send any visible text reply after the upload.";
...
const WHATSAPP_PENDING_HISTORY_TRIGGER_PROMPT = [
  "moqa pending history context check",
...
const WHATSAPP_BROADCAST_PROMPT = `moqa broadcast fanout check ${WHATSAPP_BROADCAST_TOKEN}`;
...
describe("qa mock openai server", () => {
  ...
  it("answers WhatsApp pending-history prompts only with injected prior group context", async () => {
    ...
    const contextWithoutCurrentTrigger = await expectResponsesJson(server, {
      ...
      input: [
        makeUserInput(
          [historyContext, "moqa pending history context check without current trigger"].join(
            "\n",
          ),
        ),
      ],
  ...
  it("answers reply-to-bot seed and implicit quoted-trigger markers deterministically", async () => {
    ...
    expect(WHATSAPP_REPLY_TO_BOT_TRIGGER_PROMPT).not.toMatch(/\bmoqa\b/iu);
  ...
  it("supports advanced QA memory and subagent recovery prompts", async () => {
    ...
    const threadMemorySearch = await fetch(`${server.baseUrl}/v1/responses`, {
      ...
      body: JSON.stringify({
        stream: true,
        instructions:
          "@mo Thread memory check: what is the hidden thread codename stored only in memory? Use memory tools first and reply only in this thread.",
        input: [
          {
            role: "user",
            content: [
      ...
      input: [
        ...
        ),
        {
          ...
          output: JSON.stringify({
            ...
            value: {
              tool: {
                id: `mo:tool-search-e2e-fixture:${targetTool}`,
                source: "mo",
  ...
  it("plans QA tool-search failure calls with denied-input args", async () => {
    ...
    expect(toolPlanOutput.type).toBe("function_call");
    expect(toolPlanOutput.name).toBe("web_search");
    expect(String(toolPlanOutput.arguments)).toContain("MO_QA_WEB_SEARCH_DENIED_INPUT");
  });
  ...
  it("serves deterministic WhatsApp group audio transcription for the trigger fixture", async () => {
    ...
    await expect(triggered.json()).resolves.toEqual({
      text: "moqa reply with only this exact marker after group audio preflight: WHATSAPP_QA_GROUP_AUDIO_TRANSCRIPT_OK",
    });
