...
import { escapeRegExp } from "mo/plugin-sdk/text-utility-runtime";
import { readRequestBodyWithLimit } from "mo/plugin-sdk/webhook-ingress";
...
const QA_WHATSAPP_BROADCAST_PROMPT_RE = /\bmoqa broadcast fanout check\s+([A-Z0-9_]+)\b/i;
...
const QA_GROUP_AUDIO_TRANSCRIPTION_TEXT =
  "moqa reply with only this exact marker after group audio preflight: WHATSAPP_QA_GROUP_AUDIO_TRANSCRIPT_OK";
const QA_GROUP_AUDIO_TRIGGER_SENTINEL = "MO_QA_GROUP_AUDIO_TRIGGER";
  ...
  if (
    ...
    if (targetTool && hasDeclaredTool(body, "tool_search_code")) {
      return buildToolCallEventsWithArgs("tool_search_code", {
        code: [
          `const hits = await mo.tools.search(${JSON.stringify(targetTool)}, { limit: 1 });`,
          "const match = hits.find((tool) => tool.name === " + JSON.stringify(targetTool) + ");",
          "if (!match) throw new Error('target tool not found');",
          `return await mo.tools.call(match.id, ${JSON.stringify(plannedArgs)});`,
        ].join("\n"),
      });
    }
