...
} from "mo/plugin-sdk/provider-auth";
...
export const QA_LIVE_ANTHROPIC_SETUP_TOKEN_ENV = "MO_QA_LIVE_ANTHROPIC_SETUP_TOKEN";
export const QA_LIVE_SETUP_TOKEN_VALUE_ENV = "MO_LIVE_SETUP_TOKEN_VALUE";
const QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE_ENV = "MO_QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE";
...
const QA_LIVE_API_KEY_ALIASES: Readonly<Record<string, readonly string[]>> = Object.freeze({
  anthropic: ["MO_LIVE_ANTHROPIC_KEY"],
  gemini: ["MO_LIVE_GEMINI_KEY"],
  openai: [
    "CODEX_API_KEY",
    "MO_LIVE_CODEX_API_KEY",
    "MO_LIVE_OPENAI_KEY",
    "OPENAI_API_KEY",
  ],
});
...
function qaLiveRequiresCodexAuth(params: {
  ...
  const forcedRuntime = params.env.MO_QA_FORCE_RUNTIME?.trim().toLowerCase();
  if (forcedRuntime === "mo") {
    return false;
  }
...
export function assertQaLiveCodexAuthAvailable(params: {
  ...
  throw new Error(
    [
      ...
      "Set OPENAI_API_KEY or MO_LIVE_OPENAI_KEY for an API-key fallback, or set CODEX_HOME to a logged-in Codex CLI home.",
