...
vi.mock("mo/plugin-sdk/gateway-runtime", () => ({
...
describe("startQaGatewayRpcClient", () => {
  ...
  it("calls the in-process gateway cli helper without mutating process.env", async () => {
    const originalHome = process.env.MO_HOME;
    ...
    try {
      ...
      if (originalHome === undefined) {
        ...
        process.env.MO_HOME = originalHome;
      }
    }

    expect(process.env.MO_HOME).toBe(originalHome);
  });
