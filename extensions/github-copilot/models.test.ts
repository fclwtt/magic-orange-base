...
import { createProviderUsageFetch, makeResponse } from "mo/plugin-sdk/test-env";
import { beforeEach, describe, expect, it, vi } from "vitest";
import { deriveCopilotApiBaseUrlFromToken, resolveCopilotApiToken } from "./token.js";
import { fetchCopilotUsage } from "./usage.js";

vi.mock("mo/plugin-sdk/provider-model-shared", async (importOriginal) => ({
  ...(await importOriginal<typeof import("mo/plugin-sdk/provider-model-shared")>()),
...
vi.mock("mo/plugin-sdk/json-store", () => ({
  loadJsonFile: jsonStoreMocks.loadJsonFile,
  saveJsonFile: jsonStoreMocks.saveJsonFile,
}));

vi.mock("mo/plugin-sdk/state-paths", () => ({
  resolveStateDir: () => "/tmp/mo-state",
}));

import type { ProviderResolveDynamicModelContext } from "mo/plugin-sdk/core";
...
describe("github-copilot token", () => {
  const cachePath = "/tmp/mo-state/credentials/github-copilot.token.json";
