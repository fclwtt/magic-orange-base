...
function stateDir(): string {
  const dir = process.env.MO_STATE_DIR;
  if (!dir) {
...
describe("Feishu doctor state repair", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "mo-feishu-doctor-"));
    process.env.HOME = tempHome;
    process.env.MO_HOME = tempHome;
    process.env.MO_STATE_DIR = path.join(tempHome, ".mo");
    fs.mkdirSync(process.env.MO_STATE_DIR, { recursive: true, mode: 0o700 });
  });
