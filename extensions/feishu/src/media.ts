...
import type { MessageReceipt } from "mo/plugin-sdk/channel-outbound";
import { mediaKindFromMime } from "mo/plugin-sdk/media-mime";
import {
  MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS,
  runFfmpeg,
  runFfprobe,
} from "mo/plugin-sdk/media-runtime";
import { saveMediaBuffer, saveMediaStream, type SavedMedia } from "mo/plugin-sdk/media-store";
import { readRegularFile, writeExternalFileWithinRoot } from "mo/plugin-sdk/security-runtime";
import { normalizeLowercaseStringOrEmpty } from "mo/plugin-sdk/string-coerce-runtime";
import {
  resolvePreferredOpenClawTmpDir,
  withTempWorkspace,
  withTempDownloadPath,
} from "mo/plugin-sdk/temp-path";
...
async function saveMessageResourceWithType(params: {
  ...
  const saved = await saveFeishuResponseMedia({
    response,
    tmpDirPrefix: "mo-feishu-resource-",
