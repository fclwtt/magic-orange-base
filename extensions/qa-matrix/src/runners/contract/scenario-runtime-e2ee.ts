...
import type { MatrixVerificationSummary } from "@mo/matrix/test-api.js";
...
function requireMatrixQaGatewayConfigPath(context: MatrixQaScenarioContext) {
  const configPath = requireMatrixQaCliRuntimeEnv(context).MO_CONFIG_PATH?.trim();
  if (!configPath) {
...
async function createMatrixQaCliSelfVerificationRuntime(params: {
  ...
  const rootDir = await mkdtemp(
    path.join(resolvePreferredOpenClawTmpDir(), "mo-matrix-cli-qa-"),
  );
...
async function createMatrixQaCliE2eeSetupRuntime(params: {
  ...
  const outputDir = requireMatrixQaE2eeOutputDir(params.context);
  const rootDir = await mkdtemp(
    path.join(resolvePreferredOpenClawTmpDir(), "mo-matrix-e2ee-setup-qa-"),
  );
  const artifactDir = path.join(
    outputDir,
    ...
    try {
      ...
      try {
        ...
        const cliSas = parseMatrixQaCliSasText(
          sasOutput.text,
          "interactive mo matrix verify self",
        );
        ...
        return {
          ...
          details: [
            "Matrix CLI self-verification established full Matrix identity trust through interactive mo matrix verify self",
    ...
    async ({ client, driverUserId, roomId, roomKey }) => {
      ...
      return {
        ...
        details: [
          ...
          "observed-event artifacts redact body/formatted_body unless MO_QA_MATRIX_CAPTURE_CONTENT=1",
          `encrypted room id: ${result.roomId}`,
          `isolated driver user: ${driverUserId}`,
          ...buildMatrixReplyDetails("E2EE reply", result.reply),
        ].join("\n"),
