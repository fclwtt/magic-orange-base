...
import { loadQaRuntimeModule } from "mo/plugin-sdk/qa-runner-runtime";
import {
...
function shouldWriteMatrixQaProgress() {
  const override = process.env.MO_QA_MATRIX_PROGRESS;
  if (override === "0") {
    return false;
  }
  if (override === "1") {
...
function resolveMatrixQaCanaryTimeoutMs() {
  return parsePositiveMatrixQaEnvMs(
    "MO_QA_MATRIX_CANARY_TIMEOUT_MS",
...
async function cleanupMatrixQaResource(params: {
  ...
  const timeoutMs = parsePositiveMatrixQaEnvMs(
    "MO_QA_MATRIX_CLEANUP_TIMEOUT_MS",
...
export async function runMatrixQaLive(params: {
  ...
  const includeObservedEventContent = process.env.MO_QA_MATRIX_CAPTURE_CONTENT === "1";
          ...
          const measuredScenario = await measureMatrixQaStep(() =>
            withMatrixQaRunDeadline(runDeadline, `Matrix scenario ${scenario.id}`, () =>
              runMatrixQaScenario(scenario, {
                ...
                gatewayStateDir: scenarioGateway.harness.gateway.runtimeEnv?.MO_STATE_DIR,
