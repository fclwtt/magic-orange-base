...
const {
  ...
  formatMatrixQaCliCommand: (args: string[]) => `mo ${args.join(" ")}`,
...
async function writeMatrixInboundDedupePluginStateEntry(params: {
  ...
  const databasePath = path.join(params.stateRoot, "state", "mo.sqlite");
...
describe("matrix live qa scenarios", () => {
  ...
  it("runs the Matrix E2EE state_after regression scenario through the gateway fault proxy", async () => {
    ...
    try {
      ...
      const result = await runMatrixQaScenario(scenario, {
        ...
        driverDeviceId: "DRIVERDEVICE",
        gatewayRuntimeEnv: {
          MO_CONFIG_PATH: gatewayConfigPath,
  ...
  it("runs Matrix self-verification through the interactive CLI command", async () => {
    ...
    try {
      ...
      runMatrixQaOpenClawCli.mockImplementation(async ({ args, env, stdin }) => {
        if (!cliAccountConfigDuringRun && env.MO_CONFIG_PATH) {
          const cliConfig = JSON.parse(
            await readFile(String(env.MO_CONFIG_PATH), "utf8"),
          ) as {
            channels?: {
              matrix?: {
      ...
      const result = await runMatrixQaScenario(scenario, {
        ...
        driverPassword: "driver-password",
        gatewayRuntimeEnv: {
          MO_CONFIG_PATH: "/tmp/gateway-config.json",
          MO_STATE_DIR: "/tmp/gateway-state",
          PATH: process.env.PATH,
        },
        outputDir,
      });
      ...
      expect(cliEnv?.MO_STATE_DIR).toContain("mo-matrix-cli-qa-");
      expect(cliEnv?.MO_CONFIG_PATH).toContain("mo-matrix-cli-qa-");
      const configPath = String(cliEnv?.MO_CONFIG_PATH);
      ...
      await expectPathMissing(String(cliEnv?.MO_STATE_DIR));
  ...
  it("runs Matrix encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaOpenClawCli.mockImplementation(async ({ args, env }) => {
        if (!initialAccountConfig && env.MO_CONFIG_PATH) {
          const initialConfig = JSON.parse(
            await readFile(String(env.MO_CONFIG_PATH), "utf8"),
          ) as {
            channels?: {
  ...
  it("runs Matrix encryption setup bootstrap failure through the CLI QA scenario", async () => {
    ...
    try {
      ...
      const wait = vi
        .fn()
        .mockRejectedValue(new Error("mo matrix encryption setup exited 1"));
      const kill = vi.fn();
      startMatrixQaOpenClawCli.mockReturnValue({
      ...
      ).toContain("mo-matrix-e2ee-setup-qa-");
  ...
  it("runs Matrix multi-account encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaOpenClawCli.mockImplementation(async ({ args, env }) => {
        const configPath = String(env.MO_CONFIG_PATH);
