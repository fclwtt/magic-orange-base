...
vi.mock("mo/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<object>("mo/plugin-sdk/setup");
  return {
    ...actual,
...
vi.mock("mo/plugin-sdk/channel-config-schema", async () => {
  const actual = await vi.importActual<object>("mo/plugin-sdk/channel-config-schema");
  return {
    ...actual,
...
vi.mock("mo/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<object>("mo/plugin-sdk/webhook-ingress");
  return {
    ...actual,
...
vi.mock("./runtime.js", () => ({
  getSynologyRuntime: vi.fn(() => ({
    ...
    channel: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/mo/synology-chat-sessions.json"),
