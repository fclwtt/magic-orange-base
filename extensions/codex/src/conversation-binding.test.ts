...
import type { ExecApprovalsFile } from "mo/plugin-sdk/exec-approvals-runtime";
import { upsertSessionEntry } from "mo/plugin-sdk/session-store-runtime";
...
vi.mock("mo/plugin-sdk/agent-harness-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("mo/plugin-sdk/agent-harness-runtime")>();
  return {
    ...actual,
...
vi.mock("mo/plugin-sdk/exec-approvals-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("mo/plugin-sdk/exec-approvals-runtime")>();
  return {
    ...actual,
    loadExecApprovals: execApprovalsRuntimeMocks.loadExecApprovals,
  };
});
vi.mock("mo/plugin-sdk/agent-runtime", () => agentRuntimeMocks);
...
describe("codex conversation binding", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-codex-binding-"));
  });
  ...
  it("refreshes stale network-proxy bound app-server threads before the turn", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        ...
        threadId: "thread-old",
        cwd: tempDir,
        networkProxyProfileName: "mo-network-stale",
