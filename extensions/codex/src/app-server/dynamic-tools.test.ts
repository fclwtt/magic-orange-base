...
import type { AnyAgentTool } from "mo/plugin-sdk/agent-harness";
import {
  HEARTBEAT_RESPONSE_TOOL_NAME,
  embeddedAgentLog,
  wrapToolWithBeforeToolCallHook,
} from "mo/plugin-sdk/agent-harness-runtime";
import { createTerminalPresentationContractTool } from "mo/plugin-sdk/agent-runtime-test-contracts";
import {
...
describe("createCodexDynamicToolBridge", () => {
  it("keeps turn-yield direct while deferring OpenClaw session spawn", () => {
    ...
    expectDynamicSpec(webSearch, {
      name: "web_search",
      namespace: CODEX_MO_DYNAMIC_TOOL_NAMESPACE,
      deferLoading: true,
    });
  ...
  it("marks executed dynamic tool results as side-effect evidence", async () => {
    ...
    const result = await bridge.handleToolCall({
      ...
      namespace: null,
      tool: "exec",
      arguments: { command: "touch /tmp/mo-replay-test" },
    });
  ...
  it("keeps config out of Codex tool-result contexts", async () => {
    const config = { session: { store: "/tmp/mo-session-store.json" } };
    const registry = createEmptyPluginRegistry();
