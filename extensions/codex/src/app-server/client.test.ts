...
import { embeddedAgentLog, MO_VERSION } from "mo/plugin-sdk/agent-harness-runtime";
...
describe("CodexAppServerClient", () => {
  ...
  it("initializes with the required client version", async () => {
    ...
    harness.send({
      id: outbound.id,
      result: { userAgent: "mo/0.125.0 (macOS; test)" },
    });
  ...
  it("blocks unsupported app-server versions during initialize", async () => {
    ...
    harness.send({
      id: outbound.id,
      result: { userAgent: "mo/0.124.9 (macOS; test)" },
    });
  ...
  it("blocks same-version Codex app-server prereleases below the stable floor", async () => {
    ...
    harness.send({
      id: outbound.id,
      result: { userAgent: "mo/0.125.0-alpha.2 (macOS; test)" },
    });
  ...
  it("accepts newer Codex app-server prereleases", async () => {
    ...
    harness.send({
      id: outbound.id,
      result: { userAgent: "mo/0.126.0-alpha.1 (macOS; test)" },
    });
  ...
  it("accepts newer Codex app-server builds", async () => {
    ...
    harness.send({
      id: outbound.id,
      result: { userAgent: "mo/0.126.0+custom (macOS; test)" },
    });
  ...
  it("reads the Codex version from the app-server user agent", () => {
    ...
    expect(readCodexVersionFromUserAgent("mo/0.125.0 (macOS; test)")).toBe("0.125.0");
    ...
    expect(readCodexVersionFromUserAgent("mo/0.125.0abc")).toBeUndefined();
