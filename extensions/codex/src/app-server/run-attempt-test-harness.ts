...
import { resetDiagnosticEventsForTest } from "mo/plugin-sdk/diagnostic-runtime";
import { clearInternalHooks, resetGlobalHookRunner } from "mo/plugin-sdk/hook-runtime";
import { clearMemoryPluginState } from "mo/plugin-sdk/memory-core-host-runtime-core";
import { clearPluginCommands } from "mo/plugin-sdk/plugin-runtime";
...
export function setupRunAttemptTestHooks(): void {
  beforeEach(async () => {
    ...
    vi.stubEnv("MO_TRAJECTORY", "0");
    vi.stubEnv("CODEX_API_KEY", "");
    vi.stubEnv("OPENAI_API_KEY", "");
    tempDir = await fs.mkdtemp(path.join(resolvePreferredOpenClawTmpDir(), "mo-codex-run-"));
  });
