...
vi.mock("mo/plugin-sdk/agent-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("mo/plugin-sdk/agent-runtime")>();
  return {
    ...actual,
    resolveApiKeyForProfile: async (
      ...
      if (params.forceRefresh || (oauthCredential.expires ?? 0) <= Date.now()) {
        ...
        if (refreshed?.access) {
          ...
          if (params.agentDir || process.env.MO_STATE_DIR) {
...
describe("bridgeCodexAppServerStartOptions", () => {
  it("preserves persisted provenance when preparing a supplied base store", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-codex-app-server-"));
  ...
  it("applies native Codex CLI OAuth when no OpenClaw auth profile exists", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "mo-codex-app-server-"));
