...
import { classifyEmbeddedAgentRunResultForModelFallback } from "mo/plugin-sdk/agent-harness-runtime";
import {
  createContractRunResult,
  OUTCOME_FALLBACK_RUNTIME_CONTRACT,
} from "mo/plugin-sdk/agent-runtime-test-contracts";
...
type MirrorTaggedMessage = { __mo?: { mirrorIdentity?: string } };

async function createParams(): Promise<EmbeddedRunAttemptParams> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-codex-outcome-contract-"));
...
function readMirrorIdentity(message: unknown): string | undefined {
  const meta = (message as MirrorTaggedMessage | undefined)?.["__mo"];
