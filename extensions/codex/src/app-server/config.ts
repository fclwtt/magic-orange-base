...
} from "mo/plugin-sdk/agent-runtime";
import {
  resolveExecApprovalsFromFile,
  type ExecApprovalsFile,
} from "mo/plugin-sdk/exec-approvals-runtime";
import { resolvePositiveTimerTimeoutMs } from "mo/plugin-sdk/number-runtime";
...
import { normalizeTrimmedStringList } from "mo/plugin-sdk/string-coerce-runtime";
import { detectWindowsSpawnCommandInlineArgs } from "mo/plugin-sdk/windows-spawn";
import { z } from "zod";
import type { CodexSandboxPolicy, CodexServiceTier, JsonObject, JsonValue } from "./protocol.js";

const START_OPTIONS_KEY_SECRET_SYMBOL = Symbol.for("mo.codexAppServerStartOptionsKeySecret");
...
const DEFAULT_CODEX_APP_SERVER_NETWORK_PROXY_PROFILE_PREFIX = "mo-network";
...
function assertCodexAppServerCommandHasNoInlineArgs(params: {
  ...
  const sourceLabel =
    params.source === "env"
      ? "MO_CODEX_APP_SERVER_BIN"
      : "plugins.entries.codex.config.appServer.command";
  const argsLabel =
    params.source === "env"
      ? "MO_CODEX_APP_SERVER_ARGS"
...
export function resolveCodexAppServerRuntimeOptions(
  ...
  const envCommand = readNonEmptyString(env.MO_CODEX_APP_SERVER_BIN);
  ...
  const args = resolveArgs(config.args, env.MO_CODEX_APP_SERVER_ARGS);
  ...
  const explicitPolicyMode =
    resolvePolicyMode(config.mode) ?? resolvePolicyMode(env.MO_CODEX_APP_SERVER_MODE);
  const configuredSandbox =
    resolveSandbox(config.sandbox) ?? resolveSandbox(env.MO_CODEX_APP_SERVER_SANDBOX);
  ...
  const envApprovalPolicy = resolveApprovalPolicy(env.MO_CODEX_APP_SERVER_APPROVAL_POLICY);
...
export function resolveCodexComputerUseConfig(
  ...
  const marketplaceSource =
    ...
    readNonEmptyString(env.MO_CODEX_COMPUTER_USE_MARKETPLACE_SOURCE);
  const marketplacePath =
    readNonEmptyString(params.overrides?.marketplacePath) ??
    readNonEmptyString(config.marketplacePath) ??
    readNonEmptyString(env.MO_CODEX_COMPUTER_USE_MARKETPLACE_PATH);
  const marketplaceName =
    readNonEmptyString(params.overrides?.marketplaceName) ??
    readNonEmptyString(config.marketplaceName) ??
    readNonEmptyString(env.MO_CODEX_COMPUTER_USE_MARKETPLACE_NAME);
  const autoInstall =
    params.overrides?.autoInstall ??
    config.autoInstall ??
    readBooleanEnv(env.MO_CODEX_COMPUTER_USE_AUTO_INSTALL) ??
    false;
  const marketplaceDiscoveryTimeoutMs = normalizePositiveNumber(
    params.overrides?.marketplaceDiscoveryTimeoutMs ??
      ...
      readNumberEnv(env.MO_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS),
    DEFAULT_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS,
  );
  const enabled =
    params.overrides?.enabled ??
    config.enabled ??
    readBooleanEnv(env.MO_CODEX_COMPUTER_USE) ??
  ...
  return {
    ...
    pluginName:
      ...
      readNonEmptyString(env.MO_CODEX_COMPUTER_USE_PLUGIN_NAME) ??
    ...
    mcpServerName:
      ...
      readNonEmptyString(env.MO_CODEX_COMPUTER_USE_MCP_SERVER_NAME) ??
