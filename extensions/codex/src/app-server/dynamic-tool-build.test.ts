...
describe("Codex app-server dynamic tool build", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-codex-tools-"));
  });
  ...
  it("maps local gateway workspace suffixes to the remote Codex app-server root", () => {
    expect(
      mapCodexAppServerRemoteWorkspacePath({
        value: "/Users/kevinlin/code/mo/packages/example",
        localWorkspaceRoot: "/Users/kevinlin/code/mo",
        remoteWorkspaceRoot: "/home/oai/mo-workspaces",
      }),
    ).toBe("/home/oai/mo-workspaces/packages/example");
    expect(
      mapCodexAppServerRemoteWorkspacePath({
        value: "/Users/kevinlin/code/mo",
        localWorkspaceRoot: "/Users/kevinlin/code/mo",
        remoteWorkspaceRoot: "/home/oai/mo-workspaces",
      }),
    ).toBe("/home/oai/mo-workspaces");
  });
  ...
  it("maps Windows child paths through remote Codex app-server workspaces", () => {
    expect(
      mapCodexAppServerRemoteWorkspacePath({
        value: "C:\\Users\\kevinlin\\code\\mo\\packages\\example",
        localWorkspaceRoot: "C:\\Users\\kevinlin\\code\\mo",
        remoteWorkspaceRoot: "/home/oai/mo-workspaces",
      }),
    ).toBe("/home/oai/mo-workspaces/packages/example");
  });
  ...
  it("maps sandbox exec-server cwd through the remote workspace mapping", () => {
    expect(
      resolveCodexAppServerExecutionCwd({
        effectiveCwd: "/Users/kevinlin/code/mo",
        environment: {
          id: "sandbox-1",
          cwd: "/Users/kevinlin/code/mo/sandbox",
        } as never,
        nativeToolSurfaceEnabled: true,
        localWorkspaceRoot: "/Users/kevinlin/code/mo",
        remoteWorkspaceRoot: "/home/oai/mo-workspaces",
      }),
    ).toBe("/home/oai/mo-workspaces/sandbox");
  });
  ...
  it("exposes Docker sandbox shell tools when native Code Mode cannot honor sandbox paths", async () => {
    ...
    const tools = await buildDynamicToolsForTest(params, workspaceDir, {
      sandbox: {
        enabled: true,
        backendId: "docker",
        docker: { binds: ["/tmp/mo-data:/data:rw"] },
      } as never,
      nativeToolSurfaceEnabled: false,
    });
  ...
  it("enables gateway subagent binding for forced private QA Codex runs", async () => {
    vi.stubEnv("MO_BUILD_PRIVATE_QA", "1");
    vi.stubEnv("MO_QA_FORCE_RUNTIME", "codex");
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createParams(sessionFile, workspaceDir);
    params.disableTools = false;
  ...
  it("disables Codex native tool surfaces whenever an OpenClaw sandbox is active", () => {
    ...
    expect(
      shouldEnableCodexAppServerNativeToolSurface(params, {
        enabled: true,
        backendId: "docker",
        docker: { binds: ["/tmp/mo-data:/tmp/mo-data:rw"] },
      } as never),
    ).toBe(false);
    ...
    expect(
      shouldEnableCodexAppServerNativeToolSurface(params, {
        ...
        docker: {
          binds: [
            "/tmp/mo-data:/tmp/mo-data:rw",
            "/tmp/mo-data/secrets:/tmp/mo-data/secrets:ro",
          ],
        },
      } as never),
    ).toBe(false);
