...
import type { AuthProfileStore } from "mo/plugin-sdk/agent-runtime";
...
const MO_CODEX_APP_SERVER_ARGS_ENV_VAR = "MO_CODEX_APP_SERVER_ARGS";
...
function buildPrivateCodexAppServerStartOptions(
  ...
  const privateEnv = Object.fromEntries(
    Object.entries(start.env ?? {}).filter(
      ([name]) => name.trim().toUpperCase() !== MO_CODEX_APP_SERVER_ARGS_ENV_VAR,
    ),
  );
  const clearEnv = (start.clearEnv ?? []).filter((name) => {
    const normalized = name.trim().toUpperCase();
    return normalized !== "CODEX_HOME" && normalized !== MO_CODEX_APP_SERVER_ARGS_ENV_VAR;
  });
  return {
    ...start,
    ...
    clearEnv: [...clearEnv, MO_CODEX_APP_SERVER_ARGS_ENV_VAR],
  };
}
