...
describe("managed Codex app-server binary", () => {
  ...
  it("prefers the macOS desktop app bundle when it exists", async () => {
    const pluginRoot = path.join("/tmp", "mo", "extensions", "codex");
  ...
  it("uses the package root when the resolver is bundled into a dist chunk", () => {
    expect(testing.resolveDefaultCodexPluginRoot("/repo/mo/dist")).toBe("/repo/mo");
    expect(testing.resolveDefaultCodexPluginRoot("/repo/mo/dist-runtime")).toBe(
      "/repo/mo",
    );
    expect(
      testing.resolveDefaultCodexPluginRoot("/repo/mo/extensions/codex/src/app-server"),
    ).toBe("/repo/mo/extensions/codex");
  });

  it("finds Codex in the package install root used by packaged plugins", async () => {
    const installRoot = path.join("/tmp", "mo-plugin-package", "codex");
  ...
  it("finds Codex bins hoisted into an isolated npm project root", async () => {
    const projectRoot = path.join("/tmp", "state", "npm", "projects", "mo-codex-hash");
    const pluginRoot = path.join(projectRoot, "node_modules", "@mo", "codex");
  ...
  it("finds Windows Codex shims hoisted into an isolated npm project root", async () => {
    ...
    const pluginRoot = path.win32.join(projectRoot, "node_modules", "@mo", "codex");
  ...
  it("falls back to the resolved Codex package bin when no command shim exists", async () => {
    const installRoot = await mkdtemp(path.join(os.tmpdir(), "mo-codex-package-"));
    const pluginRoot = path.join(installRoot, "dist", "extensions", "codex");
  ...
  it("fails clearly when the managed Codex binary is missing", async () => {
    await expect(
      resolveManagedCodexAppServerStartOptions(startOptions("managed"), {
        platform: "darwin",
        pluginRoot: path.join("/tmp", "mo", "extensions", "codex"),
