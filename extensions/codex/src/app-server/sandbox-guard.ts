...
import { resolveSandboxRuntimeStatus } from "mo/plugin-sdk/sandbox";
import {
...
const DIRECT_METHOD_POLICIES = new Map<string, DirectMethodPolicy>([
  ...
  ["thread/start", "requires-mo-environment"],
...
export function resolveCodexAppServerDirectSandboxBypassBlock(params: {
  ...
  if (
    policy === "requires-mo-environment" &&
  ...
  return (
    ...
    environments.every((entry) => {
      ...
      return (
        ...
        environment.environmentId.startsWith("mo-sandbox-") &&
