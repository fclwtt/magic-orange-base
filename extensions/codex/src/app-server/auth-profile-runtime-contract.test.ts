...
} from "mo/plugin-sdk/agent-harness";
import { AUTH_PROFILE_RUNTIME_CONTRACT } from "mo/plugin-sdk/agent-runtime-test-contracts";
...
describe("Auth profile runtime contract - Codex app-server adapter", () => {
  ...
  beforeEach(async () => {
    vi.useRealTimers();
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-codex-auth-contract-"));
  });
