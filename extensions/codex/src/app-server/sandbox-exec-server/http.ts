...
import { embeddedAgentLog } from "mo/plugin-sdk/agent-harness-runtime";
import type { SandboxContext } from "mo/plugin-sdk/sandbox";
import { SsrFBlockedError, isBlockedHostnameOrIp } from "mo/plugin-sdk/ssrf-runtime";
...
tmp=$(mktemp "$TMPDIR/mo-http.XXXXXX.py" 2>/dev/null || mktemp "/tmp/mo-http.XXXXXX.py") || exit 1
trap 'rm -f "$tmp"' EXIT
cat > "$tmp" <<'PY'
import base64
import json
