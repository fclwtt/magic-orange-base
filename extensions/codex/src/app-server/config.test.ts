...
describe("Codex app-server config", () => {
  it("only auto-approves app-server approvals for full yolo runtime policy", () => {
    ...
    expect(
      shouldAutoApproveCodexAppServerApprovals({
        ...
        sandbox: "danger-full-access",
        networkProxy: {
          profileName: "mo-network",
          configFingerprint: "network-proxy-v1",
          configPatch: {
            "features.network_proxy.enabled": true,
            default_permissions: "mo-network",
            permissions: {},
          },
        },
      }),
  ...
  it("parses typed plugin config before falling back to environment knobs", () => {
    const runtime = resolveRuntimeForTest({
      ...
      env: {
        MO_CODEX_APP_SERVER_APPROVAL_POLICY: "never",
        MO_CODEX_APP_SERVER_SANDBOX: "read-only",
      },
  ...
  it("uses read-only filesystem rules for read-only network proxy profiles", () => {
    ...
    expect(profileName).toMatch(/^mo-network-[a-f0-9]{16}$/u);
    ...
    expect(
      readCodexPluginConfig({
        appServer: {
          ...
          remoteWorkspace: {
            localRoot: "/Users/kevinlin/code/mo",
            remoteRoot: "/home/oai/mo-workspaces",
          },
        },
      }),
    ).toStrictEqual({});
    ...
    const runtime = resolveRuntimeForTest({
      pluginConfig: {
        appServer: {
          ...
          url: "wss://codex-app-server.example.internal/ws",
          authToken: "capability-token",
          remoteWorkspaceRoot: " /home/oai/mo-workspaces ",
        },
      },
    });
  ...
  it("treats only explicit OpenAI model context as safe for Codex-backed auto-review", () => {
    ...
    for (const openAIProvider of [
      ...
      {
        baseUrl: "https://api.openai.com/v1",
        headers: { "x-mo-reviewer-proxy": "local" },
        models: [],
      },
      {
        baseUrl: "https://api.openai.com/v1",
...
`,
  ...
  it("keeps explicit yolo mode when requirements disallow full access", () => {
    ...
    expectRuntimePolicy(
      resolveRuntimeForTest({
        pluginConfig: {},
        env: { MO_CODEX_APP_SERVER_MODE: "yolo" },
        requirementsToml,
      }),
      {
        approvalPolicy: "never",
  ...
  it("rejects the retired dynamic tool profile key", () => {
    expect(
      readCodexPluginConfig({
        codexDynamicToolsProfile: "mo-compat",
        codexDynamicToolsLoading: "direct",
      }),
    ).toEqual({});
  });
  ...
  it("treats configured and environment commands as explicit overrides", () => {
    expectFields(
      resolveRuntimeForTest({
        ...
        env: { MO_CODEX_APP_SERVER_BIN: "/usr/local/bin/codex" },
      }).start,
      "configured start",
      {
  ...
  it("rejects Codex app-server command overrides that include inline arguments", () => {
    ...
    ).toThrow("MO_CODEX_APP_SERVER_BIN must be only the Codex app-server executable path");
  });
  ...
  it("resolves Computer Use setup from plugin config and environment fallbacks", () => {
    expect(
      resolveCodexComputerUseConfig({
        ...
        env: {
          MO_CODEX_COMPUTER_USE_PLUGIN_NAME: "env-fallback-plugin",
        },
      }),
    ).toEqual({
      enabled: true,
    ...
    expectFields(
      resolveCodexComputerUseConfig({
        pluginConfig: {},
        env: {
          MO_CODEX_COMPUTER_USE: "1",
          MO_CODEX_COMPUTER_USE_MARKETPLACE_SOURCE: "github:example/plugins",
          MO_CODEX_COMPUTER_USE_AUTO_INSTALL: "true",
          MO_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS: "30000",
        },
      }),
      "computer use config",
      {
    ...
    for (const value of ["0x10", "1e3"]) {
      expectFields(
        resolveCodexComputerUseConfig({
          ...
          env: {
            MO_CODEX_COMPUTER_USE: "1",
            MO_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS: value,
          },
        }),
        "computer use config",
        {
  ...
  it("allows environment mode fallback to opt in to guardian-reviewed local execution", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      modelProvider: "openai",
      env: { MO_CODEX_APP_SERVER_MODE: "guardian" },
    });
  ...
  it("forces guarded app-server policy fields for auto mode", () => {
    const runtime = resolveRuntimeForTest({
      ...
      env: {
        MO_CODEX_APP_SERVER_APPROVAL_POLICY: "never",
        MO_CODEX_APP_SERVER_SANDBOX: "danger-full-access",
      },
      execMode: "auto",
      modelProvider: "openai",
    });
  ...
  it("ignores removed MO_CODEX_APP_SERVER_GUARDIAN fallback", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      env: { MO_CODEX_APP_SERVER_GUARDIAN: "1" },
    });
  ...
  it("keeps runtime config keys aligned with manifest schema and UI hints", async () => {
    const manifest = JSON.parse(
      await fs.readFile(new URL("../../mo.plugin.json", import.meta.url), "utf8"),
    ) as {
      configSchema: {
        properties: {
