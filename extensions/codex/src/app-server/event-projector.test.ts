...
import type { EmbeddedRunAttemptParams } from "mo/plugin-sdk/agent-harness";
import {
  embeddedAgentLog,
...
import { withTempDir } from "mo/plugin-sdk/test-env";
...
async function createParams(): Promise<EmbeddedRunAttemptParams> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-codex-projector-"));
...
describe("CodexAppServerEventProjector", () => {
  ...
  it("saves raw Codex image-generation results as reply media", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-codex-media-state-"));
  ...
  it("prefers gateway-managed image media when the typed event arrives first", async () => {
    await withTempDir("mo-codex-media-state-", async (stateDir) => {
      vi.stubEnv("MO_STATE_DIR", stateDir);
      const projector = await createProjector();
  ...
  it("does not parse shell command text to suppress transcript progress", async () => {
    ...
    projector.recordDynamicToolCall({
      ...
      arguments: {
        command:
          '/bin/bash -lc \'/home/mo/.mo/workspace/bin/log_activity.sh "web_search" "Grilled salmon research"\'',
        cwd: "/workspace",
      },
    });
    projector.recordDynamicToolResult({
