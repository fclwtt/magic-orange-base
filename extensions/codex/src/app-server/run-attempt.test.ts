...
import { initializeGlobalHookRunner, registerInternalHook } from "mo/plugin-sdk/hook-runtime";
import { registerMemoryCapability } from "mo/plugin-sdk/memory-core-host-runtime-core";
import { MESSAGE_TOOL_DELIVERY_HINTS } from "mo/plugin-sdk/message-tool-delivery-hints";
import { registerPluginCommand } from "mo/plugin-sdk/plugin-runtime";
import { createMockPluginRegistry } from "mo/plugin-sdk/plugin-test-runtime";
...
import {
  CODEX_MO_DYNAMIC_TOOL_NAMESPACE,
...
describe("runCodexAppServerAttempt", () => {
  ...
  it("routes native Codex execution through an OpenClaw sandbox exec-server when opted in", async () => {
    ...
    try {
      ...
      expect(environmentAddParams?.environmentId).toMatch(/^mo-sandbox-/);
  ...
  it("passes OpenClaw skills as turn collaboration developer instructions", async () => {
    ...
    vi.stubEnv("MO_TRAJECTORY", "1");
    vi.stubEnv("MO_TRAJECTORY_DIR", path.join(tempDir, "trajectory"));
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
  ...
  it("keeps searchable OpenClaw dynamic tools when code-mode-only is enabled", () => {
    ...
    expect(webSearch?.namespace).toBe(CODEX_MO_DYNAMIC_TOOL_NAMESPACE);
    expect(webSearch?.deferLoading).toBe(true);
    expect(heartbeat?.namespace).toBe(CODEX_MO_DYNAMIC_TOOL_NAMESPACE);
    expect(heartbeat?.deferLoading).toBe(true);
    expect(sessionsSpawn?.namespace).toBe(CODEX_MO_DYNAMIC_TOOL_NAMESPACE);
  ...
  it("projects bounded continuity when starting Codex without a native thread binding", async () => {
    ...
    const harness = createStartedThreadHarness();
    const params = createParams(sessionFile, workspaceDir);
    params.prompt = "make the default webpage mo";
    ...
    expect(inputText).toContain("Current user request:");
    expect(inputText).toContain("make the default webpage mo");
  });
  ...
  it("projects only newer visible history when a resumed Codex binding is stale", async () => {
    ...
    const copilotMirrorMessage = {
      ...
      __mo: { mirrorIdentity: "copilot:assistant-1" },
    } as ReturnType<typeof assistantMessage> & { __mo: { mirrorIdentity: string } };
  ...
  it("does not project Codex mirrored transcript echoes as stale binding continuity", async () => {
    ...
    const codexMirrorAssistantMessage = {
      ...
      __mo: { mirrorIdentity: "codex-app-server:assistant-1" },
    } as ReturnType<typeof assistantMessage> & { __mo: { mirrorIdentity: string } };
  ...
  it("keeps lightweight cron Codex turns out of OpenClaw bootstrap context", async () => {
    ...
    const workspaceDir = path.join(tempDir, "workspace");
    const exactCommand =
      "cd /Users/phaedrus/Projects/mo && /Users/phaedrus/clawd/scripts/clawsweeper-related-scan.py";
    await fs.mkdir(workspaceDir, { recursive: true });
  ...
  it("ignores invalid Codex app-server env overrides when promoting tool policy approval", async () => {
    ...
    vi.stubEnv("MO_CODEX_APP_SERVER_MODE", " ");
    vi.stubEnv("MO_CODEX_APP_SERVER_APPROVAL_POLICY", "always");
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const harness = createStartedThreadHarness();
