...
import { resolveAgentDir, resolveSessionAgentIds } from "mo/plugin-sdk/agent-runtime";
import { parseStrictPositiveInteger } from "mo/plugin-sdk/number-runtime";
import type { PluginCommandContext, PluginCommandResult } from "mo/plugin-sdk/plugin-entry";
...
const CODEX_DIAGNOSTICS_SOURCE = "mo-diagnostics";
    ...
    if (!deps.codexPluginsManagementIo) {
      return {
        text:
          ...
          "Edit ~/.mo/mo.json or use `mo config patch` until the runtime exposes the IO.",
      };
    }
