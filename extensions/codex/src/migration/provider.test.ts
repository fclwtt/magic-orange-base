...
import type { MigrationProviderContext } from "mo/plugin-sdk/plugin-entry";
import { upsertAuthProfile } from "mo/plugin-sdk/provider-auth";
...
async function makeTempRoot(): Promise<string> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "mo-migrate-codex-"));
  tempRoots.add(root);
  return root;
}
...
describe("buildCodexMigrationProvider", () => {
  it("parses target marketplace discovery timeout env strictly", () => {
    expect(
      targetCodexMarketplaceDiscoveryTimeoutMs({
        MO_CODEX_MIGRATION_PLUGIN_LIST_TIMEOUT_MS: "0",
      }),
    ).toBe(0);
    expect(
      targetCodexMarketplaceDiscoveryTimeoutMs({
        MO_CODEX_MIGRATION_PLUGIN_LIST_TIMEOUT_MS: "250",
      }),
    ).toBe(250);
    ...
    for (const value of ["0x10", "1e3", "2.5"]) {
      expect(
        targetCodexMarketplaceDiscoveryTimeoutMs({
          MO_CODEX_MIGRATION_PLUGIN_LIST_TIMEOUT_MS: value,
        }),
      ).toBe(30_000);
    }
  });
  ...
  it("leaves selected Codex plugins as warnings when target curated plugins never load", async () => {
    vi.stubEnv("MO_CODEX_MIGRATION_PLUGIN_LIST_TIMEOUT_MS", "1");
    const fixture = await createCodexFixture();
    const configState: MigrationProviderContext["config"] = {
      agents: { defaults: { workspace: fixture.workspaceDir } },
    } as MigrationProviderContext["config"];
    ...
    expect(result.warnings).toContain(
      "Some Codex plugins could not be migrated. Run `mo migrate codex` after onboarding.",
    );
