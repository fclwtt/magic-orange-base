...
import { asBoolean, isRecord } from "mo/plugin-sdk/string-coerce-runtime";
    ...
    items.push(
      createMigrationManualItem({
        ...
        message:
          ...
          `Codex native plugin "${plugin.name}" was found but not activated automatically.`,
        recommendation:
          "Review the plugin bundle first, then install trusted compatible plugins with mo plugins install <path>.",
      }),
    );
  }
  return items;
...
export async function buildCodexMigrationPlan(
  ...
  return {
    ...
    warnings,
    nextSteps: [
      "Run mo doctor after applying the migration.",
