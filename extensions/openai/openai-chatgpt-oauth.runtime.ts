...
import type { ProviderAuthContext } from "mo/plugin-sdk/plugin-entry";
import { ensureGlobalUndiciEnvProxyDispatcher } from "mo/plugin-sdk/runtime-env";
import { formatCliCommand } from "mo/plugin-sdk/setup-tools";
...
const openAICodexOAuthOriginator = "mo";
const localManualFallbackDelayMs = 15_000;
const localManualFallbackGraceMs = 1_000;
const openAIAuthProbeUrl =
  "https://auth.openai.com/oauth/authorize?response_type=code&client_id=mo-preflight&redirect_uri=http%3A%2F%2Flocalhost%3A1455%2Fauth%2Fcallback&scope=openid+profile+email";
  ...
  if (/unsupported_country_region_territory/i.test(message)) {
    return createOpenAICodexOAuthError(
      ...
      [
        ...
        "If you normally use a proxy, verify HTTPS_PROXY, HTTP_PROXY, or ALL_PROXY is set for the OpenClaw process and then retry `mo models auth login --provider openai`.",
      ].join(" "),
      error,
    );
  }
...
export async function loginOpenAICodexOAuth(params: {
  ...
  try {
    ...
    await prompter.note("Trouble with OAuth? See https://docs.mo.ai/start/faq", "OAuth help");
