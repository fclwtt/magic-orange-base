...
import type { ResolvedTtsConfig } from "mo/plugin-sdk/agent-runtime";
import { AuthStorage, ModelRegistry } from "mo/plugin-sdk/agent-sessions";
import type { OpenClawConfig } from "mo/plugin-sdk/config-contracts";
import { encodePngRgba, fillPixel } from "mo/plugin-sdk/media-runtime";
import {
  registerProviderPlugin,
  requireRegisteredProvider,
} from "mo/plugin-sdk/plugin-test-runtime";
import { runRealtimeSttLiveTest } from "mo/plugin-sdk/provider-test-contracts";
...
const LIVE_MODEL_ID = process.env.MO_LIVE_OPENAI_PLUGIN_MODEL?.trim() || "gpt-5.5";
const LIVE_IMAGE_MODEL = process.env.MO_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-2";
const LIVE_VISION_MODEL = process.env.MO_LIVE_OPENAI_VISION_MODEL?.trim() || "gpt-5.4-mini";
const liveEnabled = OPENAI_API_KEY.trim().length > 0 && process.env.MO_LIVE_TEST === "1";
