...
import type { ProviderRuntimeModel } from "mo/plugin-sdk/plugin-entry";
...
describe("openai transport policy", () => {
  ...
  it("builds native turn state for direct OpenAI routes", () => {
    ...
    expect(state?.headers?.["x-mo-session-id"]).toBe("session-123");
    expect(state?.headers?.["x-mo-turn-id"]).toBe("turn-123");
    expect(state?.headers?.["x-mo-turn-attempt"]).toBe("2");
    expect(state?.metadata?.mo_session_id).toBe("session-123");
    expect(state?.metadata?.mo_turn_id).toBe("turn-123");
    expect(state?.metadata?.mo_turn_attempt).toBe("2");
    expect(state?.metadata?.mo_transport).toBe("websocket");
  });
  ...
  it("returns websocket session headers and cooldown for native routes", () => {
    ...
    expect(policy?.headers?.["x-mo-session-id"]).toBe("session-123");
