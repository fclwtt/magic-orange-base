...
import { CODEX_CLI_PROFILE_ID, type OAuthCredential } from "mo/plugin-sdk/provider-auth";
...
import { fetchCodexUsage } from "mo/plugin-sdk/provider-usage";
import {
...
async function runOpenAICodexDeviceCode(ctx: ProviderAuthContext) {
  ...
  try {
    ...
    await ctx.prompter.note(
      "Trouble with device code login? See https://docs.mo.ai/start/faq",
      "OAuth help",
    );
    throw error;
  }
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `mo models auth login --provider openai` or `mo configure`.";
}
