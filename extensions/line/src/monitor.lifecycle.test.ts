...
import { createMockIncomingRequest } from "mo/plugin-sdk/test-env";
import { WEBHOOK_IN_FLIGHT_DEFAULTS } from "mo/plugin-sdk/webhook-request-guards";
...
vi.mock("mo/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<typeof import("mo/plugin-sdk/webhook-ingress")>(
    "mo/plugin-sdk/webhook-ingress",
  );
  return {
    ...actual,
...
describe("monitorLineProvider lifecycle", () => {
  ...
  afterAll(() => {
    vi.doUnmock("./bot.js");
    vi.doUnmock("mo/plugin-sdk/reply-runtime");
    vi.doUnmock("mo/plugin-sdk/runtime-env");
    vi.doUnmock("mo/plugin-sdk/webhook-ingress");
