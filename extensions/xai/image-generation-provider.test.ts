...
const {
  ...
  resolveProviderHttpRequestConfigMock: vi.fn((params: Record<string, unknown>) => {
    ...
    if (params.provider === "xai") {
      const version = process.env.MO_VERSION?.trim() || "unknown";
      headers.set("User-Agent", `mo/${version}`);
      headers.set("originator", "mo");
...
vi.mock("mo/plugin-sdk/provider-auth-runtime", () => ({
...
vi.mock("mo/plugin-sdk/provider-http", async () => {
  const actual = await vi.importActual<typeof import("mo/plugin-sdk/provider-http")>(
    "mo/plugin-sdk/provider-http",
  );
  return {
...
vi.mock("mo/plugin-sdk/string-coerce-runtime", () => ({
...
describe("xai image generation provider", () => {
  ...
  it("builds provider with correct models, default, and capabilities", () => {
    ...
    expect(isConfigured({ agentDir: "/tmp/mo-xai-test" })).toBe(true);
    expect(isProviderApiKeyConfiguredMock).toHaveBeenCalledWith({
      provider: "xai",
      agentDir: "/tmp/mo-xai-test",
    });
  });
  ...
  it("forwards xAI attribution User-Agent through the SDK image request", async () => {
    ...
    const request = requirePostJsonCall();
    expect(request.headers?.get("user-agent")).toBe("mo/2026.3.22");
    expect(request.headers?.get("originator")).toBe("mo");
