...
import { MO_VERSION as PACKAGE_VERSION } from "mo/plugin-sdk/agent-harness-runtime";
...
function resolveXaiUserAgentVersion(): string {
  ...
  const envVersion = trimToUndefined(process.env.MO_VERSION);
  if (envVersion) {
    return envVersion;
  }
  ...
  return (
    trimToUndefined(process.env.MO_SERVICE_VERSION) ??
...
// an empty record so the mo identity is not forwarded to the proxy.
