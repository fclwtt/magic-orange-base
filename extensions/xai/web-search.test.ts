...
import { createTestWizardPrompter } from "mo/plugin-sdk/plugin-test-runtime";
import { NON_ENV_SECRETREF_MARKER } from "mo/plugin-sdk/provider-auth-runtime";
import { createNonExitingRuntime } from "mo/plugin-sdk/runtime-env";
import { withEnv, withEnvAsync, withFetchPreconnect } from "mo/plugin-sdk/test-env";
...
vi.mock("mo/plugin-sdk/provider-auth", async (importOriginal) => {
  const original = await importOriginal<typeof import("mo/plugin-sdk/provider-auth")>();
  return {
    ...original,
...
vi.mock("mo/plugin-sdk/provider-auth-runtime", async (importOriginal) => {
  const original =
    await importOriginal<typeof import("mo/plugin-sdk/provider-auth-runtime")>();
  return {
    ...original,
...
vi.mock("mo/plugin-sdk/provider-web-search", async (importOriginal) => {
  const original = await importOriginal<typeof import("mo/plugin-sdk/provider-web-search")>();
  return {
    ...original,
...
describe("xai web search config resolution", () => {
  ...
  it("uses xAI OAuth auth before API-key fallback for web search", async () => {
    ...
    const tool = provider.createTool({
      config: {
        agents: {
          list: [{ id: "main", default: true, agentDir: "/tmp/mo-xai-main-agent" }],
        },
        plugins: {
          entries: {
            xai: {
    ...
    expect(providerAuthRuntimeMocks.resolveApiKeyForProvider).toHaveBeenCalledWith(
      expect.objectContaining({
        provider: "xai",
        agentDir: "/tmp/mo-xai-main-agent",
      }),
    );
  ...
  it("uses the active agentDir for xAI OAuth web search auth", async () => {
    ...
    const provider = createXaiWebSearchProvider();
    const tool = provider.createTool({
      agentDir: "/tmp/mo-xai-active-agent",
      config: {
        agents: {
          list: [
            { id: "main", default: true, agentDir: "/tmp/mo-xai-main-agent" },
            { id: "side", agentDir: "/tmp/mo-xai-active-agent" },
          ],
        },
      },
    });
