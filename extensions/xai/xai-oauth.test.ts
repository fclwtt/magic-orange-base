...
import type { OAuthCredential } from "mo/plugin-sdk/provider-auth";
...
describe("xAI OAuth", () => {
  ...
  it("validates discovered endpoints before using them", async () => {
    ...
    expect(discoveryHeaders.get("user-agent")).toBe("mo/2026.3.22");
    vi.unstubAllEnvs();
  ...
  it("refreshes with the cached token endpoint and preserves refresh fallback", async () => {
    ...
    const fetchImpl = vi.fn<typeof fetch>(async (_url, init) => {
      ...
      expect(headers.get("user-agent")).toBe("mo/2026.3.22");
      return jsonResponse({
