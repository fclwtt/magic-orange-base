...
function compileManifestConfigSchema() {
  const manifest = JSON.parse(
    fs.readFileSync(new URL("../mo.plugin.json", import.meta.url), "utf8"),
...
describe("resolveDiffsPluginViewerBaseUrl", () => {
  ...
  it("normalizes configured viewer base URLs", () => {
    expect(
      resolveDiffsPluginViewerBaseUrl({
        viewerBaseUrl: "https://example.com/mo/",
      }),
    ).toBe("https://example.com/mo");
  });
});
...
describe("diffs plugin schema surfaces", () => {
  it("rejects invalid viewerBaseUrl values at manifest-validation time too", () => {
    ...
    expect(validate({ viewerBaseUrl: "https://example.com/mo?x=1" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/mo#frag" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/mo/" })).toBe(true);
  });
  ...
  it("preserves defaults and security for direct safeParse callers", () => {
    ...
    expect(data.viewerBaseUrl).toBe("https://example.com/mo");
...
describe("diffs viewer URL helpers", () => {
  ...
  it("joins viewer path under baseUrl pathname", () => {
    ...
    ).toBe("https://example.com/mo/plugins/diffs/view/id/token");
  });
...
describe("viewer assets", () => {
  ...
  it("serves the runtime bundle body", async () => {
    ...
    expect(String(runtime?.body)).toContain("moDiffsReady");
...
describe("resolveDiffsLanguagePackAvailability", () => {
  it.each(["assets", "dist/assets"])(
    ...
    (assetDir) => {
      const root = fs.mkdtempSync(join(os.tmpdir(), "mo-diffs-language-pack-"));
      try {
        ...
        fs.writeFileSync(
          join(languagePackRoot, "mo.plugin.json"),
