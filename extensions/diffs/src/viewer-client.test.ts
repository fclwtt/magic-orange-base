...
const disableAutoStartKey = Symbol.for("mo.diffs.disableAutoStart");
...
function renderCard(payloadOverride?: string): void {
  ...
  document.body.insertAdjacentHTML(
    ...
    `<section class="oc-diff-card">
      <div data-mo-diff-host></div>
      <script type="application/json" data-mo-diff-payload>${payload}</script>
    </section>`,
  );
}
...
describe("hydrateViewer", () => {
  beforeEach(() => {
    ...
    delete document.documentElement.dataset.moDiffsError;
    delete document.documentElement.dataset.moDiffsReady;
    vi.clearAllMocks();
  });
  ...
  it("continues hydrating later cards when one card throws", async () => {
    ...
    expect(document.documentElement.dataset.moDiffsError).toBeUndefined();
    warn.mockRestore();
  });
...
describe("ensureShadowRoot", () => {
  ...
  it("attaches shadow root from template and removes template element", async () => {
    renderCard();
    const host = document.querySelector<HTMLElement>("[data-mo-diff-host]")!;
