...
import { createMockServerResponse } from "mo/plugin-sdk/test-env";
...
describe("PlaywrightDiffScreenshotter", () => {
  ...
  beforeEach(async () => {
    vi.useFakeTimers();
    ({ rootDir, cleanup: cleanupRootDir } = await createTempDiffRoot("mo-diffs-browser-"));
    ...
    let configFile: OpenClawConfig = {
      ...
      plugins: {
        entries: {
          diffs: {
            config: {
              viewerBaseUrl: "https://startup.example.com/mo",
              defaults: {
                mode: "view",
                theme: "light",
                background: false,
