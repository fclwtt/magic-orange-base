...
import type { AnyAgentTool, OpenClawPluginApi } from "mo/plugin-sdk/plugin-entry";
...
describe("Canvas plugin entry", () => {
  ...
  it("defers Canvas host implementation until a registered route is used", async () => {
    ...
    await routes[0]?.handler({ url: "/__mo__/canvas" } as never, {} as never);
  ...
  it("defers Canvas resolver, CLI, and tool implementations until use", async () => {
    ...
    await expect(resolvers[0]?.("/__mo__/canvas/documents/id/index.html")).resolves.toBe(
      "/tmp/canvas-asset",
    );
