...
const hashFile =
  process.env.MO_A2UI_BUNDLE_HASH_FILE ??
  path.join(pluginDir, "src", "host", "a2ui", ".bundle.hash");
const outputFile =
  process.env.MO_A2UI_BUNDLE_OUT ??
...
async function main() {
  ...
  if (!hasA2uiPackage || !hasAppDir) {
    ...
    if (process.env.MO_SPARSE_PROFILE || process.env.MO_A2UI_SKIP_MISSING === "1") {
      console.error(
        "A2UI package missing; skipping bundle because MO_A2UI_SKIP_MISSING=1 or MO_SPARSE_PROFILE is set.",
      );
      return;
    }
