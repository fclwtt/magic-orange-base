...
import { detectMime } from "mo/plugin-sdk/media-mime";
import { isTruthyEnvValue, type RuntimeEnv } from "mo/plugin-sdk/runtime-env";
import { resolveStateDir } from "mo/plugin-sdk/state-paths";
import {
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.moCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.moCanvasA2UIAction &&
        typeof window.moCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.moSendUserAction === "function";
  ...
  window.addEventListener("mo:a2ui-action-status", onStatus);
  ...
  function send(name, sourceComponentId) {
    ...
    const sendUserAction =
      typeof window.moSendUserAction === "function"
        ? window.moSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.MO_SKIP_CANVAS_HOST)) {
    return true;
  }
