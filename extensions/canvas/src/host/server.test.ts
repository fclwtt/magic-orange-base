...
import { defaultRuntime } from "mo/plugin-sdk/runtime-env";
...
describe("canvas host", () => {
  ...
  beforeAll(async () => {
    ...
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mo-canvas-fixtures-"));
  });
  ...
  it("injects live reload script", () => {
    ...
    expect(out).toContain("moCanvasA2UIAction");
    expect(out).toContain("moSendUserAction");
  });
  ...
  it("creates a default index.html when missing", async () => {
    ...
    try {
      ...
      expect(response.body).toContain("moSendUserAction");
  ...
  it("serves A2UI scaffold and blocks traversal/symlink escapes", async () => {
    ...
    await fs.writeFile(
      ...
      `<mo-a2ui-host></mo-a2ui-host>
<script>moCanvasA2UIAction</script>`,
      "utf8",
    );
    await fs.writeFile(path.join(a2uiRoot, "a2ui.bundle.js"), "window.moA2UI = {};", "utf8");
