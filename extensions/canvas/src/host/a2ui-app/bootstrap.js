...
import { themeContext } from "@mo/a2ui-theme-context";
...
class OpenClawA2UIHost extends LitElement {
  ...
  connectedCallback() {
    ...
    globalThis.moA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["mo:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    const handler =
      globalThis.webkit?.messageHandlers?.moCanvasA2UIAction ??
      globalThis.moCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.moCanvasA2UIAction) {
...
if (!customElements.get("mo-a2ui-host")) {
  customElements.define("mo-a2ui-host", OpenClawA2UIHost);
}
