...
vi.mock("mo/plugin-sdk/memory-host-search", () => ({
...
describe("active-memory plugin", () => {
  ...
  const api: any = {
    ...
    runtime: {
      agent: {
        runEmbeddedAgent,
        session: {
          resolveStorePath: vi.fn(() => "/tmp/mo-session-store.json"),
  ...
  beforeEach(async () => {
    ...
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-active-memory-test-"));
    configFile = {
      plugins: {
        entries: {
          "active-memory": {
  ...
  it("keeps subagent transcripts off disk by default by using a temp session file", async () => {
    ...
    expect(sessionFile).toMatch(/mo-active-memory-.*\/session\.jsonl$/);
