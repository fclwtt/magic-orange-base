 ...
 * silk-wasm availability checks — all without importing `mo/plugin-sdk`.
 ...
 * `MO_HOME`. QQ Bot still checks this tree for legacy state imports and
 ...
 * package with `mo` as a peer dependency), so this re-implements the
 * minimal contract:
 *
 * 1. `MO_HOME` when set (with `~` / `~/...` expanded against the OS home).
...
function resolveOpenClawHome(): string {
  const raw = process.env.MO_HOME?.trim();
...
/**
 * Return a legacy path under `~/.mo/qqbot` without creating it.
 *
...
export function getQQBotDataPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".mo", "qqbot", ...subPaths);
}

/** Return a path under `~/.mo/qqbot`, creating it on demand. */
...
/**
 * Return a path under `<mo-home>/.mo/media/qqbot` without creating it.
 *
 ...
 * The base honors `MO_HOME` (when set) so files written by agents into
 * the OpenClaw-managed media tree are reachable by this plugin even when
 * `HOME` and `MO_HOME` differ (Docker, multi-user hosts). Fixes #83562.
 */
export function getQQBotMediaPath(...subPaths: string[]): string {
  return path.join(resolveOpenClawHome(), ".mo", "media", "qqbot", ...subPaths);
}

/** Return a path under `<mo-home>/.mo/media/qqbot`, creating it on demand. */
...
/**
 * Return `<mo-home>/.mo/media`, OpenClaw's shared media root.
 *
 * This mirrors the directory that core's `buildMediaLocalRoots` exposes as an
 * allowlisted location (see `mo/src/media/local-roots.ts`). Using it as a
 ...
 * {@link getQQBotMediaPath}, the base honors `MO_HOME`.
 */
function getOpenClawMediaDir(): string {
  return path.join(resolveOpenClawHome(), ".mo", "media");
}
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const moHomeDir = resolveOpenClawHome();
  const mediaRoot = getQQBotMediaPath();
  const dataRoot = getQQBotDataPath();
  // When MO_HOME differs from HOME we have to consider workspace roots
  ...
  const workspaceRoots = Array.from(
    new Set([
      path.join(osHomeDir, ".mo", "workspace", "qqbot"),
      path.join(moHomeDir, ".mo", "workspace", "qqbot"),
    ]),
  );
...
export function resolveQQBotPayloadLocalFilePath(p: string): string | null {
  ...
  // Trust both the QQ Bot-owned subdirectory and OpenClaw's shared `~/.mo/media`
