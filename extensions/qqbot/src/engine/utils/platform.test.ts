...
describe("qqbot local media path remapping", () => {
  ...
  function createOpenClawTestRoot() {
    ...
    const moDir = path.join(actualHome, ".mo");
    fs.mkdirSync(moDir, { recursive: true });
    const testRoot = fs.mkdtempSync(path.join(moDir, "qqbot-platform-test-"));
  ...
  it("allows structured payload files inside sibling OpenClaw media subdirectories", () => {
    ...
    // trust the shared `~/.mo/media` root so auto-routed sends can access
    // those files without the path-outside-storage guard firing.
    const actualHome = getHomeDir();
    const outboundDir = path.join(actualHome, ".mo", "media", "outbound");
...
// Regression coverage for https://github.com/mo/mo/issues/83562 —
// when HOME and MO_HOME diverge (Docker, multi-user hosts), QQ Bot media
// paths must be anchored on MO_HOME so files written under
// `$MO_HOME/.mo/media/qqbot/` are accepted by the outbound
// allowlist.
//
...
// home as the baseline and only varies `process.env.MO_HOME`.
describe("qqbot media path resolution honors MO_HOME (#83562)", () => {
  ...
  it("accepts files under $MO_HOME/.mo/media/qqbot when MO_HOME differs from HOME", () => {
    const fakeOpenclawHome = makeFakeOpenclawHome();
    vi.stubEnv("MO_HOME", fakeOpenclawHome);

    const mediaFile = path.join(fakeOpenclawHome, ".mo", "media", "qqbot", "repro.png");
    ...
    const oldHomeMediaRoot = path.join(realOsHome, ".mo", "media", "qqbot");
    expect(isPathInsideOrEqual(mediaFile, oldHomeMediaRoot)).toBe(false);
    fs.mkdirSync(path.dirname(mediaFile), { recursive: true });
    fs.writeFileSync(mediaFile, "image", "utf8");

    expect(getQQBotMediaPath()).toBe(path.join(fakeOpenclawHome, ".mo", "media", "qqbot"));
    expect(resolveQQBotPayloadLocalFilePath(mediaFile)).toBe(fs.realpathSync(mediaFile));
  });

  it("expands tilde-prefixed MO_HOME against the OS home", () => {
    ...
    vi.stubEnv("MO_HOME", `~/${sub}`);

    expect(getQQBotMediaPath()).toBe(path.join(expectedHome, ".mo", "media", "qqbot"));

    const mediaFile = path.join(expectedHome, ".mo", "media", "qqbot", "tilde.png");
    fs.mkdirSync(path.dirname(mediaFile), { recursive: true });
    fs.writeFileSync(mediaFile, "image", "utf8");
  ...
  it("falls back to OS home when MO_HOME is unset (no regression)", () => {
    vi.stubEnv("MO_HOME", "");

    expect(getQQBotMediaPath()).toBe(path.join(realOsHome, ".mo", "media", "qqbot"));
  });

  it("treats sentinel strings 'undefined' and 'null' as unset", () => {
    for (const sentinel of ["undefined", "null"]) {
      vi.stubEnv("MO_HOME", sentinel);
      expect(getQQBotMediaPath()).toBe(path.join(realOsHome, ".mo", "media", "qqbot"));
    }
  });
  ...
  it("keeps persisted QQ Bot data anchored on the OS home (compatibility)", () => {
    ...
    // operator adds MO_HOME — otherwise existing deployments would lose
    // their session state. Only the media root follows MO_HOME.
    expect(getQQBotDataPath()).toBe(path.join(realOsHome, ".mo", "qqbot"));
  });

  it("rejects files that live under HOME tree when MO_HOME is the active root", () => {
    const fakeOpenclawHome = makeFakeOpenclawHome();
    vi.stubEnv("MO_HOME", fakeOpenclawHome);

    // File under the HOME-side mirror — exactly the path that *worked* on
    // current main and *broke* the MO_HOME setup. After the fix the
    // active media root is MO_HOME, so a file under HOME is no longer
    ...
    const homeOnlyFile = path.join(realOsHome, ".mo", "media", "qqbot", stale);
  ...
  it("remaps workspace paths under either HOME or MO_HOME to the MO_HOME media root", () => {
    const fakeOpenclawHome = makeFakeOpenclawHome();
    vi.stubEnv("MO_HOME", fakeOpenclawHome);

    const baseName = `remap-${process.pid}-${Date.now()}`;

    // Real file lives under the MO_HOME media tree.
    const mediaFile = path.join(
      fakeOpenclawHome,
      ".mo",
      "media",
    ...
    const homeWorkspaceDir = path.join(realOsHome, ".mo", "workspace", "qqbot");
    ...
    // HOME-side path, so nothing to clean up there beyond the MO_HOME tree.
    expect(resolveQQBotLocalMediaPath(homeWorkspacePath)).toBe(mediaFile);

    // Same path but under MO_HOME should also remap.
    const moWorkspacePath = path.join(
      fakeOpenclawHome,
      ".mo",
      "workspace",
      "qqbot",
    ...
    expect(resolveQQBotLocalMediaPath(moWorkspacePath)).toBe(mediaFile);
  });
});
