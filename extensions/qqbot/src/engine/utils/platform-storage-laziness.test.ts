...
describe("qqbot storage laziness", () => {
  ...
  it("does not create ~/.mo/qqbot from module imports or read-only probes", async () => {
    ...
    const qqbotRoot = path.join(homeDir, ".mo", "qqbot");
  ...
  it("creates storage when qqbot persists runtime state", async () => {
    ...
    const qqbotRoot = path.join(homeDir, ".mo", "qqbot");
    const sqlitePath = path.join(stateDir, "state", "mo.sqlite");
