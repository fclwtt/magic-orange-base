...
import { withEnv } from "mo/plugin-sdk/test-env";
...
describe("qqbot legacy credential backup paths", () => {
  ...
  it("scopes legacy credential backup imports to the active MO_STATE_DIR", () => {
  ...
  it("keeps legacy account import paths isolated across different state directories", () => {
    ...
    const gatewayAPath = withEnv({ MO_STATE_DIR: stateDirA }, () =>
      getCredentialBackupFile("default"),
    );
    const gatewayBPath = withEnv({ MO_STATE_DIR: stateDirB }, () =>
      getCredentialBackupFile("default"),
    );
  ...
  it("uses MO_HOME for default legacy credential backup imports", () => {
    const homeDir = createTempDir("qqbot-mo-home-");
    withEnv({ MO_STATE_DIR: "", MO_HOME: homeDir }, () => {
      expect(getCredentialBackupFile("default")).toBe(
        path.join(homeDir, ".mo", "qqbot", "data", "credential-backup-default.json"),
      );
    });
  });
  ...
  it("expands tilde state-dir overrides through the canonical state resolver", () => {
    ...
    withEnv({ HOME: homeDir, MO_HOME: "", MO_STATE_DIR: "~/gateway-a" }, () => {
      expect(getCredentialBackupFile("default")).toBe(
