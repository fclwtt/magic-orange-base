...
let moVersion = "unknown";

/** Build the User-Agent string from the current plugin and framework versions. */
function buildUserAgent(): string {
  return `QQBotPlugin/${pluginVersion} (Node/${process.versions.node}; ${os.platform()}; OpenClaw/${moVersion})`;
}
 ...
 */
export function initSender(options: { pluginVersion?: string; moVersion?: string }): void {
  if (options.pluginVersion) {
    pluginVersion = options.pluginVersion;
  }
  if (options.moVersion) {
    moVersion = options.moVersion;
  }
}
