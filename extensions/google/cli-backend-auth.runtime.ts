...
import type { CliBackendPreparedExecution } from "mo/plugin-sdk/cli-backend";
...
function throwUnstageableSelectedGeminiProfile(
  ...
  if (!credential) {
    throw new Error(
      "Gemini CLI auth profile was selected but no credential material was found. Re-authenticate with `mo models auth login --provider google-gemini-cli --force`.",
    );
  }
  ...
  throw new Error(
    "Gemini CLI execution supports google-gemini-cli OAuth or API-key auth profiles. Re-authenticate with `mo models auth login --provider google-gemini-cli --force`.",
  );
}
...
function requireGeminiOAuthCredential(
  ...
  if (
    ...
    throw new Error(
      "Gemini CLI OAuth profile is missing usable token material. Re-authenticate with `mo models auth login --provider google-gemini-cli --force`.",
    );
  }
...
async function prepareGeminiCliProfileHome(
  ...
  const systemSettingsDir = await fs.mkdtemp(
    path.join(resolvePreferredOpenClawTmpDir(), "mo-gemini-cli-"),
  );
