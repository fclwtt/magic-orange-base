...
describe("extractGeminiCliCredentials", () => {
  const ENV_KEYS = [
    "MO_GEMINI_OAUTH_CLIENT_ID",
    "MO_GEMINI_OAUTH_CLIENT_SECRET",
...
describe("loginGeminiCliOAuth", () => {
  ...
  function userInfoResponse(): Response {
    return responseJson({ email: "lobster@mo.ai" });
  }
  ...
  beforeEach(() => {
    envSnapshot = Object.fromEntries(ENV_KEYS.map((key) => [key, process.env[key]]));
    process.env.MO_GEMINI_OAUTH_CLIENT_ID = "test-client-id.apps.googleusercontent.com";
    process.env.MO_GEMINI_OAUTH_CLIENT_SECRET = "GOCSPX-test-client-secret"; // pragma: allowlist secret
