...
vi.mock("mo/plugin-sdk/sandbox", async (importOriginal) => {
  const actual = await importOriginal<typeof import("mo/plugin-sdk/sandbox")>();
  return {
    ...actual,
...
describe("openshell backend exec workdir validation", () => {
  beforeEach(() => {
    ...
    cliMocks.createOpenShellSshSession.mockResolvedValue({
      command: "ssh",
      configPath: "/tmp/mo-openshell-test-ssh-config",
      host: "openshell-test",
    });
    ...
    sdkMocks.runSshSandboxCommand.mockImplementation(async ({ remoteCommand }) => ({
      stdout: String(remoteCommand).includes("mo-validate-workdir")
