...
import type { CreateSandboxBackendParams } from "mo/plugin-sdk/sandbox";
import {
...
describe("openshell backend manager", () => {
  ...
  it("uploads staged directory snapshots to the managed remote directory itself", () => {
    expect(
      buildOpenShellDirectoryUploadArgs({
        sandboxName: "mo-session",
        localPath: "/tmp/mo-upload/sandbox/seed.txt",
        remotePath: "/sandbox",
      }),
    ).toEqual([
      "sandbox",
      ...
      "--no-git-ignore",
      "mo-session",
      "/tmp/mo-upload/sandbox/seed.txt",
      "/sandbox",
    ]);
    expect(
      buildOpenShellDirectoryUploadArgs({
        sandboxName: "mo-session",
        localPath: "/tmp/mo-upload/project",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const realParent = await makeTempDir("mo-openshell-real-");
      const root = path.join(realParent, "sandbox");
      const target = path.join(root, ".mo", "sandbox-skills");
  ...
  it("checks runtime status with config override from OpenClaw config", async () => {
    ...
    const result = await manager.describeRuntime({
      entry: {
        containerName: "mo-session-1234",
        backendId: "openshell",
        runtimeLabel: "mo-session-1234",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: {
        sandboxName: "mo-session-1234",
        config: expectedConfig,
      },
      args: ["sandbox", "get", "mo-session-1234"],
    });
  });
  ...
  it("removes runtimes via openshell sandbox delete", async () => {
    ...
    await manager.removeRuntime({
      entry: {
        containerName: "mo-session-5678",
        backendId: "openshell",
        runtimeLabel: "mo-session-5678",
        sessionKey: "agent:main",
        createdAtMs: 1,
        lastUsedAtMs: 1,
        image: "mo",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: {
        sandboxName: "mo-session-5678",
        config: expectedConfig,
      },
      args: ["sandbox", "delete", "mo-session-5678"],
    });
  });
  ...
  it("preserves a local sandbox skills shadow when mirror sync crosses filesystems", async () => {
    const workspaceDir = await makeTempDir("mo-openshell-workspace-");
    const shadowFile = path.join(workspaceDir, ".mo", "sandbox-skills", "user-note.txt");
    ...
    cliMocks.runOpenShellCli.mockImplementation(async ({ args }: { args: string[] }) => {
      if (args[0] === "sandbox" && args[1] === "download") {
        ...
        await fs.mkdir(path.join(tmpDir, ".mo", "sandbox-skills", "skills"), {
          recursive: true,
        });
        await fs.writeFile(
          path.join(tmpDir, ".mo", "sandbox-skills", "skills", "generated.txt"),
          "generated",
          "utf8",
        );
      }
    ...
    try {
      ...
      await expectPathMissing(
        path.join(workspaceDir, ".mo", "sandbox-skills", "skills", "generated.txt"),
      );
    } finally {
  ...
  it("drops non-directory materialized sandbox skills from mirror downloads", async () => {
    ...
    cliMocks.runOpenShellCli.mockImplementation(async ({ args }: { args: string[] }) => {
      if (args[0] === "sandbox" && args[1] === "download") {
        const tmpDir = args[4];
        await fs.writeFile(path.join(tmpDir, "from-remote.txt"), "remote", "utf8");
        await fs.mkdir(path.join(tmpDir, ".mo"), { recursive: true });
        await fs.writeFile(path.join(tmpDir, ".mo", "sandbox-skills"), "poison", "utf8");
      }
      return { code: 0, stdout: "", stderr: "" };
    });
    ...
    await expectPathMissing(path.join(workspaceDir, ".mo", "sandbox-skills"));
  });
  ...
  it("restores a local sandbox skills shadow when mirror download has a file parent", async () => {
    ...
    cliMocks.runOpenShellCli.mockImplementation(async ({ args }: { args: string[] }) => {
      if (args[0] === "sandbox" && args[1] === "download") {
        const tmpDir = args[4];
        await fs.writeFile(path.join(tmpDir, "from-remote.txt"), "remote", "utf8");
        await fs.writeFile(path.join(tmpDir, ".mo"), "poison", "utf8");
      }
      return { code: 0, stdout: "", stderr: "" };
    });
    ...
    await expect(fs.readFile(shadowFile, "utf8")).resolves.toBe("local shadow");
    expect((await fs.stat(path.join(workspaceDir, ".mo"))).isDirectory()).toBe(true);
  });
});
...
function createOpenShellBackendSandboxConfig(): CreateSandboxBackendParams["cfg"] {
  return {
    ...
    scope: "session",
    workspaceAccess: "rw",
    workspaceRoot: "/tmp/mo-sandboxes",
    docker: {
      image: "mo-sandbox:bookworm-slim",
      containerPrefix: "mo-sbx-",
      workdir: "/workspace",
      readOnlyRoot: false,
      tmpfs: [],
      network: "none",
    ...
    ssh: createSandboxSshConfig("/tmp/mo-sandboxes"),
...
async function makeExecutable(params: { name: string; script: string }): Promise<string> {
  const dir = await makeTempDir("mo-openshell-bin-");
...
describe("openshell fs bridges", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const stateDir = await makeTempDir("mo-openshell-remote-pin-");
  ...
  it("writes locally and syncs the file to the remote workspace", async () => {
    const workspaceDir = await makeTempDir("mo-openshell-fs-");
  ...
  it("rejects cross-root mirror renames before the remote backend commit", async () => {
    const workspaceDir = await makeTempDir("mo-openshell-fs-");
    const agentWorkspaceDir = await makeTempDir("mo-openshell-agent-fs-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = await makeTempDir("mo-openshell-fs-");
      const outsideDir = await makeTempDir("mo-openshell-outside-");
  ...
  it("reads materialized sandbox skills from the protected skills workspace", async () => {
    const workspaceDir = await makeTempDir("mo-openshell-fs-");
    const skillsWorkspaceDir = await makeTempDir("mo-openshell-skills-");
    ...
    await expect(
      bridge.readFile({
        filePath: "/sandbox/.mo/sandbox-skills/skills/demo/SKILL.md",
      }),
    ).resolves.toEqual(Buffer.from("# Demo\nmaterialized\n"));
    await expect(
      bridge.readFile({
        filePath: ".mo/sandbox-skills/skills/demo/SKILL.md",
      }),
    ).resolves.toEqual(Buffer.from("# Demo\nmaterialized\n"));
    await expect(
      bridge.writeFile({
  ...
  it("maps agent mount paths when the sandbox workspace is read-only", async () => {
    const workspaceDir = await makeTempDir("mo-openshell-fs-");
    const agentWorkspaceDir = await makeTempDir("mo-openshell-agent-");
