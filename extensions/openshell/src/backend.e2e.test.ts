...
import { createSandboxTestContext } from "mo/plugin-sdk/test-fixtures";
import {
...
const MO_OPENSHELL_E2E = process.env.MO_E2E_OPENSHELL === "1";
const MO_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const MO_OPENSHELL_COMMAND =
  process.env.MO_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
const MO_OPENSHELL_CONFIG_HOME =
  process.env.MO_E2E_OPENSHELL_CONFIG_HOME?.trim() || null;
const MO_OPENSHELL_HOST_IP = process.env.MO_E2E_OPENSHELL_HOST_IP?.trim() || null;
...
RUN echo "mo-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
async function resolveOpenShellHostIp(): Promise<string> {
  if (MO_OPENSHELL_HOST_IP) {
    return MO_OPENSHELL_HOST_IP;
  }
  ...
  throw new Error(
    "OpenShell E2E could not resolve the OpenShell Docker network gateway; set MO_E2E_OPENSHELL_HOST_IP",
  );
}
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && MO_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: MO_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        throw new Error("OpenShell E2E requires a working Docker daemon");
      }
      if (!(await commandAvailable(MO_OPENSHELL_COMMAND))) {
        throw new Error(`OpenShell CLI is unavailable: ${MO_OPENSHELL_COMMAND}`);
      }
      if (!MO_OPENSHELL_CONFIG_HOME) {
        throw new Error(
          "OpenShell E2E requires MO_E2E_OPENSHELL_CONFIG_HOME because tests isolate HOME and XDG_CONFIG_HOME",
        );
      }
      const openshellConfigHome = MO_OPENSHELL_CONFIG_HOME;
      const hostIp = await resolveOpenShellHostIp();
      const gatewayName = await activeOpenShellGateway(MO_OPENSHELL_COMMAND, {
        ...process.env,
      ...
      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-openshell-e2e-"));
      ...
      const allowSandboxName = `mo-policy-allow-${scopeSuffix}`;
      ...
      try {
        ...
        expect(stdout).toContain("mo-openshell-e2e");
