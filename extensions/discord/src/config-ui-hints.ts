...
import type { ChannelConfigUiHint } from "mo/plugin-sdk/channel-core";
...
export const discordChannelConfigUiHints = {
  ...
  gatewayInfoTimeoutMs: {
    ...
    help: "Timeout for Discord /gateway/bot metadata lookup before falling back to the default gateway URL. Default is 30000; MO_DISCORD_GATEWAY_INFO_TIMEOUT_MS can override when config is unset.",
  },
  gatewayReadyTimeoutMs: {
    label: "Discord Gateway READY Timeout (ms)",
    help: "Startup wait for the Discord gateway READY event before restarting the socket. Default is 15000; MO_DISCORD_READY_TIMEOUT_MS can override when config is unset.",
  },
  gatewayRuntimeReadyTimeoutMs: {
    label: "Discord Gateway Runtime READY Timeout (ms)",
    help: "Runtime reconnect wait for the Discord gateway READY event before force-stopping the lifecycle. Default is 30000; MO_DISCORD_RUNTIME_READY_TIMEOUT_MS can override when config is unset.",
  },
  "voice.enabled": {
