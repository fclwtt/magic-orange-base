...
import type { NativeCommandSpec } from "mo/plugin-sdk/command-auth-native";
import { resolveDirectStatusReplyForSession } from "mo/plugin-sdk/command-status-runtime";
...
import { dispatchReplyWithDispatcher } from "mo/plugin-sdk/reply-dispatch-runtime";
import {
...
describe("Discord native plugin command dispatch", () => {
  ...
  beforeEach(() => {
    ...
    discordNativeCommandTesting.setMatchPluginCommand(
      runtimeModuleMocks.matchPluginCommand as typeof import("mo/plugin-sdk/plugin-runtime").matchPluginCommand,
    );
    discordNativeCommandTesting.setExecutePluginCommand(
      runtimeModuleMocks.executePluginCommand as typeof import("mo/plugin-sdk/plugin-runtime").executePluginCommand,
    );
    ...
    discordNativeCommandTesting.setGetSessionEntry(
      runtimeModuleMocks.getSessionEntry as typeof import("mo/plugin-sdk/session-store-runtime").getSessionEntry,
    );
  });
  ...
  it("passes the configured binding agent to plugin-owned Discord command sessions", async () => {
    const cfg = createConfig();
    const interaction = createInteraction();
    const pluginSessionKey = "plugin-binding:mo-codex-app-server:dm";
