...
import * as commandRegistryModule from "mo/plugin-sdk/command-auth-native";
import type {
  ChatCommandDefinition,
  CommandArgsParsing,
} from "mo/plugin-sdk/command-auth-native";
import type { ModelsProviderData } from "mo/plugin-sdk/command-auth-native";
import type { OpenClawConfig } from "mo/plugin-sdk/config-contracts";
import * as globalsModule from "mo/plugin-sdk/runtime-env";
import {
  loadSessionStore,
  resolveStorePath,
  upsertSessionEntry,
} from "mo/plugin-sdk/session-store-runtime";
import * as commandTextModule from "mo/plugin-sdk/text-utility-runtime";
...
describe("Discord model picker interactions", () => {
  beforeEach(async () => {
    tempDir = await mkdtemp(path.join(os.tmpdir(), "mo-discord-model-picker-"));
    vi.useRealTimers();
    vi.restoreAllMocks();
  });
    ...
    pickerData.runtimeChoicesByProvider = new Map([
      [
        ...
        [
          ...
          { id: "mo", label: "OpenClaw Default", description: "Use OpenClaw." },
        ],
      ],
    ]);
    const modelCommand = createModelCommandDefinition();
