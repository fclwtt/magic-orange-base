...
vi.mock("mo/plugin-sdk/channel-outbound", async () => {
  const actual = await vi.importActual<typeof import("mo/plugin-sdk/channel-outbound")>(
    "mo/plugin-sdk/channel-outbound",
  );
  return {
    ...actual,
...
describe("deliverDiscordReply", () => {
      ...
      replies: [
        {
          text: [
            ...
            "🛠️ run git status",
            "⚠️ 🛠️ `run mo definitely-not-a-real-subcommand (agent)` failed",
            "🛠️ `gh pr view`",
            "🛠️ `docker compose up`",
      ...
      replies: [
        {
          text: [
            "[tool:exec]",
            "<parameter=command>",
            'cat /proc/mounts 2>/dev/null | grep -i "libra|rav|mo" | head -20',
            "</parameter>",
            "",
            "<function=exec>",
            "<parameter=command>",
  ...
  it("passes media roots and explicit off-mode payload reply tags to shared outbound", async () => {
    ...
    expect(params.mediaAccess).toEqual({ localRoots: ["/tmp/mo-media"] });
  });
