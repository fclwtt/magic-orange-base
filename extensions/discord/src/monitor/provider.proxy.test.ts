...
vi.mock("mo/plugin-sdk/proxy-capture", () => ({
  ...
  resolveEffectiveDebugProxyUrl: (configuredProxyUrl?: string) =>
    configuredProxyUrl?.trim() || process.env.MO_DEBUG_PROXY_URL,
...
describe("createDiscordGatewayPlugin", () => {
  ...
  beforeEach(() => {
    vi.unstubAllEnvs();
    vi.stubEnv("MO_DEBUG_PROXY_ENABLED", "");
    vi.stubEnv("MO_DEBUG_PROXY_URL", "");
  ...
  it("uses env gateway metadata timeout when config is unset", async () => {
    vi.useFakeTimers();
    vi.stubEnv("MO_DISCORD_GATEWAY_INFO_TIMEOUT_MS", "6000");
    const runtime = createRuntime();
    globalFetchMock.mockImplementation(() => new Promise(() => {}));
    const plugin = createDiscordGatewayPlugin({
      discordConfig: {},
