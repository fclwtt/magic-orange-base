...
import { DEFAULT_EMOJIS, DEFAULT_TIMING } from "mo/plugin-sdk/channel-feedback";
import {
  recordChannelBotPairLoopAndCheckSuppression,
  type ChannelBotLoopProtectionFacts,
} from "mo/plugin-sdk/channel-inbound";
import type { ReplyPayload } from "mo/plugin-sdk/reply-dispatch-runtime";
import { setReplyPayloadMetadata } from "mo/plugin-sdk/reply-payload-testing";
import * as runtimeEnvModule from "mo/plugin-sdk/runtime-env";
...
function createNonTerminalToolWarningPayload(): ReplyPayload {
  return setReplyPayloadMetadata(
    {
      text: "⚠️ 🛠️ `run mo definitely-not-a-real-subcommand (agent)` failed",
      isError: true,
    },
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/mo-discord-process-test-sessions.json",
  ),
}));
...
vi.mock("mo/plugin-sdk/reply-runtime", () => ({
...
vi.mock("mo/plugin-sdk/conversation-runtime", () => ({
...
vi.mock("mo/plugin-sdk/session-store-runtime", () => ({
...
vi.mock("mo/plugin-sdk/session-transcript-runtime", () => ({
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/mo-discord-process-test-sessions.json");
    ...
    try {
      ...
      const ctx = await createBaseContext({
        ...
        cfg: {
          ...
          session: { store: "/tmp/mo-discord-process-test-sessions.json" },
        },
  ...
  it("pins Discord text DM main-route updates to the single configured DM owner", async () => {
    const ctx = await createBaseContext({
      ...
      cfg: {
        messages: { ackReaction: "👀" },
        session: {
          store: "/tmp/mo-discord-process-test-sessions.json",
          dmScope: "main",
        },
      },
