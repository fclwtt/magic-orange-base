...
import type { RealtimeVoiceAgentControlResult } from "mo/plugin-sdk/realtime-voice";
...
vi.mock("mo/plugin-sdk/routing", async () => {
  const actual = await vi.importActual<typeof import("mo/plugin-sdk/routing")>(
    "mo/plugin-sdk/routing",
  );
  return {
    ...actual,
...
vi.mock("mo/plugin-sdk/agent-runtime", async () => {
  ...
  return {
    ...
    resolveAgentDir: vi.fn(() => "/tmp/mo-agent"),
    resolveTtsConfig: vi.fn(() => ({
      modelOverrides: {},
      providerConfigs: {},
    })),
    resolveTtsPrefsPath: vi.fn(() => "/tmp/mo-tts.json"),
  };
});

vi.mock("mo/plugin-sdk/realtime-bootstrap-context", async () => {
  const actual = await vi.importActual<
    typeof import("mo/plugin-sdk/realtime-bootstrap-context")
  >("mo/plugin-sdk/realtime-bootstrap-context");
  return {
    ...actual,
...
vi.mock("mo/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("mo/plugin-sdk/runtime-env")>(
    "mo/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("mo/plugin-sdk/realtime-voice", async () => {
  const actual = await vi.importActual<typeof import("mo/plugin-sdk/realtime-voice")>(
    "mo/plugin-sdk/realtime-voice",
  );
  return {
    ...actual,
...
describe("DiscordVoiceManager", () => {
  ...
  it("uses agent-proxy realtime voice by default", async () => {
    ...
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("mo_agent_consult");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("mo_agent_control");
    const player = getLastAudioPlayer();
    bridgeParams?.audioSink?.sendAudio(Buffer.alloc(24_000));
  ...
  it("accepts OpenClaw as a default wake name before realtime agent-proxy consults", async () => {
    agentCommandMock.mockResolvedValueOnce({ payloads: [{ text: "mo wake answer" }] });
    const manager = createManager(
      {
        groupPolicy: "open",
        voice: {
    ...
    expectUserMessageIncludes("mo wake answer");
  });
  ...
  it("keeps forced agent-proxy fallback diagnostics out of agent prompts", async () => {
    ...
    expect(lastAgentCommandArgs().message).not.toContain("mo_agent_consult");
  ...
  it("starts Discord realtime voice in bidi mode with the consult tool", async () => {
    ...
    expect(bridgeParams?.autoRespondToAudio).toBe(true);
    expect(bridgeParams?.interruptResponseOnInputAudio).toBe(false);
    expect(bridgeParams?.instructions).toContain("Call mo_agent_consult");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("mo_agent_consult");
