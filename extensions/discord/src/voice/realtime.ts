...
import type { DiscordAccountConfig, OpenClawConfig } from "mo/plugin-sdk/config-contracts";
import {
  asDateTimestampMs,
...
import { asBoolean, uniqueStrings } from "mo/plugin-sdk/string-coerce-runtime";
...
const DISCORD_REALTIME_FORCED_CONSULT_REASON =
  "provider_final_transcript_without_mo_agent_consult";
...
export function buildDiscordSpeakExactUserMessage(text: string): string {
  return [
    ...
    "Do not call mo_agent_consult or any other tool for this message.",
...
function buildDiscordRealtimeInstructions(params: {
  ...
  if (isDiscordAgentProxyVoiceMode(params.mode)) {
    return [
      ...
      "Delegate substantive requests, actions, tool work, current facts, memory, workspace context, and user-specific context with mo_agent_consult.",
