...
async function evaluatePolicyUncached(ctx: HealthCheckContext): Promise<PolicyEvaluation> {
  ...
  if (
    ...
    findings.push({
      ...
      requirement: "oc://mo.config/plugins/entries/policy/config/expectedHash",
  ...
  return [
    {
      ...
      requirement: "oc://mo.config/plugins/entries/policy/config/expectedAttestationHash",
...
function isGroupIngressDisabled(
  ...
  const channelDefaultsParent = "oc://mo.config/channels/defaults";
    ...
    if (invalidIndex >= 0) {
      findings.push(
        policyShapeFinding(
          ...
          "Use non-empty source names such as env, file, exec, or mo.",
        ),
      );
    }
  }
...
function resolvePolicyArtifactHomeDir(): string | undefined {
  const explicitHome = normalizedEnvValue(process.env.MO_HOME);
  ...
  return [
    ...new Set(
      findings
        ...
        .map((finding) => finding.ocPath?.match(/^oc:\/\/mo\.config\/channels\/(.+)$/)?.[1])
...
function execApprovalsArtifactLocation(ctx: HealthCheckContext): {
  ...
  const stateDir = normalizedEnvValue(process.env.MO_STATE_DIR);
