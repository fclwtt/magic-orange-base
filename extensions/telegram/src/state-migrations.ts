...
import type { ChannelLegacyStateMigrationPlan } from "mo/plugin-sdk/channel-contract";
import { resolveChannelAllowFromPath } from "mo/plugin-sdk/channel-pairing";
...
} from "mo/plugin-sdk/persistent-dedupe";
import { createPluginStateSyncKeyedStore } from "mo/plugin-sdk/runtime-doctor";
import { statRegularFileSync } from "mo/plugin-sdk/security-runtime";
import { resolveStorePath } from "mo/plugin-sdk/session-store-runtime";
...
function detectTelegramMessageDispatchLegacyStateMigration(params: {
  ...
  const storePath = resolveStorePath(params.cfg.session?.store, { env: params.env });
  const legacyStorePath = resolveLegacySessionStorePath(params);
  const env = params.stateDir ? { ...params.env, MO_STATE_DIR: params.stateDir } : params.env;
