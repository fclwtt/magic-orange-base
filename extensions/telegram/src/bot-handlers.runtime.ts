...
import { parseExecApprovalCommandText } from "mo/plugin-sdk/approval-reply-runtime";
import { resolveChannelConfigWrites } from "mo/plugin-sdk/channel-config-helpers";
import {
  buildMentionRegexes,
...
} from "mo/plugin-sdk/channel-inbound-debounce";
import { resolveStoredModelOverride } from "mo/plugin-sdk/command-auth-native";
import { hasControlCommand } from "mo/plugin-sdk/command-detection";
import { isAbortRequestText } from "mo/plugin-sdk/command-primitives-runtime";
import { buildCommandsMessagePaginated } from "mo/plugin-sdk/command-status";
import type { DmPolicy, OpenClawConfig } from "mo/plugin-sdk/config-contracts";
import type {
  TelegramGroupConfig,
  TelegramTopicConfig,
} from "mo/plugin-sdk/config-contracts";
import { mutateConfigFile } from "mo/plugin-sdk/config-mutation";
import { resolveChannelContextVisibilityMode } from "mo/plugin-sdk/context-visibility-runtime";
import {
...
import { applyModelOverrideToSessionEntry } from "mo/plugin-sdk/model-session-runtime";
import { formatModelsAvailableHeader } from "mo/plugin-sdk/models-provider-runtime";
import { parseStrictPositiveInteger } from "mo/plugin-sdk/number-runtime";
import { resolveAgentRoute } from "mo/plugin-sdk/routing";
import { resolveThreadSessionKeys } from "mo/plugin-sdk/routing";
import { danger, logVerbose, warn } from "mo/plugin-sdk/runtime-env";
import { evaluateSupplementalContextVisibility } from "mo/plugin-sdk/security-runtime";
import {
  getSessionEntry,
  listSessionEntries,
  patchSessionEntry,
        ...
        if (modelCallback.type === "select") {
          ...
          try {
            ...
            const scopeText = isDefaultSelection
              ...
              : `Session-only model selection. Runtime unchanged. Use /model ${escapeHtml(selection.provider)}/${escapeHtml(selection.model)} --runtime &lt;runtime&gt; to switch harnesses. The agent default in mo.json is unchanged; /reset or a new session may return to that default.`;
            await editMessageWithButtons(
              `✅ Model ${actionText}\n\n${scopeText}`,
