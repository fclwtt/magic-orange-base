...
import type { MsgContext } from "mo/plugin-sdk/reply-runtime";
import { loadSessionStore } from "mo/plugin-sdk/session-store-runtime";
import { mockPinnedHostnameResolution } from "mo/plugin-sdk/test-env";
...
async function loadEnvelopeTimestampHelpers() {
  return await import("mo/plugin-sdk/channel-test-helpers");
}
...
describe("createTelegramBot", () => {
  ...
  it("blocks DM model-selection callbacks for unpaired users when inline buttons are DM-scoped", async () => {
    ...
    const storePath = `/tmp/mo-telegram-callback-authz-${process.pid}-${Date.now()}.json`;
  ...
  it("blocks group model-selection callbacks for senders who are not authorized for /models", async () => {
    ...
    const storePath = `/tmp/mo-telegram-group-model-authz-${process.pid}-${Date.now()}.json`;
  ...
  it("recomputes group model-selection callback auth from runtime command config", async () => {
    ...
    const storePath = `/tmp/mo-telegram-group-model-authz-runtime-${process.pid}-${Date.now()}.json`;
  ...
  it("routes compact model callbacks against the configured provider", async () => {
    ...
    const storePath = `/tmp/mo-telegram-model-compact-${process.pid}-${Date.now()}.json`;
  ...
  it("renders model callback lists with configured display names", async () => {
    ...
    const storePath = `/tmp/mo-telegram-model-display-names-${process.pid}-${Date.now()}.json`;
  ...
  it("resets overrides when selecting the configured default model", async () => {
    ...
    const storePath = `/tmp/mo-telegram-model-default-${process.pid}-${Date.now()}.json`;
    const config: OpenClawConfig = {
      agents: {
        defaults: {
  ...
  it("formats non-default model selection confirmations with Telegram HTML parse mode", async () => {
    ...
    const storePath = `/tmp/mo-telegram-model-html-${process.pid}-${Date.now()}.json`;
    ...
    try {
      ...
      expect(editCall[2]).toBe(
        `${CHECK_MARK_EMOJI} Model changed to <b>openai/gpt-5.4</b>\n\nSession-only model selection. Runtime unchanged. Use /model openai/gpt-5.4 --runtime &lt;runtime&gt; to switch harnesses. The agent default in mo.json is unchanged; /reset or a new session may return to that default.`,
      );
  ...
  it("persists non-default model override using fresh config, not stale startup snapshot", async () => {
    ...
    const storePath = `/tmp/mo-telegram-model-fresh-cfg-${process.pid}-${Date.now()}.json`;
  ...
  it("keeps direct Telegram media context when transcript context exists", async () => {
    ...
    const storePath = `/tmp/mo-telegram-dm-media-context-${process.pid}-${Date.now()}.json`;
    const config = {
      channels: {
        telegram: {
          dmPolicy: "open",
  ...
  it("skips direct transcript context for hard reset messages", async () => {
    ...
    const storePath = `/tmp/mo-telegram-dm-reset-context-${process.pid}-${Date.now()}.json`;
    const config = {
      channels: {
        telegram: {
          dmPolicy: "open",
    ...
    try {
      ...
      await handler({
        ...
        message: {
          chat,
          message_id: 103,
          text: "@mo_bot explain what went wrong",
          date: 1736380800,
