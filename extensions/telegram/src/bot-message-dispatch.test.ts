...
const getAgentScopedMediaLocalRoots = vi.hoisted(() =>
  vi.fn((_cfg: unknown, agentId: string) => [`/tmp/.mo/workspace-${agentId}`]),
);
...
vi.mock("mo/plugin-sdk/channel-outbound", async (importOriginal) => {
  const actual = await importOriginal<typeof import("mo/plugin-sdk/channel-outbound")>();
  return {
    ...actual,
...
vi.mock("mo/plugin-sdk/session-transcript-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("mo/plugin-sdk/session-transcript-runtime")>();
  return {
    ...actual,
...
describe("dispatchTelegramMessage draft streaming", () => {
  ...
  function createContext(overrides?: Partial<TelegramMessageContext>): TelegramMessageContext {
    ...
    base.turn = {
      storePath: "/tmp/mo/telegram-sessions.json",
  ...
  it("streams drafts in private threads and forwards thread id", async () => {
    ...
    expect(mediaLocalRoots?.some((root) => /[\\/]\.mo[\\/]workspace-work$/u.test(root))).toBe(
      true,
    );
  ...
  it("records streamed final replies into the prompt context cache", async () => {
    const storePath = `/tmp/mo-telegram-stream-context-${process.pid}-${Date.now()}.json`;
