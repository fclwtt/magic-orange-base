...
vi.mock("mo/plugin-sdk/config-mutation", async () => {
  const actual = await vi.importActual<typeof import("mo/plugin-sdk/config-mutation")>(
    "mo/plugin-sdk/config-mutation",
  );
  return {
    ...actual,
...
vi.mock("mo/plugin-sdk/cron-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("mo/plugin-sdk/cron-store-runtime")>(
    "mo/plugin-sdk/cron-store-runtime",
  );
  return {
    ...actual,
    loadCronStore,
...
export function installMaybePersistResolvedTelegramTargetTests(params?: {
  ...
  describe("maybePersistResolvedTelegramTarget", () => {
    ...
    it("writes back matching config and cron targets", async () => {
      ...
      expect(writeOptions.expectedConfigPath).toBe("/tmp/mo.json");
      expect(saveCronStore).toHaveBeenCalledTimes(1);
