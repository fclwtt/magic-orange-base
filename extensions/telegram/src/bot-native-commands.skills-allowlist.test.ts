...
import { listSkillCommandsForAgents as listActualSkillCommandsForAgents } from "mo/plugin-sdk/skill-commands-runtime";
...
describe("registerTelegramNativeCommands skill allowlist integration", () => {
  ...
  it("registers only allowlisted skills for the bound agent menu", async () => {
    const workspaceDir = await makeWorkspace("mo-telegram-skills-");
    await writeSkill({
