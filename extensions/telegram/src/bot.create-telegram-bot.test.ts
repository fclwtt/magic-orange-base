...
import { escapeRegExp, formatEnvelopeTimestamp } from "mo/plugin-sdk/channel-test-helpers";
import type { TelegramGroupConfig } from "mo/plugin-sdk/config-contracts";
import type { GetReplyOptions, MsgContext } from "mo/plugin-sdk/reply-runtime";
import { withEnvAsync } from "mo/plugin-sdk/test-env";
import { sanitizeTerminalText } from "mo/plugin-sdk/test-fixtures";
...
const pluginStateTestRuntime = await import("mo/plugin-sdk/plugin-state-test-runtime");
const conversationRuntime = await import("mo/plugin-sdk/conversation-runtime");
const configMutation = await import("mo/plugin-sdk/config-mutation");
const sessionStoreRuntime = await import("mo/plugin-sdk/session-store-runtime");
...
function createTelegramBotTestStateDir(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "mo-telegram-bot-"));
...
describe("createTelegramBot", () => {
  ...
  afterEach(() => {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.MO_STATE_DIR = previousStateDir;
    }
  ...
  beforeEach(async () => {
    previousStateDir = process.env.MO_STATE_DIR;
    process.env.MO_STATE_DIR = createTelegramBotTestStateDir();
    resetTelegramForumFlagCacheForTest();
  ...
  it.each(["stop", "/stop@mo_bot"] as const)(
  ...
  it("handles pairing DM flows for new and already-pending requests", async () => {
    ...
    for (const [index, testCase] of cases.entries()) {
      ...
      expect(pairingText, testCase.name).toContain("mo pairing approve telegram");
      expectRecordFields(
  ...
  it("ignores private self-authored message updates instead of issuing a pairing challenge", async () => {
    ...
    await handler({
      message: {
        ...
        message_id: 1884,
        date: 1736380800,
        from: { id: 7, is_bot: true, first_name: "OpenClaw", username: "mo_bot" },
        pinned_message: {
          message_id: 1883,
          date: 1736380799,
          chat: { id: 1234, type: "private", first_name: "Harold" },
          from: { id: 7, is_bot: true, first_name: "OpenClaw", username: "mo_bot" },
          text: "Binding: Review pull request 54118 (mo)",
        },
      },
      me: { id: 7, is_bot: true, first_name: "OpenClaw", username: "mo_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  ...
  it("reloads topic agent overrides between messages without recreating the bot", async () => {
    ...
    const sendTopicMessage = async (chatId: number, messageId: number, text: string) => {
      await handler({
        ...
        me: { username: "mo_bot", has_topics_enabled: true },
        getFile: async () => ({ download: async () => new Uint8Array() }),
      });
    };
  ...
  it("marks explicit Telegram bot-handle mentions in the inbound context", async () => {
    ...
    await dispatchMessage({
      message: {
        chat: { id: 7, type: "group", title: "Test Group" },
        text: "@mo_bot status",
        entities: [{ type: "mention", offset: 0, length: "@mo_bot".length }],
        date: 1736380800,
        message_id: 4,
        from: { id: 9, first_name: "Ada" },
      },
      me: { id: 999, username: "mo_bot" },
    });
    ...
    expect(payload.BotUsername).toBe("mo_bot");
  });
  ...
  it("honors routed group activation from session store", async () => {
    const storePath = "/tmp/mo-telegram-group-activation.json";
