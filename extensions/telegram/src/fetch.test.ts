...
import { resolveFetch } from "mo/plugin-sdk/fetch-runtime";
import { MAX_DATE_TIMESTAMP_MS } from "mo/plugin-sdk/number-runtime";
...
function writeTempCa(contents: string): string {
  const dir = mkdtempSync(path.join(os.tmpdir(), "mo-telegram-proxy-ca-"));
  tempDirs.push(dir);
...
describe("resolveTelegramFetch", () => {
  ...
  it("uses the OpenClaw debug proxy URL when no explicit proxy fetch is provided", async () => {
    vi.stubEnv("MO_DEBUG_PROXY_ENABLED", "1");
    vi.stubEnv("MO_DEBUG_PROXY_URL", "http://127.0.0.1:7777");
    undiciFetch.mockResolvedValue({ ok: true } as Response);
  ...
  it("uses MO_PROXY_URL as a Telegram explicit proxy when proxy env is absent", async () => {
    vi.stubEnv("MO_PROXY_URL", "http://127.0.0.1:7788");
    undiciFetch.mockResolvedValue({ ok: true } as Response);
  ...
  it("preserves caller-provided custom fetch when MO_PROXY_URL is present", async () => {
    vi.stubEnv("MO_PROXY_URL", "http://127.0.0.1:7788");
    const proxyFetch = vi.fn(async () => ({ ok: true }) as Response) as unknown as typeof fetch;
  ...
  it("prefers standard proxy env over MO_PROXY_URL for Telegram", async () => {
    vi.stubEnv("MO_PROXY_URL", "http://127.0.0.1:7788");
    vi.stubEnv("https_proxy", "http://127.0.0.1:7890");
    undiciFetch.mockResolvedValue({ ok: true } as Response);
  ...
  it("skips sticky IPv4 fallback when the DNS result order env override is verbatim", async () => {
    vi.stubEnv("MO_TELEGRAM_DNS_RESULT_ORDER", "verbatim");
    undiciFetch.mockResolvedValueOnce({ ok: true } as Response);
