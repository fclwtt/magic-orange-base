...
import type { ResolvedAgentRoute } from "mo/plugin-sdk/routing";
...
type ResolveConfiguredBindingRouteFn =
  typeof import("mo/plugin-sdk/conversation-runtime").resolveConfiguredBindingRoute;
type EnsureConfiguredBindingRouteReadyFn =
  typeof import("mo/plugin-sdk/conversation-runtime").ensureConfiguredBindingRouteReady;
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("mo/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
type LoadModelCatalogFn = typeof import("mo/plugin-sdk/agent-runtime").loadModelCatalog;
...
vi.mock("mo/plugin-sdk/conversation-runtime", async () => {
  const actual = await vi.importActual<typeof import("mo/plugin-sdk/conversation-runtime")>(
    "mo/plugin-sdk/conversation-runtime",
  );
  return {
    ...actual,
...
vi.mock("mo/plugin-sdk/session-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("mo/plugin-sdk/session-store-runtime")>(
    "mo/plugin-sdk/session-store-runtime",
  );
  return {
    ...actual,
...
vi.mock("mo/plugin-sdk/session-transcript-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("mo/plugin-sdk/session-transcript-runtime")
  >("mo/plugin-sdk/session-transcript-runtime");
  return {
    ...actual,
    resolveSessionTranscriptLegacyFileTarget: sessionMocks.resolveSessionTranscriptLegacyFileTarget,
  };
});
vi.mock("mo/plugin-sdk/command-auth-native", async () => {
  const actual = await vi.importActual<typeof import("mo/plugin-sdk/command-auth-native")>(
    "mo/plugin-sdk/command-auth-native",
  );
...
vi.mock("mo/plugin-sdk/agent-runtime", async () => {
  const actual = await vi.importActual<typeof import("mo/plugin-sdk/agent-runtime")>(
    "mo/plugin-sdk/agent-runtime",
  );
  return {
    ...actual,
...
vi.mock("mo/plugin-sdk/plugin-runtime", async () => {
  const actual = await vi.importActual<typeof import("mo/plugin-sdk/plugin-runtime")>(
    "mo/plugin-sdk/plugin-runtime",
  );
  return {
    ...actual,
...
describe("registerTelegramNativeCommands — session metadata", () => {
  ...
  beforeEach(() => {
    ...
    sessionMocks.resolveSessionTranscriptLegacyFileTarget.mockClear().mockResolvedValue({
      ...
      sessionFile: "/tmp/mo-sessions/sess-topic-topic-42.jsonl",
      targetKind: "runtime-session",
    });
    sessionMocks.resolveStorePath.mockClear().mockReturnValue("/tmp/mo-sessions.json");
  ...
  it("uses the target session model when building native argument menus", async () => {
    ...
    expect(sessionMocks.getSessionEntry).toHaveBeenCalledWith({
      storePath: "/tmp/mo-sessions.json",
  ...
  it("passes a persisted topic session file to plugin commands", async () => {
    sessionMocks.resolveStorePath.mockReturnValue("/tmp/mo-sessions/sessions.json");
    ...
    expectRecordFields(
      ...
      ),
      {
        ...
        sessionId: "sess-topic",
        sessionKey: "agent:main:telegram:group:-1001234567890:topic:42",
        storePath: "/tmp/mo-sessions/sessions.json",
        threadId: 42,
      },
  ...
  it("passes a resolved transcript file to plugin commands when the entry has no file", async () => {
    ...
    sessionMocks.resolveSessionTranscriptLegacyFileTarget.mockResolvedValue({
      ...
      sessionFile: "/tmp/mo-sessions/sess-main.jsonl",
      sessionId: "sess-main",
      sessionKey: "agent:main:main",
      targetKind: "runtime-session",
    });
