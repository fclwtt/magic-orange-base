...
import type { ChannelGatewayContext } from "mo/plugin-sdk/channel-contract";
import { createClaimableDedupe } from "mo/plugin-sdk/persistent-dedupe";
import { resetPluginStateStoreForTests } from "mo/plugin-sdk/plugin-state-test-runtime";
...
function makeTempDir(prefix: string): string {
  // mo-temp-dir: allow extension tests cannot import root test helpers
...
function createContext(accountId = "default") {
  ...
  const ctx = {
    ...
    channelRuntime: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/mo-agent.sqlite"),
...
describe("Raft wake gateway", () => {
  ...
  it("persists accepted wake dedupe across restarts without crossing accounts", async () => {
    const stateDir = makeTempDir("mo-raft-wake-dedupe-");
    try {
