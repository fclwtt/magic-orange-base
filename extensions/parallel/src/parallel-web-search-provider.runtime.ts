...
import { readPluginPackageVersion } from "mo/plugin-sdk/extension-shared";
import {
...
const USER_AGENT = `mo-parallel/${PLUGIN_VERSION} (${process.platform})`;
...
export async function executeParallelWebSearchProviderTool(
  ...
  // Generic `query` arg fallback: mo's operator-facing CLI
  // (`mo capability web.search ...`) always passes the shared
  ...
  // Always pass max_results so Parallel matches the mo web_search default
