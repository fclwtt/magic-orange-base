...
import { normalizeOptionalAccountId } from "mo/plugin-sdk/account-id";
import { getChatChannelMeta, type ChannelPlugin } from "mo/plugin-sdk/core";
import {
  formatDocsLink,
...
export const twitchSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId: requestedAccountId, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.MO_TWITCH_ACCESS_TOKEN?.trim();
