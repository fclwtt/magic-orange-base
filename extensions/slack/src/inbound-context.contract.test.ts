...
import { expectChannelInboundContextContract } from "mo/plugin-sdk/channel-contract-testing";
import type { OpenClawConfig } from "mo/plugin-sdk/config-contracts";
import { createTempHomeEnv } from "mo/plugin-sdk/test-env";
...
describe("Slack inbound context contract", () => {
  it("keeps inbound context finalized", async () => {
    const tempHome = await createTempHomeEnv("mo-slack-inbound-contract-");
    try {
