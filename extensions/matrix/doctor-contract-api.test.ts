...
import type { PluginDoctorStateMigrationContext } from "mo/plugin-sdk/runtime-doctor";
...
describe("matrix doctor contract state migrations", () => {
  ...
  it("migrates legacy sync cache JSON to SQLite plugin state", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "mo-matrix-doctor-"));
  ...
  it("migrates Matrix IndexedDB snapshot JSON to SQLite plugin state", async () => {
    ...
    const snapshot = [
      {
        name: "mo-matrix::matrix-sdk-crypto",
        version: 1,
        stores: [
          {
            name: "sessions",
