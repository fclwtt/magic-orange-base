...
function hasTrustedOpenClawRootIndicator(packageRoot, packageJson) {
  ...
  const hasOpenClawBin =
    (typeof packageJson?.bin === "string" &&
      normalizeLowercaseStringOrEmpty(packageJson.bin).includes("mo")) ||
    (typeof packageJson?.bin === "object" &&
      packageJson.bin !== null &&
      typeof packageJson.bin.mo === "string");
  const hasOpenClawEntrypoint = fs.existsSync(path.join(packageRoot, "mo.mjs"));
...
function findOpenClawPackageRoot(startDir) {
  ...
  for (let i = 0; i < 12; i += 1) {
    ...
    if (pkg?.name === "mo" && hasTrustedOpenClawRootIndicator(cursor, pkg)) {
