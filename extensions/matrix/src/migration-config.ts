...
import { DEFAULT_ACCOUNT_ID, normalizeAccountId } from "mo/plugin-sdk/account-id";
  ...
  if (!channel) {
    return {
      warning:
        ...
        'Configure Matrix, then rerun "mo doctor --fix" or restart the gateway.',
    };
  }
  if (requiresExplicitMatrixDefaultAccount(params.cfg)) {
    return {
      warning:
        ...
        'Set "channels.matrix.defaultAccount" to the intended target account before rerunning "mo doctor --fix" or restarting the gateway.',
    };
  }
  ...
  if (!target) {
    ...
    return {
      warning:
        ...
        'Start the gateway once with a working Matrix login, or rerun "mo doctor --fix" after cached credentials are available.',
    };
  }
