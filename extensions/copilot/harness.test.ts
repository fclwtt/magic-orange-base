...
import { attachModelProviderRequestTransport } from "mo/plugin-sdk/agent-harness-runtime";
import {
...
describe("createCopilotAgentHarness", () => {
  ...
  describe("reset", () => {
    ...
    it("does not invoke deleteSession for a session belonging to a different moSessionId", async () => {
      const pool = makePoolMock();
      const deleteSession = vi.fn().mockResolvedValue(undefined);
      const client = { deleteSession } as any;
      mocks.runCopilotAttempt.mockImplementation(async (params, deps) => {
