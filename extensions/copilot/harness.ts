...
import type { PluginStateSyncKeyedStore } from "mo/plugin-sdk/plugin-state-runtime";
...
export function createCopilotAgentHarness(
  ...
  return {
    ...
    async runAttempt(params: AgentHarnessAttemptParams): Promise<AgentHarnessAttemptResult> {
      const attemptPromise = (async () => {
        ...
        const compactionCleanupPending =
          moSessionId !== undefined && hasPendingDeferredCompactionCleanup(moSessionId);
        const replayBlocked =
          moSessionId !== undefined &&
          (compactionCleanupPending || resetBlockedStoredSessions.has(moSessionId));
        const tracked =
          moSessionId && !replayBlocked ? trackedSessions.get(moSessionId) : undefined;
        const stored = moSessionId
          ? replayBlocked
            ? undefined
            : lookupStoredBinding(options?.sessionStore, moSessionId)
          : undefined;
        ...
        return runCopilotAttempt(effectiveParams, {
          pool,
          onSessionEstablished: moSessionId
            ? ({
                ...
                trackedSessions.set(moSessionId, {
                  sdkSessionId,
                ...
                registerStoredBinding(options?.sessionStore, moSessionId, {
                  schemaVersion: 2,
                  sdkSessionId,
                  compatKey: currentCompatKey,
                  compactKey: currentCompactKey,
                ...
                resetBlockedStoredSessions.delete(moSessionId);
              }
            : undefined,
          onDeferredCompaction: moSessionId
            ? ({
                abort,
                cleanup,
                sdkSessionId,
                ...
                const trackedBinding = trackedSessions.get(moSessionId);
                const storedBinding = lookupStoredBinding(options?.sessionStore, moSessionId);
                ...
                resetBlockedStoredSessions.add(moSessionId);
                void cleanup.then((outcome) => {
                  const currentTracked = trackedSessions.get(moSessionId);
                  ...
                  if (stillOwnsTrackedSession) {
                    trackedSessions.delete(moSessionId);
                  }
                  if (stillOwnsStoredSession) {
                    deleteStoredBinding(options?.sessionStore, moSessionId);
                  }
                  if (stillOwnsTrackedSession || stillOwnsStoredSession) {
                    resetBlockedStoredSessions.add(moSessionId);
                  }
    ...
    async reset(params: AgentHarnessResetParams): Promise<void> {
      const moSessionId = typeof params.sessionId === "string" ? params.sessionId : undefined;
      if (!moSessionId) {
        return;
      }
      ...
      const tracked = trackedSessions.get(moSessionId);
      const stored = lookupStoredBinding(options?.sessionStore, moSessionId);
      resetBlockedStoredSessions.add(moSessionId);
      await abortDeferredCompactionCleanups(moSessionId);
      const currentStored = lookupStoredBinding(options?.sessionStore, moSessionId);
      const stillOwnsStoredSession =
        stored !== undefined && currentStored?.sdkSessionId === stored.sdkSessionId;
      if (stillOwnsStoredSession) {
        if (deleteStoredBinding(options?.sessionStore, moSessionId)) {
          resetBlockedStoredSessions.delete(moSessionId);
        }
      } else {
        resetBlockedStoredSessions.delete(moSessionId);
      ...
      if (trackedSessions.get(moSessionId)?.sdkSessionId === tracked.sdkSessionId) {
        trackedSessions.delete(moSessionId);
      }
      try {
    ...
    async compact(
      ...
      if (hasPendingDeferredCompactionCleanup(moSessionId)) {
        return {
          ok: false,
          compacted: false,
