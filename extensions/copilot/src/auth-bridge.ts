 ...
 *     use cases.
 *   - Falls back to MO_GITHUB_TOKEN, COPILOT_GITHUB_TOKEN,
 ...
 *     (`<openClawHome>/.mo/agents/<agentId>/copilot`, or
 *     `<agentDir>/copilot` when an agent directory is supplied) that
 *     respects `MO_HOME` for the home directory root.
 ...
 *   4. MO_GITHUB_TOKEN, then COPILOT_GITHUB_TOKEN, then
...
function resolveCopilotHome(args: {
  ...
  const openClawHome = readString(args.env.MO_HOME);
  const rootHome = openClawHome ? resolve(openClawHome) : safeHomeDir(args.homeDir);
  // Per-agent isolation per proposal section 3.6:
  //   <openClawHome>/.mo/agents/<agentId>/copilot
  return resolve(join(rootHome, ".mo", "agents", args.agentId, "copilot"));
}
...
function readEnvTokenFallback(
  ...
  // MO_GITHUB_TOKEN is the harness-specific override and stays at
  ...
  const candidates: Array<{ name: string; value: string | undefined }> = [
    { name: "MO_GITHUB_TOKEN", value: readString(env.MO_GITHUB_TOKEN) },
