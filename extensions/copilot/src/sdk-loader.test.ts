...
describe("sdk-loader", () => {
  ...
  it("default fallback importer resolves and imports the installed SDK entry", async () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        "module.exports = { moDefaultImporterSentinel: true };",
      );
      ...
      })) as unknown as { moDefaultImporterSentinel?: boolean };

      expect(sdk.moDefaultImporterSentinel).toBe(true);
      expect(primaryImport).toHaveBeenCalledTimes(1);
    } finally {
      rmSync(tmp, { recursive: true, force: true });
    }
  ...
  it("throws an actionable error with plugin install instructions when both probes fail", async () => {
    ...
    await expect(
      ...
      message: expect.stringContaining("mo plugins install @mo/copilot"),
    });
  ...
  it("error message includes the fallback path and underlying primary error", async () => {
    ...
    expect(message).toContain("mo plugins install @mo/copilot");
  });
  ...
  it("resolves the fallback dir from MO_STATE_DIR for relocated profiles", () => {
    expect(
      resolveCopilotSdkFallbackDir({
        ...process.env,
        MO_STATE_DIR: "/tmp/mo-state",
      }),
    ).toBe(path.join("/tmp/mo-state", "npm-runtime", "copilot"));
  });
