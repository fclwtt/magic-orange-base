...
import type { AgentHarnessAttemptParams } from "mo/plugin-sdk/agent-harness-runtime";
...
vi.mock("mo/plugin-sdk/agent-harness", async (importOriginal) => {
  const actual = await importOriginal<typeof import("mo/plugin-sdk/agent-harness")>();
...
const LIVE = isLiveTestEnabled(["MO_COPILOT_AGENT_LIVE_TEST"]);
const TOKEN =
  process.env.MO_COPILOT_AGENT_LIVE_TOKEN ||
...
describeLive("copilot agent runtime live smoke", () => {
  it("runs one turn on gpt-4.1 with one custom tool", async () => {
    ...
    const copilotHome = await mkdtemp(join(tmpdir(), "mo-copilot-live-"));
