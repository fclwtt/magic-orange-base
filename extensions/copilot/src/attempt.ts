...
// mo/plugin-sdk/agent-harness-runtime declares auth, messages,
// onAssistantDelta, and initialReplayState.sdkSessionId fields. Tracked by
// project mo-copilot-harness; reviewer-attempt-bridge note.
...
export interface CopilotAttemptDeps {
   ...
   * `mo/plugin-sdk/agent-harness-runtime#resolveSandboxContext`, which is
   ...
   * harness uses this to track the moSessionId -> sdkSessionId
...
function hasMirrorIdentity(message: AgentMessage): boolean {
  const record = message as unknown as { __mo?: unknown };
  const meta = record["__mo"];
