...
import { readProviderJsonResponse } from "mo/plugin-sdk/provider-http";
import { runPluginCommandWithTimeout } from "mo/plugin-sdk/run-command";
import type { RuntimeEnv } from "mo/plugin-sdk/runtime-env";
import { CONFIG_DIR, extractArchive, resolveBrewExecutable } from "mo/plugin-sdk/setup-tools";
...
export async function installSignalCliFromRelease(
  ...
  const tmpDir = await fs.mkdtemp(path.join(resolvePreferredOpenClawTmpDir(), "mo-signal-"));
