...
import type { MockFn } from "mo/plugin-sdk/plugin-test-runtime";
...
const signalToolResultSessionStorePath = vi.hoisted(
  () => `/tmp/mo-signal-tool-result-sessions-${process.pid}.json`,
);
...
vi.mock("mo/plugin-sdk/runtime-config-snapshot", async () => {
  const actual = await vi.importActual<
    typeof import("mo/plugin-sdk/runtime-config-snapshot")
  >("mo/plugin-sdk/runtime-config-snapshot");
  return {
    ...actual,
...
vi.mock("mo/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("mo/plugin-sdk/reply-runtime")>(
    "mo/plugin-sdk/reply-runtime",
  );
  return {
    ...actual,
...
vi.mock("mo/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("mo/plugin-sdk/security-runtime")>(
    "mo/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
vi.mock("mo/plugin-sdk/system-event-runtime", async () => {
  const actual = await vi.importActual<typeof import("mo/plugin-sdk/system-event-runtime")>(
    "mo/plugin-sdk/system-event-runtime",
  );
  return {
    ...actual,
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("mo/plugin-sdk/reply-runtime"),
      import("mo/plugin-sdk/system-event-runtime"),
    ]);
    resetInboundDedupe();
    config = {
