...
import { CUSTOM_LOCAL_AUTH_MARKER } from "mo/plugin-sdk/provider-auth";
import type { OpenClawConfig } from "mo/plugin-sdk/provider-auth";
import type { ModelDefinitionConfig } from "mo/plugin-sdk/provider-model-shared";
import { resolveAgentModelPrimaryValue } from "mo/plugin-sdk/provider-onboard";
import {
  SELF_HOSTED_DEFAULT_CONTEXT_WINDOW,
  type ProviderAuthMethodNonInteractiveContext,
  type ProviderCatalogContext,
} from "mo/plugin-sdk/provider-setup";
import type { WizardPrompter } from "mo/plugin-sdk/setup";
...
vi.mock("mo/plugin-sdk/provider-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("mo/plugin-sdk/provider-auth")>();
  return {
    ...actual,
...
vi.mock("mo/plugin-sdk/provider-setup", async (importOriginal) => {
  const actual = await importOriginal<typeof import("mo/plugin-sdk/provider-setup")>();
  return {
    ...actual,
...
afterAll(() => {
  ...
  vi.doUnmock("mo/plugin-sdk/provider-auth");
  vi.doUnmock("mo/plugin-sdk/provider-setup");
  vi.resetModules();
});
...
describe("lmstudio setup", () => {
  ...
  it("non-interactive setup fails when requested model is missing", async () => {
    ...
    const dockerSetup = ["1", "true", "yes", "on"].includes(
      process.env.MO_DOCKER_SETUP?.trim().toLowerCase() ?? "",
    );
