...
const LIVE = process.env.MO_LIVE_TEST === "1" && process.env.MO_LIVE_OLLAMA === "1";
const OLLAMA_BASE_URL =
  process.env.MO_LIVE_OLLAMA_BASE_URL?.trim() || "http://127.0.0.1:11434";
const CHAT_MODEL = process.env.MO_LIVE_OLLAMA_MODEL?.trim() || "llama3.2:latest";
const EMBEDDING_MODEL =
  process.env.MO_LIVE_OLLAMA_EMBED_MODEL?.trim() || "embeddinggemma:latest";
const PROVIDER_ID = process.env.MO_LIVE_OLLAMA_PROVIDER_ID?.trim() || "ollama-live-custom";
const RUN_WEB_SEARCH = process.env.MO_LIVE_OLLAMA_WEB_SEARCH !== "0";
const RUN_EMBEDDINGS =
  process.env.MO_LIVE_OLLAMA_EMBEDDINGS === "1" ||
  (process.env.MO_LIVE_OLLAMA_EMBEDDINGS !== "0" && !isOllamaCloudBaseUrl(OLLAMA_BASE_URL));
...
function requireOllamaRuntimeApiKey(): string | undefined {
  ...
  if (!apiKey) {
    throw new Error(
      "MO_LIVE_OLLAMA_BASE_URL points at a remote Ollama host; set OLLAMA_API_KEY.",
    );
  }
  return apiKey;
}
...
async function withTempOpenClawState<T>(run: (paths: { root: string }) => Promise<T>): Promise<T> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "mo-ollama-cli-live-"));
  try {
    await fs.writeFile(
      path.join(root, "mo.json"),
      JSON.stringify(
        {
          models: {
            providers: {
...
async function runOpenClawCli(args: string[], env: NodeJS.ProcessEnv) {
  ...
  const commandArgs = sourceRunnerAvailable
    ...
    : ["mo.mjs", ...args];
  const outputRoot = fsSync.mkdtempSync(path.join(os.tmpdir(), "mo-ollama-cli-output-"));
...
function buildCliEnv(root: string): NodeJS.ProcessEnv {
  ...
  return {
    ...
    MO_LIVE_TEST: "1",
    MO_LIVE_OLLAMA: "1",
    MO_LIVE_OLLAMA_WEB_SEARCH: "0",
    MO_STATE_DIR: path.join(root, "state"),
    MO_CONFIG_PATH: path.join(root, "mo.json"),
    MO_NO_RESPAWN: "1",
    MO_TEST_FAST: "1",
