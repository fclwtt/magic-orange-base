...
import { FsSafeError, pathExists, root as fsRoot } from "mo/plugin-sdk/security-runtime";
...
- Use \`.mo-wiki/cache/agent-digest.json\` and \`claims.jsonl\` for machine reads; markdown pages are the human view.
`);
}
...
- \`.mo-wiki/cache/agent-digest.json\` is the agent-facing compiled digest.
...
export async function initializeMemoryWikiVault(
  ...
  await writeFileIfMissing(rootDir, ".mo-wiki/log.jsonl", "", createdFiles);
