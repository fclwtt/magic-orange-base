...
import { runTasksWithConcurrency } from "mo/plugin-sdk/concurrency-runtime";
import { retryTransientMemoryRead } from "mo/plugin-sdk/memory-core-host-engine-storage";
import {
  replaceManagedMarkdownBlock,
  withTrailingNewline,
} from "mo/plugin-sdk/memory-host-markdown";
import { root as fsRoot } from "mo/plugin-sdk/security-runtime";
import {
...
const AGENT_DIGEST_PATH = ".mo-wiki/cache/agent-digest.json";
const CLAIMS_DIGEST_PATH = ".mo-wiki/cache/claims.jsonl";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- mo:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- mo:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
...
export async function compileMemoryWikiVault(
  ...
  if (
    await writeManagedMarkdownFile({
      ...
      title: "Wiki Index",
      startMarker: "<!-- mo:wiki:index:start -->",
      endMarker: "<!-- mo:wiki:index:end -->",
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        ...
        relativePath,
        title: group.heading,
        startMarker: `<!-- mo:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- mo:wiki:${group.dir}:index:end -->`,
