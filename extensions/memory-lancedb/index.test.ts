...
} from "mo/plugin-sdk/memory-host-core";
...
async function withMockedOpenAiMemoryPlugin<T>(params: {
  ...
  vi.resetModules();
  vi.doMock("mo/plugin-sdk/runtime-env", () => ({
  ...
  try {
    ...
    vi.doUnmock("mo/plugin-sdk/runtime-env");
    vi.doUnmock("openai");
...
describe("memory plugin e2e", () => {
  const { getDbPath, getTmpDir } = installTmpDirHarness({ prefix: "mo-memory-test-" });
  ...
  test("uses provider adapter auth when embedding apiKey is omitted", async () => {
    ...
    vi.resetModules();
    vi.doMock("mo/plugin-sdk/memory-core-host-engine-embeddings", () => ({
    ...
    try {
      ...
      expect(providerOptions.agentDir).toBe("/tmp/mo-agent");
      ...
      vi.doUnmock("mo/plugin-sdk/memory-core-host-engine-embeddings");
      vi.doUnmock("openai");
      vi.doUnmock("./lancedb-runtime.js");
      vi.resetModules();
    }
    ...
    await withMockedOpenAiMemoryPlugin({
      ...
      run: async (dynamicMemoryPlugin) => {
        ...
        try {
          ...
          await program.parseAsync(["node", "mo", "ltm", "list", "--limit", "+03"]);
  ...
  test("escapeMemoryForPrompt strips media attached annotations before escaping", () => {
    expect(
      escapeMemoryForPrompt(
        "User sent image [media attached: /Users/alex/.mo/media/photo.jpg (image/jpeg)] and said hello",
      ),
