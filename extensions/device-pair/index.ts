...
function resolveAuthLabel(cfg: OpenClawPluginApi["config"]): ResolveAuthLabelResult {
  ...
  const token =
    pickFirstDefined([process.env.MO_GATEWAY_TOKEN, cfg.gateway?.auth?.token]) ?? undefined;
  const password =
    pickFirstDefined([process.env.MO_GATEWAY_PASSWORD, cfg.gateway?.auth?.password]) ??
    undefined;
