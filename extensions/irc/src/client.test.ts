...
describe("irc client fallback nick", () => {
  ...
  it("falls back to mo when nick consists entirely of special characters", () => {
    const nick = buildFallbackNick("!!!");
    expect(nick).toMatch(/^mo_\d*$/);
  });

  it("falls back to mo when nick is empty after sanitization", () => {
    const nick = buildFallbackNick("");
    expect(nick).toMatch(/^mo_\d*$/);
  });
