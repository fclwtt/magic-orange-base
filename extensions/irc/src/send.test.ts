...
import { verifyChannelMessageAdapterCapabilityProofs } from "mo/plugin-sdk/channel-outbound";
import { createSendCfgThreadingRuntime } from "mo/plugin-sdk/channel-test-helpers";
...
vi.mock("mo/plugin-sdk/plugin-config-runtime", async () => {
  const original = (await vi.importActual("mo/plugin-sdk/plugin-config-runtime")) as Record<
    string,
    unknown
  >;
  return {
...
vi.mock("mo/plugin-sdk/text-chunking", async () => {
  const original = (await vi.importActual("mo/plugin-sdk/text-chunking")) as Record<
    string,
    unknown
  >;
  return {
...
afterAll(() => {
  ...
  vi.doUnmock("mo/plugin-sdk/plugin-config-runtime");
  vi.doUnmock("mo/plugin-sdk/text-chunking");
  vi.resetModules();
});
