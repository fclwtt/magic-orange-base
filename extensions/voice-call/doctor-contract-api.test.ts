...
describe("voice-call doctor state migration", () => {
  ...
  beforeEach(async () => {
    resetPluginStateStoreForTests();
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mo-voice-call-doctor-"));
    storePath = createTestStorePath();
    env = { ...process.env, HOME: stateDir, MO_STATE_DIR: stateDir };
    installStateRuntime();
  });
  ...
  it("reports top-level and per-number session-store agents", () => {
    ...
    expect(
      resolveSessionStoreAgentIds({
        cfg: {
          plugins: { entries: { "@mo/voice-call": { config: {} } } },
        },
      }),
    ).toEqual(["main"]);
    expect(
