...
describe("voice-call plugin", () => {
  ...
  afterEach(() => {
    ...
    vi.restoreAllMocks();
    vi.unstubAllEnvs();
    delete (globalThis as Record<PropertyKey, unknown>)[Symbol.for("mo.voice-call.runtime")];
    delete (globalThis as Record<PropertyKey, unknown>)[
      Symbol.for("mo.voice-call.runtimePromise")
    ];
    delete (globalThis as Record<PropertyKey, unknown>)[
      Symbol.for("mo.voice-call.runtimeStopPromise")
    ];
  });
  ...
  it("still starts the webhook runtime for gateway CLI processes", async () => {
    ...
    vi.stubEnv("MO_CLI", "1");
    process.argv = ["node", "mo", "gateway", "run"];
    const { service } = setup({ provider: "mock" });
  ...
  it("normalizes legacy config through runtime creation and warns to run doctor", async () => {
    ...
    expectWarningIncludes('Run "mo doctor --fix"');
  });
