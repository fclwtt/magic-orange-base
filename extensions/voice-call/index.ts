...
import { ErrorCodes, errorShape } from "mo/plugin-sdk/gateway-runtime";
import { timestampMsToIsoString } from "mo/plugin-sdk/number-runtime";
...
const voiceCallConfigSchema = {
  ...
  uiHints: {
    ...
    "realtime.toolPolicy": {
      ...
      help: "Controls the shared mo_agent_consult tool.",
      advanced: true,
    },
    "realtime.consultPolicy": {
      label: "Realtime Consult Policy",
      help: "Guides when the realtime voice model should call mo_agent_consult.",
      advanced: true,
    },
...
function isCliOnlyProcess(): boolean {
  return process.env.MO_CLI === "1" && !process.argv.slice(2).includes("gateway");
}
...
const VOICE_CALL_RUNTIME_KEY = Symbol.for("mo.voice-call.runtime");
const VOICE_CALL_RUNTIME_PROMISE_KEY = Symbol.for("mo.voice-call.runtimePromise");
const VOICE_CALL_RUNTIME_STOP_PROMISE_KEY = Symbol.for("mo.voice-call.runtimeStopPromise");
