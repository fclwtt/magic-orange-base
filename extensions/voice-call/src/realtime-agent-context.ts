...
import { buildRealtimeVoiceAgentConsultPolicyInstructions } from "mo/plugin-sdk/realtime-voice";
import { root } from "mo/plugin-sdk/security-runtime";
import { normalizeOptionalString as normalizeString } from "mo/plugin-sdk/string-coerce-runtime";
...
export async function buildRealtimeVoiceInstructions(params: {
  ...
  const capsule: string[] = [
    ...
    "- Treat this as compact context only; call mo_agent_consult when the caller needs the full agent brain, tools, memory, or workspace state.",
  ];
