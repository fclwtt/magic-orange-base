...
import type { OpenClawConfig } from "mo/plugin-sdk/plugin-entry";
...
function getVoiceCallConfigStore(config: PluginDoctorStateMigrationParams["config"]): string {
  for (const pluginId of ["voice-call", "@mo/voice-call"]) {
...
function resolveVoiceCallStorePath(params: {
  ...
  return path.join(resolveHome(params.env), ".mo", "voice-calls");
}
...
export const stateMigrations: PluginDoctorStateMigration[] = [
  {
    ...
    async migrateLegacyState(params) {
      ...
      const env = { ...params.env, MO_STATE_DIR: storePath };
